...
/**
 * $Id: CurvaTermica.inc.php,v 1.2 2005/06/07 18:28:17 leydi Exp $
...
function GraficarCurvaTermica($Vmax,$Vmin,$DatosF,$todos,$fechaE,$DatosH)
{
	...
	$seq=$result->fields[0];
	$Dir="cache/CurvaTermica$seq.png";

	if(!$todos){
		foreach($DatosH as $key =>$value){
			$ydata[count($ydata)]=$value['temp_piel'];
...
//			$VmaxY=$Vmax;
		...
		//echo "<br><br>v_max->".$v_max."-- VmaxY->".$VmaxY."-- VminY->".$VminY."-- VarLine->".$VarLine;
		$i=0;
