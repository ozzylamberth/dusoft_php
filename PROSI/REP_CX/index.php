...
procesar_entrada("GET", "pagina", "input9", "input10", "order", "orientation", "color");
$pagina = $_GET["pagina"];

$input9 = $_GET["input9"].' 00:00:00';
$input10 = $_GET["input10"].' 23:59:59';

$t_tercero = $_GET["t_tercero"];
$t_cirujano = $_GET["t_cirujano"];
$t_anestesiologo = $_GET["t_anestesiologo"];
$t_especialidades = $_GET["t_especialidades"];
$t_realizadas = $_GET["t_realizadas"];
$t_canceladas = $_GET["t_canceladas"];
$t_anestesia = $_GET["t_anestesia"];
$t_ambito = $_GET["t_ambito"];
$t_urg_realizadas = $_GET["t_urg_realizadas"];
$t_procedimientos = $_GET["t_procedimientos"];
  ...
  <table width="90%" border="0" cellspacing="0" align="center">
    <tr> 
      ...
      <a href="javascript:cal9.popup();"><img src="img/cal.gif" width="16" height="16" border="0" alt="Click Here to Pick up the date"></a>
				<input type="Text" name="input9" value="<?php echo $_GET_REQUEST['input9'];?>">
				</td>
				<td bgcolor="#EEEEEE">
				Fecha Hasta:<br>
				<input type="Text" name="input10" value="<?php echo $_GET_REQUEST['input10'];?>">
...
if (isset ($t_canceladas)){
 ...
 $result_canceladas=execute_query($dbh, "SELECT distinct(qx_programaciones_canceladas.qx_motivo_cancelacion_programacion_id), qx_motivos_cancelacion_programaciones.descripcion, count(*)
 FROM   ((public.qx_programaciones_canceladas qx_programaciones_canceladas INNER JOIN public.qx_programaciones qx_programaciones ON qx_programaciones_canceladas.programacion_id=qx_programaciones.programacion_id) INNER JOIN public.qx_motivos_cancelacion_programaciones qx_motivos_cancelacion_programaciones ON qx_programaciones_canceladas.qx_motivo_cancelacion_programacion_id=qx_motivos_cancelacion_programaciones.qx_motivo_cancelacion_programacion_id) INNER JOIN public.pacientes pacientes ON (qx_programaciones.tipo_id_paciente=pacientes.tipo_id_paciente) AND (qx_programaciones.paciente_id=pacientes.paciente_id)
 WHERE  (qx_programaciones.fecha_registro>='$input9' AND qx_programaciones.fecha_registro<='$input10')
group by 1, 2
ORDER BY 1"); 
...
<table width="90%" border="0" cellspacing="0" align="center">
<TR>
<td background="imagenes/cellpic1.gif" align="left"><input type="button" name="Submit3" value="Imprimir Listado" onClick="javascript: window.open('reportes_cx.php?input9=<?=$input9?>&amp;input10=<?=$input10?>&amp;rep_tercero=<?=$rep_tercero?>&amp;rep_cirujano=<?=$rep_cirujano?>&amp;rep_especialidades=<?=$rep_especialidades?>&amp;rep_anestesiologo=<?=$rep_anestesiologo?>&amp;rep_realizadas=<?=$rep_realizadas?>&amp;rep_canceladas=<?=$rep_canceladas?>&amp;rep_anestesia=<?=$rep_anestesia?>&amp;rep_ambito=<?=$rep_ambito?>&amp;rep_urg_realizadas=<?=$rep_urg_realizadas?>&amp;rep_procedimientos=<?=$rep_procedimientos?>', 'imprimirlistado');" ></td>
</TR>
</TABLE>
<?php
}
