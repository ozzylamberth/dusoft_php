...
class informe_factura extends FPDF {
    var $db; // Base de datos
    var $idnota; // Identificador del albaran
    var $idfactura; // Identificador de la factura
    var $sumatotal = 0; // Suma de todos los importes
    var $iva = 0;
    var $dto = 0;
    var $inc = 0;
    ...
    function Header() // Cabecera del albaran
    {
        ...
        $this->Cell(0, 6, "Factura Nº:" . $this->numfactura . $this->serie, 0, 0, 'R');
        $this->Ln();
        $this->SetFont('Times', '', 10);
        $this->Cell(0, 6, "Fecha:" . $this->fechafactura, 0, 0, 'R');
        $this->Ln();
        $this->SetFont('Times', '', 10);
        $this->Cell(0, 6, "Hora:" . $this->horafactura, 0, 0, 'R');
        $this->Ln(20);
    ...
    function CargarFactura()
    {
        ...
        $sql = "SELECT A.*, B.IDNOTA, C.NOMBRE FROM FACTURAS A INNER JOIN NOTAS_ENTREGA B ON A.IDFACTURA = B.IDFACTURA
        INNER JOIN USUARIOS C ON A.IDUSUARIO = C.IDUSUARIO WHERE A.IDFACTURA = " . $this->idfactura;
        $result = execute_query($this->db, $sql);
        $row = fetch_object($result);
        $this->numfactura = $row->NUMFACTURA;
        $this->fechafactura = $row->FECHA;
        $this->horafactura = $row->HORA;
        $this->idnota = $row->IDNOTA;
        $this->usuario = $row->NOMBRE;
        free_result($result);
        $this->CargarNotaEntrega();
    } 
    ...
    { 
        ...
        $this->SetTextColor(0, 0, 0);
        $this->Cell(0, 0, "Entrega: " . $this->numnota . $this->serie . " del " . $this->fecha, 0, 0, "R");
        $this->Ln(4);
        $this->SetFont('Times', '', 10);
        $i = 1;
