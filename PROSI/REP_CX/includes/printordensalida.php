...
class informe_salida extends FPDF {
    var $db; // Base de datos
    var $iddespacho; // Identificador del albaran
    var $idsalida;
    ...
    function Header() // Cabecera del albaran
    {
        ...
        $this->Cell(0, 6, "Ord Salida Nº:" . $this->numordensa . $this->serie, 0, 0, 'R');
        $this->Ln();
        $this->SetFont('Times', '', 10);
        $this->Cell(0, 6, "Fecha:" . $this->fechaordensa, 0, 0, 'R');
        $this->Ln(20);
        ...
        $this->MultiCell(0, 6, "Cod.Mens: " . $this->idmensajero . "\n" . $this->mensajero, 1, 'L');
        $this->Ln(18);
...
function CalculaTotales()
    ...
    function CargarSalida()
    {
        ...
        $sql = "SELECT A.*, B.IDDESPACHO FROM ORDEN_SALIDA A INNER JOIN DESPACHOS B ON A.IDORDENSA = B.IDORDENSALIDA
        WHERE A.IDORDENSA = " . $this->idsalida;
        $result = execute_query($this->db, $sql);
        $row = fetch_object($result);
        $this->numordensa = $row->NUMORDENSA;
        $this->fechaordensa = $row->ORDENSAFECHA;
        $this->iddespacho = $row->IDDESPACHO;
        free_result($result);
        $this->CargarDespacho();
    } 
