...
/**
 * $Id: hojacargos5.inc.php,v 1.11 2007/02/27 21:12:44 lorena Exp $
 ...
 */
  ...
  function CargosFacturaHoja5($cuenta,$paquete)
  {
        if($paquete==1){        
          $filPaquete=" AND a.paquete_codigo_id IS NOT NULL";  
          $filFacturado="a.sw_paquete_facturado";
        }else{
          $filPaquete=" AND a.paquete_codigo_id IS NULL";  
          $filFacturado="a.facturado";
        }
        ...
        (CASE WHEN $filFacturado='1' THEN a.precio ELSE '0' END) as precio,
        sum(a.cantidad) as cantidad,'' as unidad,
        (CASE WHEN $filFacturado='1' THEN sum(a.valor_cargo) ELSE '0' END) as valor_cargo, 
        (CASE WHEN $filFacturado='1' THEN sum(a.valor_nocubierto) ELSE '0' END) as valor_paciente,
        (CASE WHEN $filFacturado='1' THEN sum(a.valor_cubierto) ELSE '0' END) as valor_cliente
        ...
        $filFacturado,a.precio,unidad) as a
        WHERE a.cantidad > 0
        )
        UNION
        (SELECT a.*
        ...
        $filFacturado,a.precio,unidad,a.cargo) as a
  ...
  function CargosQXFacturaHoja5($numerodecuenta,$paquete){
    ...
    ter.nombre_tercero,d.transaccion
    ...
    if($result->RecordCount()>0){
      while ($data = $result->FetchRow()){
        ...
        [$data['nom_procedimiento']][$data['via']][$data['transaccion']][$data['cargo_cups']]=$data;
      }        
    }
    return $datos;
  ...
  function GenerarHojaCargos5($datos)
  {
      ...
      $Dir="cache/hojacargos5".$datos[numerodecuenta].".pdf";
              ...
              foreach($vector3 as $cargo_proc => $vector4){
                foreach($vector4 as $nombre_proc => $vector5){
                  foreach($vector5 as $via_proc => $vector6){                    
                    ...
                    foreach($vector6 as $transaccion => $vector7){
                      foreach($vector7 as $cargo => $vector8){                      
                        $html.="<tr>";                  
                        $html.="<td width=70><B>".$cargo."</B></td>";
                        $html.="<td width=390><B>".substr($vector8['cargo_nom'],0,60)."</B></td>";
                        $var=explode('.',$vector8['cantidad']);                
                        if($var[1]==0)$vector8['cantidad']=$var[0];
                        $html.="<td width=50 align=\"RIGHT\"><B>".$vector8['cantidad']."</B></td>";
                        $html.="<td width=30 align=\"RIGHT\"><B></B></td>";                
                        $html.="<td width=50 align=\"RIGHT\"><B>".FormatoValor($vector8['precio'])."</B></td>";
                        $html.="<td width=80 align=\"RIGHT\"><B>".FormatoValor($vector8['valor_paciente'])."</B></td>";
                        $html.="<td width=80 align=\"RIGHT\"><B>".FormatoValor($vector8['valor_cliente'])."</B></td>";                
                        $html.="</tr>";
                        if(!empty($vector8['nombre_tercero'])){
                          $html.="<tr><td width=80></b>PROFESIONAL:</td><td width=400></b>".$vector8['nombre_tercero']."</td></tr>";
                        }
                        $SumValorPaciente+=$vector8['valor_paciente'];
                        $SumValorCliente+=$vector8['valor_cliente'];
                      }  
                    }
      ...
      $vector=CargosFacturaHoja5($datos[numerodecuenta],1); 
      $vectorUno=CargosQXFacturaHoja5($datos[numerodecuenta],1);            
      if(!empty($vector) || !empty($vectorUno)){
      ...
      foreach($vectorUno as $liquidacionid => $vector0){
        $SumValorPaciente=0;
        $SumValorCliente=0;
        foreach($vector0 as $agrupamiento => $vector1){
          $html.="<tr><td width=60></td><td width=700><B>".$agrupamiento."</B></td></tr>";      
