...
<title>RELACION DE HISTORIAS CLINICAS</title>
...
procesar_entrada("GET", "id", "action", "action1", "relacion_hcid", "departamento_solicita", "departamento_recibe", "ingreso","paciente_id","tipo_id_paciente","proceso", "cod_menu");
...
$sess = session_id();
		...
		function relacion_despachada($relacion_hcid, $dbh){
      ...
      <td  background=imagenes/cellpic1.gif colspan=8 bgcolor=#EEEEEE><div align=left><strong><font color=#003366 size=1>RELACION: ".$row_c[0]." FECHA: ".$row_c[1]." USUARIO: ".$row_c[2]."</font></strong></div></td>
    </tr>";
   ...
   <td width=10% background=imagenes/cellpic1.gif>
    ...
    <td>
    <form method=\"POST\" action=\"relaciones_hc.php?action1=confirmar&amp;relacion_hcid=$relacion_hcid&amp;proceso=$proceso&amp;cod_menu=$cod_menu\">
	<input type='checkbox' name='hc_recibidas[]' value='$row_d->rel_hcdet_id'>
		...
		$result_ingreso = execute_query($dbh, $busca_ingreso_paciente);
		   ...
		   if($num_records>0){
			...
			while ($row_ingreso = fetch_object($result_ingreso)) {
			 ...
			 $ingreso_encontrado .= "
			 <form method=\"POST\" action=\"relaciones_hc.php?action1=add&amp;departamento_solicita=$departamento_solicita&amp;departamento_recibe=$departamento_recibe&amp;relacion_hcid=$relacion_hcid&amp;proceso=$proceso&amp;cod_menu=$cod_menu\">
			  <tr bgcolor='#EEEEEE'> 
			    ...
			    <td><input name='cant_dias[]' type='text' id='cant_dias' value='1' size='5' class='textbox'></td>
			    
			  <td>
				<input type='checkbox' name='insertarlinea[]' value=$row_ingreso->tipo_id_paciente"."AFOZ"."$row_ingreso->paciente_id"."AFOZ"."$row_ingreso->ingreso"."AFOZ"."$j>
			  </td>  
			 ...
			 $ingreso_encontrado .= "<input type=\"hidden\" name=\"departamento_solicita\" value=\"$departamento_solicita\">";
		$ingreso_encontrado .= "<input type=\"hidden\" name=\"departamento_recibe\" value=\"$departamento_recibe\">";
		$ingreso_encontrado .= "<input type=\"hidden\" name=\"relacion_hcid\" value=\"$relacion_hcid\">";
...
while ($p_insert = fetch_object($pre_insert)) {
 	...
 	if($p_insert->ingreso == Null){
		$query = "Select rel_hcdet_id, relacion_hcid, estado, relacion_entrega_id 
    ...
    $estado = $comprobar[2];
    $relacion_entrega_encontrada = $comprobar[3];
    
    free_result($result);
    
    	if(isset($relacion_entrega_encontrada)){
    	 
    	 	$query1 = "SELECT departamento_entrega, departamento_recibe, estado FROM relacion_entrega_hc 
			 WHERE relacion_entrega_id = $relacion_entrega_encontrada";
		    $result1 = execute_query($dbh, $query1);
		    
		    $comprobar1 = pg_fetch_row($result1);
		    
		    $departamento_e = $comprobar1[0];
		    $departamento_r = $comprobar1[1];
		    $estado_entrega = $comprobar1[2];
		    ...
		    free_result($result1);
		    	...
		    	if($estado_entrega == '1'){
		    	 	...
		    	 	$consulta1 = "SELECT descripcion FROM departamentos 
						 		WHERE departamento = '$departamento_r'";
					...
					$message .= 
				 	...
				 	<font size=2 color=RED face=Verdana, Arial, Helvetica, sans-serif><strong>La HC ".$p_insert->tipo_id_paciente." ".$p_insert->paciente_id." esta siendo devuelta al departamento de ".$departamento."</strong></font></td></tr>";
    			...
    			else if($estado == '6' AND $departamento_r <> $departamento_recibe){
    			 	$consulta1 = "SELECT descripcion FROM departamentos 
						 		WHERE departamento = '$departamento_r'";
		    		$resultado1 = execute_query($dbh, $consulta1);
		       		$dpto1 = pg_fetch_row($resultado1);
    			 	...
    			 	$message .=
					"<tr><td bgcolor=#EEEEEE>
				 	<font size=2 color=RED face=Verdana, Arial, Helvetica, sans-serif><strong>La HC ".$p_insert->tipo_id_paciente." ".$p_insert->paciente_id." esta en poder del departamento de ".$departamento."</strong></font></td></tr>";
					$numero_control = $numero_control + 1;	
				}
				else{
...
<form name="tstest">
<form name="buscaringresos" action="<?=$_SERVER['PHP_SELF']?>" method="POST" onSubmit="return Verificar()">
<input name="relacion_hcid" type="hidden" id="relacion_hcid" value="<?php echo $relacion_hcid;?>">
...
<td>
  <table align="CENTER%" width="100%" border="0" cellspacing="0">
    <tr> 
      <td  background="imagenes/cellpic1.gif" colspan="4" bgcolor="#EEEEEE"><div align="center"><strong><font color="#003366" size="2">RELACION DE SOLICITUD DE HISTORIAS CLINICAS</font></strong></div></td>
    </tr>
    <tr> 
...
$result_dpto_entrega = execute_query($dbh, "SELECT departamentos.departamento, departamentos.descripcion, system_usuarios.usuario_id
 ...
 WHERE  system_usuarios.usuario_id=$usuario_id AND departamentos.departamento IN (SELECT departamento FROM departamentos_relacion_cuentas WHERE sw_relacion_hc = '1') ORDER BY 2");
...
$result_dpto_recibe = execute_query($dbh, "SELECT * FROM departamentos WHERE departamentos.departamento IN (SELECT departamento FROM departamentos_relacion_cuentas WHERE sw_relacion_hc = '1') ORDER BY 5");
...
<?php echo "<form method=\"GET\" action=\"relaciones_hc.php\">";?>    
    <?php if($departamento_solicita <> '' AND $departamento_recibe <> ''){
...
?>
    ...
    <?php echo 	"<input type=\"hidden\" name=\"departamento_solicita\" value=\"$departamento_solicita\">";
			  echo	"<input type=\"hidden\" name=\"departamento_recibe\" value=\"$departamento_recibe\">";
			  echo	"<input type=\"hidden\" name=\"relacion_hcid\" value=\"$relacion_hcid\">";
