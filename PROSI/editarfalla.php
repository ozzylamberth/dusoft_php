...
<script language="Javascript">
  ...
  function sw_Cantidad(){
  var sw_cantidad = document.getElementById("sw_cantidad");
  var cantidad = document.getElementById("cantidad");
  
    if (sw_cantidad.selectedIndex == 0){ // Estado Pendiente
    selobservacionllamada.disabled = true;
  } else {
  	selobservacionllamada.disabled = false;
  }
}
...
$administrador = $_SESSION["administrador"];
			...
			function nombre_usuario($usuario_id, $dbh){
				$result_user = execute_query($dbh, "SELECT a.nombre
													FROM system_usuarios a
													WHERE a.usuario_id = $usuario_id");
				$row_user = fetch_object($result_user);
...
procesar_entrada("GET", "id", "action", "registro_id", "tipo_falla_id", "descripcion", "fecha_registro", "fecha_ocurrio", "departamento",
	"solucion", "estado", "fecha_solucion", "informante", "usuario_registro", "user_solucion", "cod_menu", "cantidad");
...
switch ($action) {
	case "1": // Insertar
		...
		if ($administrador == "1" AND $estado == "2"){
			$usuario_solucion = $usuario_id;
	...
	case "2": // Modificar
		...
		if ($administrador == "1" AND $estado == "2"){
		 	...
		 	if($user_solucion == $usuario_solucion OR $user_solucion == ""){
	...
	default:
		if ($id != "") {
			$result = execute_query($dbh, "SELECT a.registro_id, b.tipo_falla_id, a.descripcion as problema, c.usuario_id, a.fecha_registro, a.fecha_ocurrio, d.departamento, a.solucion, e.estado, e.descripcion as descripcion_estado, a.fecha_solucion, a.informante, a.usuario_solucion, a.cantidad FROM registros_fallas_siis a LEFT JOIN tipos_fallas_sistema b ON a.tipo_falla_id = b.tipo_falla_id INNER JOIN system_usuarios c ON a.usuario_id = c.usuario_id INNER JOIN departamentos d ON a.departamento = d.departamento INNER JOIN fallas_estado e ON a.estado = e.estado WHERE registro_id = " . $id . "");
			...
			if ($row) {
				...
				$usuario_registro = $row->usuario_id;
				...
				$estado = $row->estado;
				$user_solucion = $row->usuario_solucion;
...
<input type=hidden name='user_solucion' value='$user_solucion'>";
...
?>
        ...
        <td><input name="informante" type="text" class="textbox" id="informante" value="<?=$informante?>" size="30" maxlength="50" <?php if($administrador == 1){}else{echo "readonly class='textbox'";}?>></td>
        <td>&nbsp;</td>
      </tr>
      <tr bgcolor="#CCCCCC"> 
        <td><div align="right"><strong><font size="1" face="Verdana, Arial, Helvetica, sans-serif">Fecha 
	  ...
	  <a href="javascript:cal10.popup();"><img src="img/cal.gif" width="16" height="16" border="0" alt="Click Here to Pick up the date"></a></td>
        ...
        <tr bgcolor="#CCCCCC">
        <td width="8%"><div align="right"><strong><font size="1" face="Verdana, Arial, Helvetica, sans-serif">Cantidad</font></strong></div></td>
    <td>
    <input name="cantidad" type="text" class="textbox" id="cantidad" value="<?=$cantidad?>" size="5" maxlength="10"></td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
      </tr>
