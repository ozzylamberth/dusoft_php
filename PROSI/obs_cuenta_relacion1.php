...
$rel_det_id = get_value($_GET["rel_det_id"], "C");
...
switch ($action) {
  ...
  case "grabar":
  	...
  	$sql = "UPDATE tmp_relacion_cuentas_detalle SET "
		  . " observacion = '" . $observacion
		 . "' WHERE rel_det_id = " . $rel_det_id . "";
		 
		 
		if (execute_query($dbh, $sql)){
			$next_action = "2";
			$self = $self . "?rel_det_id=" . $rel_det_id;
...
$consulta2 = "SELECT a.rel_det_id, a.observacion
 				FROM   tmp_relacion_cuentas_detalle a
 				WHERE  a.rel_det_id = $rel_det_id";
...
<?php
echo"
<input type=hidden name='rel_det_id' value='$rel_det_id'>";
?>
