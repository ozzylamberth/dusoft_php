...
/**
 * $Id: despacho_medicamentos.inc.php,v 1.20 2006/01/12 22:22:56 lorena Exp $
      ...
      $tipoCargo='IMD';
        ...
        $datos=$result->RecordCount();
            ...
            $vars[$i]=$result->GetRowAssoc($toUpper=false);						
						$varsCuenDet=LiquidarIyM($cuenta,$codigo,$vars[$i]['cantidad'],$descuento_manual_empresa=0,$descuento_manual_paciente=0,$aplicar_descuento_empresa=false,$aplicar_descuento_paciente=false,NULL,$planId,$autorizar=false,$departamento,$Empresa,$evolucion_id,&$dbconn);						
...
*/
	...
	function LiquidarIYMOrdenServicio($cuenta,&$dbconn){
      ...
      $Empresa=$vars[$i]['empresa_id'];
        ...
        $this->FrmError["MensajeError"]="Error en la Creacion de Los Documentos de Bodega, Consulte al Administrador de la Bodega";
				GuardarNumeroDocumentoDespa($commit=false,&$dbconn);
				return false;
			}
        ...
        $costoProducto=HallarCostoProductoLib($Empresa,$codigoProducto,&$dbconn);
				...
				if($InsertarDocumentod==1){						 
				  if(LiquidacionDespachoMed($Consecutivo,$varsPr[$j]['fecha_cargo'],$cuenta,$codigoProducto,$Cantidad,$varsPr[$j]['precio'],$codigoAgrupamiento,$Plan,$Servicio,$Empresa,$CentroUtili,$departamento,'0','0','IMD',&$dbconn)==false){					  
						$this->FrmError["MensajeError"]="Error al Guardar en la Cuenta del Paciente Verifique la Contratacion";
						GuardarNumeroDocumentoDespa($commit=false,&$dbconn);
						return false;
					}else{
            $query="SELECT existencia,sw_control_fecha_vencimiento FROM existencias_bodegas WHERE empresa_id='$Empresa' AND centro_utilidad='$CentroUtili' AND bodega='$BodegaId' AND codigo_producto='$codigoProducto'";						
						...
						if(($Existencias-$Cantidad)<0){
							$this->FrmError["MensajeError"]="Imposible Realizar la Transaccion, La bodega no Cuenta con las Existencias Solicitadas Disponibles";
							GuardarNumeroDocumentoDespa($commit=false,&$dbconn);
							return false;							
						}
					...
					$this->FrmError["MensajeError"]="Error en la Creacion del Detalle del Documento de Bodega, Consulte al Administrador de la Bodega";
					GuardarNumeroDocumentoDespa($commit=false,&$dbconn);
					return false;								          
				}
			}
		...
		return true;
			...
			$this->FrmError["MensajeError"]="Error Consulte al Administrador";
			//GuardarNumeroDocumentoDespa($commit=false,&$dbconn);
			return false;		
		}		
