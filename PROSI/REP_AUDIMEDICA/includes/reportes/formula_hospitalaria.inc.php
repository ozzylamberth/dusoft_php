...
/**
 * $Id: formula_hospitalaria.inc.php,v 1.13 2006/03/30 16:29:59 tizziano Exp $
 ...
 */
	...
	function final($datos)
	{
		...
		{
			$html.="<TR>";
			$html.="<TD WIDTH='760' HEIGHT=25>".strtoupper($datos[0][nombre_tercero])."<br>".$datos[0][tipo_id_medico].': '.$datos[0][medico_id].' - T.P.: '.$datos[0][tarjeta_profesional].' - '.$datos[0][tipo_profesional]."</TD>";
			$html.="</TR>";
		}
		else
		{
			$html.="<TR>";
			$html.="<TD WIDTH='760' HEIGHT=25>".strtoupper($datos[0][nombre_tercero])."<br>".$datos[0][tipo_id_medico].': '.$datos[0][medico_id]."</TD>";
			$html.="</TR>";
		}
		$html.="</TABLE>";
		return $html;
