...
/**
 * $Id: control_cierre.inc.php,v 1.17 2006/12/13 18:49:30 carlos Exp $
...
function TraerDatoCierre($feha_confirmacion_cierre)
{
		$secuencia=$_SESSION['TMP']['CONTROL_CIERRE']['CIERRE'];
		list($dbconn) = GetDBconn();
		if($feha_confirmacion_cierre)
		{
					$query = "SELECT fecha_confirmacion,observaciones,cierre_de_caja_id
...
*
*/
function TraerDatosPaciente($recibo,$prefijo,$cuenta,$caja_id)
{
			list($dbconn) = GetDBconn();
			$cuenta_tipo=ConsultarCuentaTipo($caja_id);
			...
			elseif ($cuenta=='04')
			{
					$query = "SELECT  btrim(f.primer_nombre||' '||f.segundo_nombre||' ' ||
									...
									AND a.prefijo=b.prefijo 
									AND x.numerodecuenta=b.numerodecuenta
									AND a.centro_utilidad='".$_SESSION['CONTROL_CIERRE']['CENTRO']."' 
									AND a.empresa_id='".$_SESSION['CONTROL_CIERRE']['EMP']."'
									AND x.ingreso=s.ingreso
									AND s.paciente_id=f.paciente_id
...
function TraerPacienteDev($recibo,$prefijo)
{
	...
	function GenerarControlCierreCaja($arr,$id,$caja_id,$dpto,$cierre,$dev,$cuenta)
	{
		...
		$observaciones_confirmacion=$arr[observaciones_confirmacion];
		...
		for($i=0;$i<sizeof($arr);$i++)
		{
					...
					if(empty($_SESSION['TMP']['CONTROL_CIERRE']['DPTO']))
					{
						...
						$pac=TraerDatosPaciente($arr[$i][recibo_caja],$arr[$i][prefijo],$cuenta,$caja_id);
					}
					else
					{
					...
					{
						$salida.="  <td width='60' bgcolor='red'><b><font color='white'>ANULADO</font></b></td>";
					}
					$salida.="</tr>";
		}
		$salida.="</table>";
  	...
  	$salida.= "				<td align='center' width='150' bgcolor=$estilo><font color='black'><b>TOTAL CHEQUES :"." ".FormatoValor($che)."</b></font></td>";
		...
		if(sizeof($dev)>0)
		{
				...
				for($i=0;$i<$ciclo;$i++)
				{
						$salida.="  <td  width='75'>".$dev[$i][prefijo]."$sp $sp".$dev[$i][recibo_caja]."</td>";
  	...
  	if($_SESSION['observa'])
		...
		if($observaciones)
		{
			$salida1=$pdf->MultiCell(170,3,"OBSERVACION CIERRE :".$observaciones,0,'J',0);
			$pdf->WriteHTML($salida1);
		}

		if($observaciones_confirmacion)
		{
			$salida1=$pdf->MultiCell(170,3,"OBSERVACION CONFIRMACION CIERRE:".$observaciones_confirmacion,0,'J',0);
			$pdf->WriteHTML($salida1);
		}
