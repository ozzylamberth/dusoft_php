...
/**
 * $Id: malla_validadora.inc.php,v 1.26 2006/03/10 20:07:15 mauricio Exp $
...
* $Source: /var/cvs/SIIS/includes/malla_validadora.inc.php,v $
* $Revision: 1.26 $
* $Date: 2006/03/10 20:07:15 $
**************************************************************************
* $Id: malla_validadora.inc.php,v 1.26 2006/03/10 20:07:15 mauricio Exp $
**************************************************************************/
...
function MallaValidadora($evolucion)
{
    ...
    $query = "SELECT cargo,hc_os_solicitud_id,cantidad,sw_estado,sw_ambulatorio  
    ...
    $query = "SELECT evento
							FROM ingresos_soat
							WHERE ingreso=$ingreso AND evento IS NOT NULL";
    ...
    if(!$resultado->EOF){
			list($evento_soat)= $resultado->FetchRow();
    }
		else
		{
			$evento_soat = NULL;
...
function MallaValidadoraGenerarOS($cargos,$tipo_id_paciente,$paciente_id,$plan,$tipo_afiliado,$rango,$semanas_cotizadas,$servicio,$fecha_solicitud,$tipo_empleador='',$empleador_id='',$evento_soat=null)
{
//echo "tipo emp=>".$tipo_empleador."  emp id=>".$empleador_id;
        ...
        '".$rango."',".$semanas_cotizadas.",'".$servicio."','".$tipo_id_paciente."','".$paciente_id."',".UserGetUID().",'now()','', $evento_soat)";
        $dbconn->BeginTrans();
...
//OJO REVISAR AQUI
    ...
    *
    */
    function GenerarVariasOS($arreglo,$datos,$autorizacion,$ext,$tipo,$paciente,$plan,$afiliado,$rango,$semana,$servicio,$msg,$empresa,$trascripcion,$eventos_soat)
    {
