...
/**
 * $Id: factura_DUMIAN.inc.php,v 1.6 2008/01/31 22:56:33 carlos Exp $
 ...
 */
  ...
  function CargosFactura($cuenta)
  {
        ...
        $querys = " SELECT 	a.*, 
          ...
          ORDER BY b.descripcion asc,c.descripcion asc
 ";
...
//IS NOT NULL
  ...
  function GenerarFactura($datos,$swTipoFactura)
  {
    ...
    $moderadora = $total = $descuentos = $pagado = $valor_total_cantidad = $valor_total_cargos = $tmpcantidad = $tmpvalor = 0;
    $direc='';
		$cont=0;
		$subtotal=0;    
    for($i=0; $i<sizeof($var);)
    {
      ...
      if(!empty($var[$i][codigo_agrupamiento_id]) AND empty($var[$i][consecutivo]))
      {
				...
				{
					$k=$j;
					while($var[$j][cargo]==$var[$k][cargo])
					{
						$tmpcantidad += $var[$k][cantidad];
						$tmpvalor +=  ($var[$k][valor_cargo]-$var[$k][honorar]);
						$k++;
					}
					$j=$k;
          //$val=$var[$j][valor_cargo]-$var[$j][honorar];
					...
					$html.="<tr><td width=10>&nbsp;</td><td width=80 align='center'>".$var[$k-1][cargo]."</td><td width=550>".substr($var[$k-1][desccargo],0,90)."</td><td width=50 align=CENTER>".FormatoValor($tmpcantidad)."</td><td width=80 align=\"RIGHT\">".FormatoValor($tmpvalor)."</td></tr>";
					//$valor_total_cantidad += $var[$j][cantidad];
					//$valor_total_cargos += $val;
					//$subtotal+=$val;
					$valor_total_cantidad += $tmpcantidad;
					$valor_total_cargos += $tmpvalor;
					$subtotal+=$tmpvalor;
					$tmpcantidad = $tmpvalor = 0;
					//$j++;
				}
				$i=$j;
      }
      elseif(!empty($var[$i][codigo_agrupamiento_id]) AND !empty($var[$i][consecutivo]))
      {
				...
				for($k=0; $k<sizeof($dat); $k++)
				{
					...
					$valor_total_cantidad += $var[$j][cantidad];
					$valor_total_cargos += $var[$j][valor_cargo];
			...
			elseif(empty($var[$i][codigo_agrupamiento_id]))
      {
				...
				$valor_total_cantidad += $var[$i][cantidad];
				$valor_total_cargos += $val;
				$subtotal+=$val;
				$i++;
      }
  ...
  //fin cambio  
	...
	$html.="<tr><td width=530>&nbsp;</td><td width=130>TOTAL CANTIDAD</td><td width=80 align=RIGHT>".FormatoValor($valor_total_cantidad)."</td></tr>";
	$html.="<tr><td width=530>&nbsp;</td><td width=130>VALOR TOTAL CARGOS</td><td width=80 align=RIGHT>".FormatoValor($valor_total_cargos)."</td></tr>";
	...
	if($datos[sw_tipo]===0 OR $datos[sw_tipo]==2)
	{
			...
			$html.="<tr><td width=530>&nbsp;</td><td width=130>VALOR A PAGAR</td><td width=80 align=RIGHT>".$total."</td></tr>";
	...
	elseif($datos[sw_tipo]==1 OR empty($datos[sw_tipo]))
	{
			...
			$html.="<tr><td width=530>SON :"."  ".convertir_a_letras($totall)."</td><td width=130>&nbsp;</td><td width=80 align=RIGHT><b>&nbsp;</b></td></tr>";
	}
	$html.="<tr><td width=760>--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------</td></tr>";
	$html.="<tr><td width=760>&nbsp;</td></tr>";
	...
	$html.="<tr><td width=760>&nbsp;</td></tr>";
	$html.="<tr><td width=20>&nbsp;</td><td width=740 align='center'>ESTA FACTURA CAMBIARIA DE COMPRAVENTA SE ASIMILA PARA TODOS SUS EFECTOS LEGALES A LA LETRA DE CAMBIO</td></tr>";
	$html.="<tr><td width=20>&nbsp;</td><td width=740 align='center'>(ARTICULO 621 - 774 DEL CODIGO DE COMERCIO), EL COMPRADOR ACEPTA QUE LA FIRMA QUE APARECE COMO RECIBIDO</td></tr>";
	$html.="<tr><td width=280>&nbsp;</td><td width=480 align='center'>ESTA AVALANDO LA FIRMA DEL MISMO.</td></tr>";
	$html.="</table>";
