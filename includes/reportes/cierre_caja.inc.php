...
/**
 * $Id: cierre_caja.inc.php,v 1.26 2007/01/11 20:59:23 carlos Exp $
...
//
function TraerDatosPacienteControlConfirmado($recibo,$prefijo,$cuenta_tipo)
{
	list($dbconn) = GetDBconn();
	if($cuenta_tipo=='06')
	{
...
//, a.total_abono
			$query = "SELECT a.factura_fiscal,a.prefijo, a.fecha_registro, a.total_efectivo,
								...
								AND d.caja_id=a.caja_id;";//AND b.sw_facturado='0'
			}
			else
			if($_SESSION['CAJA']['TIPOCUENTA']!='01' AND $_SESSION['CAJA']['TIPOCUENTA']!='06')
			{
//, a.total_abono
			$query = "SELECT a.factura_fiscal,a.prefijo, a.fecha_registro, a.total_efectivo,
										a.total_tarjetas, a.total_cheques, 
	...
	{ 
		...
		for($j=0;$j<sizeof($ar);$j++)
		{
			...
			//echo $arr[$i][recibo_caja].'--'.$arr[$i][prefijo]; exit;
				...
				{
					$salida.="  <td width='60' bgcolor='red'><font color='white'>ANULADO</font></td>";
				}
				else
				{
...
//DEVOLUCIONES
		...
		if($ciclo>0 AND $arrdev!=1)
		{
				...
				$salida.="  <td width='310'>CLIENTE</td>";
...
/*				$salida .= "			<tr>";
				...
				$salida.= "				<td  width='310'   ><font color='white'><b>CLIENTE</b></font></td>";
...
/*				$salida .= "			<tr>";
				...
				if(!empty($ar[0][descripcion_caja]))
				{
					$salida .= "			<tr>";
					$TITULO='REPORTE DE : '.$ar[0][descripcion_caja];
					$salida.= "				<td width=780 ><font color='black' size=10><b>$TITULO</b></font></td>";
					$salida.= "			</tr>";
				}
				$salida .= "			<tr>";
...
//FIN ENCABEZADO
			//$arr=ConsultarRecibos($ar[$j][cierre_caja_id]);
			$arr=$ar;
			unset($arr[observaciones]);
			unset($arr[observaciones_confirmacion]);
			for($i=0;$i<sizeof($arr);$i++)
			{
				if( $i % 2){ $estilo2='#CCCCCC';}
				else {$estilo2='#DDDDDD';}
				...
				$salida.="  <td  width='58' bgcolor=$estilo>".$arr[$i][prefijo]."$sp $sp".$arr[$i][recibo_caja]."</td>";
					//si entra aqui es por q ws cierre normal osea hospitalario
					$pac=TraerDatosPacienteControlConfirmado($arr[$i][recibo_caja],$arr[$i][prefijo],$arr[$i][cuenta_tipo_id]);
			//echo $arr[$i][recibo_caja].'--'.$arr[$i][prefijo]; exit;
				if(empty($pac)){$pac="-------";}
				$fecha=explode(" ",$arr[$i][fecha_registro])	;
				$salida.="  <td  width='75' bgcolor=$estilo>".$fecha[0]."</td>";
				...
				{
					$salida.="  <td width='60' bgcolor='red'><font color='white'><b>ANULADO</b></font></td>";
				}
				$salida.="</tr>";
			}
		$salida.="</table>";
...
//FIN DEVOLUCIONES
		...
		if($ar[observaciones])
		{
			$salida1=$pdf->MultiCell(170,3,"OBSERVACION CIERRE :".$ar[observaciones],0,'J',0);
			$pdf->WriteHTML($salida1);
		}
		if($ar[observaciones_confirmacion])
		{
			$salida1=$pdf->MultiCell(170,3,"OBSERVACION CONFIRMACION CIERRE:".$ar[observaciones_confirmacion],0,'J',0);
			$pdf->WriteHTML($salida1);
		}
			unset($_SESSION['observa']);
	//	unset($_SESSION['CAJA']['VECTOR_CIERRE']);
