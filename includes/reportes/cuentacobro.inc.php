...
/**
 * $Id: cuentacobro.inc.php,v 1.5 2007/05/14 14:57:01 carlos Exp $
  ...
  function DevolucionesProducto($cuenta,$noFacturado)
  {
        ...
        list($dbconn) = GetDBconn();
	      $querys = " SELECT sum(a.cantidad), sum(a.valor_cargo), c.codigo_producto, 
										g.bodega, sum(a.valor_nocubierto), sum(a.valor_cubierto), a.departamento 
										...
										and g.bodega=b.bodega
										$filFacturado
										GROUP BY a.departamento,g.bodega,c.codigo_producto";
        $result = $dbconn->Execute($querys);
        if ($dbconn->ErrorNo() != 0) {
                $this->error = "Error al Cargar el Modulo";
                $this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
        ...
        if(!$result->EOF)
        {
						while(!$result->EOF)
						{
                $var[$result->fields[6].$result->fields[3].$result->fields[2]]=array('cantidad'=>$result->fields[0],'valor'=>$result->fields[1],'cubierto'=>$result->fields[5],'nocubierto'=>$result->fields[4]);
                $result->MoveNext();						
						}             
						$result->Close();
        }
...
function GenerarCuentaCobro($datos=array())
{
	...
	$pdf2->SetFont('Arial','',6);
					...
					if($dat[total_cuenta] >0)
					{
						...
						$html.="<TD bgcolor='#CCCCCC'  COLSPAN='1' WIDTH=80><b>TOTAL EN LETRAS</b>:</TD>  <TD bgcolor='#CCCCCC'  COLSPAN='1' WIDTH=600>".convertir_a_letras($dat[total_cuenta])." CON 00/100</TD>";
						$html.="  </TR>";
						$html.="  </table>";
					}
          ...
          for($i=0; $i<sizeof($ins);)
          {
              ...
              while($ins[$i][bodega]==$ins[$d][bodega])
              {
									...
									while($ins[$k][departamento]==$ins[$d][departamento])
									{
											...
											$valor = $valor + $medDevo[$ins[$k][departamento].$ins[$k][idbodega].$ins[$k][codigo_producto]]['valor'];
											$valpac = $valpac + $medDevo[$ins[$k][departamento].$ins[$k][idbodega].$ins[$k][codigo_producto]]['nocubierto'];
											$valCub = $valCub + $medDevo[$ins[$k][departamento].$ins[$k][idbodega].$ins[$k][codigo_producto]]['cubierto'];
											$cant = $cant - $medDevo[$ins[$k][departamento].$ins[$k][idbodega].$ins[$k][codigo_producto]]['cantidad'];
											$sub+=$valor;
											$totalins+=$valor;
									...
									$d=$k;
									$html.="<tr><td width=80>&nbsp;</td><td width=440><B>  ".$ins[$i][bodega]."-----------------------------------------------------------------------------------------</B></td><td width=60 align=\"CENTER\"><B>".FormatoValor($sub)."</B></td></tr>";
              }
              $i=$d;
          }
