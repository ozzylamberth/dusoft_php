...
/**
 * $Id: FacturaPaciente.inc.php,v 1.5 2008/06/23 19:58:47 cahenao Exp $
 ...
 */
  ...
  function CargosFacturaPaciente($cuenta)
  {
  ...
  function ValoresFacturaPaciente($cuenta)
  {
  		list($dbconn) = GetDBconn();
       $querys = " SELECT 	C.*
  ...
  function InsumosCuentaPaciente($cuenta,$codigo)
  {
    
		list($dbconn) = GetDBconn();
		$querys = "SELECT 	e.descripcion, c.codigo_producto, a.transaccion
  ...
  function InsumosCuentaNoFacturadosPaciente($cuenta,$codigo)
  {
    ...
    $querys = "SELECT   count(*) as total
           ...
           WHERE  a.codigo_agrupamiento_id='".$codigo."'
           AND    a.numerodecuenta=$cuenta
           AND    ((a.facturado = '0' AND a.paquete_codigo_id IS NULL) OR (a.paquete_codigo_id IS NOT NULL AND a.sw_paquete_facturado='0'))
           AND    a.consecutivo=c.consecutivo
           AND    c.codigo_producto=e.codigo_producto
           ";
        $result = $dbconn->Execute($querys);
  ...
  function NombreUsuarioPaciente($factura)
  {
        list($dbconn) = GetDBconn();
  ...
  function NombreUsuarioCierrePaciente($factura)
  {
        list($dbconn) = GetDBconn();
  ...
  function GenerarFacturaPaciente($datos,$swTipoFactura)
  {
    ...
    $_SESSION['REPORTES']['VARIABLE']='facturapaciente';
    $_SESSION['REPORTES']['HOJACARGOS']['ARREGLO']=$datos;
    IncludeLib("tarifario");
		IncludeLib("funciones_admision");
		IncludeLib("funciones_facturacion");
    if($swTipoFactura==1){
      $Dir="cache/facturapaciente".$datos[numerodecuenta]."".$datos[prefijo]."".$datos[factura_fiscal].".pdf";
    }else{
    ...
    $pdf2=new PDF();
    $pdf2->AddPage();		
    $usu=NombreUsuarioPaciente($datos);
    if(!is_array($usu)){
      $usu=NombreUsuarioCierrePaciente($datos);
    }
    ...
    $html.="<tr><td width=760>--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------</td></tr>";
    $var = CargosFacturaPaciente($datos[numerodecuenta]);
    ...
    for($i=0; $i<sizeof($var);)
    {
      ...
      if(!empty($var[$i][codigo_agrupamiento_id]) AND empty($var[$i][consecutivo]))
      {
				...
				{          
					$html.="<tr><td width=10>&nbsp;</td><td width=80 align='center'>".$var[$j][cargo]."</td><td width=550>".substr($var[$j][desccargo],0,90)."</td><td width=50 align=CENTER>".FormatoValor($var[$j][cantidad])."</td><td width=80 align=\"RIGHT\">".FormatoValor($var[$j][valor_cargo])."</td></tr>";
      ...
      elseif(!empty($var[$i][codigo_agrupamiento_id]) AND !empty($var[$i][consecutivo]))
      {
				...
				$j=$i;
				$dat=InsumosCuentaPaciente($var[$i][numerodecuenta],$var[$i][codigo_agrupamiento_id]);
        
				for($k=0; $k<sizeof($dat); $k++)
				{
					if($dat[$k][transaccion] == $var[$j][transaccion])
					{
        ...
        $datNF=InsumosCuentaNoFacturadosPaciente($var[$i][numerodecuenta],$var[$i][codigo_agrupamiento_id]);
        $j+=$datNF;
				$i=$j;
			}
			elseif(empty($var[$i][codigo_agrupamiento_id]))
      {				
				$direc.="<tr><td width=80 align='center'>".$var[$i][cargo]."</td><td width=550>".substr($var[$i][desccargo],0,90)."</td><td width=50 align=CENTER>".FormatoValor($var[$i][cantidad])."</td><td width=80 align=\"RIGHT\">".FormatoValor($var[$i][valor_cargo])."</td></tr>";
				$subtotal+=$var[$i][valor_cargo];
				$i++;
      }
		}
	}
	...
	{
		$var = ValoresFacturaPaciente($datos[numerodecuenta]);
		for($i=0; $i<sizeof($var);$i++)
		{
			$html.="<tr><td width=10>&nbsp;</td><td width=80 align='center'>&nbsp;</td><td width=550>PAGOS PACIENTE</td><td width=50 align=CENTER>&nbsp;</td><td width=80 align=\"RIGHT\">".FormatoValor($var[$i][valor_total_paciente])."</td></tr>";
			$subtotal+=$var[$i][abono_efectivo]+$var[$i][abono_cheque]+$var[$i][abono_tarjetas]+$var[$i][abono_chequespf]+$var[$i][abono_letras];
		}
	}
		$html.=$direc;
  ...
  //fin cambio  
	...
	$html.="<tr><td width=530>&nbsp;</td><td width=130>VALOR PACIENTE</td><td width=80 align=RIGHT>".FormatoValor($subtotal)."</td></tr>";
	...
	elseif($datos[sw_tipo]==1 OR empty($datos[sw_tipo]))
	{
			...
			$total=FormatoValor($valpagarpaciente);
			$totall=str_replace(".","",$total);
	...
	$html.="<tr><td width=760>&nbsp;</td></tr>";
//	$html.="<tr><td width=760 align='center'>ESTA FACTURA CAMBIARIA DE COMPRAVENTA SE ASIMILA PARA TODOS SUS EFECTOS LEGALES A LA LETRA DE CAMBIO</td></tr>";
//	$html.="<tr><td width=760 align='center'>(ARTICULO 621 - 774 DEL CODIGO DE COMERCIO), EL COMPRADOR ACEPTA QUE LA FIRMA QUE APARECE COMO RECIBIDO</td></tr>";
//	$html.="<tr><td width=760 align='center'>ESTA AVALANDO LA FIRMA DEL MISMO.</td></tr>";
	$html.="</table>";
