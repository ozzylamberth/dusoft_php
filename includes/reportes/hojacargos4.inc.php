...
/**
 * $Id: hojacargos4.inc.php,v 1.5 2007/06/06 16:36:38 carlos Exp $
 ...
 */
  ...
  function InsumosCuentaC($cuenta,$noFacturado,$paquete)
  {
        ...
        list($dbconn) = GetDBconn();
	       ...
	       $querys = " select f.descripcion as desagru, a.*, 
													c.codigo_producto, g.descripcion as bodega, g.empresa_id, 
													g.centro_utilidad, g.bodega as idbodega, 
                          ...
                          $filPaquete                         
													order by g.empresa_id,g.centro_utilidad,
													g.bodega,a.departamento,e.descripcion, c.codigo_producto";
  ...
  function GenerarHojaCargos4($datos)
  {
      ...
      //IncludeLib("tarifario");
			...
			$Dir="cache/hojacargos4".$datos[numerodecuenta].".pdf";
      ...
      //$pdf2->Output($Dir,'F');
			...
			//********************************************
			$var=CargosFacturaHoja($datos[numerodecuenta],1);
			if(!empty($var))
			{
				$total=$descuentos=$pagado=0;
				$direc='';
			...
			$medDevo = DevolucionesProducto($datos[numerodecuenta],1);	
				...
				for($i=0; $i<sizeof($ins);){
					...
					while($ins[$i][bodega]==$ins[$d][bodega]){
						...
						while($ins[$k][departamento]==$ins[$d][departamento]){
							...
							$html.="<tr><td width=60>&nbsp;</td><td width=60 align='CENTER'>".$ins[$k][codigo_producto]."</td><td width=50 align=\"CENTER\">&nbsp;</td><td width=50 align=\"CENTER\">".$ins[$k][departamento]."</td><td width=180>".substr($ins[$k][descripcion],0,37)."</td><td width=20 align=\"CENTER\">&nbsp;</td><td width=40 align=CENTER>".$cant." ".$ins[$k][unidad_venta]."</td><td width=60 align=\"CENTER\">0</td><td width=60 align=\"CENTER\">0</td><td width=60 align=\"CENTER\">0</td><td width=60 align=\"CENTER\">0</td><td width=60 align=\"CENTER\">&nbsp;</td></tr>";
              //Esta parte se desarrollo para la impresion de las cadenas que son muy largas
              //y no se alcanzan a mostrar 
              if(strlen($ins[$k][descripcion])>37){
              $totalCadenas=(int)(strlen($ins[$k][descripcion])/37);
