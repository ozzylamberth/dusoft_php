...
/**
 * $Id: factura_DITO.inc.php,v 1.2 2008/10/22 21:33:36 cahenao Exp $
 ...
 */
  ...
  function InsumosCuenta($cuenta,$codigo)
  {
		...
		list($dbconn) = GetDBconn();
		$querys = "SELECT 	e.descripcion, c.codigo_producto, a.consecutivo
			     ...
			     ORDER BY a.codigo_agrupamiento_id,a.facturado desc ";
        $result = $dbconn->Execute($querys);
        if ($dbconn->ErrorNo() != 0) {
                $this->error = "Error al Cargar el Modulo";
                $this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
  ...
  function GenerarFactura($datos,$swTipoFactura)
  {
    ...
    for($i=0; $i<sizeof($var);)
    {
      ...
      if(!empty($var[$i][codigo_agrupamiento_id]) AND empty($var[$i][consecutivo]))
      {
				...
				{          
					$html.="<tr><td width=10>&nbsp;</td><td width=80 align='center'>".$var[$j][cargo_cups]."</td><td width=550>".substr($var[$j][desccargo],0,90)."</td><td width=50 align=CENTER>".FormatoValor($var[$j][cantidad])."</td><td width=80 align=\"RIGHT\">".FormatoValor($var[$j][valor_cargo])."</td></tr>";
      ...
      {				
				$direc.="<tr><td width=80 align='center'>".$var[$i][cargo_cups]."</td><td width=550>".substr($var[$i][desccargo],0,90)."</td><td width=50 align=CENTER>".FormatoValor($var[$i][cantidad])."</td><td width=80 align=\"RIGHT\">".FormatoValor($var[$i][valor_cargo])."</td></tr>";
...
/*		$pro=DatosHonorariosVariasCuentas($cuentas);
  ...
  //fin cambio  
	...
	$html.="<tr><td width=100>&nbsp;</td><td width=130>FIRMA RESPONSABLE</td><td width=150>&nbsp;</td><td width=500>ELABORADO POR:  ".$usu['nombre']."  C = ".$datos[numerodecuenta]."</td></tr>";
	$html.="</table>";
