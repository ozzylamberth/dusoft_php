...
/**
 * $Id: hojacargosPaciente.inc.php,v 1.1 2007/09/03 20:52:24 carlos Exp $
 ...
 */
  ...
  function DevolucionesProducto($cuenta,$noFacturado,$paquete)
  {
        ...
        if(!$result->EOF)
        {
                ...
                //$var[$result->fields[3].$result->fields[4].$result->fields[5].$result->fields[2]]=array('cantidad'=>$result->fields[0],'valor'=>$result->fields[1],'cubierto'=>$result->fields[7],'nocubierto'=>$result->fields[6]);
                $result->MoveNext();						
						}             
						$result->Close();
        }
	...
	function DatosPrincipales($cuenta)
	{
        ...
        if ($result->fields[0] > 0)
        {
					$query = "select (a.valor_total_paciente -(a.abono_efectivo + a.abono_cheque + a.abono_tarjetas + a.abono_chequespf + a.abono_bonos)) as saldo,
										...
										k.municipio, d.fecha_registro, a.valor_total_empresa, a.total_cuenta, b.porcentaje_descuento_iym,
										a.valor_nocubierto,a.valor_cuota_paciente, a.valor_cuota_moderadora
  ...
  function CargosFacturaHoja($cuenta,$noFacturado,$paquete)
  {
        ...
        $querys = "select a.*, b.grupo_tipo_cargo, b.descripcion as desccargo,
                    ...
                    y.descripcion as descargoliquidacion,usu.usuario,za.descripcion as via,
										CLQP.cargo_cups --cargo cuando se tiene la misma equivalencia en dos procedimientos diferentes
                    from tarifarios_detalle as b,
										...
										left join cuentas_liquidaciones_qx_procedimientos CLQP ON (CLQP.consecutivo_procedimiento = x.consecutivo_procedimiento)  --traer cargo cuando se tiene la misma equivalencia en dos procedimientos diferentes
  ...
  //function GenerarHojaCargosPacinete($datos)
	function generarhojacargospaciente($datos)
  {        
      ...
      //IncludeLib("tarifario");
			...
			$Dir="cache/hojacargosPaciente".$datos[numerodecuenta].".pdf";
			...
			for($i=0; $i<sizeof($var);)
			{
						if(!empty($var[$i][codigo_agrupamiento_id]) AND empty($var[$i][consecutivo]))
						{
								...
								while($var[$i][codigo_agrupamiento_id]==$var[$x][codigo_agrupamiento_id]
											AND $var[$i][cargo_cups]==$var[$x][cargo_cups])
								{
										$d=$x;
										if($var[$d][cargoliquidacion]){
											//if($var[$d][cargoliquidacion]!=$var[$d-1][cargoliquidacion]){
											if($var[$d][cargo_cups]!=$var[$d-1][cargo_cups]){
      ...
      $ins=InsumosCuentaC($datos[numerodecuenta]);
			...
			//PROCENTAJE DE DESCUENTO PARA IyM
			//Aplica solo para los planes q tengan el campo porcentaje_descuento_iym
			//mayor a cero
      if($dat[porcentaje_descuento_iym] > 0)
      {
       $porcentaje = $dat[porcentaje_descuento_iym];
      }
      else
      {
       $porcentaje = 0;
      }
			//FIN PROCENTAJE DE DESCUENTO PARA IyM
      $totalins=0;$totalDcto=0;
      if(!empty($ins))
      {
           $sub=0;
          ...
          for($i=0; $i<sizeof($ins);)
          {
              ...
              {   
                          ...
                          $precioUnitario=$ins[$h][precio];		
											...
											/*$valor = $valor + $medDevo[$ins[$k][empresa_id].$ins[$k][centro_utilidad].$ins[$k][numerodecuenta].$ins[$k][codigo_producto]]['valor'];
											$valpac = $valpac + $medDevo[$ins[$k][empresa_id].$ins[$k][centro_utilidad].$ins[$k][numerodecuenta].$ins[$k][codigo_producto]]['nocubierto'];
											$valCub = $valCub + $medDevo[$ins[$k][empresa_id].$ins[$k][centro_utilidad].$ins[$k][numerodecuenta].$ins[$k][codigo_producto]]['cubierto'];
											$cant = $cant - $medDevo[$ins[$k][empresa_id].$ins[$k][centro_utilidad].$ins[$k][numerodecuenta].$ins[$k][codigo_producto]]['cantidad'];
											unset($medDevo[$ins[$k][empresa_id].$ins[$k][centro_utilidad].$ins[$k][numerodecuenta].$ins[$k][codigo_producto]]);
											*/
                      ...
                      $valor = $valor + $medDevo[$ins[$k][empresa_id].$ins[$k][centro_utilidad].$ins[$k][idbodega].$ins[$k][departamento].$ins[$k][codigo_producto]]['valor'];
											...
											//DESCUENTOS POR INSUMO Y MEDICAMENTO
                      $Dcto = ($valCub*$porcentaje)/100;
                      $totalDcto += $Dcto;
                      $Neto = $valpac*(1-($porcentaje/100));
											$sub+=$Neto;
											$totalins+=$Neto;
											$totalBodega+=$Neto;
											
											$html.="<tr><td width=60>&nbsp;</td><td width=60 align='CENTER'>".$ins[$k][codigo_producto]."</td><td width=50 align=\"CENTER\">&nbsp;</td><td width=50 align=\"CENTER\">".$ins[$k][departamento_al_cargar]."</td><td width=180>".substr($ins[$k][descripcion],0,37)."</td><td width=20 align=\"CENTER\">&nbsp;</td><td width=40 align=LEFT>".$cant." ".$ins[$k][unidad_venta]."</td><td width=60 align=\"CENTER\">".FormatoValor($precioUnitario)."</td><td width=60 align=\"CENTER\">".FormatoValor($valor)."</td><td width=60 align=\"CENTER\">".FormatoValor($valCub)."</td><td width=60 align=\"CENTER\">".FormatoValor($valpac)."</td><td width=30 align=\"CENTER\">".FormatoValor($Dcto)."</td><td width=30 align=\"CENTER\">".FormatoValor($Neto)."</td></tr>";
											//FIN DESCUENTOS POR INSUMO Y MEDICAMENTO
                      //Esta parte se desarrollo para la impresion de las cadenas que son muy largas
                      //y no se alcanzan a mostrar 
                      ...
                      //fin
									...
									$d=$k;
									$html.="<tr><td width=80>&nbsp;</td><td width=650><B>  ".$ins[$i][bodega]."----------------------------------------------------------------------------------------------------------------------------------------</B></td><td width=30 align=\"CENTER\"><B>".FormatoValor($totalBodega)."</B></td></tr>";
									/*							
      ...
      $MoviemientosCamas = BuscarMoviemientosCamas($dat[ingreso]);
      if(is_array($MoviemientosCamas))
      {
        //$html.= "  <table width=\"100%\" cellspacing=\"2\" border=\"0\" cellpadding=\"2\" align=\"center\" class=\"normal_10\">";
        $html.="<tr><td width=700><b>ESTANCIAS</b></td></tr>";
        $html.= "<tr>";
        $html.= "<td width=50>CARGO</td>";
        $html.= "<td width=300>DESCRIPCION</td>";
        $html.= "<td width=100>FECHA INGRESO</td>";
        $html.= "<td width=100>FECHA EGRESO</td>";
        //$html.= "<td width=50>PIEZA</td>";
        $html.= " <td width=50>CAMA</td>";
        //$html.= "<td width=50>UBICACION</td>";
        $html.= "<td width=100>DEPARTAMENTO</td>";
        //$html.= "<td width=50>ESTADO</td>";
        $html.= "</tr>";
        for($i=0; $i<sizeof($MoviemientosCamas); $i++)
        {
          $html.= "<tr>";
          $html.= "<td align=\"left\" width=50>".$MoviemientosCamas[$i][cargo]."</td>";
          $html.= "<td align=\"left\" width=300 >".$MoviemientosCamas[$i][descar]."</td>";
          $html.= "<td align=\"center\" width=100>".$MoviemientosCamas[$i][fecha_ingreso]."</td>";
          if(empty($MoviemientosCamas[$i][fecha_egreso]))
          {
              $MoviemientosCamas[$i][fecha_egreso]='CAMA ACTUAL';
              $class='label_mark';
          }
          $html.= "<td align=\"center\" width=100>".$MoviemientosCamas[$i][fecha_egreso]."</td>";
          //$html.= "<td align=\"center\" width=50>".$MoviemientosCamas[$i][pieza]."</td>";
          $html.= "<td align=\"center\" width=50>".$MoviemientosCamas[$i][cama]."</td>";
          //$html.= "<td align=\"center\">".$MoviemientosCamas[$i][ubicacion]."</td>";
          $html.= "<td align=\"center\" width=100>".$MoviemientosCamas[$i][descripcion]."</td>";
/*          $estado='Cargada';
          if(empty($MoviemientosCamas[$i][transaccion]))
          {  $estado='Sin Cargar';}
          $html.= "<td align=\"center\" width=50>$estado</td>";*/
          $html.= "</tr>";
        }
        $html.="<tr><td width=760>--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------</td></tr>";
        //$html.= "</table>";
            ...
            for($i=0; $i<sizeof($ins);)
            {
                ...
                {   
                    ...
                    while($ins[$k][departamento]==$ins[$d][departamento] && $ins[$k][bodega]==$ins[$d][bodega])
                    {
                        ...
                        $valor = $valor + $medDevo[$ins[$k][empresa_id].$ins[$k][centro_utilidad].$ins[$k][idbodega].$ins[$k][codigo_producto]]['valor'];
                        $valpac = $valpac + $medDevo[$ins[$k][empresa_id].$ins[$k][centro_utilidad].$ins[$k][idbodega].$ins[$k][codigo_producto]]['nocubierto'];
                        $valCub = $valCub + $medDevo[$ins[$k][empresa_id].$ins[$k][centro_utilidad].$ins[$k][idbodega].$ins[$k][codigo_producto]]['cubierto'];
                        $cant = $cant - $medDevo[$ins[$k][empresa_id].$ins[$k][centro_utilidad].$ins[$k][idbodega].$ins[$k][codigo_producto]]['cantidad'];
                        //$sub+=$valor;
                        //$totalins+=$valor;
                        //$totalBodega+=$valor;
                        //$html.="<tr><td width=60>&nbsp;</td><td width=60 align='CENTER'>".$ins[$k][codigo_producto]."</td><td width=50 align=\"CENTER\">&nbsp;</td><td width=50 align=\"CENTER\">".$ins[$k][departamento_al_cargar]."</td><td width=180>".substr($ins[$k][descripcion],0,37)."</td><td width=20 align=\"CENTER\">&nbsp;</td><td width=40 align=LEFT>".$cant." ".$ins[$k][unidad_venta]."</td><td width=60 align=\"CENTER\">".FormatoValor($precioUnitario)."</td><td width=60 align=\"CENTER\">".FormatoValor($valor)."</td><td width=60 align=\"CENTER\">".FormatoValor($valCub)."</td><td width=60 align=\"CENTER\">".FormatoValor($valpac)."</td><td width=60 align=\"CENTER\">&nbsp;</td></tr>";
      ...
      //$html.="<tr><td width=520>SUBTOTAL CUENTA: </td><td width=240 align=\"RIGHT\">".FormatoValor($dat[total_cuenta]+$totalEstancia)."</td></tr>";
      //$html.="<tr><td width=520><B>SUBTOTAL DESCUENTOS DE MEDICAMENTOS E INSUMOS ($porcentaje%): </B></td><td width=240 align=\"RIGHT\"><B>".FormatoValor($totalDcto)."</B></td></tr>";
      //$html.="<tr><td width=520><B>TOTAL CUENTA: </B></td><td width=240 align=\"RIGHT\"><B>".FormatoValor(($dat[total_cuenta]+$totalEstancia)-$totalDcto)."</B></td></tr>";
      //$html.="<tr><td width=150>CARGO A CUENTA DE: </td><td width=370>".$dat[nombre_tercero]."</td><td width=240 align=\"RIGHT\">".FormatoValor($dat[valor_total_empresa]-$totalDcto)."</td></tr>";
			if($dat[valor_nocubierto] > 0)
			$html.="<tr><td width=520>VALOR NO CUBIERTO: </td><td width=240 align=\"RIGHT\">".FormatoValor($dat[valor_nocubierto])."</td></tr>";
			if($dat[valor_cuota_paciente] > 0)
			{$html.="<tr><td width=520>VALOR COPAGO: </td><td width=240 align=\"RIGHT\">".FormatoValor($dat[valor_cuota_paciente])."</td></tr>";}
			if($dat[valor_cuota_moderadora] > 0)
      ...
      $html.="<tr><td width=520>TOTAL DE ABONOS: </td><td width=240 align=\"RIGHT\">".FormatoValor($abono)."</td></tr>";
			...
			if(!empty($apro))
			{
      		$html.="<tr><td width=520>APROVECHAMIENTOS POR REDONDEO: </td><td width=240 align=\"RIGHT\">".FormatoValor($apro[precio])."</td></tr>";
			}
			//DESCUENTO
...
/*      $html.="<tr><td width=520>SUBTOTAL CUENTA: </td><td width=240 align=\"RIGHT\">".FormatoValor($dat[total_cuenta]+$totalEstancia)."</td></tr>";
      ...
      $html.="<tr><td width=760>--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------</td></tr>";
      //$html.="<tr><td width=520><B>TOTAL CUENTA: </B></td><td width=240 align=\"RIGHT\"><B>".FormatoValor(($dat[total_cuenta]+$totalEstancia)-$totalDcto)."</B></td></tr>";
      $html.="<tr><td width=520>TOTAL DE ABONOS: </td><td width=240 align=\"RIGHT\">".FormatoValor($dat[valor_nocubierto]+$dat[valor_cuota_moderadora]+$dat[valor_cuota_paciente])."</td></tr>";
      //$html.="<tr><td width=150>CARGO A CUENTA DE: </td><td width=370>".$dat[nombre_tercero]."</td><td width=240 align=\"RIGHT\">".FormatoValor($dat[valor_total_empresa]-$totalDcto)."</td></tr>";
      $html.="<tr><td width=760>--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------</td></tr>";

			/*
			*CAMPO PARA CARGOS NO FACTURADOS
			*/    
