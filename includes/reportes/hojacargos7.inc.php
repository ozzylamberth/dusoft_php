...
/**
 * $Id: hojacargos.inc.php,v 1.36 2007/03/08 20:59:03 lorena Exp $
 ...
 */
  ...
  {     
        ...
        $querys = "select f.descripcion as desagru,
                  ...
                  $filPaquete									
                  order by g.empresa_id,g.centro_utilidad,g.bodega,e.descripcion,c.codigo_producto";
  ...
  function DevolucionesProducto($cuenta,$noFacturado,$paquete)
  {
        ...
        $querys = " SELECT sum(a.cantidad), sum(a.valor_cargo), c.codigo_producto, 
										g.empresa_id,g.centro_utilidad,g.bodega, sum(a.valor_nocubierto), sum(a.valor_cubierto) 
										...
										bodegas_doc_numeraciones as b, bodegas as g, inventarios_productos e
										...
										$filFacturado                    
                    $filPaquete                    
										GROUP BY g.empresa_id,g.centro_utilidad,g.bodega,e.descripcion,c.codigo_producto
										ORDER BY g.empresa_id,g.centro_utilidad,g.bodega,e.descripcion,c.codigo_producto
	...
	function DatosPrincipales($cuenta)
	{
        ...
        if ($result->fields[0] > 0)
        {
					$query = "select (a.valor_total_paciente -(a.abono_efectivo + a.abono_cheque + a.abono_tarjetas + a.abono_chequespf + a.abono_bonos)) as saldo,
										...
										i.razon_social, CU.ubicacion as direccion, CU.telefono as telefonos, i.tipo_id_tercero as tipoid, i.id, j.departamento,
										...
										fac_facturas_cuentas l, centros_utilidad CU
										where a.numerodecuenta=$cuenta 
										and a.empresa_id = CU.empresa_id
										and a.centro_utilidad = CU.centro_utilidad
										...
										and CU.tipo_pais_id=j.tipo_pais_id and CU.tipo_dpto_id=j.tipo_dpto_id
										and CU.tipo_pais_id=k.tipo_pais_id and CU.tipo_dpto_id=k.tipo_dpto_id and CU.tipo_mpio_id=k.tipo_mpio_id
				...
				else
				{
					$query = "select (a.valor_total_paciente -(a.abono_efectivo + a.abono_cheque + a.abono_tarjetas + a.abono_chequespf + a.abono_bonos)) as saldo,
										...
										a.valor_nocubierto,a.valor_cuota_paciente, a.valor_cuota_moderadora
										from cuentas as a, planes as b, terceros as c, pacientes as e, departamentos as  h,
										empresas as i, tipo_dptos as j, tipo_mpios as k, ingresos as d, centros_utilidad CU
										where a.numerodecuenta=$cuenta 
										and a.empresa_id = CU.empresa_id
										and a.centro_utilidad = CU.centro_utilidad
										...
										and d.paciente_id=e.paciente_id
										and CU.tipo_pais_id=j.tipo_pais_id 
										and CU.tipo_dpto_id=j.tipo_dpto_id
										and CU.tipo_pais_id=k.tipo_pais_id 
										and CU.tipo_dpto_id=k.tipo_dpto_id 
										and CU.tipo_mpio_id=k.tipo_mpio_id
										and d.departamento_actual=h.departamento";
				}
				$result = $dbconn->Execute($query);
				if ($dbconn->ErrorNo() != 0) {
  ...
  function CargosFacturaHoja($cuenta,$noFacturado,$paquete)
  {
        ...
        $querys = "select a.*, b.grupo_tipo_cargo, b.descripcion as desccargo,
                    ...
                    and a.cargo!='DCTOREDON'
										...
										$filFacturado                    
                    $filPaquete                   
                    order by a.codigo_agrupamiento_id,x.consecutivo_procedimiento, b.descripcion asc, a.fecha_cargo asc";
	...
	function CantidadDIMD($cuenta)
	{
       list($dbconn) = GetDBconn();
       //$dbconn->debug=true; 
        $querys = "  SELECT   c.codigo_producto, 
        ...
        while(!$result->EOF)
        {
                $varIyM[$result->fields[0]]=$result->GetRowAssoc($ToUpper = false);
                //$varIyM[$result->fields[2]][$result->fields[0]]=$result->GetRowAssoc($ToUpper = false);
                $result->MoveNext();
        }        
        return $varIyM;
	}

  function GenerarHojaCargos7($datos)
  {
      ...
      //IncludeLib("tarifario");
			...
			$Dir="cache/hojacargos7".$datos[numerodecuenta].".pdf";
...
//                  $direc.="<tr><td width=60>&nbsp;</td><td width=60 align='CENTER'>&nbsp;</td><td width=50 align=\"CENTER\">&nbsp;</td><td width=50 align=\"CENTER\">&nbsp;</td><td width=180>".substr($var[$i][desccargo],$inicioCad,37)."</td><td width=20 align=\"CENTER\">&nbsp;</td><td width=40 align=CENTER>&nbsp;</td><td width=60 align=\"CENTER\">&nbsp;</td><td width=60 align=\"CENTER\">&nbsp;</td><td width=60 align=\"CENTER\">&nbsp;</td><td width=60 align=\"CENTER\">&nbsp;</td><td width=40 align=\"CENTER\">&nbsp;</td><td width=40 align=\"CENTER\">&nbsp;</td></tr>";                                       
			...
			$CantidadIMD = CantidadDIMD($datos[numerodecuenta]);
      ...
      if(!empty($ins))
      {
          ...
          for($i=0; $i<sizeof($ins);)
          {
              ...
              {   
									...
									{
											$h=$k;
											$cant=$valor=$precio=$valCub=$valpac=0;
											...
											if(empty($CantidadIMD[$ins[$k][codigo_producto]]))
											$CantidadIMD[$ins[$k][codigo_producto]]['cantidad'] = 0;
          
											$cantidad = $cant - $CantidadIMD[$ins[$k][codigo_producto]]['cantidad'];
											$flag = true;
											/*if($cantidad < 0)
											{
												$CantidadIMD[$ins[$k][codigo_producto]]['cantidad'] = $cantidad * (-1);
												$flag = false;
											}
											else
											{
												$flag = ($cantidad > 0)? true:false;
												$CantidadIMD[$ins[$k][codigo_producto]]['cantidad'] = 0;
											}*/
											...
											$totalBodega+=$valor;
											$valor_abs=abs($valor);
											$totalbodega_abs=abs($totalBodega);
											if($flag)
											{
												$html.="<tr><td width=60>&nbsp;</td><td width=60 align='CENTER'>".$ins[$k][codigo_producto]."</td><td width=50 align=\"CENTER\">&nbsp;</td><td width=50 align=\"CENTER\">".$ins[$k][departamento]."</td><td width=180>".substr($ins[$k][descripcion],0,37)."</td><td width=20 align=\"CENTER\">&nbsp;</td><td width=40 align=LEFT>".FormatoValor($cant)." ".$ins[$k][unidad_venta]."</td><td width=60 align=\"CENTER\">".FormatoValor($precioUnitario)."</td><td width=60 align=\"CENTER\">".FormatoValor($valor_abs)."</td><td width=60 align=\"CENTER\">".FormatoValor($valCub)."</td><td width=60 align=\"CENTER\">".FormatoValor($valpac)."</td><td width=60 align=\"CENTER\">&nbsp;</td></tr>";
											}
					  //Esta parte se desarrollo para la impresion de las cadenas que son muy largas
                      //y no se alcanzan a mostrar 
...
//                        $html.="<tr><td width=60>&nbsp;</td><td width=60 align='CENTER'>&nbsp;</td><td width=50 align=\"CENTER\">&nbsp;</td><td width=50 align=\"CENTER\">&nbsp;</td><td width=180>".substr($ins[$k][descripcion],$inicioCad,37)."</td><td width=20 align=\"CENTER\">&nbsp;</td><td width=40 align=CENTER>&nbsp;</td><td width=60 align=\"CENTER\">&nbsp;</td><td width=60 align=\"CENTER\">&nbsp;</td><td width=60 align=\"CENTER\">&nbsp;</td><td width=60 align=\"CENTER\">&nbsp;</td><td width=60 align=\"CENTER\">&nbsp;</td></tr>";                                                  
                      ...
                      //fin
									...
									$d=$k;
									$html.="<tr><td width=80>&nbsp;</td><td width=440><B>  ".$ins[$i][bodega]."-----------------------------------------------------------------------------------------</B></td><td width=60 align=\"CENTER\"><B>".FormatoValor($totalbodega_abs)."</B></td></tr>";
									/*							
