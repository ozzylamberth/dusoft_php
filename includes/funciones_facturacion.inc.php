...
/**
 * $Id: funciones_facturacion.inc.php,v 1.49 2009/07/30 12:50:18 johanna Exp $
    ...
    */
    function PagosCajaRapida($cuenta)
    {
                ...
                $query = "SELECT  b.fecha_registro, 
                                  ...
                                  b.usuario_id, 
                                  c.nombre
                          FROM    fac_facturas_cuentas as a, 
                                  fac_facturas_contado as b, 
                                  system_usuarios as c,
                                  fac_facturas ff
                          WHERE   a.numerodecuenta=$cuenta
                          AND     a.prefijo=b.prefijo 
  ...
  function BuscarCargoAjusteApro($Cuenta)
  {
    ...
    */
    function PendientesCargar($ingreso)
    {
            ...
            $query = "SELECT  e.fecha, 
                              f.nombre, 
                              g.descripcion as desdpto,
                              a.cargo_cups, 
                              d.descripcion as descups,
                              a.procedimiento_pendiente_cargar_id, 
                              g.empresa_id,
                              g.centro_utilidad, 
                              g.servicio, 
                              a.autorizacion_int,
                       ...
                       LEFT JOIN hc_sub_procedimientos_realizados_caracteristicas i ON(a.ingreso=i.ingreso and a.evolucion_id=i.evolucion_id and a.cargo_cups=i.cargo_cups),
                              ...
                              profesionales as f, 
                              departamentos as g, 
                              tipos_profesionales as h,
                              terceros ter,
                              hc_sub_procedimientos_realizados_cups_dpto x
                        WHERE a.ingreso=$ingreso 
                        AND   a.cargo_cups=d.cargo
                        ...
                        AND   ter.tipo_id_tercero=f.tipo_id_tercero 
                        and   ter.tercero_id=f.tercero_id 
                        AND   a.cargo_cups=x.cargo_cups 
                        AND   x.departamento=g.departamento";
            $result = $dbconn->Execute($query);
            if ($dbconn->ErrorNo() != 0) {
                    $this->error = "Error al Cargar el Modulo";
                    $this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
        ...
        $dbconn->debug=true;
            $query = "(  SELECT b.plan_id, 
                        ...
                        FROM    tarifarios_detalle a, 
                                ...
                                tarifarios_equivalencias as g
                        WHERE   g.cargo_base='$cargo' 
                        and     g.cargo=a.cargo
                        and     g.tarifario_id=a.tarifario_id 
    ...
    */
    ...
    function BuscarLiqHabitacion($clase_cama,$plan)
    {
        list($dbconn) = GetDBconn();
                $query = "SELECT a.* 
                          FROM     planes_liq_camas as a
                          WHERE    a.plan_id=$plan 
                          AND      a.tipo_clase_cama_id=$clase_cama";
        $result=$dbconn->Execute($query);
        if ($dbconn->ErrorNo() != 0) {
          $this->error = "Error al Cargar el Modulo";
          $this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
    ...
    */
    function InsertarTmpCuentasDetalle($EmpresaId,$CUtilidad,$cuenta,$plan,$arr)
    {
        ...
        list($dbconn) = GetDBconn();
                ...
                //print_r($_SESSION['CUENTAS']['ADD_CARGOS');
                                $costo_variable = false;
    ...
    function BuscarMoviemientosCamas($ingreso,$cuenta,$FechaI,$FechaF,$hora_inicial,$hora_final)
    {
                 list($dbconn) = GetDBconn();
		 $condicionRangoFechas = "";
		 if(!empty($FechaI) OR !empty($FechaF) OR !empty($hora_inicial) OR !empty($hora_final))
		 {
		 	$condicionRangoFechas = " AND ((a.fecha_ingreso >= '$FechaI $hora_inicial:00') AND (a.fecha_egreso <= '$FechaF $hora_final:59' OR a.fecha_egreso ISNULL)) ";
		 }
		 //echo '<pre>';
                $query = "SELECT a.cargo,a.fecha_ingreso,a.fecha_egreso,a.precio,a.cama,b.pieza,b.ubicacion,
                                        ...
                                        g.descripcion as descar, a.numerodecuenta, a.ingreso
    ...
    {       //$vector el vector de loquidacion con todos los cargos, posiciones elegidas
		    ...
		    //
		    if(empty($liq[cantidad]))
		    {
		    	$liq[valor_cargo] = $liq[precio_plan];
			$liq[valor_cubierto] = $liq[precio_plan];
			$liq[cantidad] = 1;
		    }
		    //
                    ...
                    if($dbconn->ErrorNo() != 0) {
                            ...
                            echo $this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg()."[".get_class($this)."][".__LINE__."]";
                            $this->fileError = __FILE__;
                            $this->lineError = __LINE__;
                            $dbconn->RollbackTrans();
                            return false;
