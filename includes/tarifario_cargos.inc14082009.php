...
/**
 * $Id: tarifario_cargos.inc.php,v 1.40 2008/01/09 16:33:35 alexgiraldo Exp $
...
//Funcion que calcula el copago acumulado a� de un paciente en un plan (excluye la cuenta actual).
...
function LiquidarCargosCuentaVirtual($cargos=array(),$inasistencias=array(),$otrosCargos=array(),$modificacionCopagosYCuotasM=array(), $planId, $tipo_afiliado_id='', $rango, $semanas_cotizacion=0, $Servicio, $tipo_id_paciente = '', $paciente_id = '', $tipo_empleador_id = '', $empleador = '')
{
    ...
    {
        $cargo_liquidado = LiquidarCargoCuenta($cuenta=0 ,$cargoliquidar['tarifario_id'] ,$cargoliquidar['cargo'], $cargoliquidar['cantidad'] ,$cargoliquidar['descuento_empresa'], $cargoliquidar['descuento_paciente'], $aplicar_descuento_empresa=true ,$aplicar_descuento_paciente=true ,$precio=0, $planId, $Servicio, $semanas_cotizacion, $tipo_empleador_id, $empleador, $tipo_id_paciente, $paciente_id);
    ...
    if(!empty($otrosCargos['IYM']))
    {
        ...
        {
//*********************************************************
            if($IM['facturado'] && $IM['valor_cubierto']>0)
            {
                $sw_copagos += $IM['sw_cuota_paciente'];
                $sw_cm += $IM['sw_cuota_moderadora'];
            }
//*********************************************************
            $cargoIyM['valor_no_cubierto']=0;
            $cargoIyM['valor_cubierto']=0;
            $cargoIyM['valor_cargo']=0;
            $cargoIyM['porcentaje_gravamen']=0;
            $cargoIyM['valor_descuento_empresa']=0;
            ...
            $cargoIyM['valor_no_cubierto']= $IM['valor_nocubierto'];
            ...
            $cargoIyM['tmp_cuenta_insumos_id'] = $IM['tmp_cuenta_insumos_id'];
...
function LiquidarCargoCuenta($cuenta=0 ,$tarifario ,$cargo ,$cantidad=1 ,$descuento_manual_empresa=0 ,$descuento_manual_paciente=0 ,$aplicar_descuento_empresa=true ,$aplicar_descuento_paciente=true, $precio=0, $planId='', $Servicio='', $semanas_cotizacion='', $tipo_empleador_id = '', $empleador = '',$tipo_id_paciente = '', $paciente_id = '', $tipoUninadTiempo = NULL)
{
...
//                     AND d.tipo_id_paciente= b.tipo_id_paciente";
    ...
    $valor = LiquidarCargo($DatosCuenta['plan_id'],$tarifario,$cargo,$cantidad,$precio=0,$datosExcepcionesAdicionales,$tipoUninadTiempo);
    ...
    $hayDescuentos=false;
    if(!empty($descuento_manual_empresa))
    {
        if(is_numeric($descuento_manual_empresa))
        {
            $descuento_manual_empresa = abs($descuento_manual_empresa);
            $valor['valor_descuento_empresa'] = $valor['valor_cubierto'] - $descuento_manual_empresa;
            $valor['valor_cubierto'] = $descuento_manual_empresa;
            $hayDescuentos = true;
        }
    }


    if(!empty($descuento_manual_paciente))
    {
        if(is_numeric($descuento_manual_paciente))
        {
            $descuento_manual_paciente = abs($descuento_manual_paciente);
            $valor['valor_descuento_paciente'] = $valor['valor_no_cubierto'] - $descuento_manual_paciente;
            $valor['valor_no_cubierto'] = $descuento_manual_paciente;
            $hayDescuentos = true;
        }
    }
    ...
    if($hayDescuentos)
    {
        $valor['valor_cargo'] = round(($valor['valor_cubierto'] + $valor['valor_no_cubierto']),GetDigitosRedondeo());
        $valor['precio_plan'] = round(($valor['valor_cargo'] / $valor['cantidad']),GetDigitosRedondeo());
    }
...
function LiquidarCargo($plan,$tarifario,$cargo,$cantidad=1,$precio=0,$excepcionesAdicionales=array(),$tipoUninadTiempo=NULL)
{
    list($dbconn) = GetDBconn();
    ...
    $camas_liq = ConsultaPrecio($plan,$cargo,$tarifario);
    if(!empty ($camas_liq))
    {
        $cargo_info['porcentaje']= $camas_liq['porcentaje'];
        $cargo_info['precio']=$camas_liq['precio'];
    ...
    switch($cargo_info['tipo_unidad_id'])
    {
        ...
        case '03' :
            ...
            //$cargo_info['precio']=round(round($cargo_info['precio']/100,0)*100,GetDigitosRedondeo());
	    $cargo_info['precio']=round(round($cargo_info['precio']/100,0)*100,-2);
        break;
        case '04' :
        ...
        case '05' :
            ...
            $cargo_info['precio']=$cargo_info['precio']*$uvr['uvr_valor'][0];
        break;
        case '06' :
    ...
    function UVRPaquetePlan($plan,$tarifario)
    {
        ...
        if($resultado->EOF)
        {
            ...
            if(!$resultado->EOF)
            {
                ...
                $uvr['uvr_valor']=$resultado->FetchRow();
                $resultado->Close();
                return $uvr;
            }
        }
...
//ECHO "<PRE>" . PRINT_R($datosAdicionales,TRUE). "</PRE>";
    ...
    * @access public
    */
    function LiquidarIyMQX($cuenta=NULL ,$producto='' ,$cantidad=1 ,$descuento_manual_empresa=0 ,$descuento_manual_paciente=0 ,$aplicar_descuento_empresa=TRUE ,$aplicar_descuento_paciente=TRUE ,$precio=NULL ,$planId=NULL,$autorizar=FALSE,$departamento=NULL,$empresa=NULL,$cuenta_liquidacion_qx_id=NULL)
    {
        ...
        $datosAdicionales['departamento'] = $departamento;
        $datosAdicionales['servicio'] = NULL;
        $datosAdicionales['cuenta_liquidacion_qx_id'] = $cuenta_liquidacion_qx_id;
        $datosAdicionales['descuento_manual_empresa'] = $descuento_manual_empresa;
        $datosAdicionales['descuento_manual_paciente'] = $descuento_manual_paciente;
        $datosAdicionales['aplicar_descuento_empresa'] = $aplicar_descuento_empresa;
        $datosAdicionales['aplicar_descuento_paciente'] = $aplicar_descuento_paciente;
    ...
    *
    */
    function ConsultaPrecio($plan_id,$cargo,$tarifario)
    {
        list($dbconn) = GetDBconn();
       //$dbconn->debug = true;
        $query = "SELECT  p.porcentaje ,
                          p.valor_lista AS precio
                  FROM    planes_tipos_camas p,
                          cups c
                  WHERE   c.sw_internacion =1
                  AND     p.cargo_cups = c.cargo
                  AND     p.plan_id = ".$plan_id."
                  AND     p.cargo  = '".$cargo."' 
                  AND     p.tarifario_id = '".$tarifario."'";
				$result = $dbconn->Execute($query);
