...
/**
 * $Id: api.inc.php,v 1.3 2009/12/18 16:38:56 hugo Exp $
...
function GetStyleFrames()
{
  ...
  function SIIS_sfrtime($fecha,$tipo='M',$abreviado=false)
  {
    if(!empty($fecha))
    {
      ...
      $tiempo =  mktime($c[0],$c[1],0,$a[1],$b[0],$a[0]);
      if($tipo=='M')
      {
        if($abreviado==false)
          return strftime("%A %d de %B de %Y a las %H:%M",$tiempo);
        else
          return strftime("%a %d de %b de %Y a las %H:%M",$tiempo);
      }
      else
      {
        $p='pm';
        if($c[0]>=0 and $c[0]<12)  $p='am';
        
        $diaNombre = GetDia(date("w",$tiempo), $abreviado);
        $mesNombre = GetMes(date("n",$tiempo), $abreviado);
        $horas = date("h",$tiempo);
        
        $cadena = $diaNombre." ".$b[0]." de ".$mesNombre." de ".$a[0]." a las ".$horas.":".$c[1]." ".$p;
        return $cadena;
      }
    }
    return false;
...
function download($archivo,$nombre="Descargar",$link=false,$comprimir=false,$boton=true)
{
  ...
  *
  * @param integer $mes Identificador del mes
  * @param boolean $abreviado Indica si se debe retornar el nombre abreviado o completo
  *
  * @return String
  */
  function GetMes($mes)
  {
    ...
    $rest = $meses[$mes];
    if($abreviado) $rest = substr($rest, 0, 3);

    return $rest;
  }
  /**
  * Funcion que retorna el nombre del dia de la semana
  *
  * @param integer $dia Identificador del día
  * @param boolean $abreviado Indica si se debe retornar el nombre abreviado o completo
  *
  * @return String
  */
  function GetDia($dia, $abreviado = false)
  {
    if(empty($dia)) return "";
