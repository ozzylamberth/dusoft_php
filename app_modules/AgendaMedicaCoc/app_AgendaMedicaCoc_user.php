<?php
/*MovimientoInventarioPorProducto
ConsultaSolicitudesRealizadasPorMedicamento*/
/**
 * $Id: app_AgendaMedicaCoc_user.php,v 1.1 2009/09/02 13:08:12 hugo Exp $
 * @copyright (C) 2005 IPSOFT - SA (www.ipsoft-sa.com)
 * @package IPSOFT-SIIS
 *
 * Clase para accesar los metodos privados de la clase de presentaci�, se compone de metodos    
 * publicos para insertar en la base de datos, actualizar y borrar de la base de datos y mostrar
 * la forma de inserci� y la consulta de la agenda medica.
 */


class app_AgendaMedicaCoc_user extends classModulo
{
	...
	*/
	function app_AgendaMedicaCoc_user()
	{
		return true;
	}
...
//print_r($var);
      ...
      $this->mensajeDeError = "No se pudo Incluir el archivo : classes/reports/reports.class.php";
        ...
        $reporte=$classReport->PrintReport($tipo_reporte='pos',$tipo_modulo='app',$modulo='AgendaMedicaCoc',$reporte_name='tiquete_cita',$var,$impresora,$orientacion='',$unidades='',$formato='',$html=1);
        if(!$reporte)
				{
...
*/
	...
	////////////////////////////////////////
	///funcion  declarada para la busque da de la fecha de bnnacimeinto para los paciente requerimiento de la clinica de rehabilitacion
	/////////////////////////////////////
...
*/
	...
	function PedirDatosPaciente()
	{
		...
		$_SESSION['PACIENTES']['RETORNO']['modulo']='AgendaMedicaCoc';
		$_SESSION['PACIENTES']['RETORNO']['tipo']='user';
		$_SESSION['PACIENTES']['RETORNO']['metodo']='PedirDatosPaciente';
		if(!$Paciente->BuscarIngresoActivoPaciente($_SESSION['CumplirCita']['tipo_id_paciente'] ,$_SESSION['CumplirCita']['paciente_id'] ,$_SESSION['CumplirCita']['empresa'] ,$_SESSION['CumplirCita']['plan_id'], $accion=array('contenedor'=>'app','modulo'=>'AgendaMedicaCoc','tipo'=>'user','metodo'=>'RetornoPaciente')))
		{
...
*/
	...
	function CitasIncumplidasPaciente($tipoid,$paciente,$plan)
	{
		...
		WHERE h.numero_orden_id=i.numero_orden_id AND a.agenda_cita_asignada_id=h.agenda_cita_asignada_id AND a.agenda_cita_id=b.agenda_cita_id and
		b.agenda_turno_id=c.agenda_turno_id and date(c.fecha_turno) < date(now()) and date(c.fecha_turno)>(date(now())-".$dato.") and a.paciente_id='$paciente' and
		tipo_id_paciente='$tipoid' and c.profesional_id=e.tercero_id and c.tipo_id_profesional=e.tipo_id_tercero and i.sw_estado!=3 and a.sw_atencion!=3 and
		e.tercero_id=f.tercero_id and e.tipo_id_tercero=f.tipo_id_tercero AND a.agenda_cita_asignada_id NOT IN (SELECT agenda_cita_asignada_id FROM agenda_citas_asignadas_cancelacion);";
...
*/
	...
	function CitasPorCumplirPaciente($tipoid,$paciente,$plan)
	{
		...
		FROM agenda_citas_asignadas as a, agenda_citas as b, agenda_turnos as c, profesionales as e, terceros as f, os_cruce_citas as h, os_maestro as i
		WHERE h.numero_orden_id=i.numero_orden_id AND a.agenda_cita_asignada_id=h.agenda_cita_asignada_id AND a.agenda_cita_id=b.agenda_cita_id and
		b.agenda_turno_id=c.agenda_turno_id and date(c.fecha_turno) >= date(now()) and a.paciente_id='$paciente' and
		tipo_id_paciente='$tipoid' and c.profesional_id=e.tercero_id and c.tipo_id_profesional=e.tipo_id_tercero and i.sw_estado!=3 and a.sw_atencion!=3 and
		e.tercero_id=f.tercero_id and e.tipo_id_tercero=f.tipo_id_tercero AND a.agenda_cita_asignada_id NOT IN (SELECT agenda_cita_asignada_id FROM agenda_citas_asignadas_cancelacion);";
		//echo $sql1="select c.fecha_turno || ' ' || b.hora as fecha, e.nombre, f.agenda_cita_asignada_id from agenda_citas_asignadas as a, agenda_citas as b, agenda_turnos as c, profesionales as e where a.agenda_cita_id=b.agenda_cita_id and b.agenda_turno_id=c.agenda_turno_id and date(c.fecha_turno)&lt;date(now()) and date(c.fecha_turno)>(date(now())-".$result->fields[0].") and a.paciente_id='".$paciente."' and tipo_id_paciente='".$tipoid."' and c.profesional_id=e.tercero_id and c.tipo_id_profesional=e.tipo_id_tercero and a.sw_atencion!=3 and a.sw_estado!=1;";
		$result = $dbconn->Execute($sql);
...
*/
	...
	function CitasPacienteAtender2()
	{
		...
		f.numerodecuenta, j.tipo_afiliado_id, j.rango, j.semanas_cotizadas,
		j.autorizacion_int, j.autorizacion_ext, f.numero_orden_id, f.sw_estado,
		h.os_maestro_cargos_id, k.descripcion
		...
		agenda_citas as b,agenda_turnos as c,profesionales as d,terceros as e,tipos_consulta as k
		WHERE
		a.paciente_id='".$_REQUEST['Documento']."' and
		a.tipo_id_paciente='".$_REQUEST['TipoDocumento']."' and
		a.agenda_cita_id=b.agenda_cita_id and b.agenda_turno_id=c.agenda_turno_id and
		...
		and
		(b.sw_estado='0' OR b.sw_estado='1') order by (c.fecha_turno || ' ' || b.hora);";
	...
	function CitasPacienteAtenderNombre()
	{
      ...
      $doc=" AND a.paciente_id='".$_REQUEST['Documento']."' and a.tipo_id_paciente='".$_REQUEST['TipoDocumento']."'";
		...
		$sql="SELECT a.agenda_cita_asignada_id,
								...
								a.nombre,a.fecha_turno,
								f.sw_estado,a.agenda_cita_id,								
								i.ingreso,j.plan_id, h.cargo, 
								h.tarifario_id, f.numerodecuenta, 
								j.tipo_afiliado_id, j.rango, 
								j.semanas_cotizadas, j.autorizacion_int, 
								j.autorizacion_ext, f.numero_orden_id, 
								f.sw_estado,h.os_maestro_cargos_id, 
								j.orden_servicio_id,a.paciente_id, 
		 						a.tipo_id_paciente,f.cargo_cups,
								a.nombre_pac
...
*/
	function ListadoCitasAtender(){
		...
		c.agenda_cita_id_padre,carcit.descripcion as cargo_cita
		from agenda_citas as b
		left join agenda_citas_asignadas as c on (b.agenda_cita_id=c.agenda_cita_id and c.sw_atencion!=1)
		left join cargos_citas as carcit on(c.cargo_cita=carcit.cargo_cita)
		...
		where b.agenda_turno_id in(".$var.") and (b.sw_estado='0' OR b.sw_estado='1' OR b.sw_estado='2')
		order by b.hora;";
		$result = $dbconn->Execute($sql);
		$i=0;
		if ($dbconn->ErrorNo() != 0){
...
*/
	...
	function LlamarIngresarNota()
     {
          $_SESSION['INSERTAR']['AGENDAMEDICACOC']['INGRESO'] = $_REQUEST['ingreso'];
          $_SESSION['INSERTAR']['AGENDAMEDICACOC']['EVOLUCION'] = $_REQUEST['evolucion'];
          $_SESSION['INSERTAR']['AGENDAMEDICACOC']['NOMBRE'] = $_REQUEST['nombre'];
          $_SESSION['INSERTAR']['AGENDAMEDICACOC']['ACCION'] = 'AgendaMedicaCoc';
	...
	function LLegadaAutorizarPaciente()
	{
		...
		if(empty($_SESSION['AsignacionCitas']['cita']))
		{
			if(empty($_SESSION['CumplirCita']['cita']))
			{
				...
				{
					$this->FormaMensaje('No tiene autorizacion','Autorizaci�',ModuloGetURL('app','AgendaMedicaCoc','','DatosPaciente'),'Volver');
					return true;
				}
			...
			else
			{
				if(empty($_SESSION['AUTORIZACIONES']['RETORNO']['Autorizacion']) OR empty($_SESSION['AUTORIZACIONES']['RETORNO']['plan_id']))
				{
					...
					$this->FormaMensaje('No tiene autorizacion','Autorizaci�',ModuloGetURL('app','AgendaMedicaCoc','',$metodo),'Volver');
					unset($_SESSION['AUTORIZACIONES']);
					return true;
				}
				$_SESSION['DATOSPACIENTE']=$_SESSION['AUTORIZACIONES']['RETORNO']['ARREGLO'];
		...
		{//print_r($_SESSION['AUTORIZACIONES']['RETORNO']['plan_id']);
			...
			{
				$this->FormaMensaje('Se cancelo el proceso de autorizaci�','Autorizaci�',ModuloGetURL('app','AgendaMedicaCoc','','DatosPaciente'),'Volver');
				unset($_SESSION['AUTORIZACIONES']);
				return true;
			}*/
	...
	function AutorizarPaciente()
	{
		...
		if(empty($_REQUEST['CUMPLIR']['paciente']))
		{
			if(empty($_SESSION['AsignacionCitas']['cita']))
			{
				...
				$_SESSION['AUTORIZACIONES']['RETORNO']['modulo']='AgendaMedicaCoc';
        ...
        and a.tarifario_id=b.tarifario_id and a.cargo=b.cargo and b.grupo_tarifario_id=c.grupo_tarifario_id and b.subgrupo_tarifario_id=c.subgrupo_tarifario_id and c.plan_id=".$_SESSION['AsignacionCitas']['Responsable']." and b.tarifario_id=c.tarifario_id;";
...
*/
	...
	function DatosIniciales()
	{
		...
		if(!$Paciente->BuscarIngresoActivoPaciente($TipoDocumento,$Documento,$_SESSION['AsignacionCitas']['empresa'],$Responsable,$accion=array('contenedor'=>'app','modulo'=>'AgendaMedicaCoc','tipo'=>'user','metodo'=>'DatosPaciente')))
		{
						$this->error = $Paciente->error ;
						$this->mensajeDeError = $Paciente->mensajeDeError;
						unset($Paciente);
...
*/
    ...
    else
    {
      ...
      if(!$this->FormaPedirDatos($TipoDocumento,$Documento,$mensaje,false,null,$datos['primer_apellido'],$datos['segundo_apellido'],$datos['primer_nombre'],$datos['segundo_nombre'],$datos['telefono_residencia'],$datos['tipo_sexo_id'],$datos['direccion_residencia']))
      {
        return false;
      }
      return true;
...
*/
	...
	function GetProgramasPYP()
	{
		...
		if($dbconn->ErrorNo() != 0)
		{
			$this->error = "Error al Cargar el Modulo AgendaMedicaCoc - GetProgramasPYP - SQL";
			$this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
			return false;
		}
		else
	...
	function ValidaInscripcionPaciente($tipo_id_paciente,$paciente_id,$programa)
	{
		...
		if ($dbconn->ErrorNo() != 0)
		{
			$this->error = "Error al Cargar el Modulo AgendaMedicaCoc - ValidaInscripcionPaciente - SQL";
			$this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
			return false;
		}
		else
...
*/
	...
	function BuscarInformacionCitaPrincipal($cita)
	{
		...
		$sql="SELECT $cita as id_cita, fecha_turno || ' ' || hora as fecha, nombre_tercero, f.sw_estado,
					...
					WHERE a.agenda_cita_asignada_id=$cita	AND  a.agenda_cita_id=b.agenda_cita_id
					AND b.agenda_turno_id=c.agenda_turno_id AND c.profesional_id=d.tercero_id
					AND c.tipo_id_profesional=d.tipo_id_tercero AND a.agenda_cita_id=a.agenda_cita_id_padre
...
*/
    ...
    $_SESSION['AsignacionCitas']['servicio']=$this->BusquedaServicio($_SESSION['AsignacionCitas']['departamento']);
      ...
      if($_SESSION['AgendaMedicaCoc']['CitaPrioritaria']==1){
		...
		if($_SESSION['AsignacionCitas']['Existe']){
				...
				$sql="update pacientes set residencia_direccion='".$_SESSION['AsignacionCitas']['Direccion']."',
							residencia_telefono='".$_SESSION['AsignacionCitas']['Telefono']."',fecha_nacimiento='".$_SESSION['AsignacionCitas']['FechaNacimiento']."'
				...
				$sql="insert into pacientes(paciente_id, tipo_id_paciente, primer_apellido, segundo_apellido, primer_nombre,
							...
							'".$_SESSION['AsignacionCitas']['Telefono']."','".$_SESSION['AsignacionCitas']['FechaNacimiento']."', '".$_SESSION['AsignacionCitas']['Sexo']."', '".date("Y-m-d H:i:s")."', '".GetVarConfigAplication('DefaultPais')."', '".GetVarConfigAplication('DefaultDpto')."', '".GetVarConfigAplication('DefaultMpio')."',
        ...
        if($_SESSION['AgendaMedicaCoc']['CitaPrioritaria']==1){
          $prioritaria=1;
				...
				if(empty($_REQUEST['historia'])){
					 if(empty($_SESSION['AsignacionCitas']['CentroRemision']))
					 	$sql="insert into agenda_citas_asignadas (agenda_cita_asignada_id, agenda_cita_id, paciente_id, tipo_id_paciente, tipo_cita, plan_id,cargo_cita,observacion,usuario_id,agenda_cita_id_padre,fecha_registro,sw_prioritaria) values(".$b.",".$a.",'".$_SESSION['AsignacionCitas']['Documento']."','".$_SESSION['AsignacionCitas']['TipoDocumento']."', '".$_SESSION['AsignacionCitas']['TipoCita']."','".$_SESSION['AsignacionCitas']['Responsable']."', '".$_SESSION['AsignacionCitas']['cargo_cups']."','".$_SESSION['AsignacionCitas']['Observacion']."',".UserGetUID().",".$agendaCitaPadre.",'".date("Y-m-d H:i:s")."','$prioritaria');";
					 else		 
					 	$sql="insert into agenda_citas_asignadas (agenda_cita_asignada_id, agenda_cita_id, paciente_id, tipo_id_paciente, tipo_cita, plan_id,cargo_cita,observacion,usuario_id,agenda_cita_id_padre,fecha_registro,sw_prioritaria,centro_remision_id) values(".$b.",".$a.",'".$_SESSION['AsignacionCitas']['Documento']."','".$_SESSION['AsignacionCitas']['TipoDocumento']."', '".$_SESSION['AsignacionCitas']['TipoCita']."','".$_SESSION['AsignacionCitas']['Responsable']."', '".$_SESSION['AsignacionCitas']['cargo_cups']."','".$_SESSION['AsignacionCitas']['Observacion']."',".UserGetUID().",".$agendaCitaPadre.",'".date("Y-m-d H:i:s")."','$prioritaria','".$_SESSION['AsignacionCitas']['CentroRemision']."');";
					
				}else{
					 if(empty($_SESSION['AsignacionCitas']['CentroRemision']))
					 	$sql="insert into agenda_citas_asignadas (agenda_cita_asignada_id, agenda_cita_id, paciente_id, tipo_id_paciente, tipo_cita, plan_id,cargo_cita,observacion,usuario_id, sw_historia,agenda_cita_id_padre,fecha_registro,sw_prioritaria) values(".$b.",".$a.",'".$_SESSION['AsignacionCitas']['Documento']."','".$_SESSION['AsignacionCitas']['TipoDocumento']."', '".$_SESSION['AsignacionCitas']['TipoCita']."','".$_SESSION['AsignacionCitas']['Responsable']."', '".$_SESSION['AsignacionCitas']['cargo_cups']."','".$_SESSION['AsignacionCitas']['Observacion']."',".UserGetUID().", '1',".$agendaCitaPadre.",'".date("Y-m-d H:i:s")."','$prioritaria');";
					 else	
					 	$sql="insert into agenda_citas_asignadas (agenda_cita_asignada_id, agenda_cita_id, paciente_id, tipo_id_paciente, tipo_cita, plan_id,cargo_cita,observacion,usuario_id, sw_historia,agenda_cita_id_padre,fecha_registro,sw_prioritaria, centro_remision_id) values(".$b.",".$a.",'".$_SESSION['AsignacionCitas']['Documento']."','".$_SESSION['AsignacionCitas']['TipoDocumento']."', '".$_SESSION['AsignacionCitas']['TipoCita']."','".$_SESSION['AsignacionCitas']['Responsable']."', '".$_SESSION['AsignacionCitas']['cargo_cups']."','".$_SESSION['AsignacionCitas']['Observacion']."',".UserGetUID().", '1',".$agendaCitaPadre.",'".date("Y-m-d H:i:s")."','$prioritaria','".$_SESSION['AsignacionCitas']['CentroRemision']."');";
				}
      ...
      unset($_SESSION['CONSULTAEXT']); 
		...
		else
		{
			...
			if(ModuloGetVar('app','AgendaMedicaCoc','NoManejoCaja') == 1)
			{
				...
				$_REQUEST['modulo'] ='AgendaMedicaCoc';
	...
	function IngresoSegumientoPYP($agendaCitaPadre)
	{
		...
		switch(SessionGetVar("Programa_id"))
		{
			case 1:
				...
				if ($dbconn->ErrorNo() != 0)
				{
					$this->error = "Error en el Modulo AgendaMedicaCoc- IngresoSegumientoPYP - SQL 1";
					$this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
			...
			case 2:
				...
				if ($dbconn->ErrorNo() != 0)
				{
					$this->error = "Error en el Modulo AgendaMedicaCoc - InsertarSeguimientoPYP - SQL 2";
					$this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
					$this->frmError["MensajeError"]=$this->error."<br>".$this->mensajeDeError;
					$this->ban=1;
					$dbconn->RollBackTrans();
			...
			case 3:
				...
				if ($dbconn->ErrorNo() != 0)
				{
					$this->error = "Error en el Modulo AgendaMedicaCoc- InsertarSeguimientoPYP - SQL 3";
					$this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
					$this->frmError["MensajeError"]=$this->error."<br>".$this->mensajeDeError;
					$this->ban=1;
					$dbconn->RollBackTrans();
	...
	function GetCitasID($paciente_id,$tipo_id_paciente,$cita_padre,$fecha_turno=null)
	{
		...
		if(!empty($con))
		{
			...
			";
				...
				if ($dbconn->ErrorNo() != 0)
				{
					$this->error = "Error en el Modulo AgendaMedica Coc- GetCitasID - SQL";
					$this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
					$this->frmError["MensajeError"]=$this->error."<br>".$this->mensajeDeError;
					$this->ban=1;
					$dbconn->RollBackTrans();
...
*/
		...
		function MenuCaja()
		{
			...
			if(empty($_SESSION['CumplirCita']['cita']))
			{
				$this->salida.= gui_theme_menu_acceso('CAJA RAPIDA',$_SESSION['SEGURIDAD']['CAJARAPIDA']['arreglo'],$_SESSION['SEGURIDAD']['CAJARAPIDA']['caja'],$_SESSION['SEGURIDAD']['CAJARAPIDA']['url'],ModuloGetURL('app','AgendaMedicaCoc','user','PantallaFinal',array('DiaEspe'=>$_REQUEST['DiaEspe'])));
			}
			else
			{
...
*/
	...
	function ProfeConsul()
	{
		...
		$query="select distinct a.nombre_tercero as nombre, a.descripcion, a.consultorio, a.profesional_id, a.tipo_id_profesional 
						from (select nombre_tercero, d.descripcion, c.consultorio, a.profesional_id, a.tipo_id_profesional, e.estado from agenda_turnos as a left join profesionales_estado as e on (a.profesional_id=e.tercero_id and a.tipo_id_profesional=e.tipo_id_tercero and e.empresa_id='".$_SESSION['CumplirCita']['empresa']."' and e.departamento='".$_SESSION['CumplirCita']['departamento']."') left join consultorios as c on (a.consultorio_id=c.consultorio) left join tipos_consultorios as d on(c.tipo_consultorio=d.tipo_consultorio), profesionales as b, terceros as g 
...
*/
	...
	function ListadoCitas(){
		...
		$query="SELECT b.hora, c.agenda_cita_asignada_id,
						...
						null as nombre_paciente,p.sw_estado,
						c.observacion,c.agenda_cita_id,
						c.plan_id, h.cargo,
						i.ingreso, h.tarifario_id,
						p.numerodecuenta, p.numero_orden_id,
						j.tipo_afiliado_id, j.rango,
						j.semanas_cotizadas, j.autorizacion_int,
						j.autorizacion_ext, h.os_maestro_cargos_id,
						j.orden_servicio_id,b.agenda_cita_id,
						p.cargo_cups,c.agenda_cita_id_padre,
						AT.agenda_turno_id,
		...
		while(!$result->EOF){
			if(!empty($result->fields[2]) or !empty($result->fields[5])){
				...
				/*while($t<sizeof($datos[0])){
				  //Pregunta si es el mismo Paciente
					if($datos[2][$t]==$result->fields[2] and $datos[3][$t]==$result->fields[3]){
					  //Pregunta si el turno de agenda es igual al Anterior
      ...
      return true;
		...
		if($swPlan == 1)
		{
			...
			$arreglo = array('TipoConsulta'=>$this->TipoConsulta,'Documento'=>$this->Documento,'TipoDocumento'=>$this->TipoDoc,
											 'Responsable'=>$this->Plan,$arr);
			$this->action1 = ModuloGetURL('app','AgendaMedicaCoc','user','DatosPaciente');
			$this->action2 = ModuloGetURL('app','AgendaMedicaCoc','user','ValidarEventoSoat',$arreglo);
...
// 		print_r($cnt1);
	...
	function CantidadCitasPendientesPaciente($TipoId,$PacienteId)
	{
	
		$departamento = $this->GetDepartamento($_SESSION['Atencion']['cita']);
		list($dbconn) = GetDBconn();
		$SQL=" select COUNT(a.orden_servicio_id) FROM os_ordenes_servicios as a, os_maestro as b WHERE 
		a.tipo_id_paciente = '".$TipoId."' 
		AND a.paciente_id = '".$PacienteId."' 
		AND a.departamento = '".$departamento."' 
		AND a.orden_servicio_id = b.orden_servicio_id 
		AND b.sw_estado NOT IN ('3', '9')
		;";
		
		
		$rst = $dbconn->Execute($SQL);
			if($dbconn->ErrorNo() != 0){
				$this->error = "Error al traer informacion  metodo CantidadCitasPendientesPaciente.";
				$this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg().'['.get_class($this).']['.__LINE__.']';
				return false;
			}
			else{
...
// 					
			...
			if($asignada){
				...
				if(ModuloGetVar('app','AgendaMedicaCoc','NoManejoCaja') == 1)
				{
					$this->frmError["MensajeError"]="SE ASIGNO LA CITA<br>PARA EL DIA ".$FechaTurno."-".$HoraTurno." CON EL PREFESIONAL $nombreprofesional";
