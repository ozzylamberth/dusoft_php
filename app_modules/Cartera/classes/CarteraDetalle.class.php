<?php
  /**
  * $Id: CarteraDetalle.class.php,v 1.6 2009/06/26 13:53:16 hugo Exp $
  ...
  */
	class CarteraDetalle
	{
		...
		*/
		function ObtenerFacturasEnviadas($datos)
		{
      $sql  = "         SELECT  A.intervalo, ";
      $sql .= "                 A.prefijo, ";
      ...
      $sql .= "                 A.retencion, ";
      $sql .= "                 B.saldo, ";
      $sql .= "                 A.total_nota_debito,";
      $sql .= "                 A.total_recibo, ";
      $sql .= "                 A.total_nota_glosa, ";
      $sql .= "                 A.total_nota_ajuste, ";
      $sql .= "                 A.total_nota_credito, ";
      ...
      $sql .= "                           factura_fiscal,";
      $sql .= "                           empresa_id,";
      $sql .= "                           (NOW()::date - (substring(MIN(intervalo)::text from 1 for 4)||'-'||substring(MIN(intervalo)::text from 5 for 6)||'-01')::date)/30  AS intervalo, ";
      ...
      $sql .= "				          LEFT JOIN ";
			...
			$sql .= "					          GROUP BY 4,5,6 ";
			$sql .= "				          ) AS GL ";
      $sql .= "				          ON(	GL.prefijo = B.prefijo AND ";
			$sql .= "						          GL.factura_fiscal = B.factura_fiscal AND ";
			$sql .= "						          GL.empresa_id = B.empresa_id ";
			$sql .= "					        ) ";
      $sql .= "         WHERE  A.empresa_id = B.empresa_id ";
      $sql .= "         AND    A.prefijo = B.prefijo ";
      $sql .= "         AND    A.factura_fiscal = B.factura_fiscal ";
      if($datos['intervalo'])
        $sql .= "         AND     A.intervalo = ".$datos['intervalo']." ";
      $sql .= "         ORDER BY A.intervalo,A.prefijo,A.factura_fiscal ";
			...
			{		
				$this->conteo ++;
        $facturas[$this->ObtenerLabel($rst->fields[0])][]  = $rst->GetRowAssoc($ToUpper = false);
				$rst->MoveNext();
			}
			$rst->Close();
    ...
    /**
		...
		* @return array facturas del cliente
		*/
		function ObtenerFacturasNoEnviadas($datos)
		{
      $sql  = "         SELECT  A.intervalo, ";
      $sql .= "                 A.prefijo, ";
      $sql .= "                 A.factura_fiscal, ";
    ...
    /**
		* Funcion donde se obtiene el tipo id y el id del terceor al que
    * pertenece el plan  
		* 
    ...
    *
      ...
      $sql .= "WHERE  plan_id  = ".$plan_id." ";
			...
			while(!$rst->EOF)
			{
				$rtn = $rst->GetRowAssoc($ToUpper = false);;
				$rst->MoveNext();
		  }
			$rst->Close();
    ...
    /**
		...
		* @return array facturas del cliente
		*/
		function ObtenerFacturasPlan($datos,$opcion)
		{
      ...
      $sql .= "                    FROM   cartera.facturas_resumen";
      $sql .= "                    WHERE  empresa_id = '".$datos['empresa_id']."'";
      $sql .= "                    AND    tipo_id_tercero = '".$tercero['tipo_tercero_id']."'";
      $sql .= "                    AND    tercero_id = '".$tercero['tercero_id']."' ";  
      $sql .= "                    GROUP BY prefijo, factura_fiscal,empresa_id ";
      $sql .= "                    HAVING  SUM(total_nota_anulacion) = 0 ";
      $sql .= "                 ) AS A, ";
      $sql .= "                 ( ";
      ...
      if($datos['intervalo'])
      {
        $sql_int = $this->ObtenerCondicion($this->ObtenerLabel($datos['intervalo']));
        $sql .= "         AND     A.intervalo ".$sql_int." ";
      }
      $sql .= "         ORDER BY A.intervalo,A.prefijo,A.factura_fiscal ";
    ...
    /**
    * Funcion donde se obtiene la etiqueta correspondiente a cada intervalo
    *
    * @param integer $intervalo Identificador del intervalo
    *
    * @return string
    */
    function ObtenerLabel($intervalo)
    {
      $label = "";
      switch($intervalo)
      {
        case '0': $label = "Corriente"; break;
        case '1': $label = "0 a 30"; break;
        case '2': $label = "31 a 60"; break;
        case '3': $label = "61 a 90"; break;
        case '4': $label = "91 a 120"; break;
        case '5': $label = "121 a 150"; break;
        case '6': $label = "151 a 180"; break;
        case '7': 
        case '8': 
        case '9': 
        case '10': 
        ...
        case '12': 
          $label = "181 a 360"; 
        break; 
        default: $label = "Mas de 360"; break;
      }
      return $label;
    }
    /**
    ...
    * @return string
    */
    function ObtenerCondicion($rango)
    {
      $condicion = "";
      
      switch($rango)
      {
        case "Corriente": $condicion = " = 0"; break;
        case "0 a 30"; $condicion = " = 1"; break;
        case "31 a 60"; $condicion = " = 2"; break;
        case "61 a 90"; $condicion = " = 3"; break;
        case "91 a 120"; $condicion = " = 4"; break;
        case "121 a 150"; $condicion = " = 5"; break;
        case "151 a 180"; $condicion = " = 6"; break;
        case "181 a 360"; $condicion = " IN (7,8,9,10,11,12)"; break; 
        default: $condicion = " >= 13"; break;
      }
      return $condicion;
    }
		/*********************************************************************************
