<?php
  /**
  * @package IPSOFT-SIIS
  * @version $Id: IngresoAfiliados.php,v 1.2 2009/09/23 21:42:42 hugo Exp $
  ...
  */
  function SeleccionarSubGrupoPrincipal($form)
  {
    ...
    {
      $html .= "document.registrar_afiliacion.sub_grupos_principales.options[".$i."] = new Option('".substr($dtl['descripcion_ciuo_88_subgrupo_principal'],0,40)."','".$key."',false, false);\n";
      $html .= "document.registrar_afiliacion.sub_grupos_principales.options[".($i++)."].title = '".$dtl['descripcion_ciuo_88_subgrupo_principal']."';\n";
    }
  ...
  */
  function SeleccionarSubGrupos($form)
  {
    ...
    {
      $html .= "document.registrar_afiliacion.sub_grupo.options[".$i."] = new Option('".substr($dtl['descripcion_ciuo_88_subgrupo'],0,40)."','".$key."',false, false);\n";
      $html .= "document.registrar_afiliacion.sub_grupo.options[".($i++)."].title = '".$dtl['descripcion_ciuo_88_subgrupo']."';\n";
    }
    $objResponse = new xajaxResponse();
    $objResponse->script($html);
  ...
  */
  function SeleccionarGruposPrimarios($form)
  {
    ...
    {
      $html .= "document.registrar_afiliacion.grupos_primarios.options[".$i."] = new Option('".substr($dtl['descripcion_ciuo_88_grupo_primario'],0,40)."','".$key."',false, false);\n";
      $html .= "document.registrar_afiliacion.grupos_primarios.options[".($i++)."].title = '".$dtl['descripcion_ciuo_88_grupo_primario']."';\n";
    }
    $objResponse = new xajaxResponse();
    $objResponse->script($html);
  ...
  */
  function SeleccionarActividad($form)
  {
    ...
    $html .= "document.registrar_afiliacion.clase_actividad.options.length = 0 ;\n";
    $html .= "document.registrar_afiliacion.clase_actividad.options[0] = new Option('--SELECCIONAR--','-1',false, false);\n";
    $i = 1;
    foreach($grupo_primario as $key => $dtl)
    {
      $html .= "document.registrar_afiliacion.grupo_actividad.options[".$i."] = new Option('".substr($dtl['descripcion_ciiu_r3_grupo'],0,40)."','".$key."',false, false);\n";
      $html .= "document.registrar_afiliacion.grupo_actividad.options[".($i++)."].title = '".$dtl['descripcion_ciiu_r3_grupo']."';\n";
    }  
    $objResponse = new xajaxResponse();
    $objResponse->script($html);
  ...
  * @return object
  */
  function SeleccionarClase($form)
  {
    $afi = AutoCarga::factory("Afiliaciones", "", "app","UV_Afiliaciones");
    $grupo_primario = $afi->ObtenerClasesActividadEconomica($form['division_actividad'],$form['grupo_actividad']);
    ...
    foreach($grupo_primario as $key => $dtl)
    {
      $html .= "document.registrar_afiliacion.clase_actividad.options[".$i."] = new Option('".substr($dtl['descripcion_ciiu_r3_clase'],0,40)."','".$key."',false, false);\n";
      $html .= "document.registrar_afiliacion.clase_actividad.options[".($i++)."].title = '".$dtl['descripcion_ciiu_r3_clase']."';\n";
    }
    $objResponse = new xajaxResponse();
    $objResponse->script($html);
  ...
  */
  function AdicionarBeneficiario($forma)
  {
    ...
    else
    {
      ...
      $beneficiarios[$forma['tipo_id_beneficiario']][$forma['documento']]['parentesco_texto'] = $datos[$forma['parentesco']]['descripcion_parentesco'];
  ...
  * @param string $grupo Identificador del grupo de la actividad economica
  * @param string $grupo_ocupacion Identificador del grupo de la ocupacion
  * @param string $clase Identificador de la clase de la actividad economica
  *
  * @return object
  */
  function SeleccionarDatosDefectoClase($form,$grupo,$grupo_ocupacion,$clase)
  {
    $objResponse = new xajaxResponse();
    $html = "";
    if($clase) $html .= SeleccionarDefaultActividad($form,$grupo,$clase,&$objResponse);
  ...
  * @return String
  */
  function SeleccionarDefaultActividad($form,$grupo,$clase,$objResponse)
  {
    $afi = AutoCarga::factory("Afiliaciones", "", "app","UV_Afiliaciones");
    
    $grupo_primario = $afi->ObtenerGruposActividadEconomica($form['division_actividad']);
    
    $clases = $afi->ObtenerClasesActividadEconomica($form['division_actividad'],$grupo);
    ...
    $html .= "document.registrar_afiliacion.clase_actividad.options[0] = new Option('--SELECCIONAR--','-1',false, false);\n";
    $i = 1;
    foreach($clases as $key => $dtl)
    {
      $html .= "document.registrar_afiliacion.clase_actividad.options[".$i."] = new Option('".substr($dtl['descripcion_ciiu_r3_clase'],0,40)."','".$key."',false, false);\n";
      if($clase == $key)
      {
        $texto = $dtl['descripcion_ciiu_r3_clase'];
        $html .= "document.registrar_afiliacion.clase_actividad.options[".$i."].selected = true;\n";
      }
      $html .= "document.registrar_afiliacion.clase_actividad.options[".($i++)."].title = '".$dtl['descripcion_ciiu_r3_clase']."';\n";
    }
  ...
  */
  function SeleccionarOcupacionDefecto($grupo,$objResponse)
  {
    ...
    foreach($grupo_primario as $key => $dtl)
    {
      ...
      if($key == $grupo)
      {
        $texto = $dtl['descripcion_ciuo_88_grupo_primario'];
  ...
  * @return String
  */
  function CambiarSubEstados($form,$subestado)
  {
    $objResponse = new xajaxResponse();
    ...
    foreach($subestados as $key => $dtl)
    {
      ...
      $html .= "document.cambiarestado.subestado_afiliado_id.options[".$i++."] = new Option('".$dtl['descripcion_subestado']."','".$key."',false, $sl);\n";
    }
  ...
  */
  ...
  /**
  * Permite mostrar los tipos de afiliados y rangos correspondientes a 
  * un plan
  *
  ...
  * return object  
  */
  function MostrarInformacionPlan($form)
  {
    $objResponse = new xajaxResponse();
    $sla = AutoCarga::factory('Afiliaciones','classes','app','UV_Afiliaciones');
     
    $tipos = $sla->ObtenerTiposAfiliadosPlan($form['plan_atencion']);
    $rangos = $sla->ObtenerRangosNiveles($form['plan_atencion']);
    
    $html1 = "";
    $s = "";
    if(sizeof($tipos) > 1)
    {
      $html1 .= "			<select name=\"tipo_afiliado_plan\" class=\"select\">\n";
      $html1 .= "				<option value=\"\">-- Seleccionar --</option>\n";
      foreach($tipos as $key => $valor)
      {
        ($form['tipo_afiliado_plan'] == $valor['tipo_afiliado_id'])? $s = "selected": $s = "";
        $html1 .= "				<option value=\"".$valor['tipo_afiliado_id']."\" ".$s.">".$valor['tipo_afiliado_nombre']."</option>\n";
      }
      $html1 .= "			</select>\n";
    }
    else
    {
      $html1 .= "			".$tipos[0]['tipo_afiliado_nombre']."\n";
      $html1 .= "			<input type=\"hidden\" name=\"tipo_afiliado_plan\" value=\"".$tipos[0]['tipo_afiliado_id']."\">\n";
    }
    
    $html2 = "";
    if(sizeof($rangos) > 1)
    {
      $html2 .= "			<select name=\"rango_afiliado_plan\" class=\"select\">\n";
      $html2 .= "				<option value=\"\">-- Selecc --</option>\n";
      foreach($rangos as $key => $valor)
      {
        ($form['rango_afiliado_plan'] == $valor['rango'])? $s = "selected": $s = "";
        $html2 .= "				<option value=\"".$valor['rango']."\" ".$s.">".$valor['rango']."</option>\n";
      }
      $html2 .= "			</select>\n";
    }
    else
    {
      ...
      $html2 .= "			".$cotizante['rango']."\n";
      $html2 .= "			<input type=\"hidden\" name=\"rango_afiliado_plan\" value=\"".$cotizante['rango']."\">\n";
    }
    
    $objResponse->assign("tipo_afiliado_div","innerHTML",$html1);
    $objResponse->assign("rango_afiliado_div","innerHTML",$html2);
