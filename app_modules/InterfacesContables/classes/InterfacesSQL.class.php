<?php
  /******************************************************************************
  * $Id: InterfacesSQL.class.php,v 1.3 2007/04/26 19:30:59 jgomez Exp $
  ...
  ********************************************************************************/
  ...
  IncludeClass('InterfacesContables');
  class InterfacesSQL
	{
		
    var $MOVIMIENTO;
      
    function InterfacesSQL()
    {
      $this->MOVIMIENTO="cg_mov_".SessionGetVar("EMPRESA").".cg_mov_contable_".SessionGetVar("EMPRESA")."";
    }
/*****************************************************************************
  *metodo para la revision de documentos por lapso
*****************************************************************************/
    
    function RevisionLapso($empresa_id,$lapso,$dia1,$dia2)
   {
    ...
    $contabilizador= new InterfacesContables;
    $resultado=$contabilizador->GetInformacionDocumentosLapso($empresa_id,$lapso,$dia1,$dia2);
    if($resultado === false)
     {
    ...
    {
        $cad.= "RESULTADO NO ESPERADO :<BR><BR>";
        $cad.= "<pre>".$resultado."</pre><br>";
        return $cad; 
    }
...
*
*************************************************************************************/    
function GIC($empresa_id,$interface_id,$lapso,$dia1,$dia2,$prefijos)
{ 
      if($dia1==='0')
      {
        $dia1=null;
      ...
      if($dia2==='0')
      {
       ...
       $interconta= new InterfacesContables;
       $resultado = $interconta->SetInterface($empresa_id,$interface_id);
       if(!$resultado)
        {
            $cad= "ERRORES RETORNADOS POR EL METODO SetInterface($empresa_id): <br>" . $interconta->Err() . "<br>" . $interconta->ErrMsg() . "<br><br>";
            return $cad;
        }
         
       $resultado = $interconta->GenerarInterfaceDocumentosLapso($prefijos,$lapso,$dia1,$dia2);
       if($resultado === false)
        {
            $cad= "ERRORES : " . $interconta->Err() . "<br>" . $interconta->ErrMsg() . "<br>";
            return $cad;
        }
...
{ 
       $sql=" select * from tipo_id_terceros"; 
...
function ContabilizarDocx($datos)
   {
    ...
    for($i=0;$i<count($datos);$i++)
    {
        ...
        if($resultad ===false)
        {
          $CAD="problemas".$contabilizador->ErrMsg();
          RETURN $CAD;
        }
        else
        {
  ...
  function Contabilizarlapso($empresa_id,$prefijo,$lapso,$actualizar=false)
   {
    ...
    {
        $cad.= "RESULTADO NO ESPERADO :<BR><BR>";
        $cad.= "<pre>".print_r($resultado,true)."</pre><br>";
    }
