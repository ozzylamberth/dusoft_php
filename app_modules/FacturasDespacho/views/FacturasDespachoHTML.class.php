...
class FacturasDespachoHTML {
    ...
    function Listado_PedidosFacturar($action, $datos, $documento_id, $Tercero, $Pedidos, $conteo, $pagina) {
        ...
        $html .= "
                    ...
                    function validacion(form){
                        valor = document.getElementById('forma_pago'+form).value;
                        if(valor==-1){
                            alert('Debe seleccionar una forma de Pago');
                            return false;
                        };

                        document.getElementById('submit_generar').disabled = true;
                        return true;
                    }
        ";
        $html .= "</script>";
        ...
        foreach ($Pedidos as $key => $valor) {
            $html .= "				<form name=\"" . $valor['pedido_cliente_id'] . "\" id=\"" . $valor['pedido_cliente_id'] . "\" method=\"POST\" onSubmit=\"return validacion(".$valor['pedido_cliente_id'].");\" action=\"" . $action['guardar'] . "\" >";
            $html .= "                  <tr class=\"modulo_list_claro\" onMouseOver=\"mOvr(this,'#ffffff');\" onMouseOut=\"mOut(this,'#DDDDDD');\">\n";
            $html .= "						<td width=\"100%\">";
            $html .= "<table width=\"100%\" align=\"center\" class=\"modulo_table_list\">\n";
            $html .= "<tr class=\"modulo_list_claro\" onMouseOver=\"mOvr(this,'#ffffff');\" onMouseOut=\"mOut(this,'#DDDDDD');\">\n";
            ...
            $html .= "</td>";
            $html .= "<td colspan='2'>";
            $id=$valor['pedido_cliente_id'];
            $html .= "<select id=\"forma_pago$id\" name=\"forma_pago$id\" class=\"select\">";
            ...
            $html .= "									<input id=\"submit_generar\" type=\"submit\" " . $disabled . " value=\"GENERAR FACTURA DE PEDIDO. #" . $valor['pedido_cliente_id'] . "\" class=\"input-submit\">";
            $html .= "									</td>";
            $html .= "                  			</tr>\n";
            $html .= "							</table>";
            $html .= "						</td>";
     ...
     * @return string $html retorna la cadena con el codigo html de la pagina
     */
    function Listado_FacturasGeneradas($action, $datos, $facturas_cliente, $tipos_ids_terceros, $Prefijos_Facturas, $conteo, $pagina, $tipo_id_tercero = "", $tercero_id = "") {
        ...
        $html .= "	</tr>";
        $html .= "  <tr align=\"center\" >\n";
        $html .= "      <input type=\"hidden\"  name=\"tipo_id_tercero\" id=\"tipo_id_tercero\" value=\"".$tipo_id_tercero."\">";
        ...
        if ($tipo_id_tercero != "" && $tercero_id != ""){
            //$action['volver'] = ModuloGetURL("app", "FacturasDespacho", "controller", "Crear_Facturas", array("tipo_id_tercero" => $tipo_id_tercero, "tercero_id" => $tercero_id));
                ...
                function generar_facturacion_agrupada(){
                    ...
                    if(documentos.length == 0){
                        ...
                        if(documentos.length > 1)
                            xajax_enviar_documentos_agrupados('{$datos_cliente['tipo_id_tercero']}', '{$datos_cliente['tercero_id']}', documentos,forma_pago);
                        else
