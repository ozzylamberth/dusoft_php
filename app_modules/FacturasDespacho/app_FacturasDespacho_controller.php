...
class app_FacturasDespacho_controller extends classModulo {
	...
	/**
	* +Descripcion Metodo encargado de registrar el detalle de la factura al ser generada
	**/
    function Generar_FacturaCliente() {
        ...
        $forma_pago=$request['forma_pago'.$request['pedido_cliente_id']]; 
        ...
        $query = "";
        $query = $sql->InsertarFactura($Documento_Factura, $Tercero, $_REQUEST, $Parametros_Retencion,$forma_pago);
        $query .= $sql->InsertarPcFactura($Documento_Factura,'0');
        ...
        $query .= $sql->actualizarEstadoFacturaPedido($_REQUEST);
        ...
        if (!$token) {
            if($ip==""){
             $html .= "alert('LA IP NO TIENE PERMISO PARA REALIZAR LA PETICIÓN');";
            }
            $html .= " history.go(-1) ";
        } else {
            ...
            $action['recarga'] = ModuloGetURL("app", "FacturasDespacho", "controller", "Facturas_Generadas", array("buscador" => $request['buscador'], 'resultado_ws' => $request['resultado_sincronizacion_ws'], "tipo_id_tercero" => $request["tipo_id_tercero"], "tercero_id" => $request["tercero_id"]));
    ...
    function generar_facturacion_agrupada() {
        ...
        if (count($factura_agrupada) == 0) {  
            ...
            $query .= $sql_facturacion->insertar_factura_agrupada($documento_facturacion, $datos_cliente, $parametros_retencion,$forma_pago);
            $query .= $sql_facturacion->InsertarPcFactura($documento_facturacion,'1');
            foreach ($documentos as $key => $value) {
                ...
                foreach ($obtener_documento_despacho['DETALLE'] as $k => $medicamentos) {
					...
					$query .= $sql_facturacion->actualizarEstadoFacturaPedido($datos_documento_despacho);
                }
            }
            ...
            $token = $sql_facturacion->EjecutarSql($query);
             ...
             $ip=$sql_facturacion->getRealIP();
        
            if (!$token) {
                $html = "<script>";
               if($ip==""){
                 $html .= "alert('LA IP NO TIENE PERMISO PARA REALIZAR LA PETICION');";
                }else{
                $html .= "alert('SE HA GENERADO UN ERROR AL CREAR LA FACTURA No.[{$documento_facturacion['prefijo']}-{$documento_facturacion['numeracion']}]');";
                }
                $html .= "        history.go(-1);
                    </script>
                ";
    ...
    function Facturas_Generadas() {
        ...
        $action['volver'] = ModuloGetURL("app", "FacturasDespacho", "controller", "Menu");
        $this->salida = $act->Listado_FacturasGeneradas($action, $datos, $Listado_FacturasCliente, $tipos_ids_terceros, $Prefijos_Facturas, $sql->conteo, $sql->pagina, $request["tipo_id_tercero"], $request["tercero_id"]);
