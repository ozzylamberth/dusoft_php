...
/**
* app_GestionSeguimientoPFliar_user.php
*
//*
**/


class app_GestionSeguimientoPFliar_user extends classModulo
{

	function app_GestionSeguimientoPFliar_user()
	{
			return true;
	}
	...
	function UsuariosPyP()//Función de permisos
	{
		...
		if($i<>0)
		{
				...
				$com[1]=$prueba6;
				$url[0]='app';
				$url[1]='GestionSeguimientoPFliar';
				$url[2]='user';
				$url[3]='FrmGestionSeguimientoPFliar';
				$url[4]='SeguimientoPFliar';
				if(empty($_SESSION['SEGURIDAD'][$url[4]]['Arreglo'][0]))
				{
		...
		else
		{
				...
				$titulo = "GESTION DE SEGUIMIENTO PLANIFICACION FAMILIAR";
	...
	function IngresarSeguimientoPFliar()
	{
		list($dbconn) = GetDBconn();
		$dbconn->BeginTrans();
		...
		if ($dbconn->ErrorNo() != 0)
		{
			$this->error = "Error en el Modulo GestionSeguimientoPFliar - IngresarSeguimientoPFliar - SQL";
			$this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
		...
		else
		{
			...
			$query="SELECT MAX(pyp_plan_fliar_seguimiento_id) 
							FROM pyp_plan_fliar_seguimiento
			...
			if ($dbconn->ErrorNo() != 0)
			{
				$this->error = "Error en el Modulo app_GestionSeguimientoPFliar - InsertarSeguimientoPFliar - SQL 2";
				$this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
				$this->frmError["MensajeError"]=$this->error."<br>".$this->mensajeDeError;
				$this->ban=1;
				$dbconn->RollBackTrans();
			...
			else
			{
				...
				if ($dbconn->ErrorNo() != 0)
				{
					$this->error = "Error en el Modulo app_GestionSeguimientoPFliar - InsertarSeguimientoPFliar - SQL 3";
					$this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
					$this->frmError["MensajeError"]=$this->error."<br>".$this->mensajeDeError;
					$this->ban=1;
					$dbconn->RollBackTrans();
		...
		$this->ban=1;
		$this->FrmSeguimientoPFliar($_REQUEST['seguimiento'],$_REQUEST['opcion'],$_REQUEST['ordenar']);
	...
	function GetDatosSeguimiento($evolucion,$inscripcion)
	{
		...
		$sql="SELECT	a.pyp_plan_fliar_seguimiento_id,
									a.paciente_id,
									a.tipo_id_paciente,
									a.cita_asignada_id,
									a.observacion,
					...
					FROM pyp_plan_fliar_seguimiento AS a
		...
		if ($dbconn->ErrorNo() != 0)
		{
			$this->error = "Error en el Modulo GestionSeguimientoPFliar - GetDatosSeguimiento - SQL";
			$this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
			$this->frmError["MensajeError"]=$this->error."<br>".$this->mensajeDeError;
			$this->ban=1;
			$dbconn->RollBackTrans();
	...
	function GetCitasID($paciente_id,$tipo_id_paciente,$cita_padre,$fecha_turno)
	{
		...
		if(!empty($con))
		{
			...
			";
				...
				if ($dbconn->ErrorNo() != 0)
				{
					$this->error = "Error en el Modulo app_GestionSeguimientoPFliar - GetCitasID - SQL";
					$this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
					$this->frmError["MensajeError"]=$this->error."<br>".$this->mensajeDeError;
					$this->ban=1;
					$dbconn->RollBackTrans();
	...
	function MonitoreoPacientes($ordenar,$monitoreo)
	{
		...
		$sqlCont="
						...
						JOIN pyp_inscripcion_planificacion_fliar AS c 
						ON
						(
						...
						LEFT JOIN pyp_plan_fliar_datos_evolucion as e
						ON
						(
						...
						LEFT JOIN pyp_plan_fliar_seguimiento AS i
						ON
						(
						...
						(
							...
							AND ef.tipo_consulta_id=".$_SESSION['SeguimientoPFliar']['tc_id']."
						)
						$where";
		...
		$query="
						...
						JOIN pyp_plan_fliar_datos_evolucion as e
						ON
						(
							d.inscripcion_id=e.inscripcion_id 
							AND d.evolucion_id=e.evolucion_id
		...
		if ($dbconn->ErrorNo() != 0) 
		{
			$this->error = "Error en el Modulo GestionSeguimientoPFliar - MonitoreoPacientes - SQL";
			$this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
			$this->frmError["MensajeError"]=$this->error."<br>".$this->mensajeDeError;
			$this->ban=1;
			return false;
	...
	function ProcesarSqlConteo($sqlCont)
	{
		...
		if ($dbconn->ErrorNo() != 0) 
		{
			$this->error = "Error en el Modulo GestionSeguimientoPFliar - ProcesarSqlConteo - SQL";
			$this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
			$this->frmError["MensajeError"]=$this->error."<br>".$this->mensajeDeError;
			$this->ban=1;
			return false;
