<?php
    ...
    function Registro_Accidente($vector,$tipo_id_tercero,$tercero_id)
    {
        $objResponse = new xajaxResponse();
        //var_dump($vector);
        //var_dump($tipo_id_tercero);
        //var_dump($tercero_id);
        ...
        if($vector["fecha_acc"]=="")
        {   $salida="FATA LLENAR EL CAMPO DE FECHA DEL ACCIDENTE";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
        }
        
        if($vector["hora"]=="-1")
        {   $salida="FATA COLOCAR LA HORA DEL ACCIDENTE";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
        }
        ...
        if($vector["jornada"]=="")
        {   $salida="FALTA SELECCIONAR LA JORNADA EN QUE OCURRIO EL ACCIDENTE";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
        }

        if($vector["trabajo_habitual"]=="")
        {   $salida="FALTA SELECCIONAR SI EL TRABAJADOR SE ENCONTRABA REALIZANDO SU TRABAJO HABITUAL";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
        }


        if($vector["trabajo_habitual"]=="0"  && $vector["descripcion_trabajo_no_habitual"]=="")
        {   $salida="DEBIDO A QUE EL TRABAJADOR NO SE ENCONTRABA REALIZANDO SU TRABAJO HABITUAL FALTA DESCRIBIR CUAL ERA ESE TIPO DE TRABAJO";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
        }
        if($vector["hpa"]=="-1")
        {   $salida="FALTA SELECCIONAR LA CANTIDAD DE TIEMPO(HH:MM) QUE EL TRABAJADOR LLEVABA PREVIO AL ACCIDENTE";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
        }
        if($vector["mpa"]=="-1")
        ...
        if($vector["tipo_accidente"]=="-1")
        {   $salida="FALTA SELECCIONAR EL TIPO DE ACCIDENTE";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
        }
        if($vector["depto"]=="-1")
        {   $salida="FALTA SELECCIONAR EL DEPARTAMENTO DEL PAIS DONDE OCURRIO EL ACCIDENTE";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
        }
        if($vector["ciudades"]=="-1")
        {   $salida="FALTA SELECCIONAR EL MUNICIPIO DONDE OCURRIO EL ACCIDENTE";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
        }
        if($vector["zona_acc"]=="")
        {   $salida="FALTA SELECCIONAR SI EL ACCIDENTE OCURRIO EN UNA ZONA URBANA O RURAL";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
        }
        if($vector["lugar_acc"]=="")
        {   $salida="FALTA SELECCIONAR SI EL ACCIDENTE OCURRIO DENTRO O FUERA DE LA EMPRESA";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
        }
        
        if($vector["sitio_accidente"]=="-1")
        {   $salida="FATA INDICAR EL SITIO DEL ACCIDENTE";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
        }
        if(EMPTY($vector["lesiones"]))
        {   $salida="SE DEBE SELECCIONAR QUE TIPO DE LESION SUFRIO EL TRABADOR";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
        }

        if(EMPTY($vector["partes_cuerpo"]))
        {   $salida="SE DEBE SELECCIONAR CUALES FUERON LAS PARTES DEL CUERPO APARENTEMENTE AFECTADAS DEL TRABADOR POR EL ACCIDENTE";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
        }

        if(EMPTY($vector["Agentes_accidente"]))
        {   $salida="SE DEBE SELECCIONAR AL MENOS UN AGENTE DEL ACCIDENTE (CON QUE SE LESIONO EL TRABAJADOR)";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
        }

        if(EMPTY($vector["formas_accidente"]))
        {   $salida="SE DEBE SELECCIONAR AL MENOS UN MECANISMOS O FORMAS DE ACCIDENTE (CON QUE SE LESIONO EL TRABAJADOR)";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
        }

        if($vector["detalle_accidente"]=="")
        {   $salida="FALTA LLENAR EL RECUADRO DE DETALLE O INFORMACION DEL ACCIDENTE";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
        }

        if($vector["espacios"]=="-1")
        {   $salida="SE DEBE SELECCIONAR EL TIPO DE ESPACIO EN EL QUE OCURRIO EL ACCIDENTE";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
        }

        if(EMPTY($vector["agentes"]))
        {   $salida="SE DEBE SELECCIONAR AL MENOS UN AGENTE DE RIESGO AL CUAL EL TRABAJADOR ESTUVO EXPUESTO";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
        }

        if(EMPTY($vector["personas_presentes"]))
        {   $salida="SE DEBE SELECCIONAR SI HABIAN PERSONAS QUE PRESENCIARON EL ACCIDENTE";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
        }


        if($vector["personas_presentes"]=="1" && EMPTY($vector["tipo_id_ter"]))
        {   $salida="SE DEBEN REGISTRAR LOS DATOS DE AL MENTOS UNA PERSONA";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
        }
        elseif(!EMPTY($vector["tipo_id_ter"]) && $vector["personas_presentes"]=="1")
        {
            for($i=1;$i<=count($vector["tipo_id_ter"]);$i++)
            {    $Y=$i;
                if($vector["tipo_id_ter"][$i]=="-1" && $i==1)
                {
                   
                    $salida="SE DEBE SELECCIONAR EL TIPO DE IDENTIFICACION DE LA PERSONA ".($Y)."";
                    $objResponse->assign("resultado","innerHTML",$salida);
                    return $objResponse;
                }
            
                if($vector["tipo_id_ter"][$i]!="-1" && $vector["id_ter"][$i]=="")
                {

                    $salida="SE DEBE INGRESAR LA IDENTIFICACION DE LA PERSONA ".$Y."";
                    $objResponse->assign("resultado","innerHTML",$salida);
                    return $objResponse;
                }

                if($vector["tipo_id_ter"][$i]!="-1" && $vector["nom_ter"][$i]=="")
                {

                    $salida="SE DEBE INGRESAR EL NOMBRE DE LA PERSONA ".$Y."";
                    $objResponse->assign("resultado","innerHTML",$salida);
                    return $objResponse;
                }
     ...
     $usuario=UserGetUID();
     $agentes_por_espacio=$registrar->RegistarAccidenteLaboral($vector,$tipo_id_tercero,$tercero_id,$usuario);
     //var_dump($agentes_por_espacio);
    if($agentes_por_espacio===true)
    {
            $salida="ACCIDENTE DE TRABAJO REGISTRADO SATISFACTORIAMENTE";
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;
    }
    else
    {
            $salida="PUTA VIDA PROBLEMAS".$registrar->error.$registrar->mensajeDeError;
            $objResponse->assign("resultado","innerHTML",$salida);
            return $objResponse;

    }
    
    // var_dump($registrar->error);
    // var_dump($registrar->mensajeDeError);
    ...
    function PintarAgentesdeRiesgoPorEspacio($espacio)
    {
        $path = SessionGetVar("rutaImagenes");
        $objResponse=new xajaxResponse();
        $registrar = AutoCarga::factory("AccidentesdeTrabajoLogica", "", "app","UV_AccidentesdeTrabajo");

        $agentes_por_espacio=$registrar->Obteneragentes_de_riesgo_por_espacio($espacio);
    if(!empty($agentes_por_espacio))
    {
        ...
        $salida .= "                     </td>\n";
        $salida .= "                     <td class=\"\">\n";
        $salida .= "                      AGENTE RIESGO\n";
        $salida .= "                     </td>\n";
        $salida .= "                     <td class=\"\">\n";
        $salida .= "                      &nbsp;\n";
        $salida .= "                     </td>\n";
        $salida .= "                   </tr>\n";
        for($i=0;$i<count($agentes_por_espacio);$i++)
        {
            $salida .= "                   <tr class=\"modulo_list_oscuro\">\n";
            $salida .= "                     <td>\n";
            $salida .= "                       ".$agentes_por_espacio[$i]['tipo_descripcion']."";
            $salida .= "                       </td>\n";
            $salida .= "                     <td bgcolor='".$agentes_por_espacio[$i]['color']."'>\n";
            $salida .= "                       \n";
            $salida .= "                       </td>\n";
            $salida .= "                     <td>\n";
            $salida .= "                       ".$agentes_por_espacio[$i]['agente_descripcion']."\n";
            $salida .= "                       </td>\n";
            $salida .= "                     <td>\n";
            //$salida .= "                       ".$agentes_por_espacio[$i]['agente_descripcion']."\n";
            $salida .= "                          <input type=\"checkbox\" name=\"agentes[".$agentes_por_espacio[$i]['agente_riesgo_id']."]\" value=\"".$agentes_por_espacio[$i]['tipo_riesgo_id']."\">";
            $salida .= "                       </td>\n";
            $salida .= "                   </tr>\n";
        }
        $salida .= "                 </table>\n";
    ...
    {
        $salida .= " <label CLASS='label_error'>ESTE ESPACIO NO TIENE AGENTES DE RIESGO ASIGNADOS              </label>\n";

    }
        $objResponse->assign("agentesx_riesgo","innerHTML",$salida);
        return $objResponse;
...
//                 $salida .= "                       </td>\n";
                ...
                if($afiliados[$i]['eps_tipo_afiliado_id']=='C')
                {
                    ...
                    $salida .="                         <a title='CONSULTAR HISTORIAL DE ACCIDENTES' href=\"".$historial."\">";
                    $salida .="                          <sub><img src=\"".$path."/images/show.gif\" border=\"0\" width=\"21\" height=\"21\"></sub>\n";//usuarios.png
                    $salida .="                         </a>\n";
                }
                else
                {
                ...
                $medicos =ModuloGetURL('app','UV_AccidentesdeTrabajo','controller','RegistrarAccidenteTrabajo',array('afiliado_tipo_id'=>$afiliados[$i]['afiliado_tipo_id'],'afiliado_id'=>$afiliados[$i]['afiliado_id']));
                //"javascript:MostrarCapa('ContenedorMed');ExtraerMedicos('".$afiliados[$i]['eps_afiliacion_id']."','".$afiliados[$i]['afiliado_tipo_id']."','".$afiliados[$i]['afiliado_id']."','".$td."');Iniciar3('MEDICOS FAMILIARES');\"";
                $salida .="                         <a title='REGISTRAR ACCIDENTE LABORAL' href=\"".$medicos."\">";
                $salida .="                          <sub><img src=\"".$path."/images/accidente.png\" border=\"0\" width=\"21\" height=\"21\"></sub>\n";//usuarios.png
                $salida .="                         </a>\n";
                $salida .= "                       </td>\n";
                $salida .= "                    </tr>\n";
            } 
