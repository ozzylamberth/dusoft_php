<?php
  ...
  */
  function ValidarPaciente($form)
  {
	...
	if(empty($_SESSION["form_asignar_turno"]))
		$mensaje = "DEBE DE SELECCIONAR UN TURNO";
	
	if(!empty($_SESSION["form_asignar_turno"]))
	{
		$turno;
		
		foreach($_SESSION["form_asignar_turno"] as $k=>$v)
		{
        ...
        $var = $sla->ObtenerValidacionTurno($turno,$plan);
		//$mensaje = "EL TURNO ".$turno." - ".$plan." - ".$var." - ";
		if($var)
		{
			$mensaje = "EL TURNO DE ESTA AGENDA NO SE PUEDE ASIGNAR A ESTE PLAN (AGENDA EXCLUSIVA PARA ".$var.")";
		}
		
	   $var = $sla->ObtenerFecha($turno);
	   if(!$var)
		{
			$mensaje = "DEBE ELEGIR UNA HORA MAYOR A LA ACTUAL.";
		}
	}
          ...
          else
          {
            ...
            else
            {
              ...
              else
              {
				...
				IngresarSesion($form,'form_buscar_paciente');//= array_merge($_SESSION,$form);
	...
	/**
	* Funcion que recive un formulario y lo ingresa a la Session del usuario
	*/
	function IngresarSesion($form_datos,$form_nombre)
	{
		$objResponse = new xajaxResponse();
		if("form_asignar_turno" == $form_nombre)
		{
			unset($_SESSION["form_asignar_turno"]);
			$encontro = false;
			foreach($form_datos as $k=>$v)
			{
				if (substr_count ($k,'seleccion')==1)
				{
					$encontro =true;
         ...
         //$objResponse->script('alert("'.$form_nombre.'")');	
		$_SESSION[$form_nombre] = $form_datos;
		
		$objResponse->call("enviarformvolver");
		return $objResponse;
	}
  /**
  *
  */
  function TiposAfiliados($form)
  {
	...
	// </Nuevo>
	 ...
	 $datosAfiliado = $sla->ObtenerRangoPlanPaciente($form);
	...
	//Nuevo
	 $html = $mdl->FormularioDatosAfiliado($tipos,$rangos,$UltimaCita,$datosAfiliado);
  ...
  *
  */
  function TraerPlanAtencion($form)
  {
	$objResponse = new xajaxResponse();
    $mensaje = "";
	$script="";
    ...
    if($mensaje == "")
    {
      ...
      $datos = $sla->ObtenerPlanAfiliado($form);
      
      if(!empty($datos))
      {
		$mensaje .= "<br>PLAN REGISTRADO: ".$datos['plan_descripcion']." ";
        	$script  = " document.formabuscar.Responsable.value=".$datos['plan_id'].";";
		$script .= " xajax_TiposAfiliados(xajax.getFormValues('formabuscar'));";
      } 
     else
      {
		$datos = $sla->ObtenerPlanAfiliadoUltimaCita($form);
		if(!empty($datos))
		{
			$mensaje .= "<br>PLAN REGISTRADO: ".$datos['plan_descripcion']." ";
			$script  = " document.formabuscar.Responsable.value=".$datos['plan_id'].";";
			...
			$mensaje .= "<br>No se encontro Plan para el paciente.";
		}
      }
    }
  ...
  */
  function ValidarCitasPrioridades($form)
  {
    ...
    $datos = $sla->PrioridadCitasConsulta($form['Documento'],$form['TipoDocumento'],$_SESSION['AsignacionCitas']['cita']);
    ...
    $bandera = 0;
    for($a=$total_cargos-1; $a>=0; $a--)
    { 

	  if($diferencia <= $tiempo_cargos[$a]['tiempo_cargo'])
      {
        $cargos = $tiempo_cargos[$a];
		$bandera = 1;
