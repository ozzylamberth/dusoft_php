<?php
	/**
	* $Id: Debitos.php,v 1.2 2010/03/12 18:41:36 hugo Exp $ 
	...
	*/	
	...
	include "../../../app_modules/FacturacionNotaCreditoAjuste/classes/NotasDebito.class.php";
	include "../../../app_modules/FacturacionNotaCreditoAjuste/classes/NotasDebitoHTML.class.php";
	
	class procesos_admin extends rs_server
	{
		function CrearNotaDebito($param)  
		{
			$html = "";
			...
			$nd = new NotasDebito();
			$rst = $nd->CrearNotaDebito($empresa,$usuario,$param[0],$param[1],$param[2],$tercero,$param[3]);
			if($rst)
			{
				$Notas = $nd->ObtenerNotasPorAnticipos($usuario,$tercero,$empresa);	
				$ndhtml = new NotasDebitoHTML();
				$html .= $ndhtml->CrearListadoNotasCredito($Notas);
			}
			
			return $html."~".$nd->frmError['MensajeError'];
		}
		/*********************************************************************************
		*
		**********************************************************************************/
		function EliminarNotaDebito($param)
		{
			$html = "";
			$nd = new NotasDebito();
			$rst = $nd->EliminarNotaDC($param[0],$param[1]);
			
			if($rst)
			{
				$Notas = $nd->ObtenerNotasPorAnticipos(UserGetUID(),SessionGetVar("DatosTercero"),$_SESSION['NotasAjuste']['empresa'],"D");	
				$ndhtml = new NotasDebitoHTML();
				$html .= $ndhtml->CrearListadoNotasCredito($Notas);
			}
      ...
      $param[4] = str_replace("'","",$param[4]);
			...
			if(str_replace("\\'","",$param[4]) != "undefined")
			{
      ...
      $nat = str_replace("'","",$nat);
			...
			$rst = $nd->AdicionarConceptosDebito($con,$param[2],$nat,$dep,$trid,$trdc,$empresa,$id);
			
			if($rst)
			{
				$ndhtml = new NotasDebitoHTML();
				$conceptos = $nd->ObtenerConceptosAdicionados(SessionGetVar("TmpIdentificador"),$empresa);
				$html = $ndhtml->CrearListaConceptos($conceptos);
			}
			return $html."~".$nd->frmError['MensajeError']."~".$rst;
		}
		/**********************************************************************************
		*
		***********************************************************************************/
		function EliminarConceptos($param)
		{
			...
			$rst = $nd->EliminarConceptosDebito($param[0],$param[1],$id);
		...
		***********************************************************************************/
		function ActualizarInformacion($param)
		{
			...
			$nd = new NotasDebito();			
			$rst = $nd->ActualizarInformacion($id,$empresa,$param[0]);

			return $nd->frmError['MensajeError'];
		}
		/*********************************************************************************
		*
		**********************************************************************************/
		function CerrarNotaDebito($param)
		{
			$nd = new NotasDebito();
			$empresa = $_SESSION['NotasAjuste']['empresa'];
			
			$doc = ModuloGetVar('app','FacturacionNotaCreditoAjuste','notadebito_'.$empresa);
			$rst = $nd->CerrarNotaDebito($param[0],$empresa,$doc);
			...
			$nota = explode("~",$nd->frmError['MensajeError']);
			SessionSetVar("PrefijoNotaDebito",$nota[1]);
			SessionSetVar("NumeroNotaDebito",$nota[2]);
			...
			$mensaje .= "				".$nota[0]."<br>SE GENERO LA NOTA DEBITO Nº ".$nota[1]." ".$nota[2]."\n";;
			$mensaje .= "			</td>\n";
			$mensaje .= "	</tr>\n";
			$mensaje .= "	<tr>\n";
			$mensaje .= "			<td align=\"center\">\n";
			$mensaje .= "				<input type=\"button\" class=\"input-submit\" value=\"Imprimir Nota Debito\" onclick=\"$funcion\">\n";
			$mensaje .= "			</td>\n";
			$mensaje .= "	</tr>\n";
			$mensaje .= "</table><br>\n";
		...
		*
		***********************************************************************************/
		function AnularNotaDebito($param)
		{
			$empresa = $_SESSION['NotasAjuste']['empresa'];
			
			$nd = new NotasDebito();
			$rst = $nd->AnularNotaDebito($param[0],$param[1],$empresa,$param[2],$param[3],UserGetUID());
			
			if($rst)
				$html = "SE HA ANULADO";
			else
				$html = "NO SE PUDO ANULAR <br>".$nd->frmError['MensajeError'];
				
			$mensaje .= "<table border=\"0\" width=\"100%\" align=\"center\" >\n";
			$mensaje .= "	<tr>\n";
			$mensaje .= "			<td align=\"center\" class=\"normal_10AN\">\n";
			$mensaje .= "				<br>LA NOTA DEBITO Nº ".$param[0]." ".$param[1]." $html\n";;
			$mensaje .= "			</td>\n";
			$mensaje .= "	</tr>\n";
			$mensaje .= "</table><br>\n";
			return $mensaje; 
		...
		***********************************************************************************/
		function BuscarFacturas($param)
		{
			...
			$facturas = $nd->ObtenerFacturas($param[0],$param[1],$_SESSION['NotasAjuste']['empresa'],SessionGetVar("DatosTercero"),$param[2]);
			...
			{
				$html .= $this->ObtenerPaginado($param[2],$action,$path,$nd,1);
