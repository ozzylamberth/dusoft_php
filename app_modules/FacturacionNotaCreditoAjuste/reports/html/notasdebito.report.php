<?php
	/**
	* $Id: notasdebito.report.php,v 1.1.1.1 2009/09/11 20:36:46 hugo Exp $ 
	* @copyright (C) 2005 IPSOFT - SA (www.ipsoft-sa.com)
	* @package IPSOFT-SIIS
	* 
	*/
	include_once "./app_modules/FacturacionNotaCreditoAjuste/classes/NotasDebito.class.php";
	include_once "./app_modules/FacturacionNotaCreditoAjuste/classes/NotasCredito.class.php";
	class notasdebito_report 
	{ 
		...
		var $g_config = array('media'=> "Legal",
													'margins'=> array('left'=> 0));
		
		//CONSTUCTOR DE LA CLASE - RECIBE EL VECTOR DE DATOS - METODO PRIVADO NO MODIFICAR
	  function notasdebito_report($datos=array())
	  {
			$this->datos=$datos;
			if(!$this->datos['prefijo_nota'])
			{
				$this->datos['prefijo_nota'] = SessionGetVar("PrefijoNotaDebito");
				$this->datos['numero_nota'] = SessionGetVar("NumeroNotaDebito");
			}
		...
		function GetMembrete()
		{
			...
			$titulo .= $this->datos['tipo_tercero_s']." ".$this->datos['id_s']." ";
		...
		function CrearReporte()
	  {
			...
			$Notas = $nd->ObtenerInformacionNotaDebitoCerrada($this->datos['prefijo_nota'],$this->datos['numero_nota'],$this->datos['empresa']);
			$paciente = $nd->ObtenerInformacionCuentas($Notas['prefijo_factura'],$Notas['factura_fiscal'],$this->datos['empresa']);
			$creditos = $nc->ObtenerValoresFactura($Notas['prefijo_factura'],$Notas['factura_fiscal'],$this->datos['empresa']);
			$valores = $nd->ObtenerValorNotaDebito($Notas['prefijo_factura'],$Notas['factura_fiscal'],$this->datos['empresa']);

			$estilo  = " style=\"font-family: sans_serif, Verdana, helvetica, Arial; font-size:10px\"";
			
			$Salida .= "<table border=\"0\" width=\"100%\">\n";
			$Salida .= "	<tr height=\"21\">\n";
			$Salida .= "		<td class=\"label\" width=\"12%\">Nota Debito:</td>\n";			
			...
			$Salida .= "	</tr>\n";
			$Salida .= "	<tr height=\"21\">\n";
			$Salida .= "		<td colspan=\"2\" class=\"label\">Valor Nota Debito:</td>\n";			
			$Salida .= "		<td colspan=\"2\" class=\"label\" style=\"text-indent:10pt\" align=\"right\">$".formatoValor($Notas['valor_nota'])."</td>\n";
			$Salida .= "		<td></td>\n";
			$Salida .= "	</tr>\n";
			
			if(($valores['abono'] - $Notas['valor_nota']) > 0)
			{
				$Salida .= "	<tr height=\"21\">\n";
				$Salida .= "		<td colspan=\"2\" class=\"label\">Otras Notas Debito:</td>\n";			
				$Salida .= "		<td colspan=\"2\" class=\"label\" style=\"text-indent:10pt\" align=\"right\">$".formatoValor($valores['abono'] - $Notas['valor_nota'])."</td>\n";
				$Salida .= "		<td></td>\n";
				$Salida .= "	</tr>\n";
			}			
			...
			if($creditos)
			{
				...
				{
					$Salida .= "	<tr height=\"21\">\n";
					$Salida .= "		<td colspan=\"2\" class=\"label\">Notas Credito:</td>\n";			
					$Salida .= "		<td colspan=\"2\" class=\"label\" style=\"text-indent:10pt\" align=\"right\">$".formatoValor($credito)."</td>\n";
					$Salida .= "		<td></td>\n";
					$Salida .= "	</tr>\n";
				}
			}
			...
			$Conceptos = $nd->ObtenerConceptosNotaDebito($this->datos['prefijo_nota'],$this->datos['numero_nota'],$this->datos['empresa']);
