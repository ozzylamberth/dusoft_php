...
class MovDocE012 extends ConexionBD
{
/***********************
* constructora
*************************/
...
function Listar_TercerosProveedores($TipoIdTercero,$TerceroId,$Descripcion,$Empresa_Id,$offset)
      ...
      SELECT
            ...
            ter.direccion,
            ter.telefono,
            terpro.codigo_proveedor_id,
            tp.pais
      FROM
          terceros ter,
          terceros_proveedores terpro,
          tipo_pais tp
      WHERE
      ...
      --  AND
      --    ter.empresa_id = '".$Empresa_Id."'
          AND
          ter.tipo_id_tercero = terpro.tipo_id_tercero
          AND
          ter.tercero_id = terpro.tercero_id
          AND
          ter.tipo_pais_id = tp.tipo_pais_id
          and
          terpro.codigo_proveedor_id IN 
                                        (
                                        SELECT
                                        fact.codigo_proveedor_id
                                        from
    ...
    /**************************************************************************************
     ...
     function Listar_FacturasProveedor($CodigoProveedorId,$Empresa_Id,$NumeroFactura)
		{
		//$this->debug=true;
      $sql = "
      SELECT
      ...
      WHERE
          ...
          AND
          fact.numero_factura ILIKE '%".$NumeroFactura."%'
          AND
          fact.codigo_proveedor_id = '".$CodigoProveedorId."' ";
