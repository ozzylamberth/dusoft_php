<?php
  /******************************************************************************
  * $Id: doc_Bodegas_T004.class.php,v 1.3 2010/07/07 15:51:31 hugo Exp $
...
//   }
   function SacarProductosTMP($doc_tmp_id,$usuario_id)//
   {
    ...
    function CrearDoc($bodegas_doc_id, $observacion,$centro,$bodega,$Documento)
    {
       $ClassDOC= new BodegasDocumentos($bodegas_doc_id);
       $OBJETO=$ClassDOC->GetOBJ();
       $RETORNO=$OBJETO->NewDocTemporal($observacion,$centro,$bodega,UserGetUID(),$Documento);
    ...
    */
    function ObtenerInformacionTraslado($tmp_doc_id)
    {
      ...
      return $datos;  
		...
		function CantidadesEntregadas($Producto)
		{

		//$this->debug=true;
		$sql  = " UPDATE inv_bodegas_movimiento_d ";
		$sql .= " SET 
					cantidad_recibida = ".$Producto['total']." ";
		$sql .= " Where ";
		$sql .= " movimiento_id = ".$Producto['item_id_compras']." ";
	...
	function Cantidades_ProductosTMP($doc_tmp_id)
    {
      ...
      $sql .= "and    usuario_id = ".UserGetUID()." ";
      $sql .= " GROUP BY codigo_producto,item_id_compras " ;
	 if(!$resultado = $this->ConexionBaseDatos($sql))
                return $this->frmError['MensajeError'];
	...
	function Cantidad_ProductoTemporal($doc_tmp_id,$codigo_producto,$item_id_compras)
    {
      ...
      $sql .= "and    usuario_id = ".UserGetUID()." ";
      $sql .= "and    codigo_producto = '".$codigo_producto."' ";
      $sql .= "and    item_id_compras = ".$item_id_compras." ";
    ...
    */
		function Consultar_DocumentoDetalle($Formulario)
		{
    	//$this->debug=true;
		...
		if($Formulario['descripcion']!="" || $Formulario['codigo_barras']!="")
        {
    		...
    		$sql = "SELECT fc_descripcion_producto_alterno(invp.codigo_producto) as producto,
					(ord.cantidad-ord.cantidad_recibida) as cantidad_sol,
				ord.*
                FROM 
                inv_bodegas_movimiento_d ord,
                inventarios_productos invp ";
    		$sql .= " where ";
    		$sql .= "           ord.empresa_id = '".$Formulario['empresa_id']."'  ";
    		$sql .= "      and	ord.prefijo = '".$Formulario['prefijo']."'  ";
    		$sql .= "      and	ord.numero = ".$Formulario['numero']."  ";
    		$sql .= "      and  ord.codigo_producto = invp.codigo_producto ";
    		$sql .= "      and  invp.descripcion ILIKE '%".$Formulario['descripcion']."%'  ";
    		$sql .= "     ".$filtro;
        $sql .= "      and  ord.cantidad <> ord.cantidad_recibida ";
    	}
		//print_r($sql);
    		if(!$resultado = $this->ConexionBaseDatos($sql))
                return $this->frmError['MensajeError'];
    ...
    */
        ...
        $filtro = " and lote = '".$Formulario['lote']."' ";
			...
			$sql .= "      and  codigo_producto||''||lote NOT IN ( 	select ";
			$sql .= "      										   	codigo_producto||''||lote as producto";
			$sql .= "      										  	from";
			$sql .= "      										  	inv_bodegas_movimiento_tmp_d";
			$sql .= "      										  	where";
			$sql .= "      										  		doc_tmp_id=".$Formulario['doc_tmp_id']." ";
			$sql .= "      										  	and	usuario_id=".UserGetUID()." ";
			$sql .= "      										 ) ";
