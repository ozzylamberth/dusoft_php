<?php
	/**************************************************************************************  
	* $Id: doc_Bodegas_E002_HTML.class.php,v 1.1 2009/07/17 19:08:17 johanna Exp $ 
	* 
...
include "app_modules/Inv_MovimientosBodegas/Doc_Mov_Bodegas/E002/doc_Bodegas_E002.class.php";
...
class doc_bodegas_E002_HTML
{
 function doc_bodegas_E002_HTML(){}
...
{    //var_dump($DATOS);
     ...
     $file ='app_modules/Inv_MovimientosBodegas/Doc_Mov_Bodegas/E002/RemoteXajax/definirBodegas_E002.php';
...
function FormaDocNuevo($DATOS)
{
      ...
      $consulta = new doc_bodegas_E002();
      //$objdoc = $consulta->CrearDoc($bodegas_doc_id);
      $datos = $consulta->TraerDatos($DATOS['bodegas_doc_id']);
      //$salida.="<script>Jaimito();</script>";
      $salida .=$this->Cabecera($datos);
//       $DATOS1=$DATOS['bodega'];
//       $DATOS1['nom_bodega']=$DATOS['nom_bodega'];
//       $DATOS1['utility']=$DATOS['utility'];
//       $DATOS1['accion']=$DATOS['accion'];
//       $DATOS1['bodegas_doc_id']=$DATOS['bodegas_doc_id'];
//       $DATOS1['tipo_doc_bodega_id']=$DATOS['tipo_doc_bodega_id'];
          //$DATOS['accion']='EDITAR';
...
function FormaDocEditar($DATOS)
{
	     ...
	     //SessionSetVar("empresa_id");
      //var_dump($DATOS);
      //echo $DATOS['doc_tmp_id'];
      $salida .= ThemeAbrirTabla("TIPO DE DOCUMENTO -".$DATOS['tipo_doc_bodega_id']);
      $consulta = new doc_bodegas_E002();
    ...
    $tipo_prestamo_id = $tipo_prestamo[0]['tipo_prestamo_id'];
	  ...
	  //print_r($Result);
		...
		*/
      $salida .=$this->ColocarProductos($DATOS['bodegas_doc_id'],$datos,$DATOS['doc_tmp_id'],$tipo_prestamo_id,$CodigoProveedorId,$tercero_id,$tipo_id_tercero,$datox['empresa_id']);
...
***********************************************************************************/   
  ...
  $java = "javascript:MostrarCapa('ContenedorMov1');Bus_ter('1','0','0','0','ContenidoMov1','unocreate');Iniciar2('BUSCAR TERCERO');\"";//
...
function ColocarProductos($bodegas_doc_id,$datos,$tmp_doc_id,$tipo_prestamo,$CodigoProveedorId,$tercero_id,$tipo_id_tercero,$empresa_id)
 {
    $consulta = new MovBodegasSQL();
...
**********************************************************************************/
	...
	*/
	if($tipo_prestamo == '06')
			{
			$salida .= "                 <form name=\"BusquedaProductosFactura\">\n";
            $salida .= "                 <table width=\"95%\" align=\"center\" class=\"modulo_table_list\">\n";         
            $salida .= "                    <tr class=\"modulo_table_list_title\">\n";
            ...
            $salida .= "                   <input type=\"text\" class=\"input-text\" id=\"codigo_prod\" name=\"codigo_prod\" >\n";
            $salida .= "                       </td>\n";
            $salida .= "                       <td width=\"55%\" align=\"left\" id=\"ventanatabla\">\n";
            $salida .= "                          DESCRIPCION";                                                                                                             
            $salida .= "                         <input type=\"text\" class=\"input-text\" id=\"descripcion_prod\" name=\"criterio\"	 >\n";//
            
            $salida .= "                       </td>\n";
			
			$salida .= "                       <td width=\"55%\" align=\"left\" id=\"ventanatabla\">\n";
																																																	//$empresa_id,$centro_utilidad,$bodega,$CodigoProveedorId,$NumeroFactura,$Codigo,$Descripcion,$offset
			$salida .= "                         <input type=\"button\" class=\"modulo_table_list\" value=\"Buscar\" onclick=\"xajax_BuscarProductoFactura('".$datos['empresa_id']."','".$datos['centro_utilidad']."','".$datos['bodega']."','".$CodigoProveedorId."',document.getElementById('num_factura').value,document.getElementById('codigo_prod').value,document.getElementById('descripcion_prod').value,1)\">\n";//
            $salida .= "                       </td>\n";
            ...
            $salida .= "                </form>\n";
            $salida .= "                 <br>\n";
			$salida .= "<div id=\"forma_busprod_factura\"></div>";
      
      $sql = new doc_bodegas_E002();
      $PoliticasVencimiento=$sql->PoliticasVencimiento($tercero_id);
    //  print_r($PoliticasVencimiento);
      ...
      $politicas .= "                     POLITICAS DEL PROVEEDOR";
      $politicas .= "                     </td>";
      $politicas .= "                     </tr>";
      $i=1;
      ...
      $politicas .= "                 </table>\n";         
				...
				if($tipo_prestamo == '07')
						{
						$salida .= "                 <form name=\"BusquedaProductosFacturaDespacho\">\n";
						$salida .= "                 <table width=\"95%\" align=\"center\" class=\"modulo_table_list\">\n";         
						$salida .= "                    <tr class=\"modulo_table_list_title\">\n";
						$salida .= "                       <td COLSPAN='3' align=\"center\">\n";
						$salida .= "                          BUSCADOR DE PRODUCTOS";
						...
						$salida .= "                </form>\n";
						$salida .= "                 <br>\n";
						$salida .= "<div id=\"forma_busprod_facturadespacho\"></div>";
...
***********************************************************************************/   
  ...
  /*
    * Para Digitar el Numero De Factura $tipo_prestamo Proveedor
    */
 if($tipo_prestamo=="06")
	{
  $salida .= "                         <input type=\"text\" class=\"input-text\" id=\"num_factura\" name=\"num_factura\">\n";
  $salida .= "<input type=\"button\" class=\"modulo_table_list\" value=\"Buscar Factura Proveedor\" onclick=\"xajax_RegistrarFacturaProveedor('".$datos['empresa_id']."','".$datos['centro_utilidad']."','".$datos['bodega']."','".$CodigoProveedorId."',document.getElementById('num_factura').value,'".$tmp_doc_id."','".UserGetUID()."');\">\n"; 
  //$salida .= "                         <input type=\"button\" class=\"modulo_table_list\" value=\"Buscar Factura\" onclick=\"".$java."\">\n";   
  $salida .= "<div class=\"label_error\" id=\"mensaje_registro\"></div>";
  
   $doc_temp=$sql->DatosParaEditar($tmp_doc_id,UserGetUID());
      //print_r($doc_temp);
      $salida.="
      <script language=\"javaScript\">
                 xajax_RegistrarFacturaProveedor('".$datos['empresa_id']."','".$datos['centro_utilidad']."','".$datos['bodega']."','".$CodigoProveedorId."','".$doc_temp['factura_proveedor']."','".$tmp_doc_id."','".UserGetUID()."');
                 document.getElementById('num_factura').value='".$doc_temp['factura_proveedor']."';
      </script>";
      }
	else    
			if($tipo_prestamo=="07")
			{
			// Campo para Ejecutar Busquedas, En caso de que Sea el EGRESO MANO A MANO  con un tercero Cliente
			/*
			  * Nuevo desarrollo, donde se utilizará para hacer el Ingreso de Facturas 
			  * Relacionadas con una devolucion de un Cliente.
			  */
  
  $sql = AutoCarga::factory("MovDocE002","classes","app","Inv_MovimientosBodegas");
  
  
  /*
  * Para Los Select que sacan los prefijos con los que se han creado facturas a un proveedor
  */
  $Prefijos=$sql->ListarPrefijosFacturasTercero($tercero_id,$tipo_id_tercero,$empresa_id);
  ...
  $prefijo .="</select>";
			...
			$salida .= "                         <input type=\"text\" class=\"input-text\" id=\"num_factura\" name=\"num_factura\">\n";
			$java1 = "xajax_RegistrarFactura('".$empresa_id."','".$datos['centro_utilidad']."','".$datos['bodega']."','".$tmp_doc_id."',document.getElementById('num_factura').value,document.getElementById('prefijo_').value,'".$tercero_id."','".$tipo_id_tercero."');";
			$java2 = "MostrarCapa('ContenedorBus');Bus_ProductosFacturaDespachos('".$empresa_id."','".$datos['centro_utilidad']."','".$datos['bodega']."','".$CodigoProveedorId."',document.getElementById('num_factura').value,'','','1');Iniciar4('BUSCAR PRODUCTOS DE FACTURA PROVEEDOR');Clear3000();\"";
			$salida .= $prefijo."<input type=\"button\" class=\"modulo_table_list\" value=\"Buscar Factura Despacho\" onclick=\"".$java1."\">\n";   
			$salida .= "<div id=\"mensaje\" class=\"label_error\"></div>";
			}
					 else
					  {
					  $java = "javascript:MostrarCapa('ContenedorBus');Bus_Pro('".$datos['empresa_id']."','".$datos['centro_utilidad']."','".$datos['bodega']."','0','0','1');Iniciar4('BUSCAR PRODUCTO');Clear3000();\"";
...
$salida .= "                       <td align=\"left\" class=\"modulo_table_list_title\">\n";
  ...
  //Nuevo desarrollo: En caso de devolucion de un producto... guardar el lote seleccionado
  $salida .= "                          <input type=\"hidden\" id=\"lote_devuelto\"  value=\"0\">\n";
  $salida .= "                       </td>\n";
//   $salida .= "                       <td  ROWSPAN='3' id='unidad_pro' align=\"left\" class=\"modulo_list_claro\"> \n";
//                                      
//                                   $salida .= "                 <form name=\"ventana_hill2\">\n";
...
//   $salida .= "                       </td>\n";
  //$salida .= "                    </tr>\n"; 
  $salida .= "                    <tr>\n";
  //Aqui van a ir las Politicas de devolucion
  ...
  $salida .= "                          <input type=\"button\" id=\"nuevo\" value=\"SELECCIONAR PRODUCTO\" class=\"input-bottom\" onClick=\"javascript:GuardarProductoTemporal('".$bodegas_doc_id."','".$tmp_doc_id."',document.getElementById('codigo').value,document.getElementById('cantidad').value,'0',document.getElementById('costeno_val').value,'".UserGetUID()."',document.getElementById('fecha_venc').value,document.getElementById('lotec').value,document.getElementById('lote_devuelto').value);\">";
  $salida .= "                       </td>\n"; 
  $salida .= "                    </tr>\n";
  $salida .= "                 </table>";
  $salida .= "                </form>\n";
