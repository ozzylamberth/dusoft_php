/**************************************************************************************
* $Id: definirBodegas_E007.js,v 1.1 2009/07/17 19:08:17 johanna Exp $ 
...
function Sw_Ventanas(tipo_egreso_id)
{
    //alert(tipo_egreso_id);
//document.getElementById('pantalla').style.display = 'block';
//xajax_ventanas_tipo_egreso(tipo_egreso_id);
var str=tipo_egreso_id;

//$pres[$i]['concepto_egreso_id']."-".$pres[$i]['sw_exige_tercero']."-".$pres[$i]['sw_exige_departamento']."-".$pres[$i]['sw_permite_costo_manual'];

var manual=str.split("-",4);

var egreso_id=manual[0];
var tercero=manual[1];
var depart=manual[2];
var manual=manual[3];

if(tercero=='1')
{
 document.getElementById('pide_tercero').value = 1;
 document.getElementById('terceros_docu').style.display = 'block';
}
else
{
 document.getElementById('pide_tercero').value = 0;
 document.getElementById('terceros_docu').style.display = 'none';
}

if(depart=='1')
{
 document.getElementById('pide_depar').value = 1;
 document.getElementById('departamentos').style.display = 'block';
}
else
{
 document.getElementById('pide_depar').value = 0;
 document.getElementById('departamentos').style.display = 'none';
}

if(manual=='1')
{
 document.getElementById('pide_costo').value = 1;
 document.getElementById('manuales').style.display = 'block';
}
else
{
 document.getElementById('pide_costo').value = 0;
 document.getElementById('manuales').style.display = 'none';
}
...
function PonerDptos(centro)
{ //alert(centro);
  xajax_PintarDptos(centro);
}
...
if(document.getElementById('h_sw_manual').value=='1')
{
    document.getElementById('costo_manual').value="";
}
else
{
...
//GrabarDocumento('".$bodegas_doc_id."',document.getElementById('obser').value,document.getElementById('tipox_id').value,document.getElementById('id_tercerox').value,document.getElementById('egre').value,document.getElementById('depar').value,document.getElementById('costo').value);\">";//              
function GrabarDocumento(bodegas_doc_id, observacion, tipo_id_tercero, tercero_id, tipo_egreso, departamento)
  {

    


    var manual=tipo_egreso.split("-",4);
    var egreso_id=manual[0];

    if(egreso_id==0)
    {
        document.getElementById('errorcreartmp').innerHTML="EL DOCUMENTO EXIGE SELECCIONAR UN TIPO DE EGRESO";
        return false;
    }
    else
    {
        document.getElementById('errorcreartmp').innerHTML="";
    }

    if(document.getElementById('pide_tercero').value==1 && tercero_id=='')
    {
      document.getElementById('errorcreartmp').innerHTML="EL DOCUMENTO EXIGE UN TERCERO";
      return false;
    }

    if(document.getElementById('pide_tercero').value==0)
    {
        document.getElementById('errorcreartmp').innerHTML="";
        tipo_id_tercero='';
        tercero_id='';
    ...
    if(document.getElementById('pide_depar').value==1 && departamento==0)
    {
      document.getElementById('errorcreartmp').innerHTML="EL DOCUMENTO EXIGE SELECCIONAR UN DEPARTAMENTO";
      return false;
    }

    if(document.getElementById('pide_depar').value==0)
    {
        document.getElementById('errorcreartmp').innerHTML=""; 
       // departamento='';
    ...
    if(document.getElementById('pide_costo').value==1)
    {
        for(i=0;i<document.unocreate.costo.length;i++)
        { 
            if(document.unocreate.costo[i].checked==true)
            break;
        }
        if(i==document.unocreate.costo.length)
        {
            document.getElementById('errorcreartmp').innerHTML="NO HA SELECCIONADO UN TIPO DE COSTO A UTILIZAR";
            return false;
        }
        else
        {
            var costo=document.unocreate.costo[i].value;
            document.getElementById('errorcreartmp').innerHTML="";
        } 
    ...
    //alert("bod_id"+bodegas_doc_id);
    //alert("obser"+observacion);
    //alert("terceros"+tipo_id_tercero+"-"+tercero_id);
    //alert("tipo"+egreso_id);
    //alert("departa"+departamento);
    //alert("costo"+costo);
    //NewDocTemporal($observacion='', $concepto_egreso_id, $sw_costo_manual=false, $departamento=null, $tipo_id_tercero=null, $tercero_id=null, $usuario_id=null)
  //alert(departamento);
  xajax_GuardarTmpDoc(bodegas_doc_id, observacion, tipo_id_tercero, tercero_id, egreso_id,costo,departamento);
...
function GuardarProductoTemporal2500(doc_bodega_id,doc_tmp_id,codigo_producto,cantidad,porcentaje_gravamen,total_costo,usuario_id,fecha_venc,lotec)
{  /*alert("aa"+codigo_producto);
  alert(doc_bodega_id);
   alert(doc_tmp_id);
   alert("codigo"+codigo_producto);
...
if(document.getElementById('h_sw_manual').value=='1')
{
    document.getElementById('costo_manual').value=costo;
}
else
{
