<?php
/**************************************************************************************
* $Id: definirBodegas_E007.php,v 1.1 2009/07/17 19:08:17 johanna Exp $
...
/**
* Funcion que segun el centro de utilidad muestra sus respectivos departamentos
* @param string $centro
* @return select con la lista de departamentos disponibles del centro de utilidad
**/
function PintarDptos($centro)
{
    $path = SessionGetVar("rutaImagenes");
    $consulta=new MovBodegasSQL();
    $objResponse = new xajaxResponse();
    $centrosx=$consulta->ColocarDptos($centro);
    //var_dump($centrosx);
    /*if(!empty($centrosx))
    {
        $salida .= "                     DEPARTAMENTOS  <select id=\"depar\" name=\"depar\" class=\"select\" onchange=\"\">";//alert(this.value);
        $salida .= "                           <option value=\"0\" SELECTED>SELECCIONAR</option>\n";
        //var_dump($utility);
        for($i=0;$i<count($centrosx);$i++)
        {
            $salida .= "                      <option value=\"".$centrosx[$i]['departamento']."\">".$centrosx[$i]['descripcion']."</option>\n";
        }
        $salida .= "                       </select>\n";
        $objResponse->assign("deparx","innerHTML",$salida);
    }
    else
    {
        $salida .= "        DEPARTAMENTOS  <select disabled id=\"depar\" name=\"depar\" class=\"select\" onchange=\"\">";//alert(this.value);
        $salida .= "                           <option value=\"0\" SELECTED>SELECCIONAR</option>\n";
        $salida .= "                       </select>\n";
        $objResponse->assign("deparx","innerHTML",$salida);
    }*/
     if(!empty($centrosx))
    {
          $salida .= "                     RPPRPDEPARTAMENTOS  <select id=\"depar\" name=\"depar\" class=\"select\" onchange=\"\">";//alert(this.value);
          $salida .= "                           <option value=\"0\" SELECTED>SELECCIONAR</option>\n"; 
          //var_dump($utility);
          for($i=0;$i<count($centrosx);$i++)
          {
          ...
          $salida .= "                       </select>\n";
          //$objResponse->alert("shasjhasjshjahsjhsjh");
          $objResponse->assign("deparx1","innerHTML",$salida);
    }
...
/**
* Funcion para habilitar y deshabilitar los menus de departamentos y terceros segun el sw del tipo de egreso
* @param string tipo_egreso;
* @return sw;
***/
function ventanas_tipo_egreso($tipo_egreso_id)
{
    $path = SessionGetVar("rutaImagenes");
    $objResponse = new xajaxResponse();
    if($tipo_egreso_id)
...
//GuardarTmpDoc(bodegas_doc_id, observacion, tipo_id_tercero, tercero_id, egreso_id,costo,departamento);

function GuardarTmpDoc($bodegas_doc_id, $observacion, $tipo_id_tercero, $tercero_id, $egreso_id, $costo, $departamento)
{
    $objResponse = new xajaxResponse();
    $path = SessionGetVar("rutaImagenes");
    $consulta=new MovBodegasSQL();
    $consulta1=new doc_bodegas_E007();
    
//     $objResponse->alert($bodegas_doc_id);
//     $objResponse->alert($observacion);
//     $objResponse->alert($tipo_id_tercero);
//     $objResponse->alert($tercero_id);
//     $objResponse->alert($egreso_id);
//     $objResponse->alert($costo);
//     $objResponse->alert($departamento);
    $valor=$consulta1->CrearDoc($bodegas_doc_id, $observacion, $tipo_id_tercero, $tercero_id, $egreso_id, $costo, $departamento);
    //var_dump($valor);
