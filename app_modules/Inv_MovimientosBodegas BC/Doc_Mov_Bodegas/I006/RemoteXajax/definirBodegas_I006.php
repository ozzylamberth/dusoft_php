<?php
	/**************************************************************************************
	* $Id: definirBodegas_I006.php,v 1.1 2009/07/17 19:08:20 johanna Exp $ 
	...
	**************************************************************************************/	
	...
	include "../../../app_modules/Inv_MovimientosBodegas/Doc_Mov_Bodegas/I006/doc_Bodegas_I006.class.php";
...
{   $path = SessionGetVar("rutaImagenes");
    ...
    $cons=new doc_bodegas_I006();
    $TmpDevolucion=$consulta->TmpDevoluciones($doc_tmp_id);
    $objResponse = new xajaxResponse();
    
    $ProductosTemporal=$cons->SacarProductosTMP($doc_tmp_id,UserGetUID());
   $DocTemporal_Auxiliar=$cons->GetInfoDocTemporal($bodegas_doc_id,$doc_tmp_id);
   // $DocTemporal=$consulta->DatosParaEditar($doc_tmp_id,UserGetUID());
    
    $productos=$consulta->CrearDocumentoOriginal($bodegas_doc_id,$doc_tmp_id,UserGetUID());
    $ProductosDocumento=$cons->ProductosDocumento($DocTemporal_Auxiliar['empresa_id'],$DocTemporal_Auxiliar['prefijo'],$DocTemporal_Auxiliar['numero'],'','',$bodegas_doc_id,$doc_tmp_id,'1');
    
    //print_r($ProductosDocumento);    
   if(!empty($productos))
    {
...
//             {var_dumP($productos);
                    ...
                    $salida .= "                      <td align=\"center\">\n";
                                                       $direccion="app_modules/Inv_MovimientosBodegas/Doc_Mov_Bodegas/I006/imprimir/imprimir_docI006.php";
        ...
        foreach($ProductosTemporal as $key=>$detalle)
                          {
                          $cons->ModificarCantidadesDocumento($detalle['item_id_compras'],$detalle['cantidad']);
                          }
        $objResponse->assign("tablaoide","innerHTML","");
        $objResponse->assign("ProductosDocumento","innerHTML","");
     }
    /*
        Solo cambia de estado, si se ha recibido la totalidad del documento de prestamo
        */
        if(empty($ProductosDocumento))
        $ActPresta=$consulta->ActPrestamo($TmpDevolucion['prefijo'], $TmpDevolucion['numero'], $TmpDevolucion['tipo_id_tercero'], $TmpDevolucion['tercero_id']);
    return $objResponse;
}
...
function MostrarProductox($bodegas_doc_id,$doc_tmp_id,$usuario_id)
{
 ...
 if(!empty($vector))
 {
   ...
   {
          $SumCosto += $productos['total_costo'];
         ...
         */      
          ...
          $salida .= "                      </td>\n";
          $salida .= "                    </tr>\n";
          /*Para Pintar los Items Seleccionados*/
          $objResponse->script("document.getElementById('check_".$productos['item_id_compras']."').style.backgroundColor='green';");
          //$objResponse->assign("check_".$productos['item_id_compras']."","style.backgroundColor","green");
          
   }
   
   $salida .= "                    <tr class=\"modulo_list_claro\" onclick=\"mOvr(this,'#ffffff'); \" onMouseOver=\"mOvr(this,'#ffffff');\" onMouseOut=\"mOut(this,'#DDDDDD');\" >\n";
   $salida .= "                      <td align=\"right\" colspan=\"5\">\n";
   $salida .= "                          <sub>TOTAL COSTO:</sub>\n";
   $salida .= "                      </td>\n";
   $salida .= "                      <td align=\"right\">".FormatoValor($SumCosto)."\n";
   $salida .= "                      </td>\n";
   $salida .= "                      <td align=\"center\">&nbsp;\n";
   $salida .= "                      </td>\n";
   $salida .= "                    </tr>\n";
...
//       {
   ...
   $salida .= "                    <tr class=\"modulo_list_claro\" onclick=\"mOvr(this,'#ffffff'); \" onMouseOver=\"mOvr(this,'#ffffff');\" onMouseOut=\"mOut(this,'#DDDDDD');\" >\n";
   $salida .= "                      <td align=\"right\" colspan=\"7\">\n";
   $salida .= "                          <sub>TOTAL COSTO:</sub>\n";
   $salida .= "                      </td>\n";
   $salida .= "                      <td align=\"right\">".FormatoValor($SumCosto)."\n";
   $salida .= "                      </td>\n";
...
{                       
   ...
   //echo $tip_bus; 
    if($tip_bus==2)
    {
      $aumento="AND b.codigo_producto ILIKE(UPPER('%".$criterio."%'))";
      $aumento2="";
    }
    elseif($tip_bus==1)
    {
      $aumento="AND b.descripcion ILIKE(UPPER('%".strtoupper($criterio)."%'))";
      $aumento2="";
    }
    elseif($tip_bus==3)
    {
    ...
    elseif($tip_bus==5)
    {
      $aumento2="AND e.molecula_id ILIKE(UPPER('%".$criterio."%'))";
    }
    elseif($tip_bus==6)
    {
...
********************************************************************************/
function GuardarTmpDoc($bodegas_doc_id, $observacion, $documento_devolucion, $fecha_doc_devolucion, $tipo_id_tercero, $tercero_id)
{
    $objResponse = new xajaxResponse();
    //$objResponse->alert("EDITAR");
    $path = SessionGetVar("rutaImagenes");
    $consulta=new MovBodegasSQL();
    $consulta1=new doc_bodegas_I006();
    $valor=$consulta1->CrearDoc($bodegas_doc_id, $observacion, $documento_devolucion, $fecha_doc_devolucion, $tipo_id_tercero, $tercero_id);
    if(!empty($valor))
    {
      $cadena1="&DATOS[doc_tmp_id]=".$valor['doc_tmp_id']."&DATOS[accion]=EDITAR";
      $cadena .="document.volver.action +='".$cadena1."';";
      //$cadena.="alert(document.volver.action);";
    ...
    else
    {
      ...
      $objResponse->alert("PROBLEMAS CON LA CLASE DOCUMENTO");
...
{
    $objResponse = new xajaxResponse();
    $objResponse->alert("DOCUMENTO CREADO");
    $objResponse->call("mar");
    return $objResponse;
}
...
//       $objResponse->alert($Forma);
      ...
      $vector=$consulta->Terceros($pagina,$criterio1,$criterio2,$criterio,$op=1,SessionGetVar("EMPRESA"));
      $salida .= "                  <div id=\"ventana_terceros\">\n";
      ...
      $salida .= "                        <input type=\"button\" class=\"input-submit\" name=\"boton_bus\"  id=\"boton_bus\" value=\"BUSCAR\" onclick=\"Bus_ter(document.getElementById('buscar_x').value,document.getElementById('id').value,document.getElementById('nom_buscar').value,'".$div."','".$Forma."','1')\">\n";
      $salida .= "                       </td>\n";
      $salida .= "                       </tr>\n";
...
//         $salida .="                         <td>\n";
        if($criterio2=="0")
         $salida .="                            <input type=\"text\" class=\"input_text\" name=\"id\" id=\"id\" maxlength=\"40\" size\"40\" value=\"\" onkeypress=\"return acceptm(event)\">";
        else
         $salida .="                            <input type=\"text\" class=\"input_text\" name=\"id\" id=\"id\" maxlength=\"40\" size\"40\" value=\"".$criterio2."\"onkeypress=\"return acceptm(event)\"></td>";
        $salida .="                         </td>\n";
        $salida .= "                       </tr>\n";
      ...
      {
        $salida .= "               <div id=\"erro\" class='label_error' style=\"text-transform: uppercase; text-align:center; font-size:11px;\">\n";
        $salida .= "                No se encontraron resultados con ese tipo de descripciÃ¯Â¿Â½";       
        $salida .= "                </div>\n";       
      }
   if(count($vector)>0) 
   {
      ...
      $salida .= $ctl->ObtenerPaginadoXajax($consulta->conteo,$consulta->paginaActual,$action,"0",5);
...
//       $salida .= "".ObtenerPaginadoter($pagina,$path,$slc,$op,$tip_bus,$criterio,$div,$Forma);
    ...
    */
    function DevolucionprestamoTerce($tipo_id_tercero,$tercero_id,$tipo_doc_general)
    {
        ...
        $consulta=new MovBodegasSQL();
        $cons=new doc_bodegas_I006();
        $devolucion_prestamo=$consulta->DocumentosDePrestamo($tipo_id_tercero,$tercero_id,SessionGetVar("EMPRESA"),'E');
			$salida .= "                          <option value=\"0\" SELECTED>SELECCIONAR</option>\n";
...
function ColocarDias($lapso,$div)
{
  ...
  return $objResponse;
	...
	function ProductosDocumento($empresa_id,$prefijo,$numero,$codigo_barras,$descripcion,$bodegas_doc_id,$doc_tmp_id,$offset)
	{
  $objResponse=new xajaxResponse();
	 $objClass=new doc_bodegas_I006;
	   
    $datos = $objClass->ProductosDocumento($empresa_id,$prefijo,$numero,$codigo_barras,$descripcion,$bodegas_doc_id,$doc_tmp_id,$offset);

		$salida = ProductosDocumento_HTML($datos,$empresa_id,$prefijo,$numero,$codigo_barras,$descripcion,$bodegas_doc_id,$doc_tmp_id,$offset);
		$objResponse->assign("ProductosDocumento","innerHTML",$objResponse->setTildes($salida));
    //Para habilitar/Deshabilitar aquellos que estén en el documento Temporal
 
		return $objResponse;
	}
	
	function ProductosDocumento_HTML($datos,$empresa_id,$prefijo,$numero,$codigo_barras,$descripcion,$bodegas_doc_id,$doc_tmp_id,$offset)
	{
		$objClass=new doc_bodegas_I006;
    ...
    $salida .= "  <tr class=\"modulo_table_list_title\">";
    $salida .= "  <td colspan=\"11\" align=\"center\">PRODUCTOS EN EL DOCUMENTO</td>";
    $salida .= "  </tr>";
    $salida .= "	<tr class=\"modulo_table_list_title\">";
		$salida .= "		<td >CÓDIGO</td>";
		$salida .= "		<td >DESCRIPCIÓN</td>";
		$salida .= "		<td >CANTIDAD</td>";
		$salida .= "		<td >CANTIDAD RECIBIDA</td>";
		//$salida .= "		<td width=\"4%\">V. UNITARIO</td>";
		$salida .= "		<td >LOTE</td>";
		$salida .= "		<td >LOCALIZACION</td>";
		$salida .= "		<td >FECHA VENCIMIENTO</td>";
    ...
    if($datos)
		...
		$salida .= "	<td>".FormatoValor($valor['cantidad'])."</td>";
        $salida .= "	<td>\n";
        $salida .= "    <input style=\"width:100%\" type=\"text\" class=\"input-text\" name=\"ucantidad$i\" id=\"ucantidad$i\" value=\"".FormatoValor($valor['cantidad'])."\" size=\"8\" onkeypress=\"return acceptNum(event);\" onkeyup=\"ValidarCantidad('ucantidad$i',xGetElementById('ucantidad$i').value,'".($valor['cantidad'])."','hell$i');\">\n";
        $salida .= "  </td>";
        $salida .= "	<td>\n";
        $salida .= "    <input type=\"text\" style=\"width:100%\" class=\"input-text\" name=\"lote$i\" id=\"lote$i\" size=\"9\" >\n";
        $salida .= "  </td>";
        $salida .= "	<td>\n";
        $salida .= "    <input type=\"text\" style=\"width:100%\" class=\"input-text\" name=\"localizacion$i\" id=\"localizacion$i\" value=\"\" size=\"10\" >\n";
        $salida .= "  </td>";
        $salida .= "	<td >\n";
        //$salida .= "	  <div id=\"capa_".$valor['item_id']."\" style=\"display:none;width:150;height:40;position:absolute\"></div>\n";
        $salida .= "      <input type=\"hidden\" id=\"cantidad_total$i\" name=\"cantidad_total$i\" value='".($valor['cantidad'])."'>";
        $salida .= "      <input type=\"hidden\" id=\"empresa_id\" name=\"empresa_id\" value='".$empresa_id."'>";
        $salida .= "      <input type=\"hidden\" id=\"prefijo\" name=\"prefijo\" value='".$prefijo."'>";
        $salida .= "      <input type=\"hidden\" id=\"numero\" name=\"numero\" value='".$numero."'>";
        $salida .= "      <input type=\"hidden\" id=\"valor$i\" name=\"valor$i\" value='".($valor['costo'])."'>";
        $salida .= "      <input type=\"hidden\" id=\"porc_iva$i\" name=\"porc_iva$i\" value='".$valor['porc_iva']."'>";
        $salida .= "      <input type=\"hidden\" id=\"porc_iva$i\" name=\"porc_iva$i\" value='".$valor['porc_iva']."'>";
		
        $salida .= "		<table class=\"modulo_table_list\" width=\"100%\">";
		$salida .= "			<tr>";
		$salida .= "				<td>";
		$salida .= "      				<input type=\"text\" readonly=\"true\" style=\"width:100%\" class=\"input-text\" name=\"fecha_vencimiento$i\"id=\"fecha_vencimiento$i\" size=\"10\" >";
		$salida .= "				</td>";
		$salida .= "				<td>";
        ...
        $salida .= "	<td align=\"center\" id=\"check_".$i."\">\n";
        $salida .= "    <input type=\"checkbox\" id=\"".$i."\" name=\"".$i."\" value=\"".$valor['codigo_producto']."\" class=\"input-checkbox\"></td>";
        $salida .= "</tr>";
				$i++;
		}
      
      $salida .= "<tr>";
      $salida .= "<td colspan=\"7\" align=\"center\"><input class=\"input-submit\" type=\"button\" value=\"AGREGAR ITEMS AL DOCUMENTO\" onclick=\"xajax_AgregarItem(xajax.getFormValues(forma_checks));\"></td>";
      $salida .= "</tr>";
    }
		else
		{
  ...
  function AgregarItem($Formulario)
    ...
    for($i=0;$i<=$Formulario['cantidad'];$i++)
    {
      ...
      if($Formulario[$i]!="")
      {
        ...
        $datosItem = $objClass->ConsultarItemsExistencias($Formulario[$i],SessionGetVar('EMPRESA'),SessionGetVar('centro_utilidad'),SessionGetVar('bodega'));
        //Valido que el Producto se encuentre en la Existencia de la empresa
        if(count($datosItem)==0)
        {
          $objResponse->alert("El producto [ ".$Formulario[$i]." ] no se encuentra en bodegas_existencias");
        }
        else
        { //Valido que los Campos no estén vacíos
          if($Formulario['ucantidad'.$i]=="" || $Formulario['lote'.$i]=="" || $Formulario['localizacion'.$i]=="" || $Formulario['fecha_vencimiento'.$i]=="")
          ...
          else
          {
             //Valida que el producto no esté vencido
              if($fecha_dos<=$fecha_uno_act)
              {
                $objResponse->alert("El Producto [ ".$Formulario[$i]." ], Se Encuentra Vencido!!!");
              ...
              else
              {
                ...
                //Ya ha pasado por todas las validaciones, solo queda hacer el Ingreso del Producto al Temporal
                 

                 $total_costo = $Formulario['ucantidad'.$i]*($Formulario['valor'.$i]);
                  ...
                  $cantidad=$objClass->CantidadesPorItem($Formulario['doc_tmp_id'],UserGetUID(),$Formulario[$i]);
                  //if(empty($producto))
                  $alerta ="";
                  
                  //$objResponse->alert($Formulario['codigoproducto'.$i]);
                  //$objResponse->alert($Formulario['cantidad']);
                  $ProductosDocumento = $objClass->ProductosDocumento($Formulario['empresa_id'],$Formulario['prefijo'],$Formulario['numero'],'','',$Formulario['bodegas_doc_id'],$Formulario['doc_tmp_id'],'1',$Formulario[$i]);
                  /*print_r($ProductosDocumento);
                  print_r("<hr>");*/
				  if(($ProductosDocumento[0]['cantidad']-$Formulario['ucantidad'.$i])>=0)
                         {
						 $ItemId=$objClass->AgregarItem($Formulario['doc_tmp_id'],$Formulario[$i],$Formulario['ucantidad'.$i],$total_costo,$Formulario['porc_iva'.$i],$Formulario['bodegas_doc_id'],$Formulario['lote'.$i],$Formulario['fecha_vencimiento'.$i],$Formulario['localizacion'.$i]);
						 }
                  if($ItemId)
					$h++;
				}
		...
		if($h>0)
		{
		/*$objClass->ItemEnMovimiento($Formulario['doc_tmp_id'],$ItemId,$Formulario[$i]);*/
