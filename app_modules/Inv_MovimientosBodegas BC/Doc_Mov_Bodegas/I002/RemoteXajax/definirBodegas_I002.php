<?php
  ...
  function OrdenesCompra($proveedor)
   ...
   //$datos=$objClass->GetProductos($pagina,$orden,$tipo_param,$param);
    ...
    if(count($ProductosFOC)>0)
    { //Para habilitar/Deshabilitar aquellos que estén por Autorizar
    ...
    $salida .= "  </tr>";
    $salida .= "	<tr class=\"modulo_table_list_title\">";
		$salida .= "		<td width=\"7%\">CÓDIGO</td>";
		$salida .= "		<td width=\"15%\">DESCRIPCIÓN</td>";
...
//    $input1 .="";
//    $objResponse->alert($CodigoProducto);		                                                                          $doc_tmp_id,$codigo_producto,                               $cantidad,                                 $total_costo,                                    $porc_iva,              $bodegas_doc_id,                                  $lote,                                                          $fecha_vencimiento,                                                   $localizacion,           $item_id_compras,                 $justificacion,                           $OrdenCompra,                    $ValorUnitarioFactura,                             $ValorUnitarioCompra,                             $offset																																																																																																																																																															//Acá, este será Vacío Siempre, no tomar en cuenta este parametro
 ...
 function Actualizartmp($bodega_doc_id,$tmp,$estado,$tipo_documento)
{
 ...
 /*
 * Funcion para Agregar un producto que está fuera de la Orden de Compra
 *
 */
                          ...
                          //En caso de que la autorizacion que se solicita, viene de fuera de la orden de compra
                    		if($item_id_compras=="")
                            {
                                  $total_costo = $cantidad*($ValorUnitarioFactura+($ValorUnitarioFactura*$porc_iva)/100);
                                    ...
                                    $justificacion .= "Producto Fuera de la Orden de Compra y Además: proximo a vencer!!";
                            }
    ...
    for($i=0;$i<$Formulario['cantidad'];$i++)
    {
      ...
      if($Formulario[$i]!="")
      {
        ...
        { //Valido que los Campos no estén vacíos
          ...
          {
          //Valido que el campo de valor, no estén Negativos
            if($Formulario['valor_unitario_factura'.$i] < 0 || !is_numeric($Formulario['valor_unitario_factura'.$i]))
              $objResponse->assign("mensaje".$Formulario[$i]."","innerHTML","<center><label class=\"label_error\">".$Formulario['valor_unitario_factura'.$i]."Por Favor, Ingresar Valores Válidos</label>");
            else
            {//Valida que el producto no esté vencido
              if($fecha_dos<=$fecha_uno_act)
              {
                ...
                $objResponse->alert("EL PRODUCTO QUE INTENTA INGRESAR ESTÁ VENCIDO");
              }
              else
              {
                $mensaje ="";
                ...
                if($int_nodias < $dias_vencimiento)
                {
                  ...
                  $bandera="1";
                  $objResponse->alert("EL PRODUCTO QUE INTENTA INGRESAR ESTÁ PROXIMO A VENCER");
                }
        ...
        if(empty($Datos))
          $imagen = "folder_vacio.png";
              ...
              $imagen = "folder_lleno.png";
				...
				$total = (((($subtotal+$gravamen)-$retencion_fuente)-$retencion_ica)-$retencion_iva);	
