<?php
	/**************************************************************************************
	* $Id: definirBodegas_I015.php,v 1.1 2009/07/17 19:08:17 mauricio Exp $ 
	...
	**************************************************************************************/	
	...
	include "../../../app_modules/Inv_MovimientosBodegas/Doc_Mov_Bodegas/I015/doc_Bodegas_I015.class.php";
    ...
    if(!empty($productos))
    {
            ...
            $salida .= "                    </tr>\n";
                    ...
                    $salida .= "                      <td align=\"center\">\n";
                                                      $direccion="app_modules/Inv_MovimientosBodegas/Doc_Mov_Bodegas/I015/imprimir/imprimir_docI015.php";
...
function BorrarAjuste($tr,$item)
{
     ...
     // print_r($ProductoTemporal);
      ...
      if($buscar==1)
      {
        ...
        $objResponse->script("xajax_ListadoProductos_Farmacia_d(document.getElementById('bodegas_doc_id').value,document.getElementById('doc_tmp_id').value,document.getElementById('farmacia_id').value,document.getElementById('prefijo').value,document.getElementById('numero').value,document.getElementById('codigo_barras').value,document.getElementById('descripcion').value);");        
...
{                       
    ...
    $ProductosDocumento = $consulta ->ProductosDocumento($farmacia_id,$prefijo,$numero,$codigo_barras,$descripcion,trim(SessionGetVar("EMPRESA")),$tmp_doc_id);
    ...
    /*foreach($Novedades as $key=>$novedad)
      ...
      $salida .= "                    <td>".FormatoValor($producto['total_cantidad'])."</td>";
      ...
      $salida .= "                    <td><input title=\"Cantidad a Ingresar de ".$producto['descripcion']."\" onkeypress=\"return acceptNum(event);\" style=\"width:100%\" type=\"text\" class=\"input-text\" name=\"cantidad".$i."\" id=\"cantidad".$i."\" onkeyup=\"ValidarCantidad('cantidad$i',xGetElementById('cantidad$i').value,'".$producto['total_cantidad']."','hell$i','mensaje".$i."','".$i."');\"></td>";
      /*$salida .= "                    <td width=\"10%\">";
      $salida .= "                        <select style=\"width:100%\" class=\"select\" name=\"Novedad".$i."\" id=\"Novedad".$i."\">";
      $salida .= "                          ".$option;
      $salida .= "                        </select>";
      $salida .= "                    </td>";*/
      $salida .= "                    <td>";

      $salida .= "                    <input type=\"hidden\" name=\"cantidad_enviada".$i."\" id=\"cantidad_enviada".$i."\" value=\"".($producto['cantidad'])."\">";
      $salida .= "                    <input type=\"hidden\" name=\"porc_iva".$i."\" id=\"porc_iva".$i."\" value=\"".($producto['porcentaje_gravamen'])."\">";
      ...
      $salida .= "                    <input type=\"checkbox\" disabled id=\"".$i."\" name=\"".$i."\" value=\"".$producto['movimiento_id']."\" class=\"checkbox\">";
      $salida .= "                    </td>";
      $salida .= "                 </tr>";
      $salida .= "                <tr>";
...
function GuardarTmpDoc($bodegas_doc_id, $observacion, $farmacia_id, $documento_farmacia)
{
    $objResponse = new xajaxResponse();
    $path = SessionGetVar("rutaImagenes");
    $consulta1=new doc_bodegas_I015();
...
function BuscadorFarmacia($div,$Forma)
{
      $objResponse = new xajaxResponse(); 
      $path = SessionGetVar("rutaImagenes");
      $sql = AutoCarga::factory("MovDocI015","classes","app","Inv_MovimientosBodegas");
...
function BuscadorFarmacia_d($empresa_id,$CentroUtilidad,$Bodega,$CodigoEmpresa,$RazonSocial,$offset)
{
      ...
      $salida .= "                    </tr>\n";
        ...
        $salida .= "        <a href=\"#\" onclick=\"Seleccionado('unocreate','".$valor['codigo']."','".$valor['farmacia']."','".trim(SessionGetVar("EMPRESA"))."','".trim($CentroUtilidad)."','".trim($Bodega)."')\">\n";
...
function Listar_DocumentosFarmacia($farmacia_id,$empresa_id,$CentroUtilidad,$Bodega)
{
      ...
      $sql = AutoCarga::factory("MovDocI015","classes","app","Inv_MovimientosBodegas");
      $cons=new doc_bodegas_I015();
      $documentos=$sql->Listar_DocumentosFarmacia(trim($farmacia_id),trim($empresa_id),trim($CentroUtilidad),trim($Bodega));
