<?php
  /******************************************************************************
  * $Id: doc_Bodegas_I015.class.php,v 1.1 2009/07/17 19:08:17 mauricio Exp $
...
class doc_bodegas_I015
{
  ...
  function SacarProductosTMP($doc_tmp_id,$usuario_id)//
   {
    ...
    function CrearDoc($bodegas_doc_id, $observacion, $farmacia_id, $documento_farmacia)
    {
       ...
       $RETORNO=$OBJETO->NewDocTemporal($observacion,$farmacia_id, $documento_farmacia,UserGetUID());
 ...
 $sql="INSERT INTO inv_documento_verificacion_d (
    ...
    function ProductosDocumento($farmacia_id,$prefijo,$numero,$codigo_barras,$descripcion,$empresa_id,$tmp_doc_id)
    {
       if(!empty($codigo_barras))
          $filtro = " b.codigo_barras = '".$codigo_barras."'  ";
       
       $sql="SELECT 
                  a.*,
                  (a.cantidad - a.cantidad_recibida) as total_cantidad,
                  fc_descripcion_producto(a.codigo_producto) as descripcion
                  FROM
                  inv_bodegas_movimiento_d a
                  LEFT JOIN inventarios_productos b ON (a.codigo_producto = b.codigo_producto)
                  AND  (a.cantidad <> a.cantidad_recibida)
              WHERE
                    a.empresa_id='".trim($farmacia_id)."'
              AND   a.prefijo = '".$prefijo."'
              AND   a.numero = '".$numero."' 
              AND   b.descripcion ILIKE '%".$descripcion."%' 
              ".$filtro."
              and   a.movimiento_id NOT IN (
                                            Select
                                            item_id_compras
                                            from
              ...
              ";
              //print_r($sql);
              $sql .= " Order By b.descripcion ASC, a.fecha_vencimiento ASC ";
