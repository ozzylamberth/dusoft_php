...
/**
* app_Mantenimiento_userclasses_HTML.php
*
//*
**/
...
class app_Mantenimiento_userclasses_HTML extends app_Mantenimiento_user
{
	...
	//Función principal que da las opciones para tener acceso a los datos 
...
/*  		$this->salida .= "<td>";
		...
		$temp=sizeof($atributos)-3;
		$this->salida .= "      <tr>";
		$this->salida .= "      <td align=\"center\" colspan=\"$temp\">";
		...
		$this->salida .= "  	</table>";
		//$this->salida .= "  <br>";
		$var=$this->RetornarBarraDatos($tabla);
		if(!empty($var))
		{
			$this->salida .= "  <table border=\"0\" width=\"60%\" align=\"center\">";
			$this->salida .= "  <tr>";
...
/*    if ($this->vercolumna=='y')
		...
		while($i<$ciclo)
		{
   	  ...
   	  $funcion=ModuloGetURL('app','Mantenimiento','user','LlamaFormaVerCamposTabla',array("tablename"=>$planempr[$i]['tablename'],"comentario"=>$planempr[$i]['observaciones']));
			...
			if($planempr[$i]['sw_tipo_mantenimiento'] == 0)
			{
				...
				<img title=\"TABLA SIN MANTENIMIENTO\" src=\"".GetThemePath()."/images/mantenimiento/checkno.png\" border=\"0\"></a>";
			}
			else
        if($planempr[$i]['sw_tipo_mantenimiento'] == 1)
        {
          ...
          array('Of'=>$_REQUEST['Of'],'paso'=>$_REQUEST['paso'],'ctradescri'=>$_REQUEST['ctradescri'],
          'codigoctra'=>$_REQUEST['codigoctra'],'table'=>$planempr[$i]['tablename'],'estado'=>$planempr[$i]['sw_tipo_mantenimiento'])) ."\">
          <img title=\"TABLA CON MANTENIMIENTO\" src=\"".GetThemePath()."/images/mantenimiento/checksi.png\" border=\"0\"></a>";
        }
        else
	...
	{
		unset($_SESSION['mantenimiento']['fk2tmp']);
		unset( $_SESSION['mantenimiento']['seleccion']);
    //variable que almacena las referencias a campos de otra tabla con distinto nombre
		unset($_SESSION['mantenimiento']['refdisnombre1']);
		unset($_SESSION['mantenimiento']['refdisnombre2']);
		unset($_SESSION['mantenimiento']['camposrefarray']);//$_SESSION['mantenimiento']['camposrefarray']=$campos2;
		unset($_SESSION['mantenimiento']['tablaref']);
...
// 				<img title=\"REALIZAR MANTENIMIENTO\" src=\"".GetThemePath()."/images/mantenimiento/mantenimiento.gif\" border=\"0\"></a>";
			$this->salida .= "<a href=\"". ModuloGetURL('app','Mantenimiento','user','LlamaFormaInsertarDatos',
				array('Of'=>'y','tabla'=>$planempr[$i]['tablename'],'schemaname'=>'public')) ."\">
				<img title=\"REALIZAR MANTENIMIENTO\" src=\"".GetThemePath()."/images/mantenimiento/mantenimiento.gif\" border=\"0\"></a>";
			$this->salida .= "</td>";
	    $this->salida .= "</tr>";
			$i++;
	...
	function FormaInsertarDatos($tabla,$atributos,$fk1,$fk2,$camposrefarray,$tablaref,$nombre_campo1,$dato_1,$nombre_campo2,$dato_2,$nombre_campo3,$dato_3,$nombre_campo4,$dato_4,$nombre_campo5,$dato_5,$insertar,$tipos_campos)
  {

/*    print_r($fk1);echo '<BR>';
    print_r($fk2);echo '<BR>';
     echo ENTRO;*/
/*  echo $dato_1.'AAA'.$dato_2.'AAA'.$dato_3.'AAA'.$dato_4.'AAA'.$dato_5."<br><br>";
  echo $nombre_campo1.'AAA'.$nombre_campo2.'AAA'.$dato_3.'AAA'.$dato_4.'AAA'.$dato_5;*/
		  if (empty($fk2)) // and empty($tipos_campos)
				$fk2=$_SESSION['mantenimiento']['fk2tmp'];
      else
				$_SESSION['mantenimiento']['fk2tmp']=$fk2;
      
      if (empty($tipos_campos))
        $tipos_campos=$_SESSION['mantenimiento']['tipos_campos'];
      else
        $_SESSION['mantenimiento']['tipos_campos']=$tipos_campos;
    
    //print_r($atributos);echo '<BR>';

      if (is_array($tabla))
				$tabla=$_SESSION['mantenimiento']['tablainsertar'];

  	 if (empty($tabla))
			//$tabla=$_SESSION['mantenimiento']['tmptabla'];
      $tabla=$_SESSION['mantenimiento']['tablainsertar'];
     else
			$_SESSION['mantenimiento']['tablainsertar']=$tabla;
    //else
  	  //$_SESSION['mantenimiento']['tmptabla']=$tabla;

    $accion=ModuloGetURL('app','Mantenimiento','user','InsertarDatos', array("tabla"=>$tabla,"campos"=>$atributos,"fk2"=>$fk2));
		...
		for($i=0; $i<sizeof($fk2);$i++)
    {
      ...
      for($j=0; $j<sizeof($tipos_campos);$j++)
      {  
       if ($fk2[$i]==$tipos_campos[$j][nombre_campo])  
       { 
        //echo $_SESSION['mantenimiento']['dato_1'].'EEE';
        if (trim($fk2[$i])==trim($nombre_campo1) and empty($_SESSION['mantenimiento']['dato_1']))
        { //echo 'entro1'.$dato_1.$i;
          $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_1."\" size=\"35\" maxlength=\"55\"></td></tr>";
          $_SESSION['mantenimiento']['nombre_campo1']=$nombre_campo1;
          $_SESSION['mantenimiento']['dato_1']=$dato_1;
        }
        else
          if (trim($fk2[$i])==trim($nombre_campo1) and empty($_SESSION['mantenimiento']['dato_2']))
          {//echo 'entro2'.$dato_1;
            $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_1."\" size=\"35\" maxlength=\"55\"></td></tr>";
            $_SESSION['mantenimiento']['nombre_campo2']=$nombre_campo1;
            $_SESSION['mantenimiento']['dato_2']=$dato_1;
          }
          else
            if (trim($fk2[$i])==trim($nombre_campo1) and empty($_SESSION['mantenimiento']['dato_3']))
            {//echo entro3;
              $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_1."\" size=\"35\" maxlength=\"55\"></td></tr>";
              $_SESSION['mantenimiento']['nombre_campo3']=$nombre_campo1;
              $_SESSION['mantenimiento']['dato_3']=$dato_1;
            }
            else
              if (trim($fk2[$i])==trim($nombre_campo1) and empty($_SESSION['mantenimiento']['dato_4']))
              {//echo entro4;
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_1."\" size=\"35\" maxlength=\"55\"></td></tr>";
                $_SESSION['mantenimiento']['nombre_campo4']=$nombre_campo1;
                $_SESSION['mantenimiento']['dato_4']=$dato_1;
              }
            else
              if (trim($fk2[$i])==trim($nombre_campo1) and empty($_SESSION['mantenimiento']['dato_5']))
              {
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_1."\" size=\"35\" maxlength=\"55\"></td></tr>";
                $_SESSION['mantenimiento']['nombre_campo5']=$nombre_campo1;
                $_SESSION['mantenimiento']['dato_5']=$dato_1;
              }
							//AMPLIACIÓN DE VARIABLES PARA LAS FK DE LAS TABLA SELECCIONADA
							else
								if (trim($fk2[$i])==trim($nombre_campo1) and empty($_SESSION['mantenimiento']['dato_6']))
								{
									$this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_1."\" size=\"35\" maxlength=\"55\"></td></tr>";
									$_SESSION['mantenimiento']['nombre_campo6']=$nombre_campo1;
									$_SESSION['mantenimiento']['dato_6']=$dato_1;
								}
							else
								if (trim($fk2[$i])==trim($nombre_campo1) and empty($_SESSION['mantenimiento']['dato_7']))
								{
									$this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_1."\" size=\"35\" maxlength=\"55\"></td></tr>";
									$_SESSION['mantenimiento']['nombre_campo7']=$nombre_campo1;
									$_SESSION['mantenimiento']['dato_7']=$dato_1;
								}
							else
								if (trim($fk2[$i])==trim($nombre_campo1) and empty($_SESSION['mantenimiento']['dato_8']))
								{
									$this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_1."\" size=\"35\" maxlength=\"55\"></td></tr>";
									$_SESSION['mantenimiento']['nombre_campo8']=$nombre_campo1;
									$_SESSION['mantenimiento']['dato_8']=$dato_1;
								}
							else
								if (trim($fk2[$i])==trim($nombre_campo1) and empty($_SESSION['mantenimiento']['dato_9']))
								{
									$this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_1."\" size=\"35\" maxlength=\"55\"></td></tr>";
									$_SESSION['mantenimiento']['nombre_campo9']=$nombre_campo1;
									$_SESSION['mantenimiento']['dato_9']=$dato_1;
								}
							else
								if (trim($fk2[$i])==trim($nombre_campo1) and empty($_SESSION['mantenimiento']['dato_10']))
								{
									$this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_1."\" size=\"35\" maxlength=\"55\"></td></tr>";
									$_SESSION['mantenimiento']['nombre_campo10']=$nombre_campo1;
									$_SESSION['mantenimiento']['dato_10']=$dato_1;
								}
							else
								if (trim($fk2[$i])==trim($nombre_campo1) and empty($_SESSION['mantenimiento']['dato_11']))
								{
									$this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$i."\" value=\"".$dato_1."\" size=\"35\" maxlength=\"55\"></td></tr>";
									$_SESSION['mantenimiento']['nombre_campo11']=$nombre_campo1;
									$_SESSION['mantenimiento']['dato_11']=$dato_1;
								}
							else
								if (trim($fk2[$i])==trim($nombre_campo1) and empty($_SESSION['mantenimiento']['dato_12']))
								{
									$this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_1."\" size=\"35\" maxlength=\"55\"></td></tr>";
									$_SESSION['mantenimiento']['nombre_campo12']=$nombre_campo1;
									$_SESSION['mantenimiento']['dato_12']=$dato_1;
								}

							//FIN AMPLIACIÓN DE VARIABLES PARA LAS FK DE LAS TABLA SELECCIONADA
            else
            if (trim($fk2[$i])==trim($nombre_campo2) and empty($_SESSION['mantenimiento']['dato_2']))
              {//echo entro22;
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_2."\"  size=\"35\" maxlength=\"55\"></td></tr>";
                $_SESSION['mantenimiento']['nombre_campo2']=$nombre_campo2;
                $_SESSION['mantenimiento']['dato_2']=$dato_2;
                //$this->salida.= "<input type=\"text\" name=\"campo2\" size=\"35\" maxlength=\"55\" class=\"input-text\" value=\"".$dato_2."\">";
              }
              else
                if (trim($fk2[$i])==trim($nombre_campo2) and empty($_SESSION['mantenimiento']['dato_3']))
                {//echo entro23;
                  $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_2."\" size=\"35\" maxlength=\"55\"></td></tr>";
                  $_SESSION['mantenimiento']['nombre_campo3']=$nombre_campo2;
                  $_SESSION['mantenimiento']['dato_3']=$dato_2;
                }
                else
                  if (trim($fk2[$i])==trim($nombre_campo2) and empty($_SESSION['mantenimiento']['dato_4']))
                  {//echo entro24;
                    $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_2."\" size=\"35\" maxlength=\"55\"></td></tr>";
                    $_SESSION['mantenimiento']['nombre_campo4']=$nombre_campo2;
                    $_SESSION['mantenimiento']['dato_4']=$dato_2;
                  }
                else
                  if (trim($fk2[$i])==trim($nombre_campo2) and empty($_SESSION['mantenimiento']['dato_5']))
                  {//echo entro25;
                    $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_2."\" size=\"35\" maxlength=\"55\"></td></tr>";
                    $_SESSION['mantenimiento']['nombre_campo5']=$nombre_campo2;
                    $_SESSION['mantenimiento']['dato_5']=$dato_2;
                  }
									//AMPLIACIÓN DE VARIABLES PARA LAS FK DE LAS TABLA SELECCIONADA
									else
										if (trim($fk2[$i])==trim($nombre_campo2) and empty($_SESSION['mantenimiento']['dato_6']))
										{//echo entro25;
											$this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_2."\" size=\"35\" maxlength=\"55\"></td></tr>";
											$_SESSION['mantenimiento']['nombre_campo6']=$nombre_campo2;
											$_SESSION['mantenimiento']['dato_6']=$dato_2;
										}
									else
										if (trim($fk2[$i])==trim($nombre_campo2) and empty($_SESSION['mantenimiento']['dato_7']))
										{//echo entro25;
											$this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_2."\" size=\"35\" maxlength=\"55\"></td></tr>";
											$_SESSION['mantenimiento']['nombre_campo7']=$nombre_campo2;
											$_SESSION['mantenimiento']['dato_7']=$dato_2;
										}
									else
										if (trim($fk2[$i])==trim($nombre_campo2) and empty($_SESSION['mantenimiento']['dato_8']))
										{//echo entro25;
											$this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_2."\" size=\"35\" maxlength=\"55\"></td></tr>";
											$_SESSION['mantenimiento']['nombre_campo8']=$nombre_campo2;
											$_SESSION['mantenimiento']['dato_8']=$dato_2;
										}
									else
										if (trim($fk2[$i])==trim($nombre_campo2) and empty($_SESSION['mantenimiento']['dato_9']))
										{//echo entro25;
											$this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_2."\" size=\"35\" maxlength=\"55\"></td></tr>";
											$_SESSION['mantenimiento']['nombre_campo9']=$nombre_campo2;
											$_SESSION['mantenimiento']['dato_9']=$dato_2;
										}
									else
										if (trim($fk2[$i])==trim($nombre_campo2) and empty($_SESSION['mantenimiento']['dato_10']))
										{//echo entro25;
											$this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_2."\" size=\"35\" maxlength=\"55\"></td></tr>";
											$_SESSION['mantenimiento']['nombre_campo10']=$nombre_campo2;
											$_SESSION['mantenimiento']['dato_10']=$dato_2;
										}
									else
										if (trim($fk2[$i])==trim($nombre_campo2) and empty($_SESSION['mantenimiento']['dato_11']))
										{//echo entro25;
											$this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_2."\" size=\"35\" maxlength=\"55\"></td></tr>";
											$_SESSION['mantenimiento']['nombre_campo11']=$nombre_campo2;
											$_SESSION['mantenimiento']['dato_11']=$dato_2;
										}
									else
										if (trim($fk2[$i])==trim($nombre_campo2) and empty($_SESSION['mantenimiento']['dato_12']))
										{//echo entro25;
											$this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_2."\" size=\"35\" maxlength=\"55\"></td></tr>";
											$_SESSION['mantenimiento']['nombre_campo12']=$nombre_campo2;
											$_SESSION['mantenimiento']['dato_12']=$dato_2;
										}
									//FIN AMPLIACIÓN DE VARIABLES PARA LAS FK DE LAS TABLA SELECCIONADA
					else
            if (trim($fk2[$i])==trim($nombre_campo3) and empty($_SESSION['mantenimiento']['dato_3']))
            {
              $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_3."\" size=\"35\" maxlength=\"55\"></td></tr>";
              $_SESSION['mantenimiento']['nombre_campo3']=$nombre_campo3;
              $_SESSION['mantenimiento']['dato_3']=$dato_3;
            }
            else
              if (trim($fk2[$i])==trim($nombre_campo3) and empty($_SESSION['mantenimiento']['dato_4']))
              {
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_3."\" size=\"35\" maxlength=\"55\"></td></tr>";
                $_SESSION['mantenimiento']['nombre_campo4']=$nombre_campo3;
                $_SESSION['mantenimiento']['dato_4']=$dato_3;
              }
            else
              if (trim($fk2[$i])==trim($nombre_campo3) and empty($_SESSION['mantenimiento']['dato_5']))
              {
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_3."\" size=\"35\" maxlength=\"55\"></td></tr>";
                $_SESSION['mantenimiento']['nombre_campo5']=$nombre_campo3;
                $_SESSION['mantenimiento']['dato_5']=$dato_3;
              }  
							//AMPLIACIÓN DE VARIABLES PARA LAS FK DE LAS TABLA SELECCIONADA
            else
              if (trim($fk2[$i])==trim($nombre_campo3) and empty($_SESSION['mantenimiento']['dato_6']))
              {
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_3."\" size=\"35\" maxlength=\"55\"></td></tr>";
                $_SESSION['mantenimiento']['nombre_campo6']=$nombre_campo3;
                $_SESSION['mantenimiento']['dato_6']=$dato_3;
              }  
            else
              if (trim($fk2[$i])==trim($nombre_campo3) and empty($_SESSION['mantenimiento']['dato_7']))
              {
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_3."\" size=\"35\" maxlength=\"55\"></td></tr>";
                $_SESSION['mantenimiento']['nombre_campo7']=$nombre_campo3;
                $_SESSION['mantenimiento']['dato_7']=$dato_3;
              }  
            else
              if (trim($fk2[$i])==trim($nombre_campo3) and empty($_SESSION['mantenimiento']['dato_8']))
              {
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_3."\" size=\"35\" maxlength=\"55\"></td></tr>";
                $_SESSION['mantenimiento']['nombre_campo8']=$nombre_campo3;
                $_SESSION['mantenimiento']['dato_8']=$dato_3;
              }  
            else
              if (trim($fk2[$i])==trim($nombre_campo3) and empty($_SESSION['mantenimiento']['dato_9']))
              {
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_3."\" size=\"35\" maxlength=\"55\"></td></tr>";
                $_SESSION['mantenimiento']['nombre_campo9']=$nombre_campo3;
                $_SESSION['mantenimiento']['dato_9']=$dato_3;
              }  
            else
              if (trim($fk2[$i])==trim($nombre_campo3) and empty($_SESSION['mantenimiento']['dato_10']))
              {
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_3."\" size=\"35\" maxlength=\"55\"></td></tr>";
                $_SESSION['mantenimiento']['nombre_campo10']=$nombre_campo3;
                $_SESSION['mantenimiento']['dato_10']=$dato_3;
              }  
            else
              if (trim($fk2[$i])==trim($nombre_campo3) and empty($_SESSION['mantenimiento']['dato_11']))
              {
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_3."\" size=\"35\" maxlength=\"55\"></td></tr>";
                $_SESSION['mantenimiento']['nombre_campo11']=$nombre_campo3;
                $_SESSION['mantenimiento']['dato_11']=$dato_3;
              }  
            else
              if (trim($fk2[$i])==trim($nombre_campo3) and empty($_SESSION['mantenimiento']['dato_12']))
              {
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_3."\" size=\"35\" maxlength=\"55\"></td></tr>";
                $_SESSION['mantenimiento']['nombre_campo12']=$nombre_campo3;
                $_SESSION['mantenimiento']['dato_12']=$dato_3;
              }  
							//FIN AMPLIACIÓN DE VARIABLES PARA LAS FK DE LAS TABLA SELECCIONADA
            else
              if (trim($fk2[$i])==trim($nombre_campo4) and empty($_SESSION['mantenimiento']['dato_4']))
              {
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_4."\" size=\"35\" maxlength=\"55\"></td></tr>";
                $_SESSION['mantenimiento']['nombre_campo4']=$nombre_campo4;
                $_SESSION['mantenimiento']['dato_4']=$dato_4;
              }
              else
                if (trim($fk2[$i])==trim($nombre_campo4) and empty($_SESSION['mantenimiento']['dato_5']))
                {
                  $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_4."\" size=\"35\" maxlength=\"55\"></td></tr>";
                  $_SESSION['mantenimiento']['nombre_campo5']=$nombre_campo4;
                  $_SESSION['mantenimiento']['dato_5']=$dato_4;
                }
								//AMPLIACIÓN DE VARIABLES PARA LAS FK DE LAS TABLA SELECCIONADA
              else
                if (trim($fk2[$i])==trim($nombre_campo4) and empty($_SESSION['mantenimiento']['dato_6']))
                {
                  $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_4."\" size=\"35\" maxlength=\"55\"></td></tr>";
                  $_SESSION['mantenimiento']['nombre_campo6']=$nombre_campo4;
                  $_SESSION['mantenimiento']['dato_6']=$dato_4;
                }
	            else
                if (trim($fk2[$i])==trim($nombre_campo4) and empty($_SESSION['mantenimiento']['dato_7']))
                {
                  $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_4."\" size=\"35\" maxlength=\"55\"></td></tr>";
                  $_SESSION['mantenimiento']['nombre_campo7']=$nombre_campo4;
                  $_SESSION['mantenimiento']['dato_7']=$dato_4;
                }
	             else
                if (trim($fk2[$i])==trim($nombre_campo4) and empty($_SESSION['mantenimiento']['dato_8']))
                {
                  $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_4."\" size=\"35\" maxlength=\"55\"></td></tr>";
                  $_SESSION['mantenimiento']['nombre_campo8']=$nombre_campo4;
                  $_SESSION['mantenimiento']['dato_8']=$dato_4;
                }
	             else
                if (trim($fk2[$i])==trim($nombre_campo4) and empty($_SESSION['mantenimiento']['dato_9']))
                {
                  $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_4."\" size=\"35\" maxlength=\"55\"></td></tr>";
                  $_SESSION['mantenimiento']['nombre_campo9']=$nombre_campo4;
                  $_SESSION['mantenimiento']['dato_9']=$dato_4;
                }
	             else
                if (trim($fk2[$i])==trim($nombre_campo4) and empty($_SESSION['mantenimiento']['dato_10']))
                {
                  $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_4."\" size=\"35\" maxlength=\"55\"></td></tr>";
                  $_SESSION['mantenimiento']['nombre_campo10']=$nombre_campo4;
                  $_SESSION['mantenimiento']['dato_10']=$dato_4;
                }
	             else
                if (trim($fk2[$i])==trim($nombre_campo4) and empty($_SESSION['mantenimiento']['dato_11']))
                {
                  $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_4."\" size=\"35\" maxlength=\"55\"></td></tr>";
                  $_SESSION['mantenimiento']['nombre_campo11']=$nombre_campo4;
                  $_SESSION['mantenimiento']['dato_11']=$dato_4;
                }
	             else
                if (trim($fk2[$i])==trim($nombre_campo4) and empty($_SESSION['mantenimiento']['dato_12']))
                {
                  $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"campo1"=>$_POST['campo1']))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_4."\" size=\"35\" maxlength=\"55\"></td></tr>";
                  $_SESSION['mantenimiento']['nombre_campo12']=$nombre_campo4;
                  $_SESSION['mantenimiento']['dato_12']=$dato_4;
                }
								//FIN AMPLIACIÓN DE VARIABLES PARA LAS FK DE LAS TABLA SELECCIONADA
           	  else
                if (trim($fk2[$i])==trim($nombre_campo5) and empty($_SESSION['mantenimiento']['dato_5']))
                {
                  $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_5."\" size=\"35\" maxlength=\"55\"></td></tr>";
                  $_SESSION['mantenimiento']['nombre_campo5']=$nombre_campo5;
                  $_SESSION['mantenimiento']['dato_5']=$dato_5;
                }
								//AMPLIACIÓN DE VARIABLES PARA LAS FK DE LAS TABLA SELECCIONADA
          	   else
                if (trim($fk2[$i])==trim($nombre_campo5) and empty($_SESSION['mantenimiento']['dato_6']))
                {
                  $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_5."\" size=\"35\" maxlength=\"55\"></td></tr>";
                  $_SESSION['mantenimiento']['nombre_campo6']=$nombre_campo5;
                  $_SESSION['mantenimiento']['dato_6']=$dato_5;
                }
          	   else
                if (trim($fk2[$i])==trim($nombre_campo5) and empty($_SESSION['mantenimiento']['dato_7']))
                {
                  $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_5."\" size=\"35\" maxlength=\"55\"></td></tr>";
                  $_SESSION['mantenimiento']['nombre_campo7']=$nombre_campo5;
                  $_SESSION['mantenimiento']['dato_7']=$dato_5;
                }
          	   else
                if (trim($fk2[$i])==trim($nombre_campo5) and empty($_SESSION['mantenimiento']['dato_8']))
                {
                  $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_5."\" size=\"35\" maxlength=\"55\"></td></tr>";
                  $_SESSION['mantenimiento']['nombre_campo8']=$nombre_campo5;
                  $_SESSION['mantenimiento']['dato_8']=$dato_5;
                }
          	   else
                if (trim($fk2[$i])==trim($nombre_campo5) and empty($_SESSION['mantenimiento']['dato_9']))
                {
                  $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_5."\" size=\"35\" maxlength=\"55\"></td></tr>";
                  $_SESSION['mantenimiento']['nombre_campo9']=$nombre_campo5;
                  $_SESSION['mantenimiento']['dato_9']=$dato_5;
                }
          	   else
                if (trim($fk2[$i])==trim($nombre_campo5) and empty($_SESSION['mantenimiento']['dato_10']))
                {
                  $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_5."\" size=\"35\" maxlength=\"55\"></td></tr>";
                  $_SESSION['mantenimiento']['nombre_campo10']=$nombre_campo5;
                  $_SESSION['mantenimiento']['dato_10']=$dato_5;
                }
          	   else
                if (trim($fk2[$i])==trim($nombre_campo5) and empty($_SESSION['mantenimiento']['dato_11']))
                {
                  $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_5."\" size=\"35\" maxlength=\"55\"></td></tr>";
                  $_SESSION['mantenimiento']['nombre_campo11']=$nombre_campo5;
                  $_SESSION['mantenimiento']['dato_11']=$dato_5;
                }
          	   else
                if (trim($fk2[$i])==trim($nombre_campo5) and empty($_SESSION['mantenimiento']['dato_12']))
                {
                  $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$dato_5."\" size=\"35\" maxlength=\"55\"></td></tr>";
                  $_SESSION['mantenimiento']['nombre_camp12']=$nombre_campo5;
                  $_SESSION['mantenimiento']['dato_12']=$dato_5;
                }
								//FIN AMPLIACIÓN DE VARIABLES PARA LAS FK DE LAS TABLA SELECCIONADA
                else
                if (trim($fk2[$i])==$_SESSION['mantenimiento']['nombre_campo1'])
                {
                  $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$_SESSION['mantenimiento']['dato_1']."\" size=\"35\" maxlength=\"55\"></td></tr>";
                  //echo $fk2[$i]."<br><br>";
                }
                else
                  if (trim($fk2[$i])==$_SESSION['mantenimiento']['nombre_campo2'])
                  {
                    $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$_SESSION['mantenimiento']['dato_2']."\" size=\"35\" maxlength=\"55\"></td></tr>";
                    //echo $fk2[$i]."<br><br>";
                  }
                  else
                  if (trim($fk2[$i])==$_SESSION['mantenimiento']['nombre_campo3'])
                      $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$_SESSION['mantenimiento']['dato_3']."\" size=\"35\" maxlength=\"55\"></td></tr>";
                  else
                    if (trim($fk2[$i])==$_SESSION['mantenimiento']['nombre_campo4'])
                      $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$_SESSION['mantenimiento']['dato_4']."\" size=\"35\" maxlength=\"55\"></td></tr>";
                    else
                    if (trim($fk2[$i])==$_SESSION['mantenimiento']['nombre_campo5'])
                        $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$_SESSION['mantenimiento']['dato_5']."\" size=\"35\" maxlength=\"55\"></td></tr>";
										//INICIO AMPLIACIÓN DE VARIABLES PARA LAS FK DE LAS TABLA SELECCIONADA
                    else
                    if (trim($fk2[$i])==$_SESSION['mantenimiento']['nombre_campo6'])
                        $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$_SESSION['mantenimiento']['dato_6']."\" size=\"35\" maxlength=\"55\"></td></tr>";
                    else
                    if (trim($fk2[$i])==$_SESSION['mantenimiento']['nombre_campo7'])
                        $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$_SESSION['mantenimiento']['dato_7']."\" size=\"35\" maxlength=\"55\"></td></tr>";
                    else
                    if (trim($fk2[$i])==$_SESSION['mantenimiento']['nombre_campo8'])
                        $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$_SESSION['mantenimiento']['dato_8']."\" size=\"35\" maxlength=\"55\"></td></tr>";
                    else
                    if (trim($fk2[$i])==$_SESSION['mantenimiento']['nombre_campo9'])
                        $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$_SESSION['mantenimiento']['dato_9']."\" size=\"35\" maxlength=\"55\"></td></tr>";
                    else
                    if (trim($fk2[$i])==$_SESSION['mantenimiento']['nombre_campo10'])
                        $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$_SESSION['mantenimiento']['dato_10']."\" size=\"35\" maxlength=\"55\"></td></tr>";
                    else
                    if (trim($fk2[$i])==$_SESSION['mantenimiento']['nombre_campo11'])
                        $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$_SESSION['mantenimiento']['dato_11']."\" size=\"35\" maxlength=\"55\"></td></tr>";
                    else
                    if (trim($fk2[$i])==$_SESSION['mantenimiento']['nombre_campo12'])
                        $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value=\"".$_SESSION['mantenimiento']['dato_12']."\" size=\"35\" maxlength=\"55\"></td></tr>";
										//FIN AMPLIACIÓN DE VARIABLES PARA LAS FK DE LAS TABLA SELECCIONADA
                    else
                    {
                      $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaInsertarFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" size=\"35\" maxlength=\"55\"></td></tr>";
                    //echo $fk2[$i]."<br><br>";
                    }
        }            
      }//FIN FOR PARA TRAER LOS TIPOS DE DATOS DEL LOS CAMPOS A INSERTAR              
		}

   //SI LA TABLA NO TIENE REFERENCIAS A OTRAS TABLAS
  if(sizeof($fk2)==0) 
  {
    for($i=$i; $i<sizeof($atributos); $i++)
    {
     //CICLO PARA EXTRAER EL TIPO DE DATO PARA CADA CAMPO
     for($k=0; $k<sizeof($tipos_campos);$k++)
     {
        if ($atributos[$i][nombre_campo]==$tipos_campos[$k][nombre_campo])  
        { 
         $this->salida .="<tr><td class=\"".$this->SetStyle("".$atributos[$i]['nombre_campo']."")."\">".$atributos [$i]['nombre_campo'].":</td><td>".$tipos_campos[$k][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$atributos [$i]['nombre_campo']."\" value='".$_REQUEST["campo".$i]."' size=\"35\" maxlength=\"55\"></td></tr>";
        }
     }  
    }    
  }
  else
  {   //print_r($atributos); echo '<BR>'; print_r($fk2);
		$tmp=$i;
		$j=0;
		for($i=$i; $i<sizeof($atributos)+$tmp; $i++)
		{
					$n=0;
					while($n<=sizeof($fk2))
					{
							if ($fk2[$n]!=$atributos[$j]['nombre_campo'])
							{
									...
									if($x==false)
									{
										...
										{  
											...
											{ 
												$this->salida .= "<tr><td class=\"".$this->SetStyle("".$atributos[$j]['nombre_campo']."")."\">".$atributos[$j]['nombre_campo'].": </td><td>".$tipos_campos[$k][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$atributos[$j]['nombre_campo']."\" value='".$_REQUEST["campo".$i]."' size=\"35\" maxlength=\"55\"></td></tr>";
												$n=sizeof($fk2);
											}
										}  
									}
    ...
    $this->salida .= "</form>";
    $actionM=ModuloGetURL('app','Mantenimiento','user','LlamaFormaDatosBrowser', array("table"=>$tabla));
    $this->salida .= "<form name=\"formabuscar\" action=\"$actionM\" method=\"post\">";
    $this->salida .= "<td align=\"center\"><br><input class=\"input-submit\" type=\"submit\" name=\"insertar\" value=\"VOLVER\"><br></td></form>";
    $this->salida .= "</tr>";
  ...
  function FormaEditar($tabla,$datos,$atributos,$fk1,$fk2,$tablaref,$camposrefarray,$dato_1,$nombre_campo1,$dato_2,$nombre_campo2,$dato_3,$nombre_campo3,$dato_4,$nombre_campo4,$dato_5,$nombre_campo5,$dato_6,$nombre_campo6,$dato_7,$nombre_campo7,$dato_8,$nombre_campo8,$dato_9,$nombre_campo9,$dato_10,$nombre_campo10,$dato_11,$nombre_campo11,$dato_12,$nombre_campo12,$tipos_campos,$camposdiferentes)
  {
//  echo '<br><br>ATRIBUTOS'.$tablaref.'TTT'.$nombre_campo1.$dato_1;
// 	print_r($atributos);
    $accion=ModuloGetURL('app','Mantenimiento','user','EditarDatos', array("tabla"=>$tabla,"datos"=>$datos,"atributos"=>$atributos,"fk1"=>$fk1,"fk2"=>$fk2,"tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"dato1"=>$dato_1,"nombre_campo1"=>$nombre_campo1,"dato2"=>$dato_2,"nombre_campo2"=>$nombre_campo2,"dato3"=>$dato_3,"nombre_campo3"=>$nombre_campo3,"dato4"=>$dato_4,"nombre_campo4"=>$nombre_campo4,"dato5"=>$dato_5,"nombre_campo5"=>$nombre_campo5,"dato6"=>$dato_6,"nombre_campo6"=>$nombre_campo6,"dato7"=>$dato_7,"nombre_campo7"=>$nombre_campo7,"dato8"=>$dato_8,"nombre_campo8"=>$nombre_campo8,"dato9"=>$dato_9,"nombre_campo9"=>$nombre_campo9,"dato10"=>$dato_10,"nombre_campo10"=>$nombre_campo10,"dato11"=>$dato_11,"nombre_campo11"=>$nombre_campo11,"dato12"=>$dato_12,"nombre_campo12"=>$nombre_campo12,"p"=>$fk1));
     ...
     {  
      ...
      {   
         ...
         if (trim($fk2[$i])==trim($nombre_campo1))
          {
           $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"camposdiferentes"=>$camposdiferentes))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value='".$dato_1."' size=\"35\" maxlength=\"55\" readonly=\"true\"></td></tr>";
           //echo $atributostmp[$i]['nombre_campo']=$fk2[$i];
          }
         else
          if (trim($fk2[$i])==trim($nombre_campo2))
          {
            $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"camposdiferentes"=>$camposdiferentes))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value='".$dato_2."' size=\"35\" maxlength=\"55\" readonly=\"true\"></td></tr>";
            //echo $atributostmp[$i]['nombre_campo']=$fk2[$i];
          }
          else
          if (trim($fk2[$i])==trim($nombre_campo3))
          {
            $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"camposdiferentes"=>$camposdiferentes))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value='".$dato_3."' size=\"35\" maxlength=\"55\" readonly=\"true\"></td></tr>";
          //echo $atributostmp[$i]['nombre_campo']=$fk2[$i];
          }
          else
            if (trim($fk2[$i])==trim($nombre_campo4))
            {
              $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"camposdiferentes"=>$camposdiferentes))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value='".$dato_4."' size=\"35\" maxlength=\"55\" readonly=\"true\"></td></tr>";
              //echo $atributostmp[$i]['nombre_campo']=$fk2[$i];
            }
            else
              if (trim($fk2[$i])==trim($nombre_campo5))
              {
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"camposdiferentes"=>$camposdiferentes))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value='".$dato_5."' size=\"35\" maxlength=\"55\" readonly=\"true\"></td></tr>";
                //echo $atributostmp[$i]['nombre_campo']=$fk2[$i];
              }
							//MODIFICACIÓN VARIABLES PARA CONSERVAR LAS FK
							else
              if (trim($fk2[$i])==trim($nombre_campo6))
              {
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"camposdiferentes"=>$camposdiferentes))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value='".$dato_6."' size=\"35\" maxlength=\"55\" readonly=\"true\"></td></tr>";
                //echo $atributostmp[$i]['nombre_campo']=$fk2[$i];
              }
							else
              if (trim($fk2[$i])==trim($nombre_campo7))
              {
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"camposdiferentes"=>$camposdiferentes))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value='".$dato_7."' size=\"35\" maxlength=\"55\" readonly=\"true\"></td></tr>";
                //echo $atributostmp[$i]['nombre_campo']=$fk2[$i];
              }
							else
              if (trim($fk2[$i])==trim($nombre_campo8))
              {
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"camposdiferentes"=>$camposdiferentes))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value='".$dato_8."' size=\"35\" maxlength=\"55\" readonly=\"true\"></td></tr>";
                //echo $atributostmp[$i]['nombre_campo']=$fk2[$i];
              }
							else
              if (trim($fk2[$i])==trim($nombre_campo9))
              {
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"camposdiferentes"=>$camposdiferentes))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value='".$dato_9."' size=\"35\" maxlength=\"55\" readonly=\"true\"></td></tr>";
                //echo $atributostmp[$i]['nombre_campo']=$fk2[$i];
              }
							else
              if (trim($fk2[$i])==trim($nombre_campo10))
              {
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"camposdiferentes"=>$camposdiferentes))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value='".$dato_10."' size=\"35\" maxlength=\"55\" readonly=\"true\"></td></tr>";
                //echo $atributostmp[$i]['nombre_campo']=$fk2[$i];
              }
							else
              if (trim($fk2[$i])==trim($nombre_campo11))
              {
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"camposdiferentes"=>$camposdiferentes))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value='".$dato_11."' size=\"35\" maxlength=\"55\" readonly=\"true\"></td></tr>";
                //echo $atributostmp[$i]['nombre_campo']=$fk2[$i];
              }
							else
              if (trim($fk2[$i])==trim($nombre_campo12))
              {
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"camposdiferentes"=>$camposdiferentes))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value='".$dato_12."' size=\"35\" maxlength=\"55\" readonly=\"true\"></td></tr>";
                //echo $atributostmp[$i]['nombre_campo']=$fk2[$i];
              }
							//FIN ADICIÓN VARIABLES PARA CONSERVAR LAS FK
              else
              {
                $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i],"camposdiferentes"=>$camposdiferentes))."\">".$fk2[$i]."</a>: </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$fk2[$i]."\" value='".$datos[0][$fk2[$i]]."' size=\"35\" maxlength=\"55\" readonly=\"true\"></td></tr>";
              //echo  $atributostmp[$i]['nombre_campo']=$fk2[$i];
              }
				 $val=true;
			  }// FIN IF ($fk2)>0
/*			else
        if (sizeof($fk2)==0)
        {
          $this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\">".$fk2[$i].": </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$i."\" value='".$datos[0][$fk2[$i]]."' size=\"35\" maxlength=\"55\"></td></tr>";
          //echo  $atributostmp[$i]['nombre_campo']=$fk2[$i];
        }*/
      }//FIN IF DEL TIPOS CAMPOS
     } //FIN FOR  TIPOS CAMPOS  
    }
		$l=$i;
    $m=0;
		for($i=$l; $i<sizeof($atributos)+$l; $i++)
		{
			...
			while($n<sizeof($fk2))
			{
                ...
                for($j=0; $j<sizeof($tipos_campos);$j++)
                {  
                  if ($atributos[$m]['nombre_campo']==$tipos_campos[$j][nombre_campo])  
                  { 
                    if (!empty($datos[0][$atributos[$m]['nombre_campo']]))      
                      $this->salida .= "<tr><td class=\"".$this->SetStyle("".$atributos[$m]['nombre_campo']."")."\">".$atributos[$m]['nombre_campo'].": </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$atributos[$m]['nombre_campo']."\" value='".$datos[0][$atributos[$m]['nombre_campo']]."' size=\"35\" maxlength=\"55\"></td></tr>";
                    else
                      $this->salida .= "<tr><td class=\"".$this->SetStyle("".$atributos[$m]['nombre_campo']."")."\">".$atributos[$m]['nombre_campo'].": </td><td>".$tipos_campos[$j][tipo_campo]."</td><td><input type=\"text\" class=\"input-text\" name=\"campo".$atributos[$m]['nombre_campo']."\" value='".$_REQUEST["campo".$i]."' size=\"35\" maxlength=\"55\"></td></tr>";
                    $n=sizeof($fk2);
                    $atributostmp[$i]['nombre_campo']=$atributos[$m]['nombre_campo'];
                  } //FIN IF TIPOS CAMPOS
                }  //FIN FOR TIPOS CAMPOS 
							}
					}
      ...
      $m++;
		...
		$limite=sizeof($atributostmp);
		//$limite=sizeof($atributos);
    $t=0;
    $i=0;
    while ($i<$limite)
    {
      if ($atributostmp[$t]['nombre_campo']==NULL)
      {
        $t+=1;
      }
      else
      {
        $atributostmp2[$i]['nombre_campo']=$atributostmp[$t]['nombre_campo'];
        $t+=1;
        $i+=1;
      }
		}
		//print_r($atributos); exit;
    unset($_SESSION['mantenimiento']['atributostmp']);
		$_SESSION['mantenimiento']['atributostmp']=$atributostmp2;
    $this->salida .= "<tr><td colspan = 2 align=\"center\" ><table>";
	  			...
	  			if (trim($fk2[$i])==trim($nombre_campo3))
				  	...
				  	if (trim($fk2[$i])==trim($nombre_campo4))
					  	...
					  	if (trim($fk2[$i])==trim($nombre_campo5))
							...
							//INICIO MODI NUEVAS VARIABLES PARA LAS FK
							else
								if (trim($fk2[$i])==trim($nombre_campo6))
									$this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td><input type=\"text\" class=\"input-text\" name=\"campo".$i."\" value='".$dato_6."' size=\"35\" maxlength=\"55\" readonly=\"true\"></td></tr>";
								else
									if (trim($fk2[$i])==trim($nombre_campo7))
										$this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td><input type=\"text\" class=\"input-text\" name=\"campo".$i."\" value='".$dato_7."' size=\"35\" maxlength=\"55\" readonly=\"true\"></td></tr>";
								else
									if (trim($fk2[$i])==trim($nombre_campo8))
										$this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td><input type=\"text\" class=\"input-text\" name=\"campo".$i."\" value='".$dato_8."' size=\"35\" maxlength=\"55\" readonly=\"true\"></td></tr>";
								else
									if (trim($fk2[$i])==trim($nombre_campo9))
										$this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td><input type=\"text\" class=\"input-text\" name=\"campo".$i."\" value='".$dato_9."' size=\"35\" maxlength=\"55\" readonly=\"true\"></td></tr>";
								else
									if (trim($fk2[$i])==trim($nombre_campo10))
										$this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td><input type=\"text\" class=\"input-text\" name=\"campo".$i."\" value='".$dato_10."' size=\"35\" maxlength=\"55\" readonly=\"true\"></td></tr>";
								else
									if (trim($fk2[$i])==trim($nombre_campo11))
										$this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td><input type=\"text\" class=\"input-text\" name=\"campo".$i."\" value='".$dato_11."' size=\"35\" maxlength=\"55\" readonly=\"true\"></td></tr>";
								else
									if (trim($fk2[$i])==trim($nombre_campo12))
										$this->salida .= "<tr><td class=\"".$this->SetStyle("".$fk2[$i]."")."\"><a href=\"".ModuloGetURL('app','Mantenimiento','user','llamaFormaFk',array("tablaref"=>$tablaref,"camposrefarray"=>$camposrefarray,"nombre_campo1"=>$fk2[$i]))."\">".$fk2[$i]."</a>: </td><td><input type=\"text\" class=\"input-text\" name=\"campo".$i."\" value='".$dato_12."' size=\"35\" maxlength=\"55\" readonly=\"true\"></td></tr>";
							//FIN MODI NUEVAS VARIABLES PARA LAS FK
						  else
...
//       }
  ...
  function FormaSecuencias($datos)
  {
	...
	//FormaFk
	function FormaFk($datos,$atributos,$tabla,$nombre_campo1,$nombre_campo2,$nombre_campo3,$nombre_campo4,$nombre_campo5,$nombre_campo6,$nombre_campo7,$nombre_campo8,$nombre_campo9,$nombre_campo10,$nombre_campo11,$nombre_campo12,$camposdiferentes)
	{
		//print_r($datos);
		//echo "<br><br>";
		//print_r($atributos); exit;
    ...
    $this->salida .= "      <td width=\"1%\">SELECCIÓN</td>";
      ...
      $i++;
		...
		$i=0;
		$j=0;
		//CAMBIO DE CAMPOS CUANDO SON DISTINTOS EN LAS TABLAS DE LA FK
		$bool=false;
		if (is_array($camposdiferentes) AND $camposdiferentes)
		{
			//ES SUFICIENTE CON RECORRER SOLO LA MITAD DEL VECTOR
			//SE BUSCAN SOLO LOS CAMPOS DE LA PRIMERA TABLA DE LA REFERENCIA
			for($i=0;$i<sizeof($camposdiferentes)/2;$i++)
			{
				if($camposdiferentes[$i]==$nombre_campo1)
				{
					if(sizeof($camposdiferentes)==4)
						{
							$nombre_campo1tmp=$nombre_campo1;
							$nombre_campo1=$camposdiferentes[$i+2];
							$i=sizeof($camposdiferentes);
						}
					else
						if(sizeof($camposdiferentes)==6)
						{
							$nombre_campo1tmp=$nombre_campo1;
							$nombre_campo1=$camposdiferentes[$i+3];
							$i=sizeof($camposdiferentes);
						}
					else
						if(sizeof($camposdiferentes)==8)
						{
							$nombre_campo1tmp=$nombre_campo1;
							$nombre_campo1=$camposdiferentes[$i+4];
							$i=sizeof($camposdiferentes);
						}
					$bool=true;
				}
		...
		//FIN CAMBIO DE CAMPOS CUANDO SON DISTINTOS EN LAS TABLAS DE LA FK
		$ciclo=sizeof($planempr);
		while($i<$ciclo)
		{
			if($j==0)
      ...
      //$acc=ModuloGetURL('app','Mantenimiento','user','LlamaFormaEditar',array("tabla"=>$tabla,"atributo1"=>$campo,"pk1"=>$planempr[$i][$campo],"nro_pk"=>'1',"atributos"=>$atributos,"fk1"=>$fk1,"fk2"=>$fk2,"fktabla"=>$fktabla,"camposrefarray"=>$camposrefarray));
			...
			//echo $planempr[$i][$nombre_campo1];
			if (empty($nombre_campo2) and empty($nombre_campo3) and empty($nombre_campo4) and empty($nombre_campo5))
				$acc=ModuloGetURL('app','Mantenimiento','user','LlamaFormaEditar',array("dato1"=>$planempr[$i][$nombre_campo1],"nombre_campo1"=>$nombre_campo1tmp));
			else
...
/*		$accion=ModuloGetURL('app','Mantenimiento','user','',
	...
	//FormaInsertarFk
	function FormaInsertarFk($datos,$atributos,$tabla,$nombre_campo1,$nombre_campo2,$nombre_campo3,$nombre_campo4,$nombre_campo5,$nombre_campo1_sel,$camposrefarray,$camposdiferentes)
	{
		//print_r($datos);
		//echo "<br><br>";
		//print_r($camposdiferentes); 
...
//print_r($planempr);
		while($i<$ciclo)
		{
			...
			$this->salida .= "<td align=\"center\" width=\"1%\">";
			if (empty($nombre_campo2) and empty($nombre_campo3) and empty($nombre_campo4) and empty($nombre_campo5))
         $acc=ModuloGetURL('app','Mantenimiento','user','LlamaFormaInsertarDatos',array("dato1"=>$planempr[$i][$nombre_campo1],"nombre_campo1"=>$nombre_campo1tmp));
			else
  			if (empty($nombre_campo3) and empty($nombre_campo4) and empty($nombre_campo5))
...
/*    $this->salida .= "  <table border=\"0\" width=\"90%\" align=\"center\">";
  ...
  //BARRA PARA EL BROWSER DE LOS DATOS
 	function RetornarBarraDatos($tabla)//Barra paginadora de las tablas en mantenimiento
	{
		if($this->limit>=$this->conteo)
		{
			return '';
		...
		//'codigoctra'=>$_REQUEST['codigoctra'],'ctradescri'=>$_REQUEST['ctradescri']));
		$accion=ModuloGetURL('app','Mantenimiento','user','LlamaFormaDatosBrowser',array("table"=>$tabla));
		$barra=$this->CalcularBarra($paso);
		$numpasos=$this->CalcularNumeroPasos($this->conteo);
		$colspan=1;
		$salida .= "<table border='1' class='modulo_table' align='center' cellpadding='4'><tr><td class='label' bgcolor=\"#D3DCE3\">Paginas :</td>";
