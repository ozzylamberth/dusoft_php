<?php
	/**************************************************************************************
	* $Id: definirBodegas_E016.php,v 1.2 2010/07/07 15:40:25 hugo Exp $ 
...
{   $path = SessionGetVar("rutaImagenes");
    ...
    $plan = $sql->ObtenerContratoId($Temporal['empresa_id']);         
    if(!empty($productos))
    {
                $total=0;
    $Precio=0;
...
foreach($Productos as $key => $valor)
            {
            $Precio=$sql->Buscar_PrecioProducto_Lista($plan['plan_id'],$valor['codigo_producto'],$Temporal['empresa_id'],$Temporal['sw_bodegamindefensa'],'0');
            $total += $Precio['precio'];
...
//             {var_dumP($productos);
            ...
            $salida .= "                      <td align=\"center\">\n";
                                     $direccion="app_modules/Inv_MovimientosBodegas/Doc_Mov_Bodegas/E016/imprimir/imprimir_E016.php";
        ...
        $objResponse->script("document.getElementById('boton_adicional').disabled=true;");
...
function GuardarTmpDoc($bodegas_doc_id, $observacion,$orden_requisicion_id)
{
    $objResponse = new xajaxResponse();
    $path = SessionGetVar("rutaImagenes");
    $consulta=new MovBodegasSQL();
   ...
   // print_r($OrdenRequisicion);
    $consulta1=new doc_Bodegas_E016();
    $valor=$consulta1->CrearDoc($bodegas_doc_id, $observacion,$OrdenRequisicion);
    //print_r($valor);
...
function ColocarDias($lapso,$div)
{
 ...
 function GuardarInfoAdicional($Formulario)
  {
    $objResponse = new xajaxResponse();
    $consulta=new doc_Bodegas_E016();
    
   // $consulta->Guardar_InfoAdicional($Formulario);
      
	if($consulta->Guardar_InfoAdicional($Formulario))
	{
		$objResponse->assign('mensaje_adicional','innerHTML',"DATOS GUARDADOS CORRECTAMENTE");
	}
   else
    {
      $objResponse->assign('mensaje_adicional','innerHTML',"ERROR");
    }
