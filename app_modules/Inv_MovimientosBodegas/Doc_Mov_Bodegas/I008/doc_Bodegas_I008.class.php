...
function ProductosDocumento($empresa_id,$prefijo,$numero,$codigo_barras,$descripcion,$bodegas_doc_id,$doc_tmp_id,$codigo_producto,$lote,$fecha_vencimiento,$caja)

  {
     if(!empty($codigo_barras))
       ...
       if(!empty($caja))
          ...
          $filtro .=" AND a.numero_caja = '".trim($caja)."'  ";
	  /*$sql=" SELECT     
           a.codigo_producto,
		   a.lote,
		   a.fecha_vencimiento,
       ...
       /*$sql=" SELECT     
		   ...
		   a.fecha_vencimiento,
           a.porcentaje_gravamen as porc_iva,
         sum (a.total_costo/a.cantidad) as costo,
          ((sum(a.cantidad))- ((COALESCE(sum(a.cantidad_recibida),0)))-(COALESCE(c.cantidad,0))) as cantidad, 
          COALESCE(c.cantidad,0) as cantidad_temporal,
          fc_descripcion_producto(a.codigo_producto) as descripcion
           FROM     
           inv_bodegas_movimiento_d as a
           ...
           WHERE   
            ...
            AND     a.numero = ".trim($numero)."
            AND     b.descripcion ILIKE '%".trim($descripcion)."%'    			
            ".$filtro."  GROUP BY a.codigo_producto,a.lote,a.fecha_vencimiento,a.porcentaje_gravamen, c.cantidad";
			
            //limit ".$this->limit." OFFSET ".$this->offset."";*/
       
       $sql = "select a.*,
               ((a.cantidad_documento) - ((a.cantidad_recibida))-(COALESCE(c.cantidad,0))) as cantidad
                from (
                    ...
                    a.numero,
		    a.numero_caja,
                    sum(a.cantidad) as cantidad_documento,
                    sum(COALESCE(a.cantidad_recibida,0)) as cantidad_recibida,
                    sum(a.total_costo/a.cantidad)/(
                        select count(*) from inv_bodegas_movimiento_d aa where 
						aa.prefijo=a.prefijo and aa.numero = a.numero and aa.empresa_id = a.empresa_id and aa.codigo_producto = a.codigo_producto
						and aa.fecha_vencimiento = a.fecha_vencimiento and aa.lote = a.lote    
                    ) as costo,
                     fc_descripcion_producto(a.codigo_producto) as descripcion
                 FROM inv_bodegas_movimiento_d as a 
                 GROUP BY a.codigo_producto,a.lote,a.fecha_vencimiento,a.porcentaje_gravamen, a.empresa_id, a.prefijo, a.numero,a.numero_caja

                ) as a 
                 JOIN inventarios_productos as b ON (a.codigo_producto = b.codigo_producto) AND (a.cantidad_documento <> COALESCE(a.cantidad_recibida,0)) 
                 JOIN inventarios as d ON (a.codigo_producto = d.codigo_producto) AND (a.empresa_id = d.empresa_id) 
                 LEFT JOIN (
                         SELECT x.codigo_producto, x.lote, x.fecha_vencimiento, x.numero_caja, SUM(x.cantidad)as cantidad  
                     FROM inv_bodegas_movimiento_tmp_d as x 
                     WHERE   doc_tmp_id = ".trim($doc_tmp_id)."
                AND usuario_id = ".UserGetUID()."
                     GROUP BY x.codigo_producto,x.lote,x.fecha_vencimiento ,x.numero_caja
                 ) as c ON (a.codigo_producto = c.codigo_producto) AND (a.lote = c.lote) AND (a.fecha_vencimiento = c.fecha_vencimiento) AND (a.numero_caja = c.numero_caja) 
                  WHERE   
                    a.empresa_id = '".trim($empresa_id)."'
                    AND     a.prefijo = '".trim($prefijo)."'
                    AND     a.numero = ".trim($numero)."
   ...
   function ProductosDocumentoSinCaja($empresa_id,$prefijo,$numero,$codigo_barras,$descripcion,$bodegas_doc_id,$doc_tmp_id,$codigo_producto,$lote,$fecha_vencimiento,$caja)
  {
     if(!empty($codigo_barras))
          $filtro =" AND   b.codigo_barras = '".trim($codigo_barras)."'  ";
       if(!empty($codigo_producto))
               ...
               ((a.cantidad_documento) - ((a.cantidad_recibida))-(COALESCE(c.cantidad,0))) as cantidad
                from (
                    ...
                    sum(a.total_costo/a.cantidad)/(
                     ...
                     fc_descripcion_producto(a.codigo_producto) as descripcion
                 FROM inv_bodegas_movimiento_d as a 
                 GROUP BY a.codigo_producto,a.lote,a.fecha_vencimiento,a.porcentaje_gravamen, a.empresa_id, a.prefijo, a.numero

                ) as a 
                 JOIN inventarios_productos as b ON (a.codigo_producto = b.codigo_producto) AND (a.cantidad_documento <> COALESCE(a.cantidad_recibida,0)) 
                 JOIN inventarios as d ON (a.codigo_producto = d.codigo_producto) AND (a.empresa_id = d.empresa_id) 
                 LEFT JOIN (
                            SELECT x.codigo_producto, x.lote, x.fecha_vencimiento,SUM(x.cantidad)as cantidad  
			    FROM inv_bodegas_movimiento_tmp_d as x 
			    WHERE   doc_tmp_id = ".trim($doc_tmp_id)."
			    AND usuario_id = ".UserGetUID()."
			    GROUP BY x.codigo_producto,x.lote,x.fecha_vencimiento
                 ) as c ON (a.codigo_producto = c.codigo_producto) AND (a.lote = c.lote) AND (a.fecha_vencimiento = c.fecha_vencimiento)
                  WHERE   
                    a.empresa_id = '".trim($empresa_id)."'
                    AND     a.prefijo = '".trim($prefijo)."'
                    AND     a.numero = ".trim($numero)."
    ...
    function TmpInv_Movimientos($doc_tmp_id,$bodegas_doc_id)
  {
     ...
     function GuardarTemporal($bodegas_doc_id,$doc_tmp_id, $codigo_producto, $cantidad, $porcentaje_gravamen, $total_costo, $usuario_id=null,$fecha_venc,$lotec,$caja)
     {
       $ClassDOC= new BodegasDocumentos($bodegas_doc_id);
       $OBJETO=$ClassDOC->GetOBJ();
       $RETORNO=$OBJETO->AddItemDocTemporal($doc_tmp_id,$codigo_producto,$cantidad,$porcentaje_gravamen,$total_costo,$usuario_id=null,$fecha_venc,$lotec,'','','',$caja);
