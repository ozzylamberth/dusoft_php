/**************************************************************************************
* $Id: definirBodegas_E018.js,v 1.1 2009/07/17 19:08:17 mauricio Exp $ 
...
function CrearDocuFinal(bodegas_doc_id,doc_tmp_id,tipo_doc_bodega_id,empresa_id,centro_utilidad,bodega)
{
// alert(bodegas_doc_id);
// alert(doc_tmp_id);
xajax_CrearDocumentoFinalx(bodegas_doc_id,doc_tmp_id,tipo_doc_bodega_id,empresa_id,centro_utilidad,bodega);
...
function GrabarDocumento(bodegas_doc_id, observacion, plan_id,tipo_formula_id,requisicion)
  {
     if(plan_id=='')
    {
      document.getElementById('errorcreartmp').innerHTML="EL DOCUMENTO EXIGE UN PLAN";
      return false;
    }
    //alert(factura);
    if(tipo_formula_id=='')
    {
      document.getElementById('errorcreartmp').innerHTML="EL DOCUMENTO EXIGE UN TIPO DE DISPENSACION";
      return false;
    }

    if(requisicion.replace(/^\s+/g,'').replace(/\s+$/g,'') =="")
    {
      document.getElementById('errorcreartmp').innerHTML="EL DOCUMENTO EXIGE UN NUMERO SOLICITUD";
      return false;
    }

    
  xajax_GuardarTmpDoc(bodegas_doc_id, observacion,plan_id,tipo_formula_id,requisicion);
...
function recogerTeclaBus(evt)
{
  ...
  if(keyCode==13)  //Si se pulsa enter da directamente el resultado
   {                                                         
     xajax_ListadoProductos_Farmacia_d(document.getElementById('bodegas_doc_id').value,document.getElementById('tmp_doc_id').value,document.getElementById('empresa_id').value,document.getElementById('centro_utilidad').value,document.getElementById('bodega').value,document.getElementById('codigo_barras').value,document.getElementById('descripcion').value);
   }
}   
...
***********************************************/
function Bus_Pro(bodegas_doc_id,tmp_doc_id,empresa_id,centro_utilidad,bodega,codigo_barras,descripcion,offset)
{  
    xajax_ListadoProductos_Farmacia_d(bodegas_doc_id,tmp_doc_id,empresa_id,centro_utilidad,bodega,codigo_barras,descripcion,offset);
...
{
xajax_BuscadorFarmacia_d(empresa_id,CentroUtilidad,Bodega,CodigoEmpresa,RazonSocial,offset);
}