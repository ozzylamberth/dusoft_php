/**************************************************************************************
 * $Id: definirBodegas_I007.js,v 1.1 2009/07/17 19:08:20 johanna Exp $ 
...
function CrearDocuFinal(bodegas_doc_id, doc_tmp_id, tipo_doc_bodega_id, tipo_prestamo_id, numero_factura, prefijo, numero)
{
    xajax_CrearDocumentoFinalx(bodegas_doc_id, doc_tmp_id, tipo_doc_bodega_id, tipo_prestamo_id, numero_factura, prefijo, numero);
...
/**************************************************MAUROOOOOO**********************************************/
function GuardarProductoTemporalTraslados(doc_bodega_id, doc_tmp_id, codigo_producto, cantidad, porcentaje_gravamen, total_costo, usuario_id, fecha_venc, lotec, fecha_total, limite)
{
    ...
    //if(cantidad>document.getElementById('cantidad_limite').value)
    if (cant > limit || cant < 0)
    {
        //alert("Cantidad Limite"+limit);
        //alert("Cantidad Digitada"+cant);
        document.getElementById('error_doc').innerHTML = "LA CANTIDAD EXCEDE A LA DEL DOCUMENTO";
        //return false;
    }
    else
    {//sigue
...
{
    var cant = parseInt(cantidad);
    var limit = parseInt(document.getElementById('cantidad_limite').value);
    ...
    if (document.getElementById('tipo_prestamo_id').value == "07")
    {
        //if(cantidad>document.getElementById('cantidad_limite').value)
        if (cant > limit || cant < 0)
        {
 ...
 *
 ******************************************************/
function AsignarPro(codigo_producto, descripcion, descripcion_unidad, fecha, lote, costo, iva)
{

    document.getElementById('op11').value = costo;
    document.getElementById('cantidad').value = "";
    document.getElementById('gravamen').value = iva;
...
function AsignarPro2(codigo_producto, descripcion, descripcion_unidad, fecha_vencimiento, lote, costo, cantidad)
{

    document.getElementById('codigo_pro').innerHTML = "<label class=\"normal_10N\">" + codigo_producto + "</label>";
    document.getElementById('desc_pro').innerHTML = "<label class=\"normal_10N\">" + descripcion + "</label>";
    document.getElementById('unidad_pro').innerHTML = "<label class=\"normal_10N\">" + descripcion_unidad + "</label>";
    document.getElementById('valor_unitario').innerHTML = "<label class=\"normal_10N\">Valor Venta $" + costo + "</label>";
    document.getElementById('lotec').disabled = true;
    document.getElementById('fecha_venc').disabled = true;
    document.getElementById('codigo').value = codigo_producto;
    ...
    document.getElementById('cantidad_limite').value = cantidad;
    document.getElementById('gravamen').value = 0;
    document.getElementById('gravamen').disabled = true;
    //document.getElementById('op11').focus;
    Cerrar('ContenedorBus');
}
/**********************************************
...
/**********************************************
 para buscar un producto pero con factura
 ***********************************************/
function Bus_ProFact(empresa_id, centro_utilidad, bodega, codigo_producto, descripcion, numero_factura, prefijo, numero, usuariotmp, tmp_doc_id, offset)
{
    xajax_BuscarProductoFacturas(empresa_id, centro_utilidad, bodega, codigo_producto, descripcion, numero_factura, prefijo, numero, usuariotmp, tmp_doc_id, offset);
...
function BorrarAjustes(bodegas_doc_id, doc_tmp_id, usuario_id, item, contenedor)
{
    xajax_Borrar(bodegas_doc_id, doc_tmp_id, usuario_id, item, contenedor);
}
 ...
 ************************************************************************************/
function Seleccionado(formulario, tipo_id_tercero, tercero_id, nombre_tercero, tipo_doc_general)
{
    // alert(tipo_doc_general);
    ...
    if (formulario == "unocreate")
    {
        ...
        xajax_DevolucionprestamoTerce(tipo_id_tercero, tercero_id, tipo_doc_general);
...
function Bus_ter(criterio1, criterio2, criterio, div, forma, tipo_doc_general, pagina)
{
    //alert("esto es lo q se va a buscar"+ criterio);
    xajax_Buscadorter(pagina, criterio1, criterio2, criterio, tipo_doc_general, div, forma);
}