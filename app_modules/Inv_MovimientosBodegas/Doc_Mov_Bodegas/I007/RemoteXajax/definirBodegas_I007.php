...
/* * ************************************************************************************
 * $Id: definirBodegas_I007.php,v 1.1 2009/07/17 19:08:20 johanna Exp $ 
...
function CrearDocumentoFinalx($bodegas_doc_id, $doc_tmp_id, $tipo_doc_bodega_id, $tipo_prestamo_id, $numero_factura, $prefijo, $numero) {
	
    $path = SessionGetVar("rutaImagenes");
	$tipoPrestamo = SessionGetVar("tipoPrestado");
    $consulta = new MovBodegasSQL();
    $objResponse = new xajaxResponse();
	$sql = new doc_bodegas_I007();
	
	//SE VALIDA EL TIPO DE PRESTAMO SI ESTA PERMITIDA LA SINCRONIZACION
    $consultaTipoPrestamo = $sql->TraerBodegasTipoPrestamos($tipoPrestamo);
	$tipo_prestamo_id = $consultaTipoPrestamo[0]['sw_sincronizar'];
    $vector = $sql->SacarProductosTMP($doc_tmp_id, UserGetUID());
    $upd = AutoCarga::factory("MovDocI007", "classes", "app", "Inv_MovimientosBodegas");
	
	if($tipo_prestamo_id == 1){
		 $dusoft_fi = AutoCarga::factory("SincronizacionDusoftFI", "SincronizacionDusoftFI", "", "");
		//$objResponse->script("alert('El tipo de prestamo no esta autorizado para sincronizar')");
	}
   

    $cabecera = $sql->DatosParaEditar($doc_tmp_id, UserGetUID());

    // Validar prefijo fi
    $sincronizar = false;
    $_prefijo_financiero = $consulta->obtener_prefijo_fi($cabecera['empresa_id'], $cabecera['prefijo']);
    if ($_prefijo_financiero['prefijo_fi'] != "") {
        $sincronizar = true;
    }

    // Validar si puede o no sincronizar con FI
    if ($cabecera['prefijo'] == "IBV" && !$sincronizar) {
        $objResponse->script("alert('El prefijo FI para el Documento IBV No esta parametrizado, No se Puede Sincronizar con Dusoft Financiero')");
    }
    ...
    if (!empty($productos)) {
        if ($tipo_prestamo_id == '07') {
            foreach ($vector as $key => $v) {
                $Token = $upd->ActualizarDevoluciones($prefijo, $numero, $v['codigo_producto'], $v['lote'], $v['cantidad']);
            }
        }

        if ($_REQUEST['DATOS']['prefijo_documento'] != '') {
            foreach ($vector as $key => $dtl) {
                $Token = $upd->ActualizarCantidadRecibida($_REQUEST['DATOS']['prefijo_documento'], $_REQUEST['DATOS']['numero_documento'], $dtl['codigo_producto'], $dtl['lote'], $dtl['cantidad']);
            }
        }
	...
	if($tipo_prestamo_id == 1){	 
        if ($cabecera['prefijo'] == "IBV") {
            // Sincronizar con FI
            $resultado_sincronizacion_ws = $dusoft_fi->ingreso_bonificaciones_fi($cabecera['empresa_id'], $productos['prefijo'], $productos['numero']);
        }
        $html .= "<center>\n";
        $html .= "  <p><label class=\"label_error\">SE HA CREADO EL DOCUMENTO EXITOSAMENTE</label></p>\n";
        if ($cabecera['prefijo'] == "IBV") {
            $html .= "  <p><label class=\"label_error\">Resultado Sincronizacion</label></p>\n";
            $html .= "  <p><label class=\"label\">Respuesta Ws: {$resultado_sincronizacion_ws['mensaje_ws']}</p></label>\n";
            $html .= "  <p><label class=\"label\">Respuesta BD: {$resultado_sincronizacion_ws['mensaje_bd']}</p></label>\n";
        }
        $html .= "</center>\n";
	}	
        ...
        $objResponse->assign("error_doc", "innerHTML", $html);
...
/*
 * Funcion para Mostrar Los Productos de un Documento de Devolucion
 * Mauro: 19-Mayo-2010
 */

function MostrarProductosDocRecibido($bodegas_doc_id, $datos, $tmp_doc_id, $tipo_prestamo_id, $CodigoProveedorId, $tercero_id, $tipo_id_tercero) {
    $objResponse = new xajaxResponse();
    ...
    $ProductosTraslado = $consulta1->ProductosTrasladoFarmacia($_REQUEST['DATOS']['prefijo_documento'], $_REQUEST['DATOS']['numero_documento']);
    ...
    $salida .= "<br>";
    $salida .= " <form name=\"productos_farmacia\"  method=\"post\">\n";
    ...
    $salida .= "         <a title='AGREGAR AL TEMPORAL'>OPC<a>";
    $salida .= "       </td>\n";
    $salida .= "      </tr>\n";
    $i = 0;
    foreach ($ProductosTraslado as $valor => $productos) {
        $CantidadTotal = $productos['cantidad'] - $productos['cantidad_recibida'];

        if ($CantidadTotal >= 1) {
            $tmp_invdetalle = $consulta1->Temporal_Inv_bodegasDetalle($tmp_doc_id, $productos['codigo_producto']);
            //$salida .= "<pre>".print_r($productos,true)."</pre>";
            ...
            $fecha_a = explode("-", $productos['fecha_vencimiento']);
            $fecha = $fecha_a[2] . "-" . $fecha_a[1] . "-" . $fecha_a[0];

            $salida .= "		  <td class=\"modulo_list_claro\" align=\"center\">\n";
            $salida .= "		  <input " . $color . " type=\"text\" readonly=\"true\" class=\"input-text\" name=\"fecha_venc" . $i . "\"  id=\"fecha_venc" . $i . "\" size=\"20\" maxlength=\"20\" onkeypress=\"return acceptDate(event)\" value=\"" . $fecha . "\"  >\n";
            $salida .= "      <input type=\"hidden\"  name=\"token\" id=\"token\"  value=\"\">\n";
            //$salida .= "			".ReturnOpenCalendario('productos_farmacia','fecha_venc'.$i,'-')."\n";
            $salida .= "		 </td>\n";
            $salida .= "    <td><input type=\"text\" readonly=\"true\" class=\"input-text\" maxlength=\"29\" name=\"lote" . $i . "\"id=\"lote" . $i . "\"value=\"" . $productos['lote'] . "\" ></td>\n";
            /* $salida .= "     <td align=\"left\" class=\"label_mark\">\n";
              $salida .= "         ".$productos['descripcion_unidad'];
              $salida .= "     </td>\n"; */
            $salida .= "    <td><input type=\"text\" class=\"input-text\" name=\"cantidad" . $i . "\"id=\"cantidad" . $i . "\"value=\"" . $CantidadTotal . "\" ></td>\n";
            ...
            $salida .= "      </td>\n";
            $salida .= "      <td>";
            //Accion de Productos En el Documento


            $salida .= "    <div>";
            $salida .= "		<a href=\"#\" id=\"AccionItem" . $productos['codigo_producto'] . "" . $productos['lote'] . "\" onclick=\"GuardarProductoTemporalTraslados('" . $bodegas_doc_id . "','" . $tmp_doc_id . "','" . $productos['codigo_producto'] . "',document.getElementById('cantidad" . $i . "').value,'" . $productos['porcentaje_gravamen'] . "','" . $productos['total_costo'] . "','" . UserGetUID() . "','" . $fecha . "','" . $productos['lote'] . "','','" . $CantidadTotal . "');\">";
            $salida .="     <img title=\"Adicionar Item Al Documento\" src=\"" . GetThemePath() . "/images/checkno.png\" border=\"0\">\n";
            $salida .= "      </a></div>";
            $salida .= "      </td>";

            /*
             * Campos que Estan en la Interfaz "ColocarProductos" y acá no se encontraban... No tiene uso acá
             ...
             */
            $salida .= "      <input type=\"hidden\" id=\"codigo_pro\">\n";
            $salida .= "      <input type=\"hidden\" id=\"unidad_pro\">\n";
            $salida .= "      <input type=\"hidden\" id=\"desc_pro\">\n";
            $salida .= "      <input type=\"hidden\" id=\"cantidad\">\n";
            $salida .= "      <input type=\"hidden\" id=\"gravamen\">\n";
            $salida .= "      <input type=\"hidden\" id=\"lotec\">\n";
            $salida .= "      <input type=\"hidden\" id=\"fecha_venc\">\n";
            $salida .= "      <input type=\"hidden\" id=\"op11\">\n";
            $salida .= "      <input type=\"hidden\" id=\"op12\">\n";
            $salida .= "      <input type=\"hidden\" id=\"op21\">\n";
            $salida .= "      <input type=\"hidden\" id=\"op22\">\n";
    ...
    $salida .= "                          <input type=\"button\" id=\"nuevo\" value=\"SELECCIONAR PRODUCTO\" class=\"input-bottom\" onClick=\"javascript:GuardarProductoTemporalFarmacia('" . $bodegas_doc_id . "','" . $tmp_doc_id . "',document.getElementById('codigo" . $i . "').value,document.getElementById('cantidad" . $i . "').value,document.getElementById('gravamen" . $i . "').value,document.getElementById('total_costo" . $i . "').value,'" . UserGetUID() . "',document.getElementById('fecha_venc" . $i . "').value,document.getElementById('lote" . $i . "').value,'" . $fecha_total . "');\">"; //
    $salida .= "                       </td>\n";
    $salida .= "                    </tr>\n";
    $salida .= "                   </tr>\n";
    $salida .= "                    </table>\n";
    ...
    $objResponse->assign("ListadoProductosDocumentoRecibido", "innerHTML", $salida);
    return $objResponse;
}
...
function MostrarProductox($bodegas_doc_id, $doc_tmp_id, $usuario_id) {
    ...
    if (!empty($vector)) {
        ...
        foreach ($vector as $valor => $productos) {
            ...
            $jaxx = "javascript:MostrarCapa('ContenedorB3');BorrarAjustes('" . $bodegas_doc_id . "','" . $doc_tmp_id . "','" . $usuario_id . "','" . $productos['item_id'] . "','ContenidoB3');IniciarB3('ELIMINAR REGISTRO');";
            ...
            $objResponse->script("
                                var link=document.getElementById('AccionItem" . $productos['codigo_producto'] . "" . $productos['lote'] . "');
...
function BorrarAjuste($bodegas_doc_id, $doc_tmp_id, $usuario_id, $item) {
    $consulta = new doc_bodegas_I007();
    $objResponse = new xajaxResponse();
    $ProductoEliminado = $consulta->ItemBorrado($item);
    // print_r($ProductoEliminado);
    $buscar = $consulta->EliminarItem($bodegas_doc_id, $item);
    if ($buscar == 1) {
        $objResponse->alert("EL ITEM $item FUE ELIMINADO CON EXITO");
        $objResponse->script("xajax_MostrarProductox('" . $bodegas_doc_id . "','" . $doc_tmp_id . "','" . $usuario_id . "');");
        $objResponse->script("
                                var link=document.getElementById('AccionItem" . $ProductoEliminado['codigo_producto'] . "" . $ProductoEliminado['lote'] . "');
                                link.style.display = '';
                                ");
    } else {
        $objResponse->alert("NO SE PUEDE BORRAR");
...
FUNCTION Borrar($bodegas_doc_id, $doc_tmp_id, $usuario_id, $item, $CONTENIDOR) {
    $objResponse = new xajaxResponse();
    ...
    $da .= "          <input type=\"button\" class=\"input-submit\" value=\"ELIMINAR\" name=\"ELIMINAR_MOV\" onclick=\"xajax_BorrarAjuste('" . $bodegas_doc_id . "','" . $doc_tmp_id . "','" . $usuario_id . "','" . $item . "');Cerrar('Contenedor" . $C . "');\">\n";
    $da .= "        </td>\n";
    $da .= "        <td align='center'>\n";
 ...
 *
 */
function GuardarFarm($doc_bodega_id, $doc_tmp_id, $codigo_producto, $cantidad, $porcentaje_gravamen, $total_costo, $usuario_id, $fecha_venc, $lotec, $fecha_total) {
    $objResponse = new xajaxResponse();
    $consulta = new doc_bodegas_I007();
    //print_r($doc_bodega_id);
    //GuardarTemporal($bodegas_doc_id,$doc_tmp_id, $codigo_producto, $cantidad, $porcentaje_gravamen, $total_costo, $usuario_id=null,$fecha_venc,$lotec)
    $Retorno = $consulta->GuardarTemporal($doc_bodega_id, $doc_tmp_id, $codigo_producto, $cantidad, $porcentaje_gravamen, $total_costo, $usuario_id = null, $fecha_venc, $lotec);
    return $objResponse;
}
...
function GuardarPT($bodegas_doc_id, $doc_tmp_id, $codigo_producto, $cantidad, $porcentaje_gravamen, $total_costo, $usuario_id, $fecha_venc, $lotec) {
    ...
    $path = SessionGetVar("rutaImagenes");
    $consulta = new doc_bodegas_I007();
    $datos = $consulta->ConsultarItemsExistencias($codigo_producto, SessionGetVar("EMPRESA"), SessionGetVar("centro_utilidad"), SessionGetVar("bodega"));
    $producto = $consulta->BuscarProductoLoteTemporal($doc_tmp_id, $usuario_id, $codigo_producto, $lotec);
    if (empty($producto)) {
        if (!empty($datos)) {
            ...
            $objResponse->assign("error_doc", "innerHTML", "El Producto No se Encuentra en el Inventario de la Empresa!! Hacer Ingreso a Existencias Bodegas!!");
        }
    } else {
        $objResponse->assign("error_doc", "innerHTML", "El Producto Ya Encuentra en el Documento!!");
    }
    ...
    if (!empty($busqueda)) {
      ...
      /*  $salida .= "                      <td align=\"center\" width=\"10%\">\n";
        ...
        /*    $salida .= "                      <td align=\"right\">\n";
            ...
            $salida .= "                         </a>\n";
            $salida .= "                      </td>\n";*/
            $salida .= "                      <td align=\"center\" onclick=\"AsignarPro('" . $busqueda[$i]['codigo_producto'] . "','" . $busqueda[$i]['descripcion'] . "','" . $busqueda[$i]['descripcion_unidad'] . "','" . $busqueda[$i]['fecha_vencimiento'] . "','" . $busqueda[$i]['lote'] . "', '{$busqueda[$i]['costo']}', '{$busqueda[$i]['porc_iva']}');\">\n";
        ...
        /* $malo=$Cont[0]['count'];
          $salida .= "".ObtenerPaginadoPro($path,$Cont,'1',$empresa_id,$centro_utilidad,$bodega,$tip_bus,$criterio,$offset); */
...
function GuardarTmpDoc($bodegas_doc_id, $observacion, $tipo_id_tercero, $tercero_id, $prestamo) {
    $objResponse = new xajaxResponse();
    $path = SessionGetVar("rutaImagenes");
    $consulta = new MovBodegasSQL();
    $consulta1 = new doc_bodegas_I007();
    $valor = $consulta1->CrearDoc($bodegas_doc_id, $observacion, $tipo_id_tercero, $tercero_id, $prestamo);
	
	
	SessionSetVar("tipoPrestado",$prestamo);
...
function Buscadorter($pagina, $criterio1, $criterio2, $criterio, $tipo_doc_general, $div, $Forma) { //echo "si";
    $objResponse = new xajaxResponse();
...
//       $objResponse->alert($Forma);
    ...
    if ($criterio == "")
        $criterio = "0";
    $vector = $consulta->Terceros_General($pagina, $criterio1, $criterio2, $criterio, $op = 1);
    $salida .= "                  <div id=\"ventana_terceros\">\n";
    ...
    if ($criterio == "0")
        $salida .= "                         <input type=\"text\" class=\"input-text\" name=\"nom_buscar\" id=\"nom_buscar\" maxlength=\"40\" size\"40\" value=\"\" onkeypress=\"return acceptm(event)\">";
    else
        $salida .= "                         <input type=\"text\" class=\"input-text\" name=\"nom_buscar\" id=\"nom_buscar\" maxlength=\"40\" size\"40\" value=\"" . $criterio . "\" onkeypress=\"return acceptm(event)\">";
    $salida .= "                       </td>\n";
    $salida .= "                       <td rowspan='2' width=\"10%\" align=\"center\">\n";                                                                 //$pagina,            $criterio1,                                   $criterio2,                           $criterio                          ,$div,      $Forma
    $salida .= "                        <input type=\"button\" class=\"input-submit\" name=\"boton_bus\"  id=\"boton_bus\" value=\"BUSCAR\" onclick=\"Bus_ter(document.getElementById('buscar_x').value,document.getElementById('buscar').value,document.getElementById('nom_buscar').value,'" . $div . "','" . $Forma . "','1')\">\n";
    $salida .= "                       </td>\n";
    $salida .= "                       </tr>\n";
...
//         $salida .="                         <td>\n";
    if ($criterio2 == "0")
        $salida .="                            <input type=\"text\" class=\"input-text\" name=\"buscar\" id=\"buscar\" maxlength=\"40\" size\"40\" value=\"\"onkeypress=\"return acceptm(event)\">";
    else
        $salida .="                            <input type=\"text\" class=\"input-text\" name=\"buscar\" id=\"buscar\" maxlength=\"40\" size\"40\" value=\"" . $criterio2 . "\"onkeypress=\"return acceptm(event)\"></td>";
    $salida .="                         </td>\n";
    $salida .= "                       </tr>\n";
    ...
    if (count($vector) > 0) {
        ...
        $action = "Bus_ter('" . $criterio1 . "','" . $criterio2 . "','" . $criterio . "','" . $div . "','" . $Forma . "','" . $tipo_doc_general . "'";
        $ctl = AutoCarga::factory("ClaseHTML");
        $salida .= $ctl->ObtenerPaginadoXajax($consulta->conteo, $consulta->paginaActual, $action, "0", 10);

        //$salida .= "".ObtenerPaginadoter($pagina,$path,$consulta->conteo,$op,$criterio1,$criterio2,$criterio,$div,$Forma);
        ...
        for ($i = 0; $i < count($vector); $i++) {
            ...
            $salida .= "                      </td>\n";
            $salida .= "                      <td align=\"center\">\n";
            $java = "javascript:Seleccionado('" . $Forma . "','" . $vector[$i]['tipo_id_tercero'] . "','" . strtoupper($vector[$i]['tercero_id']) . "','" . $vector[$i]['nombre_tercero'] . "','" . $tipo_doc_general . "');";
 ...
 *
 */
function DevolucionprestamoTerce($tipo_id_tercero, $tercero_id, $tipo_doc_general) {
    $objResponse = new xajaxResponse();
    $consulta = new MovBodegasSQL();
    $devolucion_prestamo = $consulta->PrestamoDocumentos($tipo_id_tercero, $tercero_id, $tipo_doc_general);
    //var_dump($devolucion_prestamo);
    //print_r($tercero_id."teceroooooooooooooooooooooooo2"); 
    if ($devolucion_prestamo AND $devolucion_prestamo['sw_devolucion'] == 0) {
        $salida .= "                    <tr>\n";
        $salida .= "                      <td rowspan='1' colspan='9' align=\"left\" class=\"modulo_table_list_title\"> \n";
        $salida .= "                        TIPO PRESTAMO";
        $salida .= "                       </td>\n";
        $salida .= "                      <td rowspan='1' colspan='11' align=\"left\" class=\"modulo_list_claro\"> \n";
        if (!empty($devolucion_prestamo)) {
            $salida .= "                       <select id=\"pres\" name=\"pres\" class=\"select\" onchange=\"\">";
            $salida .= "                           <option value=\"0\" SELECTED>SELECCIONAR</option>\n";
            for ($i = 0; $i < count($devolucion_prestamo); $i++) {
                $salida .= "                      <option value=\"" . $devolucion_prestamo[$i]['tipo_prestamo_id'] . "\">" . $devolucion_prestamo[$i]['descripcion'] . "</option>\n";
            }
            $salida .= "                       </select>\n";
        }
        //$salida .= "                        ".$devolucion_prestamo['descripcion']." ";
        //$salida .= "                        <input  id=\"pres\" type=\"hidden\" value=\"".$devolucion_prestamo['tipo_prestamo_id']."\">\n";
        $salida .= "                       </td>\n";
        $salida .= "                    </tr>\n";
    } else {
        $salida .= "                    <tr>\n";
        ...
        $salida .= "                    TIPO PRESTAMO";
        $pres = $consulta->Prestamos();
        if (!empty($pres)) {
            $salida .= "                       <select id=\"pres\" name=\"pres\" class=\"select\" onchange=\"\">";
            $salida .= "                           <option value=\"0\" SELECTED>SELECCIONAR</option>\n";
            //var_dump($utility);
            for ($i = 0; $i < count($pres); $i++) {
                $salida .= "                      <option value=\"" . $pres[$i]['tipo_prestamo_id'] . "\">" . $pres[$i]['descripcion'] . "</option>\n";
            }
            $salida .= "                       </select>\n";
        }
        $salida .= "                       </td>\n";
    ...
    $objResponse->assign("prestamo", "innerHTML", $salida);
    return $objResponse;
}
...
function MostrarTerceros($forma) {
    ...
    if ($forma['tipo_idfc'] == 2) {
        $salida .= " <table width=\"100%\" align=\"center\" class=\"modulo_table_list\">\n";
        $salida .= "   <tr>\n";
        $salida .= "     <td  width='30%' align=\"left\" class=\"modulo_table_list_title\">\n";
        $salida .= "       FARMACIA";
        $salida .= "     </td>\n";
        $salida .= "     <td rowspan='1' colspan='6' align=\"left\" class=\"modulo_list_claro\"> \n";
        //$salida .= "  <input type=\"hidden\" id=\"pres\" name=\"pres\" value=\"0\">\n";

        $documentos = $sql->Farmacias_prestamo("", $_SESSION['EMPRESAS']['id']);
        //print_r($_SESSION);
        if (!empty($documentos)) {

            if ($_SESSION['EMPRESAS']['sw_tipo_empresa'] == 1) {
                $salida .= "    <select id=\"id_tercerox\" name=\"id_tercerox\" class=\"select\" onchange=\"MostrarPrestamo('" . $forma['tipo_doc'] . "')\">";
                $salida .= "     <option value=\"0\" SELECTED>SELECCIONAR</option>\n";
                $salida .= "   <option value=\"" . $_SESSION['EMPRESAS']['id'] . "\" onclick=\"AdicionarId('" . $_SESSION['EMPRESAS']['tipo_id_tercero'] . "');\">" . $_SESSION['EMPRESAS']['razon_social'] . "</option>\n";
                $salida .= "   </select>\n";
            }

            $salida .= "    #DOC <select id=\"id_tercerox\" name=\"id_tercerox\" class=\"select\" onchange=\"MostrarPrestamo('" . $forma['tipo_doc'] . "')\">";
            $salida .= "     <option value=\"0\" SELECTED onclick=\"AdicionarDocumento('-1','-1');\">SELECCIONAR</option>\n";
            //var_dump($utility);
            for ($i = 0; $i < count($documentos); $i++) {
                $salida .= "   <option value=\"" . $documentos[$i]['prefijo'] . "\" onclick=\"AdicionarDocumento(this.value,'" . $documentos[$i]['numero'] . "');\">" . $documentos[$i]['prefijo'] . "-" . $documentos[$i]['numero'] . "</option>\n";
            }
            $salida .= "   </select>\n";
        }
        $salida .= "  <input type=\"hidden\" id=\"tipox_id\" name=\"tipox_id\" >\n";
        ...
        /*                                     JOHANNA
          $farmacia=$sql->Farmacias_prestamo();
          ...
          $salida .= "     <option value=\"0\" SELECTED>SELECCIONAR</option>\n";
          //var_dump($utility);
          for($i=0;$i<count($farmacia);$i++)
          {
          $salida .= "   <option value=\"".$farmacia[$i]['farmacia_id']."\" onclick=\"AdicionarId('".$farmacia[$i]['tipo_id_tercero']."');\">".$farmacia[$i]['razon_social']."</option>\n";

          }
          $salida .= "   </select>\n";
          //$salida .= "  <input type=\"hidden\" id=\"tipox_id\" name=\"tipox_id\" value=\"".$farmacia[$i]['tipo_id_tercero']."\">\n";
          $salida .= "  <input type=\"hidden\" id=\"tipox_id\" name=\"tipox_id\" >\n";
          //$salida .= "  <input type=\"hidden\" id=\"id_tercerox\" name=\"id_tercerox\" value=\"".$farmacia[$i]['farmacia_id']."\">\n";
          } */
        $salida .= "                       </td>\n";
        $salida .= "                    </tr>\n";
        ...
        //document.getElementById('tipox_id').value,document.getElementById('id_tercerox').value
        //$salida .= "                          <input type=\"hidden\" id=\"tercerito\" name=\"tercerito\" value=\"0\">\n";
        //$salida .= "                          <input type=\"hidden\" id=\"htmp_id\" name=\"htmp_id\" value=\"0\">\n";
        /* $salida .= "                       <td  width='5%'  align=\"left\" class=\"modulo_list_claro\" id=\"tipos_ids_terceroxa\"> \n";
          /* $salida .= "                         <select name=\"tipox_id\" id=\"tipox_id\" class=\"select\" onchange=\"\">";
...
function RegistrarFactura($empresa_id, $tmp_doc_id, $NumeroFactura, $Prefijo, $tercero_id, $tipo_id_tercero) {
    ...
    if ($num < 1) {
        ...
        $html = "Hay Facturas!!!";
        $objResponse->script("document.getElementById('numero_factura_').value='" . $NumeroFactura . "';");
        $objResponse->script("document.getElementById('prefijo_').value='" . $Prefijo . "';");
        $objResponse->script("document.getElementById('numero_').value='" . $Facturas[0]['numero'] . "';");
        //$objResponse->script("alert(document.getElementById('numero_factura_').value);");
    }
...
function BuscarProductoFacturas($empresa_id, $centro_utilidad, $bodega, $codigo_producto, $descripcion, $numero_factura, $prefijo, $numero, $usuariotmp, $doc_tmp_id, $offset) {
    $objResponse = new xajaxResponse();
    $path = SessionGetVar("rutaImagenes");
    $sql = AutoCarga::factory("MovDocI007", "classes", "app", "Inv_MovimientosBodegas");
    ...
    //$objResponse->alert($numero_factura);
    $Productos_Facturas = $sql->Productos_Factura($empresa_id, $centro_utilidad, $bodega, $codigo_producto, $descripcion, $numero_factura, $prefijo, $numero, $offset);



    //$busqueda=$consulta->BuscarProducto($empresa_id,$centro_utilidad,$bodega,$codigo_producto,$descripcion,$numero_factura,$prefijo,$numero,$offset);
    if (!empty($Productos_Facturas)) {
        $salida .= "                 <div id=\"erro\" class='label_error' style=\"text-transform: uppercase; text-align:center; font-size:11px;\">\n";
        $salida .= "                 </div>\n";
        $salida .= "                 <form name=\"adicionar\">\n";
        $salida .= "                  <table width=\"95%\" align=\"center\" class=\"modulo_table_list\">\n";
        ...
        $salida .= "                      </td>\n";
        $salida .= "                      <td align=\"center\" width=\"10%\">\n";
        $salida .= "                       ITEMS FACTURAS/";
        $salida .= "                      </td>\n";
        $salida .= "                      <td align=\"center\" width=\"5%\">\n";
        $salida .= "                        <a title='SELECCIONAR PRODUCTO'>SL<a>";
        $salida .= "                      </td>\n";
        $salida .= "                    </tr>\n";
        for ($i = 0; $i < count($Productos_Facturas); $i++) {
            $PDT = $sql->Buscar_ProductoEnDocTemporal($doc_tmp_id, $usuariotmp, $empresa_id, $centro_utilidad, $bodega, $Productos_Facturas[$i]['codigo_producto'], $Productos_Facturas[$i]['lote']);
            if (empty($PDT)) {
                $salida .= "                    <tr class=\"modulo_list_claro\" onclick=\"mOvr(this,'#ffffff'); \" onMouseOver=\"mOvr(this,'#ffffff');\" onMouseOut=\"mOut(this,'#DDDDDD');\" >\n";
                $salida .= "                      <td align=\"left\">\n";
                $salida .= "                        " . $Productos_Facturas[$i]['codigo_producto'];
                $salida .= "                      </td>\n";
                $salida .= "                      <td align=\"left\" class=\"label_mark\">\n";
                $salida .= "                        " . $Productos_Facturas[$i]['descripcion'];
                $salida .= "                      </td>\n";
                $salida .= "                      <td align=\"left\" class=\"label_mark\">\n";
                $salida .= "                         " . $Productos_Facturas[$i]['descripcion_unidad'];
                $salida .= "                      </td>\n";
                $salida .= "                      <td align=\"right\">\n";
                $salida .= "                         <a title='COSTO PROMEDIO'>\n";
                $salida .= "                         " . $Productos_Facturas[$i]['costo'];
                $salida .= "                         </a>\n";
                $salida .= "                      </td>\n";

                $salida .= "                      <td align=\"right\">\n";
                $salida .= "                         <a title='LOTE'>\n";
                $salida .= "                         " . $Productos_Facturas[$i]['lote'];
                $salida .= "                         </a>\n";
                $salida .= "                      </td>\n";

                $fecha = $Productos_Facturas[$i]['fecha_vencimiento'];  //esta es la que viene de la DB
                list( $ano, $mes, $dia ) = split('[/.-]', $fecha);
                $fecha = $dia . "-" . $mes . "-" . $ano;

                $CantidadXDevolver = $Productos_Facturas[$i]['cantidad'] - $Productos_Facturas[$i]['cantidad_devuelta'];
                ...
                $salida .= "                      <td align=\"right\">\n";
                $salida .= "                         <a title='ITEMS FACTURA'>\n";
                $salida .= "                         " . $CantidadXDevolver;
                $salida .= "                         </a>\n";
                $salida .= "                      </td>\n";

                $salida .= "                      <td align=\"center\" onclick=\"AsignarPro2('" . $Productos_Facturas[$i]['codigo_producto'] . "','" . $Productos_Facturas[$i]['descripcion'] . "','" . $Productos_Facturas[$i]['descripcion_unidad'] . "','" . $fecha . "','" . $Productos_Facturas[$i]['lote'] . "','" . $Productos_Facturas[$i]['costo'] . "','" . $CantidadXDevolver . "');\">\n";
                $salida .= "                         <a title='SELECCIONAR PRODUCTO'>\n";
                $salida .= "                          <sub><img src=\"" . $path . "/images/checkno.png\" border=\"0\" width=\"14\" height=\"14\"></sub>\n";
                $salida .= "                         </a>\n";
                $salida .= "                      </td>\n";
