...
class doc_bodegas_I011 {
	...
	//AND tor.empresa_id = md.empresa_id
        $sql = "SELECT 
                  md.*,
                  fc_descripcion_producto(md.codigo_producto) as descripcion,
				  (SELECT ti.descripcion FROM inv_tipo_producto as ti WHERE invp.tipo_producto_id =  ti.tipo_producto_id ) as tipo_producto,
				  CASE WHEN (SELECT tor.torre FROM param_torreproducto as tor WHERE tor.codigo_producto = invp.codigo_producto limit 1) is null THEN 'Sin definir' 
				  ELSE (SELECT tor.torre FROM param_torreproducto as tor WHERE tor.codigo_producto = invp.codigo_producto limit 1) END as torre
                  FROM
