...
function CrearDocumentoFinalx($bodegas_doc_id, $doc_tmp_id, $tipo_doc_bodega_id, $empresa_id, $centro_utilidad, $bodega) {
    ...
    if (!empty($productos)) {
        ...
        $salida .= "                        <a title='SELECCIONAR TORRE'>TORRE<a>";
        $salida .= "                      </td>\n";
        /*$salida .= "                      <td align=\"center\" width=\"7%\">\n";
...
//             {var_dumP($productos);
		...
		$salida .= "<select class = 'select' id='numero_torre' onchange=\"ImprimirDocumentoI011(this.value,'app_modules/Inv_MovimientosBodegas/Doc_Mov_Bodegas/I011/imprimir/imprimir_documentoI011.php','".SessionGetVar("EMPRESA")."','".$productos['prefijo']."','".$productos['numero']."');\">
...
function ListadoProductos_Farmacia_d($bodegas_doc_id, $tmp_doc_id, $farmacia_id, $prefijo, $numero, $codigo_barras, $descripcion) {
    ...
    $salida .= "                  <tr class=\"modulo_table_list_title\">";
    $salida .= "                      <td>TORRE</td><td>TIPO</td><td>CODIGO</td><td>NOMBRE</td><td>CANTIDAD</td><td>LOTE</td><td>FECHA VENCIMIENTO</td><td>CANT. INGRESAR</td><td>NOVEDAD</td><td>OP</td><td>MOD</td>";
    ...
    foreach ($ProductosDocumento as $key => $producto) {
        ...
        if (count($resultado) == 0) { 
			...
			$salida .= "                    <td>" . $producto['torre'] . "</td>";
			$salida .= "                    <td>" . $producto['tipo_producto'] . "</td>";
