/**************************************************************************************
 * $Id: definirBodegas_I011.js,v 1.1 2009/07/17 19:08:17 johanna Exp $ 
...
function ImprimirDocumentoI011(torre,direccion, empresa_id, prefijo, numero)
{
	...
	//console.log("IMPRIMIR torre ------------------------>", torre);
...
function GrabarDocumento(bodegas_doc_id, observacion, farmacia_id, documento_farmacia)
{
    if (farmacia_id == '')
    {
        document.getElementById('errorcreartmp').innerHTML = "EL DOCUMENTO EXIGE UNA FARMACIA";
        return false;
    }
    //alert(factura);
    if (documento_farmacia == '-1')
    {
        document.getElementById('errorcreartmp').innerHTML = "EL DOCUMENTO EXIGE UN DOCUMENTO DE LA FARMACIA";
        return false;
    }


    xajax_GuardarTmpDoc(bodegas_doc_id, observacion, farmacia_id, documento_farmacia);

}


function GuardarProductoTemporal(doc_bodega_id, doc_tmp_id, codigo_producto, cantidad, porcentaje_gravamen, total_costo, usuario_id, fecha_venc, lotec, lote_devuelto)
{
    ...
    //alert(cox);
    xajax_GuardarPT(doc_bodega_id, doc_tmp_id, codigo_producto, cantidad, porcentaje_gravamen, cox, usuario_id, fecha_venc, lotec, lote_devuelto)
}
...
function recogerTeclaBus(evt)
{
    ...
    {
        xajax_ListadoProductos_Farmacia_d(document.getElementById('bodegas_doc_id').value, document.getElementById('tmp_doc_id').value, document.getElementById('farmacia_id').value, document.getElementById('prefijo').value, document.getElementById('numero').value, document.getElementById('codigo_barras').value, document.getElementById('descripcion').value);
    }
}
...
function AsignarPro(codigo_producto, descripcion, descripcion_unidad, costo, existencia, fecha_vencimiento, lote, lote_devuelto)
{
    ...
    document.getElementById('lote_devuelto').value = lote_devuelto;
    Cerrar('ContenedorBus');
}
/**********************************************
 para buscar un producto
 ***********************************************/
function Bus_Pro(empresa_id, centro_utilidad, bodega, tip_bus, criterio, tipo_prestamo_id, offset)
{
    xajax_BuscarProducto1(empresa_id, centro_utilidad, bodega, tip_bus, criterio, tipo_prestamo_id, offset);
 ...
 ************************************************************************************/
function Seleccionado(formulario, farmacia_id, razon_social, empresa_id, CentroUtilidad, Bodega)
{
    //alert(datos);
    if (formulario == "unocreate")
    {
        document.unocreate.farmacia_id.value = farmacia_id;
        document.getElementById('farmacia').innerHTML = razon_social;

        xajax_Listar_DocumentosFarmacia(farmacia_id, empresa_id, CentroUtilidad, Bodega);
...
function BuscarProveedor(div, forma)
{
    //alert("esto es lo q se va a buscar"+ criterio);
    xajax_BuscadorFarmacia(div, forma);
}

function Paginador(empresa_id, CentroUtilidad, Bodega, CodigoEmpresa, RazonSocial, offset)
{
    xajax_BuscadorProveedor_d(empresa_id, CentroUtilidad, Bodega, CodigoEmpresa, RazonSocial, offset);
}

function CambiarInfoDevolucion(doc_tmp_id, codigo_producto, lote, fecha_vencimiento, cantidad, porcentaje_gravamen,valor_costo)
{
    xajax_CambiarInfoDevolucion(doc_tmp_id, codigo_producto, lote, fecha_vencimiento, cantidad, porcentaje_gravamen, valor_costo);
}

function guardar(doc_tmp_id, producto_base) {
    ...
    while (i < 5) {
        if (document.getElementById('loteXX' + i).value != '' && document.getElementById('fvXX' + i).value != '' && parseInt(document.getElementById('cantXX' + i).value) > 0) {
            suma = suma + parseInt(document.getElementById('cantXX' + i).value);

            productos_nuevos.push({codigo_producto: producto_base.codigo_producto, lote: document.getElementById('loteXX' + i).value, fecha_vencimiento: document.getElementById('fvXX' + i).value, cantidad: document.getElementById('cantXX' + i).value});
        }
        i = i + 1;
    }

    if (suma != producto_base.cantidad) {
        alert('Los lotes no suman la cantidad seleccionada');
    }
    else {

        var bodegas_doc_id = document.getElementById('bodegas_doc_id').value;
        var farmacia_id = document.getElementById('farmacia_id').value;
        var prefijo = document.getElementById('prefijo').value;
        var numero = document.getElementById('numero').value;
        var novedad = document.getElementById('Novedad'+producto_base.movimiento_id).value;
        
        xajax_HacerCambiarInfoDevolucion(farmacia_id, prefijo, numero, bodegas_doc_id, doc_tmp_id, producto_base, productos_nuevos, novedad)
        Cerrar('Contenedor');
    }
}