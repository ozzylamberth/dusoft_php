...
/* * ************************************************************************************
 * $Id: definirBodegas_I012.php,v 1.1 2009/07/17 19:08:17 johanna Exp $ 
...
//include "../../../app_modules/Inv_MovimientosBodegas/Doc_Mov_Bodegas/I012/doc_Bodegas_I012_HTML.class.php";
	...
	print_r($ProductosTemporal);*/
		...
		if (!empty($productos)) {
			foreach ($ProductosTemporal as $key => $Valor) {
				$cons->CantidadesDevueltasFactura($DocTemporal_Auxiliar['numero'], $DocTemporal_Auxiliar['prefijo'], $Valor['empresa_id'], $Valor['item_id_compras'], $Valor['cantidad']);
        }
        ...
        $salida .= "                      </td>\n";
		$TorresDespacho = $consulta->Listar_Torres(SessionGetVar("EMPRESA"), $productos['prefijo'], $productos['numero']);
		$salida .= "<td align=\"center\">\n";                          		
		$salida .= "<select class = 'select' id='numero_torre' onchange=\"Imprimir(this.value,'app_modules/Inv_MovimientosBodegas/Doc_Mov_Bodegas/I011/imprimir/imprimir_documentoI011.php','".SessionGetVar("EMPRESA")."','".$productos['prefijo']."','".$productos['numero']."');\">   
		<option value=\"No\">--Seleccionar--</option>";  
		foreach ($TorresDespacho as $key => $cc){
			$salida .="<option value='" . $cc['torre'] . "'>";
			$salida .=$cc['torre'];
			$salida .="</option>";
		}
		$salida .="</select>";
		$salida .= "</td>\n";
        $salida .= "<td align=\"center\">\n";
        $direccion = "app_modules/Inv_MovimientosBodegas/Doc_Mov_Bodegas/I012/imprimir/imprimir_docI012.php";
       ...
       $objResponse->script("alert('Error Generando el Documento')"); 
    }
...
function MostrarProductox($bodegas_doc_id, $doc_tmp_id, $usuario_id) {
    ...
    if (!empty($vector)) {
        ...
        foreach ($vector as $valor => $productos) {
            ...
            $salida .= "                      </td>\n";
            $salida .= "                      <td align=\"left\" class=\"normal_10AN\">\n";
            $salida .= "                        $" . FormatoValor($productos['valor_unitario'], 4);
            $subtotal +=$productos['valor_unitario'];
            $IvaTotal +=($productos['valor_unitario'] * ($productos['porcentaje_gravamen'] / 100));
            $salida .= "                      </td>\n";
            $salida .= "                      <td align=\"center\">\n";
        ...
        if ($Factura_Cabecera['subtotal'] >= $parametros_retencion['base_rtf'])
            $retencion_fuente = $subtotal * ($Factura_Cabecera['porcentaje_rtf'] / 100);

        if ($Factura_Cabecera['subtotal'] >= $parametros_retencion['base_ica'])
            $retencion_ica = $subtotal * ($Factura_Cabecera['porcentaje_ica'] / 1000);

        if ($Factura_Cabecera['subtotal'] >= $parametros_retencion['base_reteiva'])
            $retencion_iva = $IvaTotal * ($Factura_Cabecera['porcentaje_reteiva'] / 100);
...
function IngresoProductosTemporal($bodegas_doc_id, $doc_tmp_id, $Formulario, $usuario_id) {
    ...
    for ($i = 0; $i < $Formulario['registros']; $i++) {
        ...
        if ($Formulario[$i] != "") {
            ...
            if ($Formulario['cantidad' . $i] <= 0 || $Formulario['cantidad' . $i] == "") {
                $objResponse->assign("mensaje" . $i, "innerHTML", "La Cantidad No Es Correcta!!");
            } else
            if ($Formulario['cantidad' . $i] > $Formulario['total' . $i]) {
                $objResponse->assign("mensaje" . $i, "innerHTML", "La Cantidad No Puede Ser Mayor a la De la Factura!!");
            } else {
                ...
                if (empty($ProductoEnTemporal)) {
                    $Retorno = $consulta->GuardarTemporal($bodegas_doc_id, $doc_tmp_id, $Formulario['codigo_producto' . $i], $Formulario['cantidad' . $i], $Formulario['porc_iva' . $i], ($Formulario['cantidad' . $i] * $Formulario['costo' . $i]), $usuario_id = null, $FechaVencimiento, $Formulario['lote' . $i], ($Formulario['cantidad' . $i] * $Formulario['valor_unitario' . $i]));

                    $consulta->ItemEnMovimiento($doc_tmp_id, $Retorno, $Formulario[$i]);
                    $objResponse->script("document.getElementById('" . $i . "').disabled=\"true\"");
                }
                else
                    $objResponse->assign("mensaje" . $i, "innerHTML", "Ya se Hizo Insercion, no es posible Ingresarlo mas de Una Vez!!");
            }
        }
    }


    //$sql = AutoCarga::factory("MovDocI012","classes","app","Inv_MovimientosBodegas");
...
function GuardarTmpDoc($bodegas_doc_id, $observacion, $tipo_id_tercero, $tercero_id, $factura) {
    $objResponse = new xajaxResponse();
    $path = SessionGetVar("rutaImagenes");
    $consulta1 = new doc_bodegas_I012();
    $valor = $consulta1->CrearDoc($bodegas_doc_id, $observacion, $tipo_id_tercero, $tercero_id, $factura);
...
function BuscadorProveedores($div, $Forma) {
    ...
    $salida .= "                         BUSCADOR DE CLIENTE";
    $salida .= "                      </td>\n";
    $salida .= "                    </tr>\n";
    $salida .= "                    <tr class=\"modulo_list_claro\">\n";
    $salida .= "                       <td width=\"44%\"  align=\"center\">\n";
    $salida .= "                        <b>NOMBRE CLIENTE :</b>";
    $salida .= "                       </td>\n";
...
function BuscadorProveedor_d($empresa_id, $tipo_id_tercero, $tercero_id, $nombre_tercero, $offset) {
    ...
    if (!empty($Listar_Terceros)) {
        ...
        foreach ($Listar_Terceros as $key => $valor) {
            ...
            $salida .= "        <a href=\"#\" onclick=\"Seleccionado('unocreate','" . $valor['tipo_id_tercero'] . "','" . $valor['tercero_id'] . "','" . $valor['nombre_tercero'] . "','" . $empresa_id . "')\">\n";
...
function Listar_FacturasProveedor($tipo_id_tercero, $tercero_id) {
    $objResponse = new xajaxResponse();
    $path = SessionGetVar("rutaImagenes");
    $sql = AutoCarga::factory("MovDocI012", "classes", "app", "Inv_MovimientosBodegas");
    $consulta = new doc_bodegas_I012();
    $facturas = $sql->Listar_FacturasProveedor($tipo_id_tercero, $tercero_id, SessionGetVar("EMPRESA"));
    
    $select = "<select class=\"select\" name=\"factura\" id=\"factura\">";
    $select .= "<option value=\"\">---</option>";
    foreach ($facturas as $key => $valor) {
        $select .=" <option value=\"" . $valor['prefijo'] . "@" . $valor['factura_fiscal'] . "@". $valor['fac_agrupada']. "\">" . $valor['prefijo'] . "-" . $valor['factura_fiscal'] . "</option>";
    }
    $select .= "</select>";
...
/******************************************ESTE ES EL FORMULARIO QUE SE ESTA TRABAJANDO***************************************************************/

function productosDevoluciones($bodegas_doc_id, $datos, $tmp_doc_id, $empresa_id, $tipo_id_tercero, $tercero_id,$prefijo,$numero,$empresa_idD,$InformacionTercero, $ProductosFactura2, $DATOS, $offset) {

                                                                   

$objResponse = new xajaxResponse();
  $sql = AutoCarga::factory("MovDocI012", "classes", "app", "Inv_MovimientosBodegas");
   $ProductosFactura = $sql->ProductosDevolucionesSql($tipo_id_tercero, $tercero_id,$prefijo,$numero,$empresa_idD,$offset);
 
		$path = SessionGetVar("rutaImagenes");
		$action['paginador'] = "PaginadorDocumentos('" . $bodegas_doc_id . "','".$datos. "','".$tmp_doc_id. "','".$empresa_id. "','".$tipo_id_tercero. "','".$tercero_id. "','".$prefijo. "','".$numero. "','".$empresa_idD. "','".$InformacionTercero. "','".$ProductosFactura2. "','".$DATOS. "'  ";
		$pghtml = AutoCarga::factory("ClaseHTML");   
		...
		//$salida .= "                          <input type=\"button\" id=\"nuevo\" value=\"SELECCIONAR PRODUCTO\" class=\"input-bottom\" onClick=\"xajax_IngresoProductosTemporal('" . $bodegas_doc_id . "','" . $tmp_doc_id . "',xajax.getFormValues('ProductosFactura'),'" . UserGetUID() . "');\">";
		 ...
		 $objResponse->assign("Listado_Productos_Devolucion", "innerHTML", $salida);
    return $objResponse;
