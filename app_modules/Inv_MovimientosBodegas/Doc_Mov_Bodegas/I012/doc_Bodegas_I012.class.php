...
/* * ****************************************************************************
 * $Id: doc_Bodegas_I012.class.php,v 1.1 2009/07/17 19:08:17 johanna Exp $
...
class doc_bodegas_I012 {
    ...
    function ProductosFactura($tipo_id_tercero, $tercero_id, $prefijo, $numero, $empresa_id, $offset) {
      /*  $sql = "SELECT 
		a.item_id,
                a.empresa_id,
		a.prefijo,
		...
		((a.cantidad)*(a.valor_unitario+(a.valor_unitario*(a.porc_iva/100)))) as valor_total_iva,
		b.costo
                FROM
                inv_facturas_despacho_d a
		JOIN inventarios as b ON (a.codigo_producto=b.codigo_producto) AND (a.empresa_id = b.empresa_id)
              WHERE a.prefijo = '" . $prefijo . "'
              and a.factura_fiscal = " . $numero . "
              and a.empresa_id = '" . $empresa_id . "'
              and a.cantidad <> a.cantidad_devuelta
              ";*/
        ...
        $sql = "
                ...
                a.codigo_producto, 
                fc_descripcion_producto(a.codigo_producto) as descripcion_producto, 
                a.cantidad, a.cantidad_devuelta, 
                a.fecha_vencimiento, 
                a.lote, 
                a.valor_unitario, 
                a.porc_iva, 
                ...
                b.costo 
                FROM (
                  select item_id, empresa_id, prefijo, factura_fiscal, codigo_producto, cantidad, cantidad_devuelta, 
                  fecha_vencimiento, lote, valor_unitario, porc_iva, 0 from inv_facturas_despacho_d
                  union
                  select item_id, empresa_id, prefijo, factura_fiscal, codigo_producto, cantidad, cantidad_devuelta, 
                  fecha_vencimiento, lote, valor_unitario, porc_iva, 1 from inv_facturas_agrupadas_despacho_d
                ) as a 
                JOIN inventarios as b ON (a.codigo_producto=b.codigo_producto) AND (a.empresa_id = b.empresa_id) 
                WHERE a.prefijo = '{$prefijo}' and a.factura_fiscal = {$numero} and a.empresa_id = '{$empresa_id}' and a.cantidad <> a.cantidad_devuelta             
        ";
    ...
    function CantidadesDevueltasFactura($Numero, $prefijo, $Empresa, $ItemId, $cantidad) {

        //$this->debug=true;
        $sql = " UPDATE inv_facturas_despacho_d SET cantidad_devuelta = cantidad_devuelta + {$cantidad} 
                 Where factura_fiscal = {$Numero} AND empresa_id = '{$Empresa}' AND prefijo = '{$prefijo}' AND item_id = {$ItemId} ; ";
        
        $sql .= " UPDATE inv_facturas_agrupadas_despacho_d SET cantidad_devuelta = cantidad_devuelta + {$cantidad} 
                 Where factura_fiscal = {$Numero} AND empresa_id = '{$Empresa}' AND prefijo = '{$prefijo}' AND item_id = {$ItemId} ; ";
    ...
    function GuardarTemporal($bodegas_doc_id, $doc_tmp_id, $codigo_producto, $cantidad, $porcentaje_gravamen, $total_costo, $usuario_id = null, $fecha_venc, $lotec, $valor_unitario) {
        $ClassDOC = new BodegasDocumentos($bodegas_doc_id);
        $OBJETO = $ClassDOC->GetOBJ();
        $RETORNO = $OBJETO->AddItemDocTemporal($doc_tmp_id, $codigo_producto, $cantidad, $porcentaje_gravamen, $total_costo, $usuario_id = null, $fecha_venc, $lotec, '', $valor_unitario, $valor_unitario, $valor_unitario);
        //var_dump($RETORNO);
        return $RETORNO;
    }
    ...
    function ConsultaItemTemporal_($doc_tmp_id, $item_id) {
        ...
        $query = "
            SELECT *
            FROM inv_bodegas_movimiento_tmp_d
            WHERE doc_tmp_id= {$doc_tmp_id} and item_id_compras= {$item_id}
