<?php
  ...
  if(!empty($resultado['DATOS_ADICIONALES']))
  {
    ...
    foreach($resultado['DATOS_ADICIONALES'] as $doc_val=>$valor)
    {
      if($doc_val == "NUMERO DE FACTURA")
		list($prefijo,$factura_fiscal) = explode("-",$valor);
	  $salida .= "  <tr>\n";
  ...
  $filtros['numero']=$factura_fiscal;
  $factura_cabecera = $consulta->Listado_FacturasCliente($empresa_id,$filtros,'');
  ...
  /*INICIO*/ 
          ...
          $salida .= "<table width=\"90%\" border='1' align=\"center\" class=\"modulo_table_list\" rules=\"all\">\n";
                  ...
                  $IvaTotal+=($valor['total_costo_pedido']*(($valor['porcentaje_gravamen']/100)));
                  $ValorUnitario = ($valor['total_costo_pedido']/$valor['cantidad']);
				...
				$salida .= "                      </td>\n";
				$salida .= "                      <td align=\"left\">\n";
				$salida .= "                       $".FormatoValor(($valor['total_costo_pedido']*(($valor['porcentaje_gravamen']/100))),2);
				$salida .= "                      </td>\n";
				
				$subtotal += $valor['total_costo_pedido'];
				$salida .= "                      <td align=\"left\">\n";
				$salida .= "                       $".FormatoValor($valor['total_costo_pedido'],2);
				$salida .= "                      </td>\n";
                $salida .= "                    </tr>\n";
                $k++;
                  }
...
/*FIN*/
  ...
  $salida .= "<br>\n";
	...
	$retencion_fuente = $subtotal*($factura_cabecera[0]['porcentaje_rtf']/100);

	if($factura_cabecera[0]['subtotal']  >= $parametros_retencion['base_ica'])
	$retencion_ica = $subtotal*($factura_cabecera[0]['porcentaje_ica']/1000);

	if($factura_cabecera[0]['subtotal']  >= $parametros_retencion['base_reteiva'])
	$retencion_iva = $IvaTotal*($factura_cabecera[0]['porcentaje_reteiva']/100);
