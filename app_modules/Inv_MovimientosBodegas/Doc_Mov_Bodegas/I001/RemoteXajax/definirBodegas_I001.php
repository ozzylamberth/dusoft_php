<?php
	/**************************************************************************************
	* $Id: definirBodegas_I001.php,v 1.1 2009/07/17 19:08:17 johanna Exp $ 
...
{   $path = SessionGetVar("rutaImagenes");
    ...
    $pendientes=$consulta->ConsultaTmpPendientes($doc_tmp_id);
    $tmp_productos=$consulta->ConsultaTmpProdu($doc_tmp_id);
    
    $j=0;
    //foreach($pendientes as $valor=>$productos)
    //{ 
      //if($productos['codigo_producto']==$tmp_productos[$j]['codigo_producto'])
        //if(($productos['cantidad_pediente']-$tmp_productos[$j]['cantidad'])>$tmp_productos[$j]['existencia_minima'])
        //{
          //print_r("hola");
        //}
        //else
    ...
    //var_dump($buscar);
      ...
      $rst = $dca->DocumentoPedido(SessionGetVar("Empresa_id"),$productos['prefijo'],$productos['numero']);
      if(!$rst)
      {
        $objResponse->alert($dca->mensajeDeError);
      }
    if(!empty($productos))
    {
...
function MostrarProductox($bodegas_doc_id,$doc_tmp_id,$usuario_id)
{
 ...
 if(!empty($vector))
 {
   ...
   $salida .= "                      </td>\n";
   $salida .= "                      <td align=\"center\" width=\"10%\">\n";
   $salida .= "                        <a title='FECHA VENCIMIENTO'>FECHA VENCIMIENTOazzzz<a>";
   $salida .= "                      </td>\n";
   $salida .= "                      <td align=\"center\" width=\"10%\">\n";
   ...
   $salida .= "                    <tr class=\"modulo_list_claro\" onclick=\"mOvr(this,'#ffffff'); \" onMouseOver=\"mOvr(this,'#ffffff');\" onMouseOut=\"mOut(this,'#DDDDDD');\" >\n";
   $salida .= "                      <td align=\"right\" colspan=\"6\">\n";
...
function GuardarTmpDoc($bodegas_doc_id, $observacion, $tipo_id_tercero, $tercero_id, $documento_compra, $fecha_doc_compra)
{
    $objResponse = new xajaxResponse();
    //$objResponse->alert("EDITAR");
    $path = SessionGetVar("rutaImagenes");
    $consulta=new MovBodegasSQL();
    $consulta1=new doc_bodegas_I001();
	$FacturaProveedor = $consulta1->FacturaProveedor($tipo_id_tercero, $tercero_id, $documento_compra);
    if(empty($FacturaProveedor))
	$valor=$consulta1->CrearDoc($bodegas_doc_id, $observacion, $tipo_id_tercero, $tercero_id, $documento_compra, $fecha_doc_compra);
	else
		$objResponse->alert("EL NUMERO DE FACTURA YA SE ENCUENTRA REGISTRADO A ESTE PROVEEDOR!!");
...
//         $salida .="                         <td>\n";
      ...
      {
        $salida .= "               <div id=\"erro\" class='label_error' style=\"text-transform: uppercase; text-align:center; font-size:11px;\">\n";
        $salida .= "                No se encontraron resultados con ese tipo de descripciï¿½";       
        $salida .= "                </div>\n";       
      }
   if(count($vector)>0) 
   {
