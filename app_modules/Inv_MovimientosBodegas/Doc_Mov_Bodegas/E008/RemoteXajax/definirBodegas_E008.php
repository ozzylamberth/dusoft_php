...
include "../../../app_modules/Inv_MovimientosBodegas/Doc_Mov_Bodegas/E008/RemoteXajax/Services_JSON.php";
 ...
 * @return object
 */
function CrearDocumentoFinalx2($bodegas_doc_id, $doc_tmp_id, $tipo_doc_bodega_id, $identificador) {
   
    $i=0;
$sincronizar_dumian['cabecera']['nombre_tercero']="BODEGA DUMIAN CUCUTA";
$sincronizar_dumian['cabecera']['tipo_id_tercero']="NIT";
$sincronizar_dumian['cabecera']['tercero_id']="10122";
$sincronizar_dumian['cabecera']['prefijo']="EFC";
$sincronizar_dumian['cabecera']['numero']="12500010";                         
$sincronizar_dumian['cabecera']['nit']='805027743'; 

$sincronizar_dumian['detalle'][$i]['nombre']="JERINGA 20 ML 21 1 1/2 1 UNIDAD | CAJA X 50. LIFE CARE";
$sincronizar_dumian['detalle'][$i]['codigo_producto']="2111J000020005";
$sincronizar_dumian['detalle'][$i]['lote']="NQ20140210";
$sincronizar_dumian['detalle'][$i]['codigo_cum']="0";
$sincronizar_dumian['detalle'][$i]['codigo_invima']="0";
$sincronizar_dumian['detalle'][$i]['fecha_vencimiento']="2019-02-28";
$sincronizar_dumian['detalle'][$i]['cantidad']="100.0000";
$sincronizar_dumian['detalle'][$i]['valor_unitario']="173.34";
$sincronizar_dumian['detalle'][$i]['valor_total']="20107.44";

$sincronizar_dumian['detalle'][$i]['porcentaje_gravamen']="16.00";  
//error_reporting(E_ALL & ~E_NOTICE);
//ini_set("display_errors", 1);
// require_once("Services_JSON.php");
//$json = new Services_JSON();
//$json_remision = $json->encode($sincronizar_dumian, true);
//echo "<pre>"; print_r($json_remision);
$objResponse = new xajaxResponse();
error_reporting(E_ALL);
ini_set("display_errors", 1);
$cosmitet_ws = AutoCarga::factory("SincronizacionCosmitet", "SincronizacionCosmitet", "", "");
$resultado= $cosmitet_ws->ingresar_Despacho_Clientes($sincronizar_dumian,'2');                           
$objResponse->alert('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>');                        
$objResponse->alert($resultado['resultado']);                        
$objResponse->alert($resultado['parametros']);                        
                         
//                            echo "<pre>";
//print_r($resultado);
//                            echo "<pre>";
//print_r($sincronizar_dumian);

/*$sincronizar_dumian['detalle'][$i]['porcentaje_gravamen']="16.00";
$objResponse = new xajaxResponse(); 
 $cosmitet_ws = AutoCarga::factory("SincronizacionCosmitet", "SincronizacionCosmitet", "", "");
 $sincronizar_dumian = array('arreglo'=>$sincronizar_dumian);
 $resultado= $cosmitet_ws->ingresar_Despacho_Clientes($sincronizar_dumian,'2');
 $objResponse->alert($resultado['resultado']);                        
 $objResponse->alert($resultado['parametros']); */
...
function CrearDocumentoFinalx($bodegas_doc_id, $doc_tmp_id, $tipo_doc_bodega_id, $identificador) {
    ...
    if ($sirotulo) {
        ...
        //echo "<pre>"; print_r($productos_seleccionados);die; exit();
        //========================================sincronizar dumian============================================
                     $sincroniza = $consulta->ClientesSincroniza($pedido['tipo_id_tercero'],$pedido['tercero_id']);
                    if($sincroniza['sw_sincroniza']=='1'){                        
                        $i=0;
                        $sincronizar_dumian['cabecera']['nombre_tercero']=$sincroniza['nombre_tercero'];
                        $sincronizar_dumian['cabecera']['tipo_id_tercero']=$pedido['tipo_id_tercero'];
                        $sincronizar_dumian['cabecera']['tercero_id']=$pedido['tercero_id'];
                        $sincronizar_dumian['cabecera']['prefijo']=$productos['prefijo'];
                        $sincronizar_dumian['cabecera']['numero']=$productos['numero'];                         
                        $sincronizar_dumian['cabecera']['nit']='805027743';                               
                            
                           foreach ($productos_seleccionados['DETALLE'] as $producto) {
                                $sincronizar_dumian['detalle'][$i]['nombre']=$producto['nombre'];
                                $sincronizar_dumian['detalle'][$i]['codigo_producto']=$producto['codigo_producto'];
                                $sincronizar_dumian['detalle'][$i]['lote']=$producto['lote'];
                                $sincronizar_dumian['detalle'][$i]['codigo_cum']=$producto['codigo_cum'];
                                $sincronizar_dumian['detalle'][$i]['codigo_invima']=$producto['codigo_invima'];
                                $sincronizar_dumian['detalle'][$i]['fecha_vencimiento']=$producto['fecha_vencimiento'];
                                $sincronizar_dumian['detalle'][$i]['cantidad']=$producto['cantidad'];
                                $sincronizar_dumian['detalle'][$i]['valor_unitario']=$producto['valor_unitario_iva'];
                                $sincronizar_dumian['detalle'][$i]['valor_total']=$producto['valor_total_iva'];
                                $sincronizar_dumian['detalle'][$i]['porcentaje_gravamen']=$producto['porcentaje_gravamen'];    
        ...
        if ($identificador == "FM" && $pedido["farmacia_id"] == "01") {
            ...
            // Ingresar Cabecera
            $observacion_ws = "{$productos['prefijo']} - {$productos['numero']} ... {$productos['observacion']}";
            $resultado_ws_cabecera = $cosmitet_ws->ingresarDocumentoTemporal($pedido_farmacia['bodega'], 418, $observacion_ws);
            
            $registrar_log = $e8->registrar_log_despachos_ws($identificador, $pedido['solicitud_prod_a_bod_ppal_id'], SessionGetVar("empresa_id"), $productos['prefijo'], $productos['numero'], $e8->to_pg_array($resultado_ws_cabecera['parametros']), $e8->to_pg_array($resultado_ws_cabecera['resultado']), '0');            
            $resultado_ws_cabecera = $resultado_ws_cabecera['resultado'];
            
            if (count($resultado_ws_cabecera) > 0) {

                if ($resultado_ws_cabecera['estado']) {

                    $documentoTemporalFarmacia = $resultado_ws_cabecera['docTmpId'];
                   ...
                   // $productos_seleccionados = $consulta->SacarDocumento(SessionGetVar("empresa_id"), $productos['prefijo'], $productos['numero']);
                    ...
                    foreach ($productos_seleccionados['DETALLE'] as $producto) {
                        $resultado_ws = $cosmitet_ws->ingresarDetalleDocumentoTemporal($documentoTemporalFarmacia, $producto);
                        
                        $registrar_log = $e8->registrar_log_despachos_ws($identificador, $pedido['solicitud_prod_a_bod_ppal_id'], SessionGetVar("empresa_id"), $productos['prefijo'], $productos['numero'], $e8->to_pg_array($resultado_ws['parametros']), $e8->to_pg_array($resultado_ws['resultado']), '1');                                    
                        $resultado_ws = $resultado_ws['resultado'];
...
//se debe sincronizar la informacion con Dumian
                    if ($error) {
                        $objResponse->alert("Problemas al enviar los datos a cosmitet - ". $resultado_ws['descripcion']);
                        //$mjs_ws .= "\nProblemas al enviar los datos a cosmitet";
                    } else {
                        $objResponse->alert("Datos enviados correctamente a cosmitet");
                        //$mjs_ws .= "\nDatos enviados correctamente a cosmitet";
                    }
                } else {
                    // No se pudo crear Docuumento
                    $objResponse->alert("No se Pudo Ingresar Documento en Cosmitet por que - " . $resultado_ws['descripcion']);
                    //$mjs_ws .= "\nNo se Pudo Ingresar Documento en Cosmitet por que - " . $resultado_ws['descripcion'];
                }
            }
        ...
        if ($identificador == "CL" && $pedido["tipo_id_tercero"] == "CE" && $pedido["tercero_id"] == "10490") {

             
            // Sincronizar con Farmacia Cartagena
            $cosmitet_ws = AutoCarga::factory("SincronizacionCosmitet", "SincronizacionCosmitet", "", "");

            // Ingresar Cabecera
            $observacion_ws = "{$productos['prefijo']} - {$productos['numero']} ... {$productos['observacion']}";
            $resultado_ws_cabecera = $cosmitet_ws->ingresarDocumentoTemporal("BD", 418, $observacion_ws, '1');

            $registrar_log = $e8->registrar_log_despachos_ws($identificador, $pedido['pedido_cliente_id'], SessionGetVar("empresa_id"), $productos['prefijo'], $productos['numero'], $e8->to_pg_array($resultado_ws_cabecera['parametros']), $e8->to_pg_array($resultado_ws_cabecera['resultado']), '0');            
            $resultado_ws_cabecera = $resultado_ws_cabecera['resultado'];
            ...
            if (count($resultado_ws_cabecera) > 0) {
                ...
                if ($resultado_ws_cabecera['estado']) {
                    ...
                    $documentoTemporalFarmaciaCartagena = $resultado_ws_cabecera['docTmpId'];
                    ...
                    foreach ($productos_seleccionados['DETALLE'] as $producto) {
                        $resultado_ws = $cosmitet_ws->ingresarDetalleDocumentoTemporal($documentoTemporalFarmaciaCartagena, $producto, '1');
                        
                        $registrar_log = $e8->registrar_log_despachos_ws($identificador, $pedido['pedido_cliente_id'], SessionGetVar("empresa_id"), $productos['prefijo'], $productos['numero'], $e8->to_pg_array($resultado_ws['parametros']), $e8->to_pg_array($resultado_ws['resultado']), '1');                                    
                        $resultado_ws = $resultado_ws['resultado'];
                    ...
                    if ($error) {
                        $objResponse->alert("Problemas al enviar los datos a Farmacia Cartagena");
                        //$mjs_ws .= "\nProblemas al enviar los datos a Farmacia Cartagena";
                    } else {
                        $objResponse->alert("Datos enviados correctamente a Farmacia Cartagena");
                        //$mjs_ws .= "\nDatos enviados correctamente a Farmacia Cartagena";
                    }
                } else {
                    // No se pudo crear Docuumento
                    $objResponse->alert("No se Pudo Ingresar Documento en Farmacia Cartagena por que - " . $resultado_ws['descripcion']);
                    //$mjs_ws .= "\nNo se Pudo Ingresar Documento en Farmacia Cartagena por que - " . $resultado_ws['descripcion'];
                }
            }
        ...
        // ====================================== FIN SINCRONIZACION FARMACIAS CARTAGENA =========================
