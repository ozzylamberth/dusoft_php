...
function ListadoProductos_Formula($bodegas_doc_id,$doc_tmp_id,$formula_id,$formula_papel,$empresa_id,$codigo_barras,$descripcion,$tipo_paciente,$paciente_id)
{                       
    $objResponse = new xajaxResponse();
    $path = SessionGetVar("rutaImagenes");
    ...
    $ProductosDocumento = $consulta ->Productos_Formula($formula_papel,$empresa_id,$codigo_barras,$descripcion,$tipo_paciente,$paciente_id);
 ...
 *********************************************************************************/
    function ObtenerPaginadoter($pagina,$path,$slc,$op,$criterio1,$criterio2,$criterio,$div,$forma)
    {
      ...
      if ($TotalRegistros > 0)
      {
        ...
        $aviso .= "     Paginas;".$pagina." de ".$NumeroPaginas."</td>\n";
        $aviso .= "   </tr>\n";
...
*********************************************************************************/
function BuscarFormula($formula_papel,$empresa_id,$tipo_doc,$doc)
{
      $objResponse = new xajaxResponse(); 
      $path = SessionGetVar("rutaImagenes");
      $sql=new doc_bodegas_I013();
	  $formula=$sql->Formula_Buscada($formula_papel,$empresa_id,$tipo_doc,$doc);
     ...
     if(empty($formula))
		...
		//$objResponse->script("document.getElementById('documento_id').value=\"\";");
		//$objResponse->script("document.getElementById('tipoDocumento').selectedIndex ='0';");
		}
		else{
                      if($formula['estado_fecha']=='0'){
                          $objResponse->script("document.getElementById('formula_id').value=\"\";");
		          $objResponse->script("document.getElementById('formula_papel').value=\"\";");
                        $mensaje=" La Formula: ".$formula_papel." No Es Del Mes Actual.";
                       }else{
      ...
      $salida = utf8_decode($objResponse->setTildes($mensaje));
      $salida = $mensaje;
