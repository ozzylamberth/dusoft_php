...
class DispensacionHTML {
    ...
    function FormaMenu($action, $Planes) {
        ...
        $html .= "      <td  class=\"label\"  align=\"center\">\n";
        $html .= "       <a href='#'>\n";
        $html .= "       ..============ MODULO ACTUALMENTE EN REVISION INGRESE MAS TARDE ============</a><a href=\"" . $action['Formulas'] . "\">..</a>\n";
        $html .= "      </td>\n";
        $html .= "  </tr>\n";*/
        $html .= "      </table>\n";
        $html .= "    </td>\n";
      ...
      $html .= $ctl->RollOverFilas();
        if (!empty($datos)) {
            ...
            foreach ($datos as $key => $dtl) {
                ...
                if (count($medicamentos_dispensados) == 0) {
                    //$html .= "      <td> </td>\n";
                    $html .= "      <td> </td>\n";
                } else {

                    $imprimir_dispensacion_actual = $reporte->GetJavaReport('app', 'Dispensacion', 'MedicamentoDispensado', array("evolucion" => $dtl['evolucion_id'], "tipo_reporte" => 0), array('rpt_name' => '', 'rpt_dir' => 'cache', 'rpt_rewrite' => TRUE));
                    $funcion1 = $reporte->GetJavaFunction();
                    $html .= " " . $imprimir_dispensacion_actual . "\n";

                    $imprimir_dispensacion_completa = $reporte->GetJavaReport('app', 'Dispensacion', 'MedicamentoDispensado', array("evolucion" => $dtl['evolucion_id'], "tipo_reporte" => 1), array('rpt_name' => '', 'rpt_dir' => 'cache', 'rpt_rewrite' => TRUE));
                    $funcion2 = $reporte->GetJavaFunction();
                    $html .= " " . $imprimir_dispensacion_completa . "\n";

                    $imprimir_pendientes = $reporte->GetJavaReport('app', 'Dispensacion', 'MedicamentoDispensado', array("evolucion" => $dtl['evolucion_id'], "tipo_reporte" => 2), array('rpt_name' => '', 'rpt_dir' => 'cache', 'rpt_rewrite' => TRUE));
...
//echo "<pre>".$dtl['fecha_formulacion'];
...
//        $html .= "                 </table>";
    ...
    function FormaRegistrarEvento($action, $request, $datos, $informacion, $bandera, $msm, $evolucion) {
        ...
        $edad_t = htmlentities("AÃ‘OS", ENT_QUOTES,'UTF-8');
