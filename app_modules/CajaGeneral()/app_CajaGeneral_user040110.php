...
/**
 * $Id: app_CajaGeneral_user040110.php,v 1.4 2010/11/18 14:18:05 hugo Exp $
...
//FIN VALIDAR USUARIO DEVOLUCIONES
  ...
  */
  function CajaHospitalaria($Cuenta,$TipoId,$PacienteId,$Nivel,$PlanId,$Pieza,$Cama,$FechaC,$Ingreso,$FechaHoy)
  {
    ...
    function DatosFactura($cuenta,$PlanId,$TipoId,$PacienteId)
    {
                list($dbconn) = GetDBconn();
...
//                                     and d.departamento_actual=h.departamento";
              ...
              $query = "
                        ...
                        (
                            ...
                            WHERE
                            a.empresa_id = '".$_SESSION['FAC']['EMP_EMP']."'
                            AND a.prefijo = '".$_SESSION['FAC']['PRE_EMP']."'
                            AND a.factura_fiscal = ".$_SESSION['FAC']['NUM_EMP']."
                            AND b.empresa_id = '".$_SESSION['FAC']['EMP_EMP']."'
                            AND b.prefijo = '".$_SESSION['FAC']['PRE_EMP']."'
                            AND b.factura_fiscal = ".$_SESSION['FAC']['NUM_EMP']."
      ...
      if($_SESSION['CAJA']['TIPOCUENTA']=='04')
      {
        ...
        $TotalAbono=$_REQUEST['Apagar'];
            ...
            */
            ...
            $query = "SELECT
                            cuenta_tipo_id,tipo_numeracion,tipo_numeracion_devoluciones
                            FROM cajas
                            WHERE
                            caja_id='$Cajaid'
     ...
     $sqls="INSERT into recibos_caja(
        ...
        if($_SESSION['CAJA']['TIPOCUENTA']=='01')//hospitalizacion.
      {
        $consulta="INSERT INTO rc_detalle_hosp
                            ...
                            '$empresa','$cutilidad','$recibo','$prefijo','$Cuenta');";
      }
      ...
      elseif($_SESSION['CAJA']['TIPOCUENTA']=='03') //conceptos.
      {
        ...
        $consulta="INSERT INTO rc_detalle_conceptos
                            ...
                            detalle,
                            grupo_concepto,
                            valor from tmp_detalle_conceptos where cuenta='$Cuenta';";
      }
      elseif($_SESSION['CAJA']['TIPOCUENTA']=='04')//Consulta externa.
      {
        ...
        $bucon="UPDATE tmp_detalle_cargos_ambulatorios SET
         ...
         $consulta="INSERT INTO rc_detalle_cargos_ambulatorios
                            ...
                            valor_cubierto,
                             gravamen_valor_nocubierto
                            from tmp_detalle_cargos_ambulatorios where cuenta='$Cuenta';";
      }
       ...
       if($_SESSION['CAJA']['TIPOCUENTA']=='04')
      {
            ...
            $sql="INSERT INTO servicios_ambulatorios_autorizados
                ...
                $query="SELECT a.agenda_cita_asignada_id,a.cantidad,b.servicio_amb_id from
                      tmp_detalle_cargos_ambulatorios a,servicios_ambulatorios_autorizados b
      ...
      if($res->fields[0]>0)
      {
            $bucon="UPDATE tmp_cheques_mov SET  recibo_caja='$recibo', prefijo='$prefijo', estado='$estado'
            ...
            $cons1="INSERT INTO cheques_mov(
                                ...
                                usuario_id,
                                fecha_registro,
                                CASE WHEN fecha_cheque > DATE(now()) THEN '1' ELSE '0' END AS sw_postfechado from tmp_cheques_mov where numerodecuenta=$Cuenta;";
            ...
            $cons2="INSERT INTO  confirmacion_che(
                                    ...
                                    fecha,
                                    usuario_id
                                    FROM tmp_confirmacion_che where numerodecuenta=$Cuenta";
...
//FIN MODIFICACI� PARA GUARGAR EL NUMERO DOC
    ...
    */
    ...
    {
             $query = "SELECT sw_tipo_plan, sw_facturacion_agrupada FROM planes
      ...
      else
      {
        ...
        return false;
            ...
            if($TotalEmpresa>0 AND $DatPlan[sw_facturacion_agrupada]==0 AND ($DatPlan[sw_tipo_plan]==0 OR $DatPlan[sw_tipo_plan]==1))
            {
                        ...
                        $_SESSION['CAJA']['FACTURA']['EMPRESA']=array('prefijo'=>$Prefijo,'factura'=>$Factura,'tipoid'=>$_SESSION['CAJA']['AUX']['tipo_id_paciente'],'paciente'=>$_SESSION['CAJA']['AUX']['paciente_id'],'cuenta'=>$Cuenta);
...
if(empty($_SESSION['CAJA']['BONO'])){$_SESSION['CAJA']['BONO']=0;}
     ...
     $sqls="INSERT into fac_facturas_contado(
                        ...
                        '$empresa','$recibo','$cutilidad',
                        '$prefijo',$TotalAbono,
                        $efectivo,$cheque,$Ttarjeta,
                        '$TipoTercero','$Tercero',
                        $estado,'$FechaHoy',$usuario,
      ...
      if($res->fields[0]>0)
      {
            ...
            $cons2="INSERT INTO  confirmacion_chef(
                                    consecutivo,
                                    cheque_mov_id,
                                    entidad_confirma,
                                    funcionario_confirma,
