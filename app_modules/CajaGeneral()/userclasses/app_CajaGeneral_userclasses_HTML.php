 ...
 /**
    ...
    */
    function FormaEfectivo()
    {
            ...
            if(!$Efectivo || !$Total){
                    if(!$Efectivo){ $this->frmError["efectivo"]=1; }
                    if(!$Total){ $this->frmError["valorpagar"]=1; }
            ...
            $this->salida.="  <td><a href=\"$url_pos\">IMPRIMIR POS</a></td>";
                    ...
                    $this->salida.="</table>";
										...
										if ($tmp>0 AND $_SESSION['CAJA']['TIPOCUENTA']<>'06')
										{
											...
											$this->salida.="  <td>Total</td>";
											if(!$Recibo){ $this->salida.="  <td>PDF</td>";
										if($_SESSION['CAJA']['TIPOCUENTA']<>'06')
											$this->salida.="  <td>POS</td>";
											}
											$this->salida.="</tr>";
...
//                                                     }
              ...
              case "3": {
                          ...
                          if($_REQUEST['noautorizacion']){$a=$_REQUEST['noautorizacion'];}else{$a=$num;}
                                                  ...
                                                  $this->salida .='<tr>';
                                                    ...
                                                    $this->salida .='<td>&nbsp;</td>';
                                                    if($_REQUEST['autoriza']){$x=$_REQUEST['autoriza'];}else{$x=$fun;}
...
//FIN DECIDIR Y FORMAS PARA LOS ABONOS DE LOS PAGARE
    ...
    * @return boolean
    */
        function FormaCuenta($Cuenta,$TipoId,$PacienteId,$Nivel,$PlanId,$Pieza,$Cama,$FechaC,$Ingreso,$FechaHoy,$Tiponumeracion,$PagareNumero,$Cajaid,$NombrePaciente,$Empresa,$Prefijo,$Valor,$DocumentoId,$PlanId)
        { 
		      if($_SESSION['CAJA']['TIPOCUENTA']=='06')
					{
    ...
    */
    ...
    { 
                  ...
                  unset($_SESSION['CAJA']['NUMS']);//var de session de autorizacion de tar
									...
									if(!$Tefectivo){ $Tefectivo=0;}
									if(!$tarjetad){ $tarjetad=0;}
									if(!$tarjetac){ $tarjetac=0;}
									if(!$abono){ $abono=0;}
                ...
                if($tarjetad >0)
                {
                    $this->salida .= "<input type=\"text\" name=\"tarjetac\" class=\"input-text\" value=\"".FormatoValor($tarjetad)."\" size=\"15\" READONLY>&nbsp;&nbsp<a href=\"".ModuloGetURL('app','CajaGeneral','user','DecisionHospitalizacion',array('spy'=>4,'Cuenta'=>$Cuenta,'PlanId'=>$PlanId,'TipoId'=>$TipoId,'PacienteId'=>$PacienteId,'Ingreso'=>$Ingreso,'Nivel'=>$Nivel,'FechaC'=>$FechaC,'FechaHoy'=>'','Cajaid'=>$Cajaid))."\"><img src=\"". GetThemePath() ."/images/plata.png\" border='0' width='20' height='12'>&nbsp;ABONOS</a>&nbsp;&nbsp;|&nbsp;<a href=\"".ModuloGetURL('app','CajaGeneral','user','RevisarTemp',array('spy'=>4,'Cuenta'=>$Cuenta,'PlanId'=>$PlanId,'TipoId'=>$TipoId,'PacienteId'=>$PacienteId,'Ingreso'=>$Ingreso,'Nivel'=>$Nivel,'FechaC'=>$FechaC,'FechaHoy'=>$FechaHoy,'Cajaid'=>$Cajaid))."\">CONSULTA</a></td>";
                }
                else
                {
                    $this->salida .= "<input type=\"text\" name=\"tarjetac\" class=\"input-text\" value=\"".FormatoValor($tarjetad)."\" size=\"15\" READONLY>&nbsp;&nbsp<a href=\"".ModuloGetURL('app','CajaGeneral','user','DecisionHospitalizacion',array('spy'=>4,'Cuenta'=>$Cuenta,'PlanId'=>$PlanId,'TipoId'=>$TipoId,'PacienteId'=>$PacienteId,'Ingreso'=>$Ingreso,'Nivel'=>$Nivel,'FechaC'=>$FechaC,'FechaHoy'=>'','Cajaid'=>$Cajaid))."\"><img src=\"". GetThemePath() ."/images/plata.png\" border='0' width='20' height='12'>&nbsp;ABONOS</a></td>";
...
//[duvan]
       					$this->salida .= "        </tr>";
                $this->salida .= "        <tr class=modulo_list_oscuro >";
                $this->salida .= "<td class=modulo_table_title>TOTALdd</td><td><input type=\"text\" name=\"total\" class=\"input-text\" value=\"".FormatoValor($total)."\"  size=\"15\" READONLY></td>";
      				  $this->salida .= "        </tr>";
                $this->salida .= "        <tr >";
              ...
              // $this->salida .= "<pre>".print_r($_SESSION['CAJA']['AUX']['liq']['valor_total_paciente'],true)."</pre>";
...
function MostrarDatosMedico($consultorio,$nombre_profesional,$fecha,$estilo)
{
    ...
    */
  	...
  	{
      $request = $_REQUEST;
      if($request['auto']) SessionSetVar("AutorizacionCE",$request['auto']);
    ...
    {
								//IncludeLib("tarifario");
								$this->salida .= ThemeAbrirTabla('CAJA ORDENES DE SERVICIO8');
    ...
    */
    ...
    { 
        ...
        if(!$valores){$valores=$_REQUEST['arx'];}
						...
						$this->salida.="<tr><td class=\"modulo_table_title\">DETALLE CONCEPTOS - FACTURA $factura";//FACTURA CREDITO O CONTADO
						$this->salida.="</td></tr>";
						$this->salida.="</table>";
...
//FIN CONCEPTOS
					if(!empty($_REQUEST['concepto']))
					{
						...
						if($var2[0][sw_cantidad]==0)
							$this->salida.=" <td align='left'><input type='text' name='cantidad' size='10' class=\"input-text\" value=\"1\" readonly></td>";
						else
							$this->salida.=" <td align='left'><input type='text' name='cantidad' size='10' class=\"input-text\" value=\"1\"></td>";
						$_SESSION['CAJA']['CANTIDAD']=$_REQUEST['cantidad'];
						$this->salida.=" </tr>";
						$this->salida.=" <tr>";
						$this->salida.=" <td class=\"".$this->SetStyle("precio")."\">PRECIO: </td>";
						if($var2[0][sw_precio_manual]==0)
							$this->salida.=" <td align='left'><input type='text' name='precio' size='10' class=\"input-text\" value=\"".Formatovalor($var2[0][precio])."\" readonly></td>";
						else
							$this->salida.=" <td align='left'><input type='text' name='precio' size='10' class=\"input-text\" value=\"".Formatovalor($var2[0][precio])."\"></td>";
...
//CAMPO PARA EL GRAVAMEN
						...
						if($var2[0][sw_modificar_gravamen]==0)
							$this->salida.=" <td align='left'><input type='text' name='gravamen' size='2' class=\"input-text\" value=\"".Formatovalor($var2[0][porcentaje_gravamen])."\" readonly>%</td>";
						else
							$this->salida.=" <td align='left'><input type='text' name='gravamen' size='2' class=\"input-text\" value=\"".Formatovalor($var2[0][porcentaje_gravamen])."\">%</td>";
						$_SESSION['CAJA']['PRECIO']=$_REQUEST['precio'];
						$this->salida.=" </tr>";
//FIN CAMPO PARA EL GRAVAMEN
						$this->salida.=" <tr>";
						$this->salida.=" <td class=\"".$this->SetStyle("observacion")."\">OBSERVACION: </td>";
						$this->salida.=" <td align='left'><textarea name='observacion'></textarea></td>";
						$this->salida.=" </tr>";
						$this->salida.="</table>";
...
//'Cuenta'=>$Cuenta,'TipoId'=>$_TipoId,'PacienteId'=>$PacienteId,'PagareNumero'=>$PagareNumero,'Cajaid'=>$Cajaid,'Empresa'=>$Empresa,'Prefijo'=>$Prefijo,'Valor'=>$Valor,'NombrePaciente'=>$NombrePaciente,'DocumentoId'=>$DocumentoId,'PlanId'=>$PlanId
                ...
                if($_SESSION['CAJA']['TIPOCUENTA']=='03' || $_SESSION['CAJA']['TIPOCUENTA']=='04' || $_SESSION['CAJA']['TIPOCUENTA']=='05'|| $_SESSION['CAJA']['TIPOCUENTA']=='06' || $_SESSION['CAJA']['TIPOCUENTA']=='08')
                {
                        ...
                        $this->salida .= "  <tr><td><fieldset><legend class=\"field\">PAGO DE CAJA</legend>";
                        $this->salida.="<table  width='70%' align='center' border='0'>";
                        $this->salida.="<tr><td align='left' colspan=\"2\">";
                            ...
                            $this->salida .='<form name="recarga" action="'.ModuloGetURL('app','CajaGeneral','user','InsertarConceptos',array('spy'=>1,'Cuenta'=>$Cuenta,'Cama'=>$Cama,'PlanId'=>$PlanId,'TipoId'=>$TipoId,'PacienteId'=>$PacienteId,'Ingreso'=>$Ingreso,'Nivel'=>$Nivel,'FechaC'=>$FechaC,'FechaHoy'=>$FechaHoy,'efectivo'=>$efectivo,'cheque'=>$cheque,'tarjetad'=>$tarjetad,'tarjetac'=>$tarjetac,'Tiponumeracion'=>$Tiponumeracion,'TipoCuenta'=>$TipoCuenta,'Apagar'=>$total,'arx'=>$dat,'aPagar'=>$aPagar,'Devuelta'=>$Devuelta,'observacion'=>$observacion)).'" method="post">';
												}
												elseif($_SESSION['CAJA']['TIPOCUENTA']=='08')
												{
                        
														$total=str_replace(".","",$aPagar);
                            $this->salida .='<form name="recarga" action="'.ModuloGetURL('app','CajaGeneral','user','InsertarInventarios',array('spy'=>1,'Cuenta'=>$Cuenta,'Cama'=>$Cama,'PlanId'=>$PlanId,'TipoId'=>$TipoId,'PacienteId'=>$PacienteId,'Ingreso'=>$Ingreso,'Nivel'=>$Nivel,'FechaC'=>$FechaC,'FechaHoy'=>$FechaHoy,'efectivo'=>$efectivo,'cheque'=>$cheque,'tarjetad'=>$tarjetad,'tarjetac'=>$tarjetac,'Tiponumeracion'=>$Tiponumeracion,'TipoCuenta'=>$TipoCuenta,'Apagar'=>$total,'arx'=>$dat,'aPagar'=>$aPagar,'Devuelta'=>$Devuelta,'observacion'=>$observacion)).'" method="post">';
												}
												elseif($_SESSION['CAJA']['TIPOCUENTA']=='06')
												{
                         
														$total=str_replace(".","",$aPagar);
                            $this->salida .='<form name="recarga" action="'.ModuloGetURL('app','CajaGeneral','user','InsertarPagos',array('spy'=>1,'TipoId'=>$TipoId,'PacienteId'=>$PacienteId,'Cuenta'=>$Cuenta,'Cajaid'=>$Cajaid,'NombrePaciente'=>$NombrePaciente,'PagareNumero'=>$PagareNumero,'Empresa'=>$Empresa,'Prefijo'=>$Prefijo,'Valor'=>$Valor,'DocumentoId'=>$DocumentoId, 'efectivo'=>$efectivo,'cheque'=>$cheque,'tarjetad'=>$tarjetad,'tarjetac'=>$tarjetac,'devuelta'=>$Devuelta)).'" method="post">';
												}
												else
                        if($_SESSION['CAJA']['TIPOCUENTA']!='05')
                        {
                        ...
                        else
                        {
														...
														$total=str_replace(".","",$aPagar);
                            $this->salida .='<form name="recarga" action="'.ModuloGetURL('app','CajaGeneral','user','InsertarHospitalizacion2',array('spy'=>1,'Cuenta'=>$Cuenta,'Cama'=>$Cama,'PlanId'=>$PlanId,'TipoId'=>$TipoId,'PacienteId'=>$PacienteId,'Ingreso'=>$Ingreso,'Nivel'=>$Nivel,'FechaC'=>$FechaC,'FechaHoy'=>$FechaHoy,'efectivo'=>$efectivo,'cheque'=>$cheque,'tarjetad'=>$tarjetad,'tarjetac'=>$tarjetac,'Tiponumeracion'=>$Tiponumeracion,'TipoCuenta'=>$TipoCuenta,'Apagar'=>$total,'arx'=>$dat)).'" method="post">';
                        }
                        ...
                        if($Devuelta>=0)
                        {
                                ...
                                $this->salida.=" <td align=\"center\">";
                								...
                								if($_SESSION['CAJA']['TIPOCUENTA']=='03'){
                                        $this->salida .='<form name="forma" action="'.ModuloGetURL('app','CajaGeneral','user','FormaCuentaConceptos',array('arx'=>$dat)).'" method="post">';
																}
                                elseif($_SESSION['CAJA']['TIPOCUENTA']=='04')
                                {
                                        $this->salida .='<form name="forma" action="'.ModuloGetURL('app','CajaGeneral','user','FormaCuentaExterna',array('arx'=>$dat)).'" method="post">';
                                }
                                ...
                                elseif($_SESSION['CAJA']['TIPOCUENTA']=='06')
                                {
                                        $this->salida .='<form name="forma" action="'.ModuloGetURL('app','CajaGeneral','user','CajaPagares',array('Cuenta'=>$Cuenta,'TipoId'=>$_TipoId,'PacienteId'=>$PacienteId,'PagareNumero'=>$PagareNumero,'Cajaid'=>$Cajaid,'Empresa'=>$Empresa,'Prefijo'=>$Prefijo,'Valor'=>$Valor,'NombrePaciente'=>$NombrePaciente,'DocumentoId'=>$DocumentoId,'PlanId'=>$PlanId)).'" method="post">';
                                }
                								elseif($_SESSION['CAJA']['TIPOCUENTA']=='08'){
                                        $this->salida .='<form name="forma" action="'.ModuloGetURL('app','CajaGeneral','user','FormaCuentaInventarios',array('arx'=>$dat)).'" method="post">';
																}
                                $this->salida .="<input type=\"submit\" align=\"center\" name=\"Volver\" value=\"Volver\" class=\"input-submit\"></form>";
                                $this->salida.=" </td>";
                                $this->salida.=" </tr>";
                        ...
                        if($Devuelta < 0 AND $_SESSION['CAJA']['TIPOCUENTA']=='06')
                        {
													$this->salida.="<tr><td class=\"label\" align='center' colspan=\"2\">";
												...
												else
												{
													...
													$this->salida .='<form name="forma" action="'.ModuloGetURL('app','CajaGeneral','user','BorrarTemporales',array('Cuenta'=>$Cuenta,'arx'=>$dat)).'" method="post">';
    ...
    { 
         $datos=$this->DatosEncabezadoEmpresa($Caja,$tipocuenta,$tmp);
            if(!$Caja){  $var='CENTRO DE FACTURACION';  }
            else {  $var='CAJA';  }

							if($ancho)
							{$width="width=$ancho%";}else{$width="width=80%";}
...
/*						$accContado=ModuloGetURL('app','CajaGeneral','user','BusquedaTercer',array('Cajaid'=>$Caja,'Empresa'=>$empresa,'CentroUtilidad'=>$centro,'Tiponumeracion'=>$tipo,'TipoCuenta'=>$tipocuenta,'factura'=>'contado'));
						...
						$this->salida.="<tr align=\"center\">";
						if($tipocuenta=='03')
						{$cajatipo='CONCEPTO';}elseif($tipocuenta=='08'){$cajatipo='INVENTARIO';}
...
/*						$datos=$this->ConsultarConceptos();
						...
						if(empty($factura) and $tipocuenta=='03')
						{
							...
							$UsuarioConceptos=$UsuarioCpto->TraerUsuarios();
...
//							array('descripcion'=>$vectcuadre[$k][descripcion],'sw_recibo'=>1,'Caja'=>$vectcuadre[$k][caja_id],"Empresa"=>$Empresa,"dpto"=>$dpto,'usuario_id'=>$vectcuadre[$k][usuario_id],'dpto'=>$vectcuadre[$k][departamento],"CentroUtilidad"=>$CentroUtilidad,"TipoCuenta"=>$TipoCuenta,'sw_cuadrada'=>'1','cierre'=>$vectcuadre[$k][cierre_caja_id],'criterio'=>$sw));
								$acc1=ModuloGetURL('app','CajaGeneral','user','ListadoCerrarCaja',
								array('descripcion'=>$vect[$k][descripcion],'Caja'=>$vect[$k][caja_id],"Empresa"=>$Empresa,"dpto"=>$dpto,'usuario_id'=>$vect[$k][usuario_id],'dpto'=>$vect[$k][departamento],"CentroUtilidad"=>$CentroUtilidad,"TipoCuenta"=>$TipoCuenta,'criterio'=>$sw,'Caja_empresa'=>$Caja));
...
/*						$this->salida.="<tr class=\"$estilo\" >";
			...
			//VERIFICACION DE USUARIO
					...
					//$Caja,$Empresa,$CentroUtilidad,$arreglo,$TipoCuenta,$CU
					elseif($TipoCuenta=='03')
					{//$sw,$Caja,$Empresa,$CentroUtilidad,$arreglo,$TipoCuenta,$CU
	...
	{ 
        ...
        $this->EncabezadoEmpresa($Caja,'',$TipoCuenta); 
			...
			//VERIFICACION DE USUARIO
					...
					//$Caja,$Empresa,$CentroUtilidad,$arreglo,$TipoCuenta,$CU
					elseif($TipoCuenta=='03' OR $TipoCuenta=='08')
					{//$sw,$Caja,$Empresa,$CentroUtilidad,$arreglo,$TipoCuenta,$CU
						//$acc=ModuloGetURL('app','CajaGeneral','user','InsertarCierreCaja',array('Cajaid'=>$Caja,'Empresa'=>$vec[0][empresa_id],'CentroUtilidad'=>$vec[0][centro_utilidad],'Tiponumeracion'=>$tipo,'TipoCuenta'=>$tipocuenta,'user'=>$vec[0][usuario_id],'tef'=>$tef,'ttar'=>$ttar,'tche'=>$tche,'tefd'=>$tefd));
						$acc=ModuloGetURL('app','CajaGeneral','user','InsertarCuadreCaja',array('Caja'=>$Caja,'Empresa'=>$Empresa,'CentroUtilidad'=>$CentroUtilidad,'arreglo'=>$tipo,'CU'=>$CU,'TipoCuenta'=>$TipoCuenta,'tef'=>$te,'ttar'=>$tar,'tche'=>$che,'tbon'=>$tbon,'ta'=>$ta,'totaldev'=>$totaldev,'user'=>$vec[0][usuario_id],'tefd'=>$tefd));
						$volver=ModuloGetURL('app','CajaGeneral','user','MenuCajaConceptos',array('Caja'=>$Caja,'Empresa'=>$Empresa,'CentroUtilidad'=>$CentroUtilidad,'Tiponumeracion'=>$tipo,'TipoCuenta'=>$TipoCuenta));
     ...
     method=\"post\">";
		...
		*/
        ...
        $this->salida .= ThemeAbrirTabla($titulo,"50%")."<br>";
				...
				//$this->salida .= "<pre>".print_r($arreglo,true)."</pre>";
                                //$this->salida .= "<pre>".print_r($botonC."HOLA BOTON",true)."</pre>";
...
$mostrar.="    var nombre=\"\"\n";
						...
						$recibos_caja=$this->GetDatosFactura($arreglo['cuenta']);
                                                //$this->salida .= "<pre".print_r($recibos_caja,true)."</pre>";
						$funcion=$reporte->GetJavaFunction();
						...
						//GenerarFactura($var);
                                                //$this->salida .= "<pre".print_r($var."CLIENTES       ",true)."</pre>";
                                                //$this->salida .= "<pre".print_r($var1."PACIENTES",true)."</pre>";
					...
					if($botonC=='facturaconceptos')
					{
						...
						GenerarFactura($var,$arreglo['tipo_factura']);
					...
					{
							//$this->salida .= "<td align=\"left\"><input class=\"input-submit\" type=\"button\" name=\"Aceptar\" value=\"Imprimir\" onclick='abreVentana()'></td></tr>\n";
							...
							if($_SESSION['REF_DPTO'])
							{
								...
								$this->salida .= "<td colspan=\"1\" align=\"center\"><label><br><a href='$url2'>Generar Rollo fiscal Cr�dito</a></label></td></tr>\n";
							}
					}
				}
				else
				...
				if(!empty($boton))
                                {
						$this->salida .= "    <tr><td colspan=\"2\" align=\"left\"><input class=\"input-submit\" type=\"submit\" name=\"Aceptar\" value=\"<<$boton\"></td>\n";
				}
				else
                                {
						$this->salida .= "    <tr><td colspan=\"2\" align=\"left\">&nbsp;</td></tr>\n";
				}
				$this->salida .= "    </form>\n";
				$this->salida .= "</table>\n";
				$this->salida .= themeCerrarTabla();
