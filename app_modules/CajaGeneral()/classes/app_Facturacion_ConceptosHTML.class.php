...
/*        $html .= "               <tr>";
    ...
    function FormaBusquedaFacturasConceptosHTML($empresa,$arr)
    {
      ...
      $html .= ThemeCerrarTabla();
		...
		*/
		function FormaDetalleFacturaConcepto($fecha_registro,$factura_fiscal,$prefijo,$empresa,$nombre_tercero,$tipo_factura,$identificacion,$sw_clase_factura)
			{
					...
					$dat=$fact->DetalleFacturaConceptos($empresa,$prefijo,$factura_fiscal);
						...
						$html .= "      <td align=\"center\" class=\"modulo_table_title\" width=\"10%\">CANTIDAD</td>";
					...
					foreach($dat AS $i => $v)
					{
						...
						$html .= "      <td align=\"center\" class=\"modulo_list_claro\" width=\"10%\">".$v[cantidad]."</td>";
					...
					else
					{
						...
						$html .= "      <td align=\"left\" class=\"modulo_table_list\">".FormatoValor($arr[0][total_factura])."</td>";
						$html .= "   </tr>";
						$html .= "   <tr>";
...
/*			$html .= "					<form name=\"frmDescargarRips$envio_id\" action=\"$accion\" method=\"post\" target=\"_blank\">";
	  ...
	  function BuscarFacturas($empresa)
	  {
			...
			if(empty($_REQUEST['paso'])) $cantidad = 0;
			...
			if(empty($var) && $paso) $this->frmError['MensajeError'] = "LA BUSQUEDA NO ARROJO NINGUN RESULTADO";

			if(empty($_REQUEST['paso'])) $_SESSION['SPY'] = $fct->conteo;
