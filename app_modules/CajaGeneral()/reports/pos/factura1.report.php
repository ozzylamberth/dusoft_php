...
/**
 * $Id: factura1.report.php,v 1.5 2010/11/18 14:18:05 hugo Exp $
    ...
    */
    function CrearReporte()
    {
        ...
        $reporte->SaltoDeLinea();
				...
				if($datos[1][sw_tipo]==0)
				{
						if($datos[0][valor_cuota_paciente]>0)
						{   $reporte->PrintFTextoValor($datos[0][nombre_copago],$datos[0][valor_cuota_paciente],0,true,11,false,'left');  }
						if($datos[0][valor_cuota_moderadora]>0)
						{  $reporte->PrintFTextoValor($datos[0][nombre_cuota_moderadora],$datos[0][valor_cuota_moderadora],0,true,11,false,'left');  }
						if($datos[0][valor_nocubierto]>0)
						{  $reporte->PrintFTextoValor('Valor no Cubierto',$datos[0][valor_nocubierto],0,true,11,false,'left');  }
						$total=$datos[0][valor_total_paciente];
				}//factura cliente
				elseif($datos[1][sw_tipo]==1)
				{
						$reporte->PrintFTextoValor('Valor  Cubierto',$datos[0][valor_cubierto],0,true,11,false,'left');  
        ...
        $reporte->PrintFTextoValor('TOTAL CUENTA',$total,0,true,11,true,'right');
