<?php
  ...
  */ 
  /**
  * Clase : AgendaMedica
  * Clase para accesar los metodos privados de la clase de presentación,
  * se compone de metodos publicos para insertar en la base de datos, 
  * actualizar y borrar de la base de datos y mostrar
  * la forma de inserción y la consulta de la agenda medica.
  *
  * @package IPSOFT-SIIS
	...
	function LLegadaAutorizarPaciente()
	{
		...
		if(empty($_SESSION['AsignacionCitas']['cita']))
		{
			if(empty($_SESSION['CumplirCita']['cita']))
			{
				...
				{
					$this->FormaMensaje('No tiene autorizacion','Autorizaciï¿½',ModuloGetURL('app','AgendaMedica','','DatosPaciente'),'Volver');
					return true;
				}
			...
			else
			{
				if(empty($_SESSION['AUTORIZACIONES']['RETORNO']['Autorizacion']) OR empty($_SESSION['AUTORIZACIONES']['RETORNO']['plan_id']))
				{
					...
					$this->FormaMensaje('No tiene autorizacion','Autorizaciï¿½',ModuloGetURL('app','AgendaMedica','',$metodo),'Volver');
					unset($_SESSION['AUTORIZACIONES']);
					return true;
				}
				$_SESSION['DATOSPACIENTE']=$_SESSION['AUTORIZACIONES']['RETORNO']['ARREGLO'];
		...
		{//print_r($_SESSION['AUTORIZACIONES']['RETORNO']['plan_id']);
			...
			{
				$this->FormaMensaje('Se cancelo el proceso de autorizaciï¿½','Autorizaciï¿½',ModuloGetURL('app','AgendaMedica','','DatosPaciente'),'Volver');
				unset($_SESSION['AUTORIZACIONES']);
				return true;
			}*
    ...
    if(!empty($datos['autorizacion']['id_empleador']))
    {
      ...
      $_SESSION['EMPLEADOR']['tipo_empleador']=$datos['autorizacion']['tipo_empleador'];
		...
		else
		{
			if(empty($_SESSION['AUTORIZACIONES']['RETORNO']['Autorizacion']) OR empty($_SESSION['AUTORIZACIONES']['RETORNO']['plan_id']))
			{
				$this->FormaMensaje('Se cancelo el proceso de autorización','AutorizaciÃ³n',ModuloGetURL('app','AgendaMedica','','DatosPaciente'),'Volver');
				unset($_SESSION['AUTORIZACIONES']);
				return true;
			}
			$_SESSION['DATOSPACIENTE']=$_SESSION['AUTORIZACIONES']['RETORNO']['ARREGLO'];
		...
		$sql = "SELECT  $cita as id_cita, 
            ...
            FROM    agenda_citas_asignadas as a
                    ...
                    ON(e.numero_orden_id=f.numero_orden_id),
          					agenda_citas as b,
          					agenda_turnos as c
          					LEFT JOIN tipos_consultorios g 
                    ON(c.consultorio_id=g.tipo_consultorio),
          					terceros as d
...
/***********************************************************************************
  ...
  ************************************************************************************/
  ...
  function Get_datos_Adicionales($tipoidpaciente,$paciente)
  {
    ...
    if($dbconn->ErrorNo() != 0){
      $this->error = "Error al traer informaciï¿½ de la tabla pacientes_datos_adicionales ";
      $this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
      return false;
    }
