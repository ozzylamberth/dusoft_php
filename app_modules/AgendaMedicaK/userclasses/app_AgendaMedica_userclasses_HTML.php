    ...
    */
    ...
    $this->salida .= "  <tr class=\"".$clss."\">\n";
		...
		if($atencion>0)
		{
			...
			$this->salida .='<a href="'.$accion.'" class="label_error">Atención de Citas</a>';
		}
		else
		{
      ...
      $this->salida .= "  <tr class=\"".$clss."\">\n";
			...
			$this->salida .='<a href="'.$accion.'" class="label_error">Cancelación de Citas</a>';
			$this->salida .='</td>';
			$this->salida .='</tr>';
		}
...
*/
	...
	function PantallaOrdenes()
	{
		...
		$this->salida.="</td>";
		$this->salida.="<td>";
		$this->salida.="Fecha de Activaciï¿½";
		$this->salida.="</td>";
		$this->salida.="<td>";
    ...
    */
	function EscogerBusqueda($citaocupada)
	{
		...
		if(empty($_REQUEST['DiaEspe']) or date("Y-m-d")>date("Y-m-d",mktime(0,0,0,$a[1],$a[2],$a[0])))
		{
			...
			$this->salida  = ThemeAbrirTabla('FILTRO PARA ASIGNACIï¿½ DE CITA MEDICA');
			$this->salida .= "<BR>";
			$this->salida .= '<table width="80%" align="center" class="modulo_table_list">';
			$this->salida .= ' <tr align="center" class="modulo_table_title">';
      ...
      $this->salida .= "</table>";
			...
			$this->salida .= "                         <td class=\"label\">Año</td>\n";
      $this->salida .= "                         <td>\n";
      ...
      $this->salida .= "         <form name=\"formabuscar\" action=\"$accion\" method=\"post\">\n";
			...
			$this->salida .= "               <td>\n";
			$this->salida .= "                 <fieldset>\n";
			$this->salida .= "                   <legend class=\"normal_10AN\">BUSQUEDA AVANZADA</legend>";
        ...
        if(empty($_SESSION['AsignacionCitas']['profesional']) )
          ...
          if(empty($_REQUEST['TipoBusqueda']))
  				...
  				else
  				{
  					...
  					$this->salida .='<tr>';
  					$this->salida .='<td align="center">';
  					$this->salida.='<label class="label_error">No existen profesionales para este dï¿½.</label>';
  					$this->salida .='</td>';
  					$this->salida .='</tr>';
  					$this->salida .='</table>';
  					$this->salida .='<br>';
          ...
          //fin
  				...
  				if(!empty($hisant))
  				{
  					...
  					$salida .=" >    Solicitar Historia Clï¿½ica";
  					$salida .= "</td>";
  					$salida .= "</table>";
  				}else{
            ...
            foreach($difcita as $k=>$v)
            {
              ...
              $this->salida .= "}";
  						...
  						$this->salida .= " }\n";
  						$this->salida .= "window.location.href='".ModuloGetUrl('app','AgendaMedica','','EscogerBusqueda',array('profesional'=>$_REQUEST['profesional'],'nompro'=>$_REQUEST['nompro'],'DiaEspe'=>$_REQUEST['DiaEspe']))."&dato='+p+'&chequeo='+checkeado+'&datosArreglo='+arreglo;\n";
  						$this->salida .= "}\n";
  						$this->salida .= "}\n";
  						$this->salida .= "</script>\n";
...
/**
    ...
    */
      ...
      if($_REQUEST['Citas']['sw_vertodosplanes']==0)
      ...
      if($a)
      {
        ...
        $jsc .= "</script>\n";
  		...
  		$this->SetXajax(array("ListaProgramasPYP","Seleccion","ValidarPaciente","TiposAfiliados","ValidarPlanAtencion","ValidarCitasPrioridades","BuscarCargos","AdicionarCargo"),null,"ISO-8859-1");
      ...
      if($_SESSION['AsignacionCitas']['cita']==$tiempopara['tipo_consulta_id'])
      {
        ...
        $this->salida .= "          <a href=\"#validar1\" onclick=\"xajax_ValidarCitasPrioridades(xajax.getFormValues('formabuscar'))\" >\n";
    ...
    //Cambio Muestra Citas por Cumplir
      ...
      $this->salida .= "</table>\n";
		...
		if(!empty($atencion))
		{
			...
			$salida.="<td>";
			$salida.="Atencion Accidentes de Trï¿½sito";
			$salida.="</td>";
		...
		$this->salida .='	{';
		$this->salida .='		alert("Fecha Erronea!.  La fecha calculada supera los 140 años.");$obj.FechaNacimiento.focus();$obj.FechaNacimiento.value="";return false;';
		$this->salida .='	}';
...
*/
	...
	function PantallaFinal($orden,$msg)
	{
		...
		if(!empty($_SESSION['AsignacionCitas']['cuantascajas']))
		{
			...
			$this->salida .= '<td align="center">';
			//$this->salida.="<a href=\"$accion\">Pago en Caja Rï¿½ida</a>";
		...
		if(!empty($_REQUEST['Documento']) or !empty($_REQUEST['nombres']))
		{
            ...
            $this->salida.=$Cita[2][$i];
                ...
                $vec1['Semanas']=$Cita[12][$i];
						...
						{
							$this->salida.='<label class="label_error">Su cita no es para el dï¿½ de hoy</label>';
						}
						$this->salida.='</td>';
						$this->salida.='</tr>';
					}
...
*/
    ...
    SessionDelVar("Cagos_Adicionados".UserGetUID());
		...
		$this->salida .= "    <td width=\"20%\">Acción</td>";
		$this->salida .= "  </tr>";
		$i=0;
    ...
    $z = 0;
			...
			if($AgendaCitaPadre!=$AgendaCitaPadreAnt || empty($AgendaCitaPadre) || ($AgendaCitaPadre==$AgendaCitaPadreAnt && !empty($AgendaCitaPadre) && $datos[3][$i].' '.$datos[2][$i]!=$identificacionAnt))
      {
        ...
        $this->salida .= "  <tr class=\"".(($spy==0)? "modulo_list_claro":"modulo_list_oscuro")."\">\n";
          ...
          $this->salida .= $datos[7][$i];
          if($datos[26][$i])
            $this->salida .= "  <label class=\"normal_10AN\" >EL PACIENTE ".(($datos[26][$i] == '2')? "NO":"")." REQUIERE ANESTESIÓLOGO</label>";
          $this->salida .= "  </td>\n";
				}
...
*/
    ...
    */
      ...
      $file = 'app_modules/AgendaMedica/RemoteXajax/NotasAdministrativas.php';
  		...
  		if($turnosdia==true)
      {
        ...
        while($i<sizeof($turnosdia[0]))
        {
          ...
          if($AgendaCitaIdPadre!=$AgendaCitaIdPadreAnt || empty($AgendaCitaIdPadre) || ($AgendaCitaIdPadre==$AgendaCitaIdPadreAnt && !empty($AgendaCitaIdPadre) && $turnosdia[2][$i].' '.$turnosdia[3][$i]!=$identificacionAnt))
          {
            ...
            $this->salida .= '</td>';
            $this->salida .= "  <td class=\"label_error\" rowspan=\"".$conta."\" align=\"center\">";
            ...
            if(!empty($nom))
            {
              ...
              {
                $Imagen = "<img title=\"Adición de Nota Administrativa\" src=\"". GetThemePath() ."/images/news.gif\" border=\"0\">";
              }
            ...
            if($turnosdia[14][$i])
              $this->salida .= "  <label class=\"normal_10AN\" >EL PACIENTE ".(($turnosdia[14][$i] == '2')? "NO":"")." REQUIERE ANESTESIÓLOGO</label>";
            ...
            if($_SESSION['Atencion']['DiaEspe']==date("Y-m-d"))
            {
              ...
              {
            		$variable = ModuloGetVar('app','AgendaMedica','MenuAsignacionCitas');
            		if($variable)
                {
              		if(trim($turnosdia[10][$i]) != 'ASIGNACION DE CITA AUTOMATICA')
                  {	
              		...
              		else
                  {
              			if(!empty($turnosdia[7][$i]))
                    {
              				if($turnosdia[8][$i]==1)
                      {
              					$this->salida.="<td rowspan=\"$conta\" align=\"center\" width=\"10%\" colspan=\"2\">";
              					$accion=ModuloHCGetURL($turnosdia[7][$i],'','',$_SESSION['Atencion']['hc_modulo'],$_SESSION['Atencion']['hc_modulo'],array('HC_DATOS_CONTROL'=>array('DEPARTAMENTO'=>$_SESSION['Atencion']['departamento'],'ESPECIALIDAD'=>$_SESSION['Atencion']['especialidad'])));
              					$this->salida.='<a href="'.$accion.'">Continuar Atenciï¿½</a> ';
              					$this->salida.=' - '.$turnosdia[7][$i];
              					$this->salida.='</td>';
                                              	$entro=0;
              				}
          ...
          if($entro==1)
          {
            $AcionNota = ModuloGetUrl('app','AgendaMedica','user','LlamarIngresarNota',array('ingreso'=>$turnosdia[6][$i],'evolucion'=>$turnosdia[7][$i],'nombre'=>'E'));
          ...
          {
            $this->salida .= "<div id = 'nota$horaCita' style=\"cursor:pointer\">";
            $this->salida .= $Imagen;
            //$this->salida .= "<img title=\"Adiciï¿½ de Nota Administrativa\" src=\"". GetThemePath() ."/images/news.gif\" border=\"0\">";
            $this->salida .= "</div>";
            //$this->salida.="<a href=\"javascript:Iniciar();MostrarVentana('d2Container');\"><img title=\"Adiciï¿½ de Nota Administrativa\" src=\"". GetThemePath() ."/images/edita.png\" border=\"0\"></a>";
          }
          else
          {
            $this->salida.="&nbsp";
      ...
      else
      {
  			...
  			$this->salida.='<tr>';
  			$this->salida.='<td align="center">';
  			$this->salida.='<label class="label_error">No existen citas programadas para este dï¿½</label>';
  			$this->salida.='</td>';
  			$this->salida.='</tr>';
  			$this->salida.='</table>';
  			$this->salida.='<br>';
      ...
      $action = ModuloGetURL('app','AgendaMedica','','AgendaDia',$vec);

  		$this->salida .= '<table align="center" width="80%" border="0">';
  		$this->salida .= '  <tr align="center">';
  		$this->salida .= "    <td align=\"left\">\n";
  		$this->salida .= "      <form name=\"actulaizar\" action=\"".$action."\" method=\"post\">\n";
  		$this->salida .= "        <input type=\"submit\" class=\"input-submit\" name=\"actualizar\" value=\"Refrescar\">\n";
  		$this->salida .= "      </form>\n";
  		$this->salida .= "    </td>\n";
  		$this->salida .= '    <td align="center" width="55%">';
    ...
    */
	function MuetraOcupaciones()
	{
		...
		$this->salida.='<label class="label">ELIJA OCUPACIï¿½ DEL PACIENTE: </label>';
		$this->salida.='</td>';
  ...
  */
	function CalendarioAtencion()
	{
		...
		$this->salida .='<tr align="center">';
		$this->salida .="<td class=\"label\">Aï¿½</td><td><select name=\"year\" onchange=\"year1(this.form)\" class=\"select\">";
		if(empty($_REQUEST['year']))
		{
			$a=explode("-",$_SESSION['CITASMES'][0]);
			$year=$_REQUEST['year']=$a[0];
...
*/
	...
	function BuscarPacienteCancelar($mensaje,$arr,$f)
	{
		...
		$this->salida = ThemeAbrirTabla('CANCELACIï¿½ DE CITAS');
		$this->salida .= "<BR>";
		$this->salida .= '<table width="80%" align="center" class="modulo_table_list">';
		$this->salida .= '<tr align="center" class="modulo_table_title">';
		$this->salida .= '<td align="center">';
...
*/
	...
	function DatosAdicionalesBorrarCita()
	{
		...
		$this->salida = ThemeAbrirTabla('DATOS ADICIONALES CANCELACIï¿½ DE CITAS');
		$this->salida .= "<BR>";
		$this->salida .= '<table width="80%" align="center" class="modulo_table_list">';
		$this->salida .= '<tr align="center" class="modulo_table_title">';
		$this->salida .= '<td align="center">';
		...
		$this->salida.='<tr class="modulo_list_claro">';
		$this->salida.='<td width="40%">';
		$this->salida.='<label class="'.$this->SetStyle("Observacion").'">TIPO DE CANCELACIï¿½: </label>';
		$this->salida.='</td>';
		$this->salida.='<td width="60%" align="center">';
...
/**
	...
	*/
	...
	function MesesAgenda($Seleccionado='False',$Aï¿½,$Defecto)
	{
		$anoActual=date("Y");
		$vars[1]='ENERO';
    $vars[2]='FEBRERO';
...
/**
    ...
    */
  	function FormaAsignacionCitaAutomatica($msg)
  	{
  			...
  			$this->salida .= "	<table width=\"50%\" align=\"center\" >\n";
  			$this->salida .= "		<tr>\n";
  			$action1 = ModuloGetURL('app','AgendaMedica','user','Menu');
    ...
    /**
    * Funcion donde se crea la forma para la reimpresion de citas
    *
    * @return boolean
    */
    function FormaImprimirCitas()
    {
      $request = $_REQUEST;
      IncludeClass("ClaseUtil");
      $mdl = AutoCarga::factory("PacientesHTML","views","app","AgendaMedica");
      $cts = AutoCarga::factory("Citas","classes","app","AgendaMedica");
      
      $tiposDocumentos = $cts->ObtenerTiposIdPaciente();
      $listado = array();
      if($request['buscar'])
        $listado = $cts->ObtenerListadocitas($request);
      
      $dtbuscador['tipo_id_paciente'] = $request['tipo_id_paciente'];
      $dtbuscador['paciente_id'] = $request['paciente_id'];
      $dtbuscador['paciente_id'] = $request['paciente_id'];
      $dtbuscador['nombres'] = $request['nombres'];
      $dtbuscador['apellidos'] = $request['apellidos'];
      $dtbuscador['fecha_cita'] = $request['fecha_cita'];
      $dtbuscador['buscar'] = $request['buscar'];
      
      $action['buscar'] = ModuloGetURL('app','AgendaMedica','user','FormaImprimirCitas');
      $action['volver'] = ModuloGetURL('app','AgendaMedica','user','main');
      $action['paginador'] = $action['buscar'].URLRequest($dtbuscador);
      
      $this->salida = $mdl->FormaImpresionCitas($action,$request, $tiposDocumentos,$listado, $cts->conteo, $cts->pagina);
      return true;
    }
  }
?>