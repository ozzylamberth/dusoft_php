<?php
	/**************************************************************************************
	* $Id: Facturar.class.php,v 1.2 2010/11/25 18:24:34 hugo Exp $
	* @copyright (C) 2005 IPSOFT - SA (www.ipsoft-sa.com)
	* @package IPSOFT-SIIS
	* 
	* $Revision: 1.2 $ 	
	*
	* Manejador de la parte l�ica del proceso de facturaci� e Impresi�
	***************************************************************************************/
  IncludeClass('app_Facturacion_Permisos','','app','Facturacion_Fiscal');
	class Facturar
	{
		...
		function ValidarUsuarioAjuste($usuario_siis, $usuario_validar,$pwd_validar)
		{
				...
				{
					$this->mensaje="USUARIO SIN AUTORIZACI� PARA REALIZAR AJUSTE";
					return false;
				}
		}
  ...
  */
  function AjustarCuenta(&$FormaMensaje)
  {
			...
			elseif($_REQUEST[REQUEST])
			{
				...
				$MotivoId = $dat[2];
				if($_REQUEST['Saldo'] > 0)
				{  ModuloSetVar('app','Facturacion_Fiscal','CargoDescuento',$Cargo);  }
				elseif($_REQUEST['Saldo'] < 0)
				{  ModuloSetVar('app','Facturacion_Fiscal','CargoAprovechamiento',$Cargo);  }
				$Saldo = ($_REQUEST['Saldo']);
				$FechaRegistro = date("Y-m-d H:i:s");
...
//         {  $Cargo=ModuloGetVar('app','Facturacion_Fiscal','CargoAprovechamiento');  }
        ...
        if(!$result->EOF)
        {
              ...
              return true;
				...
				if(!$result->EOF)
				{
					$sql = "UPDATE cuentas_ajustes SET valor= $Saldo,
												motivos_ajuste_cuenta_id = '$MotivoId',
...
// 					return false;
  ...
  { 
        ...
        $FechaRegistro=date("Y-m-d H:i:s");
				...
				//
				$tipos = SessionGetVar("Cuenta_documento_id");
        
				$fct = new app_Facturacion_Permisos();
				$fact = $fct->DatosFactura($arreglo[0][empresa_id],$tipos,$_REQUEST[PuntoFacturacion]);
                         ...
                         if($facturaExiste!=1){
                            ...
                            $query="INSERT INTO fac_facturas
                                           ...
                                           VALUES        ('$EmpresaId',
                                                                ...
                                                                '$FechaRegistro',
                                                                '$PlanId',
                                                                '$tipoTerceroFacPaciente',
                                                                '$idTerceroFacPaciente',
                                                                0,
                                                                ".$PrefijoCon.",
                                                                '$swtipo')";
                            $result=$dbconn->Execute($query);
...
//print_r($arreglo);
                        if($arreglo[0]['valor_nocubierto']>0 AND $abono >0)
	...
	function CerrarCuenta($arreglo,&$RetornoMetodo)
  {
      ...
      IncludeLib('funciones_facturacion');
        ...
        if($fila[0]['registros']>0)
            {
                $mensaje="HAY INSUMOS PENDIENTES DESDE LA ESTACION QUE SE ENCUENTAR EL PACIENTE";
                //$this->Facturacion();
                $accion=SessionGetVar('ActionVolver');
                $RetornoMetodo->FormaMensaje($mensaje,'FACTURAR CUENTA',$accion,'ACEPTAR');
                return true;
			...
			$query = "SELECT numerodecuenta_corte
								...
								WHERE numerodecuenta=$Cuenta";
			if(!$rst1 = $this->ConexionBaseDatos($query)) return false;
			//FIN CORTE DE CUENTAS
			...
			if($rst->fields[0] == '1' OR $rst1->fields[0])
			{
      ...
      {
        $accion=SessionGetVar('ActionVolver');
        $mensaje='La Cuenta No '.$Cuenta.' no se puede Cuadrar: La cuenta posee pagares en valor cero.';
        $RetornoMetodo->FormaMensaje($mensaje,'ERROR AL CUADRAR LA CUENTA',$accion,'ACEPTAR');
        return true;
      }
  ...
  function DatosFactura($Empresa,$Prefijo,$Factura,$Cuenta)
  {
    ...
    */
    ...
    function Get_Info_NotasOperatorias($ingreso)
    {
      ...
      function PermisosActivarCuenta($usuario_id)
      {
          list($dbconn) = GetDBconn();
          $query="SELECT count(*) 
                  FROM usuarios_activacion_cuentas_cuadradas
                  WHERE usuario_id = $usuario_id;";
          $result=$dbconn->Execute($query);
          if ($dbconn->ErrorNo() != 0) {
            $this->error = "Error al Cargar el Modulo (usuarios_activacion_cuentas_cuadradas)";
            $this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
            return false;
          }
      ...
      function ActivarCuentaCuadrada(&$obj,$Cuenta)
      {
          list($dbconn) = GetDBconn();
          ...
          $result=$dbconn->Execute($query);
          if ($dbconn->ErrorNo() != 0) {
            $this->error = "Error al Cargar el Modulo (cuentas)";
            $this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
            return false;
          }
          $query="INSERT INTO auditoria_activacion_cuentas_cuadradas
                  (
                    numerodecuenta,
                    fecha_registro, 
                    usuario_id
          ...
          $result=$dbconn->Execute($query);
          if ($dbconn->ErrorNo() != 0) {
            $this->error = "Error al Cargar el Modulo (auditoria_activacion_cuentas_cuadradas)";
            $this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
            return false;
          }
        $mensaje = 'La cuenta'.$Cuenta.' se ha activado.';
        $accion=SessionGetVar('ActionVolver');;
        $obj->FormaMensaje($mensaje,'ACTIVAR LA CUENTA',$accion,'ACEPTAR');
        return true;
