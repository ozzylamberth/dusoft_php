...
class Consultas extends ConexionBD {
    ...
    function obtenerUrlMoleculas($destino) {
        ...
        if ($destino == '0') {
            ...
            $url_produccion = "http://10.0.0.62/DUMIAN/ws/codificacion_productos/ws_molecula.php?wsdl";
            ...
            //Sincronizar con CMS
            $url_produccion = "http://10.0.0.62/CMS/ws/codificacion_productos/ws_molecula.php?wsdl";
            $url_pruebas = "http://10.0.1.80/dumian/CMS/ws/codificacion_productos/ws_molecula.php?wsdl"; // Pruebas
            return ($produccion) ? $url_produccion : $url_pruebas;
        }else if ($destino == '5') {
            //Sincronizar con PEÑITAS
            $url_produccion = "http://10.60.1.11/dusoft/ws/codificacion_productos/ws_molecula.php?wsdl";
            $url_pruebas = "http://10.0.0.3/pg9/sincelejo/ws/codificacion_productos/ws_molecula.php?wsdl"; // Pruebas
            return ($produccion) ? $url_produccion : $url_pruebas;
        }else if ($destino == '6') {
            //Sincronizar con CARTAGENA
            $url_produccion = "http://10.245.1.140/dusoft/ws/codificacion_productos/ws_molecula.php?wsdl";
            $url_pruebas = "http://10.0.0.3/pg9/cartagena/dusoft/ws/codificacion_productos/ws_molecula.php?wsdl"; // Pruebas
            return ($produccion) ? $url_produccion : $url_pruebas;
        }
    }  
    ...
    function obtenerUrlLaboratorios($destino) {
        ...
        if ($destino == '0') {
            ...
            //Sincronizar con Dumian
            $url_produccion = "http://10.0.0.62/DUMIAN/ws/codificacion_productos/ws_laboratorio.php?wsdl";
            ...
            //Sincronizar con CMS
            $url_produccion = "http://10.0.0.62/CMS/ws/codificacion_productos/ws_laboratorio.php?wsdl";
            $url_pruebas = "http://10.0.1.80/dumian/CMS/ws/codificacion_productos/ws_laboratorio.php?wsdl"; // Pruebas
            return ($produccion) ? $url_produccion : $url_pruebas;
        }else if ($destino == '5') {
            //Sincronizar con PEÑITAS
            $url_produccion = "http://10.60.1.11/dusoft/ws/codificacion_productos/ws_laboratorio.php?wsdl";
            $url_pruebas = "http://10.0.0.3/pg9/sincelejo/ws/codificacion_productos/ws_laboratorio.php?wsdl"; // Pruebas
            return ($produccion) ? $url_produccion : $url_pruebas;
        }else if ($destino == '6') {
            //Sincronizar con CARTAGENA
            $url_produccion = "http://10.245.1.140/dusoft/ws/codificacion_productos/ws_molecula.php?wsdl";
    ...
    function BuscarProductos($criterio, $termino, $offset){
        ...
        $sql =  "SELECT i.codigo_producto AS codigo_producto, ip.descripcion as nombre_producto, fc_descripcion_producto_molecula(i.codigo_producto) as molecula_producto, ip.periodo_dispensacion as periodo_dispensacion
    ...
    function ActualizarPeriodoDispensacion($codigo_producto, $periodo_dispensacion, $nuevo_periodo_dispensacion){
        $usuario_id = UserGetUID();

        $sql = "BEGIN;
                UPDATE inventarios_productos SET
                periodo_dispensacion = " . $nuevo_periodo_dispensacion . "
                WHERE  codigo_producto= '" . $codigo_producto . "'; 
                INSERT INTO log_actualizacion_periodo_dispensacion (codigo_producto, usuario_id, fecha, anterior, actual) VALUES ('".$codigo_producto."', ".$usuario_id.", now(), ".$periodo_dispensacion.", ".$nuevo_periodo_dispensacion.");
                COMMIT;";
        if (!$rst = $this->ConexionBaseDatos($sql))
            return false;
        $rst->Close();
    ...
    function ConsultarLogActualizacionesPeriodoDispensacion($codigo_producto, $offset){
        $sql = "SELECT su.nombre as nombre_usuario, lapr.anterior, lapr.actual, lapr.fecha
                FROM log_actualizacion_periodo_dispensacion  lapr JOIN
