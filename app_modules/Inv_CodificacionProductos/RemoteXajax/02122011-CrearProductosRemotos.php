...
/**
 ...
 function Productos_Creados($param,$offset)
 {
    ...
    $html .= "      <td width=\"10%\">FACTOR DE CONVERSIÓN</td>\n";
    ...
    foreach($productos as $key => $prod)
    {
     ...
     // $html .= "      <td>".$prod['descripcion']." ".$prod['presentacion']." </td>";
      ...
      //<Inicia aqui>
      $html .= "  <td align=\"center\">\n";
                    $javaAccionSuministros = " MostrarCapa('ContenedorFactorConversion'); IniciarCapaSum('CREAR FACTOR DE CONVERSIÓN','ContenedorFactorConversion');CargarContenedor('ContenedorFactorConversion'); xajax_MostrarMedicamentosF('$prod[codigo_producto]');"; 
...
//  Aqui trae el contenido de la función MostrarMedicamentosF
    $html .= "    </div>\n";
    $html .= "</div>\n";
    //<Termina aqui>
...
*/
 ...
 $token=$sql->asignar($Tabla,$Valor,$CodigoProducto,$campos);
 return $objResponse;
 } 
 ...
 if(!empty($Producto))
 $html = "Codigo de Barras ya está Asignado";
 else
 ...
 * @return array.
 */
 function ModificarProductoInsumo($datos,$parametro='0')
  {
  $objResponse = new xajaxResponse();
  $sql = AutoCarga::factory("ConsultasCrearProductos","classes","app","Inv_CodificacionProductos");
 ...
 * @return array.
 */
 function InsertarProductoInsumo($datos)
  {
  $objResponse = new xajaxResponse();
  $sql = AutoCarga::factory("ConsultasCrearProductos","classes","app","Inv_CodificacionProductos");
...
// $token=$sql->Insertar_ProductoInsumo($datos); 
  ...
  */
  ...
  $Titular=$sql->BusquedaTitular($Nombre);
     ...
     $SelectTitular = '<SELECT NAME="titular_reginvima_id" SIZE="1" class="select" style="width:40%;height:40%">';
 ...
 $sql = AutoCarga::factory("ConsultasCrearProductos","classes","app","Inv_CodificacionProductos");
  ...
  $SelectViasAdministracion = '<SELECT NAME="sin_asignarvias" SIZE="4" class="select" style="width:100%;height:100%">';
			...
			{
				$SelectViasAdministracion.= "<OPTION VALUE='".$viad['via_administracion_id']."' ondblclick=\"xajax_InsertarViadxProd('".$viad['via_administracion_id']."','".$CodigoProducto."')\">".$viad['nombre']."</OPTION>";
			}
 ...
 function ListaViasAdministracionxProducto($CodigoProducto)
  ...
  $SelectViasAdministracionX = '<SELECT NAME="viasadministracion" SIZE="4" class="select" style="width:100%;height:100%">';
  ...
  */
  ...
  function ModProducto($CodigoProducto,$Sw_Medicamento)
  {
		...
		$SelectPerfilesTerapeuticos = '<SELECT NAME="cod_anatofarmacologico" SIZE="1" class="select" style="width:100%;height:100%">';
		...
		$SelectUnidadesMedida = '<SELECT NAME="unidad_id" SIZE="1" class="select" style="width:40%;height:40%">';
		...
		$html .= "      <td align=\"left\" width=\"30%\">";
		$html .=        $prod['subclase'];
		$html .= "		<input type=\"hidden\" name=\"subclase_id\" value=\"".$prod['subclase_id']."\">";
		...
		$html .= "      </td>";
		$html .= "      <td align=\"left\" width=\"30%\">";
		$html .= "      <input class=\"input-text\" type=\"text\" name=\"descripcion\" size=\"30\" value='".$prod['descripcion']."' onkeyup=\"this.value=this.value.toUpperCase()\">";
		$html .= "      </td>";
		$html .= "      </tr>";
		...
		if($prod['sw_pos']==1)
		{
		$html .= '      <input class="input-radio" type="radio" name="sw_pos" value="1" checked> Si 
		<input class="radio" type="radio" name="sw_pos" value="0"> No </td>';
		}
		else
		{
		$html .= '      <input class="input-radio" type="radio" name="sw_pos" value="1"> Si 
		<input class="radio" type="radio" name="sw_pos" value="0"  checked> No </td>';
		}
		$html .= "      </tr>";
		...
		$html .= "      </td>";
		$html .= "      <td align=\"left\" width=\"30%\">";
		$html .= "      <input class=\"input-text\" type=\"text\" name=\"codigo\" size=\"20\" value='".$prod['cod_acuerdo228_id']."' onkeyup=\"xajax_BusquedaAcuerdo228(this.value),this.value=this.value.toUpperCase()\">";
		$html .= "      <div id=\"acuerdo_228\"></div>";
		$html .= "      <input type=\"hidden\" name=\"cod_acuerdo228_id\" value=''>";
		$html .= "      </td>";
		$html .= "      </tr>";
		...
		$html .= '      <input type="hidden" name="sw_medicamento" value="'.$Sw_Medicamento.'">';
		$html .= "      <input class=\"input-submit\" type=\"button\" value=\"Enviar\" name=\"boton\" onclick=\"Confirmar_1(xajax.getFormValues('FormularioProducto'))\">";
		$html .= "      </td>";
		$html .= "      </tr>";
		$html .= "		</form>";
		$html .= "      </table>";
 ...
 return $objResponse;
  ...
  function Guardar_NuevaClasificacion($Formulario)
  {
    ...
    {
    $objResponse->script("alert(¡¡Exito Al Modificar¡¡);"); 
  ...
  */
  ...
  function IngresoProducto($CodigoGrupo,$CodigoClase,$NombreClase,$NombreGrupo,$CodSubClase,$NombreSubClase,$Sw_Medicamento)
  {
			...
			$SelectPerfilesTerapeuticos = '<SELECT NAME="cod_anatofarmacologico" SIZE="1" class="select" style="width:100%;height:100%">';
			foreach($PerfilesTerapeuticos as $key => $PerfilesTerapeuticos)
			{
			$SelectPerfilesTerapeuticos .= '<OPTION VALUE="'.$PerfilesTerapeuticos['codigo'].'">'.$PerfilesTerapeuticos['descripcion'].'-'.$PerfilesTerapeuticos['codigo'].' </OPTION>';
			}
			$SelectPerfilesTerapeuticos .='</SELECT>';
			...
			$SelectUnidadesMedida = '<SELECT NAME="unidad_id" SIZE="1" class="select" style="width:40%;height:40%">';
			foreach($UnidadesMedida as $key => $Unidades)
			{
			$SelectUnidadesMedida .= '<OPTION VALUE="'.$Unidades['codigo'].'">'.$Unidades['descripcion'].'-'.$Unidades['codigo'].'</OPTION>';
			}
			$SelectUnidadesMedida .='</SELECT>';
			...
			foreach($TratamientosProductos as $key =>$tt)
			{
			$selectT .= "		<option value=\"".$tt['tratamiento_id']."\">";
			$selectT .= "			".$tt['descripcion'];
			$selectT .= "		</option>";
			}
			$selectT .= "</select>";
			...
			$html .= "      </td>";
			$html .= "      <td align=\"left\" width=\"30%\">";
			$html .= "      <input class=\"input-text\" type=\"text\" name=\"codigo\" size=\"20\" onkeyup=\"xajax_BusquedaAcuerdo228(this.value),this.value=this.value.toUpperCase()\">";
			$html .= "      <div id=\"acuerdo_228\"></div>";
			$html .= "      <input type=\"hidden\" name=\"cod_acuerdo228_id\" value=''>";
			$html .= "      </td>";
			$html .= "      </tr>";
  ...
  */
  ...
  function ModProductoMedicamento($Formulario_Productos)
  {
	...
	$PresentacionComercial=$sql->Listar_Presentacion_Comercial();
	...
	if($PresentacionComercial['codigo'] == $medicamento[0]['cod_forma_farmacologica'])
	$SelectPresentacionComercial .=" selected ";

	$SelectPresentacionComercial .='>'.$PresentacionComercial['codigo']." ".$PresentacionComercial['descripcion'].'</OPTION>';
	}
	$SelectPresentacionComercial .='</SELECT>';
	...
	$html .= "      <tr class=\"modulo_table_list_title\">";
	$html .= "      <td align=\"center\" colspan=\"2\">";
	$html .= "      Creacion de Producto -> Medicamento";
	$html .= "      </td>";
	$html .= "      </tr>";
	...
	$html .= "      </tr>";
	/*  
	if($_REQUEST['datos']['sw_tipo_empresa']=='0')
	{
	$disabled="disabled";
	$hidden = "<input type=\"hidden\" name=\"cod_principio_activo\" value=\"".$Formulario_Productos['subclase_id']."\">";
	}
	else
	{
	$disabled ="";
	...
	$html .= "      </td>";
	$html .= "      <td align=\"left\" width=\"30%\">";
	$html .= "      <input class=\"input-text\" type=\"text\" name=\"concentracion\" value='".$medicamento[0]['concentracion_forma_farmacologica']."'>";
	$html .= "      </td>";
	$html .= "      </tr>";
	...
	/*$html .= "      <tr class=\"modulo_list_claro\">";
	$html .= "      <td class=\"formulacion_table_list\" align=\"center\" width=\"10%\">";
	$html .= "      Via De Administracion :";
	$html .= "      </td>";
	$html .= "      <td align=\"left\" width=\"30%\">";
	$html .=        $SelectViasAdministracion;
	$html .= "      </td>";
	$html .= "      </tr>";*/
	...
	$html .= '      <input class=\"input-checkbox\" type="checkbox" name="NAI" value="1" onclick="asignar(this.value)" ';
	...
	$html .= '      <input class=\"input-checkbox\" type="checkbox" name="NAII" value="2" onclick="asignar(this.value)" ';
	...
	$html .= '      <input class=\"input-checkbox\" type="checkbox" name="NAIII" value="3" onclick="asignar(this.value)" ';
	...
	$html .= '      <input class=\"input-checkbox\" type="checkbox" name="NAIV" value="4" onclick="asignar(this.value)" ';
	...
	$html .= '      <input class=\"input-checkbox\" type="checkbox" name="NUE" value="02" onclick="incluir(this.value)"';
	...
	$html .= ' checked ';
	$html .= '      > ESPECIAL<br>';
	$html .= '      <input class=\"input-checkbox\" type="checkbox" name="NUG" value="03" onclick="incluir(this.value)"';
	...
	$html .= ' checked ';
	$html .= '       > GENERAL<br>';
	$html .= '      <input class=\"input-checkbox\" type="checkbox" name="NUC" value="04" onclick="incluir(this.value)"';
	...
	$html .= "      </td>";
	$html .= "      <td align=\"left\" width=\"10%\">";
	$html .= "      Especialidades a Asignar (Doble Click para Asignar)";
	$html .='        <div id="especialidad"></div>';
	$html .= "      Especialidades Asignadas (Doble Click para Borrar)";
	$html .='        <div id="especialidades_asignadas"></div>';
	...
	*/
	...
	$objResponse->script("incluir(0);");
	$objResponse->script("asignar(0);");
	//LLamado a funcion Javascript que muestre la ventanita (en capa) con Ajax
	return $objResponse;
  } 
  ...
  */
  ...
  $objResponse = new xajaxResponse();
   ...
   $SelectPresentacionComercial = '<SELECT NAME="cod_forma_farmacologica" SIZE="1" class="select" style="width:60%;height:60%">';
				foreach($PresentacionComercial as $key => $PresentacionComercial)
			{
				$SelectPresentacionComercial .= '<OPTION VALUE="'.$PresentacionComercial['codigo'].'">'.$PresentacionComercial['codigo'].' '.$PresentacionComercial['descripcion'].'</OPTION>';
			}
			$SelectPresentacionComercial .='</SELECT>';
...
/* 
  ...
  $selectcodconcentracion .="</select>"; 
   ...
   $html .= "      <tr class=\"modulo_list_claro\">";
    ...
    $html .= "      <tr class=\"modulo_list_claro\">";
		...
		$html .= "      </td>";
		$html .= "      <td align=\"left\" width=\"10%\">";
		$html .= '      <input class=\"input-checkbox\" type="checkbox" name="NAI" value="1" onclick="asignar(this.value)"> NIVEL I<br>';
    $html .= '      <input class=\"input-checkbox\" type="checkbox" name="NAII" value="2" onclick="asignar(this.value)"> NIVEL II<br>';
    $html .= '      <input class=\"input-checkbox\" type="checkbox" name="NAIII" value="3" onclick="asignar(this.value)"> NIVEL III<br>';
    $html .= '      <input class=\"input-checkbox\" type="checkbox" name="NAIV" value="4" onclick="asignar(this.value)"> NIVEL IV<br>';
    $html .= '      <input type="hidden" name="nivel_i" value=""   >';
    $html .= '      <input type="hidden" name="nivel_ii" value=""  >';
    $html .= '      <input type="hidden" name="nivel_iii" value="" >';
    $html .= '      <input type="hidden" name="nivel_iv" value="" >';
    ...
    $html .= "      <tr class=\"modulo_list_claro\">";
		...
		$html .= "      </td>";
		$html .= "      <td align=\"left\" width=\"10%\">";
		$html .= '      <input class=\"input-checkbox\" type="checkbox" name="NUH" value="01" onclick="incluir(this.value)"> HOSPITALARIO<br>';
    $html .= '      <input class=\"input-checkbox\" type="checkbox" name="NUE" value="02" onclick="incluir(this.value)"> ESPECIAL<br>';
    $html .= '      <input class=\"input-checkbox\" type="checkbox" name="NUG" value="03" onclick="incluir(this.value)"> GENERAL<br>';
    $html .= '      <input class=\"input-checkbox\" type="checkbox" name="NUC" value="04" onclick="incluir(this.value)"> CONTROL<br>';
    $html .= '      <input type="hidden" name="nivelu_h" value="">';
    $html .= '      <input type="hidden" name="nivelu_e" value="">';
    $html .= '      <input type="hidden" name="nivelu_g" value="">';
    $html .= '      <input type="hidden" name="nivelu_c" value="">';
 ...
 function Productos_CreadosBuscados($Grupo_Id,$Clase_Id,$SubClase_Id,$Descripcion,$CodAnatofarmacologico,$CodigoBarras,$codigo_producto,$offset)
 {
 $objResponse = new xajaxResponse();
  $sql = AutoCarga::factory("ConsultasCrearProductos","classes","app","Inv_CodificacionProductos");
  
  $productos=$sql->Lista_Productos_CreadosBuscados($Grupo_Id,$Clase_Id,$SubClase_Id,$Descripcion,$CodAnatofarmacologico,$CodigoBarras,$codigo_producto,$offset);
...
//      $objResponse->alert($Grupos);
 ...
 function MostrarMedicamentosF($CodMedicamento){
    ...
    if (count($Grupos) > 0){
      ...
      $html .= "                    </tr>\n";
      $html .= "                    <tr class=\"modulo_table_list_title\">\n";
      $html .= "                        <td width= '20%' class=\"formulacion_table_list\" align = 'left'>CÓDIGO</td>\n";
      $html .= "                        <td width= '50%' align = 'left'>".$Grupos[0][codigo_producto]."</td>\n";
      $html .= "                    </tr>\n";
      $html .= "                    <tr class=\"modulo_table_list_title\">\n";
      $html .= "                        <td width= '20%' class=\"formulacion_table_list\" align = 'left'>DESCRIPCIÓN</td>\n";
      ...
      $html .= "                  <table>\n";
      $html .= "                    <tr class=\"modulo_table_list_title\">\n";
      $html .= "                        <td width= '120%' class=\"formulacion_table_list\" align = 'center' colspan = 3>INSERTAR FACTOR DE CONVERSIÓN</td>\n";
      $html .= "                    </tr>\n";
      $html .= "                    <tr class=\"modulo_table_list_title\">\n";
      $html .= "                        <td width= '40%' class=\"formulacion_table_list\" align = 'center'>Seleccione Unidad de Dosificación</td>\n";
      $html .= "                        <td width= '40%' class=\"formulacion_table_list\" align = 'center'>Ingrese el Factor de Conversión</td>\n";
...
//document.getElementById('UniDos').value
      $html .= "                        <td width= '40%' align = 'center'><input type='button' class='input-submit' name='InsFac' value='Guardar' onclick=\"var cadena=''; if (document.getElementById('UniDos').value == '') cadena = '* Debe selecionar unidad de dosificación \\n'; if((document.getElementById('CanFacCon').value == '') || (document.getElementById('CanFacCon').value == '0')) cadena += '* Debe ingresar unidad minima de conversión'; if (cadena != '') alert(cadena); if (cadena == '') xajax_InsertarFacCon(document.getElementById('FacUniFac').value, document.getElementById('FacCodPro').value, document.getElementById('UniDos').value, document.getElementById('CanFacCon').value);if (cadena == '') xajax_MostrarMedicamentosF(document.getElementById('FacCodPro').value);\"></td>\n";
      $html .= "                    </tr>\n\n\n\n\n\n";
      ...
      $html .= "                        <td width= '190' class=\"formulacion_table_list\">Unidad Dosificación</td>\n";
      $html .= "                        <td width= '130' class=\"formulacion_table_list\">Factor de Conversión</td>\n";
      ...
      $html .= "                        <td width= '40%' align = 'center'><input type = 'text' name = 'CanFacCon' id = 'CanFacCon'></td>\n";
//      $html .= "                        <td width= '40%' align = 'center'><input type='button' class='input-submit' name='InsFac' value='Insertar' onclick=\"xajax_InsertarFacCon(document.getElementById('FacUniFac').value, document.getElementById('FacCodPro').value, document.getElementById('UniDos').value, document.getElementById('CanFacCon').value);xajax_MostrarMedicamentosF(document.getElementById('FacCodPro').value);\"></td>\n";
      $html .= "                        <td width= '40%' align = 'center'><input type='button' class='input-submit' name='InsFac' value='Insertar' onclick=\"var cadena=''; if (document.getElementById('UniDos').value == '') cadena = '* Debe selecionar unidad de dosificación \\n'; if((document.getElementById('CanFacCon').value == '') || (document.getElementById('CanFacCon').value == '0')) cadena += '* Debe ingresar unidad minima de conversión'; if (cadena != '') alert(cadena); if (cadena == '') xajax_InsertarFacCon(document.getElementById('FacUniFac').value, document.getElementById('FacCodPro').value, document.getElementById('UniDos').value, document.getElementById('CanFacCon').value);if (cadena == '') xajax_MostrarMedicamentosF(document.getElementById('FacCodPro').value);\"></td>\n";
      $html .= "                    </tr>\n\n\n\n\n\n";
