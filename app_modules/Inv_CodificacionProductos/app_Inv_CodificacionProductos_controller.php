...
//            $action['volver'] = ModuloGetURL("app","Inv_CodificacionProductos","controller","AsignarEspecialidadesAMedicamentos");
    ...
    function DiasTratamientos(){
        $request = $_REQUEST;
        $objHtml2 = AutoCarga::factory("Formularios_HTML", "views", "app", "Inv_CodificacionProductos");
        $productos = array();
        $criterio = isset($request['criterio'])? $request['criterio'] : '';
        $termino = isset($request['termino'])? trim($request['termino']) : '';
        $conteo = 0;
        $pagina = 0;
        $paginador = ModuloGetURL("app","Inv_CodificacionProductos","controller","DiasTratamientos", array('criterio'=> $criterio, 'termino'=> $termino));
        ...
        if(isset($request['buscar']) || (isset($offset) && !empty($offset))){
            $objSql = AutoCarga::factory("Consultas", "", "app", "Inv_CodificacionProductos");
        ...
        $this->salida = $objHtml2->FormularioBusquedaProductos($productos, $criterio, $termino, $conteo, $pagina, $paginador, $offset);

        return true;
    }


    function ActualizarPeriodoDispensacion(){
        $request = $_REQUEST;
        $objHtml2 = AutoCarga::factory("Formularios_HTML", "views", "app", "Inv_CodificacionProductos");
        $objSql = AutoCarga::factory("Consultas", "", "app", "Inv_CodificacionProductos");
        $paginador = ModuloGetURL("app","DiasTratamientos","controller","Inv_CodificacionProductos", array(
                                                                                                                    'criterio'=> $request['criterio'],
                                                                                                                    'termino' => $request['termino'],
                                                                                                                    'codigo_producto' => $request['codigo_producto'],
                                                                                                                    'nombre_producto' => $request['nombre_producto'],
                                                                                                                    'molecula_producto' => $request['molecula_producto'],
                                                                                                                    'periodo_dispensacion' => $request['periodo_dispensacion'],
                                                                                                                    'offsetBusqueda' => $request['offsetBusqueda']
                                                                                                                ));

        if(isset($request['periodo_dispensacion']) && (!empty($request['periodo_dispensacion']) || $request['periodo_dispensacion'] == "0")){
            $rs = $objSql->ActualizarPeriodoDispensacion($request['codigo_producto'], $request['periodo_dispensacion'], $request['nuevo_periodo_dispensacion']);
        }

        $logs = $objSql->ConsultarLogActualizacionesPeriodoDispensacion($request['codigo_producto'], $request['offset']);
        $this->salida = $objHtml2->FormularioActualizacionPeriodoDispensacion($request['nuevo_periodo_dispensacion'], $logs, $objSql->conteo, $objSql->pagina, $paginador);
