	...
	**************************************************************************************/
    ...
    function ActivarTaller($Taller, $Estado)
    {
     ...
     function CrearProgramacionT($fecha, $intervalo, $HoraInicio, $HoraFin, $Nsesiones, $Taller, $ubicacion)
     {
      ...
      if($dbconn->ErrorNo() != 0)
      {
        $error = "Error al Consultar Secuencia";
        $mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
        $dbconn->RollbackTrans();
        $objResponse->alert($mensajeDeError);
        return $objResponse;
      }
      ...
      if($dbconn->ErrorNo() != 0)
      {
        $error = "Error al Insertar en hc_psicologia_roles_profesionales";
        $mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
        $dbconn->RollbackTrans();
        $objResponse->alert($mensajeDeError);
        return $objResponse;
      ...
      for($i = 0; $i < $Nsesiones; $i++)
      {
        $query = "INSERT INTO hc_psicologia_talleres_programaciones_sesiones (taller_id,
               ...
               if($dbconn->ErrorNo() != 0) 
               {
                    $error = "Error al Cargar el Modulo";
                    $mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
                    $objResponse->alert($mensajeDeError);
                    return $objResponse;
               }
