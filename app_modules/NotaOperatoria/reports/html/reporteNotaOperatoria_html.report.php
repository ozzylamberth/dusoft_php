	...
	//*******************************************termino	
		...
		if($datos){			
			...
			$Salida .= "            <tr>";
                        ...
                        $Salida .= "            <td width=\"20%\" class=\"normal_10N\">&nbsp;</td>";
                        $Salida .= "            <td width=\"30%\" align=\"left\" class=\"normal_10\">&nbsp;</td>";
                        $Salida.= "     </tr>";	
			$Salida .= "		<tr>";
			...
			for($j=0;$j<sizeof($procedimientos);$j++){
				...
				$Salida.="  <td align=\"left\">".$procedimientos[$j]['profesional']."&nbsp;&nbsp;&nbsp;<label class=\"normal_10N\">TP:</label> ".$procedimientos[$j]['tarjeta_profesional']."</td>";																			
				$Salida.="</tr>";	
				if($procedimientos[$j]['observaciones']){
					$Salida.="<tr class=\"normal_10\">";
			...
			if($datos['diag_nom'] || $datos['diag_nom1'] || $datos['diag_nom2']){
				...
				/*if($datos['diag_nom2']){	
					$Salida .= "  <tr>";
					$Salida .= "  <td width=\"15%\" class=\"normal_10N\">PRE QX</td>";
					$Salida .= "  <td align=\"left\" class=\"normal_10\">".$datos['diag_nom2']."</td>";				
					$Salida .= "  <td width=\"15%\" class=\"normal_10N\">TIPO</td>";
					if($datos['diag_nom2'] == '1'){
						$Salida.="<td class=\"normal_10N\" align=\"center\" width=\"10%\">ID</td>";
					}elseif($datos['diag_nom2'] == '2'){
			...
			$Tecnicas=$this->DescripcionTecnicaQX($datos['ingreso']);
			if($Tecnicas){
				$Salida .= "  <table border=\"1\" width=\"95%\" align=\"center\">";
			...
			$Hallazgos=$this->HallazgosQX($datos['ingreso']);
			if($Hallazgos){
				$Salida .= "  <table border=\"1\" width=\"95%\" align=\"center\">";
			...
			$materialesPatologicos=$this->RegistroPatologias($datos['ingreso']);
			if($materialesPatologicos){
				...
				for($j=0;$j<sizeof($materialesPatologicos);$j++){
					if($materialesPatologicos[$j]['descripcion_envio_patologico']){					
						$Salida .= "  <tr>";
						if($materialesPatologicos[$j]['envio_patologico']==1){
							$Salida .= "  <td class=\"normal_10N\">".$materialesPatologicos[$j]['nombre_tercero']." - MATERIAL ENVIADO A PATOLOGIA:&nbsp;&nbsp;&nbsp;SI</td>";						
						}else{
							$Salida .= "  <td class=\"normal_10N\">".$materialesPatologicos[$j]['nombre_tercero']." - MATERIAL ENVIADO A PATOLOGIA:&nbsp;&nbsp;&nbsp;NO</td>";						
						}
						$Salida .= "  </tr>";
						$Salida .= "  <tr>";					
						$Salida .= "  <td class=\"normal_10\"><label class=\"normal_10N\">CLASE DE MATERIAL ENVIADO:</label><BR>".$materialesPatologicos[$j]['descripcion_envio_patologico']."</td>";						
						$Salida .= "  </tr>";
					}
				}
				$Salida.= "		</table><BR>";							
			...
			$cultivos=$this->RegistroCultivos($datos['ingreso']);
			if($cultivos){
				$Salida .= "  <table border=\"1\" width=\"95%\" align=\"center\">";
				$Salida .= "  <tr class=\"normal_10N\"><td align=\"center\">CULTIVOS</td></tr>";								
				for($j=0;$j<sizeof($cultivos);$j++){					
					if($cultivos[$j]['descripcion_envio_cultivo']){
						$Salida .= "  <tr>";
						if($cultivos[$j]['envio_cultivo']==1){
							$Salida .= "  <td class=\"normal_10N\">".$cultivos[$j]['nombre_tercero']." - CULTIVO ENVIADO:&nbsp;&nbsp;&nbsp;SI</td>";						
						}else{
							$Salida .= "  <td class=\"normal_10N\">".$cultivos[$j]['nombre_tercero']." - CULTIVO ENVIADO:&nbsp;&nbsp;&nbsp;NO</td>";						
						}					
						$Salida .= "  </tr>";
						$Salida .= "  <tr>";
						$Salida .= "  <td class=\"normal_10\"><label class=\"normal_10N\">DESCRIPCION DEL CULTIVO:</label><br>".$cultivos[$j]['descripcion_envio_cultivo']."</td>";						
						$Salida .= "  </tr>";
					}	
				}					
				$Salida.= "		</table><BR>";							
...
//*****************************************fin de termino
	...
	function ConsultaNotasOperatoriasRealizadas($programacion,$tipoidpaciente,$paciente){
		...
		ter2.nombre_tercero as anestesiologo,ter3.nombre_tercero as ayudante,a.evolucion_id,evol.ingreso,ciru.nombre as cirujano
		...
		LEFT JOIN profesionales ciru ON (ciru.tipo_id_tercero=a.tipo_id_cirujano AND ciru.tercero_id=a.cirujano_id)
	...
	function DescripcionTecnicaQX($ingreso){
		list($dbconn) = GetDBconn();
		...
		a.usuario_id=prof.usuario_id AND 
	...
	function RegistroPatologias($ingreso){
		list($dbconn) = GetDBconn();
		$query ="SELECT a.envio_patologico,a.descripcion_envio_patologico,ter.nombre_tercero
		FROM hc_notas_operatorias_cirugias a,hc_evoluciones b,profesionales_usuarios prof,terceros ter
		WHERE b.ingreso='".$ingreso."' AND a.evolucion_id=b.evolucion_id AND
		a.usuario_id=prof.usuario_id AND 
		prof.tipo_tercero_id=ter.tipo_id_tercero AND prof.tercero_id=ter.tercero_id";
		$result = $dbconn->Execute($query);
		if($dbconn->ErrorNo() != 0){
	...
	function RegistroCultivos($ingreso){
		list($dbconn) = GetDBconn();
		$query ="SELECT a.envio_cultivo,a.descripcion_envio_cultivo,ter.nombre_tercero
		FROM hc_notas_operatorias_cirugias a,hc_evoluciones b,profesionales_usuarios prof,terceros ter
		WHERE b.ingreso='".$ingreso."' AND a.evolucion_id=b.evolucion_id AND
		a.usuario_id=prof.usuario_id AND 
		prof.tipo_tercero_id=ter.tipo_id_tercero AND prof.tercero_id=ter.tercero_id";
