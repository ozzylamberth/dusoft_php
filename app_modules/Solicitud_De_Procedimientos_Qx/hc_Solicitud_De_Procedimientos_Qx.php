...
//clzc - si
	...
	function Busqueda_Avanzada()
  {
    ...
    //$dbconn->debug=true;
		...
		$resulta->Close();
    //$this->salida = "<pre>".print_r($_REQUEST." hhhh",true)."</pre>";
     ...
     function Consulta_Procedimientos_Solicitados(){
		...
		$query1= " SELECT  a.hc_os_solicitud_id, 
                       a.evolucion_id, 
                       a.cargo, 
                       b.observacion,
		                 --b.tipo_cirugia_id, 
                       b.ambito_cirugia_id, 
                       b.finalidad_procedimiento_id,
                       j.descripcion as cirugia,
                       k.descripcion as ambito,
                       l.descripcion as finalidad, 
		                   c.diagnostico_id,
                       d.tipo_equipo_fijo_id, 
                       e.tipo_equipo_id, 
                       g.diagnostico_nombre, 
		                   h.descripcion as fijo,
                       i.descripcion as movil, 
                       m.descripcion, 
               ...
               AND      b.finalidad_procedimiento_id = l.finalidad_procedimiento_id
		           AND      a.cargo  = m.cargo  
               AND      m.grupo_tipo_cargo = n.grupo_tipo_cargo order by a.hc_os_solicitud_id";
		$criterio='';
		...
		$query = " SELECT  a.hc_os_solicitud_id, 
		        ...
		        --left join qx_tipos_cirugia j on (b.tipo_cirugia_id = j.tipo_cirugia_id) 
		          ...
		          LEFT JOIN qx_tipo_equipo_movil i ON (e.tipo_equipo_id = i.tipo_equipo_id), 
                        ...
                        hc_evoluciones p
		           WHERE    a.hc_os_solicitud_id = b.hc_os_solicitud_id
		           AND      p.ingreso = ".$this->ingreso." 
               AND      a.evolucion_id = p.evolucion_id
		         --AND      b.ambito_cirugia_id = k.ambito_cirugia_id 
               AND      b.finalidad_procedimiento_id = l.finalidad_procedimiento_id
		           AND      a.cargo  = m.cargo  
     ...
     function Insertar_Solicitud_Procedimiento(){
		...
		$query = " INSERT INTO hc_os_solicitudes_datos_acto_qx
                          (
                            acto_qx_id, 
                            nivel_autorizacion,
		...
		if($dbconn->ErrorNo() != 0){
			...
			foreach($_SESSION['SOLICITUD_PROCEDIMIENTOS_QX'.$pfj] as $cargos=>$datos){
				...
				$query = " INSERT INTO hc_os_solicitudes
                           ...
                           (
                             ...
                             '".$this->evolucion."',
                             '".$this->plan_id."',
                             '".ModuloGetVar('','','TipoSolicitudProcedimientos')."',
				                     '1',
                             '1',
                             '0',
                             '".$solicitudAmbulatoria."',
                             '0',
                             '".$this->paciente."',
                             '".$this->tipoidpaciente."'
                            )";
				...
				if($dbconn->ErrorNo() != 0){
					...
					$query = "INSERT INTO hc_os_solicitudes_acto_qx
                               (
                                 hc_os_solicitud_id,
                                 observacion,
                                 acto_qx_id
                          ...
                          (
                            '".$hc_solicitud_id."',
                            '".$_SESSION['SOLICITUD_QX_DIAGNOSTICOS'.$this->frmPrefijo]['OBSERVACIONES'][$cargos]."',
                            '".$acto_qx_id."'
                          )";
			  ...
			  foreach ($vect as $descripcion=>$cantidad){
					...
					if($dbconn->ErrorNo() != 0){
						...
						$query= " INSERT INTO hc_os_solicitudes_acto_qx
                            ...
                            (
                              '".$hc_solicitud_id."',
                              '".$_SESSION['SOLICITUD_QX_DIAGNOSTICOS'.$this->frmPrefijo]['OBSERVACIONES'][$codigo]."',
                              '".$acto_qx_id."'
                            )";
			...
			if(sizeof($_SESSION['SOLICITUD_RESERVA_SANGRE_QX'.$pfj])>0){
				...
				$query = " INSERT INTO banco_sangre_reserva
                               (
                                 solicitud_reserva_sangre_id,
                                 paciente_id,
                                 tipo_id_paciente,
				                         ubicacion_paciente,
				...
				foreach($_SESSION['SOLICITUD_RESERVA_SANGRE_QX'.$pfj] as $componente=>$cantidad){
					if($cantidad){
						...
						$query = " INSERT INTO banco_sangre_reserva_detalle
                                  (
                                    solicitud_reserva_sangre_id,
                                    tipo_componente_id,
                                    cantidad_componente,
				...
				foreach($_SESSION['SOLICITUD_RESERVA_SANGRE_QX_APOYOS'.$pfj] as $cargo=>$descripcion){
					$query = " INSERT INTO banco_sangre_reserva_otros_servicios
                                 (
                                   solicitud_reserva_sangre_id,
                                   cargo
                                 )
				...
				$query = " INSERT INTO banco_sangre_reserva_hc
                              (
                                solicitud_reserva_sangre_id,
                                evolucion_id,
                                ingreso,
	...
	function Modificar_Solicitud_Procedimiento(){
		...
		$query= " DELETE 
		          ...
		          WHERE  solicitud_reserva_sangre_id IN 
              (
                SELECT  solicitud_reserva_sangre_id 
		...
		if(sizeof($_SESSION['SOLICITUD_RESERVA_SANGRE_QX'.$pfj])>0){
			...
			$query = " INSERT INTO banco_sangre_reserva_hc
                            (
                             solicitud_reserva_sangre_id,
                             evolucion_id,ingreso,
                             hc_os_solicitud_id,acto_qx_id
                             )
			            VALUES(
                         '$SolitudReserva',
                         '".$this->evolucion."',
                         '".$this->ingreso."',NULL,'".$acto_qx_id."'
                         );";
		...
		if ($dbconn->ErrorNo() != 0)
		{
			...
			if ($dbconn->ErrorNo() != 0){
				...
				if ($dbconn->ErrorNo() != 0){
					...
					$query= " DELETE 
                    FROM  hc_os_solicitudes_procedimientos_apoyos
					          WHERE hc_os_solicitud_id = ".$hc_os_solicitud_id."";
					$resulta=$dbconn->Execute($query);
					if ($dbconn->ErrorNo() != 0){
						$this->frmError["MensajeError"]="NO SE PUEDE ELIMINAR PORQUE YA FUE AUTORIZADO";
...
//cor - clzc - spqx
	function tipoequipofijo(){
		...
		list($dbconnect) = GetDBconn();
		$query= " SELECT * 
              FROM   qx_tipo_equipo_fijo 
              ORDER BY tipo_equipo_fijo_id";
		$result = $dbconnect->Execute($query);
		if ($dbconnect->ErrorNo() != 0){
...
//cor - clzc - spqx
	function tipoequipomovil(){
		...
		list($dbconnect) = GetDBconn();
		$query= " SELECT * 
              FROM   qx_tipo_equipo_movil 
              ORDER BY tipo_equipo_id";
		$result = $dbconnect->Execute($query);
		if ($dbconnect->ErrorNo() != 0){
	...
	function Busqueda_Avanzada_ComponentesBanco(){
		...
		$pfj=$this->frmPrefijo;
		list($dbconn) = GetDBconn();
		$query = " SELECT  hc_tipo_componente,
                       componente 
...
//cor - clzc - ads
	...
	function Busqueda_Avanzada_Apoyos(){
		...
		if(empty($_REQUEST['conteo'.$pfj])){
			if($opcion == '002'){
                    $query= " SELECT  count(*)
                              FROM    apoyod_solicitud_frecuencia a, 
                                      cups b,
                                      hc_os_solicitudes_qx_grupos_cargos c,
                                      grupos_tipos_cargo d
				...
				$query = "SELECT count(*)
						FROM  cups a,
                  hc_os_solicitudes_qx_grupos_cargos b,
                  grupos_tipos_cargo d
						WHERE a.grupo_tipo_cargo = b.grupo_tipo_cargo 
		...
		if($opcion == '002'){
		  $cont=0;
			$query= " SELECT  DISTINCT a.cargo, 
                        b.descripcion, 
                        c.grupo_tipo_cargo as apoyod_tipo_id,
	  	...
	  	$query.=" FROM  apoyod_solicitud_frecuencia a, 
                      cups b,
			                hc_os_solicitudes_qx_grupos_cargos c,
                      grupos_tipos_cargo d
			          WHERE a.cargo = b.cargo
        ...
        $query.=" ordenamiento,";
			...
			$query = " SELECT  a.cargo, 
                         a.descripcion, 
                         b.grupo_tipo_cargo as apoyod_tipo_id,
			                   d.descripcion as tipo";
			if(sizeof($_SESSION['SOLICITUD_APOYOS_QX'.$pfj])>0){
				$query.=",(CASE WHEN (";
				if(sizeof($_SESSION['SOLICITUD_APOYOS_QX'.$pfj])>1){
		...
		if(empty($_REQUEST['conteo'.$pfj])){
			$query= " SELECT  count(*)
			          FROM    inventarios_productos a,
                        hc_os_solicitudes_grupos_inventarios b
	...
	function Busqueda_Avanzada_EstanciaQX(){
		...
		$query= " SELECT  a.tipo_clase_cama_id,
                      a.descripcion";
  ...
  function QuirofanosTotal(){
    list($dbconn) = GetDBconn();
		$query= " SELECT  DISTINCT a.quirofano,
                      a.descripcion
	...
	function Busqueda_Avanzada_EquiposQX(){
      ...
      $concat2=" AND b.descripcion LIKE '%".$_REQUEST['descripcionEquipo'.$pfj]."%'";
		...
		if(empty($_REQUEST['conteo'.$pfj])){
          ...
          $query= " SELECT  'FIJO' as tipo,
                             b.tipo_equipo_fijo_id as tipoid,
                             b.descripcion
                    FROM    qx_tipo_equipo_fijo b,
                            qx_equipos_quirofanos x
                    WHERE   b.tipo_equipo_fijo_id=x.tipo_equipo_fijo_id $concat $concat2";
				}
			}else{
        $query= " SELECT  DISTINCT 'MOVIL' as tipo,
                          a.tipo_equipo_id as tipoid,
                          a.descripcion
				          FROM    qx_tipo_equipo_movil a $concat3
				          UNION
				          SELECT  'FIJO' as tipo,
...
//cor - clzc- ads
	...
	function GruposTiposCargos(){
		...
		$query= " SELECT  a.grupo_tipo_cargo, 
                      b.descripcion
						  FROM    hc_os_solicitudes_qx_grupos_cargos a,
                      grupos_tipos_cargo b
						  WHERE   a.grupo_tipo_cargo=b.grupo_tipo_cargo";
		$result = $dbconnect->Execute($query);
		if($dbconnect->ErrorNo() != 0){
			$this->error = "Error al buscar en la tabla apoyod_tipos";
...
//cor - clzc - spqx
	function Apoyos_Del_Procedimiento($hc_os_solicitud_id){
		...
		$query= " SELECT  a.cargo, 
                      b.descripcion 
              FROM    hc_os_solicitudes_procedimientos_apoyos as a, 
                      cups as b 
  ...
  function Apoyos_Prequirurgicos($cargo){
		...
		$query= " SELECT  a.cargo, 
                      ...
                      a.edad_min, 
                      a.edad_max 
              FROM    hc_apoyos_prequirurgicos as a, 
                      cups as b 
              WHERE   a.cargo = '".$cargo."' 
              AND     a.cargopreqx = b.cargo 
              AND    (a.sexo_id='".$this->datosPaciente['sexo_id']."' OR a.sexo_id is null OR a.sexo_id = '0') AND (a.edad_max>=".$edad_paciente[anos]." OR a.edad_max is null) AND (a.edad_min<=".$edad_paciente[anos]." OR a.edad_min is null)";
		$result = $dbconnect->Execute($query);
		if ($dbconnect->ErrorNo() != 0){
	...
	function GrupoInventarios(){
		...
		list($dbconnect) = GetDBconn();
    $query= " SELECT  a.grupo_id,
                      b.descripcion
		          FROM    hc_os_solicitudes_grupos_inventarios a,
                      inv_grupos_inventarios b
		          WHERE   a.grupo_id=b.grupo_id";
		$result = $dbconnect->Execute($query);
		if($dbconnect->ErrorNo() != 0){
				$this->error = "Error al buscar en la tabla hc_apoyos_prequirurgicos";
	...
	function CargoXDefecto($cargo){
		...
		$pfj=$this->frmPrefijo;
		list($dbconnect) = GetDBconn();
    $query= " SELECT  hc.cargo_asociado,
                      cp.descripcion,
                      cp.grupo_tipo_cargo,
                      hc.cantidad
              ...
              LEFT JOIN tipos_planes tp ON (hc.tipo_plan=tp.sw_tipo_plan),
                      cups cp
		          WHERE   hc.cargo='".$cargo."' 
              AND     hc.cargo_asociado=cp.cargo 
              AND    (hc.plan_id='".$this->plan_id."' OR hc.plan_id IS NULL)";
		$result = $dbconnect->Execute($query);
		if($dbconnect->ErrorNo() != 0){
    ...
    $query= " SELECT  a.cargo,
                      b.descripcion
		          FROM    hc_os_solicitudes_apoyos_banco_sangre a,
                      cups b
		          WHERE   a.cargo=b.cargo ";
		if(sizeof($_SESSION['SELECCION_APOYOS_BANCO_QX'.$pfj]) > 0){
	...
	function EstanciaXDefecto($cargo){
		$pfj=$this->frmPrefijo;
		list($dbconnect) = GetDBconn();
    $query= " SELECT  hc.tipo_clase_cama_id,
                      a.descripcion,
                      hc.sw_postqx,
                      hc.dias
		          FROM    hc_os_solicitudes_cargos_asociados_tipos_camas hc,
                      tipos_clases_camas a
		          WHERE   hc.cargo='$cargo' 
              AND     hc.tipo_clase_cama_id=a.tipo_clase_cama_id";
		$result = $dbconnect->Execute($query);
		if($dbconnect->ErrorNo() != 0){
			$this->error = "Error al buscar en la tabla hc_apoyos_prequirurgicos";
			$this->mensajeDeError = "Error DB : " . $dbconnect->ErrorMsg();
	...
	function TraerVariablesdeSession(){
		...
		$pfj=$this->frmPrefijo;
		list($dbconnect) = GetDBconn();
		$query= " SELECT  a.nivel_autorizacion,
                      a.fecha_tentativa_cirugia
		          FROM    hc_os_solicitudes_datos_acto_qx a
		...
		$query= " SELECT  a.hc_os_solicitud_id,
                      ...
                      b.sw_ambulatorio,
		                  (CASE 
                      WHEN c.grupo_tipo_cargo IN (SELECT grupo_tipo_cargo FROM qx_grupos_tipo_cargo) THEN 'PROC' ELSE 'APOY' END) as procedimiento 
		          FROM    hc_os_solicitudes_acto_qx a,
                      hc_os_solicitudes b,
                      cups c,
                      tipos_cargos d
		          WHERE   a.acto_qx_id='".$_SESSION['SOLICITUD_QX'.$pfj]['INSERTADA']."' 
		...
		for($i=0;$i<sizeof($vector1);$i++){
			...
			$query= " SELECT  b.diagnostico_id,
                        ...
                        b.sw_principal
			          FROM    hc_os_solicitudes a,
                        hc_os_solicitudes_diagnosticos b,
                        diagnosticos c
			          WHERE   a.hc_os_solicitud_id='".$vector1[$i]['hc_os_solicitud_id']."' 
                AND     a.cargo='".$vector1[$i]['cargo']."' 
                AND     a.hc_os_solicitud_id=b.hc_os_solicitud_id 
                AND     b.diagnostico_id=c.diagnostico_id";
		...
		$query= " SELECT 'FIJO' as tipo,
                      a.tipo_equipo_fijo_id as codigo,
                      b.descripcion,
                      a.cantidad
              ...
              AND     a.tipo_equipo_fijo_id=b.tipo_equipo_fijo_id
		          UNION
		          SELECT 'MOVIL' as tipo,
                      a.tipo_equipo_id as codigo,
                      b.descripcion,
                      a.cantidad
		...
		$query= " SELECT  a.tipo_clase_cama_id,
                      ...
                      a.sw_pre_qx,
                      a.sw_pos_qx
		          FROM    hc_os_solicitudes_estancia a,
                      tipos_clases_camas b
		          WHERE   a.acto_qx_id='".$_SESSION['SOLICITUD_QX'.$pfj]['INSERTADA']."' 
		...
		$query= " SELECT  b.tipo_componente_id,
                      b.cantidad_componente,
                      c.componente
		          FROM    banco_sangre_reserva_hc a,
		...
		$query= " SELECT  b.cargo,
                      c.descripcion
		          FROM    banco_sangre_reserva_hc a,
                      banco_sangre_reserva_otros_servicios b,
                      cups c
		          WHERE   a.acto_qx_id='".$_SESSION['SOLICITUD_QX'.$pfj]['INSERTADA']."' 
              AND     a.solicitud_reserva_sangre_id=b.solicitud_reserva_sangre_id 
              AND     b.cargo=c.cargo";
    ...
    $query= " SELECT  a.acto_qx_id,
              ...
              AND     e.tipo_id_paciente='".$this->tipoidpaciente."' 
              AND     e.paciente_id='".$this->paciente."' 
              AND     d.evolucion_id <> '".$this->evolucion."' 
              AND     e.ingreso='".$this->ingreso."' 
              AND     c.cargo=f.cargo 
              AND     d.usuario_id=g.usuario_id 
              AND     g.tipo_tercero_id=ter.tipo_id_tercero 
              AND     g.tercero_id=ter.tercero_id 
              AND     h.tipo_cargo=f.tipo_cargo 
              AND     h.grupo_tipo_cargo=f.grupo_tipo_cargo
  ...
  function SolicitudesQXPacientefrmConsulta(){
    ...
    $query= " SELECT  a.hc_os_solicitud_id,
              ...
              AND     d.tipo_tercero_id=e.tipo_id_tercero 
              AND     d.tercero_id=e.tercero_id 
              AND     a.cargo=f.cargo 
              AND     a.os_tipo_solicitud_id='".ModuloGetVar('','','TipoSolicitudProcedimientos')."' 
    ...
    $pfj=$this->frmPrefijo;
		list($dbconnect) = GetDBconn();
    $query= " SELECT  a.procedimiento_id,
                      b.descripcion,
                      a.autorizacion as sw_estado,
                      c.observacion
		          FROM    hc_os_solicitudes_otros_procedimientos_qx a,
                      cups b,
                      hc_os_solicitudes_datos_acto_qx c
		          WHERE   a.hc_os_solicitud_id='".$SolicitudId."' 
    ...
    $pfj=$this->frmPrefijo;
		list($dbconnect) = GetDBconn();
    $query= " SELECT  a.observacion_id,
                      a.observacion,
                      a.fecha,
                      a.usuario_id,
                      a.fecha_ultima_modificacion,
                      (CASE WHEN a.usuario_id='".UserGetUID()."' THEN 1 ELSE 2 END) as propio,
                      c.nombre_tercero
		...
		if(!empty($_REQUEST['observaciones'.$pfj])){
			...
			$query= " INSERT INTO hc_os_solicitudes_observaciones
                 ...
                 fecha_ultima_modificacion
			           ...
			           VALUES
                 (
                 '".$_REQUEST['SolicituId'.$pfj]."',
                 '".$_REQUEST['observaciones'.$pfj]."',
                 '".date("Y-m-d H:i:s")."',
                 '".UserGetUID()."',
                 '".date("Y-m-d H:i:s")."'
                 )";
    ...
    $pfj=$this->frmPrefijo;
		if(!empty($_REQUEST['observaciones'.$pfj])){
			...
			$query= " UPDATE hc_os_solicitudes_observaciones
			          SET    observacion='".$_REQUEST['observaciones'.$pfj]."',
                       fecha_ultima_modificacion='".date("Y-m-d H:i:s")."'
			          WHERE  acto_qx_id='".$_REQUEST['SolicituId'.$pfj]."' 
                AND    observacion_id='".$_REQUEST['observacionId'.$pfj]."'";
			$result = $dbconnect->Execute($query);
			if($dbconnect->ErrorNo() != 0){
				$this->error = "Error al buscar en la tabla";
				$this->mensajeDeError = "Error DB : " . $dbconnect->ErrorMsg();
    ...
    $pfj=$this->frmPrefijo;
		list($dbconnect) = GetDBconn();
    $query= " DELETE 
              FROM   hc_os_solicitudes_observaciones 
              WHERE  observacion_id='".$_REQUEST['observacionId'.$pfj]."'";
		$result = $dbconnect->Execute($query);
		if($dbconnect->ErrorNo() != 0){
			$this->error = "Error al buscar en la tabla";
			$this->mensajeDeError = "Error DB : " . $dbconnect->ErrorMsg();
    ...
    $query= " SELECT  a.procedimiento_id,
                      ...
                      b.sw_estado,
                      a.autorizacion
		           FROM   hc_os_solicitudes z,
                      hc_os_solicitudes_otros_procedimientos_qx a,
                      autorizaciones_qx b,
                      cups c,
                      system_usuarios d
		          WHERE   z.hc_os_solicitud_id='$SolicitudId' 
              AND    (z.sw_estado='0' OR z.sw_estado='3') 
              AND     z.hc_os_solicitud_id=a.hc_os_solicitud_id 
              AND     a.autorizacion IS NOT NULL 
              AND     a.autorizacion=b.autorizacion 
              AND     a.procedimiento_id=c.cargo 
              AND     b.usuario_id=d.usuario_id";
		$result = $dbconnect->Execute($query);
		if($dbconnect->ErrorNo() != 0){
			$this->error = "Error al buscar en la tabla";
    ...
    $query= " SELECT  a.cargo,
                      ...
                      a.autorizacion
		           FROM   hc_os_solicitudes z,
                      hc_os_solicitudes_procedimientos_apoyos a,
                      autorizaciones_qx b,
                      cups c,
                      system_usuarios d,
                      grupos_tipos_cargo e
		           WHERE  z.hc_os_solicitud_id='$SolicitudId' 
               AND   (z.sw_estado='0' OR z.sw_estado='3') 
               AND    z.hc_os_solicitud_id=a.hc_os_solicitud_id 
               AND    a.autorizacion IS NOT NULL 
               ...
               AND    a.cargo=c.cargo 
               AND    b.usuario_id=d.usuario_id 
               AND    c.grupo_tipo_cargo=e.grupo_tipo_cargo";
		$result = $dbconnect->Execute($query);
		if($dbconnect->ErrorNo() != 0){
			$this->error = "Error al buscar en la tabla";
			$this->mensajeDeError = "Error DB : " . $dbconnect->ErrorMsg();
  ...
  function MaterialesAutorizados($SolicitudId){
    ...
    $query= " SELECT  a.codigo_producto,
                      ...
                      a.autorizacion
		          FROM    hc_os_solicitudes z,
                      hc_os_solicitudes_otros_productos_inv a,
                      autorizaciones_qx b,
    ...
    $query= " SELECT  a.tipo_clase_cama_id,
                      ...
                      b.sw_estado,
                      a.autorizacion,
                      e.descripcion as tipocama
               FROM   hc_os_solicitudes z,
                      hc_os_solicitudes_estancia a,
                      autorizaciones_qx b,
                      tipos_clases_camas c,
                      system_usuarios d,
                      tipos_camas e
		           WHERE  z.hc_os_solicitud_id='$SolicitudId' 
               AND   (z.sw_estado='0' OR z.sw_estado='3') 
               ...
               AND    a.autorizacion IS NOT NULL 
               AND    a.autorizacion=b.autorizacion 
               AND    a.tipo_clase_cama_id=c.tipo_clase_cama_id 
               AND    b.usuario_id=d.usuario_id 
               AND    a.tipo_cama_id=e.tipo_cama_id";
		$result = $dbconnect->Execute($query);
		if($dbconnect->ErrorNo() != 0){
			$this->error = "Error al buscar en la tabla";
			$this->mensajeDeError = "Error DB : " . $dbconnect->ErrorMsg();
    ...
    $query= " SELECT  a.observacion,
		                --b.descripcion as finalidad,
                      c.descripcion as ambito,
                      d.descripcion as tipocirugia,
		                  a.fecha_tentativa_cirugia,
                      e.descripcion as nivel
		          FROM    hc_os_solicitudes z,
                      hc_os_solicitudes_datos_acto_qx a
    ...
    $query= " SELECT  b.diagnostico_id,
                      ...
                      a.sw_principal
		           FROM   hc_os_solicitudes z,
                      hc_os_solicitudes_diagnosticos a,
                      diagnosticos b
		           WHERE  z.hc_os_solicitud_id='$SolicitudId' 
               AND    z.hc_os_solicitud_id=a.hc_os_solicitud_id 
