...
/**
 * $Id: reporteNotaOperatoria_html.report.php,v 1.4 2009/07/27 20:32:38 johanna Exp $
...
//FUNCION QUE RETORNA EL HTML DEL REPORTE (lo que va dentro del tag <BODY>)
	...
	//*******************************************termino	
		...
		$Salida .= "	</table><br>\n";
			//print_r($Datos);
		$datos=$this->ConsultaNotasOperatoriasRealizadas($this->datos['programacion'],$this->datos['tipoidpaciente'],$this->datos['paciente'],$this->datos['hc_nota_operatoria_cirugia_id']);
		//print_r($datos);
		if($datos){			
...
// 			$datos=$this->ConsultaNotasOperatoriasRealizadas($this->datos['programacion'],$this->datos['tipoidpaciente'],$this->datos['paciente']);
			...
			$Salida .= "		<td width=\"30%\" align=\"left\" class=\"normal_10\">".$datos['cirujano']."</td>";
			$Salida.= "     </tr>";	
			$Salida .= "		<tr>";
			...
			$Salida.= "     </tr>";
			$Salida .= "		<tr>";
			$Salida .= "		<td width=\"20%\" class=\"normal_10N\">TIPO ANESTESIA</td>";
			if($datos['tipo_anestesia']){
			$Salida .= "		<td width=\"30%\" align=\"left\" class=\"normal_10\">".$datos['tipo_anestesia']."</td>";
			}
			else{
				$Salida .= "		<td width=\"30%\" align=\"left\" class=\"normal_10\">SIN ASIGNAR</td>";
			}
			...
			$gases = $this->ConsultarGases($datos['programacion_id'],$datos['evolucion_id'],$datos['hc_nota_operatoria_cirugia_id']);
					if($gases)
					{
			...
			$Salida.="	</tr>";
			$procedimientos=$this->ProcedimientosNotaOperatoria($this->datos['programacion'],$this->datos['tipoidpaciente'],$this->datos['paciente'],$this->datos['hc_nota_operatoria_cirugia_id']);
			...
			$Tecnicas=$this->DescripcionTecnicaQX($datos['ingreso'],$datos['programacion_id'],$datos['hc_nota_operatoria_cirugia_id']);
			if($Tecnicas){
				$Salida .= "  <table border=\"1\" width=\"95%\" align=\"center\">";
			...
			$Hallazgos=$this->HallazgosQX($datos['ingreso'],$datos['programacion_id'],$datos['hc_nota_operatoria_cirugia_id']);
			if($Hallazgos){
				$Salida .= "  <table border=\"1\" width=\"95%\" align=\"center\">";
			...
			$materialesPatologicos=$this->RegistroPatologias($datos['ingreso'],$datos['programacion_id'],$datos['hc_nota_operatoria_cirugia_id']);			
			...
			$cultivos=$this->RegistroCultivos($datos['ingreso'],$datos['programacion_id'],$datos['hc_nota_operatoria_cirugia_id']);
			if($cultivos){
				$Salida .= "  <table border=\"1\" width=\"95%\" align=\"center\">";
...
//*****************************************fin de termino
	...
	function ConsultarGases($programacion,$evolucion,$notaid)
	{
		list($dbconn) = GetDBconn();
		...
		$sql = " select  h.tipo_gas_id, 
		         ...
		         where   h.ingresoid = ".$this->datos['ingreso']." 
             and     programacion_id = ".$programacion." 
             and     h.frecuencia_id=g.frecuencia_id
             and     h.hc_nota_operatoria_cirugia_id = ".$notaid."";
		$result = $dbconn->Execute($sql);
		if($result->EOF){
	...
	function ConsultaNotasOperatoriasRealizadas($programacion,$tipoidpaciente,$paciente,$notaid)
  {
		list($dbconn) = GetDBconn();
		//$dbconn->debug=TRUE;
    $query =" SELECT  a.hc_nota_operatoria_cirugia_id,
              ...
              AND   ing.tipo_id_paciente='".$tipoidpaciente."' 
              AND   ing.paciente_id='".$paciente."'
              AND   a.hc_nota_operatoria_cirugia_id= ".$notaid."
	...
	function ProcedimientosNotaOperatoria($programacion,$tipoidpaciente,$paciente,$notaid)
  {
		list($dbconn) = GetDBconn();
		 $query ="  SELECT   c.hc_nota_operatoria_cirugia_id,
                        a.procedimiento_qx,
                        b.descripcion,
                        a.observaciones,
		           ...
		           FROM     hc_notas_operatorias_procedimientos a,
                        cups b,hc_notas_operatorias_cirugias c,
		                    hc_evoluciones evol,ingresos ing,
                        profesionales_usuarios prof,
                        terceros ter,
                        profesionales usuprof
		           WHERE    c.programacion_id='".$programacion."' 
               AND      c.evolucion_id=evol.evolucion_id 
               AND      evol.ingreso=ing.ingreso 
               AND      ing.tipo_id_paciente='".$tipoidpaciente."' 
               AND      ing.paciente_id='".$paciente."' 
               AND      a.hc_nota_operatoria_cirugia_id = ".$notaid."
               AND      c.hc_nota_operatoria_cirugia_id=a.hc_nota_operatoria_cirugia_id 
               AND      a.procedimiento_qx=b.cargo 
               AND      c.usuario_id=prof.usuario_id 
               AND      prof.tipo_tercero_id=ter.tipo_id_tercero 
               AND      prof.tercero_id=ter.tercero_id 
               AND      usuprof.tipo_id_tercero=ter.tipo_id_tercero 
               AND      usuprof.tercero_id=ter.tercero_id
		           AND      a.realizado='1'";
		$result = $dbconn->Execute($query);
		if($dbconn->ErrorNo() != 0){
			$this->error = "Error al Cargar el Modulo";
	...
	function DescripcionTecnicaQX($ingreso,$programacion,$notaid)
  {
	...
	function HallazgosQX($ingreso,$programacion,$notaid)
  {
	...
	function RegistroPatologias($ingreso,$programacion,$notaid)
  {
		list($dbconn) = GetDBconn();
	...
	function RegistroCultivos($ingreso,$programacion,$notaid)
  {
