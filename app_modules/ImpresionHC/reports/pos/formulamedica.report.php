...
/**
 * $Id: formulamedica.report.php,v 1.8 2009/07/17 13:05:23 hugo Exp $
    ...
    function CrearReporte()
    {
		...
		$reporte->PrintFTexto('Sexo       : '.$datos[0][sexo_id],false,'left',false,false);
          ...
          $reporte->PrintFTexto('Tel. Res.  : '.$datos[0][residencia_telefono],false,'left',false,false);
					...
					if ($datos[0][uso_controlado]==1)
					{
						$reporte->PrintFTexto('Dpto.  : '.$datos[0][departamento],false,'left',false,false);
						$reporte->PrintFTexto('Mpio.  : '.$datos[0][municipio],false,'left',false,false);
					}
		...
		$reporte->SaltoDeLinea();
          ...
          for($i=0; $i<sizeof($Vmedicamentos);$i++)
          {
               ...
               if ($Vmedicamentos[$i][via]!='')
               {
                    $reporte->PrintFTexto('Via de Administracion: '.$Vmedicamentos[$i][via],false,'left',false,false);
               }
               ...
               if($Vmedicamentos[$i][tipo_solicitud] == "M")
               {
				...
				$reporte->PrintFTexto('Dias Tratamiento : '.$Vmedicamentos[$i][dias_tratamiento],false,'left',false,false);                    
