   ...
   //echo "<pre>";print_r($diferenciLotes);
    ...
    if (sizeof($diferenciLotes) > 0) {
      ...
      foreach ($diferenciLotes as $keys => $values) {        
          $values['titular']['exis']=$consulta->Consultar_Cantidad_Vencida($empresa,$bodega_id,$keys,$values['titular']['exis']);
       //   if(round($values['titular']['exis'])==$values['lote']['exis']){
            $lotes = $consulta->GetLotedeMayorExistencia($empresa, $bodega_id, $keys, 1);
            $descuentoaLotes=$values['titular']['exis'];            
            foreach ($lotes as $value) {
                if($descuentoaLotes>$value['existencia_actual']){
                    $resta_lote=$value['existencia_actual'];
                    $descuentoaLotes=$descuentoaLotes-$value['existencia_actual'];
                }else{                    
                    $resta_lote =$descuentoaLotes;     
                    $html .=vista_lote($keys,$values['titular']['exis']==0?'0':$values['titular']['exis'],"RESTA",$value['lote'],$value['existencia_actual']==0?'0':$value['existencia_actual'],$values['lote']['exis']==0?'0':$values['lote']['exis'],$resta_lote,$value['fecha_vencimiento']);  
                    break;
                }
...
function ProcesarLote2($empresa, $bodega_id,$bodega_nombre) {
    $consulta = new TomaFisicaSQL();
    $objResponse = new xajaxResponse();
    $diferenciLotes = $consulta->GetDiferenciasLotes($empresa, $bodega_id);
...
function ProcesarInventario($empresa,$centro_utilidad ,$bodega_id,$bodega_nombre,$cabecera_id,$nombrecabe) {
    ...
    ////////////////////////////////////////////////////////////
    $salidaI .= "                  <table width=\"100%\" align=\"center\" class=\"modulo_table_list\">\n";
    $salidaI .= "                    <tr  class=\"modulo_table_list_title\">\n";
    $salidaI .= "                      <td align=\"center\" width=\"15%\">\n";
    $salidaI .= "                       <a title='BODEGA DOCUMENTO ID'>";
    $salidaI .= "                        BOD DOC ID";
    $salidaI .= "                       </a>";
    $salidaI .= "                      </td>\n";
    $salidaI .= "                      <td align=\"center\"width=\"15%\">\n";
    $salidaI .= "                       <a title='PREFIJO'>";
    $salidaI .= "                        PREFIJO";
    $salidaI .= "                       </a>";
    $salidaI .= "                      </td>\n";
    $salidaI .= "                      <td colspan='2' align=\"center\" width=\"60%\">\n";
    $salidaI .= "                        <a title='DESCRIPCION DEL DOCUMENTO'>DESCRIPCION</a>";
    $salidaI .= "                      </td>\n";
    $salidaI .= "                     </tr>\n";
    ///////////////////////////////////////////////egreso/////////////////////////////////////////////
...
//    //    if($vector_egreso[$i]['prefijo']=='ASF'){
        ...
        $salida .= "                          <sub> 1 <img src=\"" . $path . "/images/checkno.png\" border=\"0\" width=\"17\" height=\"17\"></sub>\n";
        $salida .= "                         </a>\n";
        $salida .= "                      </td>\n";
        $salida .= "                      </tr>\n";
   //     }
...
function vista_lote($producto,$existencia,$operacion,$lote,$lote_existencia,$total_lote,$diferencia,$vencimiento){
    $html  = "<tr class=\"modulo_list_claro\">";
    $html .= "<td>".$producto."</td>";
    $html .= "<td align=\"right\">".$existencia."</td>";
    $html .= "<td>".$operacion."</td>";
    ...
    $html .= "<td align=\"right\">".$total_lote."</td>";
    $html .= "<td align=\"right\">$diferencia</td>";
    $html .= "<td align=\"right\">$vencimiento</td>";
    $html .= "</tr>";
    return $html; 
}
