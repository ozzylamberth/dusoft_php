...
function ListarDocumentosEgreso($empresa,$cu,$bodega,$usuario)
    ...
    function CrearDocumento($toma_fisica,$bodega_doc_id)
    {
        ...
        { 
            ...
            $retorno=$objeto->AddItemDocTemporal($doc_temporal['doc_tmp_id'], $valor['codigo_producto'], $valor['cantidad'], $porcentaje_gravamen=0, $total_costo=0,null,$fechavencimiento,$valor['lote']);
            //var_dump($retorno);
            if($retorno===false)
            {
                $this->mensajeDeError=$objeto->error."<br>".$objeto->mensajeDeError;
...
//         
          ...
          { 
              ...
              $retorno=$objeto->AddItemDocTemporal($doc_temporal['doc_tmp_id'], $valor['codigo_producto'], $valor['cantidad'], $porcentaje_gravamen=0, $total_costo=0,null,$fechavencimiento,$valor['lote'],null);
              //print_r($retorno);
             if($retorno===false)
              {
          ...
          $sql="  UPDATE inv_toma_fisica_update SET sw_actualizado = '1'
                  WHERE toma_fisica_id = $toma_fisica
                      ...
                      (
                          SELECT a.etiqueta
                          FROM inv_toma_fisica_update AS a
                          WHERE a.existencia > a.nueva_existencia
                          AND a.toma_fisica_id = $toma_fisica
                          AND sw_actualizado IS NULL
                      ); ";
          global $ADODB_FETCH_MODE;
  ...
  function ProductosReporteIngresoEgresoUbicacion($toma_fisica)
  {
       $sql= " SELECT   b.etiqueta_x_producto, 
                   ...
                   AND      b.fecha_vencimiento = c.fecha_vencimiento
                   AND      b.lote = c.lote
                   AND      c.ubicacion_id=d.ubicacion_id
                   AND      sw_actualizado IS NULL
...
/*
   ...
   function ObtenerReporteAutomatico($toma_fisica)
   {
    ...
    function ProductosParaAjustarPorEgreso($toma_fisica)
    {
       $sql="SELECT COUNT(*)
        ...
        AND b.existencia >= ABS(a.existencia - a.nueva_existencia)
    ...
    {//AND b.toma_fisica_id = $toma_fisica
        $sql="SELECT
        a.codigo_producto,
        (a.	existencia - a.nueva_existencia) AS cantidad, b.fecha_vencimiento, b.lote	
        FROM inv_toma_fisica_update AS a,
        ...
        AND b.lote=a.lote
        AND b.codigo_producto = a.codigo_producto
        AND b.existencia_actual >=ABS(a.existencia - a.nueva_existencia)
        AND a.sw_actualizado IS NULL";
        //print_r($sql);
        if(!$resultado = $this->ConexionBaseDatos($sql))
                    return false;
       ...
       // EXIT;
        ...
        //print_r($sql);
             ...
             if($dbconn->ErrorNo() != 0)
             {
                ...
                $dbconn->CommitTrans();
                //print_r($toma_fisica_id."toma fisica");
 ...
 /*function Productostomafisica($empresa_id,$centro_utilidad,$bodega,$toma_fisica_id)
    ...
    function ListaBodegasProductos($empresa_id)
    {
      $sql = "SELECT  a.*,
                      b.num_productos,
                      COALESCE(c.num_productos_existencia,0) AS num_productos_existencia,
              ...
              FROM    bodegas as a,
                      ...
                      (
                        SELECT  empresa_id,
                                centro_utilidad,
                                bodega,   
                                COUNT(*) AS num_productos_existencia
                        FROM    existencias_bodegas 
                        WHERE   existencia > 0
    ...
    **/
    function ReporteProductosTomaFisica($toma_fisica,$empresa_id,$centro_utilidad,$bodega,$filtro)
    {
        if($filtro=='1')
        {
            ...
            $filtro5 = " ";
            $filtro3 = " ";
            $filtro4 = " ORDER BY i.descripcion ";
        }
        if($filtro=='2')
        {
            $filtro1  = "  mol.descripcion as molecula, ";
            $filtro2  = " ,i.descripcion as laboratorio ";
            $filtro5  = " ";
            $filtro3  = " ,inv_moleculas as mol  ";
            $filtro4  = " AND  g.molecula_id=mol.molecula_id ";
            $filtro4 .= " ORDER BY mol.descripcion ";
        }
        if($filtro=='3')
        {
            $filtro1  .=" bod_ub.descripcion, ";
            $filtro2  =" ";
            $filtro5  =" LEFT JOIN bodegas_ubicaciones AS bod_ub 
                         ON(  bod_ub.empresa_id = b.empresa_id
        ...
        $sql .= " SELECT  ".$filtro1." a.toma_fisica_id, 
                            ...
                            ".$filtro2."
               FROM    inv_toma_fisica_d as a,
                            existencias_bodegas_lote_fv as b ".$filtro5.",
                            existencias_bodegas as c,
                            inventarios as d,
                            inventarios_productos as e,
                            unidades as f,
                            inv_subclases_inventarios as g, 
                            inv_clases_inventarios as h,
                            inv_laboratorios as i
                            ".$filtro3."
             ...
             AND         f.unidad_id = e.unidad_id
             AND         e.grupo_id=g.grupo_id 
             AND         e.clase_id=g.clase_id
             AND         e.subclase_id = g.subclase_id
             AND         h.grupo_id = g.grupo_id
...
function AjustaraCeroBD($toma)
{
  ...
  $sql="INSERT INTO inv_toma_fisica_update
    ...
    WHERE
        t.toma_fisica_id = $toma
        AND a.toma_fisica_id = t.toma_fisica_id
        AND y.num_conteo IS NULL
...
//print_r($sql);
        if(!$resultado = $this->ConexionBaseDatos($sql))
        {
         ...
         {
           $cad="Ajuste Hecho Satisfactoriamente";
           return $cad;
         }   
...
function SacarSinConteo($toma)
{
 $sql="SELECT
      ...
      costo,
      existencia,
      descripcion_unidad, fecha_vencimiento,lote
      FROM tomas_fisicas
      WHERE conteo_1 IS NULL
      AND toma_fisica_id=".$toma." LIMIT 500
      ";
...
function SacarInfoConteo3($conteo3,$toma)
{
...
$sql="  SELECT
        ...
        AND       num_conteo_nueva_existencia =".$conteo3;
...
function SacarInfoConteo($conteo1,$toma)
{
    //$dbconn->debug=true;
    $sql="SELECT
    toma_fisica_id,
    ...
    WHERE nueva_existencia IS NOT NULL
    AND toma_fisica_id =".$toma."
    AND num_conteo_nueva_existencia =".$conteo1."";
    //print_r($sql);
    //return $sql;
      if(!$resultado = $this->ConexionBaseDatos($sql))
        return false;
...
//print_r($_SESSION);
     $sql= " SELECT  DISTINCT  c.codigo_producto,b.etiqueta_x_producto,
                 ...
                 FROM                   inv_toma_fisica_d as b,
                                             inventarios_productos as c
                                             JOIN inv_subclases_inventarios f ON 
                                             ( 
                                                      c.grupo_id=f.grupo_id
 ...
 else
 {
    $filtro2= " ";
     ...
     $sql=" SELECT  b.etiqueta_x_producto,
             ...
             FROM    inv_toma_fisica_d as b 
                          LEFT JOIN inv_toma_fisica_update as v ON(v.toma_fisica_id = b.toma_fisica_id AND v.etiqueta  = b.etiqueta),
                          inventarios_productos as c,
                          unidades as d,
                          existencias_bodegas_lote_fv as e 
  ...
  function InsertarTomaFisica($toma_fisica_id,$etiqueta,$empresa_id,$centro_utilidad,$codigo_producto,$bodega,$fecha_vencimiento,$lote,$etiqueta_x_pro,$conteo)
  {
     ...
     /* else
       {      
        ...
        if($conteo=='1' )
       {
          $num_lista=$this->SeleccionarNumConteo($toma_fisica_id,$conteo);
  ...
  $sql=" SELECT   a.toma_fisica_id,
                          ...
                          (
                            ...
                            FROM    inv_toma_fisica_conteos a,
                                    inv_toma_fisica_d b
                            WHERE   a.toma_fisica_id =".$toma_fisica_id."  
                            AND       b.etiqueta_x_producto =".$etiqueta_normal."
                            AND      a.toma_fisica_id=b.toma_fisica_id
                            AND      a.etiqueta=b.etiqueta
                          ) as num_conteo,
           CASE WHEN v.num_conteo IS NULL THEN '0' ELSE '1' END as cuadrado 
