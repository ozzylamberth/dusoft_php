<?
	...
	//$this->CrearTurnos(fecha,dia[boolean],horario[boolean],hora_turno[integer],rango[integer],rangoTurno[integer]);
		function CrearTurnos($url,$fecha='',$dia=false,$horario=false,$hora_turno,$rango,$turno,$horas='',$resaltar_turno=false,$turnos_prgmar='',$hora_inicio_turno,$ingreso,$control_id)
		{
			...
			$cadena .= "							for (i=pos;i<parseInt(".$turno.",10);i++) {\n";
			$cadena .= "								forma.elements[i].checked=val_checked;\n\n";
			$cadena .= "							}\n";*/  //est es lo q me hicieron quitar.....
			...
			if ($horario) {
				...
				/*$frecuencia=Buscar_Evolucion_Para_Frecuencia($ingreso,$control_id);
				if($frecuencia)
				{
						if($rango==$frecuencia)//es por si los rangos son de 1,2,3,4,5 horas...
						{
							$show_horario .= "	<option value='$frecuencia' selected>$frecuencia Horas</option>\n";
						}
						else
						{
							$show_horario .= "	<option value='$frecuencia'>$frecuencia Horas</option>\n";
						}
				}*/
				$show_horario .= "</select>\n";
				$show_horario .= "\n";
