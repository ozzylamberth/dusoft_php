	...
	class AdminPedidosEstadosHTML
	{
		...
		********************************************************/
		function MenuBodegas($action, $datos, $empresa_id)
		{
			...
			{
			  $BODEGA = ModuloGetURL("app", "AdminPedidosEstados", "controller", "MenuPedidos")."&datos[empresa_id]=".$empresa_id;
	          $html .= "</tr>\n";
		...
		********************************************************/
		function MenuPedidos($action, $datos, $pedido_id)
		{
			...
			$html .= "<applet archive=\"Jigsaw.jar\" code=\"Jigsaw.class\" height=\"380\" width=\"480\"> 
			...
			{
			  $PEDIDO = ModuloGetURL("app", "AdminPedidosEstados", "controller", "ResponsablesEstadosPedido")."&datos[pedido_id]=".$pedidos[$i]['pedido']."&datos[estado]=".$pedidos[$i]['estado']."&datos[destinatario_pedido]=".$datos['destinatario_pedido'];
	          $html .= "  </tr>\n";
	          $html .= "  <tr class=\"modulo_list_claro\">\n";
	          $html .= "     <td align=\"center\" class=\"normal_10AN\">\n";
		...
		********************************************************************************************************/
		function ResponsablesEstadosPedido($action, $datos, $pedido_id, $estado, $siguiente_estado)
		{
	        ...
	        if(array_key_exists("sw_despacho", $datos) && $datos['sw_despacho'] == 0) {
	        	//$html .= "			<input class=\"input-submit\" type=\"submit\" name=\"aceptar\" value=\"Enviar a ".$siguiente_estado." y Asignar Responsable\" disabled=\"disabled\">\n";
		...
		* @return string
		************************************************************************/
		function FormaMensajeModulo($action,$mensaje,$destinatario_pedido_label)
		{
