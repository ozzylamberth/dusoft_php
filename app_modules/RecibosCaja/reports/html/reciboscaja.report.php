    ...
    function CrearReporte()
    {
        ...
        if (sizeof($ConceptosV) > 0)
        {
            for ($i = 0; $i < sizeof($ConceptosV); $i++)
            {
                ...
                {
                    case 'C': $credito = formatoValor($Celdas['valor']);
                        $c += $Celdas['valor'];
                        break;
                    case 'D': $debito = formatoValor($Celdas['valor']);
                        $d += $Celdas['valor'];
                        break;
                }
        ...
        $Salida .= "<table width=\"100%\" align=\"center\" style='margin-top:120px;'> 
                           <tr>
...
//echo $sql;			
    ...
    function ObtenerFacturasCruzadasRC()
    {
        ...
        $sql = "
                                ...
                                b.prefijo,
                                b.factura_fiscal,
                                b.valor_total as total_factura,
                                b.saldo,
                                b.porcentaje_rtf as retencion_fuente,
                                '' as estado,
                                TO_CHAR(b.fecha_registro,'DD/MM/YYYY') AS registro,
                                SUM(a.valor_abonado) AS abono 
                                from rc_detalle_tesoreria_facturas a
                                left join (
                                        select aa.empresa_id, aa.prefijo, aa.factura_fiscal,(aa.valor_total - aa.valor_notacredito) as valor_total, aa.saldo, aa.porcentaje_rtf, aa.fecha_registro 
                                    from inv_facturas_despacho aa
                                    union 
                                    select bb.empresa_id, bb.prefijo, bb.factura_fiscal,(bb.valor_total - bb.valor_notacredito) as valor_total, bb.saldo, bb.porcentaje_rtf,bb.fecha_registro 
                                    from inv_facturas_agrupadas_despacho bb
                                    
                                    union

                                    select cc.empresa_id, cc.prefijo, cc.factura_fiscal, cc.total_factura as valor_total, cc.saldo, cc.porcentaje_rtf, cc.fecha_registro
                                    from fac_facturas cc
                                ) as b on a.empresa_id = b.empresa_id and a.prefijo_factura = b.prefijo and a.factura_fiscal = b.factura_fiscal
                                where a.recibo_caja = {$this->datos['recibo_caja']} AND a.prefijo = '{$this->datos['prefijo']}' AND a.empresa_id = '{$this->datos['empresa_id']}' 
