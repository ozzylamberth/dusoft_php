...
//echo "where".$where;
    ...
    function ObtenerRecibosPendientes()
    {
        ...
        while (!$rst->EOF)
        {
			...
			$this->action10[$i] = ModuloGetURL('app', 'RecibosCaja', 'user', 'MostrarFacturasCSV', $arreglo);
...
//echo $sql;
    ...
    function ObtenerRcDetalles($tercero_tipo, $tercero_id, $prefijo = "", $factura_fiscal = "")
    {
		...
		if($prefijo && $factura_fiscal){
		
		$sql .= "AND RC.prefijo = '" . $prefijo . "'
					 AND RC.recibo_caja = " . $factura_fiscal . "";
		 }

		$sql .= " ORDER BY RD.recibo_caja ASC; ";
	...
	if (!$_SESSION['SqlBuscarF'] || !$_SESSION['SqlContarF'])
	    ...
	    $this->actionCarga = ModuloGetURL('app', 'RecibosCaja', 'user', 'subirArchivo',$arreglo);
        $this->action1 = ModuloGetURL('app', 'RecibosCaja', 'user', 'GenerarReciboCaja', $arreglo);
        $this->actionPg = ModuloGetURL('app', 'RecibosCaja', 'user', 'MostrarFacturas', $arreglo);
	...
	function subirArchivo() {
	   ...
	   $arreglo = array("pagina" => $_REQUEST['pagina'], "tercero_id" => $_REQUEST['tercero_id'],
        ...
        if (isset($_FILES) && count($_FILES) > 0) {
            ...
            if ($this->esCsvValido($_FILES['archivo']['type'])) {
				...
				if (trim($sw_cruce_recibos) == 1) {
					...
					$resultado = $this->insertarRecibosDesdePlano($csvdatos);
				
				} else {
				
					$resultado = $this->insertarCruceDesdePlano($csvdatos);
                ...
                if (count($resultado["novalidos"]) > 0) {
                    ...
                    $path = $dir_siis . "tmp/recibos_caja/errores.txt";

                    $fp = fopen($path, "w+");

                    fwrite($fp, "Total de recibos con error: " . count($resultado["novalidos"]) . PHP_EOL . PHP_EOL);

                    foreach ($resultado["novalidos"] as $r) {

                        fwrite($fp, "\r\n Error " . $r["mensaje"] . PHP_EOL . PHP_EOL);
                    }

                    fclose($fp);
                    $doc = ModuloGetURL("app", "RecibosCaja", "user", "LogProductos");
                } else {
                    $success = "recibos de caja agregados exitosamente";
                }
            } else {
                $error = "El archivo no es valido";
            }
        }
		$this->visualizaFormasCSV($error, $success, $doc);
        return true;
    }
		
	function insertarRecibosDesdePlano($csvdatos) {

        $array = $this->cargar_archivo_plano();

        $novalidos = array();

        foreach ($array["datos"] as $recibo) {
		
            $csvdatos["prefijo"] = trim($recibo[0]);
            $csvdatos['factura_fiscal'] = trim($recibo[1]);
			$csvdatos["prefijo_rc"] = trim($recibo[2]);
            $csvdatos['recibo_caja'] = trim($recibo[3]);
			$csvdatos["valor_abonado"] = trim($recibo[4]);
            $csvdatos['retefuente'] = trim($recibo[5]);
			$csvdatos["retecre"] = trim($recibo[6]);
            $csvdatos['reteica'] = trim($recibo[7]);
						
            //realiza busqueda del recibo 
            $factura = $this->BuscarFactura($csvdatos["prefijo"], $csvdatos['factura_fiscal'] );
			
			if (!empty($factura)) {
			
				if (is_numeric($csvdatos['valor_abonado']) && $csvdatos['valor_abonado'] > 0  && is_numeric($csvdatos['retefuente']) & is_numeric($csvdatos['retecre']) && is_numeric($csvdatos['reteica'])) {
				
					$suma = $csvdatos['valor_abonado'] + $csvdatos['retefuente'] + $csvdatos['retecre'] + $csvdatos['reteica'];
					
					if ($suma <= $factura['saldo']) {

						$recibo_caja = $this->ObtenerRcDetalles($_REQUEST['tercero_tipo'], $_REQUEST['tercero_id'],$csvdatos["prefijo_rc"],$csvdatos['recibo_caja']);
					
						if (!empty($recibo_caja)) {
						
							if ($suma <= $recibo_caja[0]['valor_actual']) {

								 $this->insertarReciboCSV($csvdatos,$suma);
								 
							}else{
								$novalidos[] = array("valor_actual" => $recibo_caja[0]['valor_actual'],
									"valor_abonado" => $suma,
									"mensaje" => "el valor abonado no puede superar el valor del recibo (" . $csvdatos['prefijo_rc']  . " " . $csvdatos['recibo_caja']  . ")"
								);
							}	
						
						}else{
							$novalidos[] = array("prefijo_rc" => $csvdatos["prefijo_rc"],
								"recibo_caja" => $csvdatos["recibo_caja"],
								"mensaje" => "recibo de caja " . $csvdatos['prefijo_rc']  . " " . $csvdatos['recibo_caja']  . " no encontrado o con saldo 0"
							);
						}	
						
					}else{
						$novalidos[] = array("saldo" => $factura["saldo"],
							"valor_abonado" => $suma,
							"mensaje" => "el valor abonado no puede superar el saldo de la factura (" . $csvdatos['prefijo']  . " " . $csvdatos['factura_fiscal']  . ")"
						);
					}
					
				}else{
					$novalidos[] = array("valor_abonado" => $csvdatos["valor_abonado"],
						"retefuente" => $csvdatos["retefuente"],
						"retecre" => $csvdatos["retecre"],
						"reteica" => $csvdatos["reteica"],
						"mensaje" => "el valor abonado debe ser mayor a cero y las retenciones deben ser numericas"
					);
				}	
				
			}else {
				$novalidos[] = array("prefijo" => $csvdatos["prefijo"],
					"factura_fiscal" => $csvdatos['factura_fiscal'],
					"mensaje" => "factura " . $csvdatos['prefijo']  . " " . $csvdatos['factura_fiscal']  . " no encontrada o con saldo 0"
				);
            }
        }
				
        return array("novalidos" => $novalidos);
    }
			
	function insertarCruceDesdePlano($csvdatos) {
        ...
        foreach ($array["datos"] as $recibo) {
            ...
            $csvdatos["prefijo"] = trim($recibo[0]);
            $csvdatos['factura_fiscal'] = trim($recibo[1]);
			$csvdatos["valor_abonado"] = trim($recibo[2]);
            $csvdatos['retefuente'] = trim($recibo[3]);
			$csvdatos["retecre"] = trim($recibo[4]);
            $csvdatos['reteica'] = trim($recibo[5]);
    ...
    function LogProductos() {
        $filename = GetVarConfigAplication('DIR_SIIS') . "tmp/recibos_caja/errores.txt";
    ...
    function esCsvValido($type) {
        ...
        $mimes = array('application/vnd.ms-excel', 'text/plain', 'text/csv', 'text/tsv', 'application/download','application/octet-stream');
    ...
    function BuscarFactura($prefijo, $factura_fiscal){
            ...
            $sql = "SELECT  
                    ...
                    FROM    inv_facturas_despacho IFD
                    WHERE   IFD.saldo > 0 and IFD.tercero_id={$_REQUEST['tercero_id']} and IFD.tipo_id_tercero='{$_REQUEST['tercero_tipo']}' and IFD.empresa_id='{$_SESSION['RCFactura']['empresa']}' 
					AND IFD.prefijo = '{$prefijo}'                               
					AND IFD.factura_fiscal = {$factura_fiscal}
                    UNION ALL
                    SELECT  
                    TO_CHAR(IFD.fecha_registro, 'DD/MM/YYYY') AS fecha1,
                    '0' AS envio_id,
	...
	function insertarReciboCSV($datos,$suma){
	...
	$sw_cruce_recibos = $_SESSION[Documentos][sw_cruce_recibos];
        ...
        {						
            ...
            $sql .= "UPDATE rc_detalles
                                     SET    valor_actual=    valor_actual - '" . $datos['valor_abonado'] . "'
                                     WHERE  recibo_caja = '" . $datos['recibo_caja'] . "'
                                     AND    prefijo = '" . $datos['prefijo_rc'] . "'; ";
