...
//                                     and d.departamento_actual=h.departamento";
	...
	$sql = AutoCarga::factory("FacturasDespachoSQL", "classes", "app", "FacturasDespacho");	       
        ...
        $query = "INSERT INTO fac_facturas(
                        ...
                        documento_id, tipo_factura, centro_utilidad,tipo_pago_id)
                 VALUES('$emp','$prefijo',$factura,0," . UserGetUID() . ",'now()', NULL,'$tipoTerceroFacPaciente','$idTerceroFacPaciente','$sw_clase'
                    ," . $documento . ",5,'$cutilidad',$tipoPago); ";
    
        $Documento['prefijo']=$prefijo;
        $Documento['numeracion']=$factura;
        $Documento['empresa_id']=$emp;
        $query .= $sql->InsertarPcFactura($Documento,'2');
       
        
        $dbconn->Execute($query);
        if ($dbconn->ErrorNo() != 0) {
            $this->error = "Error al Guardar fac_facturas";
            $this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg()." <pre>".$query;
...
//            print_r('<pre>fac_facturas_contado: '.$sqls);
            ...
            if ($dbconn->ErrorNo() != 0) {
                ...
                $this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg()."<pre>".$sqls;
                $dbconn->RollbackTrans();
                return false;
            }
        }
        ...
        //se comenta ya que david debe cambiar  el consecutivo y fecha despues
        ...
        if($resultado_sincronizacion_ws['resultado_ws']=='0'){
            $mensajeFI  = "INTEGRACIï¿½N FI "."<BR>";
            $mensajeFI .=  $resultado_sincronizacion_ws['mensaje_ws']." Factura Nï¿½mero: ".$prefijo.$factura."<BR>";
        }
        */
    ...
    function ActualizarImpuestosFactura($impuestos, $empresa, $prefijo, $factura){
        ...
        if(!empty($impuestos) && count($impuestos) > 0){
            
            if($impuestos['porcentaje_rtf'] != ""){
                 $porcentaje_rtf = $impuestos['porcentaje_rtf'];
            }
           
            if($impuestos['porcentaje_ica'] != ""){
                  $porcentaje_ica = $impuestos['porcentaje_ica'];
            }
            
             if($impuestos['porcentaje_reteiva'] != ""){
                  $porcentaje_reteiva = $impuestos['porcentaje_reteiva'];
            }
           
             if($impuestos['porcentaje_cree'] != ""){
        ...
        $sql = " UPDATE fac_facturas set 
            porcentaje_rtf = {$porcentaje_rtf}, 
            porcentaje_ica = {$porcentaje_ica},  
            porcentaje_reteiva = {$porcentaje_reteiva},  
            porcentaje_cree = {$porcentaje_cree}
    ...
    function subirFacturasPorPlano(){
         list($dbconn) = GetDBconn();
        $archivo = realpath('./'). '/facturastalonarios.csv';
       //  $sql_1 = AutoCarga::factory("FacturasDespachoSQL", "classes", "app", "FacturasDespacho");
     //   $sql_2 = AutoCarga::factory("ContratacionProductosClienteSQL", "classes", "app", "ContratacionProductosCliente");

        $facturasnovalidas = array();
        ...
        if (($gestor = fopen($archivo, "r")) !== FALSE) {
                ...
                $numero = count($datos);
                  ...
                  if($fila > 0){
                   ...
                   // $sql = "UPDATE  documentos_transaccionales SET fecha_registro='{$datos[3]}' WHERE prefijo= '{$datos[1]}' AND numero={$datos[0]}; ";
                      //inisializar
                      $prefijo = trim($datos[2]);
                      $factura = $datos[3];
                      $valor   = (int)$datos[5];
                      $valorgravament = (trim($datos[4]) != "")?$datos[4]:0;
                      $valorgravament = (int)$valorgravament;
                      $tipoTerceroFacPaciente = $datos[0];
                      $idTerceroFacPaciente = $datos[1];
                      $fecha = $datos[6];
                      ...
                      $id = 0;
                       ...
                       if(is_numeric($valor) && $valor > 0){
                                    $query = "SELECT documento_id FROM documentos WHERE prefijo = '{$prefijo}'";
                                    $result  = $dbconn->execute($query);
                                   $documento = 0;
                                 ...
                                 /*   while($row=pg_fetch_assoc($result)){
                                    ...
                                    while (!$result->EOF) {
                                     $documento= $result->GetRowAssoc($ToUpper = false);
                                      $result->MoveNext();
                                  }
                                    ...
                                    $query = "INSERT INTO fac_facturas(
                                              empresa_id, prefijo, factura_fiscal, estado, usuario_id, fecha_registro, plan_id, tipo_id_tercero, tercero_id, sw_clase_factura,
                                              documento_id, tipo_factura, centro_utilidad, total_factura, gravamen, concepto, saldo)
                                               VALUES('03','{$prefijo}',{$factura},0,1378,'{$fecha}', NULL,'{$tipoTerceroFacPaciente}','{$idTerceroFacPaciente}','1' ,{$documento['documento_id']},5,'1', {$valor}, {$valorgravament}, 'SALDO CARTERA 2013', {$valor})";
                                ...
                                if ($dbconn->ErrorNo() == 0) {
                                              //selecciona
                                        if($prefijo == "FB" || $prefijo == 'FV' || $prefijo == 'IN' || $prefijo == 'IM'){
                                            $caja_id = 629;
                                            $concepto_caja = 18;
                                            $grupoconcepto = 8;
                                        }  else  if($prefijo == "FE" || $prefijo == 'ME1'){
                                            $caja_id = 630;
                                             $concepto_caja = 17;
                                            $grupoconcepto = 8;
                                        ...
                                        //selecciona el numero del concepto
                                        $query = "SELECT nextval('fac_facturas_conceptos_fac_factura_concepto_id_seq') as id";
                                       $result =  $dbconn->Execute($query);
                                        ...
                                        while (!$result->EOF) {
                                           $id= $result->GetRowAssoc($ToUpper = false);
                                            $result->MoveNext();
                                        }

                                        //se inserta el concepto

                                         $query = "INSERT INTO fac_facturas_conceptos
                                                                                (fac_factura_concepto_id,
                                                                                empresa_id,
                                                                                prefijo,
                                                                                factura_fiscal,
                                         ...
                                         //  echo $query ."</br>";
                                            ...
                                            if ($dbconn->ErrorNo() != 0) {
                                                  $this->error = "Error insert  fac_facturas_conceptos";
                                                  $this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
                                                  $dbconn->RollbackTrans();
                                                  return false;
                                              }
                                           ...
                                           $query = "INSERT INTO fac_facturas_conceptos_dc(fac_factura_concepto_id,
                                                                                        empresa_id,
                                                                                        concepto_id,
                                                                                        grupo_concepto)
                                                    VALUES(     {$id['id']},
                                       ...
                                       //   echo $query. "</br>";
                                         ...
                                         if ($dbconn->ErrorNo() != 0) {
                                              $this->error = "Error UPDATE fac_facturas_conceptos_dc ";
                                              $this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
                                              $dbconn->RollbackTrans();
                                              return false;
                                          }

                                           //actualizar los impuestos de la factura
                                     //   $Tercero = $sql_2->ConsultarTercero_Contrato($_SESSION['CAJA']['EMPRESA'], $idTerceroFacPaciente, $tipoTerceroFacPaciente);
                                       // $Parametros_Retencion = $sql_1->Parametros_Retencion($_SESSION['CAJA']['EMPRESA']);  
                                         $impuestos = $this->TraerPorcentajeImpuestos($_SESSION['CAJA']['EMPRESA'], $idTerceroFacPaciente, $tipoTerceroFacPaciente);

                                         $query =  $this->ActualizarImpuestosFactura($impuestos, $_SESSION['CAJA']['EMPRESA'], $prefijo, $factura);
                                         $dbconn->execute($query);


                                         if ($dbconn->ErrorNo() != 0) {
                                              $this->error = "Error UPDATE fac_facturas  impuestos";
                                              $this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
                                              $dbconn->RollbackTrans();
                                              return false;
                                          }
                                } else {
                                    $facturasnovalidas[] = array("error" => $dbconn->ErrorMsg(), "factura" => $factura, "prefijo" => $prefijo, "sql" => $query );
                                }
                            ...
                            $facturasnovalidas[] = array("error" => "valor de factura no valido", "factura" => $factura, "prefijo" => $prefijo , "sql" => "");
                       }
            ...
            foreach($facturasnovalidas as $factura){
                echo $factura['error'] . " **** DETALLE DE LA FACTURA ***** " .$factura['prefijo'] . " - " . $factura['factura'] ."</br>" . "sql de la factura ".$factura['sql']."</br></br>";
            }
...
//*********************************************************************************************
    ...
    function InsertarDetalle() {
       ...
       if (!$_REQUEST['cantidad'] /*|| !$_REQUEST['precio'] */|| !is_numeric($_REQUEST['gravamen']) || $_REQUEST['gravamen'] < 0 || !$_REQUEST['observacion'] ||$_REQUEST['forma_pago']=='-1' || $ip=="" ) {
            if($ip==""){
             $this->frmError["MensajeError"] = 'LA IP NO TIENE PERMISO PARA REALIZAR LA PETICIÓN';
            }
            ...
            if ($_REQUEST['forma_pago']=='-1') {
                $this->frmError["forma_pago"] = 1;
                $this->frmError["MensajeError"] = 'Debe tener un tipo de Pago';
            }
        ...
        if (!empty($_REQUEST['gravamen'])) {
            ...
            $valGravamen = $_REQUEST['gravamen'] ;
            $precio = ($precio + $valGravamen);
        } else {
            $_REQUEST['gravamen'] = 0;
        }
    ...
    function ConsultaInsertadosDetalle() {
        ...
        $sql = "SELECT a.concepto_id,a.cantidad,a.precio,a.descripcion, a.valor_gravamen,a.porcentaje_gravamen,a.valor_total, a.concepto_id,a.grupo_concepto, 
                    c.descripcion as desconcepto, b.descripcion as desgrupo,a.rc_concepto_id, a.tipo_id_tercero, a.tercero_id,e.descripcion as descripcionpago,e.tipo_pago_id 
                    FROM tmp_detalle_conceptos a
                    ...
                    left join tipo_pago as e on (a.tipo_pago_id=e.tipo_pago_id)
    ...
    function WSTerceroFI($id, $nombre, $tipodea) {
        ...
        if(count($result['buscarTerceroGeneralDocumentoResult']) > 0){
            ...
            //EMPIEZA INTEGRACIï¿½N DUSOFT FINANCIERO Y DUSOFT ASISTENCIAL PARA CREAR TERCERO EN CASO DE LA NO EXISTENCIA
    ...
    function TraerReciboCaja($Caja, $tipocuenta, $usuario, $sw_cuadrada, $cierre) {
        ...
        if (($tipocuenta == '03' AND $sw_cuadrada) OR ($tipocuenta == '08' AND $sw_cuadrada)) {
            ...
            $query = "SELECT a.empresa_id,a.centro_utilidad,a.factura_fiscal as recibo_caja,
                                    ...
                                    AND a.prefijo=v.prefijo
                                    AND a.factura_fiscal=v.factura_fiscal
                                    and b.caja_id = {$_SESSION['CAJA']['CAJAID']}
                                    $tipofactura
                                    AND a.cierre_caja_id NOT IN(
                                            SELECT b.cierre_caja_id
                                            FROM cierre_de_caja a,cierre_de_caja_detalle b
...
//-----------------------------------REPORTES--------------------------------------
    ...
    function LlamaFormaBuscar() {
        ...
        $this->SetXajax(array("sincronizar_facturas_pendientes_ws_fi"), "app_modules/CajaGeneral/RemoteXajax/facturas.php", "ISO-8859-1");
