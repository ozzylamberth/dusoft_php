...
class app_Facturacion_Conceptos {
    ...
    function traerLogsSincronizacion($prefijo, $factura){
        $sql = "SELECT * FROM logs_facturacion_clientes_ws_fi WHERE factura_fiscal = '{$factura}' AND prefijo= '{$prefijo}' AND numero_nota IS NULL";
    ...
    function TraerUsuariosConceptos($caja_id = null)
    {
    ...
    function ObtenerFacturasXPrefijo($datos, $emp)
    {
        ...
        $sql .= "				I.total_cheques, D.gravamen ";
        $sql .= "FROM 	terceros C, ";
    ...
    function ConexionBaseDatos($sql)
    {
        ...
        if ($dbconn->ErrorNo() != 0)
        {
            $this->frmError['MensajeError'] = $sql."ERROR DB : " . $dbconn->ErrorMsg();
    ...
    function obtener_factura_talonario($empresa_id, $prefijo, $numero){
        $sql = "select to_char(a.fecha_registro,'YYYY') as anio_factura, a.empresa_id, a.prefijo, a.factura_fiscal, a.documento_id, 
                a.tipo_id_tercero, a.tercero_id, a.porcentaje_rtf, 
                a.porcentaje_ica, a.porcentaje_reteiva, cast(a.total_factura as double precision ) as total_factura, a.fecha_registro, to_char(a.fecha_registro, 'dd/mm/yyyy') as fecha_factura, gravamen
                from fac_facturas a 
