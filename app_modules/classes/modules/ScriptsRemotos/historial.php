<?php
	/**************************************************************************************
	* $Id: historial.php,v 1.4 2006/09/22 14:55:27 hugo Exp $ 
				...
				foreach($formulados as $key => $datos)
				{
					...
					if($datos['usuario_suministro'])
					{
						...
						foreach($formulacion['suministro'][$datos['num_reg']] as $keyII => $suministro)
						{
							...
							$html .= "									</td>\n";							
							$html .= "									<td valign=\"top\" width=\"%\" class=\"normal_10\" align=\"right\">".$suministro['cantidad_suministrada']."</td>\n";
							$html .= "									<td valign=\"top\" width=\"%\" class=\"normal_10\" align=\"right\">".$suministro['cantidad_perdidas']."</td>\n";
			...
			foreach($historial as $key=> $nivel1)
			{
				...
				if( $nivel2['usuario_suministro'])
				{
					...
					foreach($nivel1 as $keyy => $nively)
					{
						...
						$html .= "						<td align=\"right\">".$nively['cantidad_suministrada']." ".$nively['unidad_dosificacion']."</td>\n";
						$html .= "					</tr>\n";
					}
				}
			}
