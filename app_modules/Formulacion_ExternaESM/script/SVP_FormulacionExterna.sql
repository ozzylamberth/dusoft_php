...
ADD COLUMN sw_bodegamindefensa char NOT NULL DEFAULT 0;

COMMENT ON COLUMN public.bodegas.sw_bodegamindefensa
IS 'Define si la Bodega es de Mindefensa';
...
GRANT ALL ON TABLE esm_farmaco_vigilancia_d TO siis;
 ...
 ALTER TABLE public.esm_farmaco_vigilancia_d
ADD COLUMN frecuencia text null;

COMMENT ON COLUMN public.esm_farmaco_vigilancia_d.frecuencia
IS 'Frecuencia del medicamento';

 ALTER TABLE public.esm_farmaco_vigilancia_d
ADD COLUMN fecha_vencimiento date null;

COMMENT ON COLUMN public.esm_farmaco_vigilancia_d.fecha_vencimiento
IS 'Fecha de vencimiento del medicamento despachado';

 ALTER TABLE public.esm_farmaco_vigilancia_d
ADD COLUMN lote 	character varying(255) 	
  null;

COMMENT ON COLUMN public.esm_farmaco_vigilancia_d.lote
IS 'lote  del medicamento despachado';
