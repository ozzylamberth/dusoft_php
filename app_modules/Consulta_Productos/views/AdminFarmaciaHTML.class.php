<?php
  ...
  IncludeClass("ClaseUtil");
	...
	*/
		...
		{
			$ctl   = AutoCarga::factory("ClaseUtil"); 
			$emp   = AutoCarga::factory('AdminFarmaciaSQL', '', 'app', 'Consulta_Productos');
			
			$dato  =$emp->ListarEmpresas();
			$html .= $ctl->LimpiarCampos();
			$html  = ThemeAbrirTabla("BUSCAR PRODUCTO");
			$html .= $ctl->RollOverFilas();
			...
			$html .= "         <tr class=\"formulacion_table_list\" >\n";
			$html .= "		          	<td  width=\"40%\" >EMPRESA:</td>\n";
			$html .= "							<td class=\"modulo_list_claro\" align=\"left\">";
			
			$html .= "								<select name=\"buscador[empresa_]\" id=\"buscador[empresa_]\" class=\"select\" style=\"width:70%\"  >";
			$html .="<option value=\"\" >-- SELECCIONAR --</option>";
			foreach($dato as $key => $dtl){
			$html .="<option value=\"".$dtl['empresa_id']."\">".$dtl['empresa_id']." - ".$dtl['razon_social']."</option>";
			}
			$html .= "								</select>";
			$html .= "							</td>";
			$html .= "	 </tr>\n";
      	...
      	$html .= "  <tr class=\"formulacion_table_list\" >\n";
			...
			if(!empty($datos))
			{
					...
					$html .= "      <td width=\"35%\">EMPRESA</td>\n";
          $html .= "      <td width=\"20%\">FARMACIA</td>\n";
