...
//            }
    ...
    //realiza operación de sumar o restar meses a una fecha
	...
	*	-Nota (1) Se modifica el query de consulta, y se añade una consulta mas atraves de un
	*		  union que contendra la dispensacion que se hizo como todo pendiente
	*		  y la tenga en cuenta como una dispensacion mas de la formula
	*	@fecha_modificaciones: (1)01/06/2016
	**/
	...
	/* $sql = "SELECT b.*, c.numero_entregas
					FROM (
                        ...
                        select a.evolucion_id,3 from hc_dispensacion_medicamentos_tmp a                       
                            ...
                            SELECT count(desp.evolucion_id) as numero_entregas, desp.evolucion_id
	...
	*-Nota: Se modifica la funcion para que tenga en cuenta las formulas que se dispensaron
	*		como todo pendiente
	**/
	function medicamentosDespachados($evolucion) {
	  /*SELECT  dd.codigo_producto,
								...
								to_char(pend.fecha_registro,'YYYY-mm-dd') as fecha_entrega,
								to_char(now()- pend.fecha_registro,'dd') as dias_de_entregado
                                 FROM hc_formulacion_despachos_medicamentos hcf
                         RIGHT JOIN hc_formulacion_despachos_medicamentos_pendientes desp ON hcf.evolucion_id = desp.evolucion_id
                         INNER JOIN hc_pendientes_por_dispensar pend
                         ON desp.evolucion_id = pend.evolucion_id 
                            AND desp.bodegas_doc_id = pend.bodegas_doc_id
                            AND desp.numeracion     = pend.numeracion
                         INNER JOIN bodegas_documentos_d AS dd
                         ON pend.bodegas_doc_id = dd.bodegas_doc_id
   						    and pend.numeracion = dd.numeracion
                            WHERE desp.evolucion_id = ".$evolucion." AND pend.todo_pendiente = 1  */
	  $sql = " SELECT * FROM(   SELECT   dd.codigo_producto,
								dd.cantidad as numero_unidades,
								dd.fecha_vencimiento ,
								dd.lote,
								fc_descripcion_producto_alterno(dd.codigo_producto) as descripcion_prod,
                        ...
                        INNER JOIN bodegas_documentos d
                         ON  
                             hc.bodegas_doc_id = d.bodegas_doc_id
                            AND hc.numeracion     = d.numeracion
                        INNER JOIN bodegas_documentos_d dd ON
                            dd.bodegas_doc_id = d.bodegas_doc_id
                            AND dd.numeracion     = d.numeracion
                            WHERE hc.evolucion_id = ".$evolucion." AND d.todo_pendiente = 1 
	...
	*@author Cristian Ardila
	*@fecha  28/04/2016
	*+Descripcion Metodo encargado de consultar el numero total de entregas que tiene una formula
	*
	**/
    function ConsultarTotalNumeroDeEntrega($Formulario, $evolucion) {

	
        $sql = "SELECT 
                    max( ceiling(ceiling(hc.fecha_finalizacion - hc.fecha_registro)/30) ) as numero_entregas
                ...
                WHERE    hc.evolucion_id='{$evolucion}' AND invp.descripcion ILIKE '%{$Formulario['descripcion']}%' ";
// echo "<pre>".$sql;
        if (!$resultado = $this->ConexionBaseDatos($sql))
            return $this->frmError['MensajeError'];
...
//        echo "<pre>".$sql;          
	...
	*/
    ...
    function Consultar_Medicamentos_Detalle_($Formulario, $evolucion) {
        ...
        //Se le agrego  ORDER BY  ceiling(ceiling(hc.fecha_finalizacion - hc.fecha_registro)/30)
        ...
        $sql = "SELECT  hc.codigo_medicamento as codigo_producto,
               ...
               (hc.fecha_finalizacion - hc.fecha_registro)  as diferencia_final_inicio,
                ...
                LEFT JOIN  inventarios_productos invp ON(hc.codigo_medicamento=invp.codigo_producto)
                JOIN hc_medicamentos_recetados_amb a ON hc.codigo_medicamento = a.codigo_producto AND hc.evolucion_id = a.evolucion_id
                WHERE    hc.evolucion_id='{$evolucion}' AND invp.descripcion ILIKE '%{$Formulario['descripcion']}%'  {$filtro} ORDER BY  ceiling(ceiling(hc.fecha_finalizacion - hc.fecha_registro)/30) ";
// echo "<pre>".$sql;
        if (!$resultado = $this->ConexionBaseDatos($sql))
            return $this->frmError['MensajeError'];
    ...
    function Consultar_Medicamentos_Detalle_P($Formulario, $evolucion) {
        ...
        $sql = "SELECT         tmp.codigo_medicamento as codigo_producto,
                    ...
                    CASE WHEN (MAX (tmp.fecha_registro+ '" . $horas . "hr'::interval) >= (now()))
                       THEN '0' ELSE '1' END as f_rango, hc.paciente_id, hc.tipo_id_paciente, hc.sw_autorizado



           FROM   hc_pendientes_por_dispensar tmp, hc_formulacion_antecedentes hc,
           ...
           AND    tmp.codigo_medicamento= A.codigo_producto and   tmp.sw_estado='0'
			AND tmp.evolucion_id = hc.evolucion_id AND tmp.codigo_medicamento = hc.codigo_medicamento
           AND     A.descripcion ILIKE '%" . $Formulario['descripcion'] . "%'
            " . $filtro;

        $sql .= " group by  tmp.codigo_medicamento,MED.cod_principio_activo, hc.paciente_id,hc.tipo_id_paciente, hc.sw_autorizado ";
...
//          exit(); 
	...
	*-Nota Se adiciona el estado todo pendiente a la consulta,
	*	   para identificar si la dispensacion fue causada 
	*	   al dejar toda la formula pendiente = 1
	*@fecha modificacion 01/06/2016
	*
	**/
    function pendientes_dispensados_ent($evolucion) {   
		...
		e.evolucion_id  ='" . $evolucion . "' and sw_estado='1' limit 1) as fecha_pendiente
                ...
                WHERE 
                tmp.evolucion_id = '" . $evolucion . "' AND d.todo_pendiente != 1 ;";
    ...
    function ConsultarUltimoResg_Dispens_($principio_activo, $paciente_id, $tipo_id_paciente, $producto, $today, $fecha_dias) {
		...
		/*echo "principio_activo ". $principio_activo;
		echo "paciente_id ". $paciente_id;
		echo "tipo_id_paciente ". $tipo_id_paciente;
		echo "producto ". $producto;
		echo "today ". $today;
		echo "fecha_dias ". $fecha_dias;*/
        $sql = "
                SELECT A.resultado,
                A.fecha_registro,
                A.unidades,
...
union
    ...
    SELECT 										
		...
		and 		d.fecha_registro >= '" . $fecha_dias . "'::date 
           ...
           ) AS A    ORDER BY  A.resultado ASC,A.fecha_registro ";
...
//echo "<pre>".$sql;
    ...
    function pendientes_dispensados_ent_TOTAL($evolucion) {
       ...
       /* $sql = "    SELECT   dd.codigo_producto,
           ...
           and tmp.numeracion = dd.numeracion
            ...
            and  tmp.evolucion_id ='" . $evolucion . "'  AND tmp.todo_pendiente != 1 ";
			//
		 
		/*$sql = " SELECT   dd.codigo_producto,
                          hcp.cantidad as numero_unidades,
                          dd.fecha_vencimiento ,
                          dd.lote,
                          fc_descripcion_producto_alterno(dd.codigo_producto) as descripcion_prod
           ...
           FROM hc_formulacion_despachos_medicamentos_pendientes tmp 
           INNER JOIN bodegas_documentos_d AS dd 
		   ON tmp.bodegas_doc_id = dd.bodegas_doc_id and tmp.numeracion = dd.numeracion
           RIGHT JOIN hc_pendientes_por_dispensar hcp 
		   ON hcp.evolucion_id = tmp.evolucion_id AND tmp.bodegas_doc_id = hcp.bodegas_doc_id and tmp.numeracion = hcp.numeracion
           WHERE tmp.evolucion_id ='" . $evolucion . "' AND  (hcp.todo_pendiente IS NULL OR hcp.todo_pendiente = 0) AND hcp.bodegas_doc_id IS NOT NULL AND hcp.numeracion IS NOT NULL";*/
    ...
    /**
	*@author Cristian Ardila
	*@fecha  01/11/2016 09:55
	*+Descripcion: Metodo el cual consultara el numero de entrega siguiente a partir de la fecha
	*			   de refrendacion
	**/
	function consultarNumeroEntregaRefrendacion($dtl) {
        //fecha_refrendacion
        $sql = "
				SELECT 
				round(ceiling('now()' - TO_DATE(TO_CHAR(k.fecha_registro,'DD/MM/YYYY'),'DD/MM/YYYY') )/30) as cantidad

				FROM(

				SELECT (a.fecha_refrendacion) as fecha_registro, 
				(SELECT distinct(max(b.fecha_finalizacion))
				 FROM hc_formulacion_antecedentes b
				 WHERE b.numero_formula = 546618 AND b.transcripcion_medica = 0
				 )as fecha_finalizacion
				FROM medicamentos_refrendados a
				WHERE numero_formula = '{$dtl['numero_formula']}' AND transcripcion_medica = '{$dtl['transcripcion_medica']}'
				)as k ;";
