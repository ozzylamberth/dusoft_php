					...
					*/
					...
					$fech;
					 if (!empty($busqueda)) {
						...
						$medicamentos = $sql->Medicamentos_Dispensados_Esm_x_lote_total($dtl['evolucion_id']);
						//echo "<pre>";
						...
						$informacion = $sql->Medicamentos_Pendientes($dtl['evolucion_id']);  
					  ...
					  **/
					  ...
					  foreach ($busqueda as $k => $valor) {  
						...
						if ($valor['refrendar'] == '1' ) {
						   ...
						   if(sizeof($producto_refrendado)>0){
							   ...
							   if(sizeof($fech)>0){
								...
								//Se comenta esta linea de codigo por qu estaba aumentando los dias de dispensacion
								/*list($a, $m, $d) = split("-", $valor['fecha_finalizacion']);
								 if(sizeof($fech)>0){
									 $valor['fecha_formulacion']=$fech[sizeof($fech)-1];
								 }*/
						...
						$consultarNumeroEntregaRefrendacion = $sql->consultarNumeroEntregaRefrendacion($dtl);
						...
						// echo "<pre>Fecha base 2";  print_r($fecha_base['fecha']);
						  ...
						  //echo " numero_de_entregas_de_producto ". $numero_de_entregas_de_producto ."\n";
						  //echo "cantidad_dispensacion ". $cantidad_dispensacion ."\n";
						  if ($numero_de_entregas_de_producto > $cantidad_dispensacion ) {
							...
							**/
							...
							//  echo "A QUI SE DEBE REALIZAR EL CAMBIO DE LA FECHA PARA QUE LAS ENTREGAS SE CORRAN " .$producto_refrendado['fecha_refrendacion'];
							  ...
							  //$valor['fecha_formulacion'] = '2016-01-10'; 
								...
								list($ano_ref, $mes_ref, $dia_ref) = split("-",$ultimaFecharefrendacion['fecha_refrendacion']);																
								$fecha_siguiente_entregasRef = date("Y-m-d", (mktime(0, 0, 0, $mes_ref, ($dia_ref), $ano_ref)));
								...
								* -Nota : Se le quito la cantidad de entregas pues con la fecha del ultimo registro +1 sera suficiente
								**/
								/*echo "<pre>fecha_siguiente_entregas";
								print_r($fecha_siguiente_entregas);
								echo "<pre>cantidadEntregaFormula";
								print_r($cantidadEntregaFormula);
								echo "<pre>Fecha formulacion";
								print_r($valor);
								echo "<pre>ultimaFecharefrendacion";
								print_r($ultimaFecharefrendacion);*/
								//$fecha_base=$sql->operacion_meses($fecha_siguiente_entregas,$cantidadEntregaFormula['numero_entregas']+1,"+");
								
								  $fecha_base=$sql->operacion_meses($fecha_siguiente_entregasRef,$consultarNumeroEntregaRefrendacion['cantidad'],"+");               
								  //echo " operacion_meses 1 --><pre> ";
								  //print_r($fecha_base);
							/*****************VOY POR A QUI *********FALTA ARREGLAR LA FECHA MINIMA************************/		
							  ...
							  // $fecha_base=$sql->operacion_meses($today,0,"+");
							   ...
							   //echo "<pre>Original ";
							    ...
							    //SE CAMBIO A QUI A 0 PARA QUE NO AUMENTE
								//$fecha_base=$sql->operacion_meses($valor['fecha_formulacion'],0,"+");
							    //echo "<pre> operacion_meses 2";
									//print_r($valor);
								$fecha_base=$sql->operacion_meses($valor['fecha_formulacion'],0,"+");
								//echo "AY MAS 2";
								//print_r($fecha_base);
						...
						*/				  
						$cantidad_dias_habiles = $calculo_fechas->obtener_dias_habiles($fecha_base,$fecha_maximo);
						  //while($cantidad_dias_habiles < $dias_vigencia){   OJO SE CAMBIO 
						 /* while($cantidad_dias_habiles < $dias_vigencia_formula){   
