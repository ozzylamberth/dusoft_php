...
class MedicamentoDispensado_report {
    ...
    function obtener_cabecera_formula() {
        ...
        $html .= "                  <table width=\"100%\" class=\"label\" border ='0' $style>\n";
        $html .= "                      <tr >\n";
        ...
        $html .= "                          <td  colspan=\"3\" align=\"left\"   >" . utf8_decode($datos_formula['nombres']) . "" . utf8_decode($datos_formula['apellidos']) . " </td>\n";
        $html .= "                      </tr>\n";
    ...
    /*function imprimir_ultima_entrega_medicamentos() {
        ...
        if (!empty($medicamentos_entregados)) {
            foreach ($medicamentos_entregados as $key => $value) {
				echo "Fecha entrega ". $value['fecha_entrega'];
        ...
        if (!empty($pendientes_entregados)) {
            foreach ($pendientes_entregados as $key => $value) {
                $datos_medicamentos[$value['fecha_entrega']][] = $value; //$value['fecha_entrega']
            }
        }
        ...
        if (!empty($dispensacion_medicamentos)) {
            ...
            $html .= "              <table width=\"100%\" class=\"label\" border ='0' $style>\n";
            $html .= "                  <tr >\n";
            $html .= "                      <td width=\"13%\" ><U>FECHA ENTREGA</U></td>\n";
            $html .= "                      <td width=\"13%\" ><U>CODIGO</U></td>\n";
            $html .= "                      <td width=\"35%\" ><U>MEDICAMENTO</U></td>\n";
            $html .= "                      <td width=\"13%\" ><U>CANTIDAD</U></td>\n";
            $html .= "                      <td width=\"13%\" ><U>FECHA VENC</U></td>\n";
            $html .= "                      <td width=\"13%\" ><U>LOTE</U></td>\n";
            $html .= "                  </tr>\n";
        ...
        if (!empty($dispensacion_medicamentos_pendientes)) {
            ...
            foreach ($dispensacion_medicamentos_pendientes as $item => $fila) {
                if (empty($fila['pendiente'])) {
                    ...
                    $V_unitario = $fila['total_costo'] / $fila['numero_unidades'];
                    $total_formula_D +=$costo;
                    $nombreDigito = $fila['nombre']; //no se ha probado
                    $html .= "              </tr>\n";
                }
            }
            $html .= "              </table>\n";
...
function imprimir_ultima_entrega_medicamentos() {
    ...
    function imprimir_entregas_medicamentos() {
		...
		$medicamentos_entregados = $sql->medicamentosDespachados($this->datos['evolucion']);
		
        //$pendientes_entregados = $sql->pendientes_dispensados_ent($this->datos['evolucion']);pendientes_dispensados_ent_TOTAL AND hcp.bodegas_doc_id IS NOT NULL AND hcp.numeracion IS NOT NULL
		$pendientes_entregados = $sql->pendientes_dispensados_ent_TOTAL($this->datos['evolucion']);
