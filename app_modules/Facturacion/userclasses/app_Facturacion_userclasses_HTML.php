...
/*        $RUTA = $_ROOT ."cache/hojacargos".$Cuenta.".pdf";
        ...
        if($Dev)
        {
            $this->salida .= ThemeAbrirTabla('DETALLES DE LA CUENTA No. '.$Cuenta.' (Devolución Insumos y Medicamentos) '.$Nombres.' '.$Apellidos);
        }
        if(!$Dev && !$vars)
        {
...
//Esto lo comente en el momento que se diseño la nueva clase para mostrar los cargos de la cuenta  
...
//            fin comente                                    
  ...
  */
    function FormaCuentaCirugias($Cuenta,$TipoId,$PacienteId,$Nivel,$PlanId,$Cama,$Fecha,$Ingreso,$var,$Transaccion,$TotalCopago,$TotalNo,$TotalEmpresa,$ValTotal,$accionQ)
    {
        ...
        $this->salida .= ThemeAbrirTabla('DETALLES DE LA CUENTA No. '.$Cuenta.' '.$Nombres.' '.$Apellidos.' TRANSACCION No. '.$Transaccion.' (Cirugías)');
        $this->EncabezadoEmpresa($Caja);
        $argu=array('Transaccion'=>$Transaccion,'Cuenta'=>$Cuenta,'TipoId'=>$TipoId,'PacienteId'=>$PacienteId,'Nivel'=>$Nivel,'PlanId'=>$PlanId,'Fecha'=>$Fecha,'Ingreso'=>$Ingreso);
        $this->Encabezado($PlanId,$TipoId,$PacienteId,$Ingreso,$Nivel,$Fecha,$argu,$Cuenta);
  ...
  */
  function FormaTiposCargos($Cuenta,$TipoId,$PacienteId,$Nivel,$PlanId,$Ingreso,$Fecha)
  {
       ...
       /* $this->salida .= "               <tr>";
        ...
        $this->salida .= "                  <td align=\"center\" class=\"modulo_list_oscuro\"><a href=\"$accionI\">Agregar Cirugía</a></td>";
        $this->salida .= "               </tr>";*/
        $this->salida .= "           </table><BR>";
        $accion=ModuloGetURL('app','Facturacion','user','Cuenta',array('Cuenta'=>$Cuenta,'TipoId'=>$TipoId,'PacienteId'=>$PacienteId,'Nivel'=>$Nivel,'PlanId'=>$PlanId,'Cama'=>$Cama,'Fecha'=>$Fecha,'Ingreso'=>$Ingreso));
        $this->salida .= "<form name=\"formabuscar\" action=\"$accion\" method=\"post\">";
...
//          $this->salida .= "</table>";
          ...
          $this->salida .= "</form>";
          if($contcols>1){
            $msg='Esta seguro que la División de la Cuenta No. '.$Cuenta.' esta Correcta';
...
*/
    ...
    */
    function FormaActivarCuentaDivision($PlanId,$Cuenta1,$Cuenta,$TipoId,$PacienteId,$Ingreso,$Nivel,$Fecha)
    {
          ...
          $this->salida .= "          </tr>";
          $this->salida .= "          <tr>";
          $this->salida .= "          <td class=\"label\" align=\"right\">CUENTA No. $Cuenta (División)</td>";
  ...
  */
    ...
    function FormaModificarCuotaPacienteCuotaModeradora($numero_cuenta,$valor_cuota,$motivos,$tipo_cuota)
    {
        ...
        elseif($tipo_cuota == "CUOTA_MODERADORA" || $tipo_cuota == 2)
            ...
            trigger_error("Error en el método FormaModificarCuotaPacienteCuotaModeradora<br>el parametro tipo_cuota tiene un valor invalido",E_USER_ERROR);
        ...
        $this->salida .= "          </tr>\n";
        $this->salida .= "          <tr>\n";
        $this->salida .= "              <td class=\"label\" valign=\"top\">OBSERVACIÓN</td>\n";
    ...
    function CallSetCuotaPaciente()
    {
        ...
        if(!is_numeric($valor))
        {
            $this->frmError["MensajeError"] = "EL CAMPO VALOR DEBE SER NUMÉRICO";
