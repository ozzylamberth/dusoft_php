<?php
  /******************************************************************************
  * $Id: DetalleCtaHTML.class.php,v 1.9 2007/03/16 19:53:32 carlos Exp $
  ...
  ********************************************************************************/
  IncludeClass('DetalleCta','','app','Facturacion');
	...
	class DetalleCtaHTML
	{
		...
		* @return array 
		***********************************************************************************/
		function CrearFormaDetalleCta($Cuenta,$Cerradas,$TipoId,$PacienteId,$Nivel,$PlanId,$Fecha,$Ingreso,$accionM,$accionE,$accionDevol,$objeto,$modificacionCargos){	
      ...
      if($vector){
        ...
        for($i=0;$i<sizeof($vector);$i++){
          ...
          if($vector[$i][transaccion]){
            if($Cerradas!='Cerradas'){                                                     
              $accionModi=$accionM.UrlRequest(array("Cuenta"=>$Cuenta,"TipoId"=>$TipoId,"PacienteId"=>$PacienteId,"Nivel"=>$Nivel,"PlanId"=>$PlanId,"Fecha"=>$Fecha,"Ingreso"=>$Ingreso,'Transaccion'=>$vector[$i][transaccion],'Datos'=>$vector[$i]));
          ...
          if(is_array($paquetes)){
            $html.= $this->MostrarDatosCargos($paquetes,$Cerradas,$estilo1,$accionM,$accionE,$accionDevol,$Cuenta,$TipoId,$PacienteId,$Nivel,$PlanId,$Fecha,$Ingreso,$modificacionCargos);              
            ...
            $html.= $this->MostrarDatosCargos($productos,$Cerradas,$estilo1,$accionM,$accionE,$accionDevol,$Cuenta,$TipoId,$PacienteId,$Nivel,$PlanId,$Fecha,$Ingreso,$modificacionCargos);           
          }
    ...
    ***********************************************************************************/ 
    ...
    function MostrarDatosCargos($productos,$Cerradas,$estilo1,$accionM,$accionE,$accionDevol,$Cuenta,$TipoId,$PacienteId,$Nivel,$PlanId,$Fecha,$Ingreso,$modificacionCargos){
      if($productos){
      ...
      for($i=0;$i<sizeof($productos);$i++){
        ...
        if($modificacionCargos==1 && $Cerradas!='Cerradas' && empty($productos[$i][cuenta_liquidacion_qx_id])){          
          if($productos[$i][codigo_producto]){
            $accionModi=$accionM.UrlRequest(array("Cuenta"=>$Cuenta,"TipoId"=>$TipoId,"PacienteId"=>$PacienteId,"Nivel"=>$Nivel,"PlanId"=>$PlanId,"Fecha"=>$Fecha,"Ingreso"=>$Ingreso,'Transaccion'=>$productos[$i][transaccion],'Datos'=>$productos[$i],'des'=>$productos[$i][des],'codigo'=>$productos[$i][codigo_agrupamiento_id],'doc'=>$productos[$i][bodegas_doc_id],'numeracion'=>$productos[$i][numeracion]));         
            $html .= "    <td><a href=\"$accionModi\">MODI</a></td>"; 
            $html .= "    <td>&nbsp;</td>"; 
          }else{
            $accionModi=$accionM.UrlRequest(array("Cuenta"=>$Cuenta,"TipoId"=>$TipoId,"PacienteId"=>$PacienteId,"Nivel"=>$Nivel,"PlanId"=>$PlanId,"Fecha"=>$Fecha,"Ingreso"=>$Ingreso,'Transaccion'=>$productos[$i][transaccion],'Datos'=>$productos[$i],'des'=>$productos[$i][des],'codigo'=>$productos[$i][codigo_agrupamiento_id],'doc'=>$productos[$i][bodegas_doc_id],'numeracion'=>$productos[$i][numeracion]));         
