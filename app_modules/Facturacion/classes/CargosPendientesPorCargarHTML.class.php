<?php
  /******************************************************************************
  * $Id: CargosPendientesPorCargarHTML.class.php,v 1.1 2007/07/30 18:41:25 carlos Exp $
  ...
  * 
  ********************************************************************************/
  IncludeClass('CargosPendientesPorCargar','','app','Facturacion');
...
// 				unset($_SESSION['ARREGLO_LIQUIDACIONQX_CARGOS']);
				...
				for($i=0; $i<sizeof($arr); $i++)
				{
								...
								$html .= "       </td>";
								$html .= "</form>";
								$acc = ModuloGetUrl("app","Facturacion","user","LlamaFrmEliminarPendientesXCargar",array('EmpresaId'=>$arr[$i][empresa_id], 'CentroUtilidad'=> $arr[$i][centro_utilidad],'Ingreso'=>$ingreso,'cargo_cups'=>$arr[$i][cargo_cups],'Cuenta'=>$cuenta,'procedimiento_pendiente_cargar_id'=>$arr[$i][procedimiento_pendiente_cargar_id],'PlanId'=>$Plan,'Cuenta'=>$cuenta,'TipoId'=>$TipoId,'PacienteId'=>$PacienteId,'Nivel'=>$Nivel,'Cama'=>$Cama,'Fecha'=>$Fecha,'Ingreso'=>$Ingreso));
								$html .= "       <td colspan=\"3\">";
		...
		*/
		function FormaCuentaCargosLiquidadosQX($Cuenta,$TipoId,$PacienteId,$Nivel,$PlanId,$Cama,$Fecha,$Ingreso,$vars,$Transaccion,$mensaje,$Dev,$Estado,$arre,$id)
		{
				...
				$accionT=ModuloGetURL('app','Facturacion','user','GetLlamaFormaCuantaPendientesCargar',array("Cuenta"=>$Cuenta,"TipoId"=>$TipoId,"PacienteId"=>$PacienteId,"Nivel"=>$Nivel,"PlanId"=>$PlanId,"Cama"=>$Cama,"Fecha"=>$Fecha,"Ingreso"=>$Ingreso,"vars"=>$vars,"Transaccion"=>$Transaccion,"mensaje"=>$mensaje,"Dev"=>$Dev,"Estado"=>$Estado,"arre"=>$arre));
				...
				if($_SESSION['ARREGLO_LIQUIDACIONQX_CARGOS'])
				{
						...
						$html .= "    </table>";
						$html .= "    <table border=\"0\" width=\"80%\" align=\"center\" class=\"normal_10\">";
						$action=ModuloGetURL('app','Facturacion','user','GetCargarALaCuentaPaciente',array("NoLiquidacion"=>$_SESSION['Liquidacion_QX']['LIQUIDACION_ID'],"TipoDocumento"=>$TipoId,"Documento"=>$PacienteId,"externo"=>1,"Cuenta"=>$Cuenta,"TipoId"=>$TipoId,"PacienteId"=>$PacienteId,"Nivel"=>$Nivel,"PlanId"=>$PlanId,
