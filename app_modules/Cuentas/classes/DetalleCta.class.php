<?php
  /******************************************************************************
  * $Id: DetalleCta.class.php,v 1.8 2011/07/25 20:37:18 hugo Exp $
  ...
  ********************************************************************************/
    ...
    ***********************************************************************************/ 
    ...
    {              
          ...
          $query="SELECT a.*,c.codigo_producto,
               (CASE WHEN a.consecutivo IS NOT NULL THEN fc_descripcion_producto(d.codigo_producto) ELSE a.descripcion END) as descripcion
                FROM 
                (
                  SELECT 
                       a.tarifario_id,
    ...
    ******************************************************/
    function CargosMedicamentosCuentaPaciente($NoLiquidacion,$Cuenta){
        ...
        (SELECT fc_descripcion_producto(d.codigo_producto) FROM inventarios_productos d WHERE c.codigo_producto=d.codigo_producto) as descripcion
    ...
    function BuscarCargosAgrupadosCuentas($Cuenta,$descripcion){
         ...
         $query="SELECT *
               ...
               (CASE WHEN a.consecutivo IS NOT NULL THEN fc_descripcion_producto(d.codigo_producto) ELSE a.descripcion END) as descripcion
                ...
                ORDER BY fecha_cargo::date,descripcion;
              ";        
    ...
    *************************************************/
    ...
    /**
    * Metodo donde se realiza la busqueda de las cuentas, 
    * que pertenecen a una factura anulada
    * 
    * @param integer $Cuenta Identificador de la cuenta
    * @param array $adicionales Arreglo de datos adicionales
    *
    * @return mixed
    */
    function BuscarDetalleCuentaAnulada($Cuenta,$adicionales)
    {
      /*
      1er select de cargos normales
      2do select de cargos agrupados
      ...
      */
      ...
      $sql = "(
                SELECT  a.transaccion,
                        ...
                        b.descripcion,
                        NULL::integer as paquete_codigo_id
                FROM    cuentas_facturas_anuladas CU,
                        cuentas_detalle_anuladas A,
                        tarifarios_detalle b
                WHERE   CU.numerodecuenta = '".$Cuenta."'
                AND     CU.prefijo = '".$adicionales['prefijo']."'
                AND     CU.factura_fiscal = ".$adicionales['factura']."
                AND     CU.cuenta_anulada_id = A.cuenta_anulada_id
                AND     a.cargo=b.cargo 
                AND     a.tarifario_id=b.tarifario_id   
                AND     a.codigo_agrupamiento_id IS NULL 
                AND     a.consecutivo IS NULL
    ...
    /**
    * Metodo donde se busca el detalle de los acrgos asociados a una cuenta
    * que esta asociada a una factura anulada
    *
    * @param integer $Cuenta Numero de la cuenta 
    * @param string $descripcion descripcion del codigo de agrupamiento
    * @param array $adicionales datos adicionales de la factura anulada
    *
    * @return mixed
    */    
    function BuscarCargosAgrupadosAnuladosCuentas($Cuenta,$descripcion,$adicionales)
    {
      $sql = "SELECT *
                FROM(
                SELECT a.tarifario_id,
    ...
    /**
    * Metodo donde se busca el detalle de los medicamentos devueltos asociados a 
    * una cuenta asociada a una factura anulada
    *
    * @param integer $NoLiquidacion Numero de Liquidacion
    * @param integer $Cuenta Numero de la cuenta 
    * @param array $adicionales datos adicionales de la factura anulada
    *
    * @return mixed
    */
    function CargosMedicamentosCuentaPacienteDevolAnulada($NoLiquidacion,$Cuenta,$adicionales)
    {
      $sql = "SELECT  c.codigo_producto,
              ...
              FROM    cuentas_codigos_agrupamiento a,
                      cuentas_facturas_anuladas CU,
                      cuentas_detalle_anuladas B,
                      bodegas_documentos_d c
              ...
              AND     CU.numerodecuenta = '".$Cuenta."'
              AND     CU.prefijo = '".$adicionales['prefijo']."'
              AND     CU.factura_fiscal = ".$adicionales['factura']."
              AND     CU.cuenta_anulada_id = B.cuenta_anulada_id
    ...
    /**
    * Metodo donde se busca el detalle de los medicamentos asociados a 
    * una cuenta asociada a una factura anulada
    *
    * @param integer $NoLiquidacion Numero de Liquidacion
    * @param integer $Cuenta Numero de la cuenta 
    ...
    * @return mixed
    */
    function CargosMedicamentosCuentaPacienteAnulada($NoLiquidacion,$Cuenta,$adicionales)
    {
      $sql = "SELECT  c.codigo_producto,
                      sum(c.cantidad) as cantidad,
                      sum(b.valor_cargo) as valor_cargo,
    ...
    /**
    * Metodo donde se busca el detalle de los equipos asociados a 
    * una cuenta asociada a una factura anulada
    *
    * @param integer $NoLiquidacion Numero de Liquidacion
    * @param integer $Cuenta Numero de la cuenta 
    ...
    * @return mixed
    */
    function DatosEquiposQXAnulada($NoLiquidacion,$Cuenta,$adicionales)
    {
      $query = "SELECT  a.*
                FROM    (
                          SELECT  c.precio as precio_plan,
                                  c.cantidad,
                                  c.valor_cargo,
                                  c.valor_cubierto,
                                  c.valor_nocubierto as valor_no_cubierto,
                          ...
                          FROM    cuentas_liquidaciones_qx_equipos_fijos b,
                                  ...
                                  cuentas_facturas_anuladas CU,
                                  cuentas_detalle_anuladas c,
                                  tarifarios_detalle e
                          WHERE   b.cuenta_liquidacion_qx_id='".$NoLiquidacion."'  
                          AND     b.cuenta_liquidacion_qx_id=a.cuenta_liquidacion_qx_id 
                          AND     a.codigo_agrupamiento_id=c.codigo_agrupamiento_id 
                          ...
                          AND     CU.prefijo = '".$adicionales['prefijo']."'
                          AND     CU.factura_fiscal = ".$adicionales['factura']."
                          AND     CU.cuenta_anulada_id = c.cuenta_anulada_id 
                          UNION ALL
                          SELECT  c.precio as precio_plan,
                                  c.cantidad,
                                  c.valor_cargo,
    ...
    /**
    * Metodo donde se busca el detalle de las cirugias pertenecientes a 
    * una cuenta asociada a una factura anulada
    *
    * @param integer $NoLiquidacion Numero de Liquidacion
    * @param integer $Cuenta Numero de la cuenta 
    ...
    * @return mixed
    */
    function DatosCirugiaAnulada($NoLiquidacion,$Cuenta,$adicionales)
    {
      GLOBAL $ADODB_FETCH_MODE;      
      $sql = "SELECT  d.tipo_id_cirujano,
                      d.cirujano_id,
                      d.cargo_cups,
                      d.consecutivo_procedimiento,
                      c.tarifario_id as tarifario_id_procedimiento,
                      c.cargo as cargo_procedimiento,
                      c.tipo_cargo_qx_id,
                      b.tarifario_id,
                      b.cargo,
                      c.porcentaje,
                      ...
                      b.valor_nocubierto,
                      b.valor_cubierto,
                      e.tipo_tercero_id as tipo_id_profesional,
                      e.tercero_id as profesional_id,
                      f.descripcion,uv.uvrs
              FROM    cuentas_codigos_agrupamiento a,
                      cuentas_facturas_anuladas CU,
                      cuentas_detalle_anuladas B                      
                      ...
                      ON (uv.consecutivo_procedimiento=c.consecutivo_procedimiento AND 
                          uv.tarifario_id=c.tarifario_id AND 
                          uv.cargo=c.cargo)
      ...
      $sql = "SELECT a.*
              FROM  (
                      ...
                      AND     CU.prefijo = '".$adicionales['prefijo']."'
                      AND     CU.factura_fiscal = ".$adicionales['factura']."
                      AND     CU.cuenta_anulada_id = c.cuenta_anulada_id                      AND     c.paquete_codigo_id IS NULL
                      UNION ALL
                      SELECT  c.precio as precio_plan,c.cantidad,c.valor_cargo,c.valor_cubierto,c.valor_nocubierto as valor_no_cubierto,c.porcentaje_gravamen,
                              e.descripcion,c.tarifario_id,c.cargo,c.facturado,c.sw_cuota_paciente,c.sw_cuota_moderadora,c.valor_descuento_empresa,
                              c.valor_descuento_paciente,'movil' as tipo_equipo,b.equipo_id,
                      ...
                      FROM    cuentas_liquidaciones_qx_equipos_moviles b,
                              cuentas_codigos_agrupamiento a,
                              cuentas_facturas_anuladas CU,
                              cuentas_detalle_anuladas c,                              tarifarios_detalle e
                      WHERE   b.cuenta_liquidacion_qx_id='".$NoLiquidacion."'  
                      AND     b.cuenta_liquidacion_qx_id=a.cuenta_liquidacion_qx_id 
                      AND     a.codigo_agrupamiento_id=c.codigo_agrupamiento_id 
                      AND     b.transaccion=c.transaccion 
                      ...
                      AND     CU.factura_fiscal = ".$adicionales['factura']."
                      AND     CU.cuenta_anulada_id = c.cuenta_anulada_id                      AND     c.paquete_codigo_id IS NULL
                      AND     c.paquete_codigo_id IS NULL
                    ) a
              ORDER BY a.tipo_equipo";
      if(!$resultado = $this->ConexionBaseDatos($sql))
        return false;                  
    ...
    /**
    * Metodo donde se busca el detalle de los paquetes pertenecientes a 
    * una cuenta asociada a una factura anulada
    *
    * @param integer $Cuenta Numero de la cuenta 
    * @param integer $paqueteId Identificador del paquete
    * @param array $adicionales datos adicionales de la factura anulada
    *
    * @return mixed
    */
    function BuscarPaquetesCuentasAnulada($Cuenta,$paqueteId,$adicionales)
    {              
      $sql = "SELECT  a.*,
                      c.codigo_producto,
                      (CASE WHEN a.consecutivo IS NOT NULL THEN fc_descripcion_producto(d.codigo_producto) ELSE a.descripcion END) as descripcion
