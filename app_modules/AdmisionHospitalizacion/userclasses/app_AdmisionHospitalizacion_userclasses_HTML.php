<?php
	/**************************************************************************************
	* $Id: app_AdmisionHospitalizacion_userclasses_HTML.php,v 1.10 2011/03/29 14:46:17 hugo Exp $
	...
	***************************************************************************************/
	...
	class app_AdmisionHospitalizacion_userclasses_HTML extends app_AdmisionHospitalizacion_user
	{
  	function app_AdmisionHospitalizacion_user_HTML()
		...
		***********************************************************************************/
		function FormaMenuHospitalizacion()
		{
			...
			$this->salida .= "				</tr>\n";*/
			if($_SESSION['AdmHospitalizacion']['tipoadmon'] == 'HS')
			{
				$this->salida .= "				<tr>\n";
			...
			$this->salida .= "				<tr>\n";
			$this->salida .= "					<td align=\"center\" $class height=\"17\"><a href=\"".ModuloGetURL('app','AdmisionHospitalizacion','user','OrdenHospitalizacionCirugia')."\"><b>ORDEN DE CIRUG�A</b></a></td>\n";
			$this->salida .= "				</tr>\n";
			if($_SESSION['AdmHospitalizacion']['tipoadmon'] == 'UR')
			{
				$this->salida .= "				<tr>\n";
		...
		function FormaListadoAdmisionHospitalizacion()
		{
			...
			if(!empty($this->Ordenes))
			{
				if(sizeof($this->Ordenes) > 0)
				{
					...
					$this->salida .= "        <td width=\"10%\" title=\"FECHA DE PROGRAMACI�N\">FECHA PRO.</td>\n";
      ...
      $this->salida .= "  {\n";
      $this->salida .= "		document.formabuscar.action = '".$this->action1."';\n";
			...
			$this->salida .= "</table>\n";				
			$this->salida .= "<center>\n";				
			$this->salida .= "	<div id=\"errorA\" class=\"label_error\"></div>\n";				
			$this->salida .= "</center>\n";
    ...
    /**********************************************************************************
		...
		*/
		function FormaBuscar()
		{
      IncludeFileModulo("Planes","RemoteXajax","app","AdmisionHospitalizacion");
      $this->SetXajax(array("ValidarPaciente"),null,"ISO-8859-1"); 
   
      $ctl = AutoCarga::factory("ClaseUtil");
      $this->salida .= $ctl->AcceptNum(false);

      $empresa=$_SESSION['AdmHospitalizacion']['empresa'];
      $centro_utilidad=$_SESSION['AdmHospitalizacion']['ctrutilidad'];
      ...
      $this->salida .= "      xajax_ValidarPaciente(xajax.getFormValues('formabuscar'));\n";
          ...
          if(!empty($tipo_dato))
       {
               ...
               {
                    $this->salida .= "				<input type=\"text\" class=\"input-text\" name=\"Documento\" onkeypress=\"return acceptNum(event)\"  maxlength=\"32\" value=\"".$this->PacienteId."\">\n";
                    $this->salida .= "			</td>\n";
      ...
      $this->salida .= ThemeCerrarTabla();
		...
		* Forma para mensajes.
		* 
		* @param string $informacion Informacion a mostrar
    ...
    *
      ...
      {
        $this->salida .= "		<form name=\"cancelar\" action=\"".$this->action4."\" method=\"post\">\n";
        $this->salida .= "			<td align=\"center\">\n";
        $this->salida .= "				<input type=\"submit\" class=\"input-submit\" value=\"Crear Pagare\" >\n";
        $this->salida .= "			</td>\n";
        $this->salida .= "		</form>\n";
      }
		...
		*************************************************************************************/
		...
		{
			$this->salida  = ThemeAbrirTabla('LISTADO ORDENES DE CIRUGIA');
			$this->salida .= "		<script language=\"javascript\">\n";
			$this->salida .= "			function mOvr(src,clrOver)\n";
			$this->salida .= "			{\n";
			$this->salida .= "				src.style.background = clrOver;\n";
			...
			if($this->Datos)
			{
				if(sizeof($this->Datos) > 0)
				{
					...
					$this->salida .= "			<tr class=\"modulo_table_list_title\">\n";
					$this->salida .= "				<td width=\"7%\" ><b>N� PRO</b></td>\n";
					...
					$this->salida .= "			</tr>";
					$action['orden_cirugia'] = ModuloGetURL('app','AdmisionHospitalizacion','user','CrearIngreso');
					...
					{						
						...
						//$action = ModuloGetURL('app','AdmisionHospitalizacion','user','CrearIngreso');
						...
						$this->salida .= "				<td align=\"center\" >\n";
						$this->salida .= "					<a class=\"label_error\" href=\"".$action['orden_cirugia'].URLRequest(array("datos"=>$this->Datos[$i]))."\">\n";
      ...
      if(!$this->Semanas) $this->Semanas = 0;
			...
			$this->salida  = ThemeAbrirTabla('ORDENES DE CIRUG�A - DEPARTAMENTO');
			$this->salida .= "<table align=\"center\" width=\"50%\">\n";
			$this->salida .= "	".$this->SetStyle("MensajeError")."\n";
			$this->salida .= "</table>\n";
			$this->salida .= "<form name=\"forma\" action=\"".$this->action2."\" method=\"post\" >\n";
		...
		*************************************************************************************/
		function FormaCrearIngreso()
		{
			$request = $_REQUEST;
			IncludeFileModulo("Planes","RemoteXajax","app","AdmisionHospitalizacion");
      $this->SetXajax(array("BuscarPlanRangoTAfiliados"),null,"ISO-8859-1"); 
			if(!$this->Semanas)	$this->Semanas = 0;
			
			$this->salida  = ThemeAbrirTabla('ORDENES DE CIRUGIA');
			$this->salida .= "<script>\n";
			...
			$this->salida .= "		<tr class=\"modulo_table_list_title\">\n";
			$this->salida .= "			<td align=\"left\" class=\"modulo_list_claro\">\n";
			$responsables = $this->Responsables(1);
  
			$this->salida .= "<script>xajax_BuscarPlanRangoTAfiliados('".$request['datos']['plan_id']."')</script>";
			...
			$this->salida .= "				<select name=\"Responsable\" class=\"select\" onchange=\"xajax_BuscarPlanRangoTAfiliados(xajax.getFormValues(forma))\">\n";
      
			foreach($responsables as $key => $dtl)
			{
				($request['Responsable'] == $dtl['plan_id'])? $sl = "selected":$sl = "";
				if ($sl == "" && $dtl['plan_id'] == $request['datos']['plan_id'])
					$sl = "selected";
        
				$this->salida .=" 				<option value=\"".$dtl['plan_id']."\" ".$sl.">".$dtl['plan_descripcion']."</option>\n";
			}
			$this->salida .= "				</select>\n";
			...
			$this->salida .= "	</table><br>\n";
			$this->salida .= "     <div align=\"left\" id='rangoTipoAfiliadC'></div><br>\n";
		...
		***********************************************************************************/
		function FormaBuscarPacientes()
		{
			...
			if(!empty($this->paciente))
			{
				...
				$this->salida .= "			<td width=\"12%\">IDENTIFICACI�N</td>\n";
        ...
        $this->salida .= "			<td width=\"%\">OPCIONES</td>\n";
				...
				for($i=0; $i<sizeof($this->paciente); $i++)
				{
					...
					{
						case 'URG':
							$mensaje = "EL PACIENTE SE ENCUENTRA EN CONSULTA DE URGENCIAS, EN LA ESTACI�N: ".$this->paciente[$i]['estacion']." ";
						break;
						case 'EEF':
							$mensaje = "EL PACIENTE POSEE UN INGRESO PENDIENTE, EN LA ESTACI�N: ".$this->paciente[$i]['estacion']." ";
						break;
						case 'MVH':
							$mensaje = "EL PACIENTE SE ENCUENTRA HOSPITALIZADO EN LA HABITACION ".$this->paciente[$i]['pieza'].", CAMA ".$this->paciente[$i]['cama']."DE URGENCIAS, LA ESTACI�N: ".$this->paciente[$i]['estacion']." ";
						break;
						case 'CUE':
							...
							if($this->paciente[$i]['estado'] == "1")
							{
								...
								$mensaje .= "			<td align=\"center\"><b><a href=\"".$action1."\" title=\"ASIGNAR A UNA ESTACI�N\">ESTACI�N</a></b></td>\n";
		...
		***********************************************************************************/
		function FormaBuscarPacientesModificar()
		{
			...
			$this->salida .= "		</tr>\n";
			$this->salida .= "		<tr class=\"modulo_table_list_title\">\n";
			$this->salida .= "			<td $estilo>N�MERO HISTORIA</td>\n";
			$this->salida .= "			<td align=\"left\" class=\"modulo_list_claro\">\n";
		...
		***********************************************************************************/
		function FormaModificarAdmision()
		{
			...
			$this->salida .= "			<td align=\"left\" class=\"modulo_list_claro\" colspan=\"4\">\n";
			...
			$this->salida .= "			</td>\n";
 			$this->salida .= "			<td style=\"text-align:left;text-indent:11pt\">HORA:</td>\n";
      $this->salida .= "			<td align=\"left\" class=\"modulo_list_claro\" >\n";
			$this->salida .= "				<select name=\"hora_ingreso\" class=\"select\">\n";
			
      $horas = explode(":",$dat['hora_ingreso']);
      for($i = 0; $i<24 ; $i++ )
      {
        $val = str_pad($i,2,"0",STR_PAD_LEFT);
        ($horas[0] == $val)?$ch= "selected":$ch = "";
        $this->salida .= "				  <option value=\"".$val."\" ".$ch.">".$val."</option>\n";
      }
      $this->salida .= "				</select> : \n";
      $this->salida .= "				<select name=\"minuto_ingreso\" class=\"select\">\n";
			for($i = 0; $i<60 ; $i++ )
      {
        $val = str_pad($i,2,"0",STR_PAD_LEFT);
        ($horas[1] == $val)?$ch= "selected":$ch = "";
        $this->salida .= "				  <option value=\"".$val."\" ".$ch.">".$val."</option>\n";
      }
      $this->salida .= "				</select>\n";
			$this->salida .= "			</td>\n";
        ...
        {
          $this->salida .= "							<select name=\"TipoAfiliado\" class=\"select\" onchange=\"xajax_BuscarNivel(this.value,'".$this->PlanId."')\">\n";
          $this->salida .= "								<option value=\"-1\">---SELECCIONAR---</option>\n";
        ...
        $this->salida .= "						</td>\n";
        $this->salida .= "				 		<td style=\"text-align:left;text-indent:11pt\" colspan=\"2\">RANGO: </td>\n";
        $this->salida .= "						<td align=\"left\" class=\"modulo_list_claro\" width=\"25%\">\n";
        
        if(sizeof($Niveles) > 1)
        {
          $this->salida .= "						<select name=\"Nivel\" class=\"select\">\n";
          $this->salida .= "							<option value=\"-1\">--SEL--</option>\n";
            
          for($i=0; $i<sizeof($Niveles); $i++)
          {
            ($Niveles[$i]['rango'] == $dat['rango'])? $sel = "selected": $sel = "";
      ...
      $this->salida .= "		    <tr class=\"modulo_table_list_title\" height=\"20\">";
      $this->salida .= "					<td colspan=\"5\">COMENTARIOS</td>\n";
      $this->salida .= "		  	</tr>\n";
      $this->salida .= "		    <tr class=\"modulo_list_claro\">\n";
   ...
   // $this->debug = true;
			...
			if(sizeof($paciente) > 0)
			{
				...
				$this->salida .= "			<td width=\"12%\" >ESTANCIA</td>\n";
        $this->salida .= "			<td width=\"7%\" >CLASIFICACION TRIAGE</td>\n";
				...
				for($i=0; $i<sizeof($paciente); $i++)
				{
					...
					$action = ModuloGetURL('app','AdmisionHospitalizacion','user','AdmitirPaciente',
                                        ...
                                        'tipo_afiliado_id'=>$paciente[$i]['tipo_afiliado_id'],
                                        'rango'=>$paciente[$i]['rango']));
          ...
          $x = explode(".",$paciente[$i]['tiempo_estancia']);
          $y = explode("days",$x[0]);
          if(sizeof($y) == 1)
          {
            $y[1] = $y[0];
          ...
          $z = explode(":",$y[1]);
          $y[0] = ((($y[0])? $y[0]." D ":"").(($z[0])? $z[0]." H ":"").(($z[1])? $z[1]." M ":""));
          ...
          $this->salida .= "				<td align=\"center\"><b>".$y[0]."</b></td>\n";
          $this->salida .= "				<td style=\"background:".$paciente[$i]['bgcolor']."\">$nivel</td>\n";
		...
		***********************************************************************************/
		function FormaAdmitirTriage()
		{
			...
			{
				$this->salida .= "            <option value=\"".$this->Puntos[$i]['punto_triage_id']."-".$this->Puntos[$i]['descripcion']."\">".$this->Puntos[$i]['descripcion']."</option>\n";
			}
			$this->salida .= "          </select>\n";
			$this->salida .= "				</td>\n";
			$this->salida .= "			</tr>\n";
		...
		***********************************************************************************/
		function BuscadorPacientes()
		{
			...
			$buscador .= "		<table width=\"100%\" align=\"center\">\n";
			$buscador .= "			<tr>\n";
      $buscador .= "        <td width=\"20%\" class=\"normal_10AN\">TIPO DOCUMENTO:</td>\n";
		...
		**********************************************************************************************/
		function BuscadorProgramaciones()
		{
			...
			$buscador .= "	<table class=\"modulo_table_list\" width=\"100%\">\n";
			$buscador .= "		<tr>\n";
      $buscador .= "      <td width=\"40%\" class=\"modulo_table_list_title\">\n";
			$buscador .= "				BUSCADOR DE PROGRAMACIONES:\n";
			$buscador .= "			</td>\n";
			$buscador .= "			<td width=\"25%\" class=\"normal_10AN\">No PROGRAMACION:</td>\n";
			$buscador .= "			<td>\n";
			$buscador .= "				<input type=\"text\" class=\"input-text\" name=\"programacion\" style=\"width:100%\" maxlength=\"100\" onkeypress=\"return acceptNum(event)\" value=\"".$this->Programacion."\">\n";
			$buscador .= "			</td>\n";
			$buscador .= "			<td width=\"10%\">\n";
			$buscador .= "				<input type=\"submit\" class=\"input-submit\" name=\"buscar\" value=\"Buscar\">\n";
			$buscador .= "		</td></tr>\n";
			$buscador .= "	</table>\n";
			$buscador .= "</form>\n"; 
