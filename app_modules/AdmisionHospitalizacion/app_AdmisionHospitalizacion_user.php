<?php
	/**************************************************************************************
	* $Id: app_AdmisionHospitalizacion_user.php,v 1.10 2011/03/29 14:46:17 hugo Exp $
	...
	***************************************************************************************/
	class app_AdmisionHospitalizacion_user extends classModulo 
	{
		...
		*************************************************************************************/
		...
		{
			if(!empty($_SESSION['AdmHospitalizacion']['paciente']['plan_id']))
			{
				$datos['idp'] = $_SESSION['AdmHospitalizacion']['paciente']['paciente_id'];
				$datos['tipoid'] = $_SESSION['AdmHospitalizacion']['paciente']['tipo_id_paciente'];
				$datos['plan_id'] = $_SESSION['AdmHospitalizacion']['paciente']['plan_id'];
			}
			else
			{
				$datos['idp'] = $this->Documento;
				$datos['tipoid'] = $this->TipoDocumento;
				$datos['plan_id'] = $this->Plan;
			}
			
			$datos['afiliado'] = $_REQUEST['afilia'];
			if($_SESSION['AdmHospitalizacion']['tipoorden'] == 'Externa')
			{
				$action1 = ModuloGetURL('app','AdmisionHospitalizacion','user','Buscar',array('TIPOORDEN'=>'Externa'));  
				$action2 = ModuloGetURL('app','AdmisionHospitalizacion','user','RetornoAutorizacion',array('TIPOORDEN'=>'Externa'));  
			}
			else
			{
				$action1 = ModuloGetURL('app','AdmisionHospitalizacion','user','MetodoBuscarHospitalizacion');
				$action2 = ModuloGetURL('app','AdmisionHospitalizacion','user','RetornoAutorizacion');
			}
		...
		*************************************************************************************/
		function Buscar()
		{
			...
			$this->action1 = ModuloGetURL('app','AdmisionHospitalizacion','user','FormaDatosPaciente');
      ...
      $this->request = $_REQUEST;
			...
			$this->Documento = $this->request['Documento'];
			...
			$datos = array();
			$datos['tipo_id_paciente'] = $this->request['TipoDocumento'];
			if($this->request['Documento'])
				$datos['paciente_id'] = $this->request['Documento'];
			$datos['plan_id'] = $this->request['Responsable'];
			
			$_REQUEST['tipo_id_paciente'] = $this->request['TipoDocumento'];
			$_REQUEST['paciente_id'] = $this->request['Documento'];
			$_REQUEST['plan_id'] = $this->request['Responsable'];
			$_REQUEST['afiliacion'] = true;
	
			if($this->request['remision'] == "2") 
				$metodo = 'Remision';  
			else if($this->request['remision'] == "3") 
				$metodo = 'AdmitirTriage'; 
				else 
					$metodo = 'AutoHospitalizacion'; 
			
			$this->action['cancelar'] = ModuloGetURL('app','AdmisionHospitalizacion','user','Buscar');
			$this->action['volver'] = ModuloGetURL('app','AdmisionHospitalizacion','user',$metodo,$datos);
			return true;
		}
		/************************************************************************************
		...
		*************************************************************************************/
		...
		{		
			...
			$this->Plan = $this->request['plan_id'];
			$this->Documento = $this->request['paciente_id'];
			$this->TipoDocumento = $this->request['tipo_id_paciente'];
			$this->AutorizarPaciente();
      ...
      $this->Pagares = $this->PermisosPagares(UserGetUID(),$_SESSION['AdmHospitalizacion']['empresa'],$_SESSION['AdmHospitalizacion']['ctrutilidad']);
      ...
      if(!empty($this->Pagares))
      {
        $dtSessionPagares['EMPRESA'] = $this->Pagares['empresa_id'];
        $dtSessionPagares['NOMEMPRESA'] = $this->Pagares['descripcion1'];						
        $dtSessionPagares['CENTROU'] = $this->Pagares['centro_utilidad'];						
        $dtSessionPagares['NOMCENTROU'] = $this->Pagares['descripcion2'];												
        $dtSessionPagares['DOCUMENTO'] = $this->Pagares['documento_id'];						
        
        SessionSetVar("PAGARES",$dtSessionPagares);
        
        $Autoriza = $this->ReturnModuloExterno('app','Pagares','user');
				$Autoriza->SetActionVolver($this->action1);
        
        $dtIngreso = $this->ObtenerCuentaIngreso($this->Ingreso);
        
        $rqD['Ingreso'] = $dtIngreso['ingreso'] ;
        $rqD['TipoId'] = $dtIngreso['tipo_id_paciente'] ;
        $rqD['PacienteId'] = $dtIngreso['paciente_id'] ;
        $rqD['Cuenta'] = $dtIngreso['numerodecuenta'] ; 
        $rqD['PlanId'] = $dtIngreso['plan_id'] ;
        $rqD['SaldoC'] = 1 ;
        
        $this->action4 = ModuloGetURL('app','Pagares','user','RetornoFacturacion',$rqD);
      }
		...
		*************************************************************************************/
		...
		{   
			if(!ModuloGetVar('', '', 'planes_x_centro_utilidad_'.$_SESSION['AdmHospitalizacion']['empresa'].''))
			{
				...
				$sql .= " AND       a.empresa_id = '".$_SESSION['AdmHospitalizacion']['empresa']."' ";
			...
			else
			{
				...
				$sql .= "			a.tipo_tercero_id ";
				$sql .= " FROM 		planes a, ";
				$sql .= " 			planes_centro_utilidad b ";
				$sql .= " WHERE 	a.fecha_final >= now() ";
				$sql .= " AND 		a.estado=1 ";
				$sql .= " AND 		a.fecha_inicio <= now() ";
				$sql .= " AND       a.empresa_id = '".$_SESSION['AdmHospitalizacion']['empresa']."' ";
				$sql .= " AND       a.empresa_id = b.empresa_id ";
				$sql .= " AND       b.centro_utilidad = '".$_SESSION['AdmHospitalizacion']['ctrutilidad']."' ";
		...
		*************************************************************************************/
		...
		{	
			...
			{
				$_SESSION['AdmHospitalizacion']['paciente']['plan_id'] = $this->request['plan_id'];
				$_SESSION['AdmHospitalizacion']['paciente']['paciente_id'] = $this->request['paciente_id'];
				$_SESSION['AdmHospitalizacion']['paciente']['tipo_id_paciente'] = $this->request['tipo_id_paciente'];
			}
			$this->action1 = ModuloGetURL('app','AdmisionHospitalizacion','user','BuscarIngresoPaciente');
			$this->action2 = ModuloGetURL('app','AdmisionHospitalizacion','user','MenuHospitalizacion');
...
/*			
			...
			$where .= "AND   	DE.empresa_id = '".$_SESSION['AdmHospitalizacion']['empresa']."' ";
			...
			$where .= "AND		QP.qx_tipo_reserva_quirofano_id IN('3','6') ";
...
*/
			...
			$sql .= "		PA.paciente_id, ";
			$sql .= "		PA.primer_nombre||' '||PA.segundo_nombre||' '||PA.primer_apellido||' '||PA.segundo_apellido AS paciente, ";
			$sql .= "               (SELECT OS.evento_soat ";
			$sql .= "                FROM qx_programaciones_ordenes QO ";
			$sql .= "                   INNER JOIN os_maestro OM ON (OM.numero_orden_id = QO.numero_orden_id) ";
			$sql .= "                   INNER JOIN os_ordenes_servicios OS ON (OS.orden_servicio_id = OM.orden_servicio_id) ";
			$sql .= "                WHERE QO.programacion_id = QX.programacion_id ";
			$sql .= "               ) as evento_soat ";
                        
			$where .= "	FROM qx_programaciones QX ";
			$where .= "		INNER JOIN qx_quirofanos_programacion QP ON (QP.programacion_id = QX.programacion_id) ";
			$where .= "		INNER JOIN departamentos DE ON (DE.departamento = QX.departamento) ";
			$where .= "		INNER JOIN pacientes PA ON (QX.paciente_id = PA.paciente_id AND PA.tipo_id_paciente = QX.tipo_id_paciente) ";
			$where .= "	WHERE QX.estado = '1' "; 
			$where .= "	AND   DE.empresa_id = '".$_SESSION['AdmHospitalizacion']['empresa']."' ";
			$where .= "	AND	 QX.programacion_id NOT IN(";
			$where .= "									SELECT 	COALESCE(programacion_id,0) ";
		...
		*************************************************************************************/
		function CrearIngreso() 
		{
			...
			if($request['deptno'])
				$this->Departamento = $request['deptno'];
			else if($request['datos']['deptno'])
				$this->Departamento = $request['datos']['deptno'];
		...
		function SolicitarAutorizacion()
		{
			...
			else
			{
				...
				$_SESSION['AUTORIZACIONES']['AUTORIZAR']['tipo_id_paciente'] = $this->datos['tipo_id_paciente'];
 				...
 				$datos['idp'] = $this->datos['paciente_id'];
				$datos['tipoid'] = $this->datos['tipo_id_paciente'];
 				$datos['plan_id'] = $this->Responsable;
				$datos['tipo_afiliado_id'] = $_REQUEST['tipo_afiliado_id'];
				$datos['rango'] = $_REQUEST['rango'];

        
 				$action1 = ModuloGetURL('app','AdmisionHospitalizacion','user','BuscarOrdenes');
 				$action2 = ModuloGetURL('app','AdmisionHospitalizacion','user','CrearIngresoBD');
      ...
      $this->datos = $_SESSION['AdmHospitalizacion']['cirugia']['datos'];
			$this->Nivel = $request['autorizacion']['rango'];
			$this->Semanas = $request['autorizacion']['semanas'];			
			$this->Responsable = $_SESSION['AdmHospitalizacion']['cirugia']['responsable'] ;
			$this->TipoAfiliado = $request['autorizacion']['tipoafiliado'];
        ...
        $sql .= "       '".$_SESSION['AdmHospitalizacion']['cirugia']['observaciones']."');";
				...
				$sql  = "UPDATE qx_programaciones ";
				$sql .= "SET    plan_id = ".$this->Responsable." ";
				$sql .= "WHERE  programacion_id = ".$this->datos['programacion_id']." ";
				...
				$sql  = "UPDATE qx_procedimientos_programacion ";
				$sql .= "SET    plan_id = ".$this->Responsable." ";
				$sql .= "WHERE  programacion_id = ".$this->datos['programacion_id']." ";
			...
			else
			{
					...
					$Ingreso = $this->ValidarCuenta($NumeroCuenta);
			}
		...
		*************************************************************************************/
		...
		{
			$sql .= "	SELECT	a.estacion_id, a.descripcion, a.departamento
								FROM		estaciones_enfermeria as a, departamentos b ";
			if($op == "1")
				$sql .= "WHERE	a.sw_observacion_urgencia = '1' 
						 AND	a.departamento = b.departamento
						 AND    b.centro_utilidad = '".$_SESSION['ADMISIONES']['CENTROUTILIDAD']."'";
			else
				$sql .= "WHERE	a.sw_consulta_urgencia = '1' 
				         AND	a.departamento = b.departamento
						 AND    b.centro_utilidad = '".$_SESSION['ADMISIONES']['CENTROUTILIDAD']."'";
		...
		*************************************************************************************/
		function Remision()
		{
			...
			$datos = array();
			$datos['menu'] = $_SESSION['AdmHospitalizacion']['menu'];
			$datos['TIPOORDEN'] = $_SESSION['AdmHospitalizacion']['tipoorden'];
			
			$this->action2 = ModuloGetURL('app','AdmisionHospitalizacion','user','Buscar',$datos);
			
			$datos['afilia'] = $this->request['afilia'];
			$this->action1 = ModuloGetURL('app','AdmisionHospitalizacion','user','CrearRemision',$datos);
			$this->FormaRemision();
			return true;
		}
		/************************************************************************************
		...
		*************************************************************************************/
		...
		{
			$paciente = $_SESSION['AdmHospitalizacion']['paciente'];
    ...
    *************************************************************************************/
    function BuscarDatosIngresoPaciente($Ingreso)
    {
    	...
    	$sql .= "				TO_CHAR(IG.fecha_ingreso,'DD/MM/YYYY') AS fecha_ingreso, ";
    	$sql .= "				TO_CHAR(IG.fecha_ingreso,'HH:MI') AS hora_ingreso, ";
    ...
    *************************************************************************************/
		...
		function ModificarDatosAdmision()
    {
			...
			$Ingreso = $_REQUEST['Ingreso'];
			$Hora = $_REQUEST['hora_ingreso'];
			$Minuto = $_REQUEST['minuto_ingreso'];
			...
			$sql .= "UPDATE ingresos ";
			$sql .= "SET		fecha_ingreso='".$this->fec." ".$Hora.":".$Minuto."', ";
    ...
    *************************************************************************************/
    function BuscarPacientesTriages()
    {
      ...
      $sql  = "SELECT 	CASE WHEN TR.nivel_triage_id <> 0 THEN TR.nivel_triage_id
                        ELSE TR.nivel_triage_asistencial END as nivel_triage_id, ";
			$sql .= "					TR.triage_id,";
			$sql .= "					TR.plan_id,";
      ...
      $sql .= "         TR.tipo_afiliado_id, ";
      $sql .= "         TR.semanas_cotizadas, ";
      $sql .= "         TR.rango, ";
			$sql .= "					TO_CHAR(hora_llegada,'DD/MM/YYYY') AS fecha_ingreso , ";
			$sql .= "					TO_CHAR(hora_llegada,'HH24:MM:SS') AS hora_llegada, ";
			$sql .= "					PC.primer_nombre||' '||PC.segundo_nombre AS nombres, ";
			$sql .= "					PC.primer_apellido||' '||PC.segundo_apellido AS apellidos, ";
      $sql .= "					CASE WHEN TR.nivel_triage_id <> 0 THEN NTR.bgcolor
                        ELSE NTRA.bgcolor END as bgcolor, ";
      $sql .= "					now() - TR.hora_llegada AS tiempo_estancia ";
     	$where  = "FROM 	triages TR
                        LEFT JOIN niveles_triages as NTRA ON (TR.nivel_triage_asistencial = NTRA.nivel_triage_id), ";
			$where .= "				pacientes PC, ";
      $where .= "				niveles_triages NTR ";
      ...
      $where .= "AND		TR.nivel_triage_id = NTR.nivel_triage_id ";
			...
			$sql .= $where;
      $sql .= "ORDER BY		nivel_triage_id ASC, tiempo_estancia DESC ";
			$sql .= "LIMIT ".$this->limit." OFFSET ".$this->offset;
    ...
    //  $dbconn->debug=true;
		...
		*************************************************************************************/
		function AdmitirPaciente()
		{
			...
			IncludeClass('Autorizaciones','','app','NCAutorizaciones');
			$action1 = ModuloGetURL('app','AdmisionHospitalizacion','user','ListarPacientesTriages');
			$action2 = ModuloGetURL('app','AdmisionHospitalizacion','user','RetornoAutorizacion');
			
			$datos = array();			
			$datos['idp'] = $_REQUEST['PacienteId'];
			$datos['tipoid'] = $_REQUEST['TipoId'];
			$datos['plan_id'] = $_REQUEST['Responsable'];
      $datos['tipo_afiliado_id'] = $_REQUEST['tipo_afiliado_id'];
      $datos['rango'] = $_REQUEST['rango'];
    ...
    *************************************************************************************/
      ...
      if(GetIPAddress()!=$_REQUEST['Caja']['ip_address'] AND !empty($_REQUEST['Caja']['ip_address']))
      {
		...
		*************************************************************************************/
		function AdmitirTriage()
		{
			...
			$datos = array();
			$datos['afiliado'] = $this->request['afilia'];
			$datos['datos'] = $this->Puntos;
			
			$this->action[0] = ModuloGetURL('app','AdmisionHospitalizacion','user','AdmisionPacienteTriage',$datos);
    ...
    $i=0;
			...
			{
			  $rango=$_REQUEST['afiliado']['rango'];
        $tipoafiliado=$_REQUEST['afiliado']['tipoafiliado'];
        $semanasc=$_REQUEST['afiliado']['Semanas'];
    		$PuntoTriage = explode("-",$_REQUEST['punto_triage']);
				//$PuntoTriage[0];
				$paciente = $_SESSION['AdmHospitalizacion']['paciente'];
				$PacienteId = $paciente['paciente_id'];
				$Cutilidad = $_SESSION['AdmHospitalizacion']['ctrutilidad'];
				$Empresa = $_SESSION['AdmHospitalizacion']['empresa'];
		    $TipoId = $paciente['tipo_id_paciente'];
		    $PlanId = $paciente['plan_id'];
				$Dpto = $_SESSION['AdmHospitalizacion']['deptno'];
				$Pto = $_SESSION['AdmHospitalizacion']['ptoadmon'];
    ...
    *
    */
    function PermisosPagares($usuario,$empresa,$centro)
    {
    	$sql  = "SELECT a.documento_id,";
      $sql .= "       a.empresa_id, ";
      $sql .= "       a.centro_utilidad,";
      $sql .= "				b.razon_social as descripcion1, ";
      $sql .= "       c.descripcion as descripcion2 ";
      $sql .= "FROM   userpermisos_pagares as a, ";
      $sql .= "       empresas as b, ";
    ...
    *
    */
    function ObtenerCuentaIngreso($ingreso)
    {
    	$sql  = "SELECT	A.ingreso, ";
      $sql .= "       A.tipo_id_paciente,";
      $sql .= "       A.paciente_id ,";
	...
	function ValidarCuenta($numerodecuenta)
    {
			$sql  = "SELECT	A.ingreso ";
			$sql .= "FROM	cuentas A ";
			$sql .= "WHERE 	A.numerodecuenta = ".$numerodecuenta."  ";
