...
function Medicamento_Despachado($privilegios,$valor,$evolucion,$dias_dipensados,$datos_ex, $busqueda){
    ...
    $html .= "<table width=\"70%\" align=\"center\"  class=\"modulo_table_list\">";
                            ...
                            if ($privilegios['sw_privilegios'] == '1') {
                                ...
                                $html .= "      <td  >";
                                $html .= "      <input type=\"button\" class=\"input-submit\" value=\"AUTORIZAR DESPACHO DEL MEDICAMENTO\" style=\"width:100%\" onclick=\"xajax_Autorizacion_despacho(xajax.getFormValues('buscador'),'" . $evolucion . "','" . $bodega_otra . "',document.getElementById('observaciones').value,'" . $valor['codigo_producto'] . "','" . $busqueda . "');\" >";
                                $html .= "      </td>";
                                $html .= "    </tr>\n";
                                $html .= "    </table>";
                            }
...
/**
* @fecha 03/05/2016
* +Descripcion Metodo que se ejecutara en el campo TextField Descripcion,
*			   para buscar los medicamentos que se necesitan dispensar
*			   Antes de todo, se validara los medicamentos a traves de
*			   la confrontacion
*
**/
function BuscarProducto1($FormularioBuscador, $evolucion, $bodega_otra) {
    ...
    //consulta todos los medicamentos de la formula 
    ...
    if (!empty($busqueda)) {
      ...
      foreach ($informacion as $key => $valuer) {
        ...
        foreach ($busqueda as $k => $valor) {
			...
			/**
			*+Descripcion Proceso para validar la confrontacion de una formula
			**/
			...
			if (!empty($datos_ex)) {
			 ...
			 if ($today >= $fecha_registro_) {
					...
					/**
					* +Descripcion Calculola diferencia de dias entre la ultima entrega y el dia actual
					**/
					/*$segundos=strtotime($today) - strtotime($fecha_fin_despacho);
					$diferencia_dias=intval($segundos/60/60/24);
			
			if ($diferencia_dias >= 25 || $valor['sw_autorizado'] == '1' ||  !empty($valor['sw_autorizado']) == '1') { //If*/
			...
			**/
		    ...
		    if ($today >= $fecha_registro_) {
				...
				if ($today > $fecha_fin_despacho || $valor['sw_autorizado'] == '1') { //If
...
//                        $continuar=true;
            ...
            $html .=estado_por_cumplir($valor['codigo_producto'],$valor['descripcion_prod'],$continuarq['minimo']);
                break;
					...
					$html .= " </td>";
					$html .= " <td align=\"left\" width=\"94%\" class=\"label\" >";
					$html .= "  El paciente ya finalizo el tratamiento con este product [{$valor['codigo_producto']}-{$valor['descripcion_prod']}] <br> Fecha Finalizaci&oacute;n: {$valor['fecha_finalizacion']} ";
					$html .= "  </td>";
					$html .= " </tr>\n";
					$html .= "</table >";
					$html .= "<br>";
            ...
            }//CIerra today > $fecha_fin_despacho
		
					
		} else {
				/**
				*+Descripcion Se invoca el formulario encargado de autorizar un despacho
				*             una vez confrontados los medicamentos
				**/
				$html .= Medicamento_Despachado($privilegios,$valor,$evolucion,$dias_dipensados,$datos_ex,1);
                                           
           }
			
		}//today >= $fecha_registro_
    }
           $html .= "<br>";
}
...
function MostrarProductox($evolucion) {
    ...
    if (!empty($vector)) {
        ...
        $html .= "                          <input type=\"hidden\" name=\"evolucion\" id=\"evolucion\" value=\"" . $evolucion . "\">";
        $html .= "                                                <input class=\"input-submit\" type=\"button\" value=\"REALIZAR ENTREGA...\" onclick=\"xajax_Cambiarvetana('" . $evolucion . "', '0');\">";
        $html .= "                                          </td>";
        $html .= "                    </table>\n";
    } else {
...
/*
 *+Descripcion Metodo encargado de buscar los medicamentos pendientes que se dispensaran
 *			   este metodo tambien permitira confrontar 
 *
 */
function BuscarProducto2($FormularioBuscador, $evolucion, $bodega_otra) {
	...
	* +Descripcion Bloque de codigo exclusivo para confrontar los medicamentos pendientes
	*  @fecha 2016-09-08
	*
	*/		
	if (!empty($busqueda)) {                                                                   
        ...
        foreach ($busqueda as $k => $valor) {
			...
			**/
		    ...
		    if ($today >= $fecha_registro_) {
			   ...
			   /*
				*
				* +Descripcion Hasta a qui llegan los cambios de autorizacion confrontados pendientes
				*
				*/			
	...
	/**
	*+Descripcion Se cierran las tres (3) llaves del proceso de confrontacion
	**/
					}else{
						
						$html .= Medicamento_Despachado($privilegios,$valor,$evolucion,$dias_dipensados,$datos_ex,2);
					    $html .= "<br>";
...
function Autorizacion_despacho($Formulario, $evolucion, $bodega_otra, $observacion, $producto, $busqueda) {
    ...
    if ($vector == true) {
		...
		if($busqueda == 2){
			...
			$objResponse->script("xajax_BuscarProducto2(xajax.getFormValues('buscador'),'" . $evolucion . "','" . $bodega_otra . "');");
