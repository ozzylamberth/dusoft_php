...
*/
	...
	function Menu(){
		...
		$this->salida .='<a href="'.$accion.'">Modificación Agenda Médica</a>';
		$this->salida .='</td>';
		$this->salida .='</tr>';
		$this->salida .='</table>';
		$this->salida .='<br>';
...
*/
      ...
      if($_REQUEST['volver2']=='true'){
			...
			if($profesionales){
				...
				while($i<sizeof($profesionales[0])){
					...
					$this->salida .= "<a href=".ModuloGetURL('app','CreacionAgenda','user','LlamaCreacionAgendaNueva',array('tercero'=>$profesionales[1][$i],'tipoid'=>$profesionales[0][$i],'nombrep'=>$profesionales[2][$i]))."><img title=\"Creación Agenda\" border=\"0\" src=\"".GetThemePath()."/images/planblanco.png\"></a>";
					$this->salida .= '</td>';
	...
	*/
    ...
    $this->salida  = ThemeAbrirTabla('CONSULTA AGENDA MÉDICA');
		$this->Encabezado();
		$this->salida .= '<table width="70%" align="center" class="modulo_table_list">';
    ...
    $turnos=$this->TurnosProgramadosProfesional($tipoid,$tercero,1);
      ...
      /*$this->salida .= '<BR><table width="70%" align="center" class="modulo_table_list" border="0" cellpadding="4">';
        ...
        */
		...
		$this->salida .= '  <tr><td width="5%" class="modulo_list_claro">&nbsp;</td><td>Existencia de Turnos de Agenda Médica</td></tr>';
		$this->salida .= '  </table>';
    $this->salida .= ThemeCerrarTabla();
    return true;
	}
	...
	function CreacionAgendaNueva($tercero,$tipoid,$nombrep){
		...
		$this->salida .= '<tr class="modulo_table_title"><td align="center" colspan="22">AÑO:&nbsp&nbsp&nbsp&nbsp&nbsp;';
      ...
      if($v!='metodo' and $v!='modulo' and $v!='tipo' and $v!='SIIS_SID'){
		...
		$this->salida .= '<td colspan="8">Días del Mes</td>';
		$this->salida .= '<td align="center" colspan="6">Todos los días</td>';
...
*/
	function AgendaHtml()
	{
	  ...
	  $this->salida  = ThemeAbrirTabla('CREACIÓN AGENDA MÉDICA');
		...
		$this->salida .= "				  <tr class=\"modulo_list_claro\"><td class=\"label\">AÑO</td><td><select name=\"year\" onchange=\"year1(this.form)\" class=\"select\">";
      ...
      (list($ano,$mes,$dia)=explode('-',$fecha));
		...
		if($vector){
		  $this->salida .= '<table border="0" width="70%" align="center">';
			...
			$this->salida .= " </td></tr>";
      $this->salida .= '<tr align="center" class="modulo_table_title">';
      $this->salida .= '<td width="16%" nowrap>AÑO</td>';
			$this->salida .= '<td width="17%" nowrap>MES</td>';
			$this->salida .= '<td width="20%" nowrap>DÍA</td>';
	...
	function LlamaAgendaRetornoExterno($accion,$intervalo,$nombrep,$Fecha){
		...
		$this->salida .= '  <tr><td width="5%" class="modulo_table_calen">&nbsp;</td><td>Turno de Agenda Médica Creado</td></tr>';
		$this->salida .= '  </table>';
		$this->salida .= ThemeCerrarTabla();
		return true;
	}
...
*/
   ...
   if(empty($_REQUEST['DiaEspe'])){$_REQUEST['DiaEspe']=date("Y-m-d");}
		...
		$this->salida .= '<tr align="center" class="modulo_table_title">';
		$this->salida .= '<td align="center">Profesional</td>';
		$this->salida .= '<td align="center">Día Agenda</td>';
		$this->salida .= "</tr>";
		$this->salida .= '<tr class="modulo_list_oscuro">';
      ...
      $mensaje="NO SE PUEDE MODIFICAR LA AGENDA DEL DÍA:";
...
*/
	...
	function CambiarAgendaDia($mensaje)
	{
		...
		$this->salida=ThemeAbrirTabla('CAMBIAR AGENDA BUSQUEDA DE DÍA');
		$this->EncabezadoModificacion();
		$this->salida .= '<table width="70%" align="center" class="modulo_table_list">';
		$this->salida .= '<tr align="center" class="modulo_table_title">';
		$this->salida .= '<td align="center">';
		...
		if(!empty($_REQUEST['DiaEspe']))
		{
			...
			else
			{
				...
				$this->salida .= '<tr align="center">';
				$this->salida .= '<td align="center">';
				$this->salida .= '<label class="label_error">NO SE PUEDE CREAR AGENDA PARA EL DÍA '.$_REQUEST['DiaEspe'].'</label>';
				$this->salida .= "</td>";
				$this->salida .= "</tr>";
				$this->salida .= "</table>";
			}
...
/**
  ...
  function clear()
  {
    ...
    $this->salida .= '<tr><td width="50%">';
		...
		$this->salida .= '  <tr>';
		$this->salida .= "    <td rowspan=\"2\" class=\"label\">Día<BR>del<BR>Mes</td>";
		$this->salida .= '    <td align="center">';
    ...
    $this->salida .= '<td>';
    $this->salida .= '  <table width="95%" align="center" border="0" class="Normal_10">';
    $this->salida .= '  <tr><td width="10%" class="modulo_table">&nbsp;</td><td>Días hábiles del mes</td></tr>';
		$this->salida .= '  <tr><td width="10%" class="agendasab">&nbsp;</td><td>Días Sábados</td></tr>';
		$this->salida .= '  <tr><td width="10%" class="agendadomfes">&nbsp;</td><td>Días Domingos y Festivos</td></tr>';
		$this->salida .= '  <tr><td width="10%" class="modulo_table_calen">&nbsp;</td><td>Turnos de Agenda Médica creados dentro del Intervalo</td></tr>';
    $this->salida .= '  </table>';
		$this->salida .= '</td></tr>';
    $this->salida .= '</table>';
    $this->salida .= ThemeCerrarTabla();
