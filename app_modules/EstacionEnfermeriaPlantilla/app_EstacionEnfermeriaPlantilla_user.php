...
*		@package SIIS
*/
class app_EstacionEnfermeriaPlantilla_user extends classModulo
{
	var $frmError = array();
	...
	*		@return bool
	*/
	function app_EstacionEnfermeriaPlantilla_user()//Constructor padre
	{
		return true;
	}
	...
	*/
	...
	function main()
	{
	  $estacion_id=$_REQUEST['estacion'];
		...
		{
			$mensaje = "NO LLEGO LA IDENTIFICACION DE LA ESTACION";
		...
		if(!$this->GetLogueoEstacion($estacion_id))
		{
			$this->error = "No se puede la plantilla de estación de enfermeria";
			$this->mensajeDeError = "Ocurrió un error al intentar cargar la vista \"GetLogueoEstacion\"";
			return false;
		}
		return true;
	}//FIN main
	...
	*		@return bool
	*/
	function GetLogueoEstacion($estacion_id)
	{
	  $query =  "SELECT DISTINCT e.razon_social as descripcion1,
							...
							WHERE b.estacion_id='$estacion_id' AND
	...
	*/
	...
	{//pacientes de hospitalizacion
      if(empty($Estacion))
			...
			}//echo "<br>miralo";print_r($datoscenso); exit;
//print_r($datoscenso[hospitalizacion]);
			$query = "SELECT I.tipo_id_paciente,
								...
								ORDER BY P.primer_nombre,
													P.segundo_nombre,
													P.primer_apellido,
													P.segundo_apellido";//echo "<br>".$query;
			GLOBAL $ADODB_FETCH_MODE;
			list($dbconn) = GetDBconn();
			$ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
			$result = $dbconn->Execute($query);
	...
	function GetPacientesUrgencias($estacion_id)
	{
	  if(!$estacion_id)
		...
		else
		{
			...
			{
				while ($data = $result->FetchRow()){//echo "<br><br>-->";print_r($result->fields[suma]);
					$tmp[] = $data;
				}
				return $tmp;
			}
	...
	*/
	...
	function GetPacientesConMedicamentosPorDesp($datos_estacion)
	{
 		 if(!$datos_estacion)
		{
			...
			list($dbconn) = GetDBconn();
		//echo $query="(SELECT C.ingreso, B.numerodecuenta FROM ingresos_departamento A, cuentas B, ingresos C,  hc_evoluciones G, hc_medicamentos_recetados H WHERE A.estacion_id = '".$datos_estacion[estacion_id]."' AND B.numerodecuenta = A.numerodecuenta AND C.ingreso = B.ingreso AND G.ingreso = C.ingreso AND H.evolucion_id = G.evolucion_id AND H.sw_estado = '2' AND H.empresa_id = '".$datos_estacion[empresa_id]."' AND H.centro_utilidad = '".$datos_estacion[centro_utilidad]."') UNION (SELECT C.ingreso, B.numerodecuenta FROM ingresos_departamento A, cuentas B, ingresos C, hc_evoluciones G, hc_mezclas_recetadas MR, hc_mezclas_recetadas_medicamentos MRM WHERE A.estacion_id = '".$datos_estacion[estacion_id]."' AND B.numerodecuenta = A.numerodecuenta AND C.ingreso = B.ingreso AND G.ingreso = C.ingreso AND MR.evolucion_id = G.evolucion_id AND MR.sw_estado = '2' AND MRM.mezcla_recetada_id = MR.mezcla_recetada_id AND MRM.empresa_id = '".$datos_estacion[empresa_id]."' AND MRM.centro_utilidad = '".$datos_estacion[centro_utilidad]."');";
    $query="SELECT ingreso FROM cuentas
						...
						for($i=0;$i<sizeof($var);$i++)
						{
						 $query="SELECT COUNT(*) FROM hc_solicitudes_medicamentos
											...
											AND (sw_estado='1' OR sw_estado='5')";
											$resulta=$dbconn->execute($query);
								if ($dbconn->ErrorNo() != 0) {
											$this->error = "Error al Cargar el Modulo";
											$this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
	...
	function GetDevolucionMedicamentos($PacientesConOrdenes,$datos_estacion)
	{
	 ...
	 //print_r($PacientesConOrdenes);
		...
		foreach ($PacientesConOrdenes as $key=>$value)
		{
			...
			//ECHO "<BR><br>Deapchados-> ".$qDespachados;
			GLOBAL $ADODB_FETCH_MODE;
			list($dbconn) = GetDBconn();
			$ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
 ...
 //medicamentos del mod estacione_medicamentos
	...
	*/
	function GetMedicamentosPendientesPorRecibirPaciente($datos_estacion)
	{
		...
		//echo "<br><br>MedicamentosPedidosPorRecibirDel Paciente<br>".$qMedPac;
		GLOBAL $ADODB_FETCH_MODE;
		list($dbconn) = GetDBconn();
		$ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
		...
		else
		{
			...
			{
				//echo $result->fields[solicitud_id];
