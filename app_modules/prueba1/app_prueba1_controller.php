<?php
  ...
  **/
    ...
    class app_prueba1_controller extends classModulo
    {
        /**
	    ...
	    ************************************************************/
		...
		{	
			...
			$url[1]='prueba1';   			//Nombre del Modulo
        ...
        ***************************************************************/
		function MenuActasTecnicas()
		{
         ...
         $request = $_REQUEST;
  		 $Obj_Menu=AutoCarga::factory("ActasTecnicas_MenuHTML", "views", "app","prueba1");
  		
  		 //Volver a Empresas.
  		 $action['volver'] = ModuloGetURL("app","prueba1","controller","main")."&datos[empresa_id]=".$request['datos']['empresa_id']."";
       ...
       function CrearNotas()
       {
		  ...
		  IncludeFileModulo("RemotosNotasFacturas_Proveedor","RemoteXajax","app","prueba1");
          $this->SetXajax(array("TestingAjax"),null,"ISO-8859-1");
		  
		  $html_form=AutoCarga::factory("CrearNotasHTML", "views", "app","prueba1");
		  $sql=AutoCarga::factory("CrearNotasFacturasProveedores", "", "app","prueba1");
		  ...
		  $action['volver'] = ModuloGetURL("app","prueba1","controller","MenuActasTecnicas")."&datos[empresa_id]=".$_REQUEST['datos']['empresa_id']."";
		  $action['buscar'] = ModuloGetURL('app','prueba1','controller','CrearNotas');
		  //$action['crear_nota'] = ModuloGetURL('app','Inv_ActasTecnicas','controller','crear_nota_temporal');
		  $action['detalle_fac'] = ModuloGetURL('app','prueba1','controller','Det_factura_prov');
		  //$action['ver_notas'] = ModuloGetURL('app','Inv_ActasTecnicas','controller','DocumentoFinal');
		  $action['paginador'] = ModuloGetURL('app','prueba1','controller','CrearNotas',array("buscador"=>$request['buscador']))."&datos[empresa_id]=".$_REQUEST['datos']['empresa_id']."";
