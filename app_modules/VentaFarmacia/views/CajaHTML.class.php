...
class CajaHTML {
    ...
    function FormaPagoCheque($form, $confirman, $bancos) {
        ...
        $html .= "					</tr>\n";
        $html .= "					<tr $est class=\"modulo_table_list_title\">\n";
        $html .= "						<td width=\"19%\">N� CTA. CORRIENTE:</td>\n";
        $html .= "						<td width=\"30%\" class=\"modulo_list_claro\" >\n";
    ...
    function FormaPagoTarjetaDebito($form, $tarjetas) {
        ...
        $html .= "						</td>\n";
        $html .= "						<td>N� TARJETA</td>\n";
        $html .= "						<td class=\"modulo_list_claro\" width=\"27%\">\n";
        $html .= "							<input type=\"text\" class=\"input-text\" name=\"num_tarjeta\" style=\"width:90%\" maxlength=\"20\" value=\"" . $form['tarjeta_numero'] . "\">\n";
        $html .= "						</td>\n";
        $html .= "					</tr>\n";
        $html .= "					<tr $est class=\"modulo_table_list_title\">\n";
        $html .= "						<td >N� AUTORIZACI�N</td>\n";
        $html .= "						<td class=\"modulo_list_claro\" >\n";
    ...
    function FormaImprimirFactura($action, $factura) {
        ...
        $html .= "			FACTURA GENERADA SATISFACTORIAMENTE : " . $factura['prefijo'] . "-" . $factura['factura_fiscal'];
        $html .= "		</td>\n";
        $html .= "	</tr>\n";
        $html .= "</table>\n";
        //$html .= "<h2>".print_r($form, true)."</h2>";
        $host_1 = $_SERVER["HTTP_HOST"];
        $archivo = array_pop(explode('/', $_SERVER["SCRIPT_NAME"]));
        $host_2 =  str_replace('/'.$archivo, "", $_SERVER["SCRIPT_NAME"]);
        $host = $host_1.$host_2;

        $url_accion = "http://".$host."/classes/ReportesCsv/reportes.class.php?inicio=5&amp;tipo_modulo=app&amp;nombre_modulo=VentaFarmacia&amp;nombre_archivo=FacturaVenta&amp;parametros[prefijo]=".$factura['prefijo']."&amp;parametros[factura_fiscal]=".$factura['factura_fiscal']."&amp;parametros[empresa_id]=".$factura['empresa_id']."&amp;opciones[interface]=5&amp;SIIS_SID=".$_GET['SIIS_SID']."\" method=\"post\"";
        $html .= "<form name=\"volver\" action=\"".$url_accion."\" >";
        $html .= "  <table align=\"center\" width=\"50%\">\n";
        $html .= "    <tr>\n";
        $html .= "	    <td align=\"center\" width=\"50%\">\n";
        $html .= "		    <input type=\"submit\" class=\"input-submit\" name=\"imprimir\" value=\"Imprimir Factura\">\n";
        $html .= "		  </td>\n";
        $html .= "	    <td align=\"center\" width=\"50%\">\n";
        $html .= "          <a href='".$action['volver']."'>\n";
        $html .= "		        <input type=\"button\" class=\"input-submit\" name=\"cancela\" value=\"Volver\" >\n";
        $html .= "          </a>\n";
        $html .= "		  </td>\n";
        $html .= "	  </tr>\n";
        $html .= "  </table>\n";
    ...
    function FormaDatosReimpresion($action, $empresa, $centro, $bodega) {
        ...
        if($empresa == '17'){
            $pref = $cls->ObtenerPrefijos($empresa, $bodega);
        }else{
        ...
        if($empresa == '17'){
            $html .= "
                <select name=\"prefijo\" style=\"width:100px\">
                    <option value='".$pref['prefijo']."'>".$pref['prefijo']."</option>
                    <option value='".$pref['prefijo2']."'>".$pref['prefijo2']."</option>                    
                </select>
            ";
        }else{
