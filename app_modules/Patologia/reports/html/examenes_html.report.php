...
{
//*******************************************termino
	      $vector = $this->ReporteResultado($this->datos['resultado_id'],$this->datos['prefijo']);
	      $Salida.="<table  align=\"center\" border=\"0\"  width=\"100%\">";
				$Salida.="<tr>";
				$Salida.="  <td colspan=\"2\" class=\"normal_10N\" align=\"center\" width=\"100%\">RESULTADO DE PATOLOGIA ".$vector[cargo]."</td>";
				$Salida.="</tr>";
				$Salida.="<tr>";
        ...
        $Salida.="  <td class=\"normal_10\" align=\"left\" width=\"40%\">Servicio : ".$vector[servicio]."</td>";
				$Salida.="</tr>";
  	    $Salida.="</table>";
				$tejidos=$this->ReporteResultadoTejidos($this->datos['resultado_id'],$this->datos['prefijo']);
				...
				$Salida.="</BLOCKQUOTE>";
				$observaciones=$this->ReporteObservaiconesAdic($this->datos['resultado_id'],$this->datos['prefijo']);
				if($observaciones){
        $Salida.="<BR><table  align=\"center\" border=\"0\"  width=\"100%\">";
				$Salida.="<tr class=\"Normal_10N\">";
				$Salida.="  <td colspan=\"2\" align=\"left\" width=\"100%\">OBSERVACIONES ADICIONALES: </td>";
				$Salida.="</tr>";
				for($i=0;$i<sizeof($observaciones);$i++){
					...
					$Salida.="  <td width=\"30%\" nowrap><b>FECHA:&nbsp&nbsp;</b>".strftime("%b %d de %Y %H:%M",$FechaConver)."</td>";
					$Salida.="  <td nowrap><b>PROFESIONAL:&nbsp&nbsp;</b>".$observaciones[$i]['nombre']."</td>";
					$Salida.="</tr>";
          $Salida.="<tr class=\"Normal_10\">";
					$Salida.="  <td width=\"100%\" colspan=\"2\">".$observaciones[$i]['observaciones_adicionales']."</td>";
					$Salida.="</tr>";
					$Salida.="<tr class=\"Normal_10\">";
          $Salida.="  <td><BR></td>";
					$Salida.="</tr>";
				...
				$Salida.="</table>";
				//$vector[observaciones_adicionales]=str_replace("\x0a","<p></p>",$vector[observaciones_adicionales]);
				//$Salida.="<BLOCKQUOTE>";
				//$Salida.="<div align=\"justify\" width=\"100%\" class=\"Normal_10\">".$vector[observaciones_adicionales]."</div>";
				//$Salida.="</BLOCKQUOTE>";
				}
				/*if($tejidos){
				  $Salida.="<BR><table  width=\"100%\" border=\"1\" class=\"modulo_table_list\" align=\"center\">";
					$Salida.=" <td class=\"Normal_10N\" width=\"20%\" align=\"center\">FECHA RECEPCION</td>";
					$Salida.=" <td class=\"Normal_10N\" align=\"center\">TEJIDO</td>";
				  for($i=0;$i<sizeof($tejidos);$i++){
						$Salida.="<tr>";
						(list($ano,$mes,$dia)=explode('-',$tejidos[$i]['fecha']));
		        $FechaConver=mktime(0,0,0,$mes,$dia,$ano);
						$Salida.="  <td class=\"Normal_10\" align=\"left\" width=\"20%\">".strtoupper(strftime("%B %d DE %Y",$FechaConver))."</td>";
						$Salida.="  <td class=\"Normal_10\" align=\"left\">".$tejidos[$i]['nomtejido']."</td>";
						$Salida.="</tr>";
				  }
  	      $Salida.="</table>";
				}*/

				/*$procedimientos=$this->ReporteResultadoProcedimientos($this->datos['resultado_id'],$this->datos['prefijo']);
				if($procedimientos){
        ...
        $diagnosticos=$this->ReporteResultadoDiagnosticos($this->datos['resultado_id'],$this->datos['prefijo']);
				if($diagnosticos){
				  $Salida.="<BR><table  align=\"center\" border=\"1\"  class=\"modulo_table_list\" width=\"100%\">";
...
//*****************************************fin de termino
	...
	function ReporteResultado($resultado_id,$prefijo){
		...
		date(c.fecha_registro) as fecha,a.observaciones,e.nombre as nombreprof,b.responsable_solicitud,dpto.descripcion as departamento,
    ...
    esp.descripcion as especialidad,b.origen_solicitud,c.sexo_id
		FROM patologias_resultados_solicitudes a,patologias_solicitudes b
    ...
    LEFT JOIN  historias_clinicas hc ON (b.tipo_id_paciente=hc.tipo_id_paciente AND b.paciente_id=hc.paciente_id),
		...
		a.patologia_solicitud_id=b.patologia_solicitud_id AND
    ...
    list($dbconnect) = GetDBconn();
		...
		WHERE a.resultado_informe_id='$resultado_id' AND a.prefijo='$prefijo' AND
		a.diagnostico=b.diagnostico_id";
		$result = $dbconnect->Execute($query);
		if ($dbconnect->ErrorNo() != 0){
			$this->error = "Error al consultar los resultados de los examenes";
			$this->mensajeDeError = "Error DB : " . $dbconnect->ErrorMsg();
	...
	function ReporteResultadoTejidos($resultado_id,$prefijo){

    list($dbconnect) = GetDBconn();
		$query="SELECT b.descripcion as nomtejido,date(c.fecha_registro) as fecha
		FROM patologias_solicitudes_detalle_informes a,tipos_tejidos b,patologias_solicitudes_confirmadas c
		WHERE a.resultado_informe_id='$resultado_id' AND a.prefijo='$prefijo' AND
		a.tejido_id=b.tejido_id AND a.patologia_solicitud_id=c.patologia_solicitud_id AND a.tejido_id=c.tejido_id";
		$result = $dbconnect->Execute($query);
		if ($dbconnect->ErrorNo() != 0){
			$this->error = "Error al consultar los resultados de los examenes";
			$this->mensajeDeError = "Error DB : " . $dbconnect->ErrorMsg();
