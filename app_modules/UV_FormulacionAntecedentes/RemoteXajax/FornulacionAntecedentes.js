/**************************************************************************************
* $Id: definirMov.js,v 1.20 2007/06/27 19:34:31 jgomez Exp $ 
...
**************************************************************************************/
    ...
    function todoMay(evt,caja)
    {
        var keyCode = document.layers ? evt.which : document.all ? evt.keyCode : evt.keyCode;   
        //var tecla = event.keyCode;
        
        if (keyCode == 225 || keyCode == 233 || keyCode == 237 || keyCode == 241 || keyCode == 243 || keyCode == 250 || (keyCode >= 65 && keyCode <= 90))
        {
            eval(caja.value = caja.value.toUpperCase());
...
{   
    if(valor=='2')
    {
        LlamarCalendariofecha_finalizacion();
    }
    if(valor=='1')
    {
...
prueba del enter
*****************************************************************************/
function AsignarMedicamento(codigo,nombre)
{
    xajax_datos_medicamento(codigo,nombre);
    document.getElementById('errorMed').innerHTML='';
    document.getElementById('mensaje').innerHTML='';
}

function AsignarMedicamentoUp(tipo_id_paciente,paciente_id,codigo_medicamento,evolucion_id)
{
    xajax_datos_medicamentoUp(tipo_id_paciente,paciente_id,codigo_medicamento,evolucion_id);
    document.getElementById('errorMed').innerHTML='';
    document.getElementById('mensaje').innerHTML='';
}

function ReformularMedicamentoUp(tipo_id_paciente,paciente_id,codigo_medicamento,evolucion_id,swRef)
{
    xajax_datos_medicamentoUp(tipo_id_paciente,paciente_id,codigo_medicamento,evolucion_id,swRef);
    document.getElementById('errorRefMed').innerHTML='';
    document.getElementById('mensaje').innerHTML='';
}

function AsignarNuevoMedicamento()
{
    
    MostrarCapa('ContenedorMed');xajax_datos_medicamento();Iniciar('ASIGNAR MEDICAMENTO');
}
...
function Buscar_Med(tipo,producto,principio_act,ban,offset)
{
    document.getElementById('mensaje').innerHTML='';
    xajax_buscar_medicamento(tipo,producto,principio_act,ban,offset);
}


function SeleccionarCicloFamiliar(td_id,ingreso,tip_id,paciente_id,cvf,descripcion)
{
    
    xajax_Prueba(td_id,ingreso,tip_id,paciente_id,cvf,descripcion);
}

function GuardarObsCvf(ingreso,tipo_id_paciente,paciente_id,obs_cvf)
{
    xajax_GuardarObsCvf(ingreso,tipo_id_paciente,paciente_id,obs_cvf);
}
                        
function GuardarFR(ingreso,tip_pac,id_pac,cvi,fr)
{
  xajax_GuardarFR(ingreso,tip_pac,id_pac,cvi,fr);
}
...
function Aplicar(busqueda)
{
    ...
    if(busqueda==1)
    {
      cad ="DESCRIPCION <input type=\"text\" class=\"input-text\" name=\"cuentaups\" size=\"30\" onkeypress=\"return acceptNum(event);\" onkeydown=\"recogerTeclas(event)\" value=\"\">\n";//
    }
    else
    {
      cad ="DESCRIPCION <input type=\"text\" class=\"input-text\" name=\"cuentaups\" size=\"30\" onkeypress=\"return acceptm(event);\" onkeydown=\"recogerTeclas(event)\" value=\"\">\n";//
    }
