	...
	/*
	   ...
	   */
		function Empresas()
		{
			...
			//$action['buscarDoc'] = ModuloGetURL("app", "Compras_Orden_Compras", "controller", "BuscarDocumentosCreados");
		...
		//$datos=$mdl->ConsultarDetallePreOrden($preorden_id);
		...
		$tiposdoc = $mdl->ConsultarTipoId();
			if(!empty($request['buscador']))
			{
				$datos=$mdl->ConsultarOrdenComprasGeneradas($request['buscador'],$request['offset']);
				$action['buscador']=ModuloGetURL('app','Compras_Orden_Compras','controller','ConsultarOrdenes');
				$action['paginador'] = ModuloGetURL("app", "Compras_Orden_Compras", "controller", "ConsultarOrdenes",array("buscador"=>$request['buscador']));
				$conteo= $mdl->conteo;
				$pagina= $mdl-> pagina;
			}
     		$action['volver'] = ModuloGetURL("app", "Compras_Orden_Compras", "controller", "BuscarPreOrdenp");
		...
		{
			$request = $_REQUEST;
			SessionSetVar("empresa_i",$request['empresa']);
		...
		function CrearDocumentoYUnificar()
		{
			...
			$inOp=$mdl->insertarOrden_Pedido($proveedor,$empresa);
    		...
    		//$actu=$mdl->ActualizarSw_unificadaOp($empresa,$proveedor,$numero);
