...
/**
 * $Id: ventanaClasificacion.php,v 1.2 2005/06/02 19:33:44 leydi Exp $
  ...
  <?php
	    $VISTA='HTML';
		  ...
		  $result3 = $dbconn->Execute($query);
			...
			$query="SELECT inv_mostrar_serial('$grupo','$clasePr','$subclase')";
  ...
  ?>
      ...
      <tr class="modulo_list_claro">
        ...
        <?php
			...
			<tr class="modulo_list_claro">
        <td width=30% nowrap class="label">Clase:</td>
        <td colspan=2>
      ...
      <tr class="modulo_list_claro">
        <td width=30% nowrap class="label">SubClase:</td>
        <td colspan=2>
        <select name=subclase  class="select" onChange="cambioSubClase(this.form,<?php echo $Empresa?>)">
        <?php
				  if(!empty($subclase)){
					  while(!$result3->EOF){
              $SubClase=$result3->fields[0];
				...
				<input type='hidden' name='serialNum' value= "<? echo $result4->fields[0] ?>">
	    </tr>
      <?php
			}
			?>
			...
			<tr>
			  <td align=center colspan=3>
				<input type='hidden' name='bandera' value='<?echo $bandera?>'>
				<?php
				if($bandera==1){
        ?>
