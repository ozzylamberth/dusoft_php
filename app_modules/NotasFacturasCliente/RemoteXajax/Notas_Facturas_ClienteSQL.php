 ...
 */
/* ***********************************************
 * funcion para guardar modelos contables
 * ************************************************/

function GuardarModeloContable($codigo, $descripcion)
{
    $sql = Autocarga::factory("NotasFacturasClienteSQL", "", "app", "NotasFacturasCliente");

    $sql->GuardarInterfazModeloContable($codigo, $descripcion);
}


function ObtenerUltimoModeloContable($codigo, $descripcion)
{
    $sql = Autocarga::factory("NotasFacturasClienteSQL", "", "app", "NotasFacturasCliente");

    $id = $sql->ObtenerIdUltimaInterfazModeloContable();
    
    $id = $id[0]['id'];
    
    $objResponse = new xajaxResponse();

    $script = "      var lista = document.getElementsByClassName('modelo_contable');\n";
    $script .= "      var cantidadLista = lista.length;\n";
    $script .= "      for (i=0;i<cantidadLista;i++)\n";
    $script .= "      {\n";
    $script .= "        var option = document.createElement('option');\n";
    $script .= "        option.text = '".$codigo."' + ' - ' + '".$descripcion."';\n";
    $script .= "        option.value = '".$id."';\n";
    $script .= "        lista[i].add(option,null);\n";
    $script .= "      }\n";
...
function GuardarTipoDocumentoContable($codigo, $descripcion)
{
    $sql = Autocarga::factory("NotasFacturasClienteSQL", "", "app", "NotasFacturasCliente");

    $sql->GuardarInterfazTipoDocumentoContable($codigo, $descripcion);
}


function ObtenerUltimoTipoDocumentoContable($codigo, $descripcion)
{
    $sql = Autocarga::factory("NotasFacturasClienteSQL", "", "app", "NotasFacturasCliente");

    $id = $sql->ObtenerIdUltimaInterfazTipoDocumentoContable();
    
    $id = $id[0]['id'];
    
    $objResponse = new xajaxResponse();

    $script = "      var lista = document.getElementsByClassName('tipo_documento_contable');\n";
    $script .= "      var cantidadLista = lista.length;\n";
    $script .= "      for (i=0;i<cantidadLista;i++)\n";
    $script .= "      {\n";
    $script .= "        var option = document.createElement('option');\n";
...
function sincronizar_notas_pendientes_ws_fi($empresa_id, $prefijo, $numero_factura, $nota_id, $tiponota){
     $objResponse = new xajaxResponse();
      $dusoft_fi = AutoCarga::factory("SincronizacionDusoftFI", "SincronizacionDusoftFI", "", "");
      $sql = AutoCarga::factory("NotasFacturasClienteSQL", "", "app", "NotasFacturasCliente");
      
      if($tiponota == "0"){
          $concepto_nota = $sql->obtenerConceptoPorNota($nota_id);
          $resultado_sincronizacion_ws = $dusoft_fi->notas_credito_clientes_fi($nota_id, $concepto_nota);
           $url = ModuloGetURL("app", "NotasFacturasCliente", "controller", "BuscarNotaCredito", array("factura" =>$numero_factura));
      } else {
          $resultado_sincronizacion_ws = $dusoft_fi->notas_debito_cliente_fi($nota_id);
           $url = ModuloGetURL("app", "NotasFacturasCliente", "controller", "BuscarNotaDebito", array("factura" =>$numero_factura));
      }
...
function mostrarFormularioConceptoNota($factura, $tiponota, $concepto){
    $objResponse = new xajaxResponse();
    $url = ModuloGetURL("app", "NotasFacturasCliente", "controller", "CrearNota", array("factura" =>$factura , "modoNota" => $tiponota, "concepto" =>$concepto));
     $objResponse->script("window.location='{$url}';");
