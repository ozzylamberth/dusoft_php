							...
							**/
							...
							//  echo "A QUI SE DEBE REALIZAR EL CAMBIO DE LA FECHA PARA QUE LAS ENTREGAS SE CORRAN " .$producto_refrendado['fecha_refrendacion'];
							  ...
							  //$valor['fecha_formulacion'] = '2016-01-10'; 
								...
								**/
								$fecha_base=$sql->operacion_meses($fecha_siguiente_entregas,$cantidadEntregaFormula['numero_entregas'],"+");
...
>>>>>>> dfc71fd332db4aff98d5bc5e9b50dbbe070fd5b2
			 ...
			 if($numero_de_entregas_de_producto>$cantidadEntregaFormula['numero_entregas'] ){
					...
					{
						if($dato['entrar'] === 'entrar'  || !empty($refrendacionFormula)){//and $today <= $dtl['fecha_finalizacion']
							//if ($fecha_condiass >= $todays || !empty($refrendacionFormula)) { // || !empty($medicamentos_dispensados)  //refrendacionFormula
							
							 if ($numero_de_entregas_de_producto > $cantidad_dispensacion and !empty($refrendacionFormula)) {
						...
						if($dato['entrar'] === 'falta' and $cantidadEntregaFormula['numero_entregas']>1){
							$html .= "      <td  width=\"50%\" align=\"center\" class=\"label_error\">\n";
						...
						if($dato['entrar'] === 'falta'  and $today <= $dtl['fecha_finalizacion'] and !empty($refrendacionFormula)){
							//if ($fecha_condiass >= $todays || !empty($refrendacionFormula)) { // || !empty($medicamentos_dispensados)  //refrendacionFormula
						...
						**/
						if($dato['entrar'] === 'vencido') {  
							if(empty($refrendacionFormula) || !empty($refrendacionFormula))
								$html .= "      <td  width=\"50%\" align=\"center\" class=\"label_error\">\n";
					 ...
					 **/
					/*if ($numeroSiguienteEntrega ==0 and $dato['entrar'] === 'entrar'  and $today <= $dtl['fecha_finalizacion']|| !empty($refrendacionFormula)){
							//if ($fecha_condiass >= $todays || !empty($refrendacionFormula)) { // || !empty($medicamentos_dispensados)  //refrendacionFormula
...
<<<<<<< HEAD
                  /*  if ($numeroSiguienteEntrega >=1 and $dato['entrar'] === 'entrar' and empty($refrendacionFormula) and $numeroSiguienteEntrega < $numero_de_entregas_de_producto|| !empty($refrendacionFormula)){
=======
                    if ($numeroSiguienteEntrega >=1 and $dato['entrar'] === 'entrar' and empty($refrendacionFormula)|| !empty($refrendacionFormula)){
>>>>>>> dfc71fd332db4aff98d5bc5e9b50dbbe070fd5b2
                    //if ($fecha_condiass >= $todays || !empty($refrendacionFormula)) { // || !empty($medicamentos_dispensados)  //refrendacionFormula
					...
					/**
					 *+Descripcion Si aun falta dispensacion por entregar, y el estado es entrar pero la fecha de finalizacion de la formula es mayor a la fecha actual
					 *				Quiere decir que la formula ha vencido y se debe refrendar
					 **/
					/*if ($numeroSiguienteEntrega >=1 and $dato['entrar'] === 'entrar'  and $today > $dtl['fecha_finalizacion'] and empty($refrendacionFormula)){
                    
							$html .= "      <td  width=\"50%\" align=\"center\" class=\"label_error\">\n";
                            $html .= "          <img border=\"0\"  title=\"El paciente ya finalizo el tratamiento con este producto\" src=\"" . GetThemePath() . "/images/alarma.gif\">\n";
                            $html .= "      </td>\n";
                    }*/
					
					/**
					 *+Descripcion Si aun falta  dispensacion por entregar, y el estado es 'falta' y no ha sido refrendada
					 *				Quiere decir que la formula ha vencido y se debe refrendar
					 **/
					
					/* if ($numeroSiguienteEntrega >=1 and $dato['entrar'] === 'falta' and empty($refrendacionFormula) === '1' and $cantidadEntregaFormula['numero_entregas'] >0){
					...
					/** +Descripcion Se valida que el numero de entrega sera el primero de la formula  
					*	y se deja proseguir
					**/
					/*if ($numeroSiguienteEntrega >=1 and $dato['entrar'] === 'falta'){
					
					if($cantidadEntregaFormula['numero_entregas']>1){
						$html .= "      <td  width=\"50%\" align=\"center\" class=\"label_error\">\n";
                        $html .= "          <img border=\"0\"  title=\"DEBE ESPERAR A LA PROXIMA ENTREGA\" src=\"" . GetThemePath() . "/images/alarma.gif\">\n";
                        $html .= "      </td>\n";
					}/*else{
					...
					/**
					*+Descripcion Si se vencio mas de una dispensacion de la formula y tiene una refrendacion
					*			  La formula estara activa
					**/
					
				/*	if ($numeroSiguienteEntrega >=1 and $dato['entrar'] === 'falta'  and $today <= $dtl['fecha_finalizacion'] and !empty($refrendacionFormula)){
                    //if ($fecha_condiass >= $todays || !empty($refrendacionFormula)) { // || !empty($medicamentos_dispensados)  //refrendacionFormula
					...
					if ($numeroSiguienteEntrega == 0){
						...
						$html .= "      </td>\n";
						  ...
						  *+Descripcion: SI la dispensacion es el mismo mes de la anterior
						 **/
						if($dato['entrar'] === 'entrar'  and $today <= $dtl['fecha_finalizacion']|| !empty($refrendacionFormula)){
					...
					* --Fecha de reclamacion --      2016-02-10
					**/
					/*if ($numeroSiguienteEntrega >=1 and $dato['entrar'] === 'vencido') {  
						if(empty($refrendacionFormula) || !empty($refrendacionFormula))
                            $html .= "      <td  width=\"50%\" align=\"center\" class=\"label_error\">\n";
                            $html .= "          <img border=\"0\"  title=\"FORMULA VENCIDA 2\" src=\"" . GetThemePath() . "/images/alarma.gif\">\n";
                            $html .= "      </td>\n";
