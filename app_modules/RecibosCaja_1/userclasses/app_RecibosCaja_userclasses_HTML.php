...
class app_RecibosCaja_userclasses_HTML extends app_RecibosCaja_user {
    ...
    function FormaGenerarReciboCaja()
    {
        ...
        if (sizeof($rc_pendientes) > 0)
        {
            ...
            $this->salida .= "							<td width=\"\" colspan= \"10\">OPCIONES</td>\n";
            $this->salida .= "						</tr>\n";
            ...
            for ($i = 0; $i < sizeof($rc_pendientes); $i++)
            {
                ...
                $this->salida .= "							<td width=\"6%\" align=\"center\">$opcion5</td>\n";
                $this->salida .= "							<td width=\"6%\" align=\"center\">$opcion0</td>\n";
                $this->salida .= "							<td width=\"6%\" align=\"center\">$opcion2</td>\n";
                $this->salida .= "							<td width=\"6%\" align=\"center\">$opcion6</td>\n";
                $this->salida .= "							<td width=\"6%\" align=\"center\">$opcion3</td>\n";
                $this->salida .= "							<td width=\"6%\" align=\"center\">$opcion4</td>\n";
                $this->salida .= "							<td width=\"6%\" align=\"center\">$opcion1</td>\n";
                $this->salida .= "							<td width=\"6%\" align=\"center\">$opcion7</td>\n";
                $this->salida .= "							<td width=\"6%\" align=\"center\">$opcion9</td>\n";
                $this->salida .= "							<td width=\"6%\" align=\"center\">$opcion8</td>\n";
                $this->salida .= "						</tr>\n";
            }
            $this->salida .= "					</table>\n";
            $this->salida .= "				</fieldset>\n";
    ...
    function FormaMostrarFacturas()
    {
        ...
        /* $this->salida .= "  function sumatoriaExcedentes(nombres_rc,nombre_rc_general,valor_rc,nombre_valor,guia)
          {
          var rc_general = document.getElementById(nombre_rc_general),
          rc_todos = document.getElementsByName(nombres_rc),
          valor = document.getElementsByName(nombre_valor),
          rc_tot = document.getElementsByName('rc_general'),
          rc_hidden = document.getElementsByName('rc_exceden_h_'+guia),
          sum = 0,
          exce = 0;

          for(var i=0, rc; rc=rc_todos[i];i++)
          {
          if(rc.value == '')
          {
          //alert('is nan: '+rc.value);
          rc.value = 0;
          }
          sum += parseInt(rc.value);
          }
          //alert(sum+'<='+valor_rc);
          if (parseInt(sum) <= parseInt(valor_rc))
          {
          rc_general.value=parseInt(valor_rc)-parseInt(sum);
          }
          else
          {
          for(var j=0;j<rc_todos.length;j++)
          {
          //alert(rc_todos[j].value + '>' + rc_general.value + '? y... es >'+rc_general.value);
          if (parseInt(rc_todos[j].value)>parseInt(valor_rc) && parseInt(rc_todos[j].value) > parseInt(rc_general.value))
          {
          rc_todos[j].value = 0;
          valor[0].value = 0;
          //MENSAJE: EL VALOR INGRESADO SOBREPASA EL VALOR DEL RECIBO DE CAJA
          /*var mi_sum = 0;
          for(var h=0;h<rc_todos.length;h++)
          {
          mi_sum += parseInt(rc_todos[h].value);
          }
          //alert('mi_sum'+mi_sum+'------- gen:'+rc_hidden[0].value);
          if((parseInt(rc_hidden[0].value)-parseInt(mi_sum)) < 0)
          {
          rc_general.value=0;
          valor[0].value = 0;
          rc_todos[j].value = 0;
          ...
          {
          rc_general.value = parseInt(rc_hidden[0].value)-parseInt(mi_sum);
          } */
        /* }
          }
          //alert('sum2: '+valor_rc);
          }
          //alert('nombre_valor: '+nombre_valor);
          for(var s=0;s<rc_tot.length;s++)
          {
          exce += parseInt(rc_tot[s].value);
          }
          rc_total_excedente.value = exce;
          }"; */
