<?php
  ...
  * @param integer $offset Identificador de la página actual
  *
  * @return object
  */
    ...
    if(!empty($datos))
    {
	  ...
	  foreach($datos as $key => $dtl)
	  {
		...
		if(strlen($dtl['descripcion']) > strlen($dtl['descripcion_abreviada']))
		{
		  ...
		  $html .= $dtl['descripcion_abreviada']."\n";
		}
  ...
  */
  function BuscarTercero($form)
  {
    ...
    if($mensaje == "")
    {
      ...
      $objResponse->assign("telefono","value",$pct['telefono']);
      $objResponse->assign("direccion","value",$pct['direccion']);
      $objResponse->assign("celular","value",$pct['celular']);
      if($pct['tipo_pais'] != "")
      {
  ...
  */
  function RealizarPago($form,$opcion)
  {
    ...
    switch($opcion)
    {
      case "E": $html .= $cnt->FormaPagoEfectivo($form);	break;
      case "C": 
        ...
        $html .= $cnt->FormaPagoCheque($informacion,$confirman,$bancos);	break;
      case "R": 
        $confirman = $mdl->ObtenerEntidadesConfirma();
      ...
      break;
      case "B": $html .= $cnt->FormaPagoBonos($form);	break;
    }
