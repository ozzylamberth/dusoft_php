...
/**
 * $Id: app_InvBodegas_user.php,v 1.107 2007/06/14 18:44:15 tizziano Exp $
...
*/
    ...
    function HallarQueryBusqueda($grupo,$clasePr,$subclase,$codigoPro,$descripcionPro,$filtroEstado=0){
    ...
    function ConsultaExistenciasBodegas($grupo,$clasePr,$subclase,$codigoPro,$descripcionPro,$filtroEstado=0){

        list($dbconn) = GetDBconn();
        $query1=$this->HallarQueryBusqueda($grupo,$clasePr,$subclase,$codigoPro,$descripcionPro,$filtroEstado=0);
    ...
    if($_REQUEST['Salir']){
      $this->MenuInventarios4();
        ...
        $this->FormaExistenciasBodegas($_REQUEST['codigoProd'],$_REQUEST['descripcion'],$_REQUEST['grupo'],$_REQUEST['NomGrupo'],$_REQUEST['clasePr'],$_REQUEST['NomClase'],$_REQUEST['subclase'],$_REQUEST['NomSubClase']);
        return true;
    }
       ...
       $result = $dbconn->Execute($query);
        ...
        for($i=0;$i<sizeof($vars);$i++){
            ...
            if($codigoAgrupamiento!='0'){
                $query="SELECT codigo_producto,departamento,precio,fecha_cargo,plan_id,servicio_cargo,sum(cantidad) as cantidad
                FROM tmp_cuenta_insumos
                WHERE numerodecuenta='$cuenta' AND empresa_id='$Empresa' AND centro_utilidad='$CentroUtili' AND bodega='$BodegaId'
                GROUP BY codigo_producto,departamento,precio,fecha_cargo,plan_id,servicio_cargo";
                $result = $dbconn->Execute($query);
                if($dbconn->ErrorNo() !=0 ){
                    $this->error = "Error al Guardar en la Base de Datos";
                    $this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
                ...
                $_SESSION['INVENTARIOS']['RETORNO']['Mensaje_Error']='Error en la Creacion de Los Documentos de Bodega, Consulte al Administrador de la Bodega';
            ...
            $query="SELECT A.existencia, A.sw_control_fecha_vencimiento, B.sw_restriccion_stock 
                    ...
                    AND A.bodega = B.bodega";
                        ...
                        if($NonExistencias == "0")
                        {
                              if(($Existencias-$Cantidad)<0){
                                   $_SESSION['INVENTARIOS']['RETORNO']['Bodega']=false;
                                   $_SESSION['INVENTARIOS']['RETORNO']['Mensaje_Error']='Imposible Realizar la Transaccion, La bodega no Cuenta con las Existencias Solicitadas Disponibles';
...
*/
    ...
    $query = "(SELECT   SMD.solicitud_id,
              ...
              inventarios INV,inventarios_productos INVP,
                            ...
                            SMD.codigo_producto = INVP.codigo_producto AND
...
*/
    ...
    function ConfirmacionDespachoDetalleSolicitud($SolicitudId,$TipoSolicitud){
      ...
      list($dbconn) = GetDBconn();
    if($TipoSolicitud=='I'){
      $query="SELECT a.codigo_producto,a.cantidad,b.descripcion,a.consecutivo_d
        ...
        if($dbconn->ErrorNo() != 0){
          ...
          if($_REQUEST['pendiente']){
            ...
            $query="INSERT INTO hc_solicitudes_insumos_d(
                        codigo_producto,cantidad,solicitud_id)VALUES(
          ...
          (list($canti,$evolucion,$ing)=explode('||//',$cantidad));
            ...
            $query="INSERT INTO hc_solicitudes_insumos_d(
                        consecutivo_d,codigo_producto,cantidad,solicitud_id)VALUES(
  ...
  function LlamaEditarDespachoPendientes(){
    ...
    $query="SELECT bodegas_doc_id FROM bodegas_doc_numeraciones WHERE sw_transaccion_medicamentos='1' AND sw_estado='1' AND tipo_movimiento='E'
      ...
      if($TipoSolicitud!='I'){
        ...
        $query="SELECT a.consecutivo_d,a.codigo_producto as producto,a.cantidad as cantidad
...
*/
    ...
    list($dbconn) = GetDBconn();
      ...
      $query="SELECT a.codigo_producto as mto,a.cantidad as cant,b.descripcion,NULL as mezcla FROM hc_solicitudes_insumos_d a,inventarios_productos b WHERE a.solicitud_id='$SolicitudId' AND a.codigo_producto=b.codigo_producto";
            }
  ...
  function LlamaRecibirOrdenesCompra(){
    ...
    LEFT JOIN movimientos_habitacion f ON(e.numerodecuenta=f.numerodecuenta AND f.fecha_egreso is NULL)
        ...
        l.primer_nombre||' '||l.segundo_nombre||' '||l.primer_apellido||' '||l.segundo_apellido as nombrepac,
        j.cama,j.pieza,a.tipo_solicitud,b.descripcion as nomestacion,
        det.codigo_producto,invp.descripcion_abreviada as desmed,det.cantidad as cant_solicitada,bu.descripcion as ubicacion,u.abreviatura,exis.existencia
        FROM hc_solicitudes_medicamentos a,estaciones_enfermeria b,departamentos c,system_usuarios d,cuentas e
    LEFT JOIN movimientos_habitacion f ON(e.numerodecuenta=f.numerodecuenta AND f.fecha_egreso is NULL)
        LEFT JOIN camas j ON(f.cama=j.cama)
...
*/
  ...
  function liquidacionIyMResponsable(){
    ...
    $codigoAgrupamiento=$this->InsertarBodegasDocumentos($concepto,$numeracion,date("Y/m/d"),'','IMD');
    if($codigoAgrupamiento=='0'){
      $_SESSION['ESTACION_ENFERMERIA_DESPACHOS_USUARIOS_INVENTARIOS']['Mensaje_Error']='Error en la Creacion de Los Documentos de Bodega, Consulte al Administrador de la Bodega';
      $this->GuardarNumeroDocumento($commit=false);
      return false;
        }else{
      $i=1;
  ...
  function liquidacionIyMCirugia(){
    ...
    if($codigoAgrupamiento!='0'){
      ...
      FROM cuentas_liquidaciones_qx a,estacion_enfermeria_qx_iym b
      WHERE a.cuenta_liquidacion_qx_id='".$_SESSION['LIQUIDACION_QX']['NoLIQUIDACION']."' AND a.programacion_id=b.programacion_id
      GROUP BY b.codigo_producto) x
      LEFT JOIN (SELECT e.codigo_producto,sum(e.cantidad) as devolucion
      FROM cuentas_liquidaciones_qx d,estacion_enfermeria_qx_iym_devoluciones e
      WHERE d.cuenta_liquidacion_qx_id='".$_SESSION['LIQUIDACION_QX']['NoLIQUIDACION']."' AND d.programacion_id=e.programacion_id
      ...
      $_SESSION['LIQUIDACION_QX']['RETORNO']['Mensaje_Error']='Error en la Creacion de Los Documentos de Bodega, Consulte al Administrador de la Bodega';
      $this->GuardarNumeroDocumento($commit=false);
      return false;
    }
  ...
  function liquidacionIyMCirugiaGases(){
    ...
    $ProgramacionId=$vars[0]['programacion_id'];
    for($j=0;$j<sizeof($vars);$j++){
      if(array_key_exists($vars[$j]['suministro_gas_id'],$VectorDatos)){
...
*/
  ...
  function liquidacionIyMCargosCuenta(){
    ...
    foreach($_SESSION['IYM_CUENTAS_QX']['PRODUCTOS_IYM'] as $codigoProducto=>$vector){
      foreach($vector as $descripcion=>$existencias){
        if(!empty($_SESSION['IYM_CUENTAS_QX']['PRODUCTOS_IYM_CANTIDADES_DES'][$codigoProducto])){

          $Cantidad=$_SESSION['IYM_CUENTAS_QX']['PRODUCTOS_IYM_CANTIDADES_DES'][$codigoProducto];
          $departamento=$_SESSION['LIQUIDACION_QX']['Departamento'];
          $FechaCargo=date("Y-m-d");
          $costoProducto=$this->HallarCostoProducto($Empresa,$codigoProducto);
          $query="SELECT nextval('bodegas_documentos_d_consecutivo_seq')";
...
*/
    ...
    $departamento=$_SESSION['LIQUIDACION_QX']['Departamento'];
        foreach($_SESSION['IYM_CUENTAS_QX_DEVOL']['PRODUCTOS_IYM_CANTIDADES_DEV'] as $CodigoPro=>$Cantidad){
            ...
            $InsertarDocumentod=$this->InsertarBodegasDocumentosd($Consecutivo,$numeracion,$concepto,$CodigoPro,$Cantidad,$costoProducto);
            if($InsertarDocumentod==1){
              $this->InsertarBodegasDocumentosdCober($Consecutivo,date('Y-m-d H:i:s'),$numeroDeCuenta,$CodigoPro,$Cantidad,$varsPr[$j]['precio'],$codigoAgrupamiento,$PlanId,$Servicio,$Empresa,$CentroUtili,$departamento,'1','DIMD');
                ...
                $result = $dbconn->Execute($query);
                $Existencias=$result->fields[0];
                $ModifExist=$this->ModificacionExistenciasResta($Existencias,$Cantidad,$Empresa,$CentroUtili,$BodegaId,$CodigoPro);
...
*/
    ...
    IncludeLib("despacho_medicamentos");
        ...
        WHERE a.tipo_id_tercero='".$_SESSION['CAJA']['TIPO_ID_TERCERO']."' AND a.tercero_id='".$_SESSION['CAJA']['TERCEROID']."'";
        GLOBAL $ADODB_FETCH_MODE;
        $ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
    $result = $dbconn->Execute($query);
    $ADODB_FETCH_MODE = ADODB_FETCH_NUM;
...
*/
    function DevolucionIyMCargosCuenta(){
      ...
      return false;
        ...
        $codigoAgrupamiento=$this->InsertarBodegasDocumentos($concepto,$numeracion,date("Y/m/d"),'','DIMD');
    if($codigoAgrupamiento=='0'){
      $_SESSION['FACTURACION_CUENTAS']['RETORNO']['Mensaje_Error']='Error en la Creacion de Los Documentos de Bodega, Consulte al Administrador de la Bodega';
      $this->GuardarNumeroDocumento($commit=false);
      return false;
    }
    ...
    $cuenta=$_SESSION['OS_ATENCION']['CUENTA'];
      ...
      $Empresa=$vars[$i]['empresa_id'];
        ...
        $_SESSION['OS_ATENCION']['RETORNO']['Mensaje_Error']='Error en la Creacion de Los Documentos de Bodega, Consulte al Administrador de la Bodega';
