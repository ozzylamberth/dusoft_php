...
class app_Formulacion_Externa_controller extends classModulo {
    ...
    function main() {
        $request = $_REQUEST;
        $datos = AutoCarga::factory('Formulacion_ExternaSQL', '', 'app', 'Formulacion_Externa');
    ...
    function Menu_Formulacion() {
        $request = $_REQUEST;
        if ($request['Formulacion_Externa'])
            SessionSetVar("DatosEmpresaAF", $request['Formulacion_Externa']);
        ...
        $msj = $obje->ReservasProductos($empresa);
        ...
        $action['tickets'] = ModuloGetURL("app", "Formulacion_Externa", "controller", "Tickets");
        $action['reservas_pacientes'] = ModuloGetURL("app", "Formulacion_Externa", "controller", "Reservas_Pacientes");
        ...
        $this->salida = $act->FormaMenu($action, $datos_empresa);
        return true;
    }
    ...
    function Formula_Digitalizar_Inicio() {
        ...
        $datos_empresa = SessionGetVar("DatosEmpresaAF");
        IncludeFileModulo("Formulacion_E", "RemoteXajax", "app", "Formulacion_Externa");
        $this->SetXajax(array("ValidarFechas", "Eliminar_dx", "IngresarDX", "MostrarDX", "BuscadorMunicipios", "BuscadorMunicipio_d"), "app_modules/Formulacion_Externa/RemoteXajax/Formulacion_E.php", "ISO-8859-1");
        $this->IncludeJS("CrossBrowser");
        $this->IncludeJS("CrossBrowserEvent");
        $this->IncludeJS("CrossBrowserDrag");
        $obje = AutoCarga::factory('Formulacion_ExternaSQL', '', 'app', 'Formulacion_Externa');
        ...
        $Tipo_Formula = $obje->Consultar_Tipos_Formulas($datos_empresa);
    ...
    function FormaDigitalizar_Ambulatoria() {
        ...
        IncludeFileModulo("Formulacion_E", "RemoteXajax", "app", "Formulacion_Externa");
        $this->SetXajax(array("ocultar_producto_pendiente", "Marcar_Producto", "Autorizacion_despacho", "Cambiarvetana", "Eliminar_codigo_prodcto_d", "MostrarProductox", "Cancelar_Proceso", "GuardarPT", "BuscarProducto1", "Guardartmp_ambu", "IngresarDX", "MostrarDX", "GuardaEntregaEnFarmacia_tmp"), "app_modules/Formulacion_Externa/RemoteXajax/Formulacion_E.php", "ISO-8859-1");
        $this->IncludeJS("CrossBrowser");
        $this->IncludeJS("CrossBrowserEvent");
        $this->IncludeJS("CrossBrowserDrag");
        ...
        $Cabecera_Formulacion_ = $obje->consultar_Formulacion_TITMP($request, $empresa, $formula_id, $datos_empresa);
    ...
    function Tickets_Dispensacion() {
        ...
        //added: 31072012
        //$action['entregas_farmacia'] = ModuloGetURL("app", "DispensacionESM", "controller", "Pendientes_Entrega_Farmacia");
        $action['entregas_farmacia'] = ModuloGetURL("app", "Formulacion_Externa", "controller", "Pendientes_Entrega_Farmacia");
        //
    ...
    function Reservas_Pacientes() {
        $request = $_REQUEST;
        $datos_empresa = SessionGetVar("DatosEmpresaAF");
        $sql = AutoCarga::factory('Formulacion_ExternaSQL', '', 'app', 'Formulacion_Externa');
        $html = AutoCarga::factory("Formulacion_ExternaHTML", "views", "app", "Formulacion_Externa");

        if ($request['inactivar'] == '1')
            $sql->Inactivar_Pendientes($request);

        $pendientes = $sql->ObtenerPendientes_Pacientes($request['buscador'], $datos_empresa, $request['offset']);
        /* print_r($pendientes); */
        $action['volver'] = ModuloGetURL("app", "Formulacion_Externa", "controller", "Menu_Formulacion");
        $action['inactivar'] = ModuloGetURL("app", "Formulacion_Externa", "controller", "Reservas_Pacientes", array("buscador" => $request['buscador']));
        $action['buscar'] = ModuloGetURL("app", "Formulacion_Externa", "controller", "Reservas_Pacientes");

        $action['paginador'] = ModuloGetURL('app', 'Formulacion_Externa', 'controller', 'Reservas_Pacientes', array("buscador" => $request['buscador']));
        $this->salida = $html->Forma_PedientesReservas($action, $pendientes, $request, $sql->conteo, $sql->pagina);
        return true;
    }

    /*     * CONTROLLER PENDIENTES ENTREGA EN FARMACIA* */

    function Pendientes_Entrega_Farmacia() {
        $request = $_REQUEST;
        $empresa = SessionGetVar("DatosEmpresaAF");

        $formula_id = $request['formula_id'];
        $f_papel = $request['formula_papel'];
        $tipo_id_paciente = $request['tipo_id_paciente'];
        $paciente_id = $request['paciente_id'];
        $plan_id = $request['plan_id'];
        //print_r($request);
        ...
        $cls = AutoCarga::factory("Tickets_DispensacionSQL", "classes", "app", "Formulacion_Externa");
        $cls2 = AutoCarga::factory("Formulacion_ExternaSQL", "classes", "app", "Formulacion_Externa");

        $opcion = '0';

        $Cabecera_Formulacion = $cls2->Consulta_Formulacion_Real_I($formula_id);
        $Datos_Ad = $cls2->Dato_Adionales_afiliacion($request);
        $dix_r = $cls2->Diagnostico_Real($formula_id);
        $medi_form = $cls2->Medicamentos_Pendientes_Esm($formula_id);
        ...
        $action['guardarllamada'] = ModuloGetURL("app", "Formulacion_Externa", "controller", "GuardarLlamada");
        $action['iraPendientes'] = ModuloGetURL("app", "Formulacion_Externa", "controller", "Tickets");

        $view = AutoCarga::factory("Formulacion_ExternaHTML", "views", "app", "Formulacion_Externa");

        $this->salida .= $view->FormaPendientesEntregaFarmacia($action, $formula_id, $Cabecera_Formulacion, $f_papel, $request, $opcion, $medi_form, $Datos_Ad, $dix_r);
    ...
    function Tickets() {
        ...
        $action['volver'] = ModuloGetURL("app", "Formulacion_Externa", "controller", "main");
        $action['buscar'] = ModuloGetURL('app', 'Formulacion_Externa', 'controller', 'Tickets');
        $action['paginador'] = ModuloGetURL('app', 'Formulacion_Externa', 'controller', 'Tickets', array("buscador" => $request['buscador']));
        $action['pendiente'] = ModuloGetURL("app", "Formulacion_Externa", "controller", "FormulasPaciente_p");
        //added: 31072012
        $action['entregas_farmacia'] = ModuloGetURL("app", "Formulacion_Externa", "controller", "Pendientes_Entrega_Farmacia");
        //

        $this->salida .= $view->Forma($action, $request['buscador'], $datosN, $Tipo_Id_paciente, $cls->conteo, $cls->pagina, $bodegas_doc_id);
        //$this->salida = $view->FormaMenuInicial($action); //para hacer pruebas
    ...
    function GuardarLlamada() {
        ...
        $res = $obje->GuardarLlamada($request);

        $url = ModuloGetURL("app", "Formulacion_Externa", "controller", "Tickets");
        $url2 = $url . "&buscador[formula_papel]=" . $formula_papel;

        $html .= "<script>";
        if (!$res)
            $html .= " history.go(-1) ";
        else {
            $html .= "window.location=\"" . $url2 . "\";";
        }
        $html .= "</script>";
        $this->salida = $html;
        return true;
