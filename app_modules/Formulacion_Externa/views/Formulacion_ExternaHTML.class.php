...
class Formulacion_ExternaHTML {
    ...
    function FormaMensajegenerado($action, $esm_farmaco_id, $formula) {
        ...
        $mostrar = $reporte->GetJavaReport('app', 'Formulacion_Externa', 'FormatoFarmacov', array("esm_farmaco_id" => $esm_farmaco_id, "formula_r" => $formula, "bodega" => $bod, "centroU" => $Centrid), array('rpt_name' => '', 'rpt_dir' => 'cache', 'rpt_rewrite' => TRUE));
    ...
    function FormaBuscarPlantillas($action, $datos, $conteo, $pagina) {
        ...
        if (!empty($datos)) {
            ...
            foreach ($datos as $key => $dtl) {
                ...
                $mostrar = $reporte->GetJavaReport('app', 'Formulacion_Externa', 'FormatoFarmacov', array("esm_farmaco_id" => $dtl['esm_farmaco_id'], "formula_r" => $formula_id), array('rpt_name' => '', 'rpt_dir' => 'cache', 'rpt_rewrite' => TRUE));
    ...
    function FormaCabeceraFormula($action, $request, $Datos_Paciente, $Datos_Ad, $validar_paciente, $edad_paciente, $dx_ingres, $profesionales, $Tipo_Formula) {
        ...
        $html.= "   <script> 
                        function BuscadorMunicipios(div, forma){
                            xajax_BuscadorMunicipios(div, forma);
                        }
                        
                        function Paginador(termino_busqueda, offset)
                        {
                            xajax_BuscadorMunicipio_d(termino_busqueda, offset);
                        }
                        
                        function Seleccionado(formulario, objeto)
                        {
                            document.getElementById('municipio').value = objeto.municipio;
                            document.getElementById('tipo_pais_id').value = objeto.tipo_pais_id;
                            document.getElementById('tipo_dpto_id').value = objeto.tipo_dpto_id;
                            document.getElementById('tipo_mpio_id').value = objeto.tipo_mpio_id;
        ...
        $html .= "										</select>\n";
        $java = "javascript:MostrarCapa('ContenedorMov1');BuscadorMunicipios('ContenidoMov1','unocreate');Iniciar2('BUSCAR MUNICIPIO');\""; //
        $html .= "                          <a  title=\"SELECIONAR MUNICIPIO\" class=\"label_error\" href=\"" . $java . "\">SELECCIONAR MUNICIPIO</a>\n";
        $html .= "									</td>\n";
        $html .= "									<td>\n";
        $html .= "										<input type=\"text\" class=\"input-text\" id='municipio' name=\"municipio\" style=\"width:90%\" maxlength=\"30\"  value='' readonly disabled>\n";
        $html .= "										<input type=\"hidden\" id=\"tipo_pais_id\" name=\"tipo_pais_id\"  value='' >\n";
        $html .= "										<input type=\"hidden\" id=\"tipo_dpto_id\" name=\"tipo_dpto_id\"  value='' >\n";
        $html .= "										<input type=\"hidden\" id=\"tipo_mpio_id\" name=\"tipo_mpio_id\"  value='' >\n";
        $html .= "									</td>\n";       
        $html .= "								</tr>\n";
        $html .= "								</tr>\n";
        $html .= "							</table>\n";
    ...
    function FormaBuscarProductos_Ambulatoria($action, $request, $datos, $conteo, $pagina, $medi_form, $Cabecera_Formulacion, $DX_, $request, $Datos_Ad, $formula_id, $tipo_id_paciente, $paciente_id, $plan_id, $tiempo_entrega, $datos_empresa, $privilegios) {
        ...
        $html .= "
                      function ocultar_producto_pendiente(formula_id, codigo_producto, el){
                        ...
                        xajax_ocultar_producto_pendiente(formula_id, codigo_producto, sw_ocultar);
                      }  
        ...
        $html .= "							<table width=\"98%\" class=\"label\" $style>\n";
        $html .= "								<tr >\n";
        $html .= "									<td width=\"\" colspan=\"1\" style=\"text-align:left;text-indent:8pt\" class=\"formulacion_table_list\" >TIPO FORMULA</td>\n";
        $html .= "									<td colspan=\"1\">" . $Cabecera_Formulacion['descripcion_tipo_formula'] . "\n";
        ...
        $html .= "									</td>\n";
        $html .= "									<td width=\"\" colspan=\"1\" style=\"text-align:left;text-indent:8pt\" class=\"formulacion_table_list\" >MUNICPIO</td>";
       $html .= "									<td colspan=\"1\">" . $Cabecera_Formulacion['municipio'] . " - ".$Cabecera_Formulacion['departamento']."  </td>";
        $html .= "								</tr>\n";
        $html .= "								</tr>\n";
        $html .= "							</table>\n";
    ...
    function FormaPintarUltimoPaso($action, $formula_id, $pendientes, $todo_pendiente, $opcionP, $medi_form, $Cabecera_Formulacion, $dix_r) {
        ...
        $mostrar_P = $reporte2->GetJavaReport('app', 'Formulacion_Externa', 'MedicamentoDispensadosFormulacionExterna', array("formula_id" => $formula_id, "paciente_id" => $paciente_id), array('rpt_name' => '', 'rpt_dir' => 'cache', 'rpt_rewrite' => TRUE));
        ...
        if (!empty($pendientes)) {
            $mostrar_ = $reporte2->GetJavaReport('app', 'Formulacion_Externa', 'MedicamentoPendienteFormulacionExterna', array("formula_id" => $formula_id), array('rpt_name' => '', 'rpt_dir' => 'cache', 'rpt_rewrite' => TRUE));
    ...
    function Forma($action, $request, $lista, $Tipo_Id_paciente, $conteo, $pagina, $bodegas_doc_id) {
        ...
        if (!empty($lista)) {
            ...
            foreach ($lista as $k1 => $dtl) {
                $mostrar = $reporte->GetJavaReport('app', 'Formulacion_Externa', 'MedicamentoDispensadosFormulacionExterna', array("formula_id" => $dtl['formula_id'], "paciente_id" => $dtl['paciente_id'], "opc" => "1"), array('rpt_name' => '', 'rpt_dir' => 'cache', 'rpt_rewrite' => TRUE));
                $funcion = $reporte->GetJavaFunction();

                $mostrar_ = $reporte->GetJavaReport('app', 'Formulacion_Externa', 'MedicamentoPendienteFormulacionExterna', array("formula_id" => $dtl['formula_id']), array('rpt_name' => '', 'rpt_dir' => 'cache', 'rpt_rewrite' => TRUE));
    ...
    function FormaPendientesEntregaFarmacia($action, $formula_id, $Cabecera_Formulacion, $f_papel, $request, $opcion, $medi_form, $Datos_Ad, $dix_r) {
        ...
        $html .= "		<tr>";
        $html .= "		   <td class=\"formulacion_table_list\" style=\"text-align:left;text-indent:8pt\">TELEFONO CONTACTO</td>";
        $html .= "		   <td><input type=\"text\" name=\"telefono\" id=\"telefono\" class=\"input-text\" maxlength=\"12\" value=\"\"></td>";
        $html .= "		</tr>";
        ...
        $html .= "          <table width=\"98%\" class=\"label\" $style>\n";
        $html .= "              <tr>\n";
        $html .= "                  <td colspan=\"7\" style=\"text-align:center;text-indent:8pt\" class=\"formulacion_table_list\">HISTORIAL DE LLAMADAS</td>\n";
        $html .= "              </tr>\n";
        $html .= "		        <tr>\n";
        ...
        $html .= "                  <td style=\"text-align:center;text-indent:8pt\" class=\"formulacion_table_list\">TELEFONO CONTACTO</td>\n";
        ...
        for ($i = 0; $i < sizeof($historial); $i++) {
            ...
            $html .= "  <td align=\"center\" >" . $historial[$i]['tel_contacto'] . "</td>";
