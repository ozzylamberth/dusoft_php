...
function MostrarDX($form, $tipo_id_paciente, $paciente_id) {
    ...
    if (empty($datos)) {
        ...
        for ($i = 0; $i < sizeof($datos); $i++) {
            ...
            //$objResponse->alert("qry: ".$tipo_id_paciente."-".$paciente_id);
            if (!empty($dx_ingres)) {
...
function BuscarProducto1($FormularioBuscador, $descripcion, $codigo_producto, $total_cantidad, $prinicpio_activo, $formula_id, $bodega_otra) {
    ...
    if (empty($datos_ex) || $autorizados['sw_autorizado'] == '1') {
        ...
        //		
        $difer = ($CantidaEntregar - $cantidad_);
        //
        $html .= "                     </td>";
        $html .= "                    </tr>\n";
        ...
        $html .= "                                          <I>FECHA DESPACHO : </I> " . $datos_ex['fecha_registro'] . "\n<BR> ";
        $html .= "                                          <I>CANTIDAD DESPACHADA : </I> " . FormatoValor($datos_ex['unidades']) . "\n<BR> ";
...
function MostrarProductox($formula_id) {
    ...
    if (!empty($datos)) {
        ...
        foreach ($datos as $key => $detalle) {
            ...
            $cantidad_pendiente = round($detalle['cantidad']) - $cantidad_;
            ...
            $checked = "";
            if ($detalle['sw_ocultar'] == "1")
                $checked = " checked='checked' ";

            if ($cantidad_pendiente > 0) {
                $html .= "                       <td     width=\"25%\"><input type=\"checkbox\" name='sw_ocultar' onclick=\"ocultar_producto_pendiente('{$formula_id}', '{$detalle['codigo_producto']}', this);\" class=\"input-text\" {$checked}></td>\n";
            }else{
                $html .= "                       <td     width=\"25%\"></td>\n";
            ...
            if (!empty($cantidad)) {
                ...
                //added: 30072012
                $cod_prod_ant = "";
                $cantPendiente = round($detalle['cantidad']) - $cantidad_;
                $cantSolicita = round($detalle['cantidad']);
                //
                foreach ($cantidad as $ll => $v) {
                    ...
                    /* added:30072012 */
                    if ($cantPendiente > 0) {
                        if ($cod_prod_ant != "" && ($cod_prod_ant != $v['codigo_producto'])) {

                            //$html .= "                   <tr class=\"modulo_list_claro\" >\n";
                            // $html .= "                      <td class=\"formulacion_table_list\" align=\"center\" width=\"12%\">\n";
                            // $html .= "                        CANT. PENDIENTE";
                            // $html .= "                      </td>\n";

                            $html .= "        <td class=\"modulo_list_claro\" align=\"left\" class=\"label_mark\" colspan=0>&nbsp\n";
                            //$html .= "                        $cantPendienteTotal";
                            $html .= "          <input type=\"checkbox\" class=\"input-text\" name=\"reclama_en_farmacia['$cod_prod_ant']\" id=\"reclama_en_farmacia['$cod_prod_ant']\" value=\"$cod_prod_ant\" onclick=\"xajax_GuardaEntregaEnFarmacia_tmp('$formula_id','" . $cod_prod_ant . "','$cantPendiente',this.checked)\" >Entrega Pdtes. en Farmacia?";
                            $html .= "        </td>\n";
                            //$html .= "                   </tr>\n";


                            $cod_prod_ant = $v['codigo_producto'];
                            //$cantPendienteTotal = $v_cantSolicitada[$detalle['codigo_producto']];
                        } elseif ($cod_prod_ant == "") {
                            $cod_prod_ant = $detalle['codigo_producto'];
                            $html .= "        <td class=\"modulo_list_claro\" align=\"left\" class=\"label_mark\" colspan=0>&nbsp\n";
                            //$html .= "       $cantPendienteTotal";
                            $html .= "          <input type=\"checkbox\" class=\"input-text\" name=\"reclama_en_farmacia['$cod_prod_ant']\" id=\"reclama_en_farmacia['$cod_prod_ant']\" value=\"$cod_prod_ant\" onclick=\"xajax_GuardaEntregaEnFarmacia_tmp('$formula_id','" . $cod_prod_ant . "','$cantPendiente',this.checked)\" >Entrega Pdtes. en Farmacia?";
                            $html .= "        </td>\n";
...
function GuardaEntregaEnFarmacia_tmp($formula_id, $codigo_producto, $cantPendiente, $valor) {
    $objResponse = new xajaxResponse();
    //$objResponse->alert($formula_id."-".$codigo_producto."-".$cantPendiente."--".$valor);
    $obje = AutoCarga::factory("Formulacion_ExternaSQL", "classes", "app", "Formulacion_Externa");
    //$objResponse->alert("formula_id:".$formula_id.",codigo_producto:".$codigo_producto.",valor:".$valor);
    $obje->GuardaMarcaEntregaEnFarmacia($formula_id, $codigo_producto, $cantPendiente, $valor);

    // if($obje)
    // {
    // $objResponse->alert("Reclama en farmacia: OK");
    // }		
    // else 
    // {
    // $objResponse->alert("Error al marcar entrega en farmacia");
    // }
    return $objResponse;
}

function ocultar_producto_pendiente($formula_id, $codigo_producto, $sw_ocultar) {
    $objResponse = new xajaxResponse();
    $sql = AutoCarga::factory("Formulacion_ExternaSQL", "classes", "app", "Formulacion_Externa");

    $resultado = $sql->ocultar_productos_pendientes_tmp($formula_id, $codigo_producto, $sw_ocultar);

    if ($resultado) {
        $objResponse->script("console.log('Marcado Correctamente')");
    } else {
        $objResponse->script("console.log('Error Marcando Producto')");
    }
...
function BuscadorMunicipios($div, $Forma) {
    ...
    //$salida .= "                  <form name=\"buscar_farmacia\" action='#'>\n";
    $salida .= "                   <table width=\"85%\" align=\"center\" class=\"modulo_table_list\">\n";
    $salida .= "                    <tr class=\"modulo_table_list_title\">\n";
    $salida .= "                      <td  align=\"center\" colspan='3'>\n";
    $salida .= "                         BUSCADOR DE MUNICIPIOS";
    $salida .= "                      </td>\n";
    $salida .= "                    </tr>\n";
    $salida .= "                    <tr class=\"modulo_list_claro\">\n";
    $salida .= "                       <td width=\"44%\"  align=\"center\">\n";
    $salida .= "                        <b>NOMBRE MUNICIPIO :</b>";
    $salida .= "                       </td>\n";
    $salida .= "                       <td width=\"31%\" align=\"right\" >\n";
    $salida .= "                         <input style=\"width:100%;height:100%\" type=\"text\" class=\"input-text\" name=\"razon_social\" id=\"termino_busqueda\" maxlength=\"40\" size\"40\" value=''>";
    $salida .= "                       </td>\n";
    $salida .= "                       </tr>\n";   
    $salida .= "                <tr class=\"modulo_list_claro\">";
    $salida .= "                       <td colspan=\"2\" width=\"10%\" align=\"center\">\n";                                                                 //$pagina,            $criterio1,                                   $criterio2,                           $criterio                          ,$div,      $Forma
    $salida .= "                        <input type=\"button\" class=\"input-submit\" name=\"boton_bus\"  id=\"boton_bus\" value=\"BUSCAR\" onclick=\"xajax_BuscadorMunicipio_d(document.getElementById('termino_busqueda').value,'1');\")\">\n";
    $salida .= "                       </td>\n";
    $salida .= "                </tr>";
    $salida .= "                 </table>\n";
    ...
    $salida .= "            <div id=\"ListaDeMunicipios\">";
    $salida .= "            </div>";
...
function BuscadorMunicipio_d($termino_busqueda, $offset) {
    
    $objResponse = new xajaxResponse();
        
    $sql = AutoCarga::factory("Formulacion_ExternaSQL", "classes", "app", "Formulacion_Externa");
    
    $lista_municipios = $sql->consultar_municipios($termino_busqueda, $offset);
    $action['paginador'] = "Paginador('" . $termino_busqueda . "'";
    
    $pghtml = AutoCarga::factory("ClaseHTML");

    if (!empty($lista_municipios)) {
        ...
        $salida .= "                      </td>\n";
        $salida .= "                    </tr>\n";
        foreach ($lista_municipios as $key => $valor) {
            $salida .= "<tr class=\"modulo_list_claro\" >";
            $salida .= "<td>" . $valor['municipio'] . " - ( ".$valor['departamento']." )</td>";
            $salida .= "      <td align=\"center\">\n";
            $salida .= "        <a href=\"#\" onclick=\"Seleccionado('unocreate',{ tipo_pais_id : '{$valor['tipo_pais_id']}', tipo_dpto_id : '{$valor['tipo_dpto_id']}', tipo_mpio_id : '{$valor['tipo_mpio_id']}', municipio : '{$valor['municipio']}'})\">\n";
    ...
    $salida = $objResponse->setTildes($salida);
    $objResponse->assign("ListaDeMunicipios", "innerHTML", $salida);
    return $objResponse;
}
