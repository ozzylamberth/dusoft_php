...
/**
 * $Id: formulamedica.report.php,v 1.1.1.1 2009/09/11 20:36:19 hugo Exp $
		...
		$edad=CalcularEdad($datos[0][fecha_nacimiento],$datos[0][fecha_cierre]);
          ...
          if(!empty($datos[0][diagnostico_ingreso]) AND empty($datos[0][diagnostico_egreso])){
               foreach ($datos[0][diagnostico_ingreso] as $k => $v){
                    $reporte->PrintFTexto("'.$v[diagnostico_id].'",false,'left',false,false);
                    $reporte->PrintFTexto('-',false,'left',false,false);
               }
          }else{
          ...
          //nuevo dar
          $reporte->SaltoDeLinea();				
          $reporte->PrintFTexto('Señor usuario, verifique que los medicamentos ordenados correspondan a los entregados, en caso de inconsistencias o que su formula médica quede pendiente de despacho por más de 48 horas, comunicarse al 2262222 ext. 736 para dar solución a su necesidad',false,'center',false,false);
          //fin neuvo dar				
          $reporte->SaltoDeLinea();
          if ($datos[0][uso_controlado]==1)
          {
          ...
          //esta fecha es de la evolucion, pero aca se trabaja por ingreso entonces que se hace?
          /*
