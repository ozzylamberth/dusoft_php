...
// 	}
	...
	*/
	...
	function AsignarCitas()
	{
		...
		SessionDelVar('AsignacionCitas');
		unset($_SESSION['LiquidarCitas']);
		unset($_SESSION['CumplirCita']);
		SessionDelVar('DatosPaciente_CE');
		SessionDelVar('DatosPaciente');
		SessionDelVar('form_buscar_paciente');
		SessionDelVar('AdmHospitalizacion');
...
// 		$this->salida .= '<input type="hidden" name="Existe" value="'.$Existe.'">';
	...
	*/
	...
	function PantallaFinal($orden,$msg)
	{
		...
		if(!empty($_SESSION['AsignacionCitas']['cita']))
		{
			...
			$fechareg =strtoupper(SIIS_sfrtime($a['fecha_registro'],I));
		    $vectorimprimir['fecharegistro']=$fechareg;
        ...
        while($i<sizeof($turnosdia[0]))
        {
          ...
          if($AgendaCitaIdPadre!=$AgendaCitaIdPadreAnt || empty($AgendaCitaIdPadre) || ($AgendaCitaIdPadre==$AgendaCitaIdPadreAnt && !empty($AgendaCitaIdPadre) && $turnosdia[2][$i].' '.$turnosdia[3][$i]!=$identificacionAnt))
          {
            ...
            if(!empty($nom))
            {
              ...
              {
                $Imagen = "<img title=\"Adici�n de Nota Administrativa\" src=\"". GetThemePath() ."/images/news.gif\" border=\"0\">";
              }
            ...
            if($turnosdia[14][$i])
              $this->salida .= "  <label class=\"normal_10AN\" >EL PACIENTE ".(($turnosdia[14][$i] == '2')? "NO":"")." REQUIERE ANESTESI�LOGO</label>";
                ...
                else
                {
                  if(!empty($turnosdia[7][$i]))
                  {
                    if($turnosdia[8][$i]==1)
                    {
                      $this->salida.="<td rowspan=\"$conta\" align=\"center\" width=\"10%\" colspan=\"2\">";
                      $accion=ModuloHCGetURL($turnosdia[7][$i],'','',$_SESSION['Atencion']['hc_modulo'],$_SESSION['Atencion']['hc_modulo'],array('HC_DATOS_CONTROL'=>array('DEPARTAMENTO'=>$_SESSION['Atencion']['departamento'],'ESPECIALIDAD'=>$_SESSION['Atencion']['especialidad'])));
                      $this->salida.='<a href="'.$accion.'">Continuar Atenci�n </a> ';
                      $this->salida.=' - '.$turnosdia[7][$i];
                      $this->salida.='</td>';
                      $entro=0;
                    }
  ...
  */
	function CalendarioAtencion()
	{
		...
		SessionSetVar('CITASMES',$fechas);
		$this->salida .= ThemeAbrirTabla('ATENCI�N CITAS');
		$this->salida.="\n".'<script>'."\n";
		$this->salida.='function year1(t)'."\n";
		$this->salida.='{'."\n";
	...
	*/
	...
	function BuscarPacienteCancelar($mensaje,$arr,$f)
	{
		...
		if($arr){
			...
			$cargos_adicionales = $this->ObtenerSWCargosDepto($_SESSION['CancelarCita']['departamento']);
			$this->salida .= "		   <br>";
			...
			$this->salida .= "<td>PROFESIONAL</td>";
			if($cargos_adicionales[0]['sw_cargos_adicionales']==1){
			$this->salida .= "<td>CARGOS ADICIONALES</td>";
			}
          ...
          while($l<sizeof($arr)){
            ...
            $l++;
					...
					if($cargos_adicionales[0]['sw_cargos_adicionales']==1){
						$this->salida .= "<td rowspan=\"$conta\">";
						if($datosn=$this->CargosCitasAdicionales($arr[$i][7])){
							for ($n = 0; $n < count($datosn[0]); $n++)
							{
								$this->salida .="<table ><td class=\"$estilo\"border = \"1\">";
								$this->salida .=$datosn[1][$n];
