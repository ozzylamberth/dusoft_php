    ...
    * @return string
	  ...
	  { 
			...
			if(sizeof($tipos) > 1)
			{
				...
				{
					$html .= "				<option value=\"".$valor['tipo_afiliado_id']."\" SELECTED>".$valor['tipo_afiliado_nombre']."</option>\n";
				}
				$html .= "			</select>\n";
			}
			else
...
//Modificación
	  function FormularioDatosAfiliado($tipos,$rangos, $UltimaCita,$datosPaciente)
	  { 
			...
			if(sizeof($tipos) > 1)
			{
				...
				foreach($tipos as $key => $valor)
				{	
					if($valor['tipo_afiliado_id'] == $datosPaciente['tipo_afiliado_id']){
						$html .= "<option value=\"".$valor['tipo_afiliado_id']."\" SELECTED >".$valor['tipo_afiliado_nombre']."</option>\n";
					}else{
						$html .= "<option value=\"".$valor['tipo_afiliado_id']."\">".$valor['tipo_afiliado_nombre']."</option>\n";
					}
				}
				$html .= "			</select>\n";
			}
			...
			if(sizeof($rangos) > 1)
			{
				...
				foreach($rangos as $key => $valor)
				{
					if($valor['rango']== $datosPaciente['rango']){
						$html .= "<option value=\"".$valor['rango']."\" SELECTED>".$valor['rango']."</option>\n";
					}else{
						$html .= "<option value=\"".$valor['rango']."\" >".$valor['rango']."</option>\n";
					}
				}
				$html .= "			</select>\n";
			}
    ...
    function FormaImpresionCitas($action, $request,$tiposDocumentos,$listado,$conteo, $pagina)
    {
      ...
      if(!empty($listado))
      {
        ...
        foreach($listado as $key => $dtl)
        {
		  ...
		  //naydu
		  $dtl['TelefonoCancelacion'] = $dtl['telefonocancelacion'];
		  $dtl['DiasCancelacion'] = $dtl['diascancelacion'];
		  $dtl['consultorio'] = $dtl['nom_consultorio'];
		  $fecharegistro = $fechareg =strtoupper(SIIS_sfrtime($dtl['fecha_registro'],'I'));
		  $dtl['fecharegistro'] = $fecharegistro;
