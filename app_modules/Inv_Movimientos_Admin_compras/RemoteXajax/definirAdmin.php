...
//               $salida .= "                       </td>\n";
   ...
   */  
    function GetLixtadox($empresa_id,$centro_id,$bodega,$codigo_pro,$nom_pro,$grupo,$clase,$subclase,$fecha_inicio,$fecha_final,$tipo_movimiento,$tipo_doc_general_id,$centro_utilidad_bus,$bodega_bus,$molecula_bus,$pagina)
    {
     ...
     // $fecha_inicio_lap = $ano1."-".$mes1;
      $fecha_inicio_lap = $ano1."-".$mes1."-".$dia1;
      //$fecha_final_lap = $ano2."-".$mes2;
      $fecha_final_lap = $ano2."-".$mes2."-".$dia2;
    ...
    //         $salida .= "                         </a>\n";
            ...
            for($i=0;$i<count($VECTOR);$i++)
            {
                ...
                $salida .= "                       <td align=\"center\">\n";
                                                    $direccion="app_modules/Inv_Movimientos_Admin_compras/Imprimir/imprimir_producto.php";
                                                    ...
                                                    $x=RetornarImpresionDoc1(trim($direccion),$alt,trim($empresa_id),trim($centro_id),trim($bodega),$VECTOR[$i]['codigo_producto'],$fecha_inicio_lap,$fecha_final_lap,$tipo_movimiento,$tipo_doc_general_id);
                $salida .= "                       ".$x."";
   ...
   {
      $objResponse = new xajaxResponse();
      $mdl = AutoCarga::factory('MovBodSQL','classes','app','Inv_Movimientos_Admin_compras');
    ...
    **/
    function BuscarDocumentx($empresa,$centro,$bodega,$fecha_ini,$fecha_fin,$nom_bodega)
    {
        ...
        if(!empty($vector))
        {
            ...
            for($i=0;$i<count($vector);$i++)
            {
                $BODEGA=ModuloGetURL('app','Inv_Movimientos_Admin_compras','user','ListarDocumentos' ,array('empresa_idx'=>$empresa,'documento_id'=>$vector[$i]['documento_id'],'fecha1'=>$fecha_ini,'fecha2'=>$fecha_fin,'nombre_doc'=>$vector[$i]['descripcion'],'bodega'=>$bodega,'nom_bodega'=>$nom_bodega));
                $salida .= "                   <tr class=\"modulo_list_claro\">\n";
