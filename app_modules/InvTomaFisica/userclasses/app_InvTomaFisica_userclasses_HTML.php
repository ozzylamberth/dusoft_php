...
***********************************************************************************/
 ...
 function Cargue_inventario(){
     ...
     $html.= "<tr>\n";
     $html.= "<td align=\"center\">\n";
     $html.= "<b>Ejemplo de Archivo:</b> \n";
     $html.= "</td>\n";
     $html.= "</tr>\n";
     $html.= "<tr>\n";
     $html.= "<td align=\"center\">\n";
     $path = SessionGetVar("rutaImagenes");
     $html.= "<img src=\"" . $path . "/images/conteo2.PNG\" width='50%' alt='home'/>";//width='128' height='128'
     $html.= "</td>\n";
     $html.= "</tr>\n";
     $html.= "</table>\n";
        ...
        foreach ($datos as $key => $values) {
            ...
            if ($key != 0) {
                if ($values[0] == $empresa_id && trim($values[1]) == trim($_REQUEST['centro_id']) && $values[2] == $_REQUEST['bodega_id'] && $values[9] == $_REQUEST['conteo'] &&
                       ...
                       //$cantidad_producto=$consulta->ConsultarProductoExistente($empresa_id, $_REQUEST['centro_id'], $_REQUEST['bodega_id'], $_REQUEST['cabe_id'],'',$values[3],$_REQUEST['conteo'],$values[6]);
                     //   if($cantidad_producto>=0){
                        // $insert = $consulta->Update_Detalle_inventario($empresa_id, $_REQUEST['centro_id'], $_REQUEST['bodega_id'], $_REQUEST['cabe_id'], $values[3], $values[4], $values[5], $values[6], $values[7], $values[8], $values[9],$cantidad_producto);
                       /// }else{                         
                       ...
                       // }
                    if ($insert) {
                         $estilo="bgcolor='#24833E'";
                         $htmlok.=$this->cuerpo_tabla($empresa_id, $_REQUEST['centro_id'], $_REQUEST['bodega_id'], $_REQUEST['cabe_id'], $values[3], $values[4], $values[5], $values[6], $values[7], $values[8], $values[9],"SI INSERTO",$estilo);
                    } else {
                         $estilo="bgcolor='#800000'";
                         $htmlnull0.=$this->cuerpo_tabla($empresa_id, $_REQUEST['centro_id'], $_REQUEST['bodega_id'], $_REQUEST['cabe_id'], $values[3], $values[4], $values[5], $values[6], $values[7], $values[8], $values[9],"NO INSERTO",$estilo);
                    }
                } else {
                    $bandera=false;
                      if($values[0] != $empresa_id){
                          $menj.=" - EMPRESA NO COINCIDE ".$empresa_id."<br>";
                          $bandera=true;
                      }
                      if(trim($values[1]) != trim($_REQUEST['centro_id'])){
                          $menj.=" - CENTRO UTILIDAD NO COINCIDE ".$_REQUEST['centro_id']."<br>";
                          $bandera=true;
                      }
                      if($values[2] != $_REQUEST['bodega_id']){
                          $menj.=" - BODEGA NO COINCIDE ".$_REQUEST['bodega_id']."<br>";
                          $bandera=true;
                      }
                      if($values[9] != $_REQUEST['conteo']){
                          $menj.=" - EL CONTEO NO COINCIDE ".$_REQUEST['conteo']."<br>";
                          $bandera=true;
                      }
                      if($bandera!=true){
                          $menj.="FALTAN PARAMETROS";
                      }
                     $estilo="bgcolor='#800000'"; 
                     $htmlnull1.=$this->cuerpo_tabla($values[0], $values[1], $values[2], $_REQUEST['cabe_id'], $values[3], $values[4], $values[5], $values[6], $values[7], $values[8], $values[9],$menj,$estilo);
                     $menj='';
                }
            }
            
        }
         $html.=$htmlnull0."".$htmlnull1."".$htmlok;
         $html.="</table>";
 ...
 function ValidacionLotes(){
     ...
     $html.= ThemeAbrirTabla("VALIDAR LOTES");
