...
/**
* $Id: ContabilizarFacturas.class.php,v 1.8 2006/02/07 20:32:45 alex Exp $
*/
...
class ContabilizarFacturas
{
    ...
    */
    ...
    /**
    * REMIENDO TEMPORAL PARA DESCUENTO MAL REALIZADO
    *
    * @var numeric
    * @access private
    */
    var $DescuentoMalHechoPaciente=0;
    ...
    */
    function ContabilizarFactura($empresa_id,$prefijo,$factura_fiscal,$reprocesar=false)
    {
        ...
        //REMIENDO TEMPORAL PARA DESCUENTO MAL REALIZADO
        $this->DescuentoMalHechoPaciente=0;
        $reprocesar=TRUE;
        //--------------------------------------------------
        ...
        //SI LA FACTURA NO ESTA EN LA TABLA TEMPORAL DE CONTABILIZACION
        if($result->EOF)
        {
            $insertar_reg = true;
            $cg_estado = false;
        ...
        $sql = "SELECT a.*, b.numerodecuenta, c.total_cuenta
        ...
        while($Fila=$result->FetchRow())
        {
            if($first_reg)
            {
                ...
                if($Fila['estado']=='2')
                {                
                    if(!$this->ContabilizarFacturaAnulada(&$Fila))
                    {
                        return false;
                    }
                    return true;
                ...
                {
                        $sql="SELECT tipo_cliente FROM planes WHERE plan_id=" . $Fila['plan_id'];
                        
                        $resultado = $dbconn->Execute($sql);
                        
                        if($dbconn->ErrorNo() != 0)
                        {
                                $this->error = "CONTABILIZACION DE FACTURAS - SQL ERROR 5A";
                                $this->mensajeDeError = $dbconn->ErrorMsg();
                                return false;
                        }  
                        ...
                        {
                                $this->error = "CONTABILIZACION DE FACTURAS";
                                $this->mensajeDeError = "No se encontro el plan [" . $Fila['plan_id']. "] en la tabla planes.";
                                return false;
                        } 
                ...
                if(empty($tipo_cliente) && ($tipo_cliente != '00'))
                {
                        $filtro_tipo_cliente = " AND b.tipo_cliente ISNULL";
                }
                else
                {
                        $filtro_tipo_cliente = " AND b.tipo_cliente='$tipo_cliente'";
                }   
                ...
                $sql="SELECT b.empresa_id, b.cuenta, b.naturaleza, b.sw_agrupar_cuentas
                        FROM documentos a
                            LEFT JOIN cg_parametros_documentos b
                                ON (a.documento_id=b.documento_id AND a.empresa_id=b.empresa_id $filtro_tipo_cliente)
    ...
    // CONTABILIZAR CADA CUENTA
        foreach($cuentasFactura as $k=>$cuenta)
        {
            if(!$this->ContabilizarCuenta($cuenta,$reprocesar))
            {
            // SI ALGUNA CUENTA NO PUDO SER CONTABILIZADA
                $sw_ok = false;
            }
        }

    // SI TODAS LAS CUENTAS QUEDARON CONTABILIZADAS GENERO EL REGISTRO CONTABLE
        if($sw_ok)
        {
            if(!$this->GenerarDatosContables())
            {
            // SI NO SE PUDO GENERAR EL REGISTRO CONTABLE
                $sw_ok = false;
            }
        }
    ...
    */
    function ContabilizarCuenta($cuenta,$reprocesar)
    {
        ...
        $sql = "SELECT b.cg_contabilizacion_estado_id, a.transaccion, a.numerodecuenta,
                ...
                a.valor_cubierto, a.valor_descuento_paciente, d.valor_descuento_paciente as valor_descuento_paciente_cta, 
                a.departamento, a.facturado, a.consecutivo, d.plan_id, d.valor_total_empresa,
        ...
        //REMIENDO TEMPORAL PARA DESCUENTO MAL REALIZADO
        $VALOR_DCTO_PACIENTE=0;
        $VALOR_DCTO_PACIENTE_CTA=0;
        while($Fila=$result->FetchRow())
        {
            if($Fila['cg_contabilizacion_estado_id']!='OK')
            {
                ...
                switch($this->datosFactura['tipo_factura'])
                {
                    ...
                    //ECHO "<PRE>";ECHO PRINT_R($Fila,TRUE);ECHO "</PRE>";                    
                        ...
                        //REMIENDO TEMPORAL PARA DESCUENTO MAL REALIZADO
                        $VALOR_DCTO_PACIENTE += $Fila['valor_descuento_paciente'];                        
                        $VALOR_DCTO_PACIENTE_CTA = $Fila['valor_descuento_paciente_cta'];
                        break;
        ...
        //REMIENDO TEMPORAL PARA DESCUENTO MAL REALIZADO
        if($this->datosFactura['tipo_factura']==0 && $VALOR_DCTO_PACIENTE_CTA > $VALOR_DCTO_PACIENTE)
        {
            $this->DescuentoMalHechoPaciente +=  $VALOR_DCTO_PACIENTE_CTA - $VALOR_DCTO_PACIENTE;
        }
    ...
    */
    function ContabilizarCuentaTransacciones($datosTransaccion,$valor)
    {
        ...
        if($datosTransaccion['tarifario_id']=='SYS')
        {
            ...
            {
//ECHO "<PRE>";ECHO PRINT_R($datosTransaccion,TRUE);ECHO "</PRE>";        
                ...
                $sql = "SELECT d.empresa_id, d.cuenta, d.naturaleza, e.sw_tercero, e.sw_centro_costo, e.sw_documento_cruce, e.sw_cuenta_movimiento
...
//ECHO "<PRE>cuentaProducto";ECHO PRINT_R($cuentaProducto,TRUE);ECHO "</PRE>";
...
//ECHO "<PRE>CargoSistema";ECHO PRINT_R($CargoSistema,TRUE);ECHO "</PRE>";
                //Revisar si la cuenta requiere centro de costo
                if($cuentaProducto['sw_centro_costo']=='1')
                {
                ...
                //Revisar si la cuenta requiere documento cruce
                if($cuentaProducto['sw_documento_cruce']!='0')
                {
                    $doc_cruce="'1'";
                }
                else
                ...
                $sql="INSERT INTO cg_temp_contabilizacion_facturas_cargos_cuentaspuc(tipo_factura,transaccion,empresa_id,cuenta,tipo_id_tercero,tercero_id,debito,credito,departamento,unidad_funcional,centro_utilidad,consecutivo,sw_doc_cruce)
            ...
            $sql = "SELECT a.empresa_id, a.cuenta, a.naturaleza, b.sw_tercero, b.sw_centro_costo, b.sw_documento_cruce, b.sw_cuenta_movimiento
            ...
            //Revisar si la cuenta requiere documento cruce
            if($CargoSistema['sw_documento_cruce']!='0')
            {
                $doc_cruce="'1'";
            }
            else
            ...
            $sql="INSERT INTO cg_temp_contabilizacion_facturas_cargos_cuentaspuc(tipo_factura,transaccion,empresa_id,cuenta,tipo_id_tercero,tercero_id,debito,credito,departamento,unidad_funcional,centro_utilidad,sw_doc_cruce)
                    VALUES( '".$this->datosFactura['tipo_factura']."',
                            $datosTransaccion[transaccion],
        ...
        if($datosTransaccion['porcentaje_honorario'])
        {
           $sql = "SELECT a.empresa_id, a.cuenta, a.naturaleza, b.sw_tercero, b.sw_centro_costo, b.sw_documento_cruce, b.sw_cuenta_movimiento
            ...
            //Revisar si la cuenta requiere documento cruce
            if($Cuenta_Honorario['sw_documento_cruce']!='0')
            {
                $doc_cruce="'1'";
            }
            else
        ...
        //Revisar si la cuenta requiere documento cruce
        if($ParametrosCG['sw_documento_cruce']!='0')
        {
            $doc_cruce="'1'";
        }
        else
    ...
    */
    function GenerarDatosContables()
    {
       ...
       // ECHO "<PRE>";ECHO PRINT_R($d,TRUE);ECHO "</PRE>";
        
        //CONTABILIZO LA CUOTA MODERADORA                           CASO ESPECIAL EN QUE LA CUOTA ES MAYOR QUE EL CARGO
        if($this->datosFactura['valor_cuota_moderadora']>0 && ($this->datosFactura['valor_cuota_moderadora'] < $this->datosFactura['total_cuenta']))
        {
            $sql="SELECT a.empresa_id, a.cuenta_cuota_moderadora, a.cuenta_cuota_moderadora_naturaleza,
                    b.sw_tercero, b.sw_centro_costo, b.sw_documento_cruce, b.sw_cuenta_movimiento
        ...
        //REMIENDO TEMPORAL PARA DESCUENTO MAL REALIZADO
        if($this->datosFactura['tipo_factura']==0 && $this->DescuentoMalHechoPaciente)
        {
            $DescuentoMalHechoPaciente['empresa_id'] = $this->FacturaCGpuc['empresa_id'];
            $DescuentoMalHechoPaciente['cuenta']= '417510';
            $DescuentoMalHechoPaciente['tipo_id_tercero']=$this->datosFactura['tipo_id_tercero'];
            $DescuentoMalHechoPaciente['tercero_id']=$this->datosFactura['tercero_id'];
            $DescuentoMalHechoPaciente['departamento']='010301';
            $DescuentoMalHechoPaciente['unidad_funcional']='03';
            $DescuentoMalHechoPaciente['centro_utilidad']='01';
            $DescuentoMalHechoPaciente['debito']=$this->DescuentoMalHechoPaciente;
            $DescuentoMalHechoPaciente['credito']=0;
            $DescuentoMalHechoPaciente['detalle']='DESCUENTO PACIENTE MAL HECHO';

            $d[]=$DescuentoMalHechoPaciente;
            unset($DescuentoMalHechoPaciente);        
        }
        ...
        if($sumCredito != $sumDebito)
        {
            ...
            //ECHO "<PRE>$sumCredito - $sumDebito <br>";ECHO PRINT_R($d,TRUE);ECHO "</PRE>";
        ...
        if($result->EOF)
        {
            $this->error = "CONTABILIZACION DE FACTURAS  - SQL ERROR GDC-4A";
            $this->mensajeDeError = "No se pudo obtener el tipo de documento para el documento_id ".$this->datosFactura['documento_id']." de la empresa ".$this->datosFactura['empresa_id'];
            return false;
        }
        ...
        {
            $dbconn->RollbackTrans();
            $this->error = "CONTABILIZACION DE FACTURAS - SQL ERROR GDC-5A";
            $this->mensajeDeError = "Error no retorno valor la secuencia : $sql";
            return false;            
        }
    ...
    /**
    * Metodo para contabilizar una factura anulada
    *
    * @param integer $cuenta
    * @param boolean $reprocesar
    * @return boolean
    * @access private
    */
    function ContabilizarFacturaAnulada($datosFactura)    
    {
        ...
        $sql="SELECT b.tipo_movimiento_id, a.documento_id
                FROM documentos a, tipos_doc_generales b
                WHERE a.documento_id =".$datosFactura['documento_id']."
                AND a.empresa_id='".$datosFactura['empresa_id']."'
                AND b.tipo_doc_general_id=a.tipo_doc_general_id";
        ...
        if($dbconn->ErrorNo() != 0)
        {
            $this->error = "CONTABILIZACION DE FACTURAS ANULADAS - SQL ERROR FAN-1";
            $this->mensajeDeError = $dbconn->ErrorMsg();
            return false;
        }
        if($result->EOF)
        {
            $this->error = "CONTABILIZACION DE FACTURAS ANULADAS - SQL ERROR FAN-1A";
            $this->mensajeDeError = "No se pudo obtener el tipo de documento para el documento_id ".$datosFactura['documento_id']." de la empresa ".$datosFactura['empresa_id'];
            return false;
        }
        list($tipo_movimiento_id,$documento_id)=$result->FetchRow();
        $result->Close();
        ...
        {
            $dbconn->RollbackTrans();
            $this->error = "CONTABILIZACION DE FACTURAS ANULADAS - SQL ERROR FAN 2";
            $this->mensajeDeError = $dbconn->ErrorMsg();
            return false;
        }
        if($result->EOF)
        {
            $dbconn->RollbackTrans();
            $this->error = "CONTABILIZACION DE FACTURAS ANULADAS - SQL ERROR FAN-2A";
            $this->mensajeDeError = "Error no retorno valor la secuencia : $sql";
            return false;
        }
        list($documento_contable_id)=$result->FetchRow();
        ...
        $sql ="INSERT INTO cg_movimientos_contables(
                    ...
                    $documento_contable_id,
                    '".$datosFactura['fecha_registro']."',
                    'NOW()',
                    '".$datosFactura['empresa_id']."',
                    $documento_id,
                    '$tipo_movimiento_id',
                    '0',
                    0,
                    0,
                    '".$datosFactura['tipo_id_tercero']."',
                    '".$datosFactura['tercero_id']."',
                    '".UserGetUID()."'
                );";
        ...
        $sql .="INSERT INTO cg_movimientos_contables_facturas(
                   ...
                   numero
                    ...
                    $documento_contable_id,
                    '".$datosFactura['empresa_id']."',
                    '".$datosFactura['prefijo']."',
                    ".$datosFactura['factura_fiscal']."
                );";  
                
        $sql .="UPDATE fac_facturas
                SET documento_contable_id=$documento_contable_id
                WHERE empresa_id='".$datosFactura['empresa_id']."'
                AND prefijo='".$datosFactura['prefijo']."'
                AND factura_fiscal=".$datosFactura['factura_fiscal'].";";

        $sql .="DELETE FROM cg_temp_contabilizacion_facturas
                WHERE empresa_id='".$datosFactura['empresa_id']."'
                AND prefijo='".$datosFactura['prefijo']."'
                AND numero=".$datosFactura['factura_fiscal'].";";                
        ...
        {
            $dbconn->RollbackTrans();
            $this->error = "CONTABILIZACION DE FACTURAS ANULADAS - SQL ERROR FAN-3";
            $this->mensajeDeError = $dbconn->ErrorMsg();
            return false;
        }
        else
