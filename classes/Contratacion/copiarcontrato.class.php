 ...
 */
    ...
    class copiarcontrato
    {
    
            function copiarcontrato(&$dbconn,$vector)
            {
            ...
            function InsertarDatosContrato(&$dbconn,$vector)
            {
							...
							if ($dbconn->ErrorNo() != 0)
							{
									...
									//$_POST['numeroctra']='';
									$this->frmError["MensajeError"]="DATOS DUPLICADOS - VERIFICAR NÚMERO DEL CONTRATO".$dbconn->ErrorMsg();
									$this->uno=1;
									return false;
							}
							...
							if($vector['tarifario2']<>NULL AND $vector['tarifario2']<>$vector[plan_id])
							{
									...
									if($_POST['copiarplanescamas']==1)
									{
										...
										if ($dbconn->ErrorNo() != 0)
										{
												$this->frmError["MensajeError"]="OCURRIÓ UN ERROR AL COPIAR LOS DATOS(planes_tipos_camas) ".$dbconn->ErrorMsg();
												$dbconn->RollBackTrans();
												return false;
										}
										...
										if ($dbconn->ErrorNo() != 0)
										{
												$this->frmError["MensajeError"]="OCURRIÓ UN ERROR AL COPIAR LOS DATOS(planes_tipos_liq_habitacion) ".$dbconn->ErrorMsg();
												$dbconn->RollBackTrans();
												return false;
										}
									}
