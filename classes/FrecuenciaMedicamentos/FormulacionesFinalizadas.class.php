<?php
	/**************************************************************************************
	* $Id: FormulacionesFinalizadas.class.php,v 1.5 2011/02/17 13:21:27 hugo Exp $ 
	...
	**************************************************************************************/	
	...
	class FormulacionesFinalizadas
	{
      ...
      $this->scripts .= "		ImgAbajo.src = \"".GetThemePath()."/images/abajo.png\";\n";
			...
			$this->scripts .= "			window.document['Img'+indice].src = ImgAbajo.src; \n";
			$this->scripts .= "		}\n";			
      $this->scripts .= "		function EvaluarDiasTratamiento(codigoProducto,opcion,nombreProducto)\n";
			$this->scripts .= "		{\n";
			$this->scripts .= "			dias = document.getElementById('tratamiento'+codigoProducto).value; \n";
			$this->scripts .= "		  if(!IsNumeric(dias))\n";
			$this->scripts .= "		  {\n";
			$this->scripts .= "		    alert('PARA EL PRODUCTO '+nombreProducto+', FAVOR INDICAR LOS DIAS DE TRATAMIENTO');\n";
			$this->scripts .= "		    return;\n";
			$this->scripts .= "		  }\n";
			$this->scripts .= "		  Reformular(codigoProducto,opcion,nombreProducto,dias)\n";
			$this->scripts .= "		}\n";
 			$this->scripts .= "	  function acceptNum(evt)\n";
			$this->scripts .= "	  {\n";
			$this->scripts .= "		  var nav4 = window.Event ? true : false;\n";
			$this->scripts .= "		  var key = nav4 ? evt.which : evt.keyCode;\n";
      $this->scripts .= "		  return (key <= 13 ||(key >= 48 && key <= 57));\n";
			$this->scripts .= "	  }\n";
      $this->scripts .= "	  function IsNumeric(valor)\n";
			$this->scripts .= "	  {\n";
			$this->scripts .= "		  var log = valor.length; \n";
			$this->scripts .= "		  var sw='S';\n";
			$this->scripts .= "		  var puntos = 0;\n";
			$this->scripts .= "		  for (x=0; x<log; x++)\n";
			$this->scripts .= "		  { \n";
			$this->scripts .= "			  v1 = valor.substr(x,1);\n";
			$this->scripts .= "			  v2 = parseInt(v1);\n";
			$this->scripts .= "			  //Compruebo si es un valor numérico\n";
			$this->scripts .= "			  if(v1 == '.')\n";
			$this->scripts .= "			  {\n";
			$this->scripts .= "			  	puntos ++;\n";
			$this->scripts .= "			  }\n";
			$this->scripts .= "			  else if (isNaN(v2)) \n";
			$this->scripts .= "			  { \n";
			$this->scripts .= "			  	sw= 'N';\n";
			$this->scripts .= "			  	break;\n";
			$this->scripts .= "		  	}\n";
			$this->scripts .= "		  }\n";
			$this->scripts .= "		  if(log == 0) sw = 'N';\n";
			$this->scripts .= "		  if(puntos > 1) sw = 'N';\n";
			$this->scripts .= "		  if(sw=='S') return true;\n";
			$this->scripts .= "		  return false;\n";
			$this->scripts .= "	  } \n";
      $this->scripts .= "   function ProtocolosFormulacion(codigo_medicamento)\n";
      $this->scripts .= "   {\n";
      $this->scripts .= "     var url = \"ProtocolosFormulacion.class.php?codigo_medicamento=\"+codigo_medicamento+\"&ingreso=".$this->datos['ingreso']."\";\n";
      $this->scripts .= "     var str =\"width=900 ,height=300,resizable=no,location=yes,toolbar=1,status=no,scrollbars=yes\";\n";
      $this->scripts .= "     window.open(url,'PROTOCOLOS DE FORMULACION',str).focus();\n";
      $this->scripts .= "   }\n";
			$this->scripts .= "</script>\n";
      ...
      $this->salida .= ReturnBody()."<br>\n";
        ...
        if($datos['protocolo'])
        {
          $this->salida .= "ProtocolosFormulacion('".$datos['protocolo']."');";
        }
        $this->salida .= "\">\n";
				...
				$this->salida .= "												<td colspan=\"2\"><b class=\"label_mark\">".$datos['nombre']."</b></td>\n";
				$this->salida .= "											  <td>\n";
        if($datos['protocolo'])
        {
          $this->salida .= "						              <a href=\"javascript:ProtocolosFormulacion('".$datos['protocolo']."')\" title=\"Protocolos Formulacion\" >\n";
        ...
        if(!strstr($datos['frecuencia'],"Cada"))
          $this->salida .= "							            <input type=\"hidden\" id=\"tratamiento".$datos['codigo_producto']."\" name=\"tratamiento".$datos['codigo_producto']."\" value=\"1\"><label>1<label>\n";
				else
          $this->salida .= "							            <input type=\"text\" class='input-text' size=\"5\" id=\"tratamiento".$datos['codigo_producto']."\" name=\"tratamiento".$datos['codigo_producto']."\" value=\"".$datos['dias_tratamiento']."\" onkeypress=\"return acceptNum(event);\">\n";
        ...
        $this->salida .= "						            </td>\n";
				...
				if(SessionGetVar("tipoProfesionalhc") == '1')
				{
					...
					$this->salida .= "      									<a $est1 class=\"label_mark\" href=\"javascript:EvaluarDiasTratamiento('".$datos['codigo_producto']."','1','".$datos['producto']."')\">\n";
    ...
    function MedicamentosFinalizados()
    {
      ...
      $sql .= "       MP.codigo_medicamento AS protocolo ";
      ...
      $sql .= "       LEFT JOIN medicamentos_protocolos MP ";
      $sql .= "       ON(MP.codigo_medicamento = ME.codigo_medicamento) ";
			...
			$sql .= "AND		FM.sw_confirmacion_formulacion = '1' ";
