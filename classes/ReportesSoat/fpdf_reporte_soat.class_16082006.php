 ...
 */
	...
	class fpdf_reporte_soat extends FPDF
	{
			...
			function TraerDatosReclamacionEntidades($TipoDo,$Docume,$evento)
			{
				...
				$vect[0][0]=61;
				$vect[0][1]=54;
				$vect[0][2]=$_SESSION['soa1']['razonso'];//61,54,
				if($var[tipo_id_tercero] AND $var[id])
				{
					$vect[1][0]=138;
					$vect[1][1]=54;
					$vect[1][2]=$var[tipo_id_tercero].'-'.$var[id];//138,54,
				}
...
//$pdf->Text(162,79,'CALI');
			...
			//$vect[2][5][1]=$var['fecha_accidente'];
				...
				if($hora[0]<'12')
				{
					$vect[46][0]=112;
					$vect[46][1]=185;
					$vect[46][2]='X';//112,179 //fecha ingreso
				}
				else
				{
					$vect[46][0]=119;
					$vect[46][1]=185;
					$vect[46][2]='X';//119,179 //fecha ingreso
				}
				...
				$vect[46][0]=42;
				$vect[46][1]=185;
				$vect[46][2]=$fecha[0];//42,179 //fecha ingreso

				$vect[47][0]=52;
				$vect[47][1]=185;
				$vect[47][2]=$fecha[1];//52,179

				$vect[48][0]=58;
				$vect[48][1]=185;
				$vect[48][2]=$fecha[2];//58,179
				...
				if($var2['ingreso']<>NULL)
				{
					...
					$vect[50][0]=42;
					$vect[50][1]=194;
					$vect[50][2]=$fecha[0];//42,188 //fecha egreso
	
					$vect[51][0]=52;
					$vect[51][1]=194;
					$vect[51][2]=$fecha[1];//52,188
	
					$vect[52][0]=58;
					$vect[52][1]=194;
					$vect[52][2]=$fecha[2];//58,188 //fecha egreso
				...
				if($var3[0]['evolucion_id']<>NULL)
				{
					...
					for($j=0;$j<sizeof($var4);$j++)
					{
						...
						$vect[56][0]=56;
						$vect[56][1]=220;
						$vect[56][2].=$var4[$j]['ingreso'];//56,227 //diagnostico de egreso
						}
					}
...
/*					$vect1[3][5][1]='---';
		...
		//VECTOR 4
				for($j=0;$j<sizeof($var6);$j++)
				{
						$vect[67][0]=65;
						$vect[67][1]=258;
						$vect[67][2].=$var6[$j]['muerte'];//65,258 causa_muerte
				}
				return $vect;
			}
//**********************************************
...
/*			$dpto=CallMetodoExterno('app','Triage','user','nombre_dpto',$argumentos=array(
		...
		function BuscarReporteAmbulanciaSoat($ambulancia)//Busca la información de la ambulancia para imprimir
		{
			...
			$vect[1][0]=177;
			$vect[1][1]=30;
			$vect[1][2]=$fecha_accidente[2];//167 32
	
			$vect[2][0]=155;
			$vect[2][1]=30;
			$vect[2][2]=$fecha_accidente[1];//175 32
	
			$vect[3][0]=193;
			$vect[3][1]=30;
			$vect[3][2]=$fecha_accidente[0];//183 32
	
			$vect[4][0]=188;
			$vect[4][1]=35;
			$vect[4][2]=$var[placa_vehiculo];//162 37
	
			$vect[5][0]=145;
			$vect[5][1]=40;
			$vect[5][2]=$var[poliza];//140 42
	...
	// 		$vect[2][2]=$var[tipo_id_paciente];//32 50
			...
			$vect[6][0]=32;
			$vect[6][1]=50;
			$vect[6][2]=$var[conductor_id];//32 50
	
			$vect[7][0]=70;
			$vect[7][1]=50;
			$vect[7][2]=$var[exmunicipio];//70 50
	
			$vect[8][0]=102;
			$vect[8][1]=44;
			$vect[8][2]=$var[nombre_conductor];//102 44
	
			$vect[9][0]=110;
			$vect[9][1]=50;
			$vect[9][2]=$var[direccion];//110 50
	
			$vect[10][0]=158;
			$vect[10][1]=50;
			$vect[10][2]=$var[municipio];//155 50
	
			$vect[11][0]=188;
			$vect[11][1]=50;
			$vect[11][2]=$var[telefono];//180 50
	
			$vect[12][0]=73;
			$vect[12][1]=57;
			$vect[12][2]=$var[lugar_desde];//73 57
	
			$vect[13][0]=125;
			$vect[13][1]=57;
			$vect[13][2]=$var[lugar_hasta];//125 57
			...
			$vect[15][0]=180;
			$vect[15][1]=64;
			$vect[15][2]=$var[placa_ambulancia];//170 62
	
			$vect[16][0]=142;
			$vect[16][1]=72;
			$vect[16][2]=$var[nombrpa];//142 69
	
			$vect[17][0]=67;
			$vect[17][1]=77;
			$vect[17][2]=$var[paciente_id];//67 74
	
			$vect[18][0]=30;
			$vect[18][1]=86;
			$vect[18][2]=$var[empresa];//30 83
	
			$vect[19][0]=105;
			$vect[19][1]=86;
			$vect[19][2]=$var[id];//105 83
