...
/**
 * $Id: fpdf_reporte_soat.class.php,v 1.13 2007/02/27 15:08:32 carlos Exp $
 ...
 */
    ...
    /**
    * Funcion donde se obtienen los datos del reporte de soat
    *
    * @param string $TipoDo Tipo de documento del paciente
    * @param string $Docume Documento del paciente
    * @param integer $evento identificador del evento
    * @param integer $ingreso Identificador del ingreso
    *
    * @return mixed
    */
    function TraerDatosReclamacionEntidades($TipoDo,$Docume,$evento,$ingreso)
    {
      ...
      $query = "SELECT  A.residencia_direccion,
                ...
                FROM    pacientes AS A,
                        ...
                        tipo_mpios AS O,
                        soat_eventos AS D
                        LEFT JOIN condicion_accidentados AS E 
      ...
      $vect[81][0]=166;
      $vect[81][1]=98;
      $vect[81][2]=$var['lugar_expedicion_documento'];//LUGAR EXPEDICION DOCUMENTO
      ...
      else if($var['tipo_id_conductor']=='PAS')
      {
        $vect[40][0]=137;
        $vect[40][1]=229;
        $vect[40][2]='X';//132,154
      ...
      if ($dbconn->ErrorNo() != 0)
      {
        $this->error = "Error al Selecionar datos1";
        $this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
        return false;
      }
      while(!$resulta->EOF)
      ...
      $vect[46][0]=44;
      $vect[46][1]=275;
      $vect[46][2]=$fecha[0];//42,179 //fecha ingreso AÑO
      ...
      $vect[50][0]=44;
      $vect[50][1]=290;
      $vect[50][2]=$fecha[0];//42,188 //fecha egreso AÑO
      ...
      $fecha_ingrso=$vect[48][2].'/'.$vect[47][2].'/'.$vect[46][2];//42,179 //fecha ingreso
      $dias_estancia = $vect1[3][2][1]-$fecha_ingrso;
                        
      if($dias_estancia>0)
        $vect[53][2]=$dias_estancia;//dias_estancia
      ...
      else if($var['tipo_tratamiento']=='1')//HOSPITALARIO
        {
          $vect[69][0]=178;
          $vect[69][1]=298;
          $vect[69][2]='X';//158,192
        }
        else if($var['tipo_tratamiento']=='2')//AMBULATORIO
          {
            $vect[70][0]=200;
            $vect[70][1]=298;
            $vect[70][2]='X';//158,192
      ...
      $vect[55][0]=60;
      $vect[55][1]=305;
      $vect[55][2].=$var2['ingreso'];//56,194 diagnostico de ingreso
      $query = "SELECT  A.ingreso AS ingreso_id,
                        B.fecha_ingreso,
                        C.via_ingreso_nombre,
                        E.fecha_cierre,
      ...
      {$diag_egreso = $var3['egreso'];}
      else
      {$diag_egreso = $var2['ingreso'];}
      $vect[56][2].=$diag_egreso;//56,227 desc_diagnostico_de _EGRESO
      if($var2['evolucion_id']<>NULL AND $var2['ingreso_id']<>NULL)
      {
        ...
        if ($dbconn->ErrorNo() != 0)
        {
          $this->error = "Error al Cargar datos hc_conducta_diagnosticos_defuncion";
          $this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
          return false;
        }
        if(!$resulta->EOF)
      ...
      if($var2['evolucion_id']<>NULL)
      {
        ...
        if($result->RecordCount()>0)
        {
          ...
          $vect[59][0]=160;
          $vect[59][1]=335;
          $vect[59][2]=$f2[2];//155,236 fecha remision AÑO
          ...
          $f3=explode(' ',$var2[$i][fecha_cierre]);
          $f4=explode('-',$f3[0]);
          $vect[64][0]=160;
          $vect[64][1]=345;
          $vect[64][2]=$f4[0];//155,240 fecha año 
      ...
      $vect[64][0]=160;
      $vect[64][1]=345;
      $vect[64][2]=$f3[2];//155,240 fecha año
      ...
      if($var6['diagnostico_nombre']<>NULL)
      {
        $j=sizeof($var6)-1;
        $vect[67][0]=70;
        $vect[67][1]=355;
        $vect[67][2]=$var6['diagnostico_nombre'];//65,258 causa_muerte

        $f1=explode(' ',$var6['fecha']);
        $fecha_muerte=explode('-',$f1[0]);
        $hora_muerte=explode(':',$f1[1]);

        $vect[73][0]=65;
        $vect[73][1]=365;
        $vect[73][2]=$fecha_muerte[0];//AÑO DEFUNCION
        ...
        elseif(($hora_muerte[0] > 0 AND $hora_muerte[0] < 12) OR ($hora_muerte[0] === 0))
        {
          $vect[76][0]=150;
          $vect[76][1]=365;
        ...
        //DATOS DEL PROFESIONAL QUE FIRMÓ EL CERTIFICADO DE DEFUNCIÓN
        $vect[78][0]=135;
        $vect[78][1]=375;
        $vect[78][2]=$var6['nombre'];//NOMBRE PREFESIONAL

        $vect[79][0]=50;
        $vect[79][1]=385;
        $vect[79][2]=$var6['registro_salud_departamental'];//REGISTRO MEDICO
      }
        return $vect;
    }
    /**
    * Funcion donde se obtiene la informacion del reporte soat 2
    *
    * @param integer $evento identificador del evento
    * @param integer $ingreso Identificador del ingreso
    *
    ...
    */
    function BuscarAtencionMedica($evento,$ingreso)//
    {
      ...
      $query = "SELECT  C.nombres_declara,
                ...
                FROM    soat_accidente AS A,
                        ...
                        LEFT JOIN terceros AS E 
                        ON  ( C.tipo_id_tercero=E.tipo_id_tercero
                          AND C.tercero_id=E.tercero_id)
                        LEFT JOIN tipo_mpios AS F 
                        ON  ( C.extipo_pais_id=F.tipo_pais_id
    ...
    /**
    * Funcion donde se obtiene la informacion del reporte soat 3
    *
    * @param integer $ambulancia identificador de la ambulancia
    *
    * @return mixed
    */
    function BuscarReporteAmbulanciaSoat($ambulancia)//Busca la información de la ambulancia para imprimir
    {
      list($dbconn) = GetDBconn();
      ...
      $vect[3][0]=185;
      $vect[3][1]=42;
      $vect[3][2]=$fecha_accidente[2][2];//167 32 AÑO

      $vect[4][0]=190;
      $vect[4][1]=42;
      $vect[4][2]=$fecha_accidente[2][3];//167 32 AÑO
      ...
      $vect[42][0]=92;
      $vect[42][1]=118;
      $vect[42][2]=$var[lugar_expedicion_documento];//lugar_expedicion_documento
