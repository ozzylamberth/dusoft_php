  ...
  class ManejadorDeHC extends classModules
  {
    ...
    */
    function Enviroment()
    {
      ...
      if(!empty($_REQUEST['HC_DATOS_CONTROL']['ESTACION']) || $_REQUEST['estacion'] )
      {
        $id_estacion = ( $_REQUEST['estacion'])?  $_REQUEST['estacion']:$_REQUEST['HC_DATOS_CONTROL']['ESTACION'];
        list($dbconn) = GetDBconn();
        $sql="SELECT departamento FROM estaciones_enfermeria WHERE estacion_id='".$id_estacion."';";
        $result = $dbconn->Execute($sql);
        if($dbconn->ErrorNo() != 0)
        {
          $this->error = "Error en la consulta";
        ...
        if (!$result->EOF)
        {
          ...
          $this->estacion_id = ( $_REQUEST['estacion'])?  $_REQUEST['estacion']:$_REQUEST['HC_DATOS_CONTROL']['ESTACION'];
        }
        $result->Close();
      }
      return true;
          ...
          foreach($NivelSeccion as $Key_Seccion=>$NivelPaso)
          {
            foreach($NivelPaso as $Key_Paso=>$NivelSecuencia)
            {
              ...
              foreach($NivelSecuencia as $Key_Secuencia=>$DatosSubmodulo)
              {
                ...
                if($DatosSubmodulo['paso'] == -1) $I = $DatosSubmodulo['paso'];
                ...
                //if($DatosSubmodulo['paso'] == -1) $I = $aux-1 ;
              }
              $I++;
            }
          }
    ...
    */
    function CerrarHistoria()
    {
      ...
      unset($estacion_id_Panel);
      $estacion_id_Panel = $_SESSION['HISTORIACLINICA']['PIRMERA_ATENCION']['PROFESIONAL'][$this->datosEvolucion['ingreso']]['estacion_id'];
      if(!$estacion_id_Panel)
      { $estacion_id_Panel = $_SESSION['HISTORIACLINICA']['PIRMERA_ATENCION']['ENFERMERA'][$this->datosEvolucion['ingreso']]['estacion_id'];}
      
      unset($_SESSION['HISTORIACLINICA']['PIRMERA_ATENCION']['PROFESIONAL'][$this->datosEvolucion['ingreso']]['estacion_id']);
      unset($_SESSION['HISTORIACLINICA']['PIRMERA_ATENCION']['ENFERMERA'][$this->datosEvolucion['ingreso']]['estacion_id']);
      
      if($_SESSION['HISTORIACLINICA']['PIRMERA_ATENCION']['PROFESIONAL'][$this->datosEvolucion['ingreso']][$estacion_id_Panel] == true)
      {
        $this->Insert_EvolucionPrimeraAtencion(1, $this->datosEvolucion['evolucion_id'], $estacion_id_Panel);
      }
      
      if($_SESSION['HISTORIACLINICA']['PIRMERA_ATENCION']['ENFERMERA'][$this->datosEvolucion['ingreso']][$estacion_id_Panel] == true)
      {
        $this->Insert_EvolucionPrimeraAtencion(2, $this->datosEvolucion['evolucion_id'], $estacion_id_Panel);
      }
      return true;
    ...
    */
    function IncludePasoHC()
    {
      ...
      if(empty($submodulosPaso)) return false;
    ...
    */
    ...
    /**
    * Funcion que inserta la primera Evolucion de atencion a un paciente
    * el cual se la ha realizado un traslado.
    */
    function Insert_EvolucionPrimeraAtencion($sw, $evolucion, $estacion_id)
    {
      list($dbconn) = GetDBconn();
      
      if($sw == 1)
      {
        $campo = "evolucion_id_medico";
        $sql = "SELECT evolucion_id_medico FROM movimientos_habitacion
               WHERE movimiento_id = ".$_SESSION['HISTORIACLINICA']['PIRMERA_ATENCION']['PROFESIONAL'][$this->datosEvolucion['ingreso']][$estacion_id]['movimiento_id'].";";
        $profesional = "PROFESIONAL";
      }
      else
      {
        $campo = "evolucion_id_enfermera";
        $sql = "SELECT evolucion_id_enfermera FROM movimientos_habitacion
               WHERE movimiento_id = ".$_SESSION['HISTORIACLINICA']['PIRMERA_ATENCION']['ENFERMERA'][$this->datosEvolucion['ingreso']][$estacion_id]['movimiento_id'].";";
        $profesional = "ENFERMERA";
      }
      $resultado = $dbconn->Execute($sql);
      if ($dbconn->ErrorNo() != 0)
      {
        $this->error = "EE_PanelEnfermeria - Busqueda de info en movimientos_habitacion";
        $this->mensajeDeError = $dbconn->ErrorMsg();
        return false;
      }
      
      if($resultado->fields[0] == null)
      {
        $update = "UPDATE movimientos_habitacion SET $campo = ".$this->datosEvolucion['evolucion_id']."
                    WHERE movimiento_id = ".$_SESSION['HISTORIACLINICA']['PIRMERA_ATENCION'][$profesional][$this->datosEvolucion['ingreso']][$estacion_id]['movimiento_id'].";";
        $dbconn->Execute($update);
        if ($dbconn->ErrorNo() != 0)
        {
          $this->error = "EE_PanelEnfermeria - Error actualizando la evolucion del profesional (Medico o Enfermera) en movimientos_habitacion";
          $this->mensajeDeError = $dbconn->ErrorMsg();
          return false;
        }
      }
