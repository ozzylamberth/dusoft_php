<?php
  /**
  * @package IPSOFT-SIIS
  * @version $Id: InformacionBaseDatos.class.php,v 1.4 2009/02/10 16:02:02 hugo Exp $
  ...
  class InformacionBaseDatos
  {
    ...
    */
    function ObtenerInformacion($cnf,$filtros)
    {
      ...
      if(!is_null($cnf['base_datos']['dbname']) && trim($cnf['base_datos']['dbname']) != "")
      {
    ...
    */
    function ConstruirConsulta($tabla,$esquema,$campos,$datos)
    {
      ...
      else
      {
        ...
        $sql .= "WHERE ".$ctl->FiltrarNombres($datos['nombres'],$datos['apellidos'],"",$campos['primer_nombre'],$campos['segundo_nombre'],$campos['primer_apellido'],$campos['segundo_apellido']);
    ...
    */
    function CrearConexionBD($datos)
		{
			$this->dbconn = ADONewConnection($datos['dbtype']);
			$this->bdconn->debug=true;
			if (!($this->dbconn->Connect($datos['host'], $datos['user'], $datos['pass'], $datos['dbname'])))
			{
