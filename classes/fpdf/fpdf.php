<?php
/*******************************************************************************
* $Id: fpdf.php,v 1.3 2010/06/03 20:45:30 hugo Exp $
* Software: FPDF                                                               *
* Version:  1.52                                                               *
* Date:     2003-12-30                                                         *
...
/*ESTOS SON LOS PROPIOS DE LA APLICACIï¿½*/
        ...
        elseif($format=='prueba')
            $format=array(198,675);//para stickers..tickets //Config para 10 prod.
            //$format=array(200,265);//para stickers..
        elseif($format=='mcarta') //media carta
          $format=array(311,396);
        elseif($format=='mcarta1') //media carta
          $format=array(486,290);
        else
       ...
       if($_SESSION['REPORTES']['VARIABLE']=='facturaconcepto')
        {
                ...
                $this->SetFont('Arial','',8);
                $this->Cell(0,2,'Pagina No '.$this->PageNo());
                $html="".$this->image('images/logocliente.png',10,8,72)."";
               // $html="".$this->image('images/icontec.png',170,8,18)."";//$pdf->image('classes/fpdf/clinicaoccidente.gif'
                $html.="<table border=0 width=100 align='center' border=0>";
                $html.="<tr><td width=760>&nbsp;</td></tr>";
                $html.="<tr><td width=760><BR><BR></td></tr>";
                $html.="<tr><td width=460><B>".$datos[0][razon_social]."</B></td><td width=300 align='center' ><span style='font-size:28px!important;'>FACTURA DE VENTA</span></td></tr>";

                $html.="<tr><td width=460>".$datos[0][tipoid].": ".$datos[0][id]."</td><td width=300 ><span style='font-size:28px!important;'>No. ".$datos[0][prefijo]."".$datos[0][factura_fiscal]."</span></td></tr>";
                $html.="<tr><td width=240>DIRECCION: ".$datos[0][direccion]."</td><td width=230> TELEFONOS: ".$datos[0][telefonos]."</td><td width=300>".$datos[0][municipio]."-".$datos[0][departamento]."</td></tr>";
                $html.="<tr><td width=240>DIRECCION: Cll 64 G # 92-72</td><td width=230> TELEFONOS: 7422299</td><td width=300>BOGOTA</td></tr>";
...
//        $this->Cell(100,9,'Pagina No '.$this->PageNo());
    ...
    //caso de claudia liliana val
        ...
        if($_SESSION['REPORTES']['VARIABLE']=='cuadre_caja')
        {
                ...
                $info=TraerDatoUsuario($datos_cierre['usuario_id']);
                $_SESSION['observa']=$datos_cierre['observaciones'];
                $_SESSION['login']=$info['usuario'];
                $impresion="IMPRESION :";
                $html.= "<td align='center' width=780 ><font color='black'><b>".$datos['razon_social']."$space $space $space".$datos['tipo_id_tercero']."$space".$datos['id']."$space $space $space $space $impresion $space".date("Y-m-d H:i")."</b></font></td>";
...
/*              if(empty($_SESSION['TMP']['CONTROL_CIERRE']['DPTO']))
    ...
    //      $html.="<TD WIDTH='110' HEIGHT=25></TD>";
        ...
        if($_SESSION['REPORTES']['VARIABLE']=='formulacio_amb')
        {
            ...
            {
              case '1':
                $this->Image('images/logocliente.png',11,3,25);
