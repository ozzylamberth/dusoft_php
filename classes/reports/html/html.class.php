...
class html_reports_class
{
    ...
    function html_reports_class()
    {
        ...
        $this->dir_cache= GetVarConfigAplication('DIR_SIIS').GetVarConfigAplication('DirCache');
    ...
    //ESTO ES NUEVO/*-----------------------------------------*/
    /*INCLUSION DE IMPRESION PARA EPICRISIS*/
    //ESTO ES NUEVO/*-----------------------------------------*/
    
     function GetReportHTML_Epicrisis($ingreso,$opciones=array())
    	{
          if(!empty($opciones))
          {
               $this->opciones=$opciones;
          ...
          else
          {
               ...
               global $VISTA;
               if(!IncludeFile("classes/ResumenEpicrisis/ResumenEpicrisis.class.php"))
               {
                    $this->error = "No se pudo cargar el Resumen de Epicrisis";
                    $this->mensajeDeError = "El archivo de vistas 'classes/ResumenEpicrisis/ResumenEpicrisis.class.php' no existe.";
                    return false;
               }//se incluye la clase del buscador.....
               $rpt_file ="classes/ResumenEpicrisis/$VISTA/ResumenEpicrisis.$VISTA.php";
               if(!IncludeFile($rpt_file))
               {
                    $this->error = "No se pudo cargar el Modulo";
                    $this->mensajeDeError = "El archivo de vistas '" . $fileName . "' no existe.";
               ...
               $rpt_class="ResumenEpicrisis_$VISTA";
    ...
    function GetReportPDF($report_html='',$opciones=array())
    {
        ...
        $this->reporte_pdf_historia = $this->CrearReporteHistoria_PDF();
        if(!$this->reporte_pdf)
        {
            return false;
        }
    ...
    function GetReportHistoria_PDF($report_html='',$opciones=array())
    {
        if(!empty($opciones))
        {
            $this->opciones=$opciones;
        ...
        $this->reporte_pdf = $this->CrearReporteHistoria_PDF();
        if(!$this->reporte_pdf)
        {
            return false;
        }
    ...
    function CrearReportePDF()
    {
        ...
        $pdf->setTmpDir("".$ConfigAplication['DIR_SIIS']."cache");
				$result = $pdf->convert();
    ...
    function CrearReporteHistoria_PDF()
    {
		
        $file = "classes/html2fpdf/html2fpdf.php";
        if(!IncludeFile($file))
        {

            $this->error = "Error al incluir la libreria html2fpdf.php";
            $this->mensajeDeError = "Archivo ".$file;
            return false;        
        } 
        ...
        $pdf = new HTML2FPDF();
        $pdf->AddPage();
        $pdf->WriteHTML($this->rpt_HTML);
        $pdf->Output($archivo_report,'F');
        return $archivo_report;
    }
    ...
    function CrearReporteHTML()
    {
        ...
        $archivo_report = str_replace(GetVarConfigAplication('DIR_SIIS'),$this->domain,$archivo_report);
        $archivo_report = str_replace("\\","/",$archivo_report);
    ...
    function GetMembreteGenerico($datos_membrete=array())
    {
        ...
        if($datos_membrete['logo'] && file_exists($file_logo))
        {
            if(strtolower($datos_membrete['align'])=='left')
            {
                ...
                if(empty($datos_membrete['width']))
                {
                    $datos_membrete['width']='&nbsp;';
                }
                else
                {
                    $width=$datos_membrete['width'];
                }
                if(empty($datos_membrete['height']))
                {
                    $datos_membrete['height']='&nbsp;';
                }
                else
                {
                    $height=$datos_membrete['height'];
                }

                $HEADER.="<TR>";
                if($width AND $height)
                {
                  $HEADER.="<TD rowspan='2'><img src='$file_logo' align='left' border=0 WIDTH='$width' HEIGHT='$height'></TD>\n";
                }
                else
                {
            ...
            else
            {
                ...
                $HEADER.="<TR>\n";
                $HEADER.="<TD align='center'><img src='$file_logo' border=0 WIDTH='$width' HEIGHT='$height'></TD>\n";
                $HEADER.="</TR>\n";
    ...
    */
    function Open_Tags_Html($title)
    {
      ...
      $HTML .= "    <link href=".$this->domain."classes/reports/style/style-print.css rel=stylesheet type=text/css media=\"print\">\n";
      $HTML .= "  </head>\n";
      $HTML .= "<body>\n";
      return $HTML;
    }
