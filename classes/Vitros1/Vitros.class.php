<?
/**
 * $Id: Vitros.class.php,v 1.27 2006/01/10 19:36:16 mauricio Exp $
 *
			...
			if($res[0]=='inserta')
				{
					$idv=$this->InsertaDatosVitros($id_muestra,$cod_vitros,$respuesta_muestra,$unidad_id,$error_medidas_id,
						...
						if($idv)
						{
							if($this->InsertaHcResultados($cod_vitros,$id_muestra)){
							return true;
							}else{return false;}
						}
						else
				...
				}elseif($res[0]=='modifica')
				{
					...
					if(($firma==NULL)||(empty($firma))) 
					{
						...
						if($idv)
						{
							$this->ActualizaHcResultados($id_muestra,$cod_vitros,$respuesta_muestra,$unidad_id);
 							return true;
						}
						else
						{
...
$query2=$query;
		...
		*/
		function ActualizaHcResultados($id_muestra,$cod_vitros,$respuesta_muestra,$unidad_id){
	...
	*/
	function ConsultaLabExamen1($cod_vitros,$cargo_cups,$tecnica_id){
		...
		*/
		function InsertaHcResultados($cod_vitros,$id_muestra,$tipo){
			...
			if(!empty($var))
			{
				...
				//echo "<br>resul->";
				//print_r($resul_id);
				//echo " -->   ".$_SESSION['LTATRABAJO']['nuevo_resultado_id'];
