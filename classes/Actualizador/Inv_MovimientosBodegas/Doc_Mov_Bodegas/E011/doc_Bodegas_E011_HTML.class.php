<?php
	/**************************************************************************************  
	* $Id: doc_Bodegas_E011_HTML.class.php,v 1.1 2009/07/17 19:08:17 johanna Exp $ 
	* 
...
include "app_modules/Inv_MovimientosBodegas/Doc_Mov_Bodegas/E011/doc_Bodegas_E011.class.php";
class doc_bodegas_E011_HTML
{
 function doc_bodegas_E011_HTML(){}
...
{    //var_dump($DATOS);
     ...
     $file ='app_modules/Inv_MovimientosBodegas/Doc_Mov_Bodegas/E011/RemoteXajax/definirBodegas_E011.php';
     $objeto->SetXajax(array("Subtimit","CrearDocumentoFinalx","BorrarTmpAfirmativo1","MostrarProductox","Borrar","BorrarAjuste","IngresoProductosTemporal",
							...
							"BuscarProductoFactura","RegistrarFactura","lotesProducto","RegistrarFacturaProveedor"),$file,"ISO-8859-1");
...
function FormaDocEditar($DATOS)
{
	  ...
	  //print_r($datox);
      $datos = $consulta->TraerDatos($DATOS['bodegas_doc_id']);
	 	  $InformacionTercero=$consulta->Tercero($DocTemporal_Auxiliar['tipo_id_tercero'],$DocTemporal_Auxiliar['tercero_id']);
      $ProductosFactura=$consulta->ProductosFactura($DocTemporal_Auxiliar['tipo_id_tercero'],$DocTemporal_Auxiliar['tercero_id'],$DocTemporal_Auxiliar['prefijo'],$DocTemporal_Auxiliar['numero'],$DocTemporal_Auxiliar['empresa_id']);
...
***********************************************************************************/   
  ...
  $salida .= "                    <tr class=\"modulo_table_list_title\">";
  $salida .= "                          <td colspan=\"6\" align=\"center\">PRODUCTOS QUE HA DEVUELTO EL CLIENTE</td>";
  $salida .= "                    ";
  $salida .= "                    </tr>";
  $salida .= "                  <tr>";
  $salida .= "                  <td colspan=\"\">";
  ...
  $salida .= "                      <td>CODIGO</td><td>NOMBRE</td><td>CANTIDAD DEVUELTA</td><td>LOTE</td><td>FECHA VENCIMIENTO</td><td>OP</td>";
  $salida .= "                  </tr>";
      $i=0;
      ...
      $salida .= "                 <tr class=\"modulo_list_claro\">";
      $salida .= "                    <td><input ReadOnly=\"true\" class=\"input-text\" type=\"text\" name=\"codigo_producto".$producto['id_inv_facturas_despacho_d']."\" id=\"codigo_producto".$producto['id_inv_facturas_despacho_d']."\" value=\"".$producto['codigo_producto']."\"></td>";
      $salida .= "                    <td>".$producto['descripcion_producto']."</td>";
      $salida .= "                    <td>".$producto['cantidad_devuelta']."</td>";
 
      if(($producto['cantidad']-$producto['cantidad_devuelta'])<=$Existencia[0]['existencia_actual'])
      $total = ($producto['cantidad_devuelta']);
      $salida .= "                    <td><input ReadOnly=\"true\" class=\"input-text\" type=\"text\" name=\"lote".$producto['id_inv_facturas_despacho_d']."\" id=\"lote".$producto['id_inv_facturas_despacho_d']."\" value=\"".$producto['lote']."\"></td>";
    
      $salida .= "                    <td><input ReadOnly=\"true\" class=\"input-text\" type=\"text\" name=\"fecha_vencimiento".$producto['id_inv_facturas_despacho_d']."\" ".$color." id=\"fecha_vencimiento".$producto['id_inv_facturas_despacho_d']."\" value=\"".$producto['fecha_vencimiento']."\"></td>";
  
     // $salida .= "                    <td><input title=\"Cantidad a Devolver de ".$producto['descripcion_producto']."\" onkeypress=\"return acceptNum(event);\"  size=\"4\" type=\"text\" class=\"input-text\" name=\"cantidad".$producto['id_inv_facturas_despacho_d']."\" id=\"cantidad".$producto['id_inv_facturas_despacho_d']."\"></td>";
      $salida .= "                    <td>";
      //$salida .= "                    ".$Existencia[0]['existencia_actual']."<input type=\"hidden\" name=\"total".$producto['id_inv_facturas_despacho_d']."\" id=\"total".$producto['id_inv_facturas_despacho_d']."\" value=\"".$total."\">";
      $salida .= "                    <input type=\"hidden\" name=\"registros\" id=\"registros\" value=\"".$i."\">";
      //$salida .= "                    <input type=\"checkbox\" id=\"".$i."\" name=\"".$i."\" value=\"".$producto['id_inv_facturas_despacho_d']."\" class=\"checkbox\">";
      
      /*
      * Para Cambiar el Icono de Despliegue
      */
      $salida .= "                          <div id=\"Link".$producto['id_inv_facturas_despacho_d']."\">";
      $salida .= "                        <a title='LOTES DISPONIBLES PARA: ".$producto['descripcion_producto']."' onclick=\"xajax_lotesProducto('".$bodegas_doc_id."','".$tmp_doc_id."','".$empresa_id."','".$DATOS['utility']."','".$DATOS['bodegax']."','".$producto['id_inv_facturas_despacho_d']."','".$producto['codigo_producto']."');\">\n";
      $salida .= "                          <sub><img src=\"".$path."/images/abajo.png\" border=\"0\" width=\"17\" height=\"17\"></sub>\n";
      $salida .= "                         </a>\n";
      $salida .= "                            </div>";
      /*
      * Fin Para Cambiar el Icono de Despliegue
      */
      
      $salida .= "                    </td>";
      $salida .= "                 </tr>";
      $salida .= "                <tr>";
      $salida .= "                <td colspan=\"10\" align=\"center\"><div class=\"label_error\" id=\"productos".$producto['id_inv_facturas_despacho_d']."\"></div><div class=\"label_error\" id=\"mensaje".$producto['id_inv_facturas_despacho_d']."\"></div></td>";
      $salida .= "                </tr>";
      $i++;
      }
  $salida .= "                    <tr>\n";
