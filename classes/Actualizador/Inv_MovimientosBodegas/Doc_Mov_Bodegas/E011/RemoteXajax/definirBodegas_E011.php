<?php
	/**************************************************************************************
	* $Id: definirBodegas_E011.php,v 1.1 2009/07/17 19:08:17 johanna Exp $ 
  ...
  $productos=$consulta->CrearDocumentoOriginal($bodegas_doc_id,$doc_tmp_id,UserGetUID());
    if(!empty($productos))
    {
          foreach($ProductosTemporal as $key=>$Valor)
            {
            $cons->CantidadesDevueltasFactura($DocTemporal_Auxiliar['numero'],$Valor['empresa_id'],$Valor['item_id_compras'],$Valor['cantidad']);
            }
...
function lotesProducto($bodegas_doc_id,$tmp_doc_id,$empresa_id,$centro_utilidad,$bodega,$id_inv_facturas_despacho_d,$codigo_producto)
{
  $objResponse = new xajaxResponse();
  $path = SessionGetVar("rutaImagenes");
  $consulta=new doc_bodegas_E011();
  $vector=$consulta->lotesproducto($empresa_id,$centro_utilidad,$bodega,$codigo_producto);
 ...
 if(!empty($vector))
 {
   ...
   foreach($vector as $valor=>$productos)
   {
          ...
          $salida .= "                      </td>\n";
          $salida .= "                      <td align=\"left\" class=\"label_mark\">\n";
          $salida .= "                         ".$productos['descripcion_producto'];
          $salida .= "                      </td>\n";
         ...
         */      
          ...
          $salida .= "                         ".$productos['existencia_actual'];
          $salida .= "                      </td>\n";
          
          $salida .= "                        <td>";
          $salida .= "                          <input type=\"text\" class=\"input-text\" name=\"\" id=\"\" onkeypress=\"return acceptNum(event);\" >";
          $salida .= "                         </td>";
 ...
 else
 {
   ...
   $salida .= "                  <tr>\n";
   $salida .= "                  <td align=\"center\">\n";
   $salida .= "                      <label class='label_error'> ESTE PRODUCTO, NO POSEE LOTES DISPONIBLES</label>";
   $salida .= "                  </td>\n";
   $salida .= "                  </tr>\n";
   $salida .= "                  </table>\n";
  // $objResponse->call("off");
 }
      $imagen = "                        <a title='LOTES DISPONIBLES PARA: ".$producto['descripcion_producto']."' onclick=\"lotesProducto('".$bodegas_doc_id."','".$tmp_doc_id."','".$empresa_id."','".$DATOS['utility']."','".$DATOS['bodegax']."','".$producto['id_inv_facturas_despacho_d']."','".$producto['codigo_producto']."');\">\n";
      $imagen .= "                          <sub><img src=\"".$path."/images/arriba.png\" border=\"0\" width=\"17\" height=\"17\"></sub>\n";
      $imagen .= "                         </a>\n";
  $objResponse->assign("Link".$id_inv_facturas_despacho_d,"innerHTML",$imagen);
  $objResponse->assign("productos".$id_inv_facturas_despacho_d,"innerHTML",$salida);
...
function IngresoProductosTemporal($bodegas_doc_id,$doc_tmp_id,$Formulario,$usuario_id)
{
        ...
        if($Formulario['cantidad'.$Formulario[$i]]<=0 || $Formulario['cantidad'.$Formulario[$i]]=="")
          ...
          $objResponse->assign("mensaje".$Formulario[$i],"innerHTML","La Cantidad No Es Correcta!!");
              ...
              if($Formulario['cantidad'.$Formulario[$i]]>$Formulario['total'.$Formulario[$i]])
                {
                $objResponse->assign("mensaje".$Formulario[$i],"innerHTML","La Cantidad No Puede Ser Mayor a la De la Factura!!");
                }
                else
                    {
                    ...
                    {
                    $Retorno=$consulta->GuardarTemporal($bodegas_doc_id,$doc_tmp_id,$Formulario['codigo_producto'.$Formulario[$i]], $Formulario['cantidad'.$Formulario[$i]], 0, 0, $usuario_id=null,$FechaVencimiento,$Formulario['lote'.$Formulario[$i]]);
                    //print_r($Retorno);
  ...
  //$sql = AutoCarga::factory("MovDocE011","classes","app","Inv_MovimientosBodegas");
...
function GuardarTmpDoc($bodegas_doc_id, $observacion, $tipo_id_tercero,$tercero_id, $factura)
{
    $objResponse = new xajaxResponse();
    $path = SessionGetVar("rutaImagenes");
    $consulta1=new doc_bodegas_E011();
...
function Listar_FacturasProveedor($tipo_id_tercero,$tercero_id)
{
      ...
      $select .= "<option value=\"\">---</option>";
        ...
        {
        $productos=$consulta->ProductosFactura($tipo_id_tercero,$tercero_id,$valor['prefijo'],$valor['numero'],$valor['empresa_id']);
        if(!empty($productos))
