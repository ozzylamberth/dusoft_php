...
class doc_bodegas_I012 {
    ...
    function ProductosFactura($tipo_id_tercero, $tercero_id, $prefijo, $numero, $empresa_id)
    {
        /*$sql = "SELECT 
         ...
         $sql = "SELECT a.item_id, a.empresa_id, a.prefijo, a.factura_fiscal, a.codigo_producto, fc_descripcion_producto(a.codigo_producto) as descripcion_producto,
                    a.cantidad, a.cantidad_devuelta, a.fecha_vencimiento, a.lote, a.valor_unitario, a.porc_iva, (a.valor_unitario*(a.porc_iva/100)) as iva,
                    (a.valor_unitario+(a.valor_unitario*(a.porc_iva/100))) as valor_unitario_iva, ((a.cantidad)*(a.valor_unitario+(a.valor_unitario*(a.porc_iva/100)))) as valor_total_iva, b.costo 
                    FROM
                    inv_facturas_despacho_d a 
                    ...
                    SELECT a.item_id, a.empresa_id, a.prefijo, a.factura_fiscal, a.codigo_producto, fc_descripcion_producto(a.codigo_producto) as descripcion_producto,
                    a.cantidad, a.cantidad_devuelta, a.fecha_vencimiento, a.lote, a.valor_unitario, a.porc_iva, (a.valor_unitario*(a.porc_iva/100)) as iva,
                    (a.valor_unitario+(a.valor_unitario*(a.porc_iva/100))) as valor_unitario_iva, ((a.cantidad)*(a.valor_unitario+(a.valor_unitario*(a.porc_iva/100)))) as valor_total_iva, b.costo 
                    FROM 
                    inv_facturas_agrupadas_despacho_d a 
                    JOIN inventarios as b ON (a.codigo_producto=b.codigo_producto) AND (a.empresa_id = b.empresa_id) 
                    WHERE a.prefijo = '" . $prefijo . "' and a.factura_fiscal = " . $numero . " and a.empresa_id = '" . $empresa_id . "' and a.cantidad <> a.cantidad_devuelta";
    ...
    function CantidadesDevueltasFactura($Numero, $prefijo, $Empresa, $ItemId, $cantidad)
    {
       ...
       /* $sql = " UPDATE inv_facturas_despacho_d ";
        ...
        $sql .= " UPDATE inv_facturas_agrupadas_despacho_d ";
        $sql .= " SET 
	          cantidad_devuelta = cantidad_devuelta + " . $cantidad . " ";
        $sql .= " Where ";
        $sql .= " 	  factura_fiscal = " . $Numero . "";
