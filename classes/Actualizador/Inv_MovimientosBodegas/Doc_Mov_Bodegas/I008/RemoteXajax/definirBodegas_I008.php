   ...
   //echo $tip_bus; 
	...
	if(!empty($datos))
	{
		...
		foreach($datos as $k => $valor)
		  ...
		  $salida .= "                                        <tr class=\"modulo_table_list_title\">\n";
	      $salida .= "                                        <td width=\"20%\" colspan=\"7\" align=\"center\">";
              ...
              else
              {
                ...
                if($int_nodias < $dias_vencimiento)
                {
                 ...
                 $total_costo = $Formulario['cantidad'.$i]*($Formulario['valor'.$i]);
                  ...
                  $alerta ="";
                  $ProductosDocumento = $objClass->ProductosDocumento($Formulario['empresa_id'],$Formulario['prefijo'],$Formulario['numero'],'','',$Formulario['bodegas_doc_id'],$Formulario['doc_tmp_id'],$Formulario['codigo_producto'.$i],$Formulario['lote'.$i],$fecha_comparar);
				   	
				  if(($ProductosDocumento[0]['cantidad']-$Formulario['cantidad'.$i])>=0)
                         {
						 $ItemId=$objClass->GuardarTemporal($Formulario['bodegas_doc_id'],$Formulario['doc_tmp_id'],$Formulario['codigo_producto'.$i],$Formulario['cantidad'.$i],$Formulario['porc_iva'.$i],$total_costo,UserGetUID(),$Formulario['fecha_vencimiento'.$i],$Formulario['lote'.$i]);
						 /*print_r("Hoooola");*/
						 }
                  if($ItemId)
					$h++;
