...
//             {var_dumP($productos);
        ...
        $resultado = $consulta->eliminar_productos_devolucion_bodega($doc_tmp_id, UserGetUID());
...
function BorrarTmpAfirmativo1($tmp, $bodega_doc_id) {
    ...
    $resultado = $consulta->eliminar_productos_devolucion_bodega($tmp, UserGetUID());
...
function IngresoProductosTemporal($bodegas_doc_id, $doc_tmp_id, $Formulario, $usuario_id) {
    ...
    for ($i = 0; $i < $Formulario['registros']; $i++) {
        $mensaje = "";
        if ($Formulario[$i] != "") {
            if ($Formulario['cantidad' . $Formulario[$i]] <= 0 || $Formulario['cantidad' . $Formulario[$i]] == "" || $Formulario['cantidad' . $Formulario[$i]] > $Formulario['cantidad_enviada' . $Formulario[$i]]) {
...
/* function HacerCambiarInfoDevolucion($bodegas_doc_id, $doc_tmp_id, $codigo_producto, $lote, $fecha_vencimiento, $cantidad, $lote0, $fv0, $cant0, $lote1, $fv1, $cant1, $lote2, $fv2, $cant2, $lote3, $fv3, $cant3, $lote4, $fv4, $cant4) {

  $objResponse = new xajaxResponse();

  // Se dispara la operacion de eliminacion de los temporales
  $consulta = new doc_bodegas_I011();

  $consulta->HacerCambiarInfoDevolucion($doc_tmp_id, $codigo_producto, $lote, $fecha_vencimiento, $cantidad, $lote0, $fv0, $cant0, $lote1, $fv1, $cant1, $lote2, $fv2, $cant2, $lote3, $fv3, $cant3, $lote4, $fv4, $cant4);
...
function HacerCambiarInfoDevolucion($bodegas_doc_id, $doc_tmp_id, $producto_base, $productos_nuevos) {
