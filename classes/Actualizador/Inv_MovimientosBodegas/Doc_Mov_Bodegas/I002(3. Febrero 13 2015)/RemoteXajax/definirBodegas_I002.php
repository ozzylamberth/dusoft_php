...
function ListadoProductos($pagina, $orden, $tipo_param, $param, $doc_tmp_id, $bodegas_doc_id)
{
    ...
    { //Para habilitar/Deshabilitar aquellos que est�n por Autorizar
...
function ListadoProductos_HTML($datos, $pagina, $orden, $tipo_param, $param, $conteo, $datosItem, $doc_tmp_id, $bodegas_doc_id, $ProductosFOC)
{
    ...
    $salida .= "		<td width=\"3%\">CODIGO CUM</td>";
    ...
    if ($datos)
    {
        ...
        foreach ($datos as $key => $valor)
        {
            ...
            $salida .= "	<td><input style=\"width:100%\" type='text' class='input-text' name='codigocum" . $i . "' id='codigocum" . $i . "' value='" . $valor['codigo_cum'] . "' /></td>";
...
function AgregarItem($Formulario)
{
    ...
    for ($i = 0; $i < $Formulario['cantidad']; $i++)
    {
        ...
        if ($Formulario[$i] != "")
        {
            ...
            { //Valido que los Campos no est�n vac�os
                if ($Formulario['ucantidad' . $i] == "" || $Formulario['valor_unitario_factura' . $i] == "" || $Formulario['lote' . $i] == "" || $Formulario['localizacion' . $i] == "" || $Formulario['fecha_vencimiento' . $i] == "" || $Formulario['codigocum' . $i] == "")
                ...
                else
                {
                    ...
                    {//Valida que el producto no est� vencido
                        if ($fecha_dos <= $fecha_uno_act)
                        {
                        ...
                        else
                        {
                            ...
                            {//Ya ha pasado por todas las validaciones, solo queda hacer el Ingreso del Producto al Temporal                    
                                ...
                                if($objClass->modificarCodigoCum($Formulario['codigoproducto' . $i], $Formulario['codigocum' . $i])){
                                    $productos[] = array("codigo_producto" => $Formulario['codigoproducto' . $i],
                                        "codigocum" => $Formulario['codigocum' . $i]
                                    );
                                }
    ...
    //realiza sincronizacion
    ///print_r($productos);
   //$objClass->sincronizarCodigoCumProductos($productos);
