<?php
	/**************************************************************************************  
	* $Id: doc_Bodegas_T004_HTML.class.php,v 1.2 2010/07/07 15:40:25 hugo Exp $ 
	* 
...
class doc_Bodegas_T004_HTML
{
 function doc_Bodegas_T004_HTML(){}
...
*********************************************************************************/ 
    ...
    {    //var_dump($DATOS);
      ...
      $file = 'app_modules/Inv_MovimientosBodegas/Doc_Mov_Bodegas/T004/RemoteXajax/definirBodegas_T004.php';
...
function FormaDocNuevo($DATOS)
{
    ...
    */
    function PintarTabla($busqueda,$Temporal)
    {
      ...
      $salida .= "                    <tr class=\"modulo_table_list_title\">\n";
      $salida .= "                      <td align=\"center\" >\n";
      $salida .= "                        <a title='ESM'>TRASLADO A:</a>";
      $salida .= "                      </td>\n";
      $salida .= "                      <td align=\"left\" class=\"modulo_list_claro\" colspan=\"3\">\n";
      $salida .= "                        ".$Temporal['bodega_satelite'];
      $salida .= "                      </td>\n";
      $salida .= "                    </tr>\n";
      $salida .= "                    </table>\n";
      $salida .= "                    </form>";
...
function CrearDocumentosHtml($bodegas_doc_id,$dir,$tipo_doc_bodega_id,$nom_bodega,$utilidad,$bodega,$datos)
 {
    ...
    //print_r($datos);
    $consulta = new MovBodegasSQL();
    $Documentos=$consulta->Documentos_TrasladoDevolucion($datos['empresa_id']);
    $javaC = "<script>\n";
...
***********************************************************************************/   
  $select .= "<select name=\"documento\" id=\"documento\" class=\"select\">";
  $select .= "<option value=\"\">-- SELECCIONAR --</option>";
  foreach($Documentos as $key => $valor)
  {
  $dato= $valor['centro_utilidad']."@".$valor['bodega']."@".$valor['prefijo']."@".$valor['numero'];
  $documento= $valor['prefijo']."-".$valor['numero'].": ".$valor['bodega_'];
  $select .= "<option value=\"".$dato."\">".$documento."</option>";
  }
$select .= "</select>";
  $path = SessionGetVar("rutaImagenes");
  ...
  $salida .= "                      <b>DOCUMENTO DE DEVOLUCION:</b>";
  $salida .= "                    ".$select;
  $salida .= "                       </td>\n"; 
  $salida .= "                    </tr>\n";
  $salida .= "                    <tr>\n";
  $salida .= "                       <td  COLSPAN='2' align=\"center\" class=\"modulo_list_claro\">\n";                          //GrabarDocumento(bodegas_doc_id, observacion,centro,bodega)
  $salida .="<input type=\"button\" id=\"nuevo\" value=\"GRABAR DOCUMENTO\" class=\"input-bottom\" onClick=\"javascript:GrabarDocumento('".$bodegas_doc_id."',document.getElementById('obser').value,document.getElementById('documento').value,'".$utilidad."','".$bodega."');\">";//
  $salida .= "                       </td>\n"; 
  $salida .= "                    </tr>\n";
  $salida .= "                 </table>";
  $salida .= "              </form>";
      ...
      $salida .= "  <input id='empresa_id' type=\"hidden\" value=\"".SessionGetVar("EMPRESA")."\">\n";
      $salida .= " </form>\n";
  return $salida;
 }
    /**
...
***********************************************************************************/
      ...
      $salida .= "        <td>";
      //print_r($Temporal);
	  $salida .= "                          <input type=\"hidden\" id=\"prefijo\" name=\"prefijo\" value=\"".$Temporal['prefijo']."\">\n";
	  $salida .= "                          <input type=\"hidden\" id=\"numero\" name=\"numero\" value=\"".$Temporal['numero']."\">\n";
