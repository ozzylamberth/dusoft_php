<?php
	/**************************************************************************************
	* $Id: definirBodegas_T004.php,v 1.2 2010/07/07 15:40:25 hugo Exp $ 
	...
	**************************************************************************************/	
	...
	include "../../../app_modules/Inv_MovimientosBodegas/Doc_Mov_Bodegas/T004/doc_Bodegas_T004.class.php";
...
//             {var_dumP($productos);
                    ...
                    $salida .= "                      <td align=\"center\">\n";
                                                       $direccion="app_modules/Inv_MovimientosBodegas/Doc_Mov_Bodegas/T004/imprimir/imprimir_T004.php";
...
FUNCTION Borrar($tr,$item,$CONTENIDOR)
{
    ...
    for($i=0;$i<=$Formulario['registros'];$i++)
	{
		if($Formulario[$i]!="")
		{
			...
			if(($cantidad['total']+$Formulario['cantidad'.$i])<=$Formulario['cantidad_solicitada'])
			{
				...
				$objResponse->assign("".$Formulario['movimiento_id']."@".$Formulario['codigo_producto']."","innerHTML",$consulta->mensajeDeError);
				//print_r($Retorno);
				
				if($Retorno)
					$k++;
					$consulta->ItemEnMovimiento($Formulario['doc_tmp_id'],$Retorno,$Formulario['movimiento_id']);
			}
		}
    }
  ...
  function BuscarProducto1($FormularioBuscador)
  {
    ...
    $busqueda=$consulta->Consultar_DocumentoDetalle($FormularioBuscador);
    ...
    //$busqueda = $consulta->BuscarProducto($FormularioBuscador);
      ...
      {
      $cantidad = $consulta->Cantidad_ProductoTemporal($FormularioBuscador['doc_tmp_id'],$valor['codigo_producto'],$valor['movimiento_id']);
      
	  $salida .= "                 <form id=\"forma".$valor['movimiento_id']."@".$valor['codigo_producto']."\" name=\"".$valor['movimiento_id']."@".$valor['codigo_producto']."\" action=\"\" method=\"post\">\n";
      $salida .= "                  <table width=\"100%\" align=\"center\" class=\"modulo_table_list\">\n";
      $salida .= "                    <tr class=\"modulo_table_list_title\">\n";
      ...
      $salida .= "                        <input type=\"hidden\" name=\"movimiento_id\" id=\"movimiento_id\" value=\"".$valor['movimiento_id']."\">";
      $salida .= "                        <input type=\"hidden\" name=\"codigo_producto\" id=\"codigo_producto\" value=\"".$valor['codigo_producto']."\">";
      $salida .= "                        <input type=\"hidden\" name=\"valor\" id=\"valor\" value=\"".($valor['total_costo']/$valor['cantidad'])."\">";
      $salida .= "                        <input type=\"hidden\" name=\"porc_iva\" id=\"porc_iva\" value=\"".$valor['porcentaje_gravamen']."\">";
      $salida .= "                      </td>";
      $salida .= "                    </tr>\n";
	 ...
	 $fech_vencmodulo = ModuloGetVar('app','AdminFarmacia','dias_vencimiento_product_bodega_farmacia_'.SessionGetVar("EMPRESA"));
      ...
      $salida .= "                                            </td>";
      $salida .= "                                            <td>";
      $salida .= "                                              <input style=\"width:100%\" type=\"text\" class=\"input-text\" name=\"cantidad".$i."\" id=\"cantidad".$valor['movimiento_id']."@".$valor['codigo_producto']."".$i."\" onkeypress=\"return acceptNum(event);\" onkeyup=\"ValidarCantidad('cantidad".$valor['movimiento_id']."@".$valor['codigo_producto']."".$i."',xGetElementById('cantidad".$valor['movimiento_id']."@".$valor['codigo_producto']."".$i."').value,'".$v['existencia_actual']."','hell$i');\">";
      $salida .= "                                            </td>";
      $salida .= "                                            <td>";
      if($vencido!=1)
	  ...
	  $salida .= "													<div class=\"label_error\" id=\"".$valor['movimiento_id']."@".$valor['codigo_producto']."\"></div>";
	  $salida .= "                                            </td>";
      $salida .= "                                        </tr>";
	  $salida .= "                                    </table>\n";
      $salida .="                       </td>";
	  ...
	  $salida .= "                                              <input class=\"input-submit\" type=\"button\" value=\"GUARDAR TEMPORAL\" onclick=\"xajax_GuardarPT(xajax.getFormValues('forma".$valor['movimiento_id']."@".$valor['codigo_producto']."'));\">";
      $salida .= "                                        </td>";
      $salida .= "                                        </tr>\n";
...
function GuardarTmpDoc($bodegas_doc_id, $observacion,$documento,$centro_utilidad,$bodega)
{
    $objResponse = new xajaxResponse();
    $path = SessionGetVar("rutaImagenes");
    $consulta=new MovBodegasSQL();
    // print_r($OrdenRequisicion);
    $consulta1=new doc_Bodegas_T004();
    $valor=$consulta1->CrearDoc($bodegas_doc_id, $observacion,$centro_utilidad,$bodega,$documento);
    //print_r($valor);
    ...
    $objResponse->call("mar1");
      ...
      {
      $objResponse->assign("errorcreartmp","innerHTML","Error Al Crear el Documento Temporal (Revisar si el documento se Encuentra en Un Temporal)!!".$consulta1->mensajeDeError);  
