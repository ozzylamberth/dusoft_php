<?php
	/**************************************************************************************
	* $Id: definirBodegas_E004.php,v 1.1 2009/07/17 19:08:17 johanna Exp $ 
	...
	**************************************************************************************/	
	...
	include "../../../app_modules/Inv_MovimientosBodegas/Doc_Mov_Bodegas/E004/doc_Bodegas_E004.class.php";
...
{   $path = SessionGetVar("rutaImagenes");
    ...
    //$consulta1 =new definirBodegas_E004();
    //$Tmp_inv_movimientos=    
   ...
   //echo $tip_bus; 
	...
	$salida .= "<center><label class=\"label_error\">Resultado(s) Encontrado(s) (".sizeof($datos).")</label></center>";
		foreach($datos as $k => $valor)
	      {
		  $salida .= "                 <form id=\"forma".$valor['codigo_producto']."\" name=\"".$valor['codigo_producto']."\" action=\"\" method=\"post\">\n";
	      $salida .= "                  <table width=\"100%\" align=\"center\" class=\"modulo_table_list\">\n";
	      $salida .= "                    <tr class=\"modulo_table_list_title\">\n";
	      $salida .= "                      <td width=\"50%\" colspan=\"2\">PRODUCTO: ".$valor['codigo_producto']." ".$valor['descripcion'].". </td><td>CANTIDAD PENDIENTE <input readonly=\"true\" type=\"input-text\" name=\"cantidad_pendiente\" id=\"cantidad_pendiente\" value=\"".FormatoValor($valor['cantidad'],0)."\" class=\"input-text\"></td>\n";
	      $salida .= "                        <input type=\"hidden\" name=\"empresa_id\" id=\"empresa_id\" value=\"".$empresa_id."\">";
	      ...
	      $salida .= "                        <input type=\"hidden\" name=\"valor\" id=\"valor\" value=\"".$valor['costo']."\">";
	      ...
	      $i=0;
			...
			$salida .= "                                        <tr class=\"modulo_list_claro\">";
			$salida .= "                                            <td>";
			$salida .= "                                              <input style=\"width:100%\" type=\"text\" readonly=\"text\" class=\"input-text\" value=\"".$v['lote']."\" name=\"lote".$i."\" id=\"lote".$v['codigo_producto']."".$i."\" >";
			$salida .= "                                            </td>";
			$salida .= "                                            <td>";
			...
			$salida .= "                                              <input ".$color."  type=\"text\" readonly=\"text\" class=\"input-text\" value=\"".$fechavencimiento."\" name=\"fecha_vencimiento".$i."\" id=\"fecha_vencimiento".$v['codigo_producto']."".$i."\" >";
			$salida .= "                                            </td>";
			$salida .= "                                            <td>";
			...
			$salida .= "                                            </td>";
			$salida .= "                                            <td>";
			$salida .= "                                              <input ".$v['bloqueo']." style=\"width:100%\" type=\"text\" class=\"input-text\" name=\"cantidad".$i."\" id=\"cantidad".$v['codigo_producto']."".$i."\" onkeypress=\"return acceptNum(event);\" onkeyup=\"ValidarCantidad('cantidad".$v['codigo_producto']."".$i."',xGetElementById('cantidad".$v['codigo_producto']."".$i."').value,'".$v['existencia_actual']."','hell$i','".$v['codigo_producto']."".$i."');\">";
			$salida .= "                                            </td>";
			$salida .= "                                            <td>";
			if($vencido!=1)
			$salida .= "                                              <input ".$v['bloqueo']." style=\"width:100%\" disabled type=\"checkbox\" class=\"input-text\" name=\"".$i."\" id=\"".$v['codigo_producto']."".$i."\" value=\"".$v['codigo_producto']."\" >";
		  ...
		  $salida .= "                                              <input class=\"input-submit\" type=\"button\" value=\"GUARDAR TEMPORAL\" onclick=\"xajax_AgregarItem(xajax.getFormValues('forma".$valor['codigo_producto']."'));\">";
	      $salida .= "                                        </td>";
	      $salida .= "                                        </tr>\n";
    ...
    for($i=0;$i<=$Formulario['registros'];$i++)
    {
      ...
      if($Formulario[$i]!="")
      {
        ...
        $datosItem = $objClass->ConsultarItemsExistencias($Formulario[$i],SessionGetVar('empresa_id'),SessionGetVar('centro_utilidad'),SessionGetVar('bodega'));
          ...
          else
          {
              ...
              else
              {
                ...
                if($int_nodias < $dias_vencimiento)
                {
                 ...
                 $total_costo = $Formulario['cantidad'.$i]*($Formulario['valor']);
                  
                  $alerta ="";
                  $ProductosDocumento = $objClass->ProductosDocumento($Formulario['empresa_id'],$Formulario['prefijo'],$Formulario['numero'],'','',$Formulario['bodegas_doc_id'],$Formulario['doc_tmp_id'],$Formulario[$i]);
				   	
				  if(($ProductosDocumento[0]['cantidad']-$Formulario['cantidad'.$i])>=0)
                         {
						 
						 $ItemId=$objClass->GuardarTemporal($Formulario['bodegas_doc_id'],$Formulario['doc_tmp_id'],$Formulario[$i],$Formulario['cantidad'.$i],$Formulario['porc_iva'],$total_costo,UserGetUID(),$Formulario['fecha_vencimiento'.$i],$Formulario['lote'.$i]);
						 }
                  if($ItemId)
					$h++;
				}
...
//GuardarTmpDoc(bodegas_doc_id, observacion, tipo_id_tercero, tercero_id, prestamo)
function GuardarTmpDoc($bodegas_doc_id, $observacion,$tipo_id_tercero,$tercero_id,$doc_devolucion)
{
    $objResponse = new xajaxResponse();
    $path = SessionGetVar("rutaImagenes");
    $consulta=new MovBodegasSQL();
    $consulta1=new doc_bodegas_E004();
    $valor=$consulta1->CrearDoc($bodegas_doc_id, $observacion,$tipo_id_tercero,$tercero_id,$doc_devolucion);
...
*********************************************************************************/
function Buscadorter($pagina,$criterio1,$criterio2,$criterio,$div,$Forma,$tipo_doc_bodega_id)
{ //echo "si";
      $objResponse = new xajaxResponse(); 
...
//       $objResponse->alert($Forma);
      ...
      if($criterio=="")
      $criterio="0";
      $vector=$consulta->Terceros($pagina,$criterio1,$criterio2,$criterio,$op=2,SessionGetVar("EMPRESA"));
      $salida .= "                  <div id=\"ventana_terceros\">\n";
      ...
      $salida .= "                        <input type=\"button\" class=\"input-submit\" name=\"boton_bus\"  id=\"boton_bus\" value=\"BUSCAR\" onclick=\"Bus_ter(document.getElementById('buscar_x').value,document.getElementById('buscar').value,document.getElementById('nom_buscar').value,'".$div."','".$Forma."','".$tipo_doc_bodega_id."','1')\">\n";
      $salida .= "                       </td>\n";
      $salida .= "                       </tr>\n";
...
//         $salida .="                         <td>\n";
   ...
   {
      $op="1";
      $action = "Bus_ter('".$criterio1."','".$criterio2."','".$criterio."','".$div."','".$Forma."','".$tipo_doc_bodega_id."'";
      $ctl = AutoCarga::factory("ClaseHTML");
      $salida .= $ctl->ObtenerPaginadoXajax($consulta->conteo,$consulta->paginaActual,$action,"0",10);
...
//             $salida .= "                      </td>\n";
            ...
            $salida .= "                      </td>\n";
            $salida .= "                      <td align=\"center\">\n";
            $java = "javascript:Seleccionado('".$Forma."','".$vector[$i]['tipo_id_tercero']."','".strtoupper($vector[$i]['tercero_id'])."','".$vector[$i]['nombre_tercero']."','".$tipo_doc_bodega_id."');";
