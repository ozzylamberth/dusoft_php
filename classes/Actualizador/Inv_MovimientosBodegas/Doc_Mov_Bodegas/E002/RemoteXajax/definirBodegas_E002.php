<?php
	/**************************************************************************************
	* $Id: definirBodegas_E002.php,v 1.1 2009/07/17 19:08:17 johanna Exp $ 
...
{   
	...
	if($tipo_prestamo_id=='06')
			{
			$OrdenPedidoId=$sql->BuscarFacturaProveedor($empresa_id,$centro_utilidad,$bodega,$CodigoProveedorId,$num_factura);
			}
			
	//Para Guardar las cantidades devueltas por producto en las Facturas de Despacho
			if($tipo_prestamo_id=='07')
			{
					foreach ($ProductosTemporal as $k=>$pt)
					{																																		
					$sql->CantidadesDevueltasFacturasDespacho($DocTemporal['prefijo_factura'],$DocTemporal['numero_factura'],$pt['codigo_producto'],$pt['lote'],$pt['cantidad'],$pt['lote_devuelto']);
					}
			}
    ...
    if(!empty($productos))
    {
			//Para Guardar las cantidades devueltas por producto en las Ordenes de Compra
			if($tipo_prestamo_id=='06')
			{
					foreach ($ProductosTemporal as $k=>$pt)
					{
					$sql->CantidadesDevueltasProductosOC($num_factura,$pt['codigo_producto'],$pt['lote'],$pt['cantidad']);
					}
			}
			...
			if($tipo_prestamo_id=='07')
			{
					foreach ($ProductosTemporal as $k=>$pt)
					{																																		
					$sql->CantidadesDevueltasFacturasDespacho($DocTemporal[0]['prefijo_factura'],$DocTemporal[0]['numero_factura'],$pt['codigo_producto'],$pt['lote'],$pt['cantidad'],$pt['lote_devuelto']);
					}
			}
...
function GuardarPT($bodegas_doc_id,$doc_tmp_id,$codigo_producto,$cantidad,$porcentaje_gravamen,$total_costo,$usuario_id,$fecha_venc,$lotec,$lote_devuelto)
{
  ...
  //print_r($Retorno);
  /*
  * Nuevo Desarrollo... Este permite relacionar los productos que se están cambiando mano a mano
  */
  $sql = AutoCarga::factory("MovDocE002","classes","app","Inv_MovimientosBodegas");
...
function GuardarTmpDoc($bodegas_doc_id, $observacion, $tipo_id_tercero, $tercero_id, $prestamo)
{
    ...
    $path = SessionGetVar("rutaImagenes");
    $consulta=new MovBodegasSQL();
    $consulta1=new doc_bodegas_E002();
