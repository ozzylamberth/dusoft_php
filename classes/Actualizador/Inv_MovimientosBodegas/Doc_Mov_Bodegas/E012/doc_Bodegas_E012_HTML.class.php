<?php
	/**************************************************************************************  
	* $Id: doc_Bodegas_E012_HTML.class.php,v 1.1 2009/07/17 19:08:17 johanna Exp $ 
	* 
...
class doc_bodegas_E012_HTML
{
 function doc_bodegas_E012_HTML(){}
...
{    //var_dump($DATOS);
     ...
     $file ='app_modules/Inv_MovimientosBodegas/Doc_Mov_Bodegas/E012/RemoteXajax/definirBodegas_E012.php';
...
function FormaDocEditar($DATOS)
{
		...
		//print_r($datox);
		$datos = $consulta->TraerDatos($DATOS['bodegas_doc_id']);
		$InformacionTercero=$consulta->CodigoProveedor($DocTemporal_Auxiliar['codigo_proveedor_id']);
		$ProductosFactura=$consulta->ProductosFactura($DocTemporal_Auxiliar['codigo_proveedor_id'],$DocTemporal_Auxiliar['numero_factura']);
...
***********************************************************************************/   
  ...
  $java = "javascript:MostrarCapa('ContenedorMov1');BuscarProveedor('ContenidoMov1','unocreate');Iniciar2('BUSCAR PROVEEDOR');\"";//
  $salida .= "                          <a  title=\"SELECIONAR PROVEEDOR\" class=\"label_error\" href=\"".$java."\"> BUSCAR PROVEEDOR</a>\n";
  $salida .= "                        </td>";
  $salida .= "                    </tr>\n";
  $salida .= "                    <tr class=\"modulo_list_claro\">";
  ...
  $salida .="<input type=\"button\" id=\"nuevo\" value=\"GRABAR DOCUMENTO\" class=\"input-bottom\" onClick=\"javascript:GrabarDocumento('".$bodegas_doc_id."',document.getElementById('obser').value,document.getElementById('codigo_proveedor_id').value,document.getElementById('factura').value);\">";//
  $salida .= "                       </td>\n"; 
  $salida .= "                    </tr>\n";
  $salida .= "                 </table>";
  $salida .= "              </form>";
...
**********************************************************************************/
   ...
   // print_r($InformacionTercero);
      ...
      $politicas .= "                     POLITICAS DEL PROVEEDOR :".$InformacionTercero[0]['tipo_id_tercero']."-".$InformacionTercero[0]['tercero_id']."::-".$InformacionTercero[0]['nombre_tercero'];
      $politicas .= "                     </td>";
      $politicas .= "                     </tr>";
      $i=1;
...
***********************************************************************************/   
  ...
  $salida .= "                  <tr class=\"modulo_table_list_title\">";
  $salida .= "                      <td>CODIGO</td><td>NOMBRE</td><td>CANTIDAD</td><td>EXISTENCIA</td><td>LOTE</td><td>FECHA VENCIMIENTO</td><td>%IVA</td><td>VALOR</td><td>DEV</td><td>OP</td>";
  $salida .= "                  </tr>";
      $i=0;
      ...
      $porc_iva = ($producto['porc_iva']/100)+1;
      $valor_unitario = $producto['valor']/$porc_iva;
      $salida .= "                 <tr class=\"modulo_list_claro\">";
      $salida .= "                    <td><input ReadOnly=\"true\" class=\"input-text\" type=\"text\" name=\"codigo_producto".$producto['item_id']."\" id=\"codigo_producto".$producto['item_id']."\" value=\"".$producto['codigo_producto']."\"></td>";
      $salida .= "                    <td>".$producto['descripcion_producto']."</td>";
      $salida .= "                    <td>".($producto['cantidad']-$producto['cantidad_devuelta'])."</td>";
      
      $Existencia=$sql->ProductosExistencias($empresa_id,$DATOS['utility'],$DATOS['bodegax'],$producto['codigo_producto'],$producto['lote'],$producto['fecha_vencimiento']);
      
      if(($producto['cantidad']-$producto['cantidad_devuelta'])>$Existencia[0]['existencia_actual'])
      $total = $Existencia[0]['existencia_actual'];
      if(($producto['cantidad']-$producto['cantidad_devuelta'])<=$Existencia[0]['existencia_actual'])
      $total = ($producto['cantidad']-$producto['cantidad_devuelta']);
      $salida .= "                    <td>".$Existencia[0]['existencia_actual']."<input type=\"hidden\" name=\"total".$producto['item_id']."\" id=\"total".$producto['item_id']."\" value=\"".$total."\"></td>";
      $salida .= "                    <td><input ReadOnly=\"true\" class=\"input-text\" type=\"text\" name=\"lote".$producto['item_id']."\" id=\"lote".$producto['item_id']."\" value=\"".$producto['lote']."\"></td>";
      //$salida .= "                    <td>".$producto['lote']."</td>";
      $salida .= "                    <td><input ReadOnly=\"true\" class=\"input-text\" type=\"text\" name=\"fecha_vencimiento".$producto['item_id']."\" ".$color." id=\"fecha_vencimiento".$producto['item_id']."\" value=\"".$producto['fecha_vencimiento']."\"></td>";
      //$salida .= "                    <td>".$producto['fecha_vencimiento']."</td>";
      $salida .= "                    <td><input ReadOnly=\"true\" class=\"input-text\" type=\"text\" size=\"4\" name=\"porc_iva".$producto['item_id']."\" id=\"porc_iva".$producto['item_id']."\" value=\"".$producto['porc_iva']."\"></td>";
      //$salida .= "                    <td>".$producto['porc_iva']."</td>";
      $salida .= "                    <td>$<input ReadOnly=\"true\" class=\"input-text\" type=\"text\" size=\"15\" name=\"valor".$producto['item_id']."\" id=\"valor".$producto['item_id']."\" value=\"".$valor_unitario."\"></td>";
      //$salida .= "                    <td>".$producto['valor']."</td>";
      $salida .= "                    <td><input title=\"Cantidad a Devolver de ".$producto['descripcion_producto']."\" onkeypress=\"return acceptNum(event);\"  size=\"4\" type=\"text\" class=\"input-text\" name=\"cantidad".$producto['item_id']."\" id=\"cantidad".$producto['item_id']."\"></td>";
      $salida .= "                    <td>";
      ...
      $salida .= "                 </tr>";
      $salida .= "                <tr>";
      $salida .= "                <td colspan=\"10\" align=\"center\"><div class=\"label_error\" id=\"mensaje".$producto['item_id']."\"></div></td>";
      $salida .= "                </tr>";
      $i++;
      }
  $salida .= "                    <tr>\n";
