<?php
	/**************************************************************************************  
	* $Id: doc_Bodegas_E001_HTML.class.php,v 1.1 2009/07/17 19:08:17 johanna Exp $ 
	* 
...
class doc_Bodegas_E001_HTML
{
 function doc_Bodegas_E001_HTML(){}
...
{    //var_dump($DATOS);
     ...
     $file = 'app_modules/Inv_MovimientosBodegas/Doc_Mov_Bodegas/E001/RemoteXajax/definirBodegas_E001.php';
     $objeto->SetXajax(array("Subtimit","PintarBodegas",
							 "CrearDocumentoFinalx","BorrarTmpAfirmativo1",
							 "MostrarProductox","Borrar","BorrarAjuste","GuardarPT",
							 "BuscarProducto1","ObtenerPaginadoPro",
							 "GuardarTmpDoc","Cuadrar_ids_terceros",
							 "CrearUSA","Buscadorter","ObtenerPaginadoter",
							 "Actualizartmp","Devolver","GuardarDevolucion","RecargarProductosTmp"),$file);
...
***********************************************************************************/   
  ...
  $salida .= "                    TIPO DE PERDIDA";
  $perdida=$consulta->Perdidas();
    if(!empty($perdida))
    {
          $salida .= "                       <select id=\"perd\" name=\"perd\" class=\"select\">";
          $salida .= "                           <option value=\"0\" SELECTED>SELECCIONAR</option>\n"; 
          //var_dump($utility);
          for($i=0;$i<count($perdida);$i++)
          {
            $salida .= "                      <option value=\"".$perdida[$i]['tipo_perdida_id']."\">".$perdida[$i]['descripcion']."</option>\n";
          }
          $salida .= "                       </select>\n";
    }
  $salida .= "                       </td>\n";
  ...
  $salida .= "                          <input type=\"button\" id=\"nuevo\" value=\"GRABAR DOCUMENTO\" class=\"input-bottom\" onClick=\"javascript:GrabarDocumento('".$bodegas_doc_id."',document.getElementById('obser').value,document.getElementById('perd').value);\">";//
  $salida .= "                       </td>\n"; 
  $salida .= "                    </tr>\n";
  $salida .= "                 </table>";
  $salida .= "              </form>";
...
function ColocarProductos($bodegas_doc_id,$datos,$tmp_doc_id)
 {
    ...
    <script language=\"javaScript\">
      function mOvr(src,clrOver) 
				...
				xajax_RecargarProductosTmp('".$bodegas_doc_id."','".$tmp_doc_id."','".UserGetUID()."');
...
**********************************************************************************/
    ...
    /****************************************************************************/
            ...
            $salida .= "                       <select id=\"tip_bus\" name=\"tip_bus\" class=\"select\" onchange=\"onchange=\"document.jukilo.criterio.value =''\">";
