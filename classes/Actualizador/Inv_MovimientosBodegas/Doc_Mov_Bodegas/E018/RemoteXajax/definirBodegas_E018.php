<?php
	/**************************************************************************************
	* $Id: definirBodegas_E018.php,v 1.1 2009/07/17 19:08:17 mauricio Exp $ 
...
{   
   ...
   /*print_r($consulta->mensajeDeError);                    */
    if(!empty($productos))
    {
...
//             {var_dumP($productos);
                    ...
                    $salida .= "                      <td align=\"center\">\n";
                                                      $direccion="app_modules/Inv_MovimientosBodegas/Doc_Mov_Bodegas/E018/imprimir/imprimir_docE018.php";
 ...
 if(!empty($vector))
 {
   ...
   foreach($vector as $valor=>$productos)
   {
         ...
         */      
          ...
          $salida .= "                         ".FormatoValor($productos['cantidad']);
          $salida .= "                      </td>\n";
          $salida .= "                      <td align=\"center\">\n";
...
function BorrarAjuste($tr,$item)
{
     ...
     // print_r($ProductoTemporal);
      ...
      if($buscar==1)
      {
        ...
        $objResponse->script("xajax_ListadoProductos_Farmacia_d(document.getElementById('bodegas_doc_id').value,document.getElementById('tmp_doc_id').value,document.getElementById('empresa_id').value,document.getElementById('centro_utilidad').value,document.getElementById('bodega').value,document.getElementById('codigo_barras').value,document.getElementById('descripcion').value);");
        $objResponse->script($script);
    ...
    if($Formulario[$i]!="")
        {
                    ...
                    */
                    ...
                    {
                    $objResponse->assign("mensaje".$i,"innerHTML",$mensaje);
                    $Retorno=$consulta->GuardarTemporal($bodegas_doc_id,$doc_tmp_id,$Formulario['codigo_producto'.$i], $Formulario['cantidad'.$i], $Formulario['porc_iva'.$i],($Formulario['cantidad'.$i]*$Formulario['valor'.$i]), $usuario_id=null,$FechaVencimiento,$Formulario['lote'.$i]);
...
function ListadoProductos_Farmacia_d($bodegas_doc_id,$tmp_doc_id,$empresa_id,$centro_utilidad,$bodega,$codigo_barras,$descripcion,$offset)
{                       
    $objResponse = new xajaxResponse();
    $path = SessionGetVar("rutaImagenes");
    $sql=new MovBodegasSQL();
    $consulta=new doc_bodegas_E018();
      ...
      /*$Cont=$consulta->ContarProStip($empresa_id,$centro_utilidad,$bodega,$codigo_barras,$descripcion,$bodegas_doc_id,$tmp_doc_id);    */
      $Productos = $consulta ->ProductosDocumento($empresa_id,$centro_utilidad,$bodega,$codigo_barras,$descripcion,$bodegas_doc_id,$tmp_doc_id,$offset);
      $action = "Bus_Pro('".$bodegas_doc_id."','".$tmp_doc_id."','".$empresa_id."','".$centro_utilidad."','".$bodega."','".$codigo_barras."','".$descripcion."'";
      $ctl = AutoCarga::factory("ClaseHTML");
      $salida .= $ctl->ObtenerPaginadoXajax($consulta->conteo,$consulta->paginaActual,$action,"0",10); 
      
      
      $salida .= "                 <form name=\"ProductosEmpresa\" id=\"ProductosEmpresa\"  method=\"post\">\n";
      $salida .= "                 <table width=\"95%\" align=\"center\" class=\"modulo_table_list\">\n";
      $salida .= "                  <tr class=\"modulo_table_list_title\">";
      $salida .= "                      <td>CODIGO</td><td>NOMBRE</td><td>CANTIDAD</td><td>LOTE</td><td>FECHA VENCIMIENTO</td><td>CANT. DESPACHAR</td><td>OP</td>";
      $salida .= "                  </tr>";
      ...
      $salida .= "                    <td>".FormatoValor($producto['existencia_actual'])."</td>";
      $salida .= "                    <td><input ReadOnly=\"true\" class=\"input-text\" type=\"text\" name=\"lote".$i."\" id=\"lote".$i."\" value=\"".trim($producto['lote'])."\"></td>";
      ...
      $salida .= "                    <td><input title=\"Cantidad a Ingresar de ".$producto['descripcion']."\" onkeypress=\"return acceptNum(event);\" style=\"width:100%\" type=\"text\" class=\"input-text\" name=\"cantidad".$i."\" id=\"cantidad".$i."\" onkeyup=\"ValidarCantidad('cantidad".$i."',xGetElementById('cantidad".$i."').value,'".$producto['existencia_actual']."','hell$i');\" ></td>";
      $salida .= "                    <td>";
      $salida .= "                    <input type=\"hidden\" name=\"porc_iva".$i."\" id=\"porc_iva".$i."\" value=\"".$producto['porc_iva']."\">";
      $salida .= "                    <input type=\"hidden\" name=\"valor".$i."\" id=\"valor".$i."\" value=\"".$producto['costo']."\">";
      $salida .= "                    <input type=\"hidden\" name=\"registros\" id=\"registros\" value=\"".$i."\">";
      $salida .= "                    <input type=\"checkbox\" id=\"".$i."\" name=\"".$i."\" value=\"".$producto['codigo_producto']."\" class=\"checkbox\">";
      $salida .= "                    </td>";
      $salida .= "                 </tr>";
      $salida .= "                 <tr class=\"label_error\" id=\"mensaje".$i."\">";
      $salida .= "                 </tr>";
      $i++;
      }
  $salida .= "                    <tr>\n";
  ...
  $salida .= "                          <input type=\"hidden\" name=\"registros\" id=\"registros\" value=\"".$i."\">";
  $salida .= "                          <input type=\"button\" id=\"nuevo\" value=\"SELECCIONAR PRODUCTO\" class=\"input-bottom\" onClick=\"xajax_IngresoProductosTemporal('".$bodegas_doc_id."','".$tmp_doc_id."',xajax.getFormValues('ProductosEmpresa'),'".UserGetUID()."');\">";
  $salida .= "                       </td>\n"; 
  $salida .= "                    </tr>\n";
  $salida .= "                 </table>";
  $salida .= "                </form>\n";
...
function GuardarTmpDoc($bodegas_doc_id, $observacion, $plan_id,$tipo_formula_id,$requisicion)
{
    $objResponse = new xajaxResponse();
    $path = SessionGetVar("rutaImagenes");
    $consulta1=new doc_bodegas_E018();
    
    $valor=$consulta1->CrearDoc($bodegas_doc_id, $observacion, $plan_id,$tipo_formula_id,$requisicion);
...
function BuscadorFarmacia($div,$Forma)
{
      $objResponse = new xajaxResponse(); 
      $path = SessionGetVar("rutaImagenes");
      $sql = AutoCarga::factory("MovDocE018","classes","app","Inv_MovimientosBodegas");
...
function BuscadorFarmacia_d($empresa_id,$CentroUtilidad,$Bodega,$CodigoEmpresa,$RazonSocial,$offset)
{
      ...
      $salida .= "                    </tr>\n";
        ...
        {
        $salida .= "<tr class=\"modulo_list_claro\" >";
        $salida .= "<td>".$valor['farmacia']."</td>";
        $salida .= "      <td align=\"center\">\n";
        //formulario,tipo_id_tercero,tercero_id,nombre_tercero
        $salida .= "        <a href=\"#\" onclick=\"Seleccionado('unocreate','".$valor['codigo']."','".$valor['farmacia']."','".SessionGetVar("EMPRESA")."','".trim($CentroUtilidad)."','".trim($Bodega)."')\">\n";
