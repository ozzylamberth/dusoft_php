...
/**
* $Id: LiquidacionHabitaciones.class.php,v 1.14 2007/07/09 18:15:37 carlos Exp $
*/
            ...
            if(!$reliquidar)
            {
                $sql = "SELECT a.*,b.tipo_clase_cama_id,
                        ...
                        WHERE a.ingreso=(SELECT ingreso FROM cuentas WHERE numerodecuenta=$cuenta)
                            ...
                            AND a.numerodecuenta_corte IS NULL
...
//                             }
    ...
    */
    function LiqTMPHosp($datos_hosp,$datos_plan,$dptos_mvto)
    {
        ...
        for($i =1; $i<sizeof($cargos);$i++)
        {
            if($cargos[$i]['precio']<$cargos[$i-1]['precio'])
            {
              $cargos[$i]= $this->GetCargoPlan($cargos[$i]['tipo_cama_id'],&$datos_plan,$cargos[$i]['cantidad']-1);
              $cargos[$i-1]= $this->GetCargoPlan($cargos[$i-1]['tipo_cama_id'],&$datos_plan,$cargos[$i-1]['cantidad']+1);
            }
        }
