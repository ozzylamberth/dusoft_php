<?php
/**
* $Id: CGUNO85.class.php,v 1.11 2007/11/06 21:20:59 alexgiraldo Exp $
*/
...
class CGUNO85
{
    ...
    function GetInterfaceDocumentoLapso($empresa_id, $prefijo, $lapso, $dia_inicial=null, $dia_final=null, $directorio=null, $nombre_archivo=null)
    {
        ...
        $sql = "
                SELECT
                    ...
                    d.empresa_id as empresa_id_cxc,
                    d.prefijo as prefijo_cxc,
                    d.numero as numero_cxc,
                    e.empresa_id as empresa_id_cxp,
                    e.prefijo as prefijo_cxp,
                    e.numero as numero_cxp
                FROM
                (
                    SELECT
                        ...
                        b.documento_cxc,
                        b.documento_cxp,
                        b.centro_de_operacion_id

                    FROM $SHEMA.$TBL_MOV as a, $SHEMA.$TBL_MOV_D as b

                    WHERE
                        a.documento_contable_id = b.documento_contable_id
                        AND a.lapso = '$lapso'
                        $filtro_dias
                        AND a.sw_estado = '2'
                ...
                LEFT JOIN  $SHEMA.$TBL_MOV as c ON (c.documento_contable_id = x.documento_cruce_id)
                LEFT JOIN  $SHEMA.$TBL_MOV as d ON (d.documento_contable_id = x.documento_cxc)
                LEFT JOIN  $SHEMA.$TBL_MOV as e ON (e.documento_contable_id = x.documento_cxp)
                ORDER BY numero;";
    ...
    */
    function AddLine($NUM_REG,$REG)
    {
        ...
        //ESCLUIR LA CUENTAS QUE NO SON DE ACTIVOS (ALGO ESTATICO PARA C.O.C)
        if(substr($REG['cuenta'], 0, 2) != '14') $REG['centro_de_operacion_id']=null;

        if(empty($REG['centro_de_operacion_id']))//CO-MOVTO-MOV
        {
            $line .= $this->FormatearText2BATCH($this->CONF_CGUNO85['codigo_cu'],3,$relleno=$this->c,$tipo_relleno=STR_PAD_LEFT);
        }
        else
        {
            $line .= $this->FormatearText2BATCH($REG['centro_de_operacion_id'],3,$relleno=$this->c,$tipo_relleno=STR_PAD_LEFT);
        }
        ...
        if($REG['base_rtf'])
        {
            ...
            $valorParteEntera  = $valorX[0];
            $valorParteDecimal = $valorX[1];
            $line .= $this->FormatearText2BATCH($valorParteEntera,3,$relleno=$this->c,$tipo_relleno=STR_PAD_LEFT);
            $line .= $this->FormatearText2BATCH($valorParteDecimal,3,$relleno=$this->c,$tipo_relleno=STR_PAD_RIGHT);
        }
        else
        {
        ...
        if($REG['empresa_id_cxc'])
        {
            $line .= $this->FormatearText2BATCH($REG['prefijo_cxc'],2,$relleno=$this->e,$tipo_relleno=STR_PAD_RIGHT);//TIPO-MOV

            $numero_doc = (string) $REG['numero_cxc'];
            $numero_doc = trim($numero_doc);
            if(strlen($numero_doc)>6) $numero_doc = substr($numero_doc,-6);
        ...
        elseif($REG['empresa_id_cxp'])
        {
            $line .= $this->FormatearText2BATCH($REG['prefijo_cxp'],2,$relleno=$this->e,$tipo_relleno=STR_PAD_RIGHT);//TIPO-MOV

            $numero_doc = (string) $REG['numero_cxp'];
            $numero_doc = trim($numero_doc);
            if(strlen($numero_doc)>6) $numero_doc = substr($numero_doc,-6);
        ...
        $line .= $this->GetInformacionClientes(); //NUEVO
        $line .= $this->GetInformacionProveedores(); //NUEVO
        $line .= $this->EOL;
    ...
    function GetInformacionProveedores()
    {
        $line  = "0";//
        $line .= "1";//
        $line .= str_repeat ($this->e, 2);//
