...
/**
* $Id: LiquidacionCargos.class.php,v 1.12 2008/01/09 16:25:18 alexgiraldo Exp $
*/
...
class LiquidacionCargos
{
    ...
    */
    ...
    function ValidarCargoMallaSolicitudManual($cargo_base,$plan_id)
    {
        if(empty($cargo_base) || empty($plan_id))
        {
            $this->error = "CLASS LiquidacionCargos - ValidarCargoMallaSolicitudManual - ERROR 01";
            $this->mensajeDeError = "ERROR EN LOS PARAMETROS (cargo_base[$cargo_base], plan_id[$plan_id])";
            return false;
        }
        ...
        $query = " SELECT
                    ...
                    FROM
                        tarifarios_detalle AS a
                        LEFT JOIN excepciones As e
                        ON  (
                                e.plan_id = $plan_id
                                AND e.tarifario_id = a.tarifario_id
                                AND e.cargo = a.cargo
                            ),
                        plan_tarifario AS b,
                        (
                            SELECT  te.tarifario_id,
                                                                        te.cargo,
                                                                        cu.descripcion as cups
                            FROM        tarifarios_equivalencias te,
                                                                        cups cu
                            WHERE   cargo_base='$cargo_base'
                                                        AND         cu.cargo = te.cargo_base
                        ) AS c
                    ...
                    AND c.cargo = a.cargo
                    AND COALESCE(e.sw_no_contratado,'0') = '0'
                    ";
        ...
        if ($dbconn->ErrorNo() != 0)
        {
            $this->error = "CLASS LiquidacionCargos - ValidarCargoMallaSolicitudManual - ERROR 02";
        ...
        if($resultado->EOF)
        {
            $this->error = "CLASS LiquidacionCargos - ValidarCargoMallaSolicitudManual - ERROR 03";
            $this->mensajeDeError = "EL CARGO $cargo_base NO TIENE EQUIVALENCIAS O NO ESTA CONTRATADO.";
            return false;
        }
    ...
    */
    function LiquidarCargo($plan_id=NULL,$tarifario,$cargo,$cantidad=1,$precio=NULL,$excepcionesAdicionales=array(),$tipoUninadTiempo=NULL,$porcentajeDelcargo=NULL)
    {
        ...
        $valor['valor_nocubierto'] =  $valor['valor_no_cubierto'];
