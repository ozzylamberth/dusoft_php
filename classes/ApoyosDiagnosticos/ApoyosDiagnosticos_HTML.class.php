<?php
//$Id: ApoyosDiagnosticos_HTML.class.php,v 1.7 2008/04/23 13:52:07 hugo Exp $
         ...
         // 
          {
            ...
            case "5": 
            {//caso exclusivo para datalab
              IncludeLib('funciones_datalab');
              $datos = ConsultaExamenesMaquinas($vector[$i][resultado_id], $examenes[numero_orden_id]);
              if($datos)
              {
               // $this->salida.="<pre>".print_r($datos,true)."</pre>";
                $this->salida.="<tr class=\"hc_table_submodulo_list_title\">";
                $this->salida.="<td width=\"25%\">EXAMEN</td>";
                $this->salida.="<td width=\"25%\">RESULTADO</td>";
                $this->salida.="<td width=\"20%\">RANGO NORMAL</td>";
                $this->salida.="<td width=\"20%\">FECHA</td>";
                $this->salida.="<td width=\"20%\">HORA</td>";
                $this->salida.="</tr>";
                $this->salida.="<tr class=\"hc_table_submodulo_list_title\">";
                for($j=0;$j<sizeof($datos);$j++)
                {
                  ...
                  $this->salida.="  <td  align=\"left\" width=\"20%\">".$datos[$j][normal_minima]." - ".$datos[$j][normal_maxima]." ".$datos[$j][unidades]."</td>";
                  $this->salida.="  <td  align=\"left\" width=\"15%\">".$datos[$j][fecha_resultado]."</td>";
                  $this->salida.="  <td  align=\"left\" width=\"15%\">".$datos[$j][hora_resultado]."</td>";
                  $this->salida.="</tr>";
                  if($datos[$j][comentario]!='')
                  {
                    $this->salida.="<tr class=\"$estilo\">";
                    $this->salida.="  <td colspan=\"5\" align=\"left\" >COMENTARIO: ".$datos[$j][comentario]."</td>";
                    $this->salida.="</tr>";
                  }
                }
	...
	function ConsultaExamenesPaciente($resultado_id, $sw_modo_resultado)
{
    ...
    $query = '';
		...
		{
               $query="  SELECT N.*,
                                CASE WHEN F.razon_social IS NOT NULL
                                     THEN F.razon_social ELSE N.profesional END AS laboratorio
                         FROM
                         (
                              SELECT 
                                   A.resultado_id, 
                                   ...
                                   B.numero_orden_id,
                                   I.nombre_tercero AS profesional,
                                   M.titulo_examen AS descripcion, 
                                   M.informacion,
                                   C.cargo_cups
                              FROM  
                                   hc_resultados AS A, 
                              ...
                              WHERE A.resultado_id = ".$resultado_id."
                                   AND A.resultado_id = B.resultado_id
                                   AND B.usuario_id_profesional = H.usuario_id
                                   AND H.tipo_id_tercero = I.tipo_id_tercero
                                   AND H.tercero_id = I.tercero_id
                                   AND C.numero_orden_id = B.numero_orden_id
                                   AND C.cargo_cups = M.cargo
                         ) AS N
                         LEFT JOIN os_internas AS D 
                                   ON (N.numero_orden_id = D.numero_orden_id 
                                        AND N.cargo_cups = D.cargo) 
                         LEFT JOIN departamentos AS E 
                                   ON (D.departamento = E.departamento)
                         LEFT JOIN empresas AS F 
                                   ON (E.empresa_id = F.empresa_id);";
                    
                    /*SELECT b.numero_orden_id, a.resultado_id, a.fecha_realizado,
				          a.observacion_prestacion_servicio,
		...
		elseif ($sw_modo_resultado == '3')
		{
				$query="  SELECT a.resultado_id, 
						...
						FROM hc_resultados as a, 
                              	 hc_resultados_nosolicitados as b,
                                    cups l,	
                                    apoyod_cargos m
						WHERE a.resultado_id = b.resultado_id 
                              	and a.resultado_id = ".$resultado_id."
							and a.cargo = l.cargo 
                                   and l.cargo = m.cargo;";
		}
...
function ConsultaObservaciones($resultado_id)
{
		...
		$query =" SELECT a.resultado_id, a.evolucion_id, a.observacion_prof, d.nombre, 
              (CASE WHEN r1.descripcion IS NOT NULL THEN  r1.descripcion ELSE e.descripcion END) as descripcion 
							FROM hc_apoyod_lecturas_profesionales as a, hc_evoluciones as b,
							profesionales_usuarios as c, profesionales d
              left join tipos_clases_profesionales as r1 on (d.tipo_profesional = r1.tipo_profesional AND d.tipo_clase_profesional_id=r1.tipo_clase_profesional_id), 
