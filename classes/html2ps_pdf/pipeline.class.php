...
require_once(HTML2PS_DIR.'globals.inc.php');
...
class Pipeline {
  ...
  function _process_item($data_id, &$media, $offset=0) {    
    $box =& $this->_layout_item($data_id, $media, $offset, $context, $positioned_filter);
    if (is_null($box)) {
      die(_("Could not fetch data"));
    };
  ...
  function &_layout_item($data_id, &$media, $offset, &$context, &$positioned_filter) {
    $this->_reset_page_at_rules();
