...
class BoxPageMargin extends GenericContainerBox {
  ...
  function BoxPageMargin(&$pipeline, $at_rule) {
     ...
     */
    $html_content = $pipeline->_fillContent($at_rule->getCSSProperty(CSS_HTML2PS_HTML_CONTENT));
    ...
    if ($html_content !== '') {
      ...
      $box =& InlineBox::create_from_text($pipeline->_fillContent($at_rule->getCSSProperty(CSS_CONTENT)),
                                          WHITESPACE_NORMAL,
                                          $pipeline);
    }
    $this->add_child($box);
