...
/**
* $Id: LiquidacionHabitacionesCorte.class.php,v 1.1 2008/01/10 20:01:51 carlos Exp $
*/

/**
* Clase para la liquidacion de Habitaciones para corte de Cuentas
*
* @author Alexander Giraldo -- alexgiraldo@ipsoft-sa.com
* @version $Revision: 1.1 $
* @package SIIS
*/
class LiquidacionHabitacionesCorte
{
    ...
    * @access public
    */
    function LiquidacionHabitacionesCorte()
    {
    ...
    * @access public
    */
    function LiquidarCargosInternacion($cuenta,$reliquidar=false,$fecha_inicial=null,$fecha_final=null,$hora_inicial=null,$hora_final=null,$movimiento_id=null,$Cuenta_old=null)
    {
        ...
        if($movimiento_id)
        {
          $sql_movimiento_id = " AND a.movimiento_id = $movimiento_id ";
        }
        ...
        if($fecha_inicial && $fecha_final)
        {
            if(!$reliquidar)
            {
/* echo               $sql = "SELECT a.*,b.tipo_clase_cama_id,
                            CASE WHEN a.fecha_ingreso >= '$fecha_inicial $hora_inicial:00' THEN a.fecha_ingreso ELSE '$fecha_inicial $hora_inicial:00'::timestamp without time zone END AS fecha_ingreso,
                            CASE WHEN a.fecha_egreso <= '$fecha_final $hora_final:59' THEN a.fecha_egreso ELSE '$fecha_final $hora_final:59'::timestamp without time zone END AS fecha_egreso
                        ...
                        WHERE a.ingreso=(SELECT ingreso FROM cuentas WHERE numerodecuenta=$cuenta)
                            ...
                            AND ((a.fecha_egreso >= '$fecha_inicial $hora_inicial:00' OR a.fecha_egreso ISNULL) AND (a.fecha_ingreso <= '$fecha_final $hora_final:59'))
                ...
                $sql = "SELECT a.*,b.tipo_clase_cama_id,
                        ...
                        WHERE a.ingreso=(SELECT ingreso FROM cuentas WHERE numerodecuenta=$cuenta)
                            AND b.tipo_cama_id = a.tipo_cama_id
                            AND ((a.fecha_ingreso >= '$fecha_inicial $hora_inicial:00') AND (a.fecha_egreso <= '$fecha_final $hora_final:59'  OR a.fecha_egreso ISNULL))
                            AND (a.numerodecuenta_corte IS NULL OR numerodecuenta_corte = $cuenta)
                            $sql_movimiento_id
                        ORDER BY a.fecha_ingreso, b.tipo_clase_cama_id, a.tipo_cama_id;";
            }
            else
            {
                 $sql = "SELECT a.*,b.tipo_clase_cama_id,
                            CASE WHEN a.fecha_ingreso >= '$fecha_inicial $hora_inicial:00' THEN a.fecha_ingreso ELSE '$fecha_inicial $hora_inicial:59'::timestamp without time zone END AS fecha_ingreso,
                            CASE WHEN a.fecha_egreso <= '$fecha_final $hora_final:59' THEN a.fecha_egreso ELSE '$fecha_final $hora_final:59'::timestamp without time zone END AS fecha_egreso
                        FROM  movimientos_habitacion a, tipos_camas b
                        WHERE a.ingreso=(SELECT ingreso FROM cuentas WHERE numerodecuenta=$cuenta)
        ...
        elseif($sql_movimiento_id)
        {
            if(!$reliquidar)
            {
      echo          $sql = "SELECT a.*, b.tipo_clase_cama_id
                        FROM  movimientos_habitacion a, tipos_camas b
                        WHERE  b.tipo_cama_id = a.tipo_cama_id
                        AND (a.numerodecuenta_corte IS NULL OR numerodecuenta_corte=$cuenta)
                        $sql_movimiento_id
                        ORDER BY a.fecha_ingreso, b.tipo_clase_cama_id, a.tipo_cama_id;";
            }
...
//                             }
        ...
        //print_r($v_hospitalizacion); exit;
        ...
        //print_r($this->CargosRetorno); exit;
