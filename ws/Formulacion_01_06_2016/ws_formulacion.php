...
function sincronizarFormula($datos_evolucion, $medicamentos_recetados, $formulacion_antecedentes) {
     ...
     if($datos_evolucion['cantidad_medicamentos']!=($i)){
         return array('msj' => $datos_evolucion['cantidad_medicamentos']." No coincide, el numero de medicamentos recibidos con los enviados ".$i, 'estado' => (bool) '0', 'datos' => "No Formula ".$datos_evolucion['numero_formula']." paciente ".$datos_evolucion['usuario_id'] );
     }
...
function consultar_formula_existente($numeroFormula,$tipoFormula, $tipoPaciente, $paciente){
     global $conexion;
    $msj = "";
    $continuar = false;
    ...
    return array("msj" => $msj, "continuar" => $continuar, "formula" => $numeroFormula);    
}
...
function modificar_formula_refrendado($tipo_id_paciente,$paciente_id,$numero_formula,$transcripcion_medica,$detalle_refrendacion) {
    ...
    foreach ($detalle_refrendacion as $key => $value) {
        ...
        if(!$fecha_finalizacion){
            $mej.=" - Fecha FinalizaciÃƒÂ³n Vacio " ;
        }
        if(!$numero_entrega){
