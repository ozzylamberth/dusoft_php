...
$server->wsdl->addComplexType('WS_datos_evolucion', 'complexType', 'struct', 'all', '', array(
    ...
    'es_transcripcion' => array('name' => 'transcripcion', 'type' => 'xsd:string'),
));
...
function sincronizarFormula($datos_evolucion, $medicamentos_recetados, $formulacion_antecedentes) {
    ...
    $msjs = array_merge($validacion_evolucion['msj'], $validacion_medicamentos_recetados['msj'], $validacion_formulacion_antecedentes['msj'], $validar_formula['msj']);


    if ($validacion_evolucion['continuar'] && $validacion_medicamentos_recetados['continuar'] && $validacion_formulacion_antecedentes['continuar'] && $validar_formula['continuar']) {
...
function consultar_formula($numero_formula) {
    ...
    if ($result) {
        ...
        $continuar = false;
        $msj = "Se ha generado un error insertando en hc_medicamentos_recetados ( " . pg_last_error($conexion) . " ) ";
    }
...
function eliminar_formulacion($evolucion_id) {
    ...
    $sql = "DELETE FROM hc_medicamentos_recetados_amb WHERE evolucion_id = {$evolucion_id} ;            
