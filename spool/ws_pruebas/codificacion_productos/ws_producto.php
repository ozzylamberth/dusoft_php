...
$ns="http://10.0.0.45/SIIS/ws/codificacion_productos/ws_producto.php"; //Cambiar URL
...
$server->register('modificar_productoinsumo',array('descripcion' => 'xsd:string',
                                            	   'descripcion_abreviada' => 'xsd:string',
                                            	   'codigo_cum' => 'xsd:string',
                                            	   'codigo_alterno' => 'xsd:string',
                                            	   'codigo_barras' => 'xsd:string',
...
$server->register('consultar_producto', array('codigo_producto' => 'xsd:string'),
									    array('return' => 'xsd:string'), $ns);

$server->register('consultar_medicamento', array('codigo_medicamento' => 'xsd:string'),
									    array('return' => 'xsd:string'), $ns);
...
function modificar_productoinsumo($descripcion,$descripcion_abreviada,$codigo_cum,$codigo_alterno,
...
function consultar_producto($codigo_producto) {
    ...
    $sql = "select 	count(*) 
			...
			where 	ip.codigo_producto = '{$codigo_producto}' ";
...
function consultar_medicamento($codigo_medicamento) {
	require_once("conexionpg.php");

    $sql = "select 	count(*) 
			from 	medicamentos m 
			where 	m.codigo_medicamento = '{$codigo_medicamento}' ";
