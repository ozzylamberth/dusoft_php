...
function sincronizarFormula($datos_evolucion, $medicamentos_recetados, $formulacion_antecedentes) {
     ...
     foreach ($formulacion_antecedentes as $key => $value) {
        ...
        $datosEvolucion .=    $value['numero_formula'] ."".
      ...
      foreach ($medicamentos_recetados as $key => $value) {
        ...
        $medicamentosRecetados .=    $value['codigo_producto'] ."".
                        $value['cantidad'] ."".  
                        $value['observacion'] ."".
       ...
       foreach ($datos_evolucion as $key => $value) {

        $datosPrincipales .=    $value['numero_evolucion'] ."".
                        $value['numero_formula'] ."".  
                        $value['ingreso'] ."".
                         $value['fecha']  ."".
                         $value['usuario_id']  ."".
                        ...
                        $value['hc_modulo'] ."".
                         ...
                         $value['historia_clinica_tipo_cierre_id']  ."".

                         $value['observacion_cierre']  ."".
     ...
     registrar_logs('00001', '0000',$datosEvolucion, 2);
     registrar_logs('00002', '0000',$medicamentosRecetados,2);
     registrar_logs('00003', '0000',$datosPrincipales,2);
    // Validacion de Datos    
...
function validacion_medicamentos($medicamentos_recetados) {
    ...
    foreach ($medicamentos_recetados as $key => $value) {
        ...
        if ($value['cantidad'] == "" || is_null($value['cantidad']) || $value['cantidad'] < 1) {
            $continuar = false;
            array_push($msj, "{$value['cantidad']} cantidad en hc_medicamentos_recetados_amb inválido");
        }
        if ($value['observacion'] == "") {
            $continuar = false;
            array_push($msj, "observacion inválido");
        }
        if ($value['sw_paciente_no_pos'] == "" || is_null($value['sw_paciente_no_pos'])|| $value['sw_paciente_no_pos'] == "NULL") {
            $continuar = false;
...
function validacion_formulacion_antecedentes($formulacion_antecedentes) {
    ...
    foreach ($formulacion_antecedentes as $key => $value) {
        ...
        if ($value['cantidad'] == "") {
            $continuar = false;
            array_push($msj, "cantidad en hc_formulacion_antecedentes inválida");
        }
        if ($value['sw_mostrar'] == "") {
            $continuar = false;
