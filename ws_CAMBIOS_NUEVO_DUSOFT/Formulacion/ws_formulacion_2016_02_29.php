    ...
    if ($validacion_evolucion['continuar'] && $validacion_medicamentos_recetados['continuar'] && $validacion_formulacion_antecedentes['continuar'] && $validar_formula['continuar'] && $validar_dispensacion['continuar']) {
        ...
        if (!is_null($resultado_insertar_evolucion['evolucion_id'])) {
            ...
            if ($resultado_insertar_medicamentos_recetados['continuar']) {
                ...
                //Insertar Formulaciï¿½n Antecedentes
...
function modificar_formula_a_refrendar($cabecera_refrendacion,$detalle_refrendacion){
     ...
     if($paso_tipo){
         $msjs .=" FALTA tipo_id_paciente </br>";
     }
     if($paso_paciente_id){
         $msjs .=" FALTA paciente_id </br>";
     }
     if($paso_numero_formula){
         $msjs .=" FALTA numero_formula </br>";
     }
     if($paso_tipo){
         $msjs .=" FALTA transcripcion_medica </br>";
     }
     $band=false;    
     ...
     if($paso_tipo && $paso_paciente_id && $paso_numero_formula && $paso_trasncricion){
         ...
         $msjs =$continuar['msj'];
         $band=($continuar['continuar'])?1:0;         
        }     
        return array('msj' => $msjs, 'estado' => (bool) $band, 'datos' => '');       
  }
...
function _modificarMedicamentos($datos_evolucion, $medicamentos_recetados, $formulacion_antecedentes) {
    ...
    if ($validacion_evolucion['continuar'] && $validacion_medicamentos_recetados['continuar'] && $validacion_formulacion_antecedentes['continuar'] && $validar_dispensacion['continuar']) {
        ...
        if ($resultado_modificar_medicamentos_recetados['continuar']) {
            ...
            //Modificar Formulaciï¿½n Antecedentes
...
function _eliminarMedicamento($numero_formula, $codigo_producto) {
    ...
    if ($numero_formula == "") {
        $continuar = false;
        array_push($msj, "numero_formula invï¿½lida");
    }
...
function validar_datos_evolucion_formulacion($datos_evolucion) {
    ...
    if (empty($datos_evolucion)) {
        $continuar = false;
        array_push($msj, "Los datos de la evolucion estï¿½n vacios");
    }
    ...
    if (trim($datos_evolucion['modificacion']) == "") {
        $continuar = false;
        array_push($msj, "campo modificacion invï¿½lida");
    }
...
function validacion_medicamentos($medicamentos_recetados) {
    ...
    foreach ($medicamentos_recetados as $key => $value) {
        ...
        if (trim($value['codigo_producto']) == "" || is_null($value['codigo_producto'])) {
            $continuar = false;
            array_push($msj, "codigo_producto invï¿½lido");
        }
        if (trim($value['cantidad']) == "" || is_null($value['cantidad']) || $value['cantidad'] < 1) {
            $continuar = false;
            array_push($msj, "{$value['cantidad']} cantidad en hc_medicamentos_recetados_amb invï¿½lido");
        }
        if (trim($value['observacion']) == "") {
            $continuar = false;
            array_push($msj, "observacion invï¿½lido");
        }
        if (trim($value['sw_paciente_no_pos']) == "" || is_null($value['sw_paciente_no_pos'])|| $value['sw_paciente_no_pos'] == "NULL") {
            $continuar = false;
            array_push($msj, "sw_paciente_no_pos invï¿½lido");
        }
        if (trim($value['via_administracion_id']) == "") {
            $continuar = false;
            array_push($msj, "via_administracion_id invï¿½lido");
        }
        if (trim($value['dosis']) == "") {
            $continuar = false;
            array_push($msj, "dosis invï¿½lido");
        }
        if (trim($value['unidad_dosificacion']) == "") {
            $continuar = false;
            array_push($msj, "unidad_dosificacion invï¿½lido");
        }
        if (trim($value['tipo_opcion_posologia_id']) == "") {
            $continuar = false;
            array_push($msj, "tipo_opcion_posologia_id invï¿½lido");
        }
        if (trim($value['cantidadperiocidad']) == "" || $value['cantidadperiocidad'] < 1) {
            $continuar = false;
            array_push($msj, "cantidadperiocidad invï¿½lido");
        }
        if (trim($value['justificacion_reformula']) == "") {
            $continuar = false;
            array_push($msj, "justificacion_reformula invï¿½lido");
        }
        if (trim($value['numero_formula']) == "") {
            $continuar = false;
            array_push($msj, "numero_formula invï¿½lido");
        }
        if (trim($value['dias_tratamiento']) == "" || $value['dias_tratamiento'] < 1) {
            $continuar = false;
            array_push($msj, "dias_tratamiento invï¿½lido");
        }
        if (trim($value['bloqueo']) == "") {
            $continuar = false;
            array_push($msj, "bloqueo invï¿½lido");
        }
        if (trim($value['usuario_id']) == "") {
            $continuar = false;
            array_push($msj, "usuario_id invï¿½lido");
        }
        if (trim($value['usuario_id_modifica']) == "") {
            $continuar = false;
            array_push($msj, "usuario_id_modifica invï¿½lido");
        }
        if (trim($value['cantidad_dia']) == "" || $value['cantidad_dia'] < 0) {
            $continuar = false;
            array_push($msj, "cantidad_dia invï¿½lido");
        }
    }
...
function validacion_formulacion_antecedentes($formulacion_antecedentes) {
    ...
    foreach ($formulacion_antecedentes as $key => $value) {
        ...
        if (trim($value['tipo_id_paciente']) == "") {
            $continuar = false;
            array_push($msj, "tipo_id_paciente invï¿½lido");
        }
        if (trim($value['paciente_id']) == "") {
            $continuar = false;
            array_push($msj, "paciente_id invï¿½lido");
        }
        if (trim($value['medico_id']) == "") {
            $continuar = false;
            array_push($msj, "medico_id invï¿½lido");
        }
        if (trim($value['codigo_medicamento']) == "") {
            $continuar = false;
            array_push($msj, "codigo_medicamento invï¿½lido");
        }
        if (trim($value['dosis']) == "") {
            $continuar = false;
            array_push($msj, "dosis invï¿½lido");
        ...
        if (trim($value['frecuencia']) == "") {
            $continuar = false;
            array_push($msj, "frecuencia invï¿½lido");
        }
        if (trim($value['sw_permanente']) == "") {
            $continuar = false;
            array_push($msj, "sw_permanente invï¿½lido");
        }
        if (trim($value['sw_formulado']) == "") {
            $continuar = false;
            array_push($msj, "sw_formulado invï¿½lido");
        }
        if (trim($value['tiempo_total']) == "") {
            $continuar = false;
            array_push($msj, "tiempo_total invï¿½lido");
        }
        if (trim($value['perioricidad_entrega']) == "") {
            $continuar = false;
            array_push($msj, "perioricidad_entrega invï¿½lido");
        }
        ...
        if (trim($value['tiempo_perioricidad_entrega']) == "") {
            $continuar = false;
            array_push($msj, "tiempo_perioricidad_entrega invï¿½lido");
        }
        if (trim($value['unidad_perioricidad_entrega']) == "") {
            $continuar = false;
            array_push($msj, "unidad_perioricidad_entrega invï¿½lido");
        }
        if (trim($value['cantidad']) == "") {
            $continuar = false;
            array_push($msj, "cantidad en hc_formulacion_antecedentes invï¿½lida");
        }
        if (trim($value['sw_mostrar']) == "") {
            $continuar = false;
            array_push($msj, "sw_mostrar invï¿½lido");
        }
        if (trim($value['fecha_registro']) == "") {
            $continuar = false;
            array_push($msj, "fecha_registro invï¿½lido");
        }
        if (trim($value['fecha_finalizacion']) == "") {
            $continuar = false;
            array_push($msj, "fecha_finalizacion invï¿½lido");
        }
        if (trim($value['fecha_formulacion']) == "") {
            $continuar = false;
            array_push($msj, "fecha_formulacion invï¿½lido");
        }
        if (trim($value['fecha_modificacion']) == "") {
            $continuar = false;
            array_push($msj, "fecha_modificacion invï¿½lido");
        }
    }
...
*               Formulaciï¿½n_medicamentos=0
*				Formulaciï¿½n_insumos=2
*				Transcripciï¿½n_medicamentos=1
*				Transcripciï¿½n_insumos=3
*/
...
/**
*+Descripcion: Metodo encargado de almacenar en la tabla hc_medicamentos_recetados_amb los medicamentos ï¿½ insumos asociados 
...
function modificar_formula_refrendado($tipo_id_paciente,$paciente_id,$numero_formula,$transcripcion_medica,$detalle_refrendacion) {
     ...
     $detalle_refrendacion2=array();
    if(!is_array($detalle_refrendacion[0])){
     $detalle_refrendacion2[]=$detalle_refrendacion;
     
    }else{
       $detalle_refrendacion2= $detalle_refrendacion;
    }
    
    foreach ($detalle_refrendacion2 as $key => $value) {
        ...
        if(!$fecha_finalizacion){
            $mej.=" - Fecha FinalizaciÃ³n Vacio " ;
        }
        if(!$numero_entrega){
            $mej.=" - Numero de Entrega Vacio " ;
        }
        
            if($codigo_producto!=false && $fecha_finalizacion!=false && $numero_entrega!=false  ){
            $sql .= " INSERT INTO medicamentos_refrendados (
                  ...
                  return array("msj" => " FALTAN LOS SIGUIENTES PARAMETROS".$mej, "continuar" => 0);
             }
  }
