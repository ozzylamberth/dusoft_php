...
*/
 ...
 function TraerImpresorasPrdeterminadas()
 {
  ...
  $query="SELECT a.impresora,b.sw_predeterminada FROM system_printers as a left join
    ...
    function PrinterSystem()
    {
    list($dbconn) = GetDBconn();
  $query="SELECT impresora, descripcion, ubicacion
    ...
    $resulta=$dbconn->Execute($query);
        ...
        function GuardarDatos()
        {
            ...
            if($_POST['guardar'])
            {
                ...
                if($pred>1)
                {
                        $this->frmError["MensajeError"]="SELECCIONAR SOLO UNA PREDETERMINADA";
                        $this->uno=1;
                        $this->FormaImpresorasPredeterminadas();
                        return true;
                }
            ...
            if($_POST['guardar']<>NULL)
            {
                    for($j=0; $j<sizeof($vector);$j++)
                    {
                            if ($_POST['dx'.$j]==NULL)
                                    $_POST['dx'.$j]=0;
                            $impresora=$vector[$j][impresora];
                            $query="UPDATE system_printers_host
                                            SET  sw_predeterminada = ".$_POST['dx'.$j]."
                                            WHERE impresora='".$impresora."' AND ip='".$ip."'";
                            $resulta=$dbconn->Execute($query);
                            if ($dbconn->ErrorNo() != 0)
                            {
                    ...
                    $this->frmError["MensajeError"]="IMPRESORA PREDETERMIDADA ACTUALIZADA.";
                    $this->uno=1;
                    $this->FormaImpresorasPredeterminadas();
                    return true;
            }
            else
            {
                for($j=0; $j<sizeof($vector);$j++)
                {
                    ...
                    $this->frmError["MensajeError"]="IMPRESORA(S) ELIMINADA(S).";
                    $this->uno=1;
                    $this->FormaImpresorasPredeterminadas();
                    return true;
            }
    ...
    function InsertarImpresoras()
    {
        $vector=$_REQUEST['dats'];
        $ip=GetIPAddress();
        list($dbconn) = GetDBconn();
        for($j=0; $j<sizeof($vector);$j++)
        {
            ...
            {
                $impresora=$vector[$j][impresora];
                     $query="INSERT INTO system_printers_host
                                    (impresora,ip)
                                    VALUES('".$impresora."','".$ip."')";
                $resulta=$dbconn->Execute($query);
                if ($dbconn->ErrorNo() != 0)
                {
                    $this->frmError["MensajeError"]="ERROR AL INSERTAR DATOS ".$dbconn->ErrorMsg();
                    $this->uno=1;
                    $this->FormaImpresorasPredeterminadas();
                    return true;
                }
