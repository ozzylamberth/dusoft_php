...
/**
* $Id: system_Usuarios_admin.php,v 1.11 2008/07/08 15:48:34 cahenao Exp $
...
//      
        ...
        if($_REQUEST['tipoDocumentoAnt'] AND $_REQUEST['DocumentoAnt'])
        {
          $tipodocumento = $_REQUEST['tipoDocumentoAnt'];
          $documento = $_REQUEST['DocumentoAnt'];
        }
        $query = "SELECT count(*)
                      FROM terceros
                      WHERE tipo_id_tercero='$tipodocumento' AND tercero_id='$documento';";
        $result = $dbconn->Execute($query);
...
function InsertarAPerfilUsuario()
{
    ...
    list($dbconn) = GetDBconn();
    //print_r($_REQUEST);
    if(empty($_REQUEST['op']) && empty($_REQUEST['eli']))
    {
        $this->frmError["MensajeError"]="Debe escoger Alguna opcion.";
        ...
        {
                        $query = "
                        delete from system_usuarios_perfiles
                                        where
                                              usuario_id=".$_REQUEST['uid']."
                                        and  empresa_id ='".$_REQUEST['empresa']."';
                                        
                        INSERT INTO system_usuarios_perfiles
        ...
        foreach($_REQUEST['eli'] as $index=>$codigo)
        {
                        $query = "delete from system_usuarios_perfiles
                                        where
                                              usuario_id=".$_REQUEST['uid']."
                                        and  empresa_id ='".$_REQUEST['empresa']."' 
                                        and  perfil_id ='".$codigo."';
                                        ;";
                        $resulta=$dbconn->Execute($query);
                        if ($dbconn->ErrorNo() != 0){
                            $this->error = "Error al insertar en system_perfiles_menus";
...
function TraerPerfilesUser($uid,$emp)
{
    ...
    //$dbconn->debug=true;
    $query="
              select a.perfil_id,b.descripcion,a.usuario_id
              from 
              system_usuarios_perfiles as a 
              LEFT JOIN system_perfiles AS b ON (a.perfil_id = b.perfil_id)
              and  (a.empresa_id = b.empresa_id) and (a.empresa_id = '".$emp."')
              WHERE 
                    a.usuario_id=".$uid."
              UNION
              select perfil_id,descripcion,0 as usuario_id
              from 
              system_perfiles
              where
              empresa_id = '".$emp."'
              and perfil_id NOT IN(
                                    select a.perfil_id
                                    from 
                                    system_usuarios_perfiles as a 
                                    LEFT JOIN system_perfiles AS b ON (a.perfil_id = b.perfil_id)
...
*/
    ...
    function BuscarUsuariosSistema($filtro){
        ...
        $query = "select a.usuario_id,d.usuario,d.nombre,d.descripcion,
                                 d.passwd,d.activo,d.sw_admin,a.empresa_id,c.razon_social, d.tercero_id, d.tipo_id_tercero
    ...
    $i=0;
        ...
        $result->fields[6].'/'.$result->fields[7].'/'.$result->fields[8].'/'.$result->fields[9].'/'.$result->fields[10];
      $result->MoveNext();
            $i++;
        }
    $result->Close();
...
*/
    ...
    $consulta='1';
        ...
        if(!$this->FormaInsertarUsuarioSistema($nombre,$usuario,'','',$tema,$action,$consulta,$descripcion,$tercero_id,$tipo_id_tercero,true,$uid,$empresa)){
        return false;
    }
        return true;
  }
...
*/
    ...
    $loginUsuario=$_REQUEST['loginUsuario'];
		...
		//Solicitud Mauricio Barrios
		$tipo_id_tercero=$_REQUEST['tipox_id'];
		$tercero_id=$_REQUEST['tercero_id'];
		

        if($nombreUsuario=='' || $loginUsuario=='' || $empresa==-1 || $tercero_id==''){
            if($nombreUsuario==''){ $this->frmError["nombreUsuario"]=1; }
            if($loginUsuario==''){ $this->frmError["loginUsuario"]=1; }
			if($tercero_id==''){ $this->frmError["identifi"]=1; }
            $this->frmError["MensajeError"]="Faltan datos obligatorios.";
            $consulta='1';
      $action=ModuloGetURL('system','Usuarios','admin','ModificarUsuariosSistema',array("uid"=>$uid));
            if(!$this->FormaInsertarUsuarioSistema($nombreUsuario,$loginUsuario,'','',$tema,$action,$consulta,$descripcion,$tercero_id,$tipo_id_tercero,$empresa)){
                return false;
            }
            return true;
        }
    ...
    if(empty($fechacaduca))
        ...
        else
        {
                    ...
                    else
                    {
                        ...
                        $consulta='1';
                        $action=ModuloGetURL('system','Usuarios','admin','ModificarUsuariosSistema',array("uid"=>$uid));
                      $this->FormaInsertarUsuarioSistema($nombreUsuario,$loginUsuario,'','',$tema,$action,$consulta,$descripcion,$tercero_id,$tipo_id_tercero,$empresa);
                        return true;
                    }
        }
		...
		list($dbconn) = GetDBconn();
		$query = "SELECT COUNT(*) FROM system_usuarios WHERE NOT usuario_id = $uid AND tercero_id=$tercero_id AND tipo_id_tercero = '$tipo_id_tercero'";
		$res=$dbconn->Execute($query);
    if ($dbconn->ErrorNo() != 0) {
          $this->error = "Error al buscar identificacion en system_usuarios";
          $this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
          return false;
            }
            ...
            if($res->fields[0]>0)
            {
                $this->frmError["MensajeError"]="EXISTE UNA IDENTIFICACION IGUAL EN LA BASE DE DATOS,POR FAVOR CAMBIELA!";
				$consulta='1';
                $action=ModuloGetURL('system','Usuarios','admin','ModificarUsuariosSistema',array("uid"=>$uid));
              $this->FormaInsertarUsuarioSistema($nombreUsuario,$loginUsuario,'','',$tema,$action,$consulta,$descripcion,$tercero_id,$tipo_id_tercero,$empresa);
              return true;
    ...
    if($res->fields[0]<1)
        ...
        {UserSetVar($uid,'Tema',$tema);}
				...
				//ACTUALIZAR SI EL USUARIOS ES UN PROFESIONAL y/O TERCERO
				$this->ActualizarDatosProfesionales($uid,$nombreUsuario);
				//FIN ACTUALIZAR SI EL USUARIOS ES UN PROFESIONAL y/O TERCERO
		...
		function ActualizarDatosProfesionales($uid,$nombreUsuario)
		{
				list($dbconn) = GetDBconn();
				$query = "SELECT tipo_id_tercero, tercero_id
									FROM profesionales
			...
			if(!$result->EOF)
			{
				$tipo_id_tercero = $result->fields[0];
				$tercero_id = $result->fields[1];
				$query = "UPDATE profesionales 
				...
				$query = "UPDATE terceros 
									SET nombre_tercero = '$nombreUsuario'
...
//exit;
    ...
    $passwordReal=$_REQUEST['passwordReal'];
		...
		//Campos solicitados por el ingeniero Mauricio Barrios
		$tipo_id_tercero=$_REQUEST['tipox_id'];
		$tercero_id=$_REQUEST['tercero_id'];
        if($nombreUsuario=='' || $loginUsuario=='' || $tercero_id=='' ){
            if($nombreUsuario==''){ $this->frmError["nombreUsuario"]=1; }
            if($loginUsuario==''){ $this->frmError["loginUsuario"]=1; }
      ...
      //if($passwordReal==''){ $this->frmError["passwordReal"]=1; }
            $this->frmError["MensajeError"]="Faltan datos obligatorios.";
      $action=ModuloGetURL('system','Usuarios','admin','InsertarUsuariosSistema');
            $this->FormaInsertarUsuarioSistema($nombreUsuario,$loginUsuario,$password,$passwordReal,$tema,$action,'',$descripcion,$tercero_id,$tipo_id_tercero);
            return true;
        }
...
//          }
    ...
    if ($dbconn->ErrorNo() != 0) {
        ...
        //revisamos q no exista un numero de identificacion igual  en la base de datos..
        $query = "SELECT COUNT(*) FROM system_usuarios WHERE tercero_id=$tercero_id AND tipo_id_tercero = '$tipo_id_tercero'";
		$res=$dbconn->Execute($query);
    if ($dbconn->ErrorNo() != 0) {
          $this->error = "Error al buscar identificacion en system_usuarios";
          $this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
                ...
                if($_SESSION['USER']['DIAS']==0)
                {
                    ...
                    //generamos el passwd
                     ...
                     $query = "INSERT INTO system_usuarios(
                                                                        ...
                                                                        '".UserEncriptarPasswd('siis')."',$codigoAlterno,'$tipo_id_tercero','$tercero_id')";
                    ...
                    $SW_NO_CADUCA=1;
                                    ...
                                    $query = "INSERT INTO system_usuarios(
                                                                        ...
                                                                        VALUES($serial,'$loginUsuario','$nombreUsuario','$descripcion','$activo','0',".$_SESSION['USER']['DIAS'].",
                                                                        '$fecha_contraseña',$codigoAlterno,'$tipo_id_tercero','$tercero_id')";
                        }
        }
        else
        {
                ...
                if($_SESSION['USER']['DIAS']==0)
                {
                     $query = "INSERT INTO system_usuarios(
                                        usuario_id,
                                                                ...
                                                                VALUES($serial,'$loginUsuario','$nombreUsuario','$descripcion','$activo','0','$fechacaduca', ".$_SESSION['USER']['DIAS'].",$codigoAlterno,'$tipo_id_tercero','$tercero_id')";
                }else{
...
*/
    ...
    list($dbconn) = GetDBconn();
      $query = "select * from tipo_id_terceros order by indice_de_orden";
