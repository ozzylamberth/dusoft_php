	...
	$y=1;
		for($i=0;$i<sizeof($usuarios);$i++){
		  if($y % 2){
			...
			$empresa=$arreglo[7];
			$nombreE=$arreglo[8];
			$tercero_id=$arreglo[9];
			$tipo_id_tercero=$arreglo[10];
...
*/
      if($activo=='1'){
        ...
        $tipoBloqueo='ACTIVAR......';
     	...
     	$actionEditar=ModuloGetURL('system','Usuarios','admin','LlamaModificarUsuarioSistema',array("uid"=>$uid,"nombre"=>$nombre,"usuario"=>$usuario,"tema"=>$tema,"empID"=>$empresa,"descripcion"=>$descripcion,"tercero_id"=>$tercero_id,"tipo_id_tercero"=>$tipo_id_tercero,'paso'=>$_REQUEST['paso'],'Of'=>$_REQUEST['Of'],'busqueda'=>$_REQUEST['busqueda']));
...
*/
	...
	function FormaInsertarUsuarioSistema($nombreUsuario,$loginUsuario,$password,$passwordReal,$tema,$action,$consulta,$descripcion,$tercero_id,$tipox_id,$spia,$uid,$empresa){
    ...
    $this->salida.= "               <input type=\"hidden\" name=\"action\" value=\"$action\">";
		...
		$this->salida .= "				       <tr  class=\"modulo_list_oscuro\"><td class=\"".$this->SetStyle("tema")."\">TIPO DOCUMENTO: </td><td><select name=\"tipox_id\" id=\"tipox_id\" class=\"select\" onchange=\"\">";
		$TiposTercerosId = $this->tiposDocumento();
        for ($i = 0; $i < count($TiposTercerosId); $i++) {
			if($TiposTercerosId[$i]['tipo_id_tercero']==$tipox_id){
								  $this->salida .=" <option value=\"" . $TiposTercerosId[$i]['tipo_id_tercero'] . "\" selected>" . $TiposTercerosId[$i]['tipo_id_tercero'] . "</option>";
								}else{
									$this->salida .="                         <option value=\"" . $TiposTercerosId[$i]['tipo_id_tercero'] . "\">" . $TiposTercerosId[$i]['tipo_id_tercero'] . "</option> \n";
								}
       }
		$this->salida .= "       </select></td></tr>";
		$this->salida .= "				       <tr  class=\"modulo_list_claro\"><td class=\"".$this->SetStyle("identifi")."\">N. IDENTIFICACION: </td><td><input type=\"text\" class=\"input-text\" name=\"tercero_id\" maxlength=\"60\" value=\"$tercero_id\"></td></tr>";
...
function ListadoPerfilUsuario($uid,$user,$nom,$empresa,$nombreE)
{
        ...
        if($vector)
          ...
          $action=ModuloGetURL('system','Usuarios','admin','InsertarAPerfilUsuario',array("uid"=>$uid,"user"=>$user,"nom"=>$nom,"NoEmp"=>$nombreE,"empresa"=>$empresa));
					...
					$this->salida.="  <td>Descripción</td>";
					$this->salida.="  <td>TODOS<input type=checkbox name=sel onclick=chequeoTotal(this.form,this.checked)></td>";
					$this->salida.="  <td>Quitar</td>";
          $this->salida.="</tr>";
					//print_r($vector);
          for($i=0;$i<sizeof($vector);$i++)
					{
							if(!empty($vector[$i][descripcion]))
              {
              $perfil_id=$vector[$i][perfil_id];
							$desc=$vector[$i][descripcion];
							$check=$vector[$i][usuario_id];
							
              if($vector[$i][usuario_id]==$_REQUEST['uid'])
							{
               $chequeo='checked';
							}
							else
               ...
               $chequeo='';
							...
							$this->salida.="  <td align='center' width=\"10%\"><input type=checkbox name=eli[$i] value=$perfil_id></td>";
							$this->salida.="</tr>";
              }
					}
					$this->salida.="</table>";
