...
* @version 1.0
* @package SIIS
* $Id: hc_Conducta_Roles_Examen.class.php,v 1.1 2007/11/30 20:43:03 tizziano Exp $
*/
...
class Examen
{
	...
	function Examen()
     ...
     /**
     * Get_RolesAcademicos
     * Metodo para obtener los datos del rol profesional o academico del paciente.
     *
     * @return array.
     * @access public
     */
     function Get_RolesAcademicos()
     {
		list($dbconn) = GetDBconn();
          $dbconn->BeginTrans();
          
          //Consulta de datos
          $query="SELECT * FROM hc_psicologia_roles_profesionales
          	   WHERE ingreso = ".SessionGetVar("Ingreso")."
                  AND evolucion_id = ".SessionGetVar("Evolucion").";";
		$result = $dbconn->Execute($query);
		if($dbconn->ErrorNo() != 0)
		{
			$this->error = "Error al Consultar en hc_psicologia_roles_profesionales";
          ...
          while(!$result->EOF)
          {
               $RolAca = $result->GetRowAssoc($toUpper=false);
               $result->MoveNext();
          }
     ...
     /**
     * Get_RolesPareja
     * Metodo para obtener los datos del rol de pareja del paciente.
     *
     * @return array.
     * @access public
     */
     function Get_RolesPareja()
     {
		list($dbconn) = GetDBconn();
          $dbconn->BeginTrans();
          
          //Consulta de datos
          $query="SELECT * FROM hc_psicologia_roles_pareja
          	   WHERE ingreso = ".SessionGetVar("Ingreso")."
                  AND evolucion_id = ".SessionGetVar("Evolucion").";";
		$result = $dbconn->Execute($query);
		if($dbconn->ErrorNo() != 0)
		{
			$this->error = "Error al Consultar en hc_psicologia_roles_pareja";
			$this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
               $dbconn->RollbackTrans();
			return false;
		}
          ...
          while(!$result->EOF)
          {
               $RolPareja = $result->GetRowAssoc($toUpper=false);
               $result->MoveNext();
          }
     ...
     /**
     * Get_RolesSociales
     * Metodo para obtener los datos del rol social del paciente.
     *
     * @return array.
     * @access public
     */
     function Get_RolesSociales()
     {
		list($dbconn) = GetDBconn();
          $dbconn->BeginTrans();
          
          //Consulta de datos
          $query="SELECT * FROM hc_psicologia_roles_sociales
          	   WHERE ingreso = ".SessionGetVar("Ingreso")."
                  AND evolucion_id = ".SessionGetVar("Evolucion").";";
		$result = $dbconn->Execute($query);
		if($dbconn->ErrorNo() != 0)
		{
			$this->error = "Error al Consultar en hc_psicologia_roles_sociales";
			$this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
               $dbconn->RollbackTrans();
			return false;
		}
          ...
          while(!$result->EOF)
          {
               $RolSocial = $result->GetRowAssoc($toUpper=false);
               $result->MoveNext();
          }
     ...
     /**
     * Get_RolesFamiliares
     * Metodo para obtener los datos del rol familiar del paciente.
     *
     * @return array.
     * @access public
     */
     function Get_RolesFamiliares()
     {
		list($dbconn) = GetDBconn();
          $dbconn->BeginTrans();
          
          //Consulta de datos
          $query="SELECT * FROM hc_psicologia_roles_familiares
          	   WHERE ingreso = ".SessionGetVar("Ingreso")."
                  AND evolucion_id = ".SessionGetVar("Evolucion").";";
		$result = $dbconn->Execute($query);
		if($dbconn->ErrorNo() != 0)
		{
			$this->error = "Error al Consultar en hc_psicologia_roles_familiares";
			$this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
               $dbconn->RollbackTrans();
			return false;
		}
          ...
          while(!$result->EOF)
          {
               $RolFamiliar = $result->GetRowAssoc($toUpper=false);
               $result->MoveNext();
          }
