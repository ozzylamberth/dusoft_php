...
/**
* $Id: hc_DiagnosticosMedicos_CDA.php,v 1.1 2005/06/09 22:11:28 tizziano Exp $
*/

/**
* Clase para consulta CDA-HL7 del submodulo DiagnosticosMedicos
*
* @author Tizziano Perea <tperea@ipsoft-sa.com>
* @version $Revision: 1.1 $
* @package SIIS
*/ 
class DiagnosticosMedicos_CDA extends Extenciones_CDA_HC
{
    /**
    ...
    * @access public    
    */  
    function DiagnosticosMedicos_CDA()
    {
    ...
    */     
    ...
    {
          $salida.="<caption>DIAGNOSTICOS MEDICOS</caption>";
		foreach($XML_Consulta as $k => $v)
          {
               $FechaM = $this->FechaStamp($v[3]);
               $HoraM = $this->HoraStamp($v[3]);
               $User=$this->GetDatosUsuarioSistema($v[2]);
               ...
               $salida.="<b>".$FechaM." - ".$HoraM." - ".$User[0][nombre]."</b>";

               if(!empty($v[1]))
               {
                    $salida.="<br></br>";
               	$salida.="<b>Observación Diagnostico: </b>".$v[1]."";
               }
               $salida.="</content>";
               $salida.="</paragraph>";
    ...
    function GetConsultaSubmodulo($Paramdatos, $ParamTipo)
    {
          ...
          switch($ParamTipo)
          {
               ...
               while($dx_medicos = $resultado->FetchRow())
               {
               	$XML_Consulta[] = $dx_medicos;
               }
...
/*               $sql="SELECT ingreso
               ...
               if(!empty($ingreso))
			{
				for($i=0; $i<sizeof($ingreso); $i++)
				{
                         ...
                         while($evolucion = $resultado->FetchRow())
                         {
                              $XML_Consulta[] = $evolucion;
                         }
                    }
               }
