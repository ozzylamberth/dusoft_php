<?php
  /**
  * Submodulo de Diagnosticos Egreso.
  *
  * Submodulo para manejar los Diagnosticos de egreso (rips) en un paciente en una evolución.
  ...
  * @version 1.0
  * @package SIIS
  * $Id: hc_DiagnosticoE.php,v 1.11 2009/01/14 13:10:25 hugo Exp $
  */
  /**
  * DiagnosticoE
  *
  ...
  * submodulo del diagnostico de egreso .
  */
  ...
  class DiagnosticoE extends hc_classModules
  {
  	var $limit;
    var $conteo;
    /**
    ...
    function Insertar_Varios_Diagnosticos()
    {
      ...
      foreach($_REQUEST['opD'.$pfj] as $index=>$codigo)
      {
        ...
        { 
          ...
          if ($dbconn->ErrorNo() != 0)
          {
            $this->error = "Error al insertar en hc_diagnosticos_egreso";
            $this->mensajeDeError = "Error DB : " . $dbconn->ErrorMsg();
          ...
          if($resulta->fields[0]==0)
          {
            $query="INSERT into hc_diagnosticos_egreso
                      ( 
                        usuario_id,
                        tipo_diagnostico_id,
                        evolucion_id,
    ...
    */
    ...
    function Eliminar_Diagnostico_Solicitado($diagnostico_id)
    {
            $pfj=$this->frmPrefijo;
      list($dbconn) = GetDBconn();
      $query="delete from hc_diagnosticos_egreso where tipo_diagnostico_id = '$diagnostico_id'	and evolucion_id=".$this->evolucion.";";
      $resulta=$dbconn->Execute($query);
      if ($dbconn->ErrorNo() != 0)
      {
      ...
      else
      {
        ...
        {
          $sql2="update hc_diagnosticos_egreso set sw_principal='1' where evolucion_id=".$vector['evolucion_id']." and tipo_diagnostico_id='".$vector['tipo_diagnostico_id']."';";
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ///////////////NECESARIA PARA VISUALIZAR LOS DX DE INGRESO PARA LA POSIBILIDAD DE ANEXARLOS COMO EGRESO/////////////
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function ConsultaDiagnosticoE()
    {
      ...
      $query = "select b.diagnostico_id, b.diagnostico_nombre, a.evolucion_id,
                a.tipo_diagnostico_id, a.sw_principal, a.tipo_diagnostico
      ...
      if ($dbconnect->ErrorNo() != 0)
      {
           $this->error = "Error al buscar en la consulta de diagnosticos de egreso";
           $this->mensajeDeError = "Error DB : " . $dbconnect->ErrorMsg();
           return false;
      }
      else
    ...
    function CambiarDiagnosticos()
    {
      ...
      list($dbconn) = GetDBconn();
            $query="SELECT a.tipo_diagnostico_id,
                  a.evolucion_id 
                    FROM hc_diagnosticos_egreso AS a, hc_evoluciones AS b 
      ...
      {
        $sql="UPDATE hc_diagnosticos_egreso
        SET sw_principal='0' 
        ...
        $sql="update hc_diagnosticos_egreso set sw_principal='1' where evolucion_id=".$_REQUEST['evolucion'.$pfj]." and tipo_diagnostico_id='".$_REQUEST['diagnostico_id'.$pfj]."';";
        $resulta=$dbconn->Execute($sql);
        if ($dbconn->ErrorNo() != 0)
        {
          $this->error = "Error al insertar en hc_diagnosticos_egreso";
    ...
    */
    function ObtenerFichas()
      ...
      $sql .= "FROM   hc_diagnosticos_egreso HI, ";
      $sql .= "       diagnosticos DG, ";
