...
/**
* Submodulo de Pronosticos.
*
* Submodulo para manejar los Pronosticos de un paciente en una evolución.
* @author Carlos A. Henao <cahenao99@yahoo.com.com>
* @version 1.0
* @package SIIS
* $Id: hc_Pronostico.php,v 1.2 2006/12/19 21:00:14 jgomez Exp $
*/
...
class Pronostico extends hc_classModules
{
	var $limit;
	var $conteo;
	var $capitulo='';
...
*/
	...
	*/
	function GetForma()
	{
		...
		else
		{
			...
			if($_REQUEST['accion'.$pfj]=='Insertar_Pronostico')
			{
				$this->InsertarPronostico();
				$this-> frmForma();
			}
	...
	*/
	...
	function ConsultarDescripcion()
	{
		$pfj=$this->frmPrefijo;
		list($dbconnect) = GetDBconn();
		$query = "SELECT fecha_registro, descripcion, pronostico
	     			  FROM hc_pronosticos
		    		  WHERE evolucion_id = ".$this->evolucion."
	...
	*/
	...
	function InsertarPronostico()
	{
		$pfj=$this->frmPrefijo;
		list($dbconnect) = GetDBconn();

		$observacion = $_REQUEST['observacion'.$pfj];
    $pronostico = $_REQUEST['pronostico'.$pfj];   
    $usuario_id=UserGetUID();
    //$fecha=date('y-m-d H:i:s');
    
// 		$query="SELECT evolucion_id FROM hc_pronosticos
// 				WHERE evolucion_id = ".$this->evolucion.";";
// 
// 		$result = $dbconnect->Execute($query);
// 		if ($dbconnect->ErrorNo() != 0)
...
/*		if (empty($evolucion))
		{*/
			$query = "INSERT INTO hc_pronosticos (pronostico, ingreso, evolucion_id,
...
/*		}
		...
		{
			$query = "UPDATE hc_pronosticos
					  SET decripcion = '".$observacion."'
					  WHERE evolucion_id=".$this->evolucion.";";
		}*/
		$result = $dbconnect->Execute($query);
		if ($dbconnect->ErrorNo() != 0)
