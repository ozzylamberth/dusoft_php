...
/**
 * $Id: ReporteRemision.report.php,v 1.3 2007/08/31 22:44:42 tizziano Exp $
...
class ReporteRemision_report
{
	var $datos;
	
	function ReporteRemision_report($datos=array())
	{
		$this->datos=$datos;
		return true;
	}
	...
	function CrearReporte()
	{
		...
		$HTML_WEB_PAGE.="	<UL TYPE='SQUARE'>";
		$HTML_WEB_PAGE.="		<LI>";
		$HTML_WEB_PAGE.="		$font1 <b>ESTADO GENERAL Y ENFERMEDAD ACTUAL</b> </font><br>";
		$l=0;
		...
		if(!empty($remision))
			$HTML_WEB_PAGE.="  REMITIDO A : ".$remision."<br>";
		
          if(!empty($concepto))
			$HTML_WEB_PAGE.="CONCEPTO CONTROL : ".$concepto."<br>";
          
          if(!empty($recepcion))
          	$HTML_WEB_PAGE.="ESPECIFIQUE RECEPCION : ".$recepcion."<br>";
	...
	function CabeceraImprimir()
	{
		...
		$this->imprimir.="<tr>\n";
		$this->imprimir.="<TD ALIGN=\"CENTER\" width=\"60%\"><FONT SIZE='5' FACE='arial'>HOJA DE REMISION Y CONTRA-REMISION<br></FONT>";
		$this->imprimir.="<br></TD>\n";
		$this->imprimir.="</TR>\n";
		$this->imprimir.="</table>";
		...
		$this->imprimir .= "<TR>";
		$this->imprimir .= "<TD COLSPAN=\"2\" ALIGN=\"JUSTIFY\" WIDTH=\"40%\"><FONT SIZE='$size' FACE='arial'><B>NOMBRE ACOMPAÑANTE: </B></FONT></TD>";
		...
		$this->imprimir.= "<TD COLSPAN=\"4\" ALIGN=\"JUSTIFY\" WIDTH=\"40%\"><FONT SIZE='$size' FACE='arial'><B>FECHA INGRESO:</B>&nbsp;&nbsp;".$FechaI." - ".$HoraI."</FONT></TD>";
	...
	function CargarVariables()
  {
	...
	function Datos_Ingreso()
	{
		...
		$query = "SELECT A.fecha_registro, A.departamento, A.departamento_actual,
                           A.fecha_cierre, C.descripcion 
                    FROM ingresos AS A, departamentos AS C
                    WHERE A.ingreso='".$this->ingreso."'
                    AND  A.departamento_actual = C.departamento;";
          $ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
          $result = $dbconn->Execute($query);
          $ADODB_FETCH_MODE = ADODB_FETCH_NUM;
		if($dbconn->ErrorNo() != 0)
