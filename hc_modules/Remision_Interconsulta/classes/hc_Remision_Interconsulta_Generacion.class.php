...
* @version 1.0
* @package SIIS
* $Id: hc_Remision_Interconsulta_Generacion.class.php,v 1.1 2007/08/28 15:31:30 tizziano Exp $
*/
...
class Generacion
{
	...
	function GetDatosMotivosConsulta($ingreso)
	{
		...
		$query="SELECT a.descripcion,
                  ...
                  LEFT JOIN hc_remision_interconsultas_motivo_enfermedad as b
                  ON
                  (
                      a.ingreso=b.ingreso
                  )
		...
		if($vars)
		{
			...
			$query="DELETE FROM hc_remision_interconsultas_motivo_enfermedad
			...
			$query="
                         INSERT INTO hc_remision_interconsultas_motivo_enfermedad
                         (
                              ingreso,
                              motivo_consulta,
                              enfermedad_actual,
	...
	function GetDatosEnfermedad($ingreso)
	{
		...
		$query="SELECT enfermedad_actual as enfermedadactual
			   FROM hc_remision_interconsultas_motivo_enfermedad
			   WHERE ingreso=$ingreso;";
	...
	function GetDatosExamenFisico($ingreso,$sw=null)
	{
		...
		$query="SELECT a.ingreso,
                         ...
                         a.sw_normal,
                         b.nombre
			   FROM hc_remision_interconsultas_examen_fisico as a
		...
		elseif($sw)
		{
			...
			if($vars)
			{
				$query="INSERT INTO hc_remision_interconsultas_examen_fisico
					   (
                              ingreso,
                              tipo_sistema_id,
                              sw_normal
	...
	function GetDatosExamenFisicoHallazgos($ingreso,$sw=null)
	{
		...
		$query="SELECT ingreso,
                         hallazgo,
                         fecha_registro
                  FROM hc_remision_interconsultas_examen_hallazgo
                  WHERE ingreso=$ingreso;";
		...
		elseif($sw)
		{
			...
			if($vars)
			{
				$query="INSERT INTO hc_remision_interconsultas_examen_hallazgo
					   (
                              ingreso,
                              hallazgo,
                              fecha_registro
	...
	function GetDatosApoyosD($ingreso,$tipoidpaciente,$pacienteid)
	{
		...
		$query="SELECT DISTINCT	
                         ...
                         LEFT JOIN hc_remision_interconsultas_apoyod as hea
                         ON
                         (
		...
		if($vars)
		{
			$query="DELETE FROM hc_remision_interconsultas_apoyod
				   WHERE ingreso=$ingreso";
			...
			$query="
                         INSERT INTO hc_remision_interconsultas_apoyod
                         (
                              ingreso,
                              apoyo_diagnostico,
                              fecha_registro
	...
	function InsertDatosApoyosD($datos,$ingreso)
	{
		...
		$query="INSERT INTO hc_remision_interconsultas_apoyod
                  (
                       ingreso,
                       apoyo_diagnostico,
                       fecha_registro
	...
	function InsertDatosExamenFisicoHallazgos($datos,$ingreso)
	{
		...
		$query="DELETE FROM hc_remision_interconsultas_examen_hallazgo
			   WHERE ingreso=$ingreso;";
	...
	function GetDatosAntecedentesPersonales($ingreso,$evolucion,$pacienteid,$tipoidpaciente,$sw=null,$ins=1)
	{
		list($dbconn) = GetDBconn();
		$dbconn->BeginTrans();
		...
		if($sw)
		{
			$sql = "SELECT	DISTINCT HT.nombre_tipo,
                      ...
                      ORDER BY HT.hc_tipo_antecedente_detalle_personal_id ASC;";
		...
		else
		{
			...
			if($vars AND $ins)
			{
				$query="INSERT INTO  hc_remision_interconsultas_antecedentes
                            (
                              ingreso, 
	...
	function GetDiagnosticos($ingreso,$nombre,$q1=null,$q2=null)
	{
		...
		if($q1)
		{
			$query=	"  SELECT hed.ingreso,
                                   ...
                                   END as tipo_diag,
                                   hed.estado
                              FROM hc_remision_interconsultas_diagnosticos_$nombre hed
		...
		if(!$q1)
		{
			$query="DELETE FROM hc_remision_interconsultas_diagnosticos_$nombre
				   WHERE ingreso=$ingreso;";
			...
			foreach($vars as $diag)
			{
				...
				$query="INSERT INTO  hc_remision_interconsultas_diagnosticos_$nombre
                                   (
                                        ingreso,
                                        diagnostico_id,
                                        tipo_diagnostico_id,
	...
	function UpdateDiagnoticoPrimario($ingreso,$nombre,$diag_id,$diag_id_primario,$estado)
	{
		...
		$query="UPDATE hc_remision_interconsultas_diagnosticos_$nombre
			   SET sw_principal='0'
			   WHERE ingreso=$ingreso
	...
	function GetDatosCausaSalida($ingreso)
	{
		...
		$query="SELECT a.ingreso,
                         a.hc_remision_tipo_causa_salida_id as tipo_causa_id,
                         b.descripcion as causa,
                         a.descripcion_remision,
                         a.recepcion,
                         a.concepto_control
                 FROM hc_remision_interconsultas_datos_salida AS a
                 JOIN hc_epicrisis_tipos_causa_salida as b
                 ON
                 (
                    a.hc_remision_tipo_causa_salida_id = b.hc_epicrisis_tipo_causa_salida_id
                 )
                 WHERE a.ingreso=$ingreso";
	...
	function InsertDatosSalida($datos,$ingreso)
	{
		...
		$query="DELETE FROM hc_remision_interconsultas_datos_salida
			   WHERE ingreso=$ingreso;";
		...
		$query="INSERT INTO  hc_remision_interconsultas_datos_salida
						(
							ingreso,
							hc_remision_tipo_causa_salida_id,
	...
	function GetDatosEpicrisis($ingreso)
	{
		...
		$query="
                    SELECT *
                    FROM hc_remision_interconsultas
                    WHERE ingreso=$ingreso
                 ";
	...
	function InsertEpicrisis($ingreso)
	{
		...
		$query="INSERT INTO  hc_remision_interconsultas
                         (
                              ingreso,
                              usuario_id,
                              fecha_registro
	...
	function InsertDatosMotivoConsulta($dato,$ingreso,$sw_insert,$sw_dato)
	{
		...
		if(!$sw_dato)
		{
			$query="UPDATE hc_remision_interconsultas_motivo_enfermedad
	...
	function InsertDatosAntecedentesPersonales($datos,$ingreso)
	{
		...
		$query="DELETE FROM hc_remision_interconsultas_antecedentes
			   WHERE ingreso=$ingreso";
		...
		for($i=0;$i<sizeof($datos);$i++)
		{
			...
			{
				$query="UPDATE hc_remision_interconsultas_antecedentes
