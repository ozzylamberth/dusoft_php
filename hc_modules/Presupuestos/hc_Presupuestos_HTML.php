...
// $Id: hc_Presupuestos_HTML.php,v 1.25 2007/03/12 16:10:01 tizziano Exp $

class Presupuestos_HTML extends Presupuestos
{

	function Presupuestos_HTML()
	{
		$this->Presupuestos();//constructor del padre
		return true;
	}
	...
	{
		$pfj=$this->frmPrefijo;
		$this->salida =ThemeAbrirTablaSubModulo('PLAN DE TRATAMIENTO Y PRESUPUESTO');
		...
		{/*2,3,4,8*/
			...
			{
				$this->salida.="<td align=\"center\" $color colspan=\"8\">";
				...
				for($k=0;$k<$ciclo1;$k++)
				{
					...
					if(!empty($validados))
					{
						...
						$this->salida.="	</td>";
						$this->salida.="	<td align=\"center\" width=\"38%\">";
						$this->salida.="".$cargos[$k]['descripcion']."";
						$detalle=$this->BuscarCargosActivo(1,$cargos[$k]['cargo']);
						$ciclo2=sizeof($detalle);
						for($w=0;$w<$ciclo2;$w++)
						{
							$accion=ModuloHCGetURL($this->evolucion,$this->paso,0,'',false,array('accion'.$pfj=>'cancelar','odondetadi'.$pfj=>$detalle[$w]['hc_odontograma_primera_vez_detalle_id']));
							$this->salida.="".' -- '."<a href=\"$accion\">".$detalle[$w]['hc_tipo_ubicacion_diente_id']."".'-'."".$detalle[$w]['descripcion']."</a>";//<img src=\"".GetThemePath()."/images/elimina.png\"  border='0'>
						}
						$this->salida.="	</td>";
				...
				$cargos =$this->BuscarCargosPresInActivo($tiposplanes[$i]['hc_tipo_plan_tratamiento_id']);
				$ciclo1=sizeof($cargos);
				if(!($ciclo1==0 OR $cargos==NULL))
				{
					for($k=0;$k<$ciclo1;$k++)
					{
						...
						if(!empty($validados))
						{
							...
							$this->salida.="	<td align=\"center\" width=\"38%\">";
							$this->salida.="".$cargos[$k]['descripcion']."";
							$detalle=$this->BuscarCargosActivo(2,$cargos[$k]['cargo']);
							$ciclo2=sizeof($detalle);
							for($w=0;$w<$ciclo2;$w++)
							{
								$accion=ModuloHCGetURL($this->evolucion,$this->paso,0,'',false,array('accion'.$pfj=>'activar','odondetadi'.$pfj=>$detalle[$w]['hc_odontograma_primera_vez_detalle_id']));
								$this->salida.="".' -- '."<a href=\"$accion\">".$detalle[$w]['hc_tipo_ubicacion_diente_id']."".'-'."".$detalle[$w]['descripcion']."</a>";//<img src=\"".GetThemePath()."/images/elimina.png\"  border='0'>
							}
							$this->salida.="	</td>";
							$this->salida.="	<td align=\"center\" width=\"6%\">";
							$this->salida.="<strike>".$cargos[$k]['cantidad']."</strike>";
							$this->salida.="	</td>";
							$this->salida.="	<td align=\"right\" width=\"10%\">";
							$this->salida.="<strike>".$resul['cargos'][0]['precio_plan']."</strike>";
							$this->salida.="	</td>";
							$this->salida.="	<td align=\"right\" width=\"10%\">";
							$this->salida.="<strike>".$resul['cargos'][0]['valor_cargo']."</strike>";
							$this->salida.="	</td>";
							$this->salida.="	<td align=\"right\" width=\"10%\">";
							$this->salida.="<strike>".$resul['valor_total_paciente']."</strike>";
							$this->salida.="	</td>";
							$this->salida.="	<td align=\"right\" width=\"10%\">";
							$this->salida.="<strike>".$resul['valor_total_empresa']."</strike>";
							$this->salida.="	</td>";
							$this->salida.="	<td align=\"center\" width=\"8%\">";
							$this->salida.="CANCE.";
							$this->salida.="	</td>";
				...
				if($tiposplanes[$i]['hc_tipo_plan_tratamiento_id']==1)
				{
					...
					'tipo_plan_tratam'.$pfj=>$tiposplanes[$i]['hc_tipo_plan_tratamiento_id']));
					$this->salida.='<form name="forma'.$pfj.'" action="'.$accion.'" method="post">';
					$this->salida.="<td align=\"center\" $color>";
					$this->salida.="<input class=\"input-submit\" name=\"volver$pfj\" type=\"submit\" value=\"CARGOS\">";
					$this->salida.="</td>";
					$this->salida.="</form>";
					$this->salida.="</td>";
					$this->salida.="</tr>";
	...
	function frmConsulta()
	{
		...
		if(sizeof($cuentas)!=0)
		{
			...
			{/*2,3,4,8*/
				...
				{
					$this->salida.="<td align=\"center\" $color colspan=\"8\">";
					$cargos =$this->BuscarCargosPresActivoConsulta($tiposplanes[$i]['hc_tipo_plan_tratamiento_id']);
					$ciclo1=sizeof($cargos);
					for($k=0;$k<$ciclo1;$k++)
					{
						$validados=ValdiarEquivalencias($this->plan,$cargos[$k]['cargo']);
						if(!empty($validados))
						{
							...
							$this->salida.="	<td align=\"center\" width=\"38%\">";
							$this->salida.="".$cargos[$k]['descripcion']."";
							$detalle=$this->BuscarCargosActivoConsulta(1,$cargos[$k]['cargo']);
							$ciclo2=sizeof($detalle);
							for($w=0;$w<$ciclo2;$w++)
							{
					...
					$cargos =$this->BuscarCargosPresInActivoConsulta($tiposplanes[$i]['hc_tipo_plan_tratamiento_id']);
					$ciclo1=sizeof($cargos);
					if(!($ciclo1==0 OR $cargos==NULL))
					{
						for($k=0;$k<$ciclo1;$k++)
						{
							...
							if(!empty($validados))
							{
								...
								$this->salida.="	<td align=\"center\" width=\"38%\">";
								$this->salida.="".$cargos[$k]['descripcion']."";
								$detalle=$this->BuscarCargosActivoConsulta(2,$cargos[$k]['cargo']);
								$ciclo2=sizeof($detalle);
								for($w=0;$w<$ciclo2;$w++)
								{
									$this->salida.="".' -- '."".$detalle[$w]['hc_tipo_ubicacion_diente_id']."".'-'."".$detalle[$w]['descripcion']."";
					...
					if($tiposplanes[$i]['hc_tipo_plan_tratamiento_id']==1)
					{
						$this->salida.="<tr $color>";
						$this->salida.="<td align=\"center\" $color colspan=\"8\">";
						$cargos =$this->BuscarPlanTratamientoCargoConsulta($tiposplanes[$i]['hc_tipo_plan_tratamiento_id']);
						$ciclo1=sizeof($cargos);
						for($k=0;$k<$ciclo1;$k++)
						{
							$validados=ValdiarEquivalencias($this->plan,$cargos[$k]['cargo']);
				...
				{
					$this->salida.="<td align=\"center\" $color colspan=\"8\">";
					$cargos =$this->BuscarApoyosOdontologiaConsulta();
					$ciclo1=sizeof($cargos);
					for($k=0;$k<$ciclo1;$k++)
					{
						$validados=ValdiarEquivalencias($this->plan,$cargos[$k]['cargo']);
	...
	function frmHistoria()
	{
		...
		if(sizeof($cuentas)!=0)
		{
			...
			$salida.="CARGO";
			$salida.="</td>";
			$salida.="<td width=\"34%\" align=\"center\">";
			...
			{/*2,3,4,8*/
				...
				$tiposplanes[$i]['hc_tipo_plan_tratamiento_id']==8)
				{
					$salida.="<td align=\"center\" $color colspan=\"8\">";
					$cargos =$this->BuscarCargosPresActivoConsulta($tiposplanes[$i]['hc_tipo_plan_tratamiento_id']);
					$ciclo1=sizeof($cargos);
					for($k=0;$k<$ciclo1;$k++)
					{
						...
						if(!empty($validados))
						{
							...
							$salida.="	</td>";
							$salida.="	<td align=\"center\" width=\"38%\">";
					...
					if(!($ciclo1==0 OR $cargos==NULL))
					{
						for($k=0;$k<$ciclo1;$k++)
						{
							...
							if(!empty($validados))
							{
								...
								$salida.="	</td>";
								$salida.="	<td align=\"center\" width=\"6%\">";
								$salida.="<strike>".$cargos[$k]['cantidad']."</strike>";
								$salida.="	</td>";
								$salida.="	<td align=\"right\" width=\"10%\">";
								$salida.="<strike>".$resul['cargos'][0]['precio_plan']."</strike>";
								$salida.="	</td>";
								$salida.="	<td align=\"right\" width=\"10%\">";
								$salida.="<strike>".$resul['cargos'][0]['valor_cargo']."</strike>";
								$salida.="	</td>";
								$salida.="	<td align=\"right\" width=\"10%\">";
								$salida.="<strike>".$resul['valor_total_paciente']."</strike>";
								$salida.="	</td>";
								$salida.="	<td align=\"right\" width=\"10%\">";
								$salida.="<strike>".$resul['valor_total_empresa']."</strike>";
								$salida.="	</td>";
								$salida.="	<td align=\"center\" width=\"8%\">";
								$salida.="CANCE.";
								$salida.="	</td>";
