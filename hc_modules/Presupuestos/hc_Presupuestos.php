...
* @version 1.0
* @package SIIS
* $Id: hc_Presupuestos.php,v 1.25 2007/07/09 19:20:54 tizziano Exp $
*/
...
class Presupuestos extends hc_classModules
{
	var $limit;
	var $conteo;
...
*/
	...
	function BuscarCargosActivo($estado,$cargo)
	{
		list($dbconn) = GetDBconn();
		...
		hc_tipos_cuadrantes_dientes AS C,
		...
		AND B.hc_tipo_cuadrante_id=C.hc_tipo_cuadrante_id
		AND B.hc_tipo_problema_diente_id=D.hc_tipo_problema_diente_id
		AND B.hc_tipo_producto_diente_id=D.hc_tipo_producto_diente_id
		AND B.estado='".$estado."'
		AND D.cargo='".$cargo."'
	...
	function BuscarCargosActivoConsulta($estado,$cargo)
	{
		list($dbconn) = GetDBconn();
		$query="SELECT B.hc_odontograma_primera_vez_detalle_id,
		B.hc_tipo_ubicacion_diente_id,
	...
	function BuscarCargosPresActivoConsulta($tipoplan)
	{
		list($dbconn) = GetDBconn();
		$query="SELECT MAX(hc_odontograma_primera_vez_id)
		FROM hc_odontogramas_primera_vez
		WHERE tipo_id_paciente='".$this->tipoidpaciente."'
		AND paciente_id='".$this->paciente."'
		AND sw_activo='0';";//evolucion_id=".$this->evolucion." AND
		$resulta = $dbconn->Execute($query);
		if($dbconn->ErrorNo() != 0)
		{
			$this->error = "Error al Cargar el Modulo";
	...
	function BuscarCargosPresInActivoConsulta($tipoplan)
	{
		list($dbconn) = GetDBconn();
		$query="SELECT MAX(hc_odontograma_primera_vez_id)
		FROM hc_odontogramas_primera_vez
	...
	function BuscarApoyosOdontologia()
	{
		...
		C.descripcion,
		D.descripcion_ubicacion,
		D.cantidad
		FROM hc_odontogramas_primera_vez AS A,
		hc_odontogramas_apoyod AS B,
		cups AS C,
		hc_odontogramas_primera_vez_apoyod AS D
		WHERE B.cargo=C.cargo
		AND B.cargo=D.cargo
		AND D.estado='1'
		AND D.hc_odontograma_primera_vez_id=A.hc_odontograma_primera_vez_id
	...
	function BuscarApoyosOdontologiaConsulta()
	{
		$pfj=$this->frmPrefijo;
		list($dbconn) = GetDBconn();
		$query="SELECT B.cargo,
		...
		D.descripcion_ubicacion,
		D.cantidad
		FROM hc_odontogramas_apoyod AS B,
		cups AS C,
		hc_odontogramas_primera_vez_apoyod AS D
		WHERE B.cargo=C.cargo
		AND B.cargo=D.cargo
		AND D.estado='1'
		AND D.hc_odontograma_primera_vez_id=
		(SELECT MAX(A.hc_odontograma_primera_vez_id)
		FROM hc_odontogramas_primera_vez AS A
		WHERE A.tipo_id_paciente='".$this->tipoidpaciente."'
		AND A.paciente_id='".$this->paciente."'
	...
	function BuscarPlanTratamientoCargo($tipoplan)
	{
		...
		$query="SELECT B.cargo,
		B.descripcion,
		C.hc_tipo_plan_tratamiento_id,
		D.cantidad
		FROM hc_odontogramas_primera_vez AS A,
		cups AS B,
		hc_tipos_plan_tratamiento_cargo AS C,
		hc_odontogramas_primera_vez_presupuesto AS D
		WHERE C.hc_tipo_plan_tratamiento_id=".$tipoplan."
		AND C.cargo=B.cargo
		AND C.cargo=D.cargo
		AND D.estado='1'
		AND D.hc_odontograma_primera_vez_id=A.hc_odontograma_primera_vez_id
	...
	function BuscarPlanTratamientoCargoConsulta($tipoplan)
	{
		$pfj=$this->frmPrefijo;
		list($dbconn) = GetDBconn();
		$query="SELECT B.cargo,
