...
/**
* $Id: hc_PlanTerapeuticoCExterna_CDA.php,v 1.1 2005/07/25 18:31:40 tizziano Exp $
*/

/**
* Clase para consulta CDA-HL7 del submodulo PlanTerapeuticoCExterna
*
* @author Tizziano Perea <tperea@ipsoft-sa.com>
* @version $Revision: 1.1 $
* @package SIIS
*/ 
class PlanTerapeuticoCExterna_CDA extends Extenciones_CDA_HC
{
    /**
    ...
    * @access public    
    */  
    function PlanTerapeuticoCExterna_CDA()
    {
    ...
    */     
		...
		foreach($XML_Consulta as $k => $v)
          {
               ...
               $salida.="<TBODY>";
               $salida.="<TR>";
               $salida.="<TD width=\"30%\" align=\"justify\" colspan=\"3\">Via de Administración:&nbsp;".$v[via];
    ...
    function GetConsultaSubmodulo($Paramdatos, $ParamTipo)
    {
          ...
          {
               case '1':
               $this->BuscarIngreso($Paramdatos[evolucion]);
               $query="SELECT k.sw_uso_controlado, CASE WHEN k.sw_pos = 1 THEN 'POS'
               			ELSE 'NO POS' END AS item, a.codigo_producto, a.sw_paciente_no_pos,
                              a.cantidad, a.dosis, m.nombre AS via, a.unidad_dosificacion, 
                              a.observacion, a.tipo_opcion_posologia_id, h.descripcion AS producto,
                              c.descripcion AS principio_activo, h.contenido_unidad_venta,
                              l.descripcion, a.evolucion_id 
               ...
               while($medicamentos_amb = $resultado->FetchRow())
               {
                    $XML_Consulta[] = $medicamentos_amb;
               }
