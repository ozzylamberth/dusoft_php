<?php
	/**************************************************************************************
	* $Id: ConsultaAutorizaciones.class.php,v 1.3 2009/11/13 12:05:25 hugo Exp $
	...
	***************************************************************************************/
	class ConsultaAutorizaciones
	{
      ...
      if($datos['autorizacion'])
			{
				$contar = false;
				$sql .= "AND		AU.autorizacion = ".$datos['autorizacion']." ";
			}
		...
		***********************************************************************************/
		function ObtenerAutorizacionesLista($datos)
		{
			...
			$sql .= " WHEN IG.estado='2' THEN 'SALIDA' END ";
		  $sql .= " AS estado, ";
			$sql .= "				PA.primer_apellido||' '||PA.segundo_apellido AS apellidos, ";
			$sql .= "				PA.primer_nombre||' '||PA.segundo_nombre AS nombres, ";
			$sql .= "				PA.tipo_id_paciente, ";
			...
			return $ingresos;
				...
				/**********************************************************************************
						*
								***********************************************************************************/
	function ObtenerAutorizacionesListaUsaurio()
	{
	  $sql  = "SELECT * ";
	  $sql .= "FROM		usuarios_ncautorizaciones_estados_ingresos ";
	  $sql .= "WHERE	usuario_id =".UserGetUID().""; 
          if(!$rst = $this->ConexionBaseDatos($sql)) return false;													  if(!$rst->EOF)								  $cont = $rst->RecordCount();						          $rst->Close();
    ...
    ***********************************************************************************/
    ...
    /**
		*
		*/
		function ObtenerInformacionAutorizacion($datos,$tipo)
		{
			$sql  = "SELECT COALESCE(PL.plan_descripcion,'PLAN NO ESPECIFICADO') AS plan_descripcion, ";			
			$sql .= "				PL.plan_id, ";
			$sql .= "				AU.autorizacion, ";
			$sql .= "				AU.tipo_afiliado_id,";
      $sql .= " 	    AU.semanas_cotizadas,";
      $sql .= " 	    AU.rango, ";
			$sql .= "				AU.observaciones, ";
			$sql .= "				AU.tipo_autorizador, ";
			$sql .= "				AU.codigo_autorizacion, ";
			...
			$sql .= "				autorizaciones_clases AC, ";
			$sql .= "				autorizaciones_tipos AT ";
			$sql .= "WHERE	AU.usuario_id = SU.usuario_id ";
			$sql .= "AND		AU.clase_autorizacion = AC.clase_autorizacion ";
			$sql .= "AND		AU.tipo_autorizacion = AT.tipo_autorizacion ";
			$sql .= "AND		AU.autorizacion = ".$datos['autorizacion']." ";
			...
			if(!$rst->EOF)
			{
				$autorizaciones = $rst->GetRowAssoc($ToUpper = false);
				$rst->MoveNext();
			}
			$rst->Close();
			return $autorizaciones;
