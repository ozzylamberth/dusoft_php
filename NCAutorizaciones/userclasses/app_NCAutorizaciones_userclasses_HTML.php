<?php
	/**************************************************************************************
	* $Id: app_NCAutorizaciones_userclasses_HTML.php,v 1.3 2009/11/13 12:05:25 hugo Exp $
	...
	***************************************************************************************/
	class app_NCAutorizaciones_userclasses_HTML extends app_NCAutorizaciones_user
	{
		...
		*************************************************************************************/
		...
		/**
    * Funcion donde se realiza la validacion del la autorizacion
    *
		* @return boolean
		*/
      ...
      {
        if(!empty($this->auto_anteriores))
        {
          $datos['autorizacion'] = $this->auto_anteriores['autorizacion_int'];
          $this->FormaAutorizacionAnterior($this->action,$datos);
				}
        else
        {
		...
		*************************************************************************************/
		function FormaCrearAutorizacion($datos,$cargos)
		{
			...
			$this->salida .= $authtml->FormaAutorizarPaciente($this->request['plan_id'],$this->request['idp'],$this->request['tipoid'],$this->action,$this->request['afiliados'],$this->request['externo'],$this->request['ingreso']);
    ...
    /*************************************************************************************
		* @access public
		*************************************************************************************/
		function FormaAutorizacionAnterior($action,$datos)
		{
      $cnt = AutoCarga::factory('ConsultaAutorizaciones','classes','app','NCAutorizaciones');
			$mdl = AutoCarga::factory('ConsultaAutorizacionesHTML','classes','app','NCAutorizaciones');
			$autorizacion = $cnt->ObtenerInformacionAutorizacion($datos);

      $rqst['tipoafiliado'] = $autorizacion['tipo_afiliado_id'];	;
      $rqst['semanas'] = $autorizacion['semanas_cotizadas'];
      $rqst['rango'] = $autorizacion['rango'];
      $rqst['numero_autorizacion']= $autorizacion['autorizacion'];
			$rqst['plan_id']= $autorizacion['plan_id'];
			$rqst['paciente_id']= $paciente['idp'];
			$rqst['tipo_id_paciente']= $paciente['tipoid'];
			
			$this->action['continuar'] .= URLRequest(array("autorizacion"=>$rqst));
      
      $this->salida .= $mdl->FormaInformacionautorizacion($this->action,$autorizacion);
			return true;
		}
		/*************************************************************************************
		* @access public
