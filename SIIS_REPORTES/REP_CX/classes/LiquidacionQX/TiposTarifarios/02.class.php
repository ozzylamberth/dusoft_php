...
/**
* $Id: 02.class.php,v 1.15 2006/08/15 16:55:48 alex Exp $
*/
...
class TipoLiquidacionQX_02 extends LiquidacionCargos
{
        ...
        if($tV)
        {
            foreach($this->datosQX as $NumCirujano=>$procedimientos)
            {
                foreach($procedimientos as $NumProcedimiento=>$datos)
                {
                    ...
                    if(($valor_uvrs = $this->GetValoresUVRSxPlan($this->GetDatosPlan('plan_id'), $this->datosQX[$NumCirujano][$NumProcedimiento]['tarifario_id'], $clase=$tipo_cargo_qx_id, $tipo='', $NumCirujano))===false) return false;
    ...
    * @access private
    */
    function GetValoresUVRSxPlan($plan_id, $tarifario_id, $clase='', $tipo='', $NumCirujano=null)
    {
        static $ValoresUVRS;
        ...
        $profesional=$this->GetTerceroActoQX('DC',1);
        ...
        if(!$result->EOF)
        {
            ...
            {
                $ValoresUVRS[$plan_id][$tarifario_id]['DC']['VALOR']=$v['dg_valor'];
            }
        ...
        if(empty($ValoresUVRS[$plan_id][$tarifario_id]['DC']['VALOR']))
        {
            if($tipo_profesional == '1')
            {
                $ValoresUVRS[$plan_id][$tarifario_id]['DC']['VALOR']=$uvrs_tarifario['dc_valor'];
            }
            else
            {
                $ValoresUVRS[$plan_id][$tarifario_id]['DC']['VALOR']=$uvrs_tarifario['dg_valor'];
            }
        }
        ...
        {
            $ValoresUVRS[$plan_id][$tarifario_id]['DC']['CARGO']=$uvrs_tarifario['dg_cargo'];
        }
        ...
        {
            $ValoresUVRS[$plan_id][$tarifario_id]['DC']['CUPS']=$uvrs_tarifario['dg_cups'];
        }
