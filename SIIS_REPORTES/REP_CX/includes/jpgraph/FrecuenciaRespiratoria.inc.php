...
/**
 * $Id: FrecuenciaRespiratoria.inc.php,v 1.2 2005/06/07 18:28:17 leydi Exp $
...
function GraficarFrecuenciaRespiratoria($Vmax,$Vmin,$DatosF,$todos,$fechaE,$DatosH)
{
	...
	$seq=$result->fields[0];
	$Dir="cache/FrecuenciaRespiratoria$seq.png";

	if(!$todos){
		foreach($DatosH as $key =>$value){
			$ydata[count($ydata)]=$value['fr_respiratoria'];
...
*/
		...
		$graph->title->Set("FRECUENCIA RESPIRATORIA");
		$graph->xaxis->title->Set($fechaE);

		$text=new Text();
		$text->Set("Frecuencia Respiratoria");
