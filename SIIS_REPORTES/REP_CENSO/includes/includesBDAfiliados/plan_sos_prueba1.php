...
/**
 * $Id: plan_sos_prueba1.php,v 1.4 2005/06/07 19:13:17 leydi Exp $
 ...
 */
	...
	function ConfigurarBD(&$datos)
	{
		...
		{
			$sql="exec bdSiSalud.dbo.spPmBuscaValidacionCaja '".$Prestador."', '".$noAutorizacion."'";
			return $sql;
		}
		...
		function SqlGetTraerSedes($departamento)
		{
			...
			while(!$result->EOF)
			{
				...
				if($result1->fields[0]==trim($a['cdgo_ofcna']))
				{
					$datos[trim($a['cdgo_ofcna'])]=array('descripcion'=>$a['dscrpcn_ofcna'],'activo'=>1);
				}
				else
				{
					$datos[$a['cdgo_ofcna']]=array('descripcion'=>$a['dscrpcn_ofcna'],'activo'=>0);
				}
				$result->MoveNext();
			}
			$result1->close();
