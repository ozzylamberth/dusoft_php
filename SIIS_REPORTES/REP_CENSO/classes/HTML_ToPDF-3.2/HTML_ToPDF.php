...
//$Id: HTML_ToPDF.php,v 1.8 2005/03/08 22:51:27 alex Exp $
...
class HTML_ToPDF {
    ...
    function convert()
    {
        ...
        //echo $this->_getPaperSize();
        $paperSize = $this->_getPaperSize();
        //echo $paperSize = "halfletter";
        $orientation = $this->_getOrientation();
       // $orientation = '--landscape';
        // try and replace relative images with the default domain
        $this->_htmlString = preg_replace(':<img (.*?)src=["\']((?!http\://).*?)["\']:i', '<img \\1 src="'.$this->defaultDomain.'\\2"', $this->_htmlString);
...
//         
        ...
        $this->html2psrc = '
        @html2ps { option { titlepage: 0;  toc: 0;  colour: 0;  underline: 1; /* underline links */ grayscale: 0; /* Make images grayscale? */ scaledoc: 1; /* Scale the document */ } package { geturl: /usr/bin/curl -i; /* path to the geturl */ } showurl: 0; /* do not show the url next to links */  @page { size: 5.5in 8.5in; margin: 1cm;}}   
        BODY {
    ...
    function _getCSSFromFile()
    {
        if ($this->useCSS) {
            ...
            elseif (preg_match(':<link .*? href=["\'](.*?)["\'].*?text/css.*?>:i', $this->_htmlString, $matches)) {
                $cssFound = preg_replace(':(^(?!http\://).*):i', ''.$this->defaultDomain.'\\1', $matches[1]);
