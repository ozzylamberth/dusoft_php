	 ...
	 if(!$Configb['spi'])
   {
		...
		$busca=strtolower(rtrim($busca));  // quita espacios al final de la cadena  que se esta buscando.....
              ...
              $limit=$this->limit;
					...
					if($_SESSION['BUSCADOR']['DEPTO']!='-1'){
						$filtro_sql=", departamentos_cargos b ";
						$filtro.= "AND a.cargo = b.cargo  AND b.departamento = '".$_SESSION['BUSCADOR']['DEPTO']."'";
					}
					if($ConsultaConteo==true)
					{
							$resultin = BuscardoCargosCups($filtro, $campos_select,'','',$filtro_sql);
              ...
              $limit=$this->limit;
					...
					$result = BuscardoCargosCups($filtro, $campos_select,$Of,$limit, $filtro_sql);
					//print_r($result);
      ...
      while (!$result->EOF)
          ...
          $spy="0";
    		...
    		$m=1;
				for($q=0;$q<$values;$q++)
				{
    			$Separador.=htmlspecialchars($result->fields[$q]);
