...
/** $Id: rips.class.php,v 1.51 2006/12/11 18:54:00 carlos Exp $
*/
class rips
{
    var $error;
...
";
    /*
,
            RCD.ap_diagnostico_post_qx AS diagnostico_ppal,
            RCD.ap_diagnostico_post_qx2 AS diagnostico_relacionado,
            RCD.ap_diagnostico_id_complicacion AS complicacion,
            RCD.ap_forma_realizacion AS forma_realizacion
					...
					" SELECT a.*
						...
						--,a.cargo as cargo_tarifario
									...
									AND c.codigo_producto=d.codigo_producto        
										...
										GROUP BY c.codigo_producto,cargo_nom,
										a.facturado,a.precio,unidad,a.cargo,a.valor_cargo,a.valor_nocubierto,
										a.valor_cubierto,med.codigo_medicamento,a.autorizacion_ext,
    ...
    */
    function CallConstruyeAC(&$arregloAC,$d_envio,$datos_prestador_servicio,$datos_usuario,$autorizacion,$datos_entidad_administradora,$datos_cups,&$vectorAD)
    {
        ...
        if(empty($d_envio[cargo_manual]))
        {
            ...
            if(empty($datoscita[0][tipo_finalidad_id]))
    ...
    */
    function CallConstruyeAU(&$arregloAU,$d_envio,$datos_prestador_servicio,$datos_usuario,$autorizacion)
    {
        ...
        if(empty($d_envio[cargo_manual]))
        {
            ...
            if($datoscama[tipo_clase_cama_id] == 3 OR $datoscama[tipo_clase_cama_id] == 2)
            {   
...
//---------------------ENVIOS-----------------------------------------------------
    function GetCuentasEnvio($envio)
    {
			list($dbconn) = GetDBconn();
...
echo "<pre>";
        ...
        $dat=$this->GetActoQuirurgico($arregloenvio[0][envio],$ConsultaTipoRips,$arregloenvio,$datos_prestador_servicio,&$arregloAP,&$vectorAD);

        $dat1=explode('\*/', $dat);
        $arregloAP=$dat1[0];//actos para ap
        $ap=$dat1[1];       //lineas de ap

				$cuentas_envio = $this->GetCuentasEnvio($arregloenvio[0][envio]);
				if($cuentas_envio)
				{
...
//         if (!$this->ConstruirIMD($detalle_envio,$datos_prestador_servicio,&$am,&$at,&$arregloAM,&$arregloAT,&$vectorAD)) 
//         {
