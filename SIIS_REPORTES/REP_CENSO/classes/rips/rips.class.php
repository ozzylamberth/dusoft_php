...
// -------------------------------------------------------------------------------------
/** $Id: rips.class.php,v 1.54 2007/03/02 13:45:01 carlos Exp $
*/
class rips
{
    var $error;
...
*/
    ...
    *
    */
    function GetDetalleEnvio($envio,$aq,$reg)
    {
        $sql="";
        if($aq)
        {
            $sql=" AND tarifario_id<>'SYS'";
        }
        list($dbconn) = GetDBconn();
    echo    $query = "SELECT * 
                  FROM tmp_rips_cuentas_detalle
                  WHERE tmp_rips_cuentas_detalle_id=".$reg."
                  $sql ;";
/*
        $sql="";
        if($aq)
...
//*********************************************
        ...
        $dat=$this->GetActoQuirurgico($arregloenvio[0][envio],$ConsultaTipoRips,$arregloenvio,$datos_prestador_servicio,&$arregloAP,&$vectorAD,$cont);
...
//         }
        ...
        for($lim=1; $lim<=$cont; $lim++)
				{
					$detalle_envio= $this->GetDetalleEnvio($arregloenvio[0][envio],false,$lim);
...
*/
    ...
    function GetActoQuirurgico($detalle_envio,$ConsultaTipoRips,$arregloenvio,$datos_prestador_servicio,&$arregloAP,&$vectorAD,$cont)
    {
        //$detalle=$this->GetDetalleEnvio($detalle_envio,'aq');
        for($lim=1; $lim<=$cont; $lim++)
				{
					$detalle=$this->GetDetalleEnvio($detalle_envio,true,$lim);
