...
/**
 * $Id: hojacargos2.inc.php,v 1.16 2006/03/17 15:39:52 lorena Exp $
 ...
 */
  ...
  function CargosFacturaHoja2($cuenta,$noFacturado)
  {
				if($noFacturado=='1'){
					$filFacturado=" and a.facturado='0'";
				}else{
        ...
        $querys = "select a.*, b.grupo_tipo_cargo, b.descripcion as desccargo,
										c.descripcion,ter.nombre_tercero,z.cargo as cargoliquidacion,z.descripcion as descargoliquidacion,usu.usuario
  ...
  function GenerarHojaCargos2($datos)
  {
          ...
          for($i=0; $i<sizeof($ins);)
          {
              ...
              while($ins[$i][bodega]==$ins[$d][bodega])
              {
									...
									while($ins[$k][codigo_agrupamiento_id]==$ins[$d][codigo_agrupamiento_id])
									{
											...
											$html.="<tr><td width=60>".FechaStamp($ins[$k][fecha_cargo])."</td><td width=60 align='CENTER'>".$ins[$k][codigo_producto]."</td><td width=50 align=\"CENTER\">&nbsp;</td><td width=50 align=\"CENTER\">".$ins[$k][departamento]."</td><td width=180>".substr($ins[$k][descripcion],0,37)."</td><td width=20 align=\"CENTER\">&nbsp;</td><td width=40 align=CENTER>".FormatoValor($ins[$k][cantidad])."</td><td width=60 align=\"CENTER\">".FormatoValor($ins[$k][precio])."</td><td width=60 align=\"CENTER\">".FormatoValor($ins[$k][valor_cargo])."</td><td width=60 align=\"CENTER\">".FormatoValor($ins[$k][valor_cubierto])."</td><td width=60 align=\"CENTER\">".FormatoValor($valpac)."</td><td width=40 align=\"CENTER\">".$ins[$k][transaccion]."</td><td width=40 align=\"CENTER\">".$ins[$k][usuario]."</td></tr>";
      ...
      $html.="<tr><td width=150>CARGO A CUENTA DE: </td><td width=370>".$dat[nombre_tercero]."</td><td width=240 align=\"RIGHT\">".FormatoValor($dat[valor_total_empresa])."</td></tr>";
			...
			$var=CargosFacturaHoja2($datos[numerodecuenta],$noFacturado='1');		
			$ins=InsumosCuentaC2($datos[numerodecuenta],$noFacturado='1');	
			$insDev=DevolucionesInsumosCuentaC2($datos[numerodecuenta],$noFacturado='1');			
			if(!empty($var)){
				$total=$descuentos=$pagado=0;
				$direc='';
				$totalcar=$totalins=0;
			...
			if($ins){	
				...
				for($i=0; $i<sizeof($ins);){
					...
					while($ins[$i][bodega]==$ins[$d][bodega]){
						...
						while($ins[$k][codigo_agrupamiento_id]==$ins[$d][codigo_agrupamiento_id]){
							$valpac=$ins[$k][valor_nocubierto];
							$html.="<tr><td width=60>".FechaStamp($ins[$k][fecha_cargo])."</td><td width=60 align='CENTER'>".$ins[$k][codigo_producto]."</td><td width=50 align=\"CENTER\">&nbsp;</td><td width=50 align=\"CENTER\">".$ins[$k][departamento]."</td><td width=180>".substr($ins[$k][descripcion],0,37)."</td><td width=20 align=\"CENTER\">&nbsp;</td><td width=40 align=CENTER>".FormatoValor($ins[$k][cantidad])."</td><td width=60 align=\"CENTER\">0</td><td width=60 align=\"CENTER\">0</td><td width=60 align=\"CENTER\">0</td><td width=60 align=\"CENTER\">0</td><td width=40 align=\"CENTER\">".$ins[$k][transaccion]."</td><td width=40 align=\"CENTER\">".$ins[$k][usuario]."</td></tr>";
							$total+=$ins[$k][total_costo];
							$cant+=$ins[$k][cantidad];
							$valor+=$ins[$k][valor_cargo];
							$totalins+=$ins[$k][valor_cargo];
				...
				for($i=0; $i<sizeof($insDev);){
					...
					while($insDev[$i][bodega]==$insDev[$d][bodega]){
						...
						while($insDev[$k][codigo_agrupamiento_id]==$insDev[$d][codigo_agrupamiento_id]){
							...
							$html.="<tr><td width=60>".FechaStamp($insDev[$k][fecha_cargo])."</td><td width=60 align='CENTER'>".$insDev[$k][codigo_producto]."</td><td width=50 align=\"CENTER\">&nbsp;</td><td width=50 align=\"CENTER\">".$insDev[$k][departamento]."</td><td width=180>".substr($insDev[$k][descripcion],0,37)."</td><td width=20 align=\"CENTER\">&nbsp;</td><td width=40 align=CENTER>".FormatoValor($insDev[$k][cantidad])."</td><td width=60 align=\"CENTER\">0</td><td width=60 align=\"CENTER\">0</td><td width=60 align=\"CENTER\">0</td><td width=60 align=\"CENTER\">0</td><td width=40 align=\"CENTER\">".$insDev[$k][transaccion]."</td><td width=40 align=\"CENTER\">".$insDev[$k][usuario]."</td></tr>";
