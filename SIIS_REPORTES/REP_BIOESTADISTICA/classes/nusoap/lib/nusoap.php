...
/*
$Id: nusoap.php,v 1.1 2005/07/13 18:31:16 ehudes Exp $
...
* @version  $Id: nusoap.php,v 1.1 2005/07/13 18:31:16 ehudes Exp $
* @access   public
*/
class nusoap_base {
	/**
	...
	*/
	function soap_parser($xml,$encoding='UTF-8',$method='',$decode_utf8=true){
		...
		if(!empty($xml)){
			...
			if(!xml_parse($this->parser,$xml,true)){
			    // Display an error message.
			    $err = sprintf('2XML error parsing SOAP payload on line %d: %s',
			    xml_get_current_line_number($this->parser),
			    xml_error_string(xml_get_error_code($this->parser)));
				$this->debug($err);
				$this->debug("XML payload:\n" . $xml);
